...
### 6.5.1 Excel 专业分析与失败治理

用户上传的 Excel 先按普通来源解析，再异步建立类型化专业分析快照；PDF、PPTX、DOCX 等其它
文档继续走原有 parser，不为多格式 notebook 引入一个全能 Agent。逐步推理只有在用户勾选范围
包含已编译 Excel 且问题确实要求表格分析时，才复用既有 `reasoning_agent` 做一次短规划；规划
结果必须落进封闭的 profile/聚合/排序/筛选计划，计算由本地确定性执行。该分支失败不得阻塞
原有回答，也不得扩大来源范围；成功结果以可引用的 `spreadsheet` 结果卡与轨迹步进入 Ask。

系统对损坏/加密、超规模、超长单元格、无可靠表头和疑似多个分离数据区域等挑战性工作簿
宁可拒绝专业分析，也不能静默截断或猜表。专业编译失败和普通来源解析失败统一自动归档安全
记录与隔离副本；用户之后自行重新解析成功会自动解决记录并删副本，到期、删除来源或删除
notebook 都自动清理/脱敏。管理员用户总览保持用户列表列不变，新增只读「提问分析」「解析问题」
页签；管理员只能看日志与跳转存活来源，没有重新解析、批量重试、关闭、删除隔离文件或彻底
清除案例的权限和接口。
...
- 精确工具集共十一个工具（七个 Memory/context 工具与四个 knowhow 工具）：
  ...
  （以上是本方案定稿时的工具面；后续已扩展至二十八个工具——引用点查、来源管理、构建、库理解与全局问答工具组，
