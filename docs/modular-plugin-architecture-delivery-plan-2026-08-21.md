...
- **PR-F4**：KG workspace hook；
  - 状态：实现完成，待三路独立 review 与 CI；Knowledge/Schema/统一图/合并审阅与 KG build/relink/rebuild 状态已迁入 exact actor+notebook+generation owner，惰性读取、请求数、轮询节奏、权限复核与 A→B→A 收敛均已由 G1 覆盖；
