...
**Public conversation parity.** `PublicImage.reference_keys` repeats only the already-public `k1`/positional keys that bind a token-aliased image; it exposes no internal id. The anonymous `/c/{token}` page uses those keys to apply the same body-block placement, image-only presentation, trust label, caption-as-`alt`, first-occurrence deduplication, and in-page preview as authenticated Ask. The report-share allowlist remains image-free.
