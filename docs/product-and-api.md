...
Two effort-independent v2 **protocol constants** — deliberately not deployment settings — bound the aspect self-assessment the model returns inside the same reflect turn: an aspect may carry at most **8** evidence keys (`REFLECT_ASPECT_MAX_EVIDENCE_KEYS`), and an aspect's `gap` (the model's own "what is still missing") at most **240** characters (`REFLECT_ASPECT_GAP_MAX_CHARS`). They have no environment variables because changing them changes the protocol, not a budget. Both bound **model-generated internal payload only**: an over-limit payload folds that turn's whole decision into an invalid one (accounted like any other zero-I/O skip), and neither ever trims the user's topics, questions or constraints — user content in the aspect record is never truncated.
