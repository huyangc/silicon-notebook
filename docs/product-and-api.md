...
The row-detail drawer, and each physical branch in a row-title-group matrix, provide an explicit **智能补全空列** action. It produces suggestions only for stored-empty cells in that row (a missing value or exact empty string; whitespace-only stored content remains existing content): the service selects at most eight rows from the same table that fill at least one requested target column, preferring the same row-title group and then known-column similarity and coverage. Its structured response contains a suggestion or an abstention for each requested column, a confidence level, a short basis, and the reference rows used. The review dialog is draggable and shows each suggestion beside its references; users accept entries individually, and nothing is written automatically. Accepting a suggestion uses the normal cell update with `expected_before=""` and `origin="llm_complete"`, so a cell filled while the suggestion was being prepared is never overwritten and normal history and synchronization continue to apply. If the completion service is unavailable or fails, it returns no offline substitute. It does not use the knowledge graph because the inference must preserve the conditions carried by other columns in the same row.
