...
it does not claim to read every document's full text. The same classifier
(`overview_intent`) also drives reasoning Ask's first-round roster seeding (see
[Collection enumeration tools](#collection-enumeration-tools)), so one sentence
gets one verdict on both sides rather than taking the catalog lane here and
falling back to relevance ranking there.
...
**Who can trigger enumeration: three entry points.** Besides the two model actions in the reflect loop there is a **third trigger**: the catalog-question classifier (`overview_intent`, the same predicate the [document introductions in general Q&A](#retrieval-modes-ask) lane uses, so one sentence never gets two different verdicts). When the asker's own question classifies as `catalog` ("what do the papers in this notebook say"), reasoning Ask **deterministically lists the source roster once in the first round** instead of betting on the model picking the roster action. It runs the **same** execution path as the model action, so it draws on the **same run-level budget pool** and the same continuation ledger. That has a first-mover consequence worth knowing: **on a large library at a low effort tier the seed alone can exhaust `enum_rows_per_run`**, after which the model's own enumerate actions are skipped on budget (`enumeration_budget` in the trace); a later request for the same roster is skipped as already listed (`already_enumerated`), so the same list is never enumerated — or charged — twice. The seeded `enumerate` trace step carries `detail.phase="seed"` (a model-chosen one carries no phase), and every `skip` inside `_run_enumeration` carries it too — tagging only the successful step would leave "the seed was blocked" and "the model's action was blocked" identical in the trace. **The seeded roster inherits the reference-library scope the classifier resolved**: by default it lists only the current notebook's documents — a question that never mentions reference libraries ("summarize this notebook's papers") and one that excludes them explicitly ("... excluding the reference libraries") both stay local — and mounted reference libraries are listed only when the question says so ("including the reference libraries", "this notebook and its reference libraries"), the same predicate as the general-Q&A document roster lane above. That scope is recorded in the continuation ledger, so **the model's later roster continuations reuse it**: a model-chosen roster action carries no scope, and reopening a second chain at the default (the whole participant set) would let the excluded reference-library documents into the answer's evidence and charge the shared enumeration budget a second time. A local-only listing's `enumerate` trace step carries `detail.local_only=true` (the key is absent when reference libraries are included). Seeding is independent of the knowledge graph (it happens with or without one); a restricted source scope and callers with `allow_enumeration=False` (Knowhow completion) do not seed — the same single gate as the two model actions. A seeded roster **counts as first-round progress**: `no_progress=false`, `stale` starts at 0, and the first-round empty-handed note is not injected.
