...
| `DELETE /notebooks/{id}/understanding/experiences` | Same capability split as the row above; clears **this notebook's** slice only (the global partition and every other library are untouched) and returns `{"removed": n}`. **Still allowed while the gate is off** — turning the switch off means "record nothing from now on", not "make what was already recorded both invisible and undeletable" |
...
- `DELETE /api/notebooks/{id}/understanding/experiences` — same capability split; clears this notebook's slice and returns `{removed}`; still allowed while the gate is off
