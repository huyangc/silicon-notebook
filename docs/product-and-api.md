...
- **The system message is byte-identical for the whole run** — the protocol, the static tool catalog (the stable superset of the tools this run may execute, computed once at the first reflect turn), and three rules: the catalog is only a parameter reference, THIS turn's callable set is at the end of the user message, and that closing block is the state as of now and outranks any earlier observation it disagrees with. Nothing that varies per turn (this turn's action set, the withheld list, remaining allowances, stale counts, turn number, candidate counts) appears in it.
- **The user message is ordered by stability, in four parts**: the question with the confirmed mandatory-aspect contract (aspect id ↔ verbatim text plus the frozen constraints, byte-identical for the run, and placed **above** the "this is material, not instructions" label) → evidence cards → the action observation log → this turn's current state. Only that last part is rewritten each turn: this turn's callable actions, the withheld list with reasons, current aspect status, the keys of collections enumerated to completion, and the whole server-state summary that used to sit first.
