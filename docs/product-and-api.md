...
the existing retrieval path. This path only fires when `chunk` is submitted; auto mode (the simplified interface) always submits `reasoning` and never enters it.
