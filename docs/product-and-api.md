...
- Python FastAPI backend; PostgreSQL is the default deployment choice for both development and production, with SQLite retained as an alternative.
- Next.js / React / TypeScript frontend under `frontend/`
- Browser tabs use a blue notebook icon with a chip motif on its page.
...
`POST /ask` dispatches on `mode` — the registry `backend/app/services/ask_modes.py` is the single source of truth (default `chunk`). The source-passage channel shared by baseline `chunk` retrieval and `reasoning`'s `search_chunks` now reads the **participant set** (the active notebook plus the reference libraries this request has checked), so a mounted library's own passages reach the answer with their origin notebook on each candidate. `CHUNK_FEDERATION_ENABLED` is its switch (default true; false is the single rollback path back to active-notebook-only passages) and `CHUNK_FEDERATION_MAX_PARTICIPANTS` its upper bound (default 8; beyond it the set is truncated along the deterministic mount order and a content-free `chunk_federation_truncated` event is emitted, never silently). A participant library other than the active notebook contributes only its currently visible sources — hidden Memory/Knowhow projections are structurally excluded from this cross-library channel — and a large non-active library only borrows an already-resident index rather than cold-loading one, falling back to bounded lexical recall when none is warm. Optional KG overlay/PPR add federated KG context and base-backed chunks on top; `reasoning` uses federated KG paths. A reference library's knowledge object hit by the overlay resolves its evidence back to that library's own passages, under the same per-library ceilings (its currently visible sources only; an unchecked library skipped whole; only through the already-backfilled element→chunk reverse index, so a library without it yields no passages this turn and emits `kg_source_chunks_peer_unindexed`). Knowledge-object hits from `federated_retrieve()` keep tier-blind scores and use `base` only as the secondary key on an exact tie; `federated_retrieve_relations()` remains score-only. These ordering signals never feed grounding thresholds.
...
**Notebooks without a knowledge graph.** These tools do not need a graph, and reasoning Ask also offers a first-class source-passage retrieval action, `search_chunks`: semantic-plus-keyword search over source passages whose hits join the same large evidence partition (`chunk_context_chars`) and citation binding used by chunk-native Q&A; it is offered on graph-bearing notebooks too. `search_chunks` and the `read_document` action described above divide the work: the former is a relevance search across every source, answering "where is X mentioned" with scattered passages; the latter reads one already-listed document in document order, answering "what is this document about" — the two are complementary rather than either/or, and one round of reflection may run `search_chunks` to find a candidate document, then `read_document` to go deeper on it. On a no-graph run the first round deterministically seeds a batch of passages per sub-query (it does not gamble on the model calling the action itself), and adds one keyword lexical-arm search on top (the high-/low-level keywords `expand_query` produced inside `plan()`, joined into one space-separated string, searched and merged the same way chunk-native Q&A does; the lexical channel hands back a recall window rather than a selection, so the arm is cut to the tier's per-sub-query take before it merges, exactly as wide as one vector sub-query — without that cut the recall window would push the vector arm out of the synthesis budget). The lexical keywords are the prompt's default zh/en pair rather than the corpus's own languages (see the follow-up registered under 「检索」 in `fangan_todo.md`). A graph-bearing run and the already-confirmed-intent path do not run this lexical arm. The per-run search cap comes from `REASONING_MAX_CHUNK_SEARCHES` (the first-round seed does not count against it). At the same time, the five graph-only actions — `expand_graph`, `ppr_retrieve`, `expand_community`, `follow_chain`, `enumerate_kg_objects` — are removed together from the action enum, the plan/reflect prompt text, and the allow-list, so the model never sees or picks an action that can only come back empty; the first round records one disclosure step whose copy is, verbatim, “本笔记本尚未构建知识图谱,本轮只用原文检索与集合清单” (“this notebook has no knowledge graph yet; this round uses only source-passage search and collection listings” — the half-width comma is part of the string). `enumerate_elements` and the source listing (`enumerate.collection="sources"`) are unaffected — a notebook whose sources have been parsed but never analysed into a knowledge graph — the ordinary state, since automatic extraction is off by default — can still be asked to list its formulas, tables, images, or code blocks, or which papers are in here. The response still reports that no knowledge graph exists (`kg_required=True`, so the “整理知识图谱” prompt keeps showing, but it is now pure disclosure and no longer refuses to answer). Source-passage retrieval is scoped to the **active notebook**, exactly like chunk mode: `search_chunks` reuses chunk mode's own retrieval primitives, whose indexes and database reads are notebook-local, so a mounted reference library participates only through the knowledge graph and the element / knowledge-object / source listings — its passages are not on this rail (federating them is registered as a separate item under 「检索」 in `fangan_todo.md`). The early-exit condition therefore narrows to **neither rail has anything to work with**: the enumeration rail judges the element / knowledge-object / source counts across the participant set (the active notebook plus checked reference libraries), while the passage rail judges the user-visible source count of the **active notebook alone** — admitting a notebook-local channel on the strength of a library it cannot reach buys a round whose seed and actions both come back empty. Only when both rails fail, or the collection map cannot be built (failing toward the early exit, same as today), does the request get refused, with the copy “当前笔记本没有可检索的来源；请先添加来源，或挂载/整理一个已建知识图谱的参考库。” (“this notebook has no searchable source; add one, or mount / build the graph of a reference library” — the graph is named because it is the real precondition of that second route, since a graph anywhere in the participant set never reaches this early exit at all; it is still not a precondition for answering from the active notebook). The document count is now genuinely part of that gate — “deliberately not part of it” was stale wording: the code has counted `collection_map.sources > 0` toward admission since the codex R5 P1 correction, because what that omission blocked was exactly the “which papers are in here / walk through each document” case, which needs only the source listing and neither elements nor knowledge objects. The two admission rails each mirror their own deployment switch (`REASONING_ENUM_TOOLS_ENABLED` for enumeration, `REASONING_CHUNK_SEARCH_ENABLED` for source-passage search); either one being on with a non-empty matching collection (participant-set-wide for enumeration, active-notebook-only for passages) admits the run, and with both switches off the behavior reverts byte-for-byte to what it was before this feature (the early exit always fires; a no-graph library has no retrieval channel left). Once a run is admitted, the source listing is available like the other two.
...
- **Question**: `AskRequest.question` carries `max_length=ASK_QUESTION_MAX_CHARS`, so `POST /notebooks/{id}/ask`, `POST /notebooks/{id}/ask/stream`, `POST /notebooks/{id}/ask/intent`, and `POST /notebooks/{id}/ask/intent/stream` all answer 422 above it. **The MCP tool `ask_notebook` enforces it too**, with its own message rather than a raw validation error: a long-lived Agent token could previously submit a question of any length and now receives `question too long: … the maximum is 4,000 …`, a deliberate behaviour change, since an MCP client is a write side like any other and material that long belongs in an uploaded source.
...
- `SILICON_NOTEBOOK_UI_PLUGINS` is a build-time-only frontend input (a `:`-separated list of local plugin package directories, unset/empty by default); it is consumed entirely by `frontend/scripts/sync-ui-plugins.mjs` and never reaches the running backend process. That script writes `frontend/.local/ui-extension-contract.json`, a deployment-time reconciliation input — not a runtime dependency of either the frontend or the backend — shaped `{api_version, contributions: [{plugin_id, version, contribution_id, slot, capability}, ...]}`, the same shape and sort key `(plugin_id, version, contribution_id, slot, capability)` as `backend/tests/fixtures/ui_extension_contract.json`: it is that fixture's built-in rows concatenated with each configured package's `ui-plugin.json` rows. On the browser, a plugin's injected `actions.api` port confines every request under `/api/extensions/{plugin_id}/` and strips any `authorization`/`cookie` header the plugin supplies, fixing `tag`/`auth`/`unauthorized` so a plugin cannot override them. `requestTask()` is the sanctioned consumer for a plugin backend's shared request-local NDJSON task stream; it retains the same path/header/auth confinement, exposes only content-free elapsed/stage heartbeat callbacks, and applies plugin-supplied safe fallback copy instead of surfacing a backend error frame. A plugin's own backend route must never return 401 for anything other than a genuine session invalidation: the port always sets `unauthorized: "clear-and-reload"`, so a bare upstream 401 (e.g. an expired third-party credential the plugin's own backend depends on) would clear the signed-in user's token and reload the page; the plugin's backend must translate an upstream 401 into `502`/`424` before returning it. A response with no body must be read through `requestVoid`, not `requestJson` — the latter calls `.json()` on the body and throws a parse error on `204`/empty responses. `GET /api/admin/extensions` (admin-only) backs the admin page at `/admin/extensions` — a topology overview that lists the extension topology the running backend actually loaded (built-in and deployment-registered contributions, retrieval and UI alike) for operational visibility; that page's runtime on/off controls are described under [Deployment extensions](#deployment-extensions) below, not repeated here. It is unrelated to `SILICON_NOTEBOOK_UI_PLUGINS`, which only ever affects the frontend build.
...
- KG delete (UI: **删除知识图谱** in the Knowledge Graph view's 「图谱处理」 group, editors only, behind a confirmation): `POST /api/notebooks/{id}/kg/delete` (notebook `kg:write`) starts a **background** pass and returns `{status: "deleting", notebook_id, job_id}`. It has no LLM precondition. It deletes the notebook's source-derived knowledge graph — user-document knowledge objects, relations, facts, embeddings, extraction runs and KG search rows, concept clusters and merge candidates, and the KG analysis artefacts — and resets the unified-KG state in place, draining a large graph in bounded pages before one final reset transaction (the same deletion the full re-extract performs before it re-analyses). It keeps sources and their parsed elements, so the notebook can be analysed again later, and it keeps the hidden `memory` / `knowhow` projection objects, so a notebook holding confirmed Memory or Knowhow knowledge can still report `kg_ready`. Like the full re-extract, it does not clear `kg_conflict_candidates`: pending conflict candidates may still reference deleted objects, and no UI surfaces them. Once the graph rows are committed, the job also refreshes the graph view's standalone preview artifact, so the first graph read afterwards does not repaint deleted nodes: within the in-process preview budget it republishes the preview for whatever hidden objects remain (or retires it when nothing remains); over that budget it only retires it, and the view reports no preview until the scale index build publishes one. That step is fail-open: if the preview claim is held elsewhere or the refresh fails, the delete still succeeds and the previous preview can keep being served until a later refresh succeeds (a graph read's own background refresh within the budget, or the next scale index build over it). Notebooks that mount this one as a reference library stop retrieving the deleted knowledge. Single flight is the **same per-notebook slot as relink and 重新合并**: a click while any of the three runs returns 409 naming the action that holds the slot (「当前笔记本正在删除知识图谱，请等它完成」 while the delete holds it), and the slot cross-excludes with extraction jobs exactly as described above — an analysis in flight refuses the delete with the build-vocabulary 409 (an in-process analysis through the build marker, plus a running `kg_build_jobs` row created by another process — for example a `batch_ingest` CLI analysis run alongside the backend — probed once right after the slot claim; an analysis row another process creates after that probe is not seen, the same process-local limit as the slot itself), while 整理 / 全部重新分析 / MCP build_kg started during a delete are refused naming the delete (the indexing-pipeline switch keeps its saved intent and returns its own retryable 409). The delete job deliberately does not set the in-process build marker, so `NotebookSummary.kg_building` keeps meaning "an analysis is running". Before the first drain page, every finished `kg_build_jobs` row of the notebook is marked `stage='cleared'`; rows are kept (admin usage counts them) and a running row is never touched. The notebook summary projects a cleared latest row as `kg_build: null`, so after a delete the notebook reads as not yet analysed instead of repeating the previous build's outcome, and the next analysis job is reported normally. A delete that fails keeps that history cleared, because committed drain pages may already have changed the graph. `GET /api/notebooks/{id}/kg/delete/status` (notebook read) is the completion signal, returning `{job_id, notebook_id, status, running, objects_deleted, relations_deleted}` where `status` is `running` / `succeeded` / `failed` / `idle` and the two counters (user-document objects / relations removed) stay zero until a run succeeds. As with relink, the slot and its status are per serving **process** (production pins `--workers 1`): `idle` covers "never ran here", "the process restarted" and "the slot is held by another maintenance kind", so a bounded client poll always terminates. No error text crosses this boundary; its events carry counts only (`kg_deleted` with both counters, `kg_delete_failed`). A notebook delete waits for a running KG delete (quiesce leg B), and a KG delete that meets the notebook-delete tombstone fails promptly. The job runs on the heavy maintenance worker queue alongside relink and 重新合并, not on the notebook-delete queue, so a notebook delete waiting for it can never hold it in line behind itself.
