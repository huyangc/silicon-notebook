...
- Two-tier knowledge base: each notebook has a `tier` (`base` | `personal`, default `personal`). Baseline `chunk` retrieval reads chunks from the active notebook only; optional KG overlay/PPR can add federated KG context and base-backed chunks, while `reasoning` uses federated KG paths. The exact-score `base` tie-break applies only to knowledge-object hits returned by `federated_retrieve()`: scores stay unchanged and a higher-scoring personal hit still wins. `federated_retrieve_relations()` remains score-only. Separately, when base and personal evidence contradict during answer synthesis, the answer defers to the base position and surfaces the discrepancy. Citations carry their tier (`AnswerAnchor.tier`) and Ask renders a `base`/`personal` badge per cited anchor. `Citation.notebook_id` / `AnswerAnchor.notebook_id` are non-empty only for evidence from another participant notebook (the active notebook normalises to `""`); every Ask mode, including graph-mode subgraph anchors, mounted private libraries and spreadsheet workbooks, follows that rule, so the UI can name the source library and the "locate in graph" action scopes to it.
...
**Reflect v2 model protocol (design 2026-09-07 §5; off by default — `REASONING_REFLECT_V2_ENABLED`).** The action space keeps the same thirteen ids, and the transport stays a JSON chat completion; what changes is that every turn is generated from one in-memory capability projection. An action that is out of per-run budget, turned off by the caller's policy, unsafe under a narrowed source scope, meaningless without a knowledge graph, or (for `follow_chain`) without any legal start point among the retrieved candidates is removed from the model-facing action list and from the parser's whitelist together, with a bounded reason so the model switches channel instead of re-requesting a dead one; its parameter branch goes with it. The response schema's `next_action` enum deliberately keeps listing all thirteen recognizable ids: the shared shape gate reads an enum hint as a closed set, so narrowing it by availability would make a recognizable-but-unavailable choice fail in the transport instead of becoming the observation described next — which, after retries, ends the whole loop rather than costing one turn. The response carries one `arguments` object instead of one branch field per action, and the chosen action's fields are validated by type: `sufficient` must be a real boolean, `arguments` must be an object, and `next_action` must be one of the thirteen ids the schema enumerates. These three malformed shapes never reach this parser in production: the shared transport-layer shape gate (`validate_model_json_shape` / `_validate_repaired_shape`) rejects them first with `invalid_boolean` / `invalid_type` / `invalid_enum`, and after retries the whole loop ends under the existing fail-open / fail-closed contract — not as an observation in the accounting below. The parser still carries a branch for each (`invalid_sufficient`, `invalid_arguments_object`, `unknown_action`) purely as defense in depth, reachable only through test doubles or fail-closed callers that construct the payload directly. A retrieval action carrying `sufficient=true` is a contradiction that is rejected as a whole (`update_outline` and `consult_memory` retrieve nothing, so they may legitimately accompany it); and `answer` and `consult_memory` must send an empty `arguments` object — attaching retrieval arguments to them is recorded as `unexpected_arguments` rather than silently ignored. A recognizable-but-unavailable action, a missing argument or an argument whose value fails validation (with one deliberate exception, `enumerate.scope`, documented with the enumeration tools below), that contradiction, or unexpected arguments is recorded as one observation with **zero tool I/O** and goes through the same step / no-progress / stale accounting as any other skip, so repeatedly submitting an illegal action cannot evade the stale circuit breaker; the run continues while budget remains. When the projection leaves `answer` as the only executable action, the server ends retrieval itself under the stable reason `no_executable_action` rather than spending a model call on a decision that could only be "answer" — and it does not fabricate a "model judged the evidence sufficient" step. Provider or JSON failures keep the existing fail-open / fail-closed contract and are never reported as the model finding the evidence sufficient; cancellation and stage-boundary errors still propagate. This is an internal model-decision protocol: it adds no field to any Ask or MCP request. Knowhow completion stays on the legacy protocol explicitly.

**Reflect v2 budgets (design 2026-09-07 §4).** The evidence budget one reflect turn may spend is per effort and lives in its own deployment mapping rather than in the table above, because reflection and synthesis need different granularity and neither is a ratio of the other:

| effort | reflect evidence chars |
| --- | --- |
| `overview` | 4,000 |
| `standard` | 6,000 |
| `deep` | 8,000 |
...
Two effort-independent v2 **protocol constants** — deliberately not deployment settings — bound the aspect self-assessment the model returns inside the same reflect turn: an aspect may carry at most **8** evidence keys (`REFLECT_ASPECT_MAX_EVIDENCE_KEYS`), and an aspect's `gap` (the model's own "what is still missing") at most **240** characters (`REFLECT_ASPECT_GAP_MAX_CHARS`). They have no environment variables because changing them changes the protocol, not a budget. Both bound **model-generated internal payload only**: an over-limit aspect is **rejected per aspect** — that one aspect keeps its previous state and the reason is disclosed to the model on the next turn, while the rest of the payload and, crucially, **the turn's own retrieval action still run** (all aspects rejected in one turn are recorded **together** as a single zero-I/O `skip` step — its reason code is the first rejection's, and the number of rejected aspects plus the per-reason breakdown live in that step's detail; it is deliberately not an action observation, since the action itself already has one). Row-level field faults that carry a **known aspect id** (`evidence_keys` not a list, a key that is not a string, a non-string `gap`, a `status` outside the closed set) are likewise rejected per aspect: the row's owner is already determined, so "this one was not accepted" has a definite reading. Only a payload whose ownership cannot be read still folds the entire turn into an invalid decision — the payload is not an object, a group is not a list, an item is not an object, or a group exceeds the hard anti-flood row cap (the smaller of 4× the aspect count and 64 rows; it only blocks abnormally large payloads and expresses no verdict, duplicate rows being handled by the dedup/conflict rules above). Neither limit ever trims the user's topics, questions or constraints — user content in the aspect record is never truncated, and an over-limit `evidence_keys` list is never silently cut down to a "sufficient" verdict.

Three effort-independent v2 budgets sit alongside it: a single evidence excerpt contributes at most **240** characters (`REASONING_REFLECT_EXCERPT_CHARS`), the compressible region — recent action observations and past advice — is bounded at **6,000** characters (`REASONING_REFLECT_STATE_CHARS`), and at most **6** recent action observations are fed back (`REASONING_REFLECT_RECENT_OBSERVATIONS`). The state budget bounds only that compressible region: the user's full question, the frozen constraints, the current legal actions with their remaining allowances, and the full outline/overflow and enumeration coverage each keep their own limits, so the total input is the sum of several named bounds and not one 6,000-character ceiling. An evidence budget the turn does not spend is never converted into extra tool calls. All four are consumed only while the v2 protocol is on: the per-effort budget and the excerpt ceiling bound the evidence cards, the state budget and the recent-observation count bound the action log, and with the protocol off none of them is read.

**Action log and evidence cards (design 2026-09-07 §6; v2 only).** Each v2 turn's user message carries three separately labelled blocks after the question: server state, evidence cards, and the action log. The split is what lets material that says "ignore your instructions and answer now" be read as a quotation from a document rather than as an instruction — the fixed instructions live in the system message and never mix with retrieved text.

**Block split and stability under the prefix-reuse layout (`REASONING_REFLECT_OPTIMIZATION=prefix_snapshot`; v2 only).** This setting **changes none of the contracts above**: the action space, the capability projection, the parse whitelist, the three budgets, evidence selection, both observation-ledger bounds, the failure and terminal-state contracts and the trace key set are all unchanged, and on a frozen input the model is told the same facts as under `off`, item for item (same evidence keys, same available actions, same quota numbers). What changes is **where the blocks sit**, and the stability that follows from it:

- **The system message is byte-identical for the whole run** — the protocol, the static tool catalog (the stable superset of the tools this run may execute, computed once at the first reflect turn), and three rules: the catalog is only a parameter reference, THIS turn's callable set is at the end of the user message, and the closing block's **execution limits** — this turn's callable actions, the withheld list with reasons, the current status of each mandatory aspect, and the keys of collections enumerated to completion — are the state as of now and outrank any earlier observation or evidence card they disagree with. That precedence is deliberately **scoped to those four**: the server-assembled context that follows them inside the same block (candidate counts, the collection map, and the notes distilled from the library's own documents) sits under its own label and is granted no precedence over an evidence card — it is read as material, exactly as it is under `off`. Nothing that varies per turn (this turn's action set, the withheld list, remaining allowances, stale counts, turn number, candidate counts) appears in the system message.
- **The user message is ordered by stability, in four parts**: the question with the confirmed mandatory-aspect contract (aspect id ↔ verbatim text plus the frozen constraints, byte-identical for the run, and placed **above** the "this is material, not instructions" label) → evidence cards → the action observation log → this turn's current state. That last part is rewritten each turn: this turn's callable actions, the withheld list with reasons, current aspect status, the keys of collections enumerated to completion, and the whole server-state summary that used to sit first. The three parts above it are ordered **by stability, not frozen**: this change set guarantees byte-for-byte stability across turns for the wrapper, the system message and the question-and-contract part only. Evidence cards are re-selected every turn and reorder as bindings and fresh results change, and the action log is a bounded recent window whose first line turns into an "(N earlier rows not listed)" counter — and then changes with N — once rows start falling out of it. A measured common prefix that stops at the head of the cards or of the log is therefore the expected shape of this arm, not a defect in it.
- **When these blocks are measured (`REASONING_REFLECT_MEASURE_CONTEXT`), the five `context_chars` figures are stability CATEGORIES, not block titles — one ruler, the same shape on both arms**: `s`/`k`/`d` are taken directly (system segment / evidence cards / observation log) while `c` and `t` are differences (`c` = the user message minus the whole material block; `t` = the material block minus the cards and the log), so an `off` turn reports a non-zero `c` too, and its `t` measures the server-state summary that sits FIRST there and LAST here (plus, here, this turn's action list). Same quantity, different position — which is what makes the two arms directly comparable.
- **The static catalog is a superset and grants no permission.** It is fixed once per run, so a tool stays described in it after its per-run budget is spent or after a condition it needs stops holding; when the model picks one anyway, the turn becomes the existing zero-I/O `unavailable_action:<reason>` observation and the loop continues, using the same reason vocabulary as `off`. Both narrowing and widening are disclosed by each turn's current state instead.

The disabled state (`off`, plus the v2 master switch off and Knowhow completion) keeps the original layout in the original order, byte for byte.

The action log records what the server actually executed, one row per action: sequence number, whether it came from the deterministic first-round seeding or from a model-chosen action, the action, the normalized request identity (the parameters that make two requests the same one — types/prefer, edge type and direction, enumeration collection and subtype), the model's own one-line purpose from that turn, the execution status, how many items were returned versus newly added, any known truncation, a stable reason code, and the remaining step allowance. Statuses are `success`, `empty`, `duplicate`, `unavailable`, `invalid`, `failed` and `partial`, and they are kept apart on purpose: an empty result is not a failure, returning items that were all already held is not a new find, and an action that never ran is never recorded as having retrieved. Rows are derived from the executed step's structured result, never from the model's stated reason. The log is a projection, not a second source of truth: duplicate detection, visited nodes, looked-up names, enumeration cursors and outline overflow each stay owned where they already were, and the log changes none of them. Past purposes are labelled as historical model judgement and prove nothing about coverage, scope or channel availability; the stale circuit breaker is unchanged and no model self-report can reset it.

Evidence cards are built only from material already in the candidate pool — no extra model call, no extra database read, no on-demand hydration. A card carries the evidence type, the stable reference key, the source and location when the pool holds them, whether the text is verbatim source or an extracted summary, a bounded question-relevant excerpt, and applicable conditions when present. Knowledge-graph candidates have no verbatim passage, so their cards use the extracted fields (name, steps, validity scope) and are labelled as extracted rather than presented as literal source text; query-time inferences keep their existing "query-time only" marking and are never promoted to citable graph facts. Selection is deterministic: evidence currently bound in the outline first, then what this turn newly added, then historically relevant representatives with source diversity; the same evidence is rendered once, ordering and tie-breaks are repeatable, and cards that do not fit the budget are disclosed as a count of what was left out. The excerpt window prefers the passage covering the effective search terms of the question and the current action's query — reusing the existing tokenization and the `"..."` exact-phrase rule — and falls back to a prefix, always with an ellipsis marker showing it is partial. Only keys whose card was actually rendered count as shown to the model; a candidate that the budget pushed out of the window does not become bindable in the outline just because it sits in the pool.

**Mandatory aspects and termination reason (design 2026-09-07 §7; v2 only).** The mandatory list is **never decided by the model**: it comes from the frozen contract the user already reviewed — the intent contract's `mandatory_topics` for Ask, the section's `intent_questions` for a deep report, and, on the compatibility path with no formal contract, the whole input question as the single aspect. Aspect ids are generated deterministically from contract order (`a1..aN`, carrying no user content), at most 16 of them (reusing the intent contract's existing `mandatory_topics` ceiling). Related **constraints** ("7nm only") are rendered alongside the aspect list in every turn's server-state block but are **not aspects of their own**: a constraint is a predicate on the answer, not a retrievable object, and giving it an id that can be marked "supported" would only manufacture aspects that stay unresolved forever.

Each aspect is `unknown`, `partial`, `supported` or `conflicting`, reported by the model in an additional `assessment` on the **same** reflect turn — no extra model call. Aspects the model omits keep their current state (omission means "I did not speak to it this turn", not "delete it"; only the user's contract can change the mandatory list), while a reported aspect has its support judgement **replaced wholesale** rather than unioned like outline evidence keys — otherwise one wrong "supported" could never be withdrawn. The payload's top-level shape accepts either the `{supported:[...], unresolved:[...]}` list form or a by-aspect-id mapping (`{aid: {status?, supported?, evidence_keys?, gap?}}`); the server normalizes the latter into the former before validation, so the checks below make no distinction between the two. The shape test is "any of `supported`/`unresolved` present at the top level means list form" (unknown top-level keys are ignored), and in the mapping form a conflict between `supported` and `status` **resolves to the conservative side**: only `supported: true` with `status` absent or `supported` counts as supported; an explicit non-supported `status` wins and the aspect is recorded as unresolved; `status: "supported"` together with `supported: false` is recorded as unresolved/`unknown`. The server verifies exactly three things: the aspect id is legal, each cited evidence key is an identity the server issued **and actually showed** the model (a fine-grained evidence key from the candidate pool, or a collection-completeness key as described below), and the identity of an **incompletely** enumerated collection cannot impersonate fine-grained evidence. An item left without support after illegal keys are removed may not stay "supported". Two rows for the same aspect are resolved deterministically: **identical** rows (same status, same surviving keys, same gap after normalisation) are deduplicated, while **conflicting** ones reject that aspect and keep its previous state — the server never picks the first or the last one on the model's behalf. Semantic support is always labelled as the model's judgement: no new grounded score, no change to the evidence_level thresholds.

**A closing turn must self-assess.** The test is the **closing payload** itself: `next_action=answer`, or any action carrying `sufficient=true` (an action that produces no evidence, such as `update_outline`, may legally carry it — and at the exhaustive effort the most common close is precisely "bind the last batch and declare sufficiency in the same turn"). Either payload ends the whole retrieval on the spot, and at that moment the aspect ledger is the only record of what this retrieval actually got. "Assesses no aspect" means **no update actually landed in the ledger this turn**, not "the payload carried no rows": a closing payload full of rows whose every row was rejected per aspect (an aspect id outside the list, a `status` outside the closed set, too many evidence keys, …) leaves exactly the same reading in the ledger as total silence, so it is treated the same way — pushed back and prompted, which is also what gives those "server did not accept" disclosures a next turn to be rendered on. If **any** aspect really landed, the turn counts as self-assessed and the close stands. When such a payload lands no aspect at all, the server **rejects the whole turn** (one zero-I/O observation, reason `missing_assessment`, going through the same step / no-progress accounting as any other skip, but **holding stale steady rather than incrementing it** — the same treatment as a consult turn that delivered content; the prompt ceiling of `REFLECT_ASSESSMENT_MAX_PROMPTS=1` already bounds how many times this can be exploited) and, on the next turn, lists the aspect ids one by one in the server-state block and asks for a verdict on each. If that turn also carried an `update_outline` payload, the outline is **applied before the rejection**, so the bindings the model really did produce are not lost to the push-back. The prompt is attached to the **next** turn only, not to every turn thereafter. It is sent **at most once**: if the second closing payload is silent too, it is accepted as-is and the still-unjudged aspects are recorded as "we asked and the model would not say" (snapshot field `assessment_omitted`, counted as `aspects_assessment_omitted` on the closing skip step) — so an evaluation run can tell that apart from "the model never reached the closing turn". **A turn with no next turn after it is never rejected** (the step budget is exhausted; or the closing action is `update_outline` and this very apply eats the last outline-update slot, so the next turn would end on `only_answer` before the model is ever called): the whole value of asking is that the next turn fills it in, and with no next turn a rejection would only rewrite the model's own verdict into "out of steps" / "no executable action" — so that case is treated as the second silence, accepted and recorded as `assessment_omitted`. If the prompt was issued but never actually reached the model (the next turn ended before its prompt was rendered), the end of the run records `assessment_omitted` through the same path.

**Support for roster questions: collection-completeness keys.** Enumeration results deliberately never join the relevance candidate pool, and item ids are not visible to the model, so an aspect like "which documents does this library hold" cannot be supported by any single piece of fine-grained evidence — what supports it is the fact that the collection was listed in full. For every enumeration chain whose coverage reports **complete** within this run, the server issues a deterministic collection identity key (an `enum:` prefix plus collection / kind / pinned source / scope, matching the resume identity field for field and carrying no user content) and lists those keys for the model in the **server-state block**, directly after the aspect list (the model reads "a2 is unsupported" first, then "which keys could support it"); only a key that was actually shown may appear in an `assessment`'s `evidence_keys`. A chain that is not finished (still resumable, or invalidated by a concurrent change) issues **no** key, and a key the model invents is dropped just the same — the legal set is the one the server computed, not "anything with that prefix". Collection keys take no part in the delivery review: they do not travel the evidence budget and produce no `[k]` anchor.

When retrieval ends, the server produces a typed **termination fact** whose `reason` is one of a closed set of seven, each with one fixed user-facing sentence:

| reason | trigger |
| --- | --- |
| `model_sufficient` | the model ended explicitly and reported sufficiency, with no known unresolved aspect |
| `model_partial` | the model chose to stop while aspects were unknown/partial/conflicting, or its self-reported sufficiency contradicts the aspect record |
| `step_budget` | the normal step budget ran out |
| `stale` | the existing no-progress circuit breaker fired |
| `no_executable_action` | nothing but answering was executable |
| `model_degraded` | the reflect call kept failing after the existing retries on **two consecutive turns** and the run fell back (v2; legacy still falls back after one) |
| `retrieval_degraded` | a retrieval channel failed into the existing fail-open ending and evidence collection did not complete |

`retrieval_degraded` has **two preconditions and needs both**: this run carries no model-ended marker, **and** the last real I/O execution on the timeline was a failure. A single recoverable tool failure that the run later recovered from — even through a different channel — stays an observation and does not mark the whole run degraded; conversely, a run whose last retrieval attempt failed and then ended on the breaker or the step budget reports this reason over `stale`/`step_budget`. Channels that never recovered are recorded separately in a **disclosure-only** `unrecovered_channels` (meaning "this channel's last execution was still a failure") and take no part in `reason`: "which path did not work" and "did this retrieval end normally" are two registers and neither may impersonate the other. Cancellation and unrecoverable stage errors remain exceptions and are never wrapped into a successfully generated termination. The pre-exit outline-overflow repair keeps its existing rule and cannot rewrite an already-triggered stale or budget reason into "sufficient".

**One failed reflect turn no longer ends the whole retrieval (v2 only).** When the provider or its JSON still fails after the existing retries, v2 folds that turn into one zero-I/O degraded observation (reason `model_degraded:<stable code>`, recorded with the "failed" status) and carries on as a turn that made no progress — the evidence already gathered and the directions just seeded are no longer thrown away over a single blip, while repeated failures are still caught by the existing no-progress breaker. Only **two consecutive** failed turns end the run under the existing fail-open contract, with the termination reason `model_degraded`; the first turn is no exception, and any successful turn resets the count. The failure code is fed back verbatim on the observation line so the model can tell whether to shorten its output or change its approach. Cancellation and stage-boundary errors still propagate, and fail-closed callers (Knowhow completion) are unchanged. One new code joins the set: `output_budget_exhausted`, given when the body is empty **and** the provider reports the completion was cut off at the output ceiling (`finish_reason=length`); every other empty body is still `empty` — the two call for opposite remedies (raise the budget vs. retry or rephrase). On the first of those the server retries once within the **same** turn with that call's output ceiling doubled (`REASONING_MAX_TOKENS × 2`), and a success is treated as if nothing had happened. `finish_reason` is also written unconditionally to the LLM call log. The same terminal record carries two numeric keys alongside it: `attempts` is how many provider requests this ONE logical call really **issued** — a transient retry, the plain-mode retry after a rejected `response_format`, and the stream rebuild after a rejected `stream_options` each count one, so it may exceed 1; the latter two write no log row of their own, and this number is the only place they are visible. Intermediate `status="retry"` rows do **not** carry the key: a call's request count is reported once, on its terminal row, and summing it across rows would count one call several times. `response_chars` is the length of the body **handed back to the caller**, taken before the log body is clipped (`LLM_LOG_MAX_CHARS`), so reading a long reply's size out of the log does not understate it. Both are numbers only; not one extra character of request or reply text enters the log.

This fact travels with the evidence into Ask's single-pass and per-section synthesis and into a report's section writing, so the synthesis prompt gains a short **server fact**: why retrieval stopped, which mandatory questions it left open (at most 6 listed, the rest reported as a bare "and N more" count), which channels never recovered. It states on its own face that it is **not a knowledge item, carries no `[k]` id and must never be cited**, and that it only says where *this* retrieval stopped rather than proving the notebook lacks the material — the model's job is to state the gap plainly in the answer, not to refuse. The model's own `gap` text does not enter this block.

The block's closing **directive** ("say plainly which of those points the evidence does not cover") is handed out separately from the facts: single-pass synthesis gets one; per-section synthesis gives **every section the full facts** but the directive only with the **last** section — asking each section to write its own gap note leaves the reader a disclaimer at the end of every section, when the gap only needs saying once for the whole answer. When the reason is `model_sufficient` with no unresolved aspect and no unrecovered channel, the directive is dropped in every mode: it points at an empty set, and keeping it only invites the model to invent a gap to satisfy it. The block's input cost therefore has a stated bound: 1 copy for single-pass synthesis, K copies for per-section synthesis (K = sections actually synthesised), each copy ≈ fixed prefix/suffix + at most 6 aspect questions (whose length is bounded by the user contract's own per-item ceiling) + the channel-name list. It takes no `[k]` id, never enters `id_map`, and takes no part in the synthesis counts.

**After final assembly**, the server re-checks each aspect against the evidence identities that actually entered the prompt, keeping three registers apart: `model_supported` (the model says this aspect has support), `synthesis_admitted` (at least one of its bound evidence items really entered the prompt), and `answer_cited` (the finished answer really carries an anchor bound to that evidence). They are **not nested**: entering the prompt does not prove the answer cited correctly, and real citation is still answered by the existing binding and grounding paths. An aspect that bound evidence (in state supported, partial or conflicting) whose every item was removed by budget or filtering is demoted to **undelivered** and no longer displayed as supported; partial removal is not undelivered, because the model did see one of those items. `conflicting` counts here exactly as `partial` does: all three states mean the model saw those items and judged on them, and excluding conflicts would let "the two mutually contradicting items were both squeezed out of the synthesis" pass silently — the one case that most needs reporting.

"Evidence identities that actually entered the prompt" is read off the final `id_map`'s `object_id`, plus every **member folded away into the same canonical cluster**: knowledge-object evidence is deduplicated per cluster, the representative hit's id is what enters the prompt, and a folded member's content travelled in with it under the representative's identity. The fold table is handed out by the evidence assembly itself (no additional query); when the representative itself never entered the prompt it is absent from `id_map`, so its members do not count as delivered either — the register is conservative at the boundary by construction. **A deep report does not currently carry that fold table**, so its side is the conservative reading: a cross-library cluster member may be over-reported as undelivered, never the other way round (nothing genuinely undelivered is ever reported as delivered). The re-check requires no early final re-ranking and reads nothing from the database. Later gap-filling retrieval may be used in the final synthesis but never rewrites the historical fact that reflection had already stopped.

These facts are stored along the existing **private** trace and report-section paths: Ask records them on the closing skip step and the terminal synthesis step's diagnostic fields, a deep report on the section row. The two sites **spell the aspect counts differently**, and both spellings are stable: the closing skip step writes `aspects` / `unresolved_aspects`, the terminal synthesis step and the report section row write `aspects_total` / `aspects_pending` (prefixed because they sit in one detail next to `aspects_undelivered` and friends, where a bare `aspects` would not say which register it is). The front end reads both, prefixed first, and only when the value really is a number — a missing key renders nothing, because historical traces carry none of these fields and showing "unknown" as "0" would be a lie. The UI reuses the existing live trace and post-run synthesis note to show the termination sentence (**server-supplied**; the front end keeps no second reason-code-to-copy map of its own), "x of N aspects handled", and where relevant "y undelivered" and "z channels never recovered" (counts only — internal action names stay off screen); that closing step reuses the `skip` type but is labelled 「结束」 (ended) rather than 「跳过」 (skipped) — it did not skip anything, it *is* the end of this retrieval. An ordinary answer gains **no** fixed error banner, because a partially completed retrieval is not an error. Historical traces without these fields render exactly as before. The public sharing surface is unchanged: the anonymous conversation and report projections are explicit allowlists that never carried the reasoning surface at all, so no aspect assessment or termination fact crosses that boundary.
...
Retired ids `fast`, `global`, and `graph` are transparently remapped to `chunk` (old sessions/bookmarks never 422); any other unknown mode is rejected with HTTP 422.
...
The same line also reaches the model that writes the final answer, as a short server-computed block at the head of the source evidence partition (a fixed header plus the capped map line, under 800 characters, carrying no `[k]` id because it is deterministic server output rather than a retrieved item). This closes a real gap: the reflect prompt tells the model that a collection far larger than the run's listing allowance should be answered with its count instead of paged through — stated in both protocols, as prompt text in legacy and as the `enumerate.scope` parameter note in the reflect v2 capability projection, which names the same two numbers the model is shown (the map's `sources` count and the line's trailing `listing allowance left: N rows`) — and answer synthesis is a separate call that would otherwise never see that number — in the extreme case (a large collection, nothing else retrieved) synthesis would not have run at all. The block is injected whenever the tools are on and the map built, whether or not anything was enumerated, and it is placed ahead of every other block so a full evidence budget cannot make it the first casualty.

**Coverage contract.** Every enumerated collection carries a `TypedCollectionCoverage`: `returned_total` (rows returned so far across a resumed chain), `total` (the map's known size, or `null` when the size could not be established — rendered as an unknown denominator, never as `/0`), `complete`, `truncated_reason` (`budget` / `payload` / `concurrent_change` / `oversize_sample`), and `overflow_semantics` (the shared `explicit_partial`). `complete=true` requires cursor exhaustion, a stable scope identity taken before the first page and after the last, and — across a resumed chain — that the running total matches the known size; anything else is `complete=false`. `oversize_sample` is the oversize-listing guard's own reason and is deliberately **not** folded into `budget`: the run's row pool was not spent — the server asked for a single page on purpose, because the map's count for that collection dwarfs what this run could ever list (see Budget and resumption below) — so the rest of the pool is still available to later actions, and the honest statement is "this collection is far larger than one run can list; here is a sample and the count", not "the listing ran out of its allowance". "The rest of the pool" has to be true, which is why the guard only engages while the pool holds **more than one page**: below that it cannot change the row count at all, and that case keeps reporting `budget`. It carries a resumable cursor like any other partial. That scope identity includes the set of participating libraries itself, re-resolved when the walk closes: mounting, unmounting or invalidating a reference library part-way through changes what “the whole scope” means, so the result is reported as a concurrent change rather than as “all”. Alongside coverage, the result also carries `synthesis_rows` / `synthesis_complete`: the enumerated list and the bounded preview that actually entered the answer-synthesis prompt are tracked separately, so "enumerated 200/200, previewed 100" is disclosed as "enumeration complete, analysis partial" — mirroring the Knowhow batch's own enumeration/synthesis split. That preview allowance is shared across a run's collections and split rather than handed out first-come-first-served: each list is reserved an equal share first, then whatever a short list cannot fill is passed along in order. A run that enumerated two collections therefore previews both, instead of spending the whole allowance on the first and answering a multi-collection question from one list.
...
**Budget and resumption.** **An oversize listing is sampled, not paged through.** Under reflect v2 only, when the collection map's count *for the collection being asked for* is more than four times the rows the run has left, the server caps that one action at a single page (`min(rows left, enum_page_size)`) instead of handing it the whole remaining pool, and the listing reports `truncated_reason=oversize_sample`. The denominator is always the map figure matching that request — the federated `sources` count for the roster at the default `all` scope, the active-notebook count at `current_notebook`, and the kind's or object type's own count for an element or KG-object listing — so a three-formula listing is never sampled because the library happens to hold 48,839 documents. A request the map cannot price (no map, or a listing narrowed to one source) is left alone. The guard also stands down once the pool holds no more than a single page: there it could not change the row count anyway, and claiming the allowance was not spent would be false, so that case falls back to `truncated_reason=budget`. The multiple is a constant (4) rather than a deployment knob: it is the arithmetic form of "spending the whole allowance would still show less than a quarter of it", which is a prefix rather than a listing. The action is neither refused nor rewritten — the requested scope is untouched, the count is reported as usual, and the chain stays resumable, so a model that genuinely wants more can ask again while the pool lasts. Without the guard a 48,839-document scope spends a run's entire 300-row pool on an arbitrary 0.7% prefix and leaves every later enumeration in the run with nothing but a budget-exhausted skip. Legacy (reflect v2 off) is byte-for-byte unchanged; it states the same thing to the model as prompt text instead. Otherwise, one action pages automatically within its run-wide budget (`enum_rows_per_run` total rows, `enum_pages_per_run` extra page round trips, and the shared `structured_payload_chars` structured-payload ceiling) until the cursor is exhausted or a ceiling fires; only the second and later page of each visited partition (a source, for elements; a participating notebook, for KG objects) counts against the page ceiling, since a partition's first page is the only way to see it at all. All three ceilings are **per request, not per action**: each action receives only what the run has left, so several enumerations in one deep run can never return more structured payload in total than the documented request-wide ceiling. The payload ceiling is enforced twice, on two different measurements, because they answer two questions: the executor charges the compact internal row while it walks (that rail stops the traversal from reading more than the request may produce), and the response mapping then charges the *serialized* row — the shape that is actually streamed and persisted, which is materially wider. A row the second rail has to stop short reports its own `truncated_reason=payload` with `returned_total` equal to what was delivered, and the synthesis preview is rendered from the delivered list, so the prompt's coverage header and the result card always state the same number. That cursor is a run-internal handle — it never appears in the response; the client sees only the coverage badge and, when truncated, that the same reasoning run can keep listing further. `complete=false` always implies a resumable cursor, with exactly one exception: `truncated_reason=concurrent_change`, where the scope moved between calls and the chain must not silently restart. A repeated request for a not-yet-complete collection resumes from that cursor when the run's budget still has rows/pages/payload left; when the run's budget is exhausted instead, the request is skipped as budget-exhausted (still reported as a partial result) rather than as already enumerated. Only a collection whose chain already reached `complete=true` is skipped as already enumerated, since asking again could only repeat rows already reported.

**Scoping to one named source.** Internal source ids are never shown to the model — candidate summaries and citations carry source titles — so an element enumeration is narrowed to a single document by title. The server resolves that title deterministically against the sources the map already plans to visit for the requested kind: an exact match after trimming and case folding, never a fuzzy or ranked match, and bounded so a large mounted library cannot turn one reflection into a full label sweep. A title that matches nothing, or more than one source, skips the action and says so in the trace; it never silently widens into a whole-scope enumeration of a different question, and it never guesses one of several same-titled documents. When the scope holds more sources of that kind than the resolver is allowed to examine, it declines outright rather than answering from the part it did read: uniqueness is a property of the whole scope, and a second source with the same title could sit anywhere beyond the examined range. Under reflect v2 the enumerate parameter table no longer offers a `source_id` field at all — the legacy protocol still does, so its prompt, schema and trace stay byte-identical — because anything the model could put there is a guessed internal id, and a guessed id is out of scope by construction. An id stuffed in anyway still routes (defaulting it away would silently answer a different, whole-collection question) and skips that one action under its own reason code: the trace says the named source is outside the retrieval scope and to ask by name (`source_title`) instead, rather than sharing the "requested scope unavailable" wording with an unknown item kind. When a v2 payload carries **both**, the title wins: dispatch itself is id-over-title (it resolves a name only when no id was given), so a guessed id would otherwise swallow a title the model got right and turn a request that could have succeeded into an out-of-scope skip. That preference lives in the v2 parsing layer alone — a non-string `source_id` is still rejected on type as before, and both the legacy protocol and the dispatcher keep id-over-title byte-for-byte.
