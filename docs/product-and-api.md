...
**A closing turn must self-assess.** Choosing `answer` with `sufficient=true` literally means "every mandatory aspect now has support", and the aspect ledger is the only place that claim can land. When such a payload assesses no aspect at all, the server **rejects the whole turn** (one zero-I/O observation, reason `missing_assessment`, going through the same step / no-progress / stale accounting as any other skip) and, on the next turn, lists the aspect ids one by one in the server-state block and asks for a verdict on each. That prompt is sent **at most once**: if the second closing payload is silent too, it is accepted as-is and the still-unjudged aspects are recorded as "we asked and the model would not say" (snapshot field `assessment_omitted`, counted as `aspects_assessment_omitted` on the closing skip step) — so an evaluation run can tell that apart from "the model never reached the closing turn". A partial close with `sufficient=false` is **not** forced: it already says "I know something is missing", which does not contradict an unresolved ledger.

**Support for roster questions: collection-completeness keys.** Enumeration results deliberately never join the relevance candidate pool, and item ids are not visible to the model, so an aspect like "which documents does this library hold" cannot be supported by any single piece of fine-grained evidence — what supports it is the fact that the collection was listed in full. For every enumeration chain whose coverage reports **complete** within this run, the server issues a deterministic collection identity key (an `enum:` prefix plus collection / kind / pinned source / scope, matching the resume identity field for field and carrying no user content) and lists those keys for the model in the evidence-card region; only a key that was actually shown may appear in an `assessment`'s `evidence_keys`. A chain that is not finished (still resumable, or invalidated by a concurrent change) issues **no** key, and a key the model invents is dropped just the same — the legal set is the one the server computed, not "anything with that prefix". Collection keys take no part in the delivery review: they do not travel the evidence budget and produce no `[k]` anchor.
