...
- MinerU-extracted embedded images are retained and shown inline in the source view; captions and text remain searchable. **Markdown sources' embedded images share the same guardrails**: an `![alt](src)` with alt text writes that alt as the element's `metadata.caption`, which enters retrieval on the same footing as a MinerU-parsed PDF caption (indexed into chunks); an alt-less plain-path image still produces no element. When `src` is a `data:image/{png,jpeg,gif,webp};base64,...` URI (the same mime allowlist MinerU's image assets use — unrecognised mimes such as svg/bmp/avif never parse as an image — the `![alt](data:...)` literal is stripped down to just its alt text wherever it appears: alone in a paragraph, mixed with other paragraph text, or inside a list item, heading, or table cell, so the base64 payload never reaches element text), the image bytes are decoded and persisted as a source image asset, shown normally in source detail; per-image byte and per-source count ceilings reuse the MinerU image settings (`MINERU_MAX_IMAGE_BYTES`, default 5MB; `MINERU_MAX_IMAGES_PER_SOURCE`, default 200), and `MINERU_RETURN_IMAGES=false` likewise skips persistence — that switch now gates image persistence for every source, not only MinerU-parsed documents. The data URI itself never enters element metadata; a data-URI image with no alt that fails to persist produces no element. Known boundary: only images occupying an entire paragraph are handled — an image embedded inside a list item or table cell keeps only its alt text and is never persisted as an asset. Markdown referencing local image file paths can be pre-converted to data URIs with `scripts/embed_md_images.py` before upload (see the README's "Product Flow" section).
- **Markdown + image bundle upload**: the add-source dialog also accepts a zip archive or a dropped folder that keeps a markdown file's referenced images alongside it — the shape a Notion/语雀/HackMD export already has. A dependency-free browser-side pipeline (`frontend/app/md-bundle.ts` / `bundle-intake.ts`) parses the zip central directory, decompresses with the native `DecompressionStream('deflate-raw')`, resolves each `![alt](src)` relative path against the archive/folder (handling `./`, `../`, and `%20`-style encoding), sniffs matched files by magic bytes rather than extension, and inlines supported png/jpeg/gif/webp images as base64 `data:` URIs — the same shape the ordinary markdown-ingestion guardrails above already accept — before handing the self-contained markdown to the existing upload endpoint unchanged. `.zip` is a front-end interchange format only: it never enters the backend's supported-suffix allowlist, and only images occupying an entire paragraph are eligible, mirroring the server-side rule byte-for-byte so a pairing success on the client cannot silently become a dropped image on the server. A pairing receipt is listed persistently inside the dialog (not a transient toast) across five categories: inlined, not found (with up to a few near-miss path suggestions), unsupported (wrong syntax/position/mime, or over the per-image/per-source size caps), remote `http(s)` links (never fetched in v1; the original Markdown text is kept), and images with no alt text (flagged as not retrievable once uploaded, since captions are the only entry point into retrieval — see "Citation images" below). `GET /system/config` additionally returns `source_image_max_bytes` / `source_image_max_per_source` (mirroring the deployment's `MINERU_MAX_IMAGE_BYTES` / `MINERU_MAX_IMAGES_PER_SOURCE`, `null` when an older backend omits them — meaning "skip local pre-flight, let the server guardrail decide"; an explicitly sent `0` is a **legal value with the opposite meaning** — persist no images at all, so the browser treats image storage as disabled and skips inlining entirely) and `source_images_enabled` (mirroring `MINERU_RETURN_IMAGES`; missing defaults to `true`, since the switch never existed before and a stale front-end must not invent a warning against a normal deployment), so the pairing step can pre-flight per-image limits and skip inlining entirely — with a persistent notice — when image storage is disabled. Exact guardrail numbers (retained-entry cap, EOCD-declared-entry cap, decompressed-byte multiplier, near-miss suggestion cap, folder traversal depth/file-count caps) are registered in the [table below](#markdown-bundle-upload-guardrails).
...
| Evidence ids retained per claim (base chain) | 8 |
| Top concept names surfaced to the base prompt | 24, ≤48 characters each, ≤600 characters total |
| Overlay prompt's usage section | ≤3,000 characters for the question list, plus ≤12 zero-hit-query samples at ≤120 characters each, plus a fixed header |
...
The research question and the report body (`content_md`) are both served **whole** (no cap). The question used to be clipped at 2,000 chars, silently dropping the tail of the very text that produced the report (the public page serves `reports.question`, the **create-time** value — confirmation writes its edited `resolved_question` into `understanding` and never rewrites that column). Serving it whole is sound only because **creation bounds it first** (`REPORT_QUESTION_MAX_CHARS`): that is the other half of the same red line (the composer shows the guardrail, the API refuses over-limit input), and without it the anonymous response would be unbounded by client input. Note the argument is **not** "the body is bigger anyway" — `content_md` is model-generated and bounded by the generation budget, whereas the question is raw client input. A reference title/original-filename/excerpt is evidence metadata and stays bounded, but an over-length value **sets `title_truncated`/`file_name_truncated`/`snippet_truncated`** (the public page shows a "已截断" hint) rather than dropping the tail silently. `key` (24), `location` (200), and the timestamps (64) deliberately disclose nothing: they are server-derived labels (`kN`, `PDF p.3`, an ISO instant) with no user-authored tail for a cap to eat.
