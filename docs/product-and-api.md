...
Two effort-independent v2 **protocol constants** — deliberately not deployment settings — bound the aspect self-assessment the model returns inside the same reflect turn: an aspect may carry at most **8** evidence keys (`REFLECT_ASPECT_MAX_EVIDENCE_KEYS`), and an aspect's `gap` (the model's own "what is still missing") at most **240** characters (`REFLECT_ASPECT_GAP_MAX_CHARS`). They have no environment variables because changing them changes the protocol, not a budget. Both bound **model-generated internal payload only**: an over-limit aspect is **rejected per aspect** — that one aspect keeps its previous state and the reason is disclosed to the model on the next turn, while the rest of the payload and, crucially, **the turn's own retrieval action still run** (the rejection is recorded as a zero-I/O `skip` step that is deliberately not an action observation, since the action itself already has one). Only a payload whose **whole shape** is unreadable still folds the entire turn into an invalid decision. Neither limit ever trims the user's topics, questions or constraints — user content in the aspect record is never truncated, and an over-limit `evidence_keys` list is never silently cut down to a "sufficient" verdict.
