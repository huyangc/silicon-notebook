...
| `GET /notebooks/{id}/understanding/experiences` | Any reader; this notebook's own slice of the [retrieval strategy experience](#retrieval-strategy-experience) library (next section). Returns `enabled` (follows `RETRIEVAL_EXPERIENCE_ENABLED`, **not** the gate above), `count`, `updated_at` (newest in the slice, `null` when empty), `can_manage` (a pure capability bit, same rule as `can_edit_base`: the mirror-write fence is not folded in — it only refuses the two write endpoints below), and `entries` ordered by `(support desc, updated_at desc, id asc)`, at most `RETRIEVAL_EXPERIENCE_NOTEBOOK_MAX_ENTRIES` (100). An entry carries only `action`/`polarity`/`rationale`/`support`/`adopted`/`updated_at` — never the entry id, the situation fingerprint, or the provenance run ids |
| `POST /notebooks/{id}/understanding/experiences/distill` | Requires `agent_profile:write` (same tier as the shared base; 404 without it); manually schedules one consolidation for this library. 409 while the single-flight slot is taken (sharing the manual-rebuild sentence), 409 with a different sentence while `RETRIEVAL_EXPERIENCE_ENABLED` is off, and 409 with the `sync_origin`-carrying object detail when the mirror fence refuses it |
...
- `POST /api/notebooks/{id}/understanding/experiences/distill` — requires `agent_profile:write` (404 without it); manually schedules one consolidation, 409 while the single-flight slot is taken or while `RETRIEVAL_EXPERIENCE_ENABLED` is off (two different sentences)
