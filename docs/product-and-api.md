...
- Edge trust & curation: `GET /api/notebooks/{id}/edge-review-queue` → `{items: EdgeReviewItem[], total}` (edges ranked by review priority desc, `limit`-bounded — default 100; `total` is the queue's true size — a seq-gated `COUNT(*)` over non-rejected edges, independent of `limit`, so the UI can show the full remaining count instead of just the page it received), `POST /api/notebooks/{id}/relations/{rel_id}/review`
