...
| Overlay report attempt rows read per consolidation | ≤200 (a separate cap from the 10-report sample, same reason as the trace-step cap above; when it binds, the oldest report's tail directions are dropped first) |
...
| Overlay prompt's usage section | ≤3,000 characters for the ask question list, plus ≤12 zero-hit-query samples at ≤120 characters each, plus a fixed header; the report sample renders as a second section sharing the SAME 3,000-character budget rather than a separate constant |
