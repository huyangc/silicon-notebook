...
This fact travels with the evidence into Ask's single-pass and per-section synthesis and into a report's section writing, so the synthesis prompt gains a short **server fact**: why retrieval stopped, which mandatory questions it left open, which channels never recovered. It states on its own face that it is **not a knowledge item, carries no `[k]` id and must never be cited**, and that it only says where *this* retrieval stopped rather than proving the notebook lacks the material — the model's job is to state the gap plainly in the answer, not to refuse. The model's own `gap` text does not enter this block.

**After final assembly**, the server re-checks each aspect against the evidence identities that actually entered the prompt, keeping three registers apart: `model_supported` (the model says this aspect has support), `synthesis_admitted` (at least one of its bound evidence items really entered the prompt), and `answer_cited` (the finished answer really carries an anchor bound to that evidence). They are **not nested**: entering the prompt does not prove the answer cited correctly, and real citation is still answered by the existing binding and grounding paths. An aspect that bound evidence whose every item was removed by budget or filtering is demoted to **undelivered** and no longer displayed as supported; partial removal is not undelivered, because the model did see one of those items. The re-check requires no early final re-ranking and reads nothing from the database. Later gap-filling retrieval may be used in the final synthesis but never rewrites the historical fact that reflection had already stopped.

These facts are stored along the existing **private** trace and report-section paths: Ask records them on the closing skip step and the terminal synthesis step's diagnostic fields, a deep report on the section row. The UI reuses the existing live trace and post-run synthesis note to show the termination sentence, "x of N aspects handled", and where relevant "y undelivered" and "z channels never recovered" (counts only — internal action names stay off screen); an ordinary answer gains **no** fixed error banner, because a partially completed retrieval is not an error. Historical traces without these fields render exactly as before. The public sharing surface is unchanged: the anonymous conversation and report projections are explicit allowlists that never carried the reasoning surface at all, so no aspect assessment or termination fact crosses that boundary.
