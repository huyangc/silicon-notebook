...
## 7. PR-A：按篇读取原文（`read_document`）——设计取舍登记

PR-A 补的是 chunk 通用问答已有、reasoning 此前没有的一项能力：文档介绍（摘要为空的文档该
拿什么当依据）。实现在既有的枚举动作之外新增一个 reflect 动作 `read_document`；设计过程中
有几处偏离了第一版设想，逐条登记在这里，而不是散落在代码注释里让人拼凑。

- **为什么是动作，不是正则路由**（用户裁决，2026-09-14）：与 §0.2「决策哲学」同一条原则——
  模型负责决策，执行器负责事实。chunk 通用问答的文档介绍走的是问法分类器
  （`overview_intent`）确定性触发；但 reasoning 是 agentic 循环，缺什么工具就该给模型一个
  它能自己判断要不要调用的工具，而不是在服务端猜一次问法、替模型把这个决定做掉。用户在这轮
  改动里明确拍板：**reasoning 仍以「缺什么工具就提供这个工具，由模型判断调用」的方式补能力；
  不做正则路由替模型决策**。这也是为什么 `read_document` 不像 chunk 那条路径一样有「明确的
  中英文介绍表达」识别层——它没有，也不需要：模型看到摘要为空的花名册行，自己决定要不要读。

- **来源解析为什么走花名册、不走 `resolve_source_title`**：枚举那一侧限定单一来源时
  （`enumerate.source_id`/`enumerate.source_title`）已经有一个按标题解析来源的执行器——
  `collection_enumeration.resolve_source_title`，它会带着 `cancel_event` 回库做一次有界扫描。
  `read_document` **不**复用它。原因是范围：`resolve_source_title` 解析的是「这次枚举要不要
  narrow 到某一个来源」，它的搜索面是当次枚举计划要访问的那批来源，允许对**尚未列出过**的
  标题做一次真实探测。`read_document` 的标题却必须只在**本 run 已经用
  `enumerate.collection="sources"` 兑现过一次范围合法性检查的那份花名册**里精确匹配——花名册
  本身就是 `enumerate_sources` 按参与库与 `source_scope` 收窄之后的结果，零额外 I/O 地复用它
  等于让 `read_document` 继承同一次范围检查，不必、也不该再向数据库发一次标题查询。回库重新
  解析一次标题看似只是「换一种实现同一件事」，实际会打开一条模型能用标题探测范围外文档的
  路——花名册没列过的文档，标题猜得再准也不该读得到。
  匹配口径是花名册**渲染出来的显示串**（`source_title or UNNAMED_SOURCE_LABEL`，与
  `collection_enumeration_answer` 同一个常量），所以无标题文档按占位「未命名来源」也读得到；
  模型把整行「标题 · 类型: 摘要」抄回来时，精确匹配失败后只再做一次确定性兜底——按第一个
  ` · ` 分隔符切掉尾巴再精确匹配，仍不做子串或模糊匹配。多篇同名文档按标题无法区分，
  skip 文案如实说明并让模型改读别的文档或在答案里说明。

- **为什么号段 `7000` 不进 `k4001+`（参考库徽章）**：直接元素证据分区（`_ELEMENT_KEY_BASE`
  起的 `k4001+`）是给 `answer_element_items` 那条按分数排序、跨整个检索范围直接挑元素的路径
  用的，那里的元素对象在装配时 `notebook_id` 恒为空、`tier` 一律取当前笔记本——本库元素与
  挂载参考库元素在这一段里是**同一形状**，没有字段能标出「这条其实来自参考库」。`read_document`
  取样的文档却可能就是一篇挂载参考库里的文档；如果混进 `k4001+`，参考库来源会被当成本库
  来源渲染，跨库引用的「来自参考库《名》」徽章会消失。号段独立开在 `DOCUMENT_READ_KEY_BASE
  =7000`（紧邻 `_EXTERNAL_KEY_BASE`）还有第二个理由：`k4001+` 的准入受
  `answer_element_items` 这个请求级预算控制，把按篇取样也塞进同一段会让两条互不相关的预算
...
- **被否的 `sampling:"summary"/"excerpts"`**：早期讨论曾考虑把这个参数命名为
  ...
  做有界分布取样，从不调用模型合成摘要（与通用问答「文档介绍」单篇定位那条零 LLM 执行体
  完全同一份代码，那条路径本就不调用嵌入或图谱）。叫 `sampling:"summary"` 会让模型以为选了
  这一档就能拿到一份真正生成的摘要，而实际拿到的仍然是原文摘录，只是取样位置不同——参数名
  应该描述执行体**真的在做什么**（选取样方式），而不是暗示一个不存在的合成步骤。`coverage`
  这个名字加上 `"spread"/"opening"` 两个值没有这个歧义：它们只谈取样的**分布方式**，不谈
  「摘要」还是「摘录」。

- **改名 `read_document` 的原因**：计划最初写的动作名是 `read_source`。实施中发现这与仓库
  既有的隐私守卫冲突——`test_retrieval_experience_privacy_guard.py`（连同
  `test_retrieval_experience_job.py`）禁止 P2 检索策略经验的动作词表（`RETRIEVAL_ACTIONS`）
  出现 `source`/`scope`/`notebook`/`base` 这类子串，因为那张全局共享、无归属谓词的表被设计
  为「经验只能影响**怎么查**，绝不能影响**能读到什么**」（见本文档 §6.1 的隐私论证同一条
  红线）；一个名字里带 `source` 的动作，哪怕语义上只是「读一篇文档的正文」，也会让阅读代码
  或审计这张表的人第一反应是「这条经验在谈论来源范围」。动作 id、trace step_type、部署配置
  与策略字段因此统一改为 `read_document`（`REASONING_DOCUMENT_READ_ENABLED` /
  `REASONING_MAX_DOCUMENT_READS` / `max_document_reads`）；schema 内的参数名 `source`
  （取值仍是花名册标题）不受影响，因为参数名不进入 `RETRIEVAL_ACTIONS` 那张词表，前端标签
  「取样 / 按篇读取原文取样」同样不受影响。
