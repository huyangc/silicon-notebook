...
- **触发条件是「解析后仍有 ≥2 节拿得出证据」**，不是「大纲里有 ≥2 个非空节」：绑定键在
  ...
  ——那一轮压根没进过分节路径，`synthesis` 步的 detail 里三个 `outline_*` 键一个都不出现，
  与关闭态逐键一致。
...
- **`grounded = any(各节)` 是 v1 的刻意语义，不做按比例降级**（用户裁定）：11 节自报 false + 1 节
  true，只要那一节有相关度 ≥ `tau_high` 的被引用证据，整篇仍判 grounded。理由是 `classify_evidence`
  才是真正的门（`any` 单独抬不高任何东西），而按比例降级是**产品行为变更**——它会让一篇 12 节里
  只有 1 节有据的答案在徽章上从「有据」变成「概览」，那是要真机评估的取舍，不是实现细节。留真机
  评估项。
...
- **`grounded` 取各节的 `any`**：每节的 `grounded` 只描述它自己的切片，要求 `all` 会让一节合理的
  常识补充把整篇降级。真正的门仍在 `classify_evidence`（还要求存在相关度 ≥ `tau_high` 的**被引用**
  证据），所以 `any` 单独抬不高任何东西。`counts` 是各节装配计数之和——同一条证据被两节绑上就
  计两次，那是诚实的：它确实进了两次 prompt。
