...
- 放行：notebook:grant、notebook:configure、scale_index:write（kg_index/kg_viz 是目标端自有的
  ...
  **目标端编辑与源端权威在这两张子表上会正面相撞**（codex #788 r5 P1）：基线之后，同一条
  memory 两端都可能各自新增修订——目标端用户对镜像 memory 的确认/修改本就允许（上面「放行」
  那条），会让目标端自己的 memory store 给它分配下一个修订号；源端浑然不知，之后自己也给
  同一条 memory 分配了同一个修订号（各自的行 id 不同，`(memory_id, revision)` 这个二级唯一
  键却撞了）。窗口不再做快照式对账之后，这条冲突既不能靠删除重放救（源端根本不知道目标端
  那条自建行的 id，从没有对应的删除条目）、`--resume` 也没用（同一个冲突每次都撞）、还可能
  因为 §8「预检」的取代规则（同源更新的全量包只有在失败包两个删除标记齐全时才能取代它）
  被拦在链外——顺着正常「拒绝这一行、整个导入失败」的口径走下去，这条同步链会永久卡死。
  规则：对这两张**源端权威**子表，窗口（增量）模式下的二级唯一键冲突做**冲突特异**的
  源端权威消解——见 §8「导入相位」3b。其它表的二级唯一键冲突仍然是硬失败，不享受这个
  特殊通道；全量包这两张表仍然走上面那套整体 `_prune_superseded`，不受影响。
...
- KG 按笔记本以 `(kg_reset_epoch, kg_mutation_seq)` 做版本：epoch 变了，导入对该笔记本执行
  ...
  等）原样搬。导入器接受 `mode=full` 与 `mode=incremental` 的包（§8）；`to_seq` 不要求为
  0——从 v85/0065 起全量包也带着捕获到的水位（§7），但内容仍然是表扫描而不是日志重放。两种
  包 `kg_epochs.jsonl` 恒为空文件（折叠延后，见 §11），所以这里描述的「整体替换」分支目前
  都不触发：全量包靠 §8「导入相位」的对账删除相位让整个导入天然等价于「整体替换」；增量包
  靠逐行 upsert/delete 精确重放（§8 「删除重放」）达到同样效果，不依赖 `kg_epochs.jsonl`
  驱动的整体替换指令。
...
实现（SQLite v85 / PostgreSQL 0065，已合入 #784，见 §10）；`deletes.jsonl` 的重放是导入端的
工作，由 PR-3c 实现（进行中，见 §8「导入相位」）。`kg_epoch` 折叠延后，`kg_epochs.jsonl`
两种包都仍是空文件（见下「压缩」与 §11）。
...
- **前驱校验**：发布水位是一次条件写，不是后写者胜出——两个导出可能同时对准同一个 target。
  ...
  发布事务写水位之前，还会先锁住并校验自己的租约行（`sync_export_runs`）：`run_id` 是否仍然
  等于本次这次跑——PostgreSQL 对那一行 `FOR UPDATE`，SQLite 在同一个 `BEGIN IMMEDIATE` 下
  执行同一次校验。租约已经被别的导出接管（`run_id` 变成了别人的）、或者已经被接替者发布水位
  之后删掉——两种情况都意味着「这次跑已经不是这个 target 当前公认的那一个」，本次同样**不
  发布**，删掉刚落盘的包目录，报错点名 `export lease ... was taken over`。这条校验单独存在
  是因为前面两条（`base_package_id`/`exported_through_seq` 的条件写）挡不住一种更隐蔽的情形：
  一次卡住超过一小时的导出被判「死」、接替者接管租约并成功发布之后，原来那次导出才终于恢复、
  继续往下跑——它读到的 `seq` 可能与接替者发布的水位**相等**（两边看见的都是同一段日志尾巴），
  但它的读快照更旧，快照里在途、接替者已经等到提交并纳入包内（可能已经被 `prune-log` 按接替
  者的水位清理掉）的那些事务，在它的包里根本不存在。让它凭「seq 相等」这个巧合覆盖过去，会把
  这些已经进过链的晚提交事务从基线上悄悄抹掉，`base_package_id`/`exported_through_seq` 的检查
  本身看不出这一点——两边比的是数字，不是「我还是不是那个还在跑的合法进程」。
...
- **归属**：NOTEBOOK scope 表直接用行/日志自带的 `notebook_id`；PARENT scope 表的 upsert 行
  ...
  （`deletes.jsonl` 该行 `notebook_id: null`）——按目标端父链解析归属、把解析不出的孤儿删除
  处理成 no-op，是导入端的工作（§8「导入相位」3c「删除重放」）；导出端只如实记下「解析
  不出」，不在导出侧猜测或丢弃这条删除。
...
- **文件**：只复制本包 upsert 涉及的 `sources`/`notebook_assets` 行指向的文件，相对 storage
  ...
  没跟着来，不影响其它字段落库）。被删行指向的文件本包不处理，由导入端在重放 `deletes.jsonl`
  时按目标行路径删除（§8「导入相位」3c「删除重放」）。
...
影响 0 行），不会误删接管者刚写下的新租约。「姗姗来迟」不止会走到收尾删除：如果这个旧 run
没有崩溃、只是卡住超过一小时之后又恢复，它会一路跑到发布水位那一步——发布事务因此在写水位
之前先锁住并重验这一行的 `run_id` 是否还是自己（见上「导出水位」「前驱校验」），不是走到
删除才发现晚了。
...
sync-<source_env>-<from_seq>-<to_seq>-<package_id 前 8 位>/
  ...
  kg_epochs.jsonl    # 需要整体替换的 (notebook_id, epoch)；折叠延后，恒空文件（§7「压缩」、§11）
  files/notebooks/<notebook_id>/...   # 上传件；增量包只含本次新增或变更的文件；被删行的文件
                                       # 不在包里，由目标端在重放 deletes 时按目标行路径删
                                       # （§8「导入相位」3c「删除重放」）
...
重放是导入端的工作（§8「导入相位」3c「删除重放」按 `reversed(synced_tables())` 逐表重放）；
导出端只保证行序可预测、不保证行序可直接拿去执行。
...
sync import <package_dir> [--create-missing-users] [--dry-run] [--importer-user <id>]
    ...
    # 重建基线）。除这两种形态外的任何组合（mode 未知、mode=full 但 from_seq≠0、
    # mode=incremental 但 from_seq=0 或 base_package_id 为空、kg_epochs 非空）一律在预检阶段
    # 拒绝并点名具体字段。
...
1. 预检：manifest `mode` 先分类（`_classify_package`）——`full`（`mode=full` 且 `from_seq=0`；
   `deletes`/`kg_epochs` 必须为空，语义与 PR-3a/3b 时代一致）或 `incremental`（`mode=incremental`
   且 `from_seq>0` 且 `base_package_id` 非空；`kg_epochs` 必须为空，`deletes` 允许非空）。除这
   两种形态之外的任何组合——`mode` 取值未知、`mode=full` 但 `from_seq≠0`、`mode=incremental`
   但 `from_seq=0` 或 `base_package_id` 为空字符串——在读 manifest 这一步就拒绝并点名具体
   字段，不当作增量或全量放行（读 manifest 时就报，不等到下面这些检查）。schema pair 相等、
   `EMBED_RUNTIME_DIM` 相等、包校验和通过、源环境标识与已有镜像一致（包里每个笔记本若在
   ...
   **已知边界，登记而非掩盖**：导入端分辨不出一个全量包导出时变更捕获的门是开是关——
   ...
   包续跑必须显式 `--resume`。FAIL 策略的用户引用（notebooks.created_by、
   memory_items.created_by）在预检里就校验，dry-run 同样硬失败，绝不在删除相位之后才发现。
...
3. 行——四个子相位，`kg_epochs` 恒空（§11「`kg_epoch` 折叠延后」），两种包都不触发整体替换
   ...
   a. **对账删除**（仅全量包）：逆序、每表一个事务，进 `sync_import_progress`：对每张
      ...
      **增量包跳过这一相位**——它不是一份快照，没有「包内笔记本此刻的完整主键集合」这个
      基准可以拿来对账；源端的删除已经完整落在 `deletes.jsonl` 里，下面的精确重放就是它的
      对账方式，两者不叠加，对账删除会把增量包没提到、但目标端确实还该存在的行误删。
   b. **行 apply**（全量、增量基本同形，两张表例外见下）：按拷贝顺序 `INSERT … ON CONFLICT
      (pk) DO UPDATE`。`knowledge_object_sources`、`community_members` 自 PR-3a（v84/0064
      补了复合键，见 §7）起与其它表一样走这条 keyed upsert 路径，不再是 PR-2 的删后整批
      插入。一条过渡规则仍然存在：`sources.file_path` 按 shadow manifest 的 `path_columns`
      重锚到目标端的 storage 根（包里是源主机的绝对路径）。本次预留（reserved）的笔记本经
      这一步从 `importing` 的壳子填上真实内容。**「借目标端父链」接受的 PARENT 子行，归属在
      这个写事务里再核一次**（`_reassert_borrowed_parents`）：预检 `_verify_row_scopes` 对
      增量包的第二次机会（见上）读的是预检那一刻的快照，不是锁；这批「借来的」父键（预检
      返回的 `borrowed_parents`）在真正 upsert 之前，用同一套 `scope_chain` 解析在这个事务里
      重新问一遍——预检和这一刻之间，那张父表的行完全可能已经被挪到别的笔记本下，解析结果
      一旦落到 `manifest.notebooks` 之外就**拒绝整个导入**并点名，提示「另一个写者已经动过
      它，这个包的这次运行需要在目标端稳定之后重新导入」。只重新解析「借来的」那部分父键
      ——包自己带着父行的表不需要目标端确认，为每一批都重走一遍整条父链会抵消预先算好这些
      集合的意义。

      **`memory_revisions`/`memory_provenance` 的二级唯一键冲突，增量包下走源端权威消解**
      （codex #788 r5 P1；理由见 §5「目标端编辑与源端权威在这两张子表上会正面相撞」）：
      两端基线之后各自独立给同一条 memory 加修订，双方的修订行各自分配到自己一侧的
      `(memory_id, revision)`，行 id 不同、这对业务键却可能撞上——upsert 按主键 `INSERT …
      ON CONFLICT (pk)` 本来不会报错（主键不同，是两行），真正会炸的是数据库另一条独立的
      `UNIQUE (memory_id, revision)` 约束。这两张表**只在这里**（也**只对增量包**）有一条
      特殊通道：同一个写事务里，upsert 这批行之前先按包内每一行自己的 `(memory_id,
      revision)` 回查目标端是否已有一行占着这个组合——查到了、且那一行的主键不在本包携带
      的行集合里（目标端自建，不是源端曾经写过又被改了 id 的行）⇒ 只删这一行，把位置让给
      即将 upsert 的源端行；查到了但主键就在包内（正常的重放/续跑场景，同一行的更新）⇒
      不删，走普通 upsert。目标端自己那些包没提到的历史修订/溯源行——不管是不是撞了键——
      一律不碰：这不是对账，只解冲突，冲突之外的行留给上面那句「目标端自建的记忆不受影响」
      的共存承诺管。解决掉的每一条冲突都计入报告（`source_authoritative_collisions_resolved`，`sync import` 人读与 `--json` 都输出）并发 warning，点名冲突的 `(memory_id,
      revision)` 与被让位删除的目标端行；**非这两张表**的任何二级唯一键冲突仍然是硬失败，
      不享受这条特殊通道——它们没有「源端权威、目标端行天生可丢」这个前提，`chunk_questions`
      那类冲突本该失败提醒去查数据，不能悄悄丢目标端的行。**全量包不受影响**：它走的是
      §5 提到的那套整体 `_prune_superseded` 对账，不需要、也不会碰这条冲突特异的通道。
   c. **删除重放**（仅增量包）：`deletes.jsonl` 只读一遍——预检的 `_verify_delete_scopes`
      ...
      id 导出后原样不动、从不重新分配，目标端此刻这个主键下的行完全可能已经属于范围内另一本
      笔记本 M；照条目自己的话折叠，M 的这一行会被当成「已折叠进别人的删除作业」而漏删——3d
      排的作业只清它标注要删的那本；照条目自己的话跳过（copying）更糟，因为没有任何作业会再
      清它。所以每条条目都进下面这套归属校验：
      ...
      - 文件：`sources` 条目在删这一行前先回读目标端这一行的 `file_path`，按
        ...
        `storage/assets/<nb>/<asset id>.*`（同 §7「文件」一节的 glob 口径）。文件删除在
...
- 孤儿删除的目标端归属规则已实现（§8「导入相位」3c「删除重放」）：PARENT scope 表的 delete
  日志若父行也已经被删，`deletes.jsonl` 该行的 `notebook_id` 写 `null`（§7「归属」，导出端
  不猜测）；导入端按目标端当时仍存活的父链重新解析归属，解析得出就按结果删，解析不出（孤儿）
  就计入 `deletes_orphan_skipped` 并 no-op，不强删、不猜。
