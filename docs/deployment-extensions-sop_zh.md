...
`ask.engine` provider 登记一份 `mode_id` 以自身插件 id 开头的描述符，并返回 `AskEngineResult(answer_markdown, citations)`。它只收到当前问题及检索、模型、轨迹、取消端口。检索返回有界证据和本次 run 内的不透明句柄；只能引用本次调用中由同一端口签发的句柄，并在 Markdown 放入对应 `[kN]` 或 `【kN】` 标记。核心对整份引用 fail-closed 校验、自己构造 durable citations，并在保存前复核 run 身份。除元素 `search()`/`fetch()` 外，`RetrievalAccessPort` 还暴露一套有界 KG 读端口——`search_kg(query, k, object_types=())`、`kg_neighbors(evidence_key, k, edge_type="", direction="both")`、`kg_overview()`——`search_kg` 与 `kg_neighbors` 共享一份独立预算（`ASK_PLUGIN_ENGINE_KG_SEARCH_MAX_CALLS`）；KG 命中只有存在存活证据绑定才可引用，引用打开的是该对象首条存活证据元素（不是图谱视图），邻居结果不带边类型标签与截断信号。完整契约细节与护栏见[产品与 API 参考](./product-and-api_zh.md#部署问答引擎-askengine)。v1 刻意没有会话历史、意图预检、PPR/社区/图漫游、repository、settings、连接、raw model client 或实时轨迹接口；实时可用性通过时会在浏览器高级模式的第三分组出现，MCP 的 `ask_notebook` 也会把同一个已注册且实时可用的 mode id 接纳为其 `mode` 参数——插件引擎单次调用可能比内建 mode 长得多，调用方的客户端读超时需要留出真正的余量。
...
- `ReflectActionContributor.invoke` 跑在同一类私有 worker 线程上、受同一套硬 deadline 纪律约束（`REASONING_PLUGIN_ACTION_TIMEOUT_SECONDS`，可用性检查与调用本身共用同一份预算——默认值与完整预算账目见 [Reflect 插件动作](./product-and-api_zh.md#reflect-插件动作askreflect_action)）：同样不要依赖 `contextvars`/线程局部状态，`ReflectActionCallContext` 上也没有 notebook id、actor id、检索范围或任何其它核心端口的位置——只有 `question`、模型自己写的、已校验过的 `arguments`、一个 `cancellation` 令牌、`deadline_monotonic` 与 `max_items`。返回一个 `ReflectActionResult`（不是裸元组，也不要包一层 `ContributorResult`）；每个 `ReflectActionItem.excerpt` 就是引用卡将会原样显示的那段文字——绝不会被二次摘要，所以把可引用的原文放在这里，不是一段转述。每个字符串字段都会被渲染在一个以 `k<n>:` 为键的证据块的一行里，因此带换行或其它控制字符的值会被折成一个空格，而不是被信任留在自己那一行；`url` 只在是 `http`/`https` 时才被接纳。框架在启动冻结期校验你的描述符形状——超长字符串、控制字符、重复或保留的动作/参数名、或取值不足两个的枚举都会直接注册失败（`ExtensionRegistryError`，即启动失败），绝不静默夹取。描述符上的 `max_calls_per_run` 是一道天花板，部署自己的 `REASONING_MAX_PLUGIN_ACTIONS` 策略只能把它调低，不能调高。
