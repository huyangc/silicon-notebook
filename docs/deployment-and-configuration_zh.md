...
`graph_chain_verify`）。任一条不满足，进程以非零码退出，后端日志（以及 stderr，`prod.sh` /
`backend.sh` 把它追加进同一个日志文件）里是一整句可读的原因，没有 traceback：
...
**这一行不会打印配置文件的路径**——它写的是「MODEL_SERVICES_CONFIG 指向的文件」，请照着这个
环境变量去找那份 TOML。原因是后端以 `uvicorn … >>"$BACKEND_LOG" 2>&1` 启动，stderr 就是日志
文件，所以启动失败不再抛 traceback，只打印这一行。同理，来自配置文件的陈旧 id 只有形如
workload id（`^[a-z0-9_]{1,64}$`）时才原样列出，其余折叠成 `<无法显示的键名>×N`（带表名和计数）
——一个含路径或换行的 TOML 键否则就能把私有路径写进日志、甚至伪造出一整行假日志。工作负载 id
与中文标签是代码里的封闭词表，照记。其它模型配置错误（文件读不了、密钥没填、服务字段非法）
同样停进程，但只打印一行固定文案 `模型服务配置无效（MODEL_SERVICES_CONFIG 指向的文件解析失败）`，
异常文本一个字都不出现——具体哪一行错了，照着那个路径去看文件。
...
运行时独立的有界进程内 LRU 缓存，同一笔记本的新会话可复用，淘汰或重启后
重新生成。来源内容、模型与提示词指纹共同控制失效。模型输入明确只是文档
的部分内容，不是全文或全库读取；每项来源字段或摘录也遵守既有的
`EMBED_TRUNCATE_CHARS` 预算。

```text
NOTEBOOK_QUESTION_SOURCE_LIMIT    # 本库文档取样数量（默认 20，范围 1–200）
NOTEBOOK_QUESTION_INPUT_CHARS     # 来源 JSON 的总输入预算（默认 12000，范围 1000–100000）
NOTEBOOK_QUESTION_CACHE_ENTRIES   # 每个运行时缓存的笔记本代次数量（默认 128，范围 1–4096）
NOTEBOOK_QUESTION_RETRY_SECONDS   # 生成失败后的重试冷却秒数（默认 60，范围 1–3600）
```
...
CHUNK_FEDERATION_PEER_FLOOR  # 高级调优：跨库合并的相对下限，基准是本次臂里最高峰值的那个库（默认 0.5；0..1）。参与集里每个库走的是**同一条** chunk 生产者、同一个 0..1 融合量纲，所以分数跨库可比：自己最高分低于「全场最高 × 本值」的库拿不到保底名额（它的命中仍参与剩余名额竞争），剩余名额也改按 `(score/全场最高)/库内名次` 排，而不是各自与自己最高分比。调高可以让一个聚焦问题的召回预算留在真正答得上的库；设 0 回到旧行为——每个挂载的库无论多不相关都保底一条，代价是强库每多挂一个库就少一条真证据
CHUNK_FTS_WITH_ANN_ENABLED   # 仅深度报告：健康带索引 chunk ANN 后恢复通用词法 union（默认 false；普通 Ask、精确定位与 ANN fallback 不受影响）
POSTGRES_CHUNK_FTS_TIMEOUT_SECONDS # 仅 PostgreSQL：单次通用 chunk 词法调用的独立 deadline（默认 3.0，2026-09-14 前是 1.0；POSTGRES_STATEMENT_TIMEOUT_SECONDS 低于 3 的部署必须显式设置本项，否则启动校验拒绝；>0、<=10 且 <=POSTGRES_STATEMENT_TIMEOUT_SECONDS）；超时后对本 run 的该 notebook 熔断
...
Reflect 只使用 Legacy。已退役的 `REASONING_REFLECT_*` 配置不再选择协议、布局
或测量模式，应从部署配置中移除。本次清理不需要数据迁移。固定反思指令排在问题
与候选摘要之前，不新增布局开关。
