...
REASONING_REFLECT_OPTIMIZATION # reflect 上下文的**前缀复用策略**（默认 `off`；四取值 `off / prefix_snapshot / prefix_delta / prefix_delta_lean`）。`off` 是实施基线的 v2 原行为，逐字节不变。**目前这一项做的事**：登记本部署跑哪一条臂，并在非 `off` 时于本 run 第一次 reflect 生成一次静态工具目录（这次 run **可能**执行的工具的稳定超集）。它**还不改** reflect 消息的布局，不改任何 prompt、决策与输出；布局重排（稳定的协议与目录在前、逐轮变化的当前状态在末尾，让上游模型服务有机会复用不变的前缀）在本次改动的后续提交里落地，届时本条一并重写。后两格**已登记但未实现**：配上去会在启动期被响亮拒绝（`REASONING_REFLECT_OPTIMIZATION=<取值> 该取值将在后续 PR 实现,当前请用 off 或 prefix_snapshot`），而不是静默退回 `off`——一个自以为在跑增量的部署会把每一条测量都归到错误的臂上；至于压根不过这个校验器的 duck-typed settings 适配器，运行期对已实现闭集之外的任何取值一律折回 `off`。它**叠在** `REASONING_REFLECT_V2_ENABLED` 之上：v2 总闸关着时反思走 legacy 协议，根本没有「前缀」可谈，这一项被忽略；Knowhow 智能补全（显式 legacy）同理忽略它。它**不是**前端检索档位，也不由档位推导。判据的全仓唯一读点是 `reasoning_retrieval.reflect_optimization()`
REASONING_REFLECT_MEASURE_CONTEXT # reflect 上下文的**测量**开关（默认 false），与 `REASONING_REFLECT_OPTIMIZATION` **正交**——正是它让 `off` 臂也能用同一把尺子量，对照实验才可比。**目前这一项做的事**：只有登记，还没有任何测量、也不产出任何观测。逐轮那份纯内存的块长/字节/公共前缀观测在本次改动的后续提交里落地，届时本条一并重写。测量落地后同样不改任何 prompt 与决策；默认关是因为它要序列化一份本轮消息，关闭态不该多付这笔开销。同样叠在 `REASONING_REFLECT_V2_ENABLED` 之上，v2 关闭与 Knowhow 恒不测量
