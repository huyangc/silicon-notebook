...
# `prefix_delta_evidence`：显式启用的 V2 实验档，保留证据卡与 `prefix_delta` 的 S + C + K + D + T 布局、冻结卡片、只追加的动作观察、共用预算、重建及回退机制。关闭逐项自评指令与输出 schema、自评吸收及追问、动态方面状态，以及合成阶段的自评缺口注入。完整问题、必答主题、用户约束、当前合法动作及额度继续保留。上面的证据、历史及 delta 预算设置也适用于本档，不增加预算或模型调用。原有各档及默认 `off` 保持原行为。使用 `REASONING_REFLECT_V2_ENABLED=true` 配合 `REASONING_REFLECT_OPTIMIZATION=prefix_delta_evidence`；关闭 V2 时实际 optimization 仍为 `off`。前缀稳定只表示为服务端复用提供机会，不代表已命中 cache 或已有加速结论。显式配对实验用法见 `scripts/README.md`。
