...
REASONING_REFLECT_MEASURE_CONTEXT # reflect 上下文的**测量**开关（默认 false），与 `REASONING_REFLECT_OPTIMIZATION` **正交**——正是它让 `off` 臂也能用同一把尺子量，对照实验才可比。**目前这一项做的事**：只有登记，还没有任何测量、也不产出任何观测。逐轮那份纯内存的块长/字节/公共前缀观测在同一改动集的测量任务里落地，届时本条一并重写；上一条的布局已经生效，两者正交。测量落地后同样不改任何 prompt 与决策；默认关是因为它要序列化一份本轮消息，关闭态不该多付这笔开销。同样叠在 `REASONING_REFLECT_V2_ENABLED` 之上，v2 关闭与 Knowhow 恒不测量
