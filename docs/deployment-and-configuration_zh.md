...
（跨翻转在飞读者的宽限期），唯一回收口是下一次 rebuild 的预回收与启动恢复。两次
rebuild 间隔越长，双代共存期越长；按 2× 的行数与索引体量规划磁盘与 autovacuum。
...
REASONING_REFLECT_V2_ENABLED # 逐步推理 Ask 与深度报告的 reflect v2 协议总闸（默认 **false**）。开启时每一轮反思都由一次纯内存的能力投影生成：模型读到的动作说明与解析白名单是同一个值的两处呈现，所以配额耗尽、被调用方策略关掉、在所选来源范围下不安全、或无图时没有意义的动作会**同时**从两处消失（schema 的 `next_action` 枚举刻意仍列出全部可识别 id，好让这类选择变成一条被记录的观察，而不是传输层的拒绝）；同时把一轮拆成固定的 system 段（任务、材料不可信、范围规则、动作契约、结束规则）与逐轮的 user 段（问题与候选材料），响应用单一 `arguments` 对象取代逐动作的分支字段，并按所选动作类型化校验。关闭时逐字节回到接入前：旧 prompt/schema/白名单/调用次数/trace 键集，不构造能力投影、不增加任何 I/O。它**不是**前端检索档位，也不由档位推导。Knowhow 智能补全无论此开关如何都显式留在 legacy 协议上
...
REASONING_REFLECT_RECENT_OBSERVATIONS # 最多回喂几行近期动作观察（默认 6；1–20）。它与 `REASONING_REFLECT_STATE_CHARS` 一起界定 v2 的动作观察账，上面两项证据预算界定 v2 的证据卡。四项都只在 `REASONING_REFLECT_V2_ENABLED` 打开时生效——协议关闭时没有任何代码读取它们（启动期照常校验）。⚠ **本项的含义与 `REASONING_REFLECT_OPTIMIZATION` 挂钩**：在 `off` / `prefix_snapshot` 两格上它就是上面这个意思——逐轮滑窗的窗口大小，两格逐字同义（两格共用同一次装配，见下一条的「只改块的位置」）。在 `prefix_delta` 上它**已经是另一个意思，而且是当前行为不是预告**：只在**建/重建证据卡快照 K 的那一次**被读，含义是「这一版 K 里保留多少条近期详细观察」；窗外那些行折成已尝试/已失败/重复/完整性的计数披露，**不再触发逐轮滑窗**。两个切片刻意**不相交**（近期窗只吃最后 N 行、折算只吃它之前的全部行），所以「更早的 M 条未列出」与「总计已尝试 K 次」各指自己那一段，恒等式「窗内已列 + 窗内被丢 + 窗外总计 = 全部行数」由 `test_delta_rebuild_slices_the_ledger_into_two_disjoint_halves` 钉住。逐轮的增量块**一次都不读它**：那一块里的观察行是「这一轮发生了什么」的唯一记录，照单追加不裁，预算判断在追加**之前**做（`test_delta_history_charge_counts_only_the_rows_that_reached_a_block`）。`prefix_delta_lean` 与 `prefix_delta` 走的是同一套 K/D 判据——这一句对四格都是当前行为，不是任何一格的预告
REASONING_REFLECT_DELTA_CARDS_BY_EFFORT # `prefix_delta` **单块增量**里最多新展开几张证据卡，按档位的 JSON 对象（默认 `{"overview":2,"standard":4,"deep":6,"thorough":8,"exhaustive":10}`）。启动期校验逐条镜像上面那份证据字符预算：必须**恰含**五个档位 id，每个值是 1–16 的整数（JSON 布尔值会被拒绝，而不是被静默读成 0/1），且随档位不递减（`test_reflect_delta_cards_rejects_bad_mappings` 逐格钉住）。**只在 `REASONING_REFLECT_OPTIMIZATION=prefix_delta` 下被消费**：`off` / `prefix_snapshot` 两条臂一次都不读它（启动期照常校验），`core/config.py` 自己也一次都不读。**全仓唯一读点**是 `ReasoningRetriever._reflect_v2_context`——经它专用的 `_delta_max_cards`，而那个 helper 只有一个调用点，就在那个方法里；这条纪律与另外五项预算同源（AST 判据 `test_reflect_optimization_has_exactly_one_settings_read_point`，六项预算逐项参数化，**每个字段恰一个读点**），因为同一个字段有两个读点时「两条臂用了不同的预算」在字节上看着完全正常。缺字段/缺档一律回到已登记默认值，**绝不当 0**（`test_delta_budget_helpers_refuse_bools_and_never_fall_back_to_zero`）：0 会让每一块增量都装不下任何一张新卡，而那在字节上表现为「这条臂什么都没发」，不是一次响亮的失败。它是**上限不是配额**：这一轮没有新证据时块里就没有卡片那一节，没用满的卡位不换任何东西。同一个数还兼作**补充卡**（同 key 摘录升级）单轮的检测个数上限——全量比对是每轮 O(池) 次摘录重算，正是这条臂要省的开销
REASONING_REFLECT_COMPACTION_TARGET_RATIO # `prefix_delta` 建/重建证据卡快照 K 时，可压缩材料占对应池子的**目标**比例（默认 `0.5`；0.25–0.75，`true` 这类布尔值显式拒绝而不是折算成 1.0）。**目标不是硬界**：必要材料可以在硬预算之内高于它（那一步由证据卡选取自己的预算判断守着，`test_delta_keeps_the_hard_budget_when_the_target_ratio_cannot_be_met`），它的作用只有一个——给后续增量留出空间，免得首轮把池子填满、第二轮立刻重建（前缀复用最终设计 §5.2 首句）。**首版 K 与每一次重建走同一份代码、同一个比例**，所以「第一版按目标比例建、后面每一版也按目标比例建」不靠两处措辞一致。区间之外两侧各有一种坏法：低于 0.25 把首轮能给模型看的证据压到没意义，高于 0.75 留不出重建余量。同样只在 `prefix_delta` 下被消费、`config.py` 自己不读，唯一读点是 `_reflect_v2_context`（经专用的 `_delta_target_ratio`，同上那条 AST 判据）。它**不是丢弃用户内容的比例**：用户完整问题、冻结约束、本轮执行限制各按自己的边界保留，收缩只发生在证据卡与可压缩的历史观察这两个池子上。`prefix_snapshot` / `off` 的证据选取**不使用**这条收缩规则（设计 §5.2 明写；两条臂与接入前逐字节相同，`test_delta_wiring_leaves_the_other_two_arms_byte_identical`）
REASONING_REFLECT_OPTIMIZATION # reflect 上下文的**前缀复用策略**（默认 `off`；五取值 `off / prefix_snapshot / prefix_delta / prefix_delta_lean / prefix_delta_evidence`，**五格全部已实现**；证据展示实验档须显式选择）。`off` 是实施基线的 v2 原行为，逐字节不变。**`prefix_snapshot` 做的事**：把一轮 reflect 的消息按**稳定性**重排，让上游模型服务有机会复用不变的前缀。system 段变成本 run 逐字节不变的一份——协议、静态工具目录（这次 run **可能**执行的工具的稳定超集，本 run 第一次 reflect 生成一次）、以及「目录只是参数参考，本轮可调用集合在 user 段末尾」这条规则；user 段按「问题与已确认的必答契约 → 证据卡 → 动作观察账 → 本轮当前状态」排，最后那一块（本轮可执行动作、不可用清单与原因、方面当前状态、已完整枚举的集合键、服务器状态摘要）每轮重写。**真正拿到的稳定性有多少**：逐轮逐字节不变的是 wrapper、system 段与「问题与契约」那一段，**仅此三样**。它们上面/下面那几块是**按稳定性排的，不是被冻结的**：证据卡每轮重新选取，会随绑定与新增结果重排；观察账是**有界近期窗**（`REASONING_REFLECT_RECENT_OBSERVATIONS`），行掉出窗口之后它的第一行会变成「（更早的 N 条未列出）」并随 N 继续变。量到的公共前缀停在证据卡或观察账的开头，是这条臂**预期的形态**，不是它坏了。**它只改块的位置，不改任何一块的内容判据**：证据选取、观察账两道界、三项预算、动作可用性、解析白名单、失败与终态合同、轨迹键集全部逐字不变，模型看到的事实与 `off` 逐项相同（同一批证据键、同一份可用动作与额度数字）。**但「只改位置」不等于字节中性**：缓存**未命中**时按相对 `off` **每轮 +1–3KB 输入**计。目录恒为超集，所以 `off` 的 system 段会随额度收缩而变短、这条臂不会——full-house fixture 实测早轮 `off` 9,466 字符 vs 本臂 10,606（+1,140），额度耗尽的晚轮 7,883 vs 10,851（+2,968）。CPU 不构成因素（静态 system 段每轮重渲染约 11µs）。代价按设计接受并如实说明：目录一个 run 只定型一次，所以额度用光或条件变化之后那个工具仍写在目录里，模型硬选一次时按既有的 `unavailable_action:<原因>` 记一条零 I/O 的观察、循环继续。**这些字节数要另开一格才量得到**：尺子是下一条的 `REASONING_REFLECT_MEASURE_CONTEXT`，它与本项**正交**、默认关，两条臂共用同一次装配因此共用同一把尺子——本项自己一个数都不写（唯一例外是 `prefix_delta` 那三个**行为事实**键 `context_rebuilds` / `context_fallback` / `delta_blocks`，它们与要不要量字节无关，测量关着时也写，见下一条的已登记偏离）。`prefix_delta_lean` **现在也已实现**（PR-4）：消息形状与 `prefix_delta` 相同，差别只在自评合同（完整说明见下面「**`prefix_delta_lean` 做的事**」那一段）。闭集之外的任何拼写在启动期由 `Literal` 本身响亮拒绝（`Input should be 'off', 'prefix_snapshot', 'prefix_delta', 'prefix_delta_lean' or 'prefix_delta_evidence'`），不会静默折回 `off`；至于压根不过这个校验器的 duck-typed settings 适配器，运行期对已实现闭集之外的任何取值一律折回 `off`。**`prefix_delta` 做的事**（PR-3 放开的第三格）：消息形状与 `prefix_snapshot` **完全相同**（`system(S)` + 一条 `user(C+K+D+T)`，两条臂逐块同形，`test_delta_layout_takes_the_same_prefix_message_shape`），差别只在 K/D 两块的内容判据——K 那一块从「每轮从整池重选重排」换成「一份阶段快照 + 只追加的增量块」。① **K/D 分工**：K 是当前阶段的起始快照（按上面那个目标比例建），D 是**一轮一块**、块内三节（本轮新展开的卡 → 本轮动作观察 → 上一轮服务端已接受的方面更新；缺哪节不出哪节）。上文已经发出的每一块 D 逐字节不变、只追加不改写（`test_delta_only_appends_and_never_rewrites_an_earlier_block`）。② **卡片冻结**：一张证据卡第一次真的发出去时它的字节被冻结，此后重建与增量都从那张表取，不按本轮检索词重算摘录（`test_evidence_block_takes_the_frozen_bytes_instead_of_re_rendering`、`test_delta_rebuilt_snapshot_reuses_frozen_bytes_when_a_card_returns`）。③ **补充卡不就地改写**：同一条证据以后算出更好的摘录时**追加**一张 `key=` 逐字相同、标着版本的新卡，旧卡仍然有效、绑定用同一个键（`test_supplement_card_repeats_the_pool_key_verbatim`）；同一段摘录不追加第二张（`test_supplement_is_not_appended_for_an_excerpt_already_sent`），检测范围是「本轮已绑定的键 ∩ 已冻结的键」，从**上一轮服务的最后一个键**在本轮候选序里的位置往后接，取至多 `REASONING_REFLECT_DELTA_CARDS_BY_EFFORT[档位]` 个（续接点存键不存位置：候选序每轮都可能移位，位置游标在那种形态下会把队尾的键饿死，`test_delta_supplement_rotation_starves_no_candidate_when_the_order_moves`）。④ **两个池子按合计计**：证据池（上面那份按档位的字符预算）与历史池（`REASONING_REFLECT_STATE_CHARS`）在这条臂上计 **K + 全部 D 的总和**，不是每轮各拿一份满额；判断在追加**之前**做（`test_delta_keeps_the_cumulative_evidence_budget_across_blocks`）。那道判断算的是**完整的待追加块**，口径与落账完全一致——块头 + 卡片节计证据池、观察行 + 「已接受的方面更新」那一句计历史池——所以只带观察行的那些轮同样被判，而不是无条件把块头追加进去（`test_delta_counts_the_block_head_of_an_observation_only_pending_block`、`test_delta_counts_the_pending_aspect_notes_against_the_history_pool`）。**刻意留一条越界例外**：没有候选新卡、而块头或那句 note 在重建之后仍装不下时那一块照发——观察行是本轮那次动作在上下文里的唯一记录——记账如实记成超出，于是下一轮必然重建。⑤ **重建触发点是「一张新卡都装不下」或「完整的待追加块顶破某个池」**，不是「本轮某张卡塞不进剩余额度」：后者每有一张超额的大卡就压一次 K，而那张卡有省略披露、下一次重建按同一档序回得来。重建还带**迟滞**——上一轮刚压紧过一版、这一轮又连一张新卡都装不下，说明这个预算下压缩已经无效，于是不做第二次重建、直接回退（`test_delta_rebuild_has_hysteresis_instead_of_compacting_every_turn`）。重建全程零 LLM、零查库、零额外配额（`test_delta_adds_no_retrieval_or_model_calls`）。⑥ **回退不可逆，且只影响该 run**：重建之后仍装不下最小有效新证据时，本 run 剩余轮退回 `prefix_snapshot` 的有界证据选择，`context_fallback` 在投影上如实出现；回退不清空候选池、不清空已发出的 D、不清空「曾真实展示」的绑定资格、不重置任何配额，也不多一次模型调用或轨迹步（`test_delta_falls_back_irreversibly_and_keeps_every_result`、`test_delta_fallback_keeps_every_block_it_already_sent`、`test_delta_fallback_registers_no_key_from_the_snapshot_it_threw_away`）。统计上它必须如实出现，**不能**混算成「整条 run 始终在用 D」的结果（设计 §13）。**字节账（如实，且一个数都还没实测）**：S 相对 `prefix_snapshot` 多 **1,507 字符**（那四句解释 D 怎么读的规则），而它落在逐轮逐字节不变的那一段里——上游真复用前缀时这笔字节只在第一轮真的付一次，未命中时每轮都付；每一块 D 各带一个 **75 字符的块头**加一个换行（重建之后的块再多约 14 字符的版本标记），块间还有一个空行；一张补充卡相对原卡多约 26 字符。**省下来的是每轮一次的全池重选重排与摘录重算**（K 的字节直接从投影取，不重新渲染），以及由此换来的更长公共前缀。这两侧哪一侧更大**取决于题型与轮数**，本条臂的实际收益**一个数都还没量到**：上面全是结构口径，不是实测结论，真实收益等 A/B 对照实验（rig 的第二维臂，见 `scripts/README.md`）。⚠ **回退之后的每一轮 K 按「档位证据池 − 保留 D」构造**：那一轮起 K 由 `prefix_snapshot` 的有界选择每轮重选，而上文里保留的那几块 D 原样跟着走（抽掉模型已经读过的上文比多付几千字符更糟）——所以给 K 的额度先减去保留 D 已经占掉的**证据半**、近期观察窗的额度先减去它占掉的**历史半**，并且保留 D 里**已经可见的那些键**被排除在 K 的候选之外——**全部三档**，来源多样性那一档也在内（那一档的键序是在选取内部从池子里算出来的，只滤调用方传进去的那两份键序对它一点作用都没有）。于是一条消息的证据合计仍然 ≤ 档位证据池，同一条证据也不会以两份都不带版本标记的写法出现两遍（`test_delta_keeps_the_cumulative_evidence_budget_across_blocks` 的回退轮上界、`test_delta_fallback_turn_keeps_its_own_cards_out_of_the_kept_blocks`）。代价如实登记：保留 D 把池子吃得多时，回退之后的 K 可以是空的——那时模型手里那几块 D 正是它已经读过的那批证据。那两笔累计账因此**不是停用，而是收缩成保留 D 那一半**（回退时各减掉那一版 K 的对应半），它们此后唯一的读者就是上面那两条减法。**`prefix_delta_lean` 做的事**（PR-4 放开的第四格，下称 L）：**它不是第五种布局，是同一套布局上的另一份自评合同**。消息形状、K/D 的内容判据、五个块的字符口径、重建与回退规则与 `prefix_delta`（下称 D）**逐项相同**——同一次装配、同一份布局分派（模块级 `_DELTA_LAYOUTS`，`test_delta_layout_takes_the_same_prefix_message_shape` 逐臂涵盖这一格），差别**只有自评合同这三条**：① **常规轮只报变化**——模型只需在本轮的 `assessment` 里给出有变化的方面，服务端记着此前每一轮的判读，已标 supported 的项不必重述，省略不花代价、也不会被追问（S 里那一段自评合同换成 lean 版，是**替换不是追加**：两句矛盾的合同同时在场是最坏形态，`test_reflect_v2_static_prompt_lean_replaces_not_appends_the_assessment_paragraph`；T 末尾那一句同理二选一，`test_the_lean_status_block_differs_only_in_the_closing_note`）。② **收尾不为记账退回一轮**——D 下收尾载荷一个方面都没落账，整轮会被折成一条零 I/O 的伪动作、扣一步、下一轮逐个 id 追问；L 下这一轮**不发**，收尾照原样被接受（`AspectLedger.note_missing_assessment` 在 L 上最先返回 False，追问额度与 `assessment_omitted` 两格从头到尾都是初值，`test_a_lean_run_accepts_a_silent_closing_turn_without_a_follow_up`；配对断言是「同一剧本下 L 比 D 少恰好一次真实模型调用」，`test_a_lean_silent_closing_turn_costs_no_extra_model_call`）。③ **未评估不冒充「资料里没有」**——L 下省略成了常态，所以合成侧那份服务端事实块多**一行披露**：有几个方面没被逐项核验、是哪几个，再带上模型自己的结束判断，末尾钉一句这不是「笔记本里没有这份材料」（口径与边界见 `docs/product-and-api*.md` 里方面与终态那一段，`test_a_lean_termination_discloses_what_was_never_assessed`）。**动作与自评逐方面独立校验是公共基线**（PR-1 的 T-BF7，四格共用，不是这一格带来的）：某个方面不合规只作废那一个方面、本轮检索动作照常执行；整份读不出归属才作废整轮，这一条在 L 下一字未改（`test_a_malformed_assessment_still_folds_the_whole_turn_under_lean`）。**L 只改模型要提交什么，不改服务端展示什么**：方面状态块仍逐行列出**全部**方面（刻意不收窄成「只列未落定 + 计数」——L 下这一块是方面账唯一的完整落点），证据选取、动作可用性、七个终态 reason 的闭集、轨迹键集一格未动（`test_a_lean_turn_that_reports_one_aspect_leaves_the_others_verbatim`）。**开关在建账那一刻冻结一次**、中途翻策略位改不动这次 run 的追问合同（`AspectLedger.lean_assessment` 是只读的，`test_the_lean_switch_is_frozen_on_every_ledger_source`）；**上下文预算回退之后仍是 L**（回退只换 K/D 的构造方式，自评合同不受影响，设计 §5.2、`test_a_lean_run_that_falls_back_keeps_the_lean_contract`）；`off` / `prefix_snapshot` / `prefix_delta` 三臂逐字节回到 PR-3 合入态（`test_the_lean_wiring_leaves_the_other_three_arms_untouched`）。**字节账（如实）**：S 里那一段自评合同换成 lean 版之后比 D **多 1,246 字符**（实测，与 `prompts.py` 的 `_V2_LEAN_ASSESSMENT_INSTRUCTION` 那格注释同源、改文本要同 diff 更新那个数——多出来的字节是把非法键剔除、同一方面重复出现、组内行数上限这些**旧段本来就有**的规则按现行校验器重新写对，不是新增语义），T 末尾那一句多 **3 个字符**；这两个数都落在 S/T 里，**不是布局差**，读 `ctx_chars_s` 对照表时据此排除「L 更贵是因为多发了内容」的误读。**省下来的在另一侧，而且一个数都还没量**：每轮少的是**模型自己重述已支撑项的那些行**（那在模型的输出侧，不在服务端发出的上下文里），而**省的主要是一次收尾轮**（收尾缺自评不再退回追问；生产实测一轮 reflect 约 40s）。哪一侧更大取决于题型与轮数，**这条臂的实际收益一个数都还没量到**：上面全是结构口径，不是实测结论，真实收益等 D↔L 的配对 A/B（rig 的第二维，见 `scripts/README.md` 的 `ab` 小节；**D↔L 是比较自评合同的配对臂，两者之间量出的差异不得归因到前缀缓存本身**——那笔账已经在 D↔P 那一对上算过了）。它**叠在** `REASONING_REFLECT_V2_ENABLED` 之上：v2 总闸关着时反思走 legacy 协议，根本没有「前缀」可谈，这一项被忽略；Knowhow 智能补全（显式 legacy）同理忽略它。它**不是**前端检索档位，也不由档位推导。判据的全仓唯一读点是 `reasoning_retrieval.reflect_optimization()`。PR-5 新增 E1/E2/E3 三条实验通道(`scripts/reflect_shadow_rig.py` 的 `prefix-probe`/`state-probe`/`ab`)来测量这个开关的四格,但**不带任何采用决定**——四格的**实际收益仍未量**,PR-5 只交付实验通道与 dry-run/manifest(见 `scripts/README.md`),真实批次由操作者跑。本期为此**不新增配置项**:E1 在 `app/core/llm.py` 上的接缝是一个模块级 `ContextVar`,不是 `Settings` 字段;它的整批墙钟预算与随机种子是给 rig 的 CLI 参数,不是配置——不用去找一个不存在的 `E1_ENABLED`
# `prefix_delta_evidence`：显式启用的 V2 实验档，保留证据卡与 `prefix_delta` 的 S + C + K + D + T 布局、冻结卡片、只追加的动作观察、共用预算、重建及回退机制。关闭逐项自评指令与输出 schema、自评吸收及追问、动态方面状态，以及合成阶段的自评缺口注入。完整问题、必答主题、用户约束、当前合法动作及额度继续保留。上面的证据、历史及 delta 预算设置也适用于本档，不提高总预算、不增加模型调用；表格单卡额度由 `REASONING_REFLECT_TABLE_EXCERPT_CHARS` 控制。原有各档及默认 `off` 保持原行为。使用 `REASONING_REFLECT_V2_ENABLED=true` 配合 `REASONING_REFLECT_OPTIMIZATION=prefix_delta_evidence`；关闭 V2 时实际 optimization 仍为 `off`。前缀稳定只表示为服务端复用提供机会，不代表已命中 cache 或已有加速结论。显式配对实验用法见 `scripts/README.md`。
REASONING_REFLECT_MEASURE_CONTEXT # reflect 上下文的**测量**开关（默认 false），与 `REASONING_REFLECT_OPTIMIZATION` **正交**——正是它让 `off` 臂也能用同一把尺子量，对照实验才可比。**打开它做的事**：每一轮 reflect 额外算一份**纯内存**的观测，写进那一轮 reflect 轨迹步的 detail——五个上下文块各自的字符数（system 段 / 问题与契约 / 证据卡 / 动作观察账 / 本轮当前状态；五者之和恰等于两条消息正文的字符数之和）、这一轮 provider-facing 消息的**字节**总数（含服务端 wrapper 与序列化帧开销，与下一项同口径，所以那道除法两边同为字节）、这一轮消息与**上一轮**消息的公共前缀字节数（首轮如实缺值，不是 0）、本轮展示与省略的证据卡数，以及这一轮模型调用的墙钟、真实请求数与响应正文字符数（同一轮内两次调用累加；客户端不报就缺键，不写 0）。**PR-4 起还多两格自评读数，门同样是本开关、而且四格通写**（不是 `prefix_delta_lean` 专属：`off` / `prefix_snapshot` / `prefix_delta` 下模型仍每轮重述全量，这两格同样有意义，四臂共用一把尺子才比得动）——`assessment_rows` 这一轮**实际落进方面账**的自评行数（不是载荷里写了几行：逐方面被拒的行不算；整份形状越界被折时记 0），`assessment_absent` 这一份载荷压根没带 `assessment`。⚠ **两键只在「模型自己那份载荷」的那一轮写**：provider 故障的 fail-open 兜底轮，以及服务端把决定折成伪动作的那些轮（首次抖动降级、解析失败、参数越界），它们的 `assessment` 同样恒为 None，但那不是「模型省了自评」而是这一轮压根没有模型的自评意图可言，所以**两键都不写**，那一轮在读侧如实读作「未观测」。因此顶层的 `assessment_rows_total` 是 **sum-over-present**（不是「任一步缺 ⇒ 整列 unknown」），「全不全」由伴生列 `assessment_observed` 单独披露（每一条 reflect 步都带 ⇒ 真；只带了一部分 ⇒ 假，此时那个和是**真实**的部分和；一条都没带 ⇒ 缺）——一次偶发 provider 抖动不该把整条 run 的这一列读成 unknown。请求**文本本身只在内存里过一遍**：不进 LLM 调用日志、不进轨迹、不进任何投影。它不改任何 prompt、不改任何决定：测量**永不改变发出去的消息与模型的结论**——成功时如此，失败时同样如此。它关在自己的守卫里，一次算不出来的测量只表现为那一轮**那几个键缺席**（读作 unknown，绝不是 0），不会因此少发一次请求、也不会凭空多出一轮兜底。把测量整个关掉，reflect 步 detail 同样逐字节回到接入前。默认关是因为它要序列化一份本轮消息并留住上一轮那串字节——**一轮消息的量级，峰值两轮**：上一轮那串要留到本轮那串在记账时晋升为止，中间横跨整次模型调用，两串并存；关着时不构造缓存、不序列化。⚠ **两条 delta 臂（`prefix_delta` 与 `prefix_delta_lean`）是这条口径唯一的已登记偏离**：它们**无条件构造**那个载体对象，因为「重建了几次 K」「有没有不可逆地回退」「这一条消息里有几块 D」是**行为事实**，与要不要量字节无关——一个测量关着的生产 run 照样可能整段退回有界证据选择，而那件事不出现在任何一个字节数里。本开关此后管住的是**另外那半**：它为假时**一个字节都不序列化**（`serialize_provider_messages` 零调用），五块字符数、消息字节总数、公共前缀与那两个卡数如实**缺席**（读作 unknown，不是 0），detail 上只剩上面那三格行为事实——`test_delta_with_measurement_off_writes_only_the_behaviour_keys` 钉住这件事，上面那两格自评读数同样在这一半里、测量关着时如实缺席（`test_the_two_assessment_keys_are_absent_when_measurement_is_off`）。`off` / `prefix_snapshot` 两条臂在这里的判据一个字没动。另有**一格与本开关无关**的登记偏离（拍板 Q10，本期唯一允许偏离三臂字节等价的字段）：终态那一步 detail 上的 `aspects_unassessed`（模型一次都没判断过的方面数）由 **v2 四臂无条件写**，既不看本开关、也不按臂分叉——只在 L 写的话 D↔L 的配对表上这一列恒缺一半，而「L 少核验了几个方面」正是它要回答的问题；它落在终态 detail 上，三臂的 prompt 字节一个都没动。同样叠在 `REASONING_REFLECT_V2_ENABLED` 之上，v2 关闭与 Knowhow 恒不测量。**PR-5 的 E2/E3 两条实验通道要求参与对照的每一条臂都开着它**(rig 逐臂回读,不符整批停,不是逐 run 单独判)——只给一侧开会让另一侧的这几列永久 unknown,而每一行看起来都很正常。**E1(`prefix-probe`)不经这条 reflect 测量层**:它结构上剥掉了整条 reflect 循环(零数据库、零检索、零 reflect,只调 `reasoning_agent` workload 本身),既不读也不回读这个开关,这个开关对 E1 的批次没有任何效果
...
POSTGRES_LEXICAL_KNN_ENABLED # PostgreSQL KG 名词法探针的自适应 GiST `<->` KNN 早停（默认 true，设 false 回滚；SQLite 零成本短路）。符合规模且未按来源收窄时，只有短、非 CJK 词项进入形状匹配的全局 GiST KNN；CJK 与长词项绕开 ordered KNN，把 notebook-scoped `%` 与 literal-ILIKE 拆成两个各自有界的 arm 后做精确 union/top-k；KNN 短页也走同一拆分回退。PostgreSQL 可为这些 arm 选择复合 GIN 或其他可用 bitmap 组合，应用不强制物理索引。不删词项，不改候选预算、分数或最终排序。等相似度 KNN 并列成员仍保留已登记的 GiST 遍历抖动；要求位稳定集合的部署可设 false。索引 DDL、生产证据与上线/回滚见运维文档。
POSTGRES_LEXICAL_KNN_MIN_ROWS # 短且合格词项可使用 KNN 的库规模下限（nodes+chunks，默认 500000）。全局 GiST 没有 notebook 键，请设在「除主导库外最大的库」之上；低于下限时全部词项走结果等价的 notebook-scoped 拆分路径。
POSTGRES_LEXICAL_KNN_MAX_TERM_CHARS # 非 CJK 词项进入 GiST KNN 的最大长度（默认 32；范围 3..256）。更长词项绕开 ordered KNN、走拆分路径；只改变访问路径，不改变召回词、候选额度、分数或排序。
