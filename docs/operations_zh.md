...
5. **按链序逐包导入，一次一个。** `sync import` 只在增量包的 `base_package_id` 点名目标端
   ...
   `sync status` 会打印每个 `source_env` 当前的链头，据此判断下一个该导入哪个包。
...
7. 增量包的引入不像全量包那样整目录交换——它按文件逐个合并（先写 `<file>.sync-tmp`，再
   ...
   的，已经替换过的文件重放一次只是原样再覆盖一次）。一次被杀掉的导入（复制和改名之间崩溃）
   留下的 `.sync-tmp` 是无害的残留，不需要手动清理：下一次导入同一个笔记本/附件目录时，
   合并之前会先清掉找到的 `*.sync-tmp`；`sync export` 自己收集文件的两条路径也都会跳过以
   `.sync-tmp` 结尾的文件，所以一次被打断的导入留下的残片不会跟着某个后续的包传到别的环境
   去。
8. 若增量包的 `deleted_notebooks` 点名了一本已同步的镜像笔记本，具体怎么处理取决于这本
   笔记本此刻在**这个目标端**的状态——在删除相位一开始的一次读里就定下来：目标端正在对它
   做深拷贝（`status='copying'`）的，本次**整本原样不动**——行和 `status` 都不碰，同时计入
   `notebooks_delete_skipped` 与 `deletes_skipped_for_copying`——因为那份拷贝这一刻正在读
   这些行，这时候删会把拷贝搞坏；等拷贝结束或放弃之后再删这本笔记本（手动删，或者等它不再
   `copying` 之后被下一个增量包的重放接住）。目标端确实持有（不在拷贝中）的笔记本则会被
   打上墓碑——翻成 `status='deleting'` 并交给和 `DELETE /api/notebooks/{id}` 同一条删除作业
   队列（见下面「笔记本删除作业」）——它的行级删除条目**不会**逐条重放，而是计入
   `deletes_folded_into_notebook_deletion`：删除作业马上要清空这本笔记本的每一张表，先在这里
   逐条重放几千个键只是白做的工，随后又被作业撤销。**`deletes_folded_into_notebook_deletion`
   非零、而这本笔记本的行在导入返回之后仍然还在，是这套设计本该有的样子，不是 bug**——那些行
   归删除作业清，不归这次导入清；导入本身只负责把作业排进队列。真正的清理见上面
   `sync status` 的「等待删除作业的镜像」计数，以及下面「笔记本删除作业」一节，包括为什么
   需要目标端应用自己的删除作业消费者在跑它才会发生。
...
没有任何更晚的 `done` 包越过它的那个包的 `package_id`/`to_seq`/`created_at`（设计文档 §8）。
下一次增量导入的
