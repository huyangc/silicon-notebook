...
没有任何更晚的 `done` 包越过它的每一个包的 `package_id`/`to_seq`/`created_at`（设计文档
§8）。`--json` 里的 `chain_heads[source_env]` 永远是一个列表，哪怕只有一条（最常见的形态）
也是列表。**它可能真的不止一条**：一个全量基线和一个记录在附近的 `--notebook` 限定导出包
（或者门关全量包）可以打平在同一个水位上，谁都没越过谁——这两种形态都不推进源端自己的
水位，链的规则从不把其中一个当成另一个的延续。这种情况下，人读摘要会把每个候选都打出来，
并附一句「链头不唯一；源端下一个窗口的 base 会是其中推进过水位的那个（to_seq 最大且 base
非空或 to_seq>0）」——照字面读：几个打平的候选里，源端下一个窗口真正会续接的那个，是其中
真的算「链上一环」的那个（它自己的 `base_package_id` 非空，或者它的 `to_seq` 确实反映了一次
真正的日志读取），不是那个恰好落在同一个数字上的限定导出/门关全量包。拿符合这个描述的那一条
当「链头」判断下一个该导入哪个包；只有一条候选的常见情形不需要这样读。它同时打印有多少个
镜像笔记本**在等删除作业**——
