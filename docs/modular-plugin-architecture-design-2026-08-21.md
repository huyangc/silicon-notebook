...
本文不是逐任务实现计划，也不改变当前产品行为。文中的"必须"用于保护现有安全、数据和行为契约；"建议"给出优选形态；其余实现细节留给具体功能的 developer 在设计和评审中决定。**实施任何一个阶段时，若触碰开发约束，须按仓库文档同步红线同批更新 `README.md`/`README_zh.md`/`AGENTS.md` 与 `docs/` 权威文档；精确数值上限只登记在 `docs/product-and-api*.md`。`CLAUDE.md` 刻意不在这个集合里（判据与体量门见 `AGENTS.md`「Documentation Sync」）——本文描述的扩展点边界是跨改动的架构约束、已由它的「扩展点边界」一条覆盖，逐阶段的实现细节不要再往那里追加。**
...
15. **（2026-08-23 补记）五个零消费者扩展点已由 #569 整体移除**：`answer.audit`（§6.4）、`report.audit`（§6.5）、`source.element_enricher`、`knowledge.candidate_projector`（均 §6.2）与 `agent.tool_provider`（§6.6，含其四条前置红线）在仓库里从未有真实消费者，按 §17「不为以后可能有用预建没有真实消费者的 hook」的原则被整体退役；§6.2/§6.4/§6.5/§6.6 里描述它们的段落，以及 §19「遗留待后续单独决策」中「`agent.tool_provider` 正式开放时 `PUBLIC_TOOLS` 组合派生的具体实现与守卫改造」一条，均随之失效，只作原始设计记录保留，不代表当前代码状态。下一阶段方向不是重新引入这五点，而是落地**可信、同进程、仓库外（部署装入）的第三档插件**（已拍板 2026-08-23，起因是内网私有特性——如依赖会员资格的 IEEE 检索——必须零补丁接入公网基座）。这与本文 §2.4 的两档信任表、§9.1「不从 entry point / 环境变量自动启用」中关于装载来源的假设、§14 Phase 6「外部插件＝进程隔离 RPC」的前提**冲突，以本条为准**：第三档仍是启动冻结、显式配置点名装载（不扫描、不热更新），但不再要求插件随公网版本一起构建；进程隔离那一档继续推迟。仓库的权威约束（AGENTS.md / README 对）随实现 PR 同步更新，具体接入步骤以后续的扩展接入 SOP 文档为准。
