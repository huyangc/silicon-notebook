...
本文不是逐任务实现计划，也不改变当前产品行为。文中的"必须"用于保护现有安全、数据和行为契约；"建议"给出优选形态；其余实现细节留给具体功能的 developer 在设计和评审中决定。**实施任何一个阶段时，只更新负责该主题的权威文档并保持中英文对同步；入口信息、Agent 工作流或 Claude Code 常驻规则确有变化时，才分别更新 README、`AGENTS.md` 或 `CLAUDE.md`。精确数值上限只登记在 `docs/product-and-api*.md`。**
