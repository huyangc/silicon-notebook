...
- **知识库深拷贝**：PR-1 阶段拷贝**不带** knowhow 内容（隐藏源及其元素/chunk/KO/边/向量一并排除，干净的「静默缺席」，避免产生无表可管的孤儿投影）；完整表拷贝（全表 id 重映射）随 PR-2 交付。只读共享不受影响（活视图，读者可见表格与图片）。
- **KG 重建护栏**：「刷新图谱」的全量清除与抽取目标遴选均排除 knowhow 隐藏源——投影产物只由确定性投影管理，绝不进 LLM 抽取管线。
