...
- **P2-16**(T4 codex 第 11 轮评审):**裁决 P2-7 缺了另一半——撤回够不着。** P2-7 特意让
  `DELETE /notebooks/{id}/share-requests/{rid}` 只认「这条申请是你提的」,理由是 P2-6 让批准会
  拒绝已失权的申请人,两边都要管理权这类申请就**既批不了也撤不掉**。但唯一能列出申请、给出
  申请 id 的 `GET /notebooks/{id}/share-requests` 挂着 `notebook:manage`,前端那整块 UI 又在
  `readOnlyWorkspace` 后面——申请人一失权就再也拿不到 id,**那个口子在它唯一存在意义的场景里
  等于没开**。这是「全栈对等/不做半个特性」红线的形态。

  补 `GET /me/share-requests`:**路径在 `/me` 下而不是 notebook 维度**——授权轴是申请归属,
  挂成 notebook 再放宽守卫会让「谁能列这本库的申请」出现第二套口径。唯一谓词 `requested_by`
  (与 DELETE 逐字相同),只回 `pending`(正向精确匹配;已决定的撤不回来,列出来只是白扩披露
  面)。既有的笔记本维度那条**一个字不动**。前端入口放在**群组面板顶层**而不是笔记本工作区:
  申请人可能连读权都一并失去、根本打不开工作区。

  **披露面是逐字段选定的**(判据:他原本知不知道)。带 `notebook_id` / `notebook_name` ——
  他提交时对那本库有管理权,库名早就知道;`notebooks.created_by` 只在建库与深拷贝时写入
  (无转让功能,`NotebookUpdate` 里也没有这一列、有 P2-15 的护栏钉着),不存在「拿旧 id 探测
  新主人」这条通道;不带库名则多条待审批时分不清撤哪条。带 `group_id`/`group_name`(他自己
  选的、提交时是其成员)。`status` 恒 `pending` ⇒ `decided_by`/`decided_at` 恒 null,**审批者
  身份不经这条路泄露**(已决定的压根不在结果里)。库的任何当前状态一律不带。

- **P2-17**(T4 codex 第 11 轮评审):**「改动失败」与「对账失败」必须分开报。** 批准/驳回
  与随后的两三个列表请求同在一个 `run` 里,刷新任一失败就报「批准申请失败」、那条已批准的行
  还留在屏幕上,用户照提示重试 → 必然 404(它已经不是 pending)。改成「先做改动、再去对账」
  两段:改动失败才报错误并留住行让人重试;改动成功后**先把该行从本地清单摘掉**(否则对账
  失败时它还挂在那儿诱人再点),对账失败只给中性说明「已批准,但列表没能刷新,请手动刷新」。
  ⚠ **不是把刷新错误吞掉**——与 P2-13 的「重取被拒 → 清空清单 + 中性说明」同口径。

- **P2-18**(T4 codex 第 12 轮评审):**铃铛只算了数、没接失效。** `pending_actions` 的 SSE
  只在初始快照与显式发布时推,而共享申请的建立/批准/驳回/撤回、以及**组管理员角色变更**
  都没发布——开着页面的组管理员待审批数会一直停在旧值。受影响的是**该组的全部管理员**
  (投影判据是「我是这个组的 admin」+「申请仍 pending」),不是只有动手的那一个:A 批准之后
  B 的队列也少了一条;但也不全量广播,别的组这一刻什么都没变。角色变更只推**被改的那个人**。
  三条硬性:**在写事务之外**发、**fail-open**(通知发不出去绝不能让已提交的改动报失败——那
  正是 P2-17 刚修的混淆)、**一次改动只解析一次管理员名单**(一条 `list_members`)。撤回那处
  需要先读出 `group_id`(行删掉就没了),刻意**不改** `delete_share_request` 的返回契约——它的
  `"deleted"`/`"not_found"` 被十来处断言钉着,为一次尽力而为的通知去动它不划算;那是一次有界
  单人查询,不是 N+1,且申请的 `group_id` 不可变,读在写前的 TOCTOU 无害。
  **已登记未做**:删组会一并带走它的待审批申请,同属这一类,但需要在删除**之前**解析管理员
  名单(给一条破坏性路径加一次读),本轮未做。

- **P2-19**(T4 codex 第 12 轮评审):**`/me/share-requests` 的披露面漏了时间维度。** P2-16 逐
  字段论证了「他本来知不知道」,结论对,但他知道的是**提交那一刻**的标签。这条清单是他失权
  之后仍然够得着的(P2-7 就是这么设计的),于是无条件带出当前名字 = 一条对已经无权观察的对象
  的**活的**通道:对方改名,新名字持续送到他手里。与「跨库资产 `no-store`」「取消挂载即
  404」「公开报告页每次实时复核创建者读权」是同一条线。

  选**「仅在当前访问权仍成立时返回当前标签」**而不是建库时快照标签:后者要加列 → 新迁移 →
  shadow manifest/夹具/verifier/文档数字全链路,为一个名字不划算。两个标签**分别**判:笔记本
  那半按**读权**、群组那半按**是否仍是成员**——一起判的话只失去一半的人两个名字都会消失,多条
  待审批就都长成「知识库 → 群组」,分不清该撤哪条。空的那半界面给中性占位 +「你已不再有权
  查看对应的知识库或群组。撤回不受影响。」撤回本身与标签可见性无关(轴是申请归属)。

  实现上**同一条查询里用条件表达式**做完、不逐行回查(N+1),且读权那半**嵌
  `access_sql.read_access_clause()`**,绝不在这里手抄一份判定(P0 唯一定义点的规矩);群组成员
  资格那半留在 `group_store` 自己写——`access_sql` 的模块 docstring 明确把组成员关系的读划在
  唯一定义点之外,它不是 notebook 授权谓词。双后端各自跑过负向用例(PG 的占位符顺序与 SQLite
  不同,必须真跑)。
