...
- **「建过」的标记是 seq 对齐，不是「板块数 > 0」。** 全部连通块都小于
  ...
  才加的，改另一边时不得让 B1 复发，五个方向（B1 回填 / 零板块短路 / 从未建过 /
  请求的 level 没建过 / 别的层的构建不得替默认层背书）必须同时有守卫。第四个方向是
  第 11 轮补上的、第五个是第 15 轮，都见下。
...
**批 1 上 codex 第 15 轮评审后的两条订正（2026-07-28）—— 都是第 11 轮那两条修复的另一半**：

- **闸的第五个方向：别的层的构建不得替默认层背书。** 第 11 轮补的是**读侧**（请求的
  level 没建过就别短路），可**写侧**照旧对所有层推进那份不分 level 的 `community_seq`。
  于是反向的洞还在：先 `rebuild_communities(..., level=1)` 把 seq 推到当前，随后一次非
  force 的 `rebuild_communities(level=0)` 在 level 0 **一行板块都没有**时仍然「seq 对齐」，
  拿别的层的账目短路返回 0 —— 而默认层那一支是**无条件**信 seq 的（零板块是合法终态，
  方向二），它信的正是这个被顶掉的数。
  改法不加列，把那句蕴含关系变成**写侧的不变式**：那份不分 level 的共享账目
  （`community_seq` + `kg_analysis_artifacts` + `kg_community_edges` /
  `kg_source_profiles`）整体收归 `DEFAULT_COMMUNITY_LEVEL` 独有 —— 只有默认层的构建写
  它、也只有默认层的构建读它。三处同时收：`set_community_seq`、预计算与
  `discard_board_dependent_...`、以及闸自己。
  seq 那一处只是评审点名的一处；另两处是同一族、后果更重：非默认层的
  `replace_communities` 是**按 level** 删/写的，默认层的板块 id 一个都没变、那两份产物
  根本没悬空，作废它们纯属**连坐**；而随后写回的产物指向的是 level-1 的板块 id，默认层
  的读侧照旧按 seq 对齐判「与当前一致」，报告会把另一层的跨板块连线画在默认层的板块上。
  闸也一并收进默认层：**非默认层没有任何账本，所以它永不短路**。退回「其它层看
  `communities_count(level) > 0`」不行 —— 共享的 seq 只证明默认层建于当前 KG 世代，
  那一层的行可能建在好几代以前，「有行」会把陈旧划分冒充成新鲜的。
  **代价（显式承认）**：非默认层每次都全量重建（只会**多**建、绝不会漏建），且不再产出
  分析报告 —— `/kg-analysis` 因此永远描述默认层，这正好与 `community_overview` 刻意不收
  level 参数（「板块列表是 level 1、跨板块边是 level 0」那种自相矛盾的报告）同向。
  `level > 0` 今天没有任何调用点，所以真实代价是 0；拿一列 schema 去换它不值。
- **一次请求作用域内，标注也必须跟着数据走。** 与上面那条「换库不复位」同类，只是 scope
  从 notebook 缩到了一次请求：切排序只重取 `/sources`，请求在飞的那段时间 `sources` 还是
  上一次的行，而排序按钮按**控件值**标 `aria-pressed` —— 旧的行当场被写上新选的顺序。
  改法是渲染一律读 `page` 自带的 `order`（分页那一半天然已经这样：页码/范围/上下页全读
