...
- 「清空」按钮（与「立即整理」同口径：`agent_profile:write`，即 owner ∪ 管理边成员，PR-2 实施时按
  P1 共享底座「重新整理」的既有口径收敛，比草案「owner」略宽）：删该库分区，二次确认。「立即整理」
  另有 10 分钟冷却：该库没有新完成的提问且距上次整理不足 10 分钟时 409，避免每按一次白付一次模型调用
  （codex 前质量评审 P2）。
