...
- T3b 登记:待确认中心铃铛的报告条目此前有**两个**问题,只有后一个留给 T4。
  ① **条目整体缺席**(已在 T3b 修):`pending_actions_projection_rows` 把报告
  那一半写在了 `if notebook_ids:` 闸内,而它的谓词只有 `created_by`、一个
  notebook id 都不消费——于是「没有任何自有库的成员」铃铛恒为 0,报告却卡在
  `intent_ready` 等他确认,是条走不通的路。双后端已把那半提到闸外(零新增查询)。
  ② **空库名**(留给 T4):`notebook_name` 由自有库映射解析,非自有库解析成空串。
  无泄露、深链可用,T4 补非自有库的库名解析即可。
...
- 产品文档:docs/product-and-api*.md 群组章节(端点、角色矩阵、数值上限);
  ...
  - `?scope=all` 与非法 `scope` 的 422 口径、系统管理员的群组运维旁路(设计文档
    §4 已登记,产品文档的角色矩阵要跟上)。
  - **深度报告的对外行为变化(T3b 转出,逐条)**:
    - `docs/product-and-api*.md:32` 两处「owner 确认 / 只读成员只能等待」的句子
      已经不准了——共享库里的成员现在可以建、确认、生成、删除**自己的**报告,
      「只能等待」只对**别人的**报告成立。两份文档同修。
    - 建报告的能力跟着**读权**走,所以经 share-token 加入的只读成员同样获得它
      (`test_plain_readonly_member_can_also_create_a_report` 钉住)。这是**有意**
      的:能力表达的是「谁能读这本库」,不是「怎么读到的」。要在文档里写明,免得
      被当成群组特性的溢出。
    - 报告**按创建者隔离**:列表/详情/导出只含自己的,notebook owner 也不例外
      (不引入「owner 看全部」的新披露);要给别人看走公开分享链接。
...
- **P2 待办登记(P1-T2 评审转出,不在 P1 修)**:
  ...
  - P3 观察(T5 真机冒烟发现,**不修**):切换用户的瞬间,浏览器里残留的上一位用户的
    active 笔记本 id 会被带进第一次请求,于是打一次 404。deny-by-default 兜住了它
    (那本库对新用户本来就不可读,404 正是正确响应),没有任何泄露,界面下一跳就落到
    新用户自己的清单上。将来要消掉这次噪声,办法是在登出时清掉 active 库的本地存储,
    而不是在读守卫上开任何口子。
