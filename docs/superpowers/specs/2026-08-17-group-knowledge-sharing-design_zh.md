...
3. **挂载(Mount)**:机制不变,只改有效性谓词——从「公共库 ∨ 同 owner」扩展为
   ...
   **读权 ⇒ 可挂载**成为一致规则——但受限读权(everyone 以外)的借入挂载带
   「未共享门」,防转手再分享,见 §6.1(实现期安全评审补入)。
...
- **「他人不可见」在实现上是行级判定,不是 notebook 级能力**(P1-T3b 落地):
  9 个报告写端点挂 `require_notebook_read` + 体内 `reports.created_by == 当前用户`,
  列表/导出按同一谓词在 SQL 里收窄,别人的报告与「不存在」同为 404。owner **也**
  只看得见自己建的——刻意不引入「owner 看全部」这条新披露。能力名 `reports:write`
  保留在能力表里但当前零消费点,留给 P2 的组管理员管理动作。
- **成员发布的公开链接是一个「受托面」,有效期 = 他的读权**(P1-T3b 实现期裁决)。
  「分享走既有公开链接」意味着成员可以把**含本库内容**的报告发到免登录页上,这是
  设计上接受的受托关系。但它必须随授权一起终止:真机复现的形态是——成员发了 token
  后被撤授权,此后成员被读守卫挡在库外、owner 被行级判定挡在报告外,**谁都够不到
  撤销**,而公开页永远 200 服务 owner 的语料。裁决**不是**收回成员的发布权(那推翻
  本表这一行),**也不是**在每个撤销点做级联(失权路径有好几条:授权边删除、退组、
  组被删、成员行移除、库改 tier——级联要在每一处记账,漏一处就留一个永久后门),
  而是把 §6.1「挂载实时判定」的同一套哲学延伸到公开页:`public_report_route` 在
  token 命中后**实时复核创建者当前是否仍对该 notebook 有读权**
  (`user_can_read_notebook(notebook_id, created_by)`,两个 id 显式传参,不依赖
  请求用户——匿名 router 绝不能碰 `current_user` 的 seeded-admin 回退),不通过则与
  无效 token 同款 404(可区分的响应会把某人的组成员身份透露给匿名调用者)。
  失权即链接全灭,恢复授权即复活(token 幂等语义不受影响);owner 自己的报告与
  全部历史报告零变化(创建者=owner,读权恒成立)。
- 系统管理员可转移 `created_by`(人员离职交接),是 owner 的唯一旁路。
- **系统管理员对群组维度同样有运维旁路**(P1-T3 实现期裁决):可读任意组详情、可管理
  任意组成员与授权边,不要求他是该组成员。与上一条同性质。两条硬理由:①「最后一名
  组管理员保护」在并发窗口下仍可能留下零管理员的组,而所有管理端点都要求组管理员
  身份——没有旁路,那个组在 API 里**永远不可恢复**,只能改数据库;②`GET /groups?scope=all`
  是给系统管理员的全局管理面,没有旁路它是一张每行都点不开的表。旁路**不覆盖自助
  退出**(「退出」的前提是本人真的在组里),也不伪造成员身份(`my_role` 仍如实为空),
  更不放宽笔记本维度的任何读写守卫。
...
**同一套哲学的第二个落点**:§4 的「成员发布的公开报告链接实时复核创建者读权」用的
就是这里的判据形状——把「还有没有权」做成每次请求的实时判定,而不是在若干个撤销
点上各记一次账。两处的取舍也一致:宁可保守(失权即关,不比对受众/不看链接已经发给
谁),恢复手段(恢复授权)在用户手上。
