...
3. **注入默认关闭**（`RETRIEVAL_EXPERIENCE_INJECT_ENABLED=false`），独立于蒸馏闸（`RETRIEVAL_EXPERIENCE_ENABLED` 默认开）——落地设计稿风险 #4「蒸馏与注入解耦，效果不好可只留蒸馏观测不注入」的缓解方案。**交付判据因此从「失败查法可见地不再重复（有 A/B 判据再细化）」改写为「蒸馏落库 + 观测事件 + 注入通路带守卫可开」**——本期不强求可验证的检索效果提升，只要求机制到位、可被后续 A/B 观测启用。**2026-09-22 已翻为默认开（`true`），见 §10.4 第 6 条**：按笔记本分区之后注入的首先是本库自己的经验，本机试跑验证过整条链路；两把闸仍然分开，「只蒸馏不注入」仍是一个环境变量就能回到的形态。
...
### 10.4 按笔记本分区的实际形态与设计稿的偏离（登记，2026-09-22）

以下偏离由 `docs/superpowers/specs/2026-09-22-retrieval-experience-per-notebook-design_zh.md`
（用户明确裁决「Q1–Q5 都按推荐来」）落地，实现按此落地，非执行走样；完整论证见该文档 §12：

1. **推翻 P2「触发器不收 notebook 参数」的明文登记**（§10.1 偏离①原文「知道是谁的 ask 的
   触发器离记录它只差一次重构」）。本次改动正是要它记录到**库**粒度（不是人粒度）：
   `note_ask_completed` 签名从零参数改为收 `notebook_id`，隔离保证改由守卫（禁键表对 job
   模块的显式白名单 + 新增判据八/九）钉住「只到库、不到人、不进 prompt」，而不再依赖签名
   不收参数这一形态本身。
2. **推翻 §12-Q3「全局库 + notebook 特征加权」**。Q3 的两条理由——per-corpus 无文献验证、
   冷启动全空——前者在产品目标明确要求 by notebook 时不再构成否决（「机制形状可抄、作用域
   切法无前人验证」本就不等于「不能做」）；后者由保留全局分区作回退解决。原设想的「按
   notebook 特征加权」（§10.1 偏离④ 登记未采集语料特征）随之不再需要，登记为撤销，而非
   延后。
3. **单库蒸馏计数仍是进程内的**，与 P2 今天的部署级计数同款接受（重启最多少蒸一轮，不影响
   正确性，只影响蒸馏节奏）。持久游标表待后续拍板。
4. **全局分区的经验 id 哈希输入保持逐字不变**（不加空 `notebook_id`/`partition` 键）：代价
   是 `experience_id` 因此有两条分支（全局分区沿用原哈希输入，笔记本分区的哈希输入追加
   `partition` 键，且键名刻意不是 `notebook_id`——隐私守卫的禁键表连字符串常量也扫，这个键
   只是哈希输入、不落库、不渲染）；收益是零重算迁移、`merge_dbs.py` 对旧库的并集语义逐字
   兼容。
5. **不做「本库条目晋升为全局」**：全局分区继续由全局链路独立蒸馏，两条链路各看各的样本
   批次，是两次独立的统计结论，不是同一结论的两份拷贝。
6. **注入闸默认值从 `false` 翻为 `true`**（2026-09-22 PR-3，推翻 §10.1 偏离③ 与
   §12-Q3 当初「先攒观测再说」的姿态）。理由是作用域变了而不是风险变小了：分区化之前，
   一张跨库全局表按构造说不出任何一句关于「这个库」的话，开闸注入的是别的库的统计结论；
   分区之后，一次 run 读到的首先是本库自己攒下的经验，全局分区只在本库条目不足时回退。
   证据是本机试跑（deepseek-flash、仓库 docs 作语料）：同一个库走 `ask/stream` 的 10 次
   reasoning 提问触发单库分区蒸馏 `runs=10, situations=1, written=2`，开闸后两次提问的
   轨迹都出现 `experience` 步且 `entries=2, notebook_entries=2`。每 reflect 轮重复注入的
   那笔 token 成本（`exhaustive` 档一次 run 约 3 万字符）没有变小，两把闸因此仍然分开：
   把 `RETRIEVAL_EXPERIENCE_INJECT_ENABLED` 设回 `false` 逐字回到「只蒸馏、不注入」。
   仍未做的是**真实 A/B 观测**（按 `adopted` 看采纳率），登记在 `fangan_todo.md`。
...
**2026-09-22 翻案**：per-corpus 硬分区已按用户明确需求（「agent memory 应当 by notebook」）改为
**按笔记本分区 + 全局分区回退**，「无文献验证」不再构成否决理由，「冷启动全空」由保留全局
分区解决。落地见 `docs/superpowers/specs/2026-09-22-retrieval-experience-per-notebook-design_zh.md`，
偏离登记见下方 §10.4。同批（PR-3）把注入闸 `RETRIEVAL_EXPERIENCE_INJECT_ENABLED` 的默认值从
`false` 翻为 `true`——分区之后注入的首先是本库自己的经验，本机试跑验证过整条链路（10 次
reasoning 提问蒸出条目，开闸后轨迹 `experience` 步 `notebook_entries=2`）；两把闸仍分开，
关掉注入闸逐字回到「只蒸馏、不注入」。见 §10.4 第 6 条。
