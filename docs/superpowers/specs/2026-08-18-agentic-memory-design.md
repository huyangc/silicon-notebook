...
| **P2**（已完成） | 层 B-经验：trace 蒸馏 IF-THEN（全局+加权）+ 任务级注入；另加三条 P1 加固（claim 代际、迟到通知成员资格复核、报告样本入巡固） | 蒸馏落库 + 观测事件 + 注入通路带守卫可开（P2 开工前裁决改写的交付判据，见下方偏离①） |
...
2. **outcome 口径降级为「失败按步、成功仅按 run」**，而非设计稿设想的「某检索步产出的证据最终被 `[k]` 锚点绑定」。理由：步→锚点归因在今天的持久化里不可恢复——`ask_trace_steps.detail` 对 `retrieve`/`ppr`/`exact_lookup`/`expand`/`enumerate` 只记计数、从不记结果 id，只有 `outline` 步（仅 `exhaustive` 档）持久化原始证据 id。真正的步级成功归因需要改热路径的 trace 写入面并配一份新脱敏论证，成本不小，刻意留给 P3/P4。
