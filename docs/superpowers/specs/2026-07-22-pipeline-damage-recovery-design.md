...
这与体检层 H1–H3 依赖的是**同一个缺口**——它们同样要回答「这个源卡住了吗」。已合并
设计为 **P1.5**（`2026-07-22-source-completion-marker-design.md`，已批准）。P1.5 的
可判定性核查修正了两点：**① 只有完成标记需要 schema 列**（`sources.chunked_at`）；
活跃租约用**内存 dict**即可（崩溃后正确答案是「没人在处理」，内存结构重启后天然为
空，恰好就是这个答案），不必持久化。**② H1 出局**——「搁浅源滞留超阈值」既不可
memo（墙钟时间函数）、也已无必要（P0 启动清算已把崩溃遗留的 `queued`/`parsing` 翻成
`failed`，由 H6 家族覆盖）。故 P2 的体检项从 H1–H8 收敛为 **H2–H8**。
