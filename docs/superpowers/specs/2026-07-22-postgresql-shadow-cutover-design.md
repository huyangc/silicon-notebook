...
观察期开始前，PostgreSQL 安装同构的 `shadow_reverse_change_log` 和 capture control，
使用 `bigserial` 顺序记录 PostgreSQL 权威写入。SQLite 正向 capture 在切换冻结窗口中
关闭，正向同步器永久停止；因此反向 apply 即使写入 SQLite 普通表也不会形成复制回环。

回滚时顺序相反：先冻结 PostgreSQL 写入，清空反向队列并校验 SQLite，再关闭
PostgreSQL capture；只有 SQLite 重新成为权威后才允许重新启用其 capture。

SQLite 在反向观察期仍保持 `write_frozen=1`。反向 applier 每批在同一个 SQLite
事务内依次执行：把 `apply_active` 设为 1、应用业务行、更新 reverse checkpoint、把
`apply_active` 恢复为 0、提交。SQLite 同一时刻只有一个写者，因此其他进程不能在该
事务中借用 lease；任何异常都会回滚业务行、checkpoint 和 lease。这样数据库级冻结仍能
阻止意外进程，同时不会挡住受控的反向复制。
...
正向和反向同步共用一个方向参数化的 `ShadowReplicator`，但一个 run 同时只能启动一个
方向。每个方向第一版都是单消费者、严格按 `seq` 处理，不做分区乱序并发。
...
- `delete`：在目标按主键幂等删除；
- 同一批次按源 `seq` 保持语句顺序；
- 目标业务变更和 `shadow_apply_checkpoint.last_seq` 在同一目标事务提交；
- 目标提交前崩溃则两者都回滚；提交后、源确认前崩溃则重放同一批，幂等操作保持结果不变；
- checkpoint 只能连续推进，禁止跳号、跳 poison event 或人工把水位改到未来。
...
- 类型转换、约束、未知表/字段、schema epoch、run identity 和规范化失败：记录 poison event，停止该方向，checkpoint 保持原值；
...
8. 通过唯一 factory 把正式 `DATABASE_URL` 切到 PostgreSQL，仍保持全局只读；
...
11. smoke 成功后开启 PostgreSQL reverse capture，再解除只读、允许 PostgreSQL 业务写入。
...
2. 记录最终 reverse 水位并清空 PostgreSQL→SQLite 队列；
3. 对 SQLite 安全副本运行完整 verifier；
4. 停止 reverse replicator 和 PostgreSQL capture；
5. 通过 factory 把正式 repository 切回 SQLite；
6. 运行只读 smoke，成功后恢复 SQLite 写入；
...
每次状态转换都验证允许的前态、数据库身份、run id、schema epoch、checkpoint、poison
event、备份与写入闸。CLI 不提供 `--force-skip-event` 或任意修改 checkpoint 的快捷参数。

需要监控的最小指标：

- source max seq、applied seq、event lag 和估算时间 lag；
- 每批行数、字节数、耗时和重试次数；
...
- 从目标 apply 到 checkpoint 提交之间逐点注入崩溃，验证回滚或幂等重放；
...
- target apply 与 checkpoint 同事务，崩溃重试不丢失、不重复最终状态；
