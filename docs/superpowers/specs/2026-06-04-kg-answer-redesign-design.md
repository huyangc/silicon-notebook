...
## 6. 待你拍板的决策（review 重点）
- **D1 推演尺度**：保守（仅当问题显式要"建议/下一步"才推演，且醒目标注）↔ 开放（总是可向前推理，仅以"无引用"区分）。影响 prompt 与产品调性。**建议：中间偏保守——默认可推演但必须显式自陈"（推断）"，且不得把推断写成像有出处。**
- **D2 关键词指标改法**：①换成对象侧召回率；②直接降权关键词、语义为主（embedder 已上线）；③加停用词过滤。**建议 ②+③**。
- **D3 top-K**：①全局统一排序 top-N（启用 type 软先验）；②逐类型但自适应 K。**建议 ①**，更贴合"按相关度给最相关的，而非凑配比"。
- **D4 问答是否切到 unified 簇**：现 `/ask` 用原始逐文档对象（跨文档重复争 top-K）。切到 unified concept 簇可去重、跨文档更稳，但 claim/formula/procedure 仍是文档级。**建议：concept 走 unified、其它仍文档级**（混合）。
- **D5 分期取舍**：是否本轮只做 Phase 0+1（后端价值主体）+ Phase 2（前端引用），把 Phase 3 多轮单独排期？**建议：先 0+1+2，多轮单独一期。**
