...
复审在守卫这条线上连着查出四轮竞态，根因都是同一个前提：**图片上传被允许比编辑器实例活得久**。
最初的做法是「上传中点离开 → 警告一次 → 再点一次强制离开」，于是上传的 continuation 会落到已卸载的组件上；
为了不丢那些已经传到服务端的图片，曾在 `localStorage` 上自造一套「待完成上传日志 + 认领」协议。
那等于在浏览器端手写一个分布式交接协议，四轮复审分别查出：首次 mount 的 effect 顺序抢跑恢复提示、
跨标签页 claim 的非原子读改写删、部分写入失败同时造成丢失与重复、键名字符串排序打乱多图顺序。

现改为**取消那个前提**——上传绝不比编辑器实例活得久：

- **上传中点离开 = 延后**：记下离开意图，等上传落进正文后由上传收尾（`finally`）执行 `commitLeave`；
  届时草稿里已含图片引用，守卫承诺的「可恢复」是真的。
- **明确放弃 = 真取消**：等待条上另给「放弃上传并离开」，走 `AbortController` 中断这次上传。
- **组件被父级直接卸载**（返回列表 / 切笔记本）：cleanup 里 abort 在飞上传，并**同步**把正文落进草稿键
  （300ms 防抖定时器会随卸载被清掉）。此处**只写不删**——删陈旧草稿是 mount 扫描与自动草稿的职责，
  卸载也去删会在 React StrictMode 的「挂载→立刻卸载→再挂载」里删掉用户还没决定的那份草稿。
- **批量上传传一张落一张**：中途中断/失败时，已经传完的图片已在正文里，随草稿落盘，不会变成
  「服务端有资产、正文没引用」的孤儿。顺序由这个循环本身保证，与任何键名编码无关。

如实记账的残留：中断恰好落在服务端提交之后的窄窗口里，仍会留下一条无人引用的资产行。服务端的
孤儿资产回收 `sweep_orphan_assets` 已接上生产调用方，**有机会**收走它，但给不出「终会回收」的
保证——它是**前沿触发**的、没有周期性扫描：只在该 notebook **任一** knowhow 表投影完成之后（另起
后台 job，不占投影的单飞窗口）或某张表被删除时才跑，且按 notebook 限流。范围是 notebook 级而非
表级——sweep 扫的是整个 notebook 的资产，因此编辑**同 notebook 的别的表**同样能把这条孤儿行收走；
但这个 notebook 若从此再无任何 knowhow 编辑，它就一直留着。

宽限期算的是「**持续未被引用**了多久」而非「上传至今多久」：计时从某次 sweep 第一次观察到它无
引用起跑、跨多次 sweep 累计；标记要等**后续某次 sweep 观察到它重新被引用**才清掉（不是引用一回来
就立刻清——两次 sweep 之间「恢复引用又再次移除」的话，旧时间戳仍在）。这样护住的是服务端根本看不见
的活引用：图片已粘贴但格子还没保存、图片正从一个格子搬到另一个格子，此刻它们只存在于浏览器草稿里。

一致性取舍：恢复提示未决时用户直接开敲，则任何离开路径（取消 / Esc / 切格 / 卸载）都以
**屏幕上的当前正文**落草稿、覆盖那份旧草稿。同一输入不因离开方式不同而有不同结果，优于「按出口分叉」。

### 补充：恢复提示未决时禁止上传

「恢复」是**整段** `setContent(draftText)`，而恢复提示未决期间自动草稿处于暂停（否则会抢在
用户决定前改写那份待恢复草稿）。两者相加意味着：这期间落笔的图片只存在于当前正文里、没有第二份
记录，用户一点「恢复」就随整段覆盖消失，服务端那条资产再没有任何东西引用它。删掉待完成上传日志后
更没有兜底。

故从**入口**挡住：`resolveUploadBlock` 把「恢复提示未决」与「保存/优化/另一批上传在飞」并列为
拒绝上传的理由，工具栏「图片」按钮一并置灰并给出对得上的提示语；paste/drop 不经按钮，靠函数入口
那道**同步**判定（读 `restoreBannerRef.current`，不读落后一拍的 state）。

另一条路——「每张落笔的图同时并进 `draftText`/草稿键」——被否：那等于在用户还没决定之前就去改写
那份待恢复草稿，正是前几轮复审明令禁止的事。

### 再补充：草稿扫描必须在首帧之前完成

上一条把上传门禁接上「恢复提示未决」之后，仍留了一个**扫描前窗口**：`showRestoreBanner` 初值是
`false`，而扫描原先在普通 `useEffect` 里——被动 effect 通常要等首帧绘制之后才跑，界面在那之间
已经可交互，门禁读到假的 `false`（fail-open），粘贴/拖放照样能启动上传。

修法不是再加一个 `draftScanDone` 门（那只是把窗口从"放行"改成"拒绝"，窗口还在），而是把扫描
挪进 `useState` 初始化器：**banner 在第一帧就是终值**，窗口不存在，`restoreBannerRef` 的初值
也随之正确。只有「清掉与已保存内容相等的陈旧草稿」这一步是副作用，仍留在 effect 里。

在 render 里读 `localStorage` 的两点说明：这是 mount 时读一次、立刻落进 state（不是订阅外部
可变源，故不需要 `useSyncExternalStore`，也不存在并发渲染撕裂）；本组件只在用户点开某一格后
才渲染，服务端渲染阶段不存在这棵子树，故不涉及水合不一致。
