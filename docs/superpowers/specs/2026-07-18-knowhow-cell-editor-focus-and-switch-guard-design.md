...
如实记账的残留：中断恰好落在服务端提交之后的窄窗口里，仍会留下一条无人引用的资产行。它现在
由服务端的孤儿资产回收 `sweep_orphan_assets` 兜底（该 sweep 已接上生产调用方），但**不是立刻**
回收：触发点挂在该 notebook 的 knowhow 表投影完成之后（另起后台 job，不占投影的单飞窗口），
并按 notebook 前沿限流；宽限期算的是「**持续未被引用**了多久」而非「上传至今多久」——计时从
这条资产第一次被扫描观察到无引用起跑、跨多次 sweep 累计，引用一旦回来标记立刻清掉，这样才护得住
服务端看不见的活引用（图片已粘贴但格子还没保存、图片正从一个格子搬到另一个格子，此刻它们只存在于
浏览器草稿里）。即：这条孤儿行终会被回收，只是会在服务端多留一阵。
