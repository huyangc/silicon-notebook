...
`assessment` 是同一次 reflect 的附加结果，不单独发评估调用。每个方面为 `unknown / partial / supported / conflicting` 之一。模型没报告的项保留未知或已有状态，不允许借省略删除必答项。更新同一方面时全量替换其模型支撑判断，不沿用 outline 的自动并集，以便撤销错误判断。载荷的顶层形状接受两种写法：本节的 `{supported:[...], unresolved:[...]}` 列表形，或按方面 id 直接映射的 `{aid: {status?, supported?, evidence_keys?, gap?}}`；服务端在校验前把后者归一成前者，下面的合法性/上限/降级判据不区分来源（2026-09-09 本机实测收窄，见取舍记录）。
...
取舍记录（2026-09-08 本机实测后收窄，边界从「集合/来源身份不能冒充细粒度证据」改为「**未完整**枚举的集合身份不能冒充」）：目录题（「这个库里有哪些文档」）要的支撑不是任何一条细粒度证据，而是「这个集合已经被完整列出」这件事本身；而枚举条目按合同不进候选池、条目 id 对模型不可见，于是原来那条边界让一个 coverage 报了 complete 的目录方面**结构上**永远拿不到 supported——服务端一边报告「已全部列出 84 条」，一边告诉合成侧「这个方面没有支撑」。现在服务端为 `state == complete` 的枚举链签发确定性的集合身份键（`enum:` 前缀 + 集合/子类型/限定来源/范围，与续跑键一一对应、不含用户内容），在**服务器状态块**里（紧接方面清单之后，与它同受「不整体裁尾」的处理）展示给模型，并把它并进 `assessment` 的合法键集。未完整（`open`/`conflict`）的链不签发键，模型自拼的同样被剔除——合法集是服务端算出的那一份，不是按前缀放行。上一段「完整枚举的 complete 只由确定性执行器证明」因此仍然成立：被接受的是**服务端自己记下的**完整性，不是模型的自述。集合键不参与送达复核（它不走证据预算、不产生 `[k]` 锚点）。**集合键没有 epoch，「先 complete、后资料变化」这个窗口是知情接受的**：键的构成与续跑身份一一对应，不带枚举时刻的版本号，所以一条在本 run 内报过 complete 的链，若此后同一 run 里资料再变，它签发的键仍然有效。真正危险的那一半已经被挡住——枚举期间资料变化会把链判成 `conflict`，而 `conflict` 从不签发键；剩下的是「列完之后才变」，代价上限是一个方面的支撑晚了一步反映最新资料，而 run 级的一致性本来就不由这条路保证（候选池、大纲绑定同样是 run 内快照）。加 epoch 要在续跑键、方面账、展示三处各带一个版本号，换回的是这一个窗口——先记下取舍，等有实测证据说它伤人再说。

取舍记录（同上；判据于 2026-09-09 评审后收窄为「收尾载荷」）：**收尾那一轮必须自评。** 判据是 `run()` 的收尾条件本身——`next_action == "answer"`，或任何带 `sufficient=true` 的动作（不产证据的 `update_outline` / `consult_memory` 与 `sufficient=true` 并存合法，而穷尽档最常见的收尾恰恰是 `update_outline` + `sufficient=true`；只认 `answer` 的话真机上最常见的那种收尾一次都不会被追问）。这两种载荷都会当场终止整次检索，那一刻方面账就是这次检索唯一的成色记录；载荷里没有 `assessment`，同一份决定就在两处给出互相矛盾的读数（模型说够了 / 服务端清单上全是 unknown），`classify_termination` 据后者判 `model_partial`。2026-09-08 实测 16 个 run 全部落在这个形状上，14 个的直接原因就是这一格空着。所以这种载荷整轮退回（零 I/O 的 `missing_assessment` 观察，走既有的步数/无进展/熔断记账），下一轮逐个列出方面 id 追问；追问**最多一次**（`REFLECT_ASSESSMENT_MAX_PROMPTS = 1`），第二次仍然空着就接受收尾，并把没被判断过的方面标 `assessment_omitted`——它与「模型根本没走到收尾那一步」在放量评估里要能分开数。**这一轮对 stale 持平、不递增**（与送达了内容的 consult 轮同款判据）：追问的上限已经由 `REFLECT_ASSESSMENT_MAX_PROMPTS = 1` 兜住，不会被反复利用，递增只会让「退回一次换一份自评」这个纯记账动作更容易撞上熔断，而这与真正的空转背道而驰（2026-09-09 本机实测后加）。两条配套约束：同一轮带着 `update_outline` 载荷时，**先应用大纲再退回**（折成 invalid 之后 `run()` 走的是 REFLECT_INVALID 那条 skip 分支，收尾分支里的应用再也不会发生，退回的代价会变成丢掉模型这一轮真正做成的绑定）；**没有下一轮时不退回**（步数用尽；这一轮是服务端强制的大纲溢出纠错轮；或者**动作面在这一轮之后耗尽**——收尾动作是 `update_outline` 而大纲额度只剩这最后一格，下一轮会在发出模型调用之前就以 `only_answer` 收尾，追问句根本送不到模型面前）——追问的全部价值在于下一轮补上，没有下一轮时退回只会把模型自己的 `sufficient` 判读改写成 `step_budget` / `no_executable_action`，所以直接按「第二次沉默」接受收尾。动作面那条判据是**保守近似**（只看大纲额度，不核别的通道），宁可少问一次也不白折一轮。另有一道收尾兜底：run 结束时追问若仍未被消费（发出过、却一次都没渲染给模型），按同一个「第二次沉默」入口记 `assessment_omitted`，免得这种 run 的终态被改写而方面账上什么都没记。反过来，被 §5.2 折成降级轮的那一次**不算送达**：prompt 渲染出来了、追问那一句因此被消费，可回来的是 provider 故障后的兜底，模型一个字都没读到，所以降级折叠时把 `nudge_pending` 重新置位（两道闸：从来没发出过追问不置位；上一次追问已经被回应也不置位）。追问句只挂被退回的下一轮那一次（`nudge_pending`），措辞不回述上一轮发生了什么——那种句子在第三轮之后就是假话。`sufficient=false` 的 `answer` 同样是收尾载荷，因此**也**要自评一次：接入时对它的豁免（「它本来就说了还差东西」）在 2026-09-09 撤销，理由是那一刻服务端拿到的仍然是一张空账，而模型分得清哪个方面拿到了什么——追问一轮换回的正是这份逐方面读数。

取舍记录（2026-09-09 本机实测后加）：**assessment 载荷接受按方面 id 的映射写法，服务端归一。** 本机 deepseek-v4-flash 关思考实测里，55 次收尾自评有 41 次不是本节的列表形，而是按方面 id 直接映射（`{"a1": {"status": "partial", "supported": false, ...}}`，`supported` 有时是布尔、有时缺省，`status` 同理）——这份映射被服务端当成两组都没给的空载荷，「模型不填 assessment」的大多数其实是**形状不合**而不是真的空手。服务端在校验前做纯形状归一（不碰语义）：顶层**不含** `supported`/`unresolved` 任一键时按映射形处理（出现其一即按列表形，陌生顶层键忽略——与归一前 `apply()` 只遍历这两组的语义一致），`supported is True` 且 `status` 缺省或就是 `supported` 才进 `supported`，两格冲突时取保守的一边（`status` 明确给了别的值就以 status 为准进 `unresolved`；`status == "supported"` 而 `supported is False` 进 `unresolved` 的 `unknown`），其余进 `unresolved` 且 `status` 不合法或缺省一律归一成 `unknown`（不是列表形自己的默认档 `partial`——省略说的是"没细分"，不是"已知是部分支撑"）；列表形 item 里的 `id` 别名同样被认作 `aspect_id`。归一之后走的是同一套既有校验（键合法性、上限、重复、剔除后不得保持 supported），越权/超限仍然被拒——这不是放宽合同，是让同一份判断不再因为模型把它包装成另一种等价形状就被读成"没有判断"。
