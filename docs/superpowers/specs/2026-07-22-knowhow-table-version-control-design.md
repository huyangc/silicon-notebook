...
| 历史引用的图片 | 保护，不被孤儿清扫器回收（代价：图片进过格子就基本不再自动回收，靠"清理历史"释放；但若最后一次引用恰好在 head 上则需先编辑表、再清理才生效） |
...
**附加条件（Task 13 code review 实测澄清，非新决策）**：「清理历史之后可回收」不是无条件成立——`prune` 永远保留 head（§7.7，回退的前置指纹守卫需要一个参照），若一张图片最后一次被引用恰好就在 head 这条流水本身（典型场景：贴图→删图→再也不动这张表——"删图"这次编辑成为 head，它自己的 `before` 仍嵌着 `asset://`），清理历史后 head 依然引用着它，紧接着那次清扫仍然一个字节都不会释放。要真正回收，需要先有**一次与这张图片无关的编辑**把 head 挪到不提这张图的流水上，再清理历史+清扫才会生效；删掉整张表不受此限（表没了，它名下资产直接判定孤儿，无论历史长短）。
...
**③ 表管理面板「清理历史」**：清理 N 天前，二次确认，文案写明「清理后将无法回退到该时间点之前，同时会释放这些历史引用的图片」——**这句需要一个附加限定**（Task 13 code review 实测，见 §7.1 的「附加条件」）：若某张图片最后的引用恰好落在 head 这条流水上，这次清理不会释放它（`prune` 按设计永远保留 head），需要用户之后再对这张表做一次编辑、才能在下一次清理历史时真正释放。文案不必逐字解释这套机制，但不能承诺"清理后立刻释放"这种在 head-引用场景下不成立的确定性表述——具体措辞由 Task 17 实现者结合前端交互决定。
...
- **资产联动**：格子里的图片被删出格子 → 清扫器因历史引用而不删；「清理历史」后再清扫 → 删掉（但若图片最后一次引用在 head 上，需先对表做一次编辑、再清理历史才生效）
