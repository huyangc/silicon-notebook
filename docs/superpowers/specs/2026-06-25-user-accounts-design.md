...
|---|---|---|
| 用户名 | 恰好 1 字母 `^[A-Za-z]00[0-9]{6}$` | **单个字母** `^[A-Za-z]00\d{6}$` |
...
- **U1 用户名** = `^[A-Za-z]00\d{6}$`（单个字母 + 字面 `00` + 6 位数字，如 `a00123456`）。大小写均允许；**存储与唯一性用小写归一化**（防 `A/a` 撞键），登录大小写不敏感。
...
- **注册表单**：用户名输入实时正则提示（占位示例 `a00123456`，不符即时报错）、密码 + 确认密码。
- **登录表单**：用户名 + 密码。
