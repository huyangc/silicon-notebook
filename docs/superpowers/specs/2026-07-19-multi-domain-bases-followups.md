...
| 项 | 状态 | 依据 |
| --- | --- | --- |
| A1 | 已做（2026-09-07，待办收尾 PR-2） | 归一化收敛为唯一 helper `app/domain/citation_origin.py::foreign_notebook_id`；`backend/tests/test_citation_notebook_id_guard.py` AST 守卫覆盖 Citation/AnswerAnchor 构造关键字、五处 id_map builder 的 `"notebook_id"` 字典写入、splat/别名、helper 必须从 canonical 模块 import；登记站点逐个钉实参文本与数量；`_citation()` 死代码已随 PR #678 删除 |
| A2 | 已做（同上） | `render_subgraph_context` 改为必填 `active_notebook_id`，所有参与库节点按「≠ active」归一化（挂载的私有库节点也带库 id，不再只认 tier=base）；spreadsheet 引用同样带上工作簿所在库的 id 与 tier |
| A3 | 已做（同上） | `PromotionCandidateActions`：`target_base_id` 为空时批准置灰 + 紧邻提示指向 `scripts/backfill_promotion_targets.py`，拒绝仍可点 |
| A4 | 已做（同上） | 知识晋升与 Memory 晋升统一读 `NotebookSummary.base_notebooks`；owner-only `listBases` 拉取与假「需先挂载」门控删除，守卫钉住 memo 初始化式与不再导入 `listBases` |
| A5 | 已做（同上） | 共享 `PromotionTargetModal`；≥1 候选一律渲染为可点击列表（无自动提交），0 候选给中性文案；none 态文案收敛为 `PROMOTION_TARGET_NONE_REASON` 常量 |
| A6 | 已做（同上） | 深拷贝携带对接收者仍有效的挂载边（用导入的 `MOUNT_VALID_EXPR` 以目的库求值；基库在校验与插入之间被删按丢弃计），无效边丢弃并记日志；`notebook_bases` 是唯一允许行数不等的拷贝表；`docs/product-and-api*.md` 登记 |
| A7 | 已做（同上） | `migrate()` 对 `user_version > SCHEMA_VERSION` 记日志并抛「schema contains a future version」；`docs/development*.md` 写明单向性与就绪端点脱敏 |
| A8 | 已做（同上） | `backend/tests/test_mount_sql_contract.py`：形状从运行时常量提取、两侧独立比对、按相对路径豁免并钉住定义点被扫到 |
| A9 | 已做（同上） | 「基准库」残留早已清；`docs/product-and-api*.md` 补 promote 端点 `target_base_id` 与 400 态（README 不列端点）；点名的陈旧注释核对结果见 PR 描述 |
