...
- Create `backend/app/repositories/postgres/migrations/0001_initial.sql` and `0002_search_indexes.sql`.
...
- Create: `backend/app/repositories/postgres/migrations/0002_search_indexes.sql`
...
All 59 current ordinary SQLite tables listed in the approved design inventory must exist with equivalent keys, nullability, unique constraints, FK actions, and application-visible defaults. FTS5 internal tables are excluded. The PostgreSQL-only migration/version tables are explicitly classified internal. Add `ordinal bigint generated by default as identity` only where current behavior actually depends on implicit SQLite `rowid`; queries must then sort on this explicit field.
...
Compare `information_schema`/`pg_catalog` facts with an explicit mapping for type differences (`TEXT`→`text`, owned JSON→`jsonb`, embeddings→`bytea`, timestamps→`timestamptz`). Assert every ordinary SQLite table has one classification and every PG business table maps back.

- [ ] **Step 3: Write checked-in DDL**

Use application-owned string IDs, deferred FKs only where multi-row transactions require them, explicit `ON DELETE` actions, server UTC timestamps, and named constraints. Enable `pg_trgm` in `0002`; create GIN trigram indexes for current FTS candidate fields plus ordinary FK/order indexes. Do not install pgvector.

- [ ] **Step 4: Test migrations from empty and checksum drift**

Apply twice, verify idempotent version observation, then mutate a copied migration in the test fixture and assert checksum refusal.
...
Keep all PG search expressions in `postgres/search.py`; stores call typed helpers rather than scattering trigrams. Use `jsonb` containment only for application-owned JSON and preserve the existing normalized payload mapper.
...
Assert `scripts/check.sh` does not invoke PostgreSQL, `check_postgres.sh` requires `TEST_POSTGRES_URL`, CI runs a separate `postgres-integration` job with a PostgreSQL 17 service, and all three required docs describe `DATABASE_URL`/`SHADOW_DATABASE_URL`, single active backend, bytea/no-pgvector, single worker, and redaction.

- [ ] **Step 2: Implement the isolated test lane**

`scripts/check_postgres.sh` runs only `-m postgres_integration` after a connection preflight. CI service has a health check and a dedicated non-superuser application role/database. Do not add Docker/PostgreSQL startup to `scripts/check.sh`.
...
- [ ] `scripts/check_postgres.sh` passes twice against a clean PostgreSQL 17 service.
