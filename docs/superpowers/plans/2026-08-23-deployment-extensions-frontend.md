...
- R3 `ExtensionModal` 在 `source.detail_section` 下会相对卡片定位：**已从"登记限制"升级为结构性禁止**（codex #578 R4 P2）——那个 slot 的宿主自己是与 `extension` 互斥的 primary lease（`source-detail`），一条 contribution 若真开成弹窗会把宿主的 primary 冲突关掉、自己也随之卸载，留下一格永远等不到人渲染的幽灵认领。两道独立防线：`host.tsx` 的 `WorkspaceExtensionOutlet` 对该槽位下的每条 contribution 把 `openDialog`/`closeDialog` 换成不触达壳层的拒绝函数（开发模式下 `console.warn`），`context.dialog` 也无条件强制为 `CLOSED_DIALOG`（不看 `dialogHolder` 是否碰巧等于该 contribution 的 id）；`ExtensionModal` 自己在 `context.slot === "source.detail_section"` 时直接 `throw TypeError`。评估过按 `context.slot` 做判别联合、让该槽位的 `WorkspaceExtensionProps.actions` 类型上直接不含 `openDialog`/`closeDialog`——结论是不可行：`WorkspaceUiContribution.Component` 用同一个 `ComponentType<WorkspaceExtensionProps>`，要让类型随槽位分叉就得把 `WorkspaceUiContribution` 整体改成按 slot 参数化的泛型，并让 `registry.ts` 里混合两种槽位的同一个数组字面量分别推导出正确的窄类型——这会把复杂度反向压给多数场景（`workspace.side_panel` 的插件作者要额外做一次 `context.slot === "workspace.side_panel"` 窄化才能拿到 `openDialog`），所以只做运行时禁止，保持类型面不变。守卫是 `frontend/tests/component/extension-plugin-surface.component.test.tsx`（host 拒绝、defensive override、`ExtensionModal` throw 三条用例）。
