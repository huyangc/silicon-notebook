...
REASONING_REFLECT_OPTIMIZATION # PREFIX-REUSE LAYOUT POLICY for the reflect context (default `off`; four values `off / prefix_snapshot / prefix_delta / prefix_delta_lean`). `off` is the implementation baseline — byte-identical v2 behavior; `prefix_snapshot` reorders each reflect message so the stable protocol and the static tool catalog come first and the per-turn current state comes last, giving the upstream model service a chance to reuse the unchanged prefix. The last two values are REGISTERED BUT NOT IMPLEMENTED this cycle: configuring one is rejected loudly at startup (`该取值将在后续 PR 实现，当前请用 off 或 prefix_snapshot`) rather than silently falling back to `off` — a deployment that believes it is running the delta arm would attribute every measurement to the wrong arm. It STACKS ON TOP OF `REASONING_REFLECT_V2_ENABLED`: with the v2 master switch off, reflect runs the legacy protocol where there is no "prefix" to speak of and this setting is ignored; Knowhow completion (explicitly legacy) ignores it for the same reason. It is NOT a user-facing effort tier and is not derived from one. The single repo-wide read point is `reasoning_retrieval.reflect_optimization()`
REASONING_REFLECT_MEASURE_CONTEXT # MEASUREMENT switch for the reflect context (default false), ORTHOGONAL to `REASONING_REFLECT_OPTIMIZATION`: with it on, each turn additionally computes a pure in-memory observation of per-block lengths, byte totals and the common message prefix, so the `off` arm is measured with the same ruler and the two arms stay comparable. Measuring changes no prompt and no decision; it defaults to off because it serializes one turn's message and the disabled path must not pay for that. It stacks on `REASONING_REFLECT_V2_ENABLED` as well — v2 off and Knowhow never measure
