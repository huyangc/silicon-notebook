...
REASONING_REFLECT_V2_ENABLED # master switch for the reflect v2 protocol in reasoning Ask and Deep Report (default **false**). On: each reflect turn is built from a single in-memory capability projection — the action list the model reads, the `next_action` enum the schema advertises and the parser's whitelist are three renderings of one value, so an action that is out of budget, disabled by policy, unsafe for the selected source scope, or meaningless without a knowledge graph disappears from all three at once; the turn is split into a fixed system message (task, untrusted-material framing, scope rules, action contract, stopping rule) and a per-turn user message (question plus candidate material), and the response carries one `arguments` object instead of one branch field per action, validated by type against the chosen action. Off is byte-identical to the previous behavior: the legacy prompt, schema, whitelist, call counts and trace key set, with no capability projection constructed and no extra I/O. It is NOT a user-facing effort tier and is not derived from one. Knowhow completion stays on the legacy protocol explicitly regardless of this switch
...
REASONING_REFLECT_RECENT_OBSERVATIONS # how many recent action observations may be fed back (default 6; 1–20)
