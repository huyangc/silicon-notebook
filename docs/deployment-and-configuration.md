...
REASONING_REFLECT_MEASURE_CONTEXT # MEASUREMENT switch for the reflect context (default false), ORTHOGONAL to `REASONING_REFLECT_OPTIMIZATION` — it is what lets the `off` arm be measured with the same ruler, so the two arms stay comparable. WHAT IT DOES TODAY: it is registered only; nothing is measured yet and no observation is emitted anywhere. The per-turn in-memory observation of block lengths, byte totals and the common message prefix lands with the measurement task of this same change set, and this entry is rewritten with it; the layout above is already live, and the two are orthogonal. Measurement will change no prompt and no decision; it defaults to off because it has to serialize one turn's message and the disabled path must not pay for that. It stacks on `REASONING_REFLECT_V2_ENABLED` as well — v2 off and Knowhow never measure
