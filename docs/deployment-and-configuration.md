...
LEXICAL_LANGUAGE_GATE_ENABLED # drop all-CJK lexical terms when the notebook's sampled corpus holds no CJK character (default true; such probes are provably empty yet each costs a real PostgreSQL LATERAL probe — measured 64 terms/29.7s cold vs 3 terms/0.26s warm for the same 26 rows on a 7,026-chunk English corpus, with the ungated form timing out under parallel report sections. Never filters the user's quoted spans or the whole-sentence term, never applies in the Latin direction, and never applies to *genuinely* source-restricted runs, whose lexical arm is their only candidate generator and whose source predicate already bounds the scan. That exemption is keyed on narrowing, not on the presence of an allow-list: a default all-selected request also carries a frozen allow-list, but it spans the whole notebook, so exempting it would reinstate the unbounded probe set this gate exists to prevent. Set false to restore byte-identical pre-feature behaviour if a notebook's sampled language misjudges it)
