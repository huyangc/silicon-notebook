...
CHUNK_FANOUT_MAX_WORKERS     # total worker slots for one chunk arm's flattened `(library × sub-query)` fan-out (default 8; 1..16). "Per library" and "per sub-query" share ONE executor rather than nesting, so the peak concurrent database connection count is this value regardless of how many reference libraries are mounted; the default matches the fixed width the single-library multi-sub-query path used before federation existed
