...
`retrieval_rerank`, and `agent_profile_consolidate` (labeled "库理解整理" in the model
service status page; the background consolidation call behind "AI 对这个库的理解", see
`AGENT_PROFILE_ENABLED` below) to those services. Several workloads may share a service;
