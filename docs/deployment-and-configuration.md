...
REASONING_CHUNK_SEARCH_ENABLED # gates reasoning Ask's first-class source-passage retrieval action, search_chunks, and its no-graph first-round deterministic seed (default true; false removes the action from schema/prompt/allow-list, stops the seed, and leaves REASONING_MAX_CHUNK_SEARCHES with no consumer — byte-identical to pre-feature behavior, except that the request still pays one graph-existence EXISTS query inside its per-request memo, independent of this switch)
...
REASONING_REFLECT_V2_ENABLED # master switch for the reflect v2 protocol in reasoning Ask and Deep Report (default **false**). On: each reflect turn is built from a single in-memory capability projection — the action list the model reads and the parser's whitelist are two renderings of one value, so an action that is out of budget, disabled by policy, unsafe for the selected source scope, or meaningless without a knowledge graph disappears from both at once (the schema's `next_action` enum keeps listing every recognizable id on purpose, so such a choice becomes a recorded observation instead of a transport-level rejection); the turn is split into a fixed system message (task, untrusted-material framing, scope rules, action contract, stopping rule) and a per-turn user message (question plus candidate material), and the response carries one `arguments` object instead of one branch field per action, validated by type against the chosen action. Off is byte-identical to the previous behavior: the legacy prompt, schema, whitelist, call counts and trace key set, with no capability projection constructed and no extra I/O. It is NOT a user-facing effort tier and is not derived from one. Knowhow completion stays on the legacy protocol explicitly regardless of this switch
REASONING_REFLECT_EVIDENCE_CHARS_BY_EFFORT # per-effort character budget for the v2 evidence cards, as a JSON object (default `{"overview":4000,"standard":6000,"deep":8000,"thorough":12000,"exhaustive":16000}`). Validated at startup: it must contain EXACTLY the five effort ids, each value an integer (a JSON boolean is rejected rather than silently read as 0/1) in 1,000–64,000, and the values must not decrease as effort rises. Deliberately independent of the final-synthesis `kg_context_chars`/`chunk_context_chars`: deciding the next step and writing the answer need different granularity, so no ratio is derived from them
REASONING_REFLECT_EXCERPT_CHARS # per-evidence source excerpt ceiling for the v2 evidence cards (default 240; 80–1,000)
REASONING_REFLECT_TABLE_EXCERPT_CHARS # complete-row table excerpt allowance, only in prefix_delta_evidence; remains inside the shared per-effort evidence budget. Defaults and validation rails are owned by docs/product-and-api.md, Evidence display and prefix-cache experiment
REASONING_REFLECT_PROSE_DETAIL_ENABLED # demand-triggered source-passage detail after a valid retrieval continuation, with optional visibility feedback in spare history space; only in prefix_delta_evidence. First reflect and no-prose paths stay neutral; false restores the preceding prose projection. Does not enable v2 or choose an arm
REASONING_REFLECT_PROSE_DETAIL_CHARS # per-selected-passage detail allowance within the existing evidence budget; defaults and validation rails are owned by docs/product-and-api.md, Evidence display and prefix-cache experiment
REASONING_REFLECT_PROSE_DETAIL_CARDS # per-reflect selected detail keys shared by K/D/supplements, not an all-history limit; defaults and validation rails are owned by the same product/API section
REASONING_REFLECT_STATE_CHARS # projection budget for the COMPRESSIBLE region only — recent action observations and past advice (default 6,000; 1,000–32,000). It is not a ceiling on the whole prompt: the user's full question, frozen constraints, the current legal actions and their allowances, the full outline/overflow and enumeration coverage each keep their own bounds and are never truncated to fit this one
REASONING_REFLECT_RECENT_OBSERVATIONS # how many recent action observations may be fed back (default 6; 1–20). Together with `REASONING_REFLECT_STATE_CHARS` it bounds the v2 action log; the two evidence budgets above bound the v2 evidence cards. All four take effect only while `REASONING_REFLECT_V2_ENABLED` is on — with the protocol off nothing reads them (they are still validated at startup). ⚠ **What THIS one means is tied to `REASONING_REFLECT_OPTIMIZATION`**: on `off` and `prefix_snapshot` it means exactly what is written above — the size of the per-turn sliding window, word for word the same on either, since both share one assembly (see "it moves blocks and nothing else" in the next entry). On `prefix_delta` it **already means something else, and that is current behavior rather than a preannouncement**: it is read only on the one call that builds or rebuilds the evidence-card snapshot K, where it means "how many recent detailed observations this version of K keeps"; the rows outside that window fold into attempted / failed / duplicate / completeness counts and **no longer drive a per-turn window**. The two slices are deliberately **disjoint** (the recent window takes only the last N rows, the fold only the rows before them), so "(M earlier rows not listed)" and "attempted K times in total" each speak about their own slice, and the identity "listed inside the window + dropped inside the window + total outside it = every row" is pinned by `test_delta_rebuild_slices_the_ledger_into_two_disjoint_halves`. The per-turn delta block **never reads it at all**: the observation rows in that block are the only record of what happened this turn, so they are appended in full and never trimmed, and the budget check happens **before** the append (`test_delta_history_charge_counts_only_the_rows_that_reached_a_block`). `prefix_delta_lean` runs the exact same K/D criterion as `prefix_delta` — this sentence is current behavior for all four values, not a preannouncement for any of them
REASONING_REFLECT_DELTA_CARDS_BY_EFFORT # how many evidence cards `prefix_delta` may newly expand in ONE delta block, as a per-effort JSON object (default `{"overview":2,"standard":4,"deep":6,"thorough":8,"exhaustive":10}`). Startup validation mirrors the character budget above item for item: it must contain EXACTLY the five effort ids, each value an integer (a JSON boolean is rejected rather than silently read as 0/1) in 1–16, and the values must not decrease as effort rises (`test_reflect_delta_cards_rejects_bad_mappings` pins every branch). **Consumed only under `REASONING_REFLECT_OPTIMIZATION=prefix_delta`**: the `off` and `prefix_snapshot` arms never read it (it is still validated at startup), and `core/config.py` never reads it either. The single repo-wide read point is `ReasoningRetriever._reflect_v2_context` — through its dedicated `_delta_max_cards`, which itself has exactly one call site, inside that method; the discipline is the same one the other five budgets follow (AST guard `test_reflect_optimization_has_exactly_one_settings_read_point`, parameterized over all six budgets, **exactly one read point per field**), because when one field has two read points "the two arms used different budgets" looks perfectly normal at the byte level. A missing field or missing tier falls back to the registered default and is **never treated as 0** (`test_delta_budget_helpers_refuse_bools_and_never_fall_back_to_zero`): 0 would let no new card into any delta block, which shows up in the bytes as "this arm sent nothing" rather than as a loud failure. It is a CEILING, not an allowance: a turn with no new evidence simply has no card section in its block, and unused card slots buy nothing. The same number also caps how many keys the SUPPLEMENT pass (a same-key excerpt upgrade) may check per turn — comparing the whole pool would re-run the excerpt window O(pool) times per turn, exactly the cost this arm exists to avoid
REASONING_REFLECT_COMPACTION_TARGET_RATIO # the TARGET share of each pool that compressible material may occupy when `prefix_delta` builds or rebuilds the evidence-card snapshot K (default `0.5`; 0.25–0.75, with booleans such as `true` rejected outright instead of being folded into 1.0). **A target, not a hard bound**: necessary material may exceed it within the hard budget (that step is guarded by evidence selection's own budget check, `test_delta_keeps_the_hard_budget_when_the_target_ratio_cannot_be_met`), and its only job is to leave room for the delta blocks that follow, so the first turn does not fill the pool and the second turn immediately rebuild (prefix-reuse design §5.2, first sentence). **The first K and every rebuild go through the same code and the same ratio**, so "the first version is built to the target and so is every later one" does not rest on two pieces of prose agreeing. Each side of the range has its own failure mode: below 0.25 the evidence the model can see on the first turn is squeezed to the point of uselessness, above 0.75 no headroom is left for a rebuild. It too is consumed only under `prefix_delta` and never read by `config.py`; the single read point is `_reflect_v2_context` (through the dedicated `_delta_target_ratio`, same AST guard). It is **not a share of user content to discard**: the user's full question, the frozen constraints and this turn's execution limits each keep their own bounds, and the shrinking happens only in the evidence-card and compressible-history pools. `prefix_snapshot` and `off` evidence selection does **not** use this shrink rule at all (design §5.2 says so explicitly; both arms stay byte-identical to before it was wired, `test_delta_wiring_leaves_the_other_two_arms_byte_identical`)
REASONING_REFLECT_OPTIMIZATION # PREFIX-REUSE LAYOUT POLICY for the reflect context (default `off`; five values `off / prefix_snapshot / prefix_delta / prefix_delta_lean / prefix_delta_evidence`, **all five implemented**; the evidence-only experiment requires explicit selection). `off` is the implementation baseline — byte-identical v2 behavior. WHAT `prefix_snapshot` DOES: it reorders one reflect turn's message BY STABILITY so the upstream model service has a chance to reuse the unchanged prefix. The system message becomes one byte-identical block for the whole run — the protocol, the static tool catalog (the stable superset of the tools this run may execute, computed once at the first reflect turn), and the rule that the catalog is only a parameter reference while THIS turn's callable set sits at the end of the user message. The user message is ordered question-and-confirmed-contract → evidence cards → action observation ledger → current turn state, and that last block (this turn's callable actions, the withheld list with reasons, current aspect status, completed collection keys, the server-state summary) is rewritten each turn. WHAT STABILITY YOU ACTUALLY GET: byte-for-byte, turn to turn, the wrapper, the system message and the question-and-contract part — and those only. The blocks above the last one are ordered BY STABILITY, not frozen: evidence cards are re-selected every turn and reorder as bindings and fresh results change, and the observation ledger is a bounded recent window (`REASONING_REFLECT_RECENT_OBSERVATIONS`) whose first line turns into an "(N earlier rows not listed)" counter — and then changes with N — once rows fall out of it. A measured common prefix that stops at the head of the cards or of the ledger is the expected shape of this arm, not a defect in it. IT MOVES BLOCKS AND NOTHING ELSE: evidence selection, both observation-ledger bounds, the three budgets, action availability, the parse whitelist, the failure and terminal-state contracts and the trace key set are all unchanged, and the model is told the same facts as under `off`, item for item (same evidence keys, same available actions and quota numbers). MOVING BLOCKS IS NOT BYTE-NEUTRAL, though: on a cache MISS budget roughly **+1–3KB of input per turn** against `off`. The catalog is always a superset, so `off`'s system message SHRINKS as allowances are spent while this arm's does not — measured on a full-house fixture, early turns are `off` 9,466 chars vs. 10,606 here (+1,140) and late turns 7,883 vs. 10,851 (+2,968). CPU is not a factor (the static system message re-renders in about 11µs per turn). The cost is accepted by design and disclosed honestly: the catalog is fixed once per run, so a tool stays described in it after its budget is spent or a condition it needs stops holding; when the model picks one anyway the turn becomes the existing zero-I/O `unavailable_action:<reason>` observation and the loop continues. MEASURING THOSE BYTES TAKES A SECOND SWITCH: the ruler is `REASONING_REFLECT_MEASURE_CONTEXT` in the next entry, ORTHOGONAL to this one and off by default — both arms go through one assembly and therefore share one ruler, and this setting writes no number of its own — the one exception being `prefix_delta`'s three BEHAVIOURAL keys (`context_rebuilds` / `context_fallback` / `delta_blocks`), which have nothing to do with counting bytes and are written even with measurement off (see the registered deviation in the next entry). `prefix_delta_lean` IS NOW IMPLEMENTED (PR-4): same message shape as `prefix_delta`, differing only in its self-assessment contract (full write-up below, under WHAT `prefix_delta_lean` DOES). Any spelling outside the four-value closed set is rejected loudly at startup by the `Literal` type itself (`Input should be 'off', 'prefix_snapshot', 'prefix_delta', 'prefix_delta_lean' or 'prefix_delta_evidence'`), not silently folded to `off`. A duck-typed settings adapter that never passes that validator folds any value outside the implemented set back to `off` at runtime. WHAT `prefix_delta` DOES (the third value, opened up by PR-3): the message SHAPE is exactly the same as `prefix_snapshot`'s (`system(S)` plus one `user(C+K+D+T)`, block for block on both arms, `test_delta_layout_takes_the_same_prefix_message_shape`); the difference is only in what goes into K and D — K stops being "re-selected and re-ordered from the whole pool every turn" and becomes "one phase snapshot plus append-only delta blocks". (1) **K/D division of labour**: K is the current phase's opening snapshot (built to the target ratio above), and D is ONE BLOCK PER TURN with three sections inside it (cards newly expanded this turn → this turn's action observations → the aspect updates the server accepted last turn; a section with nothing in it is omitted). Every D block already sent stays byte-identical and is only ever appended to, never rewritten (`test_delta_only_appends_and_never_rewrites_an_earlier_block`). (2) **Card freezing**: the first time an evidence card is really sent its bytes are frozen, and every later rebuild and delta block takes them from that table instead of recomputing the excerpt against this turn's query terms (`test_evidence_block_takes_the_frozen_bytes_instead_of_re_rendering`, `test_delta_rebuilt_snapshot_reuses_frozen_bytes_when_a_card_returns`). (3) **Supplement cards never rewrite in place**: when a better excerpt for the same evidence turns up later, a new card is APPENDED whose `key=` cell is byte-identical and which carries a version marker; the old card stays valid and the binding uses the same key (`test_supplement_card_repeats_the_pool_key_verbatim`), and the same excerpt is never appended twice (`test_supplement_is_not_appended_for_an_excerpt_already_sent`). The detection scope is "keys bound this turn ∩ keys already frozen", continued from **the last key served on the previous turn** at its position in this turn's candidate order, at most `REASONING_REFLECT_DELTA_CARDS_BY_EFFORT[tier]` of them (the continuation point stores the key, not a position: the candidate order can shift every turn, and a positional cursor starves the tail keys there — `test_delta_supplement_rotation_starves_no_candidate_when_the_order_moves`). (4) **Both pools are counted as a total**: on this arm the evidence pool (the per-effort character budget above) and the history pool (`REASONING_REFLECT_STATE_CHARS`) count **K plus every D together**, not a fresh full allowance per turn, and the check happens BEFORE the append (`test_delta_keeps_the_cumulative_evidence_budget_across_blocks`). That check charges the **complete** pending block in the same units the ledger uses — block head plus card section against the evidence pool, observation rows plus the accepted-aspect note against the history pool — so a turn with only observation rows to append is checked too instead of appending its head unconditionally (`test_delta_counts_the_block_head_of_an_observation_only_pending_block`, `test_delta_counts_the_pending_aspect_notes_against_the_history_pool`). **One overrun is allowed, deliberately**: a turn with no candidate cards whose head or note still does not fit after the rebuild sends the block anyway — the observation rows are the only record this turn's action leaves in the context — and the ledger records the overrun as it is, which makes the next turn rebuild. (5) **The rebuild trigger is "not one new card fits" or "the complete pending block breaks a pool"**, not "some card this turn does not fit the remaining allowance": the latter would compact K every time one oversize card shows up, and that card already has a count disclosure and comes back on the next rebuild through the same ordering. Rebuilds also have HYSTERESIS — if the previous turn just compacted a version and this turn still cannot fit a single new card, compaction has stopped being effective at this budget, so there is no second rebuild and the run falls back instead (`test_delta_rebuild_has_hysteresis_instead_of_compacting_every_turn`). A rebuild involves zero LLM calls, zero database reads and zero extra allowance (`test_delta_adds_no_retrieval_or_model_calls`). (6) **Fallback is irreversible and affects that run only**: when even a rebuilt K cannot fit the minimum useful new evidence, the rest of the run falls back to `prefix_snapshot`'s bounded evidence selection and `context_fallback` shows up truthfully in the projection; falling back clears no candidate pool, drops none of the D blocks already sent, revokes no "really shown" binding eligibility, resets no allowance, and adds neither a model call nor a trace step (`test_delta_falls_back_irreversibly_and_keeps_every_result`, `test_delta_fallback_keeps_every_block_it_already_sent`, `test_delta_fallback_registers_no_key_from_the_snapshot_it_threw_away`). In the statistics it must appear as what it is and must **not** be averaged in as "a run that used D throughout" (design §13). THE BYTE ACCOUNT (honest, and not one number of it has been measured yet): S is **1,507 characters** larger than on `prefix_snapshot` (the four sentences that explain how to read D), and it sits in the part that is byte-identical turn to turn — so when the upstream really does reuse the prefix that cost is paid once on the first turn, and when it does not it is paid every turn; each D block carries a **75-character block head** plus a newline (blocks after a rebuild add roughly 14 more characters of version marker) with a blank line between blocks; a supplement card costs about 26 characters more than the original card. WHAT IS SAVED is the once-per-turn re-selection, re-ordering and excerpt recomputation over the whole pool (K's bytes come straight out of the projection and are not re-rendered), plus the longer common prefix that follows from it. Which side is larger DEPENDS ON THE QUESTION TYPE AND THE TURN COUNT, and this arm's real benefit is **not measured by a single number yet**: everything above is a structural account, not a measured conclusion, and the actual gain waits on the A/B experiment (the rig's second arm dimension, see `scripts/README.md`). ⚠ **Every turn after a fallback builds K against "the per-effort evidence pool MINUS the retained D"**: from that turn on K is re-selected by `prefix_snapshot`'s bounded selection each turn while the D blocks retained above it travel along unchanged (pulling context the model has already read out from under it is worse than paying a few thousand extra characters) — so K's budget first subtracts the EVIDENCE half those retained blocks already charge, the recent-observation window first subtracts their HISTORY half, and the keys they already show are excluded from K's candidates **in every selection tier**, the source-diversity tier included (that tier's key order is computed inside the selection itself, so filtering only the two key lists the caller passes in would leave it untouched). The evidence in one message therefore still stays within the per-effort pool, and no single piece of evidence appears twice without a version marker (`test_delta_keeps_the_cumulative_evidence_budget_across_blocks` covers the fallback turns' bound, `test_delta_fallback_turn_keeps_its_own_cards_out_of_the_kept_blocks` the disjoint keys). The price is recorded as it is: when the retained D blocks eat most of the pool, K after the fallback can be empty — those blocks are exactly the evidence the model has already read. The two cumulative counters are therefore NOT dormant after a fallback: each shrinks to the retained D's half (the fallback subtracts that K version's own half), and those two subtractions are their only readers from then on. WHAT `prefix_delta_lean` DOES (the fourth value, opened up by PR-4; L below): **it is not a fifth layout — it is a different SELF-ASSESSMENT CONTRACT on the same layout**. The message shape, the content criteria for K and D, the character definitions of the five blocks, and the rebuild and fallback rules are item for item the same as `prefix_delta`'s (D below) — one assembly, one layout dispatch (the module-level `_DELTA_LAYOUTS`, and `test_delta_layout_takes_the_same_prefix_message_shape` covers this value arm by arm). The difference is ONLY these three clauses of the assessment contract. (1) **An ordinary turn reports only what CHANGED**: the model fills `assessment` with the aspects whose judgement moved this turn, the server remembers every judgement it reported before, an aspect already marked supported needs no restating, and omitting one costs nothing and is never chased with a follow-up question. The lean paragraph in S REPLACES the shared one rather than being appended to it (two contradictory contracts in one prompt is the worst shape, `test_reflect_v2_static_prompt_lean_replaces_not_appends_the_assessment_paragraph`), and the closing note in T is the same either-or (`test_the_lean_status_block_differs_only_in_the_closing_note`). (2) **A closing turn is not sent back to square the ledger**: under D a closing payload that lands no aspect at all has the whole turn folded into a zero-I/O pseudo-action, costs a step, and is asked aspect by aspect on the next turn; under L that turn is NOT sent and the close is accepted as it stands (`AspectLedger.note_missing_assessment` returns False first thing on L, leaving the prompt allowance and `assessment_omitted` at their initial values from beginning to end — `test_a_lean_run_accepts_a_silent_closing_turn_without_a_follow_up`; the paired assertion is "L makes exactly one real model call fewer than D on the same script", `test_a_lean_silent_closing_turn_costs_no_extra_model_call`). (3) **Unassessed never impersonates "the notebook does not have it"**: omission is the norm on L, so the server-fact block on the synthesis side carries ONE MORE DISCLOSURE LINE — how many mandatory aspects were never checked one by one, which ones, then the model's own closing verdict, closed by a sentence saying this is NOT a finding that the material is missing (the definition and its edges are in the aspects-and-termination part of `docs/product-and-api*.md`; `test_a_lean_termination_discloses_what_was_never_assessed`). **Validating the action and the assessment independently, per aspect, is the SHARED baseline** (PR-1's T-BF7, common to all four values, not something this one introduced): a non-compliant row invalidates only that aspect while this turn's retrieval action goes through as normal, and only a payload the server cannot attribute at all invalidates the whole turn — unchanged, word for word, under L (`test_a_malformed_assessment_still_folds_the_whole_turn_under_lean`). **L changes what the MODEL SUBMITS, not what the SERVER SHOWS**: the aspect status block still lists EVERY aspect line by line (deliberately not narrowed to "only the unsettled ones plus a count" — on L that block is the only complete record of the aspect ledger), and evidence selection, action availability, the closed set of seven termination reasons and the trace key set are untouched (`test_a_lean_turn_that_reports_one_aspect_leaves_the_others_verbatim`). **The switch is frozen once, when the ledger is built**, so flipping the policy value mid-run cannot change this run's follow-up contract (`AspectLedger.lean_assessment` is read-only, `test_the_lean_switch_is_frozen_on_every_ledger_source`); **after a context-budget fallback it is still L** (the fallback only changes how K and D are built, never the assessment contract — design §5.2, `test_a_lean_run_that_falls_back_keeps_the_lean_contract`); and `off` / `prefix_snapshot` / `prefix_delta` are byte-identical to the PR-3 merge state (`test_the_lean_wiring_leaves_the_other_three_arms_untouched`). THE BYTE ACCOUNT (honest): swapping the assessment paragraph in S for the lean one makes S **1,246 characters larger** than D's (measured; the same number lives in the comment on `_V2_LEAN_ASSESSMENT_INSTRUCTION` in `prompts.py` and must be re-measured in the same diff as any edit to that text — those extra bytes are the illegal-key dropping, the same-aspect-twice rule and the per-aspect row cap, all of which the OLD paragraph already claimed, restated correctly against the current validator, not new semantics), and the closing note in T is **3 characters longer**. Both numbers sit in S and T and are **not a layout difference** — read the `ctx_chars_s` comparison with that in hand, so "L is more expensive because it sends more content" is off the table. **WHAT IS SAVED IS ON THE OTHER SIDE, AND NOT ONE NUMBER OF IT HAS BEEN MEASURED**: what shrinks each turn is the rows the MODEL ITSELF no longer restates for already-supported aspects (that is on the model's output side, not in the context the server sends), and what is mainly saved is **one closing turn** (a closing turn missing its assessment is no longer pushed back with a follow-up; a reflect turn measures about 40s in production). Which side is larger depends on the question type and the turn count, and **this arm's real benefit is not measured by a single number yet**: everything above is a structural account, not a measured conclusion, and the real gain waits on the paired D↔L A/B (the rig's second dimension, see the `ab` part of `scripts/README.md`; **D↔L compares self-assessment contracts, so no difference measured between them may be attributed to the prefix cache itself** — that account was already settled on the D↔P pair). It STACKS ON TOP OF `REASONING_REFLECT_V2_ENABLED`: with the v2 master switch off, reflect runs the legacy protocol where there is no "prefix" to speak of and this setting is ignored; Knowhow completion (explicitly legacy) ignores it for the same reason. It is NOT a user-facing effort tier and is not derived from one. The single repo-wide read point is `reasoning_retrieval.reflect_optimization()`. PR-5 ADDS THREE EXPERIMENT CHANNELS (E1/E2/E3, `scripts/reflect_shadow_rig.py`'s `prefix-probe` / `state-probe` / `ab`) that measure this switch's four cells without shipping any adoption decision — **the real payoff of all four cells is still unmeasured**; PR-5 ships only the experiment channels plus their dry-run output and manifest (`scripts/README.md`), and a real batch run is the operator's job. This period adds **no new configuration item** for that work: E1's seam in `app/core/llm.py` is a module-level `ContextVar`, not a `Settings` field, and its batch wall-clock budget and random seed are CLI arguments to the rig, not configuration — do not go looking for an `E1_ENABLED` that does not exist
# `prefix_delta_evidence`: an opt-in V2 experiment retaining evidence cards and the `prefix_delta` S + C + K + D + T layout, frozen card bytes, append-only observations, shared budgets, rebuild and fallback behavior. It removes per-aspect assessment instructions and output schema, assessment absorption/retries, dynamic aspect status, and self-assessed gaps from synthesis. The full question, required topics, user constraints, action availability and budgets remain. The existing evidence/history and delta budget settings above also apply to this mode; no increase to total budgets or model calls is added. Existing modes and the default `off` keep their behavior. Use `REASONING_REFLECT_V2_ENABLED=true` with `REASONING_REFLECT_OPTIMIZATION=prefix_delta_evidence`; disabling V2 still resolves the optimization to `off`. Prefix stability is an opportunity for the serving backend, not a claim of cache hits or measured speedup. See `scripts/README.md` for explicit paired experiments.
REASONING_REFLECT_MEASURE_CONTEXT # MEASUREMENT switch for the reflect context (default false), ORTHOGONAL to `REASONING_REFLECT_OPTIMIZATION` — it is what lets the `off` arm be measured with the same ruler, so the two arms stay comparable. WHAT TURNING IT ON DOES: every reflect turn computes one extra IN-MEMORY observation and writes it into that turn's reflect trace step detail — the character count of each of the five context blocks (system segment / question and contract / evidence cards / action observation ledger / this turn's current state; the five sum exactly to the character count of both message bodies), the total BYTE size of this turn's provider-facing messages (including the server-side wrapper and the serialization framing, the same unit as the next item, so that division has bytes on both sides), the common prefix in bytes between this turn's messages and the PREVIOUS turn's (absent on the first turn, truthfully, not 0), the number of evidence cards shown and omitted this turn, plus this turn's model-call wall clock, real request count and response body length (accumulated across both calls when a turn calls twice; absent, never 0, when the client does not report them). **From PR-4 there are two more assessment readings, gated on this same switch and written on ALL FOUR values** (not specific to `prefix_delta_lean`: under `off` / `prefix_snapshot` / `prefix_delta` the model still restates everything every turn, so these two are just as meaningful there, and one ruler across four arms is what makes them comparable): `assessment_rows`, how many self-assessment rows actually LANDED IN THE ASPECT LEDGER this turn (not how many the payload contained — rows rejected per aspect do not count, and a payload folded for being out of shape records 0), and `assessment_absent`, that this payload carried no `assessment` at all. ⚠ **The two keys are written only on the turn that carries THE MODEL'S OWN PAYLOAD**: a provider-failure fail-open turn, and the turns where the server folds the decision into a pseudo-action (first-strike degradation, a parse failure, an out-of-range parameter), also have `assessment` permanently None — but that is not "the model omitted its assessment", it is a turn with no model assessment intent to speak of, so **neither key is written** and the read side truthfully reports that turn as not observed. The top-level `assessment_rows_total` is therefore **sum-over-present** (not "any step missing ⇒ the whole column unknown"), and its completeness is disclosed separately by the companion column `assessment_observed` (every reflect step carried it ⇒ true; only some did ⇒ false, and the sum is then the REAL partial sum; none did ⇒ absent) — one stray provider hiccup must not read a whole run's column as unknown. The request TEXT itself only ever passes through memory: it reaches neither the LLM call log, nor the trace, nor any projection. It changes no prompt and no decision: measurement NEVER alters the messages sent or the conclusion reached — not when it succeeds, and not when it fails. It runs inside its own guard, so a measurement that cannot be computed shows up as those keys being ABSENT for that turn (read as unknown, never as 0) and in nothing else; it can neither suppress a request nor manufacture a fallback turn. Turn it off entirely and the reflect step detail is byte-for-byte what it was before as well. It defaults to off because it has to serialize one turn's message and hold on to the previous turn's bytes — one turn of messages in magnitude, peaking at two, since the previous turn's bytes are only released when this turn's are promoted at trace-recording time, which spans the whole model call: with it off nothing is cached and nothing is serialized. ⚠ **The two delta arms (`prefix_delta` and `prefix_delta_lean`) are the one registered deviation from that**: they construct the carrier object UNCONDITIONALLY, because "how many times K was rebuilt", "whether the run fell back irreversibly" and "how many D blocks are in this message" are BEHAVIOURAL FACTS with nothing to do with whether bytes are being counted — a production run with measurement off can still fall back wholesale to bounded evidence selection, and that fact appears in no byte count anywhere. What this switch then governs is THE OTHER HALF: while it is false not a single byte is serialized (`serialize_provider_messages` is called zero times), and the five block character counts, the message byte total, the common prefix and the two card counts are truthfully ABSENT (read as unknown, not 0), leaving only those three behavioural keys in the detail — `test_delta_with_measurement_off_writes_only_the_behaviour_keys` pins exactly that, and the two assessment readings above are in that same half, truthfully absent with measurement off (`test_the_two_assessment_keys_are_absent_when_measurement_is_off`). The criterion is unchanged, word for word, on the `off` and `prefix_snapshot` arms. There is one further registered deviation that has **nothing to do with this switch** (decision Q10, the one field this plan is allowed to let deviate from the three arms' byte equivalence): `aspects_unassessed` on the CLOSING step's detail (how many aspects the model never judged even once) is written UNCONDITIONALLY by all four v2 values, looking neither at this switch nor at the arm — written only on L, that column would be permanently half-missing in the D↔L pair table, and "how many aspects did L check less" is precisely what it answers; it sits on the closing detail, so not one byte of the three arms' prompts moved. It stacks on `REASONING_REFLECT_V2_ENABLED` as well — v2 off and Knowhow never measure. **PR-5's E2 and E3 experiment channels require this switch to be on on EVERY ARM under comparison** — the rig re-reads it per arm before the batch starts and stops the whole batch on a mismatch, not only per-run; a batch with it on for only one side would leave the other side's columns permanently unknown while every row still looks normal. **E1 (`prefix-probe`) does not go through this reflect measurement layer at all**: it structurally strips out the entire reflect loop (zero database, zero retrieval, zero reflect — it only calls the `reasoning_agent` workload itself), so it neither reads nor re-reads this switch, and the switch has no effect whatsoever on an E1 batch
