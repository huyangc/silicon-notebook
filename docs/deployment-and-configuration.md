...
POSTGRES_CHUNK_FTS_TIMEOUT_SECONDS # PostgreSQL-only private deadline for one generic chunk lexical call (default 3.0; >0, <=10 and <=POSTGRES_STATEMENT_TIMEOUT_SECONDS); a timeout opens a per-notebook circuit for the rest of that retrieval run
