...
AGENT_CALL_LOG_ENABLED       # records one row per notebook-scoped MCP tool call an Agent makes (who, when, under which capability), readable and clearable by that member alone under "Agent 记录" (default true; false writes nothing at all — the check precedes the transaction — and leaves already-recorded rows in place, since turning the switch off means "stop recording", not "erase what was recorded"). Independent of AGENT_PROFILE_ENABLED on purpose: that gate governs a model-calling pipeline that shapes answers, while this only books a call that already happened — it never reaches a prompt (the consolidation read pins kind='note' in SQL) and never triggers consolidation
