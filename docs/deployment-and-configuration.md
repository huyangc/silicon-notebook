...
POSTGRES_LEXICAL_KNN_ENABLED # GiST `<->` KNN early stop for the KG-name lexical probe on large PostgreSQL notebooks (default false; PostgreSQL-only, SQLite ignores it). Requires a shape-matched GiST trigram index over the knowledge-object name expression — detected by shape, not name, so a pre-existing operator-built index counts; without one the legacy statement runs unchanged. Only engages for unscoped runs on non-copyable notebooks. Scores stay `similarity()`; within equal-similarity tie classes the two access paths may pick different members (measured: 285 rows named exactly "DAC" at similarity 1.0 on a 9.1M-row base), which is why the flag ships off pending a production A/B. Measured on that base: 7.4s → 123ms per common short term (60×). See Operations for the index DDL and rollout/rollback steps.
