...
REASONING_REFLECT_RECENT_OBSERVATIONS # how many recent action observations may be fed back (default 6; 1–20). Like the three evidence/state budgets above, it has no consumer until the evidence cards land (T3): the value is still validated at startup, but no code reads it today
