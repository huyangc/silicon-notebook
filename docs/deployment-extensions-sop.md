...
- **401 translation.** A 401 raised *inside* your handler becomes `424` with core's own copy and a logged `plugin_upstream_unauthorized` event. In core, 401 has exactly one meaning to the browser — clear the token and reload — so an expired upstream credential inside one plugin route would otherwise sign the user out of the whole product. A genuine 401 from core's router-level session gate still surfaces as 401.
...
| Signal | Meaning |
| --- | --- |
| `plugin_upstream_unauthorized` event, client sees `424` | Your handler raised 401. Core translated it so the browser does not sign the user out. Translate upstream 401s yourself if you want your own wording. |
