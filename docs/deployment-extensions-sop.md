...
The eleven remaining production extension points are Protocols in the SDK; implement the Protocol, declare the matching `ContributionKind`, and register through the typed `add_*` helper.
...
An `auth.provider` contribution is the one startup-frozen external identity provider for the deployment. Its pure `describe()` and `authorization_parameters()` calls supply a stable provider id, identity-source namespace, configuration generation, public label, fixed HTTPS authorization endpoint, PKCE support and provider-owned static parameters. Core rejects any attempt to supply `state`, `redirect_uri`, `code_challenge` or `code_challenge_method`, then adds those values itself. `authenticate_code()` receives only the authorization code, core's fixed callback, an optional verifier and a monotonic deadline; it returns `ExternalIdentity(provider_namespace, subject, username, display_name)`. It never receives a browser `Request`, cookies, a site token or password, repository, target user id, role, or session issuer. Core owns the public start/callback routes, state and browser proof, account binding, username conflict handling and site session. Provider failure is fail-closed and mapped to typed content-free errors; raw tokens, upstream responses and exception text never cross the host. There may be no provider in local mode, but there may never be two. This point does not grant an anonymous plugin route: the sign-in-free callback is a fixed core route that calls the narrow host.

An `ask.engine` provider registers a descriptor whose `mode_id` starts with its own plugin id and returns `AskEngineResult(answer_markdown, citations)`. It receives only the current question plus retrieval, model, trace, and cancellation ports. Retrieval returns bounded evidence with opaque run-local handles; cite only handles issued during that same invocation and place matching `[kN]` or `【kN】` markers in the Markdown. Core validates the whole citation set fail-closed, constructs durable citations itself, and rechecks run identity before saving. Besides element `search()`/`fetch()`, `RetrievalAccessPort` exposes a bounded KG read surface — `search_kg(query, k, object_types=())`, `kg_neighbors(evidence_key, k, edge_type="", direction="both")`, `kg_overview()` — where `search_kg` and `kg_neighbors` share one independent call budget (`ASK_PLUGIN_ENGINE_KG_SEARCH_MAX_CALLS`); a KG hit is citable only when it has a surviving evidence binding, its citation opens the object's first surviving evidence element (never a graph view), and neighbor results carry no edge-type label or truncation signal. Exact contract details and rails: [product-and-api.md § Deployment Ask engines](./product-and-api.md#deployment-ask-engines-askengine). The v1 port deliberately has no conversation history, intent preflight, PPR/community/graph-walk, repository, settings, connection, raw model client, or live-trace surface. Its mode is shown in the browser's advanced-mode third group when live availability passes, and MCP's `ask_notebook` admits the same registered, live-available mode id as its `mode` parameter — a plugin engine can run considerably longer than the built-in modes, so a client calling it needs a read timeout with real headroom.
...
A `source.element_enricher` contribution registers a single-purpose `ElementEnricher` and runs after a source's elements are parsed, before they are persisted — the same element generation the chunk pipeline is about to read, not a later pass. It receives core's own read-only view of every parsed element (never a parser metadata mapping) plus an on-demand, turn-scoped reader for any element carrying an already-persisted image, and returns bounded candidates naming an already-shown element with structured `metadata` and an optional `description`. Core owns the schema this writes to (`metadata.extensions[<contribution_id>]`, plus an appended `description`/`text`); a plugin never rewrites `caption` and never receives a repository, settings, or a database connection. Exact contract details, the mount point, and the rail table: [product-and-api.md § Source element enrichment](./product-and-api.md#source-element-enrichment-sourceelement_enricher).
...
- A `ReflectActionContributor.invoke` runs on the same kind of private worker thread under the same hard-deadline discipline (`REASONING_PLUGIN_ACTION_TIMEOUT_SECONDS`, shared by the availability check and the call itself — see [Reflect plugin actions](./product-and-api.md#reflect-plugin-actions-askreflect_action) for its default and its full budget accounting): the same `contextvars`/thread-local unavailability applies, and there is no seat for a notebook id, actor id, retrieval scope, or any other core port on `ReflectActionCallContext` — only `question`, the model's own already-validated `arguments`, a `cancellation` token, `deadline_monotonic`, and `max_items`. Return a `ReflectActionResult` (not a raw tuple, and not wrapped in `ContributorResult`); each `ReflectActionItem.excerpt` is exactly the text a citation card will show verbatim — it is never re-summarized, so put the quotable material there, not a paraphrase. Every string field is rendered on one line inside a `k<n>:`-keyed evidence block, so a value carrying a newline or another control character is folded to a single space rather than trusted to stay on its own line; `url` is admitted only when it is `http`/`https`. The framework validates your descriptor's shape at freeze time — an over-length string, a control character, a duplicate or reserved action/parameter name, or an enum with fewer than two values fails registration outright (`ExtensionRegistryError`, i.e. startup), never a silent clamp. `max_calls_per_run` on your descriptor is a ceiling the deployment's own `REASONING_MAX_PLUGIN_ACTIONS` policy can only lower, never raise.
...
- An `ElementEnricher`'s availability probe and its `enrich()` call run together on one private worker thread under a hard deadline, exactly like `GapConsultContributor` — do not rely on `contextvars` or any core ContextVar surviving into that thread. `context.assets.read(ref)` only returns bytes during that same call: it always answers `None` once your `enrich()` has returned, timed out, or been abandoned, so never stash a ref or the reader itself for later use. A `description` you return must not contain control characters other than `\n`/`\t`; a credential or endpoint setting (API key, its env-var name, base URL) must never enter a candidate's `metadata` — non-secret provenance such as the model id is fine and is what the sample records.
...
| Rollback | Installing the previous wheel and UI package, then restarting, is the only path that actually swaps the code back — the runtime switch below cannot undo a bad version, only stop it from serving. To buy time before a full rollback, flip the plugin off at runtime first (see below), then prepare the rollback package at your own pace. Or set `enabled = false` on the entry and restart — a disabled entry is never even imported. |
| Disable temporarily | Prefer the runtime switch: turn the plugin off at `/admin/extensions` (`PATCH /api/admin/extensions/{plugin_id}`). It takes effect immediately in the process that handled the request and within `EXTENSION_ADMISSION_REFRESH_SECONDS` (default interval and bounds are registered in [`docs/deployment-and-configuration.md`](deployment-and-configuration.md)) on every other serving process — no restart, and turning it back on is just as fast. It only blocks *access*: the workspace entry disappears, the plugin's own HTTP routes 403, and every capability it contributes stops being offered — the plugin's code stays loaded in the process. Reach for `enabled = false` in the TOML plus a restart only when you need the deeper tier — the entry not even imported — for example before uninstalling it or when you no longer trust its code to run at all. Do **not** clear `EXTENSIONS_CONFIG` to disable one plugin: that swaps in a different discovery/registry composition for every plugin at once. |

**Every row above except the runtime switch is a process restart.** There is still no hot reload of the loaded *topology*, deliberately: the registry is frozen at startup, so which plugins exist is a fact for the lifetime of the process rather than a moving target every request has to re-derive — that half is unchanged. The runtime switch is a second, much narrower layer on top of it: it never changes which plugins are loaded, only whether an already-loaded deployment plugin is currently serving requests. The process that flips it sees the change at once; the deployment's other serving processes converge within one refresh tick; an offline CLI or batch process reads the value once, at its own startup, and not again during that run.

The generic runtime switch must not disable the `auth.provider` selected by the current non-local authentication policy. That provider is the deployment's login dependency, so changing or removing it goes through the authentication policy's own maintenance/preflight and restart procedure. Treating it like an optional workspace plugin could strand every human login path; the generic admin toggle therefore rejects that target rather than using “temporarily disable” as an authentication-mode change.
...
| The entry does not render, but `/admin/extensions` lists the plugin | One of the four visibility gates in §7 is false. Check `GET /api/system/extensions` first: that row's `available` and `unavailable_reason` (`disabled` = either your probe returned `DISABLED`, or an admin turned the plugin's runtime switch off at `/admin/extensions` — the two are indistinguishable on this wire value; `unavailable` = it returned `UNAVAILABLE`). If the row is absent altogether, the browser's local tuple does not match — the manifest `version` on the two sides has drifted. To tell an admin runtime disable apart from your own probe, check that plugin's `runtime_enabled` on `/admin/extensions` (or `GET /api/admin/extensions`) — `false` means an admin switched it off, independently of whatever your probe would have returned. |

## 10. Deliberately unsupported

| Not supported | Why, and what to do instead |
| --- | --- |
| Hot reload of the loaded topology | The registry is frozen at startup so *which plugins exist* is a fact rather than a per-request question. Adding, removing, or upgrading a plugin — and flipping `enabled` in the TOML — are always restarts. An already-loaded deployment plugin's runtime on/off switch (§8) is a separate, restart-free layer sitting on top of that fixed topology, not an exception to it. |
...
| Anonymous plugin routes | The mount always carries a router-level session dependency. Public, sign-in-free pages are a core product decision, not a plugin one. The `auth.provider` callback is the narrow exception in purpose, not mechanism: it is a fixed core route and the plugin still contributes no route. |
...
- [ ] The rollback path is written down: previous wheel + previous UI package, or `enabled = false`, plus a restart — and the runtime switch (§8) as the first-response way to stop it serving while that rollback is prepared.

## 12. The sample plugin (`examples/extensions/arxiv-search`)

This repository ships one complete, **off-by-default** sample deployment plugin so that everything above has working code to read: `examples/extensions/arxiv-search/`. Nothing loads it by default. Verification happens in three distinct layers, and collapsing them into one claim of "no test lane" would be wrong: the *package itself* is in no default deployment and no default frontend build (enabling it is exactly the same two-variable decision as enabling a real out-of-tree plugin — name it in your own `EXTENSIONS_CONFIG` TOML, and point `SILICON_NOTEBOOK_UI_PLUGINS` at its `ui/arxiv-search` package); its *source-level tests and guards* run in the ordinary **G1** lane on every PR, the same as any other test in the tree (§12's ["two ways it differs"](#two-ways-it-deliberately-differs-from-a-real-out-of-tree-plugin) below); and *full-chain verification once the package is synced into the tree* is owned by **G2**'s `scripts/check_sample_plugin.sh` lane.
...
- **Its tests live in `backend/tests/`, not in the package.** §5.3 tells a real plugin to keep its tests in its own repository, and that instruction stands. This repository's backend lane only collects `backend/tests`, so a sample that followed §5.3 literally would ship tests that never run. Do not copy this arrangement into a real plugin. Once collected there, `backend/tests/test_arxiv_sample_plugin*.py` and the two files under `frontend/tests/{unit,guards}` are ordinary tests: they run in **G1** on every PR, not only in G2.
...
## 13. W3 authentication example (`examples/extensions/w3-auth`)

`examples/extensions/w3-auth/` is an independently buildable Python package for `auth.provider`; it has no frontend package and no plugin HTTP router. Its example TOML is explicitly `enabled = false`, so a default checkout neither imports it nor exposes SSO. Install it into the backend interpreter, copy the TOML outside the repository, supply the named environment variables for the HTTPS login origin, client id, client secret and optional CA bundle, then enable and select it through the core authentication policy. The package manifest uses the same `EXTENSION_API_VERSION` discovery check as every deployment plugin, and core never imports the example package directly.

The adapter implements the fixed W3 paths documented in its README pair: browser authorization, JSON authorization-code exchange, then JSON userinfo. Redirects are disabled; TLS verification stays on, with an optional custom CA; socket timeouts are capped by the host deadline. Userinfo query-token mode matches the available supplier example, while Bearer mode is an explicit setting to use only after the provider confirms it. Neither mode logs or returns a token, full URL, upstream response or exception text.

W3 `uid` must be an original non-empty string: it is not cast, lowercased, trimmed or truncated, and it becomes `username`. Display name falls back in the order `displayNameCn` → `displayName` → `uid`. The default also uses `uid` as `subject`; do not deploy that identity rule until the provider confirms its namespace, stability and non-reassignment contract. A separately confirmed immutable top-level field may be selected explicitly as `subject_field`, while `username` remains `uid`. Changing that field or `provider_namespace` is an identity migration, not a routine package upgrade. The package README and README_zh are the operational source for its private settings and this provider-confirmation gate.
