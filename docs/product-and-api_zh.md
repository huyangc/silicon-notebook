...
`CHUNK_ANSWER_BUDGET_CHARS`，不声称已阅读全文。同一个分类器（`overview_intent`）
也驱动逐步推理的首轮目录播种（见下文[集合枚举工具](#集合枚举工具)），所以同一句话
在两侧得到同一个判断，不会一边走目录、一边掉回相关性排序。
...
**谁能触发枚举：三个入口。** 除了 reflect 循环里的两个模型动作之外，还有**第三个触发者**：目录问法分类器（`overview_intent`，与上文[检索模式（问答）](#检索模式问答)里“通用问答的文档介绍”共用同一份判据，两侧对同一句话不会给出两种判断）。原问题被判为 catalog 型（例如“当前 notebook 的文章说明了什么”）时，逐步推理会在**首轮确定性地列一次来源目录**，不赌模型是否选中目录动作。它走的是与模型动作**完全相同**的那条执行路径，因此吃**同一个 run 级预算池**、记同一份续跑账目：这有一个必须知道的先手效应——**大库在低档位下，播种这一次就可能吃满 `enum_rows_per_run`**，之后模型自己再选枚举动作会按预算跳过（轨迹上是 `enumeration_budget`）；模型之后再请求同一份目录则按「本轮已全部列出」跳过（`already_enumerated`），不会把同一份清单列两遍、扣两次预算。播种的 `enumerate` 轨迹步带 `detail.phase="seed"`（模型主动选的那次不带），`_run_enumeration` 的每一条 `skip` 也同样带它——只给成功步打标的话，「播种被挡掉了」与「模型那次被挡掉了」在轨迹上完全同形。**目录播种沿用分类器解析出的参考库范围**：默认只列当前笔记本的文档——问句没提参考库（“总结当前 notebook 的文章”）与明确排除（“……不包括参考库”）在这里同为只列本库，只有问句明确说了“包括参考库”“和参考库”才把挂载的参考库一起列进来，与上文通用问答的文档目录通道逐字同一个口径。范围记进续跑账目，**模型后续的目录续跑沿用同一范围**：模型选的目录动作不带范围信息，若按默认（全参与集）重开第二条链，被排除的参考库文档就会从那条链混进答案证据、还要再扣一次共享枚举预算。只列本库的那次 `enumerate` 轨迹步带 `detail.local_only=true`（含参考库时不带这个键）。播种与图无关（有图无图都播），受限来源范围与 `allow_enumeration=False` 的调用方（Knowhow 智能补全）不播种——与两个模型动作同一把闸。播种列出的目录**算首轮进展**：`no_progress=false`、`stale` 从 0 起步，且不注入首轮空手提示。
