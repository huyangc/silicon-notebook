...
当前选择同时传入不读语料的问答意图预检、问答执行和新建深度报告。来源维度约束当前 notebook 的内容块、来源元素、知识证据与关系、图路径/PPR 输出以及报告检索；来源范围被**收窄**时，隐藏的 Memory/Knowhow 投影证据也不参与，因为这些内部来源没有面向用户的复选框。**未收窄的请求则保留这份证据**：正因为它们没有复选框，永远进不了提交的选择，所以入口层在冻结上限时把当时存在的隐藏投影来源与全部可见来源一并冻进去，只有真收窄才把它们排除。两种情形下的冻结保证不变——校验之后新增的来源，无论可见还是隐藏，都在上限之外；可见与隐藏两半出自冻结本来就要做的那一次读取，不多付往返。这份隐藏证据里两种来源的范围不同：Knowhow 投影是笔记本级共享的，会进入每位成员的上限；Memory 投影按创建者私有，只进创建者自己的上限，且过滤就发生在那一次读取里，所以共享笔记本绝不会把一位成员的私有 Memory 暴露给另一位。参考库维度则把未勾选的参考库从集合清单与集合地图、联邦检索、共提/社区扩展、图漫游、`follow_chain` 与证据装配中移除。
...
“范围为空”的判据是**两维同时**为空：只有本地有效范围与参考库有效范围都为空时，问答/意图预检/报告创建才返回 409。也就是说判据变成了“这次请求勾了哪些库”，而不是“挂了哪些库”。请求没有**收窄**的那个维度按 notebook 的真实**证据宇宙**作答，而不是一律当作非空——否则一个证据全部来自挂载参考库的 notebook，只提交 `base_scope` 且取消勾选全部参考库时仍会被放行，白跑一整轮检索；对报告更是白落一行记录加一次意图模型调用。反方向同样成立：真正的收窄以选择为准，用户把来源一个不剩地清空就是清空了，证据宇宙不得把它翻回来。

本地那一维的证据宇宙**不等于**可见导入来源数。它还包含 Knowhow 表格子、该用户的已确认 Memory 以及本 notebook 自己的知识图谱——这些都没有可见来源行，所以只有 Knowhow（或只有已确认 Memory）的 notebook 可见来源恒为 0，却照常可搜。按来源数判本地维度，恰恰会在这类 notebook 取消勾选全部参考库的那一刻把它拒掉；而且浏览器**默认**就走到这个形状：它“来源全选”的紧凑载荷在没有可见来源可列时，冻结出来就是空的 include 列表。因此 `NotebookSummary` 在 `ask_available` 之外另外暴露 `local_evidence_available`——同一批四个可用性判据里的**本地那三个**（任意 chunk、本地可用 KG、该用户的已确认 Memory），完全复用可用性判定本来就要求值的结果，不新增任何查询；空判把它与可见来源数一起读，因此该信号只会扩大本地宇宙、绝不会缩小。两个维度都没有收窄的请求根本不是在做选择，交给既有的可用性闸判断即可，它能看到本检查看不到的证据。浏览器同步禁用问答输入和新建报告控件，但仍可查看既有报告。

这个顶层复选框范围是当前 notebook 的硬上限。仅仅提交了显式范围对象本身，并不构成“已收窄”：API 入口把每一次提交——包括前端默认的全选载荷，每次请求都会带上——都冻结成显式 include 列表，所以单看线上格式（`mode: "include"`）无法区分“用户选了全部”与“用户选了一个子集”。是否收窄要在冻结那一刻比较所选集合与该维度全集的大小，而且所选集合与全集必须来自**同一次实时读取**。来源维度不能拿 `NotebookSummary` 上更早那次读取缓存下来的可见来源计数当全集：意图预检之后、范围冻结之前上传完成的一篇，会让缓存计数恰好等于提交列表的长度，于是一次真正的收窄被判成“未收窄”——而冻结出来的 include 列表确确实实把那篇新来源排除在外，限定模式该给的保护全部落空。成员校验也合并到这同一次读取上，因此“所选集合是全集的子集”是构造性成立的，请求的数据库往返数不增反减。全选提交冻结后正好等于全集，因此**不算**收窄，当前 notebook 的 PPR、私有 Memory、社区报告与语料画像照常保持开启，如同压根没提交过范围一样。只有真正小于全集的选择，才会让运行进入下文描述的那些模式。它与问题明确点名来源时生成的 `QueryIntentContract.source_scope` 审阅子集并非同一层合同。逐步推理会将审阅子集与复选框上限取交集：模型只能保持或进一步缩小用户选择，不能扩大。收窄复选框时，无法在遍历前安全应用当前来源谓词的持久化通道（当前 notebook 的全图/PPR/关系扩展、精确章节查找与报告整库画像）会被跳过；按来源限定的 chunk、元素和 KG 直接检索仍正常执行，base 库 KG 种子也可直接映射回 base 原文，不经过组合全图遍历。

**参考库维度与“限定模式”正交。** 仅取消参考库勾选、本地来源仍全选时，绝不能让运行进入上面那种收窄来源的限定模式：那会关掉当前 notebook 自己的全图/PPR/关系扩展、精确章节查找与报告整库画像，并让私有 Memory 完全不参与。用户不该为“少借一个参考库”付出“当前库检索能力被砍”的代价。因此通道按它问的是哪个问题分成两类：当前库通道（PPR、私有 Memory、社区报告、弱支撑关系探测、精确章节查找、报告整库画像）只看来源维度；跨库通道（集合清单与集合地图、联邦检索、社区扩展、图漫游、`follow_chain`、证据装配，以及知识图谱可用性判定）必须各自认参考库维度。可用性判定值得单独点名，因为它是一个**判据**而不是结果过滤：当前 notebook 自己没有图、而唯一带图的参考库被取消勾选时，本次运行按“没有知识图谱”处理，`kg_required` 如实回报，知识图谱模式给出“请先构建知识图谱”这句可操作的提示，而不是去遍历一份本次不许读取的图。
