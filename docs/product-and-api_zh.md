...
- 边可信与策展：`GET /api/notebooks/{id}/edge-review-queue` → `{items: EdgeReviewItem[], total}`（按审核优先级降序排列的边，`limit` 截断，默认 100；`total` 是队列的真实总量——对非 rejected 边做 seq-gated 的 `COUNT(*)`，与 `limit` 无关，前端可据此展示真实剩余总数而不只是拿到的这一页），`POST /api/notebooks/{id}/relations/{rel_id}/review`
