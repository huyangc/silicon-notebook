...
详情开启时，T 还报告本次 run 中首次成为可读内容的证据视图数，按最终准入的 K/D 摘录计算，不按候选池计数。身份是证据 key 与摘录内容；仅修改卡头/版本号或重新展示相同视图不算新可读依据。检索没有新候选时，真正新展示的详情仍可计数。这是可见性观察，不是事实支撑自评，也不证明问题覆盖完整；不改变既有 hard stale/步数上限或强制作答，供模型区别重复检索和读到新证据。其他 v2 档和 Legacy 忽略这些设置。
