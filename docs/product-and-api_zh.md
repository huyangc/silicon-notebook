...
与引用徽章同属一个视觉家族；渲染只加样式，不改变文本内容与复制结果。标记开句但要排在列表序号、项目符号或标题语法之后（写 `1. （推断）…`，不写 `（推断）1. …`），否则 Markdown 不再把该行当列表项；渲染前对四个模型正文渲染面做一次确定性归一，把写反的「标记 + 列表语法」对调为「列表语法 + 标记」，只调换位置、不改一个字，历史回答同样受益；复制出去的文本仍是模型原文。
