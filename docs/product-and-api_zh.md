...
笔记本 `name` 与 `purpose` 分别记录自动／手动状态。新建时使用占位标题、留空描述，
对应字段保持自动；提供自定义标题或非空描述，则对应字段为手动。显式 PATCH 某个
字段后，仅该字段变为手动，包括清空描述或设回占位标题；自动刷新不会覆盖它。
设置表单不提交未修改的字段，顶部标题跟随自动更新，同时保留正在输入的草稿。

来源类型修正也会刷新自动字段。来源解析／重新解析写入摘要后刷新自动字段；解析失败也刷新，但失败来源只贡献
标题，不把错误文本当作摘要。失败处理先清除已有摘要并保留当前处理状态，刷新
笔记本信息后再发布失败终态，确保轮询页面取得最终信息。删除来源后，在返回响应前刷新。输入覆盖当前笔记本
全部可见来源的标题、文档类型及已有摘要，按创建顺序稳定排列；排除隐藏的
Memory／Knowhow 投影与挂载公共知识库。仍在处理的来源先贡献标题，摘要就绪后再
更新。这里综合的是来源信息，并不通读全文：单篇来源摘要仍沿用已有的文档投影。

来源信息全部参与，不再只取前若干篇或每篇摘要的短前缀。较长输入分批生成并逐层
合并，不设来源数量截断。按稳定来源分组，并使用与发布相同的严格输出校验器接入
已有 LLM 响应缓存。追加来源或摘要就绪时复用未变分组，只重新生成受影响的上层摘要；
缓存沿用已有的端点／模型／参数／提示词键、容量及过期规则。关闭缓存或缓存被淘汰会
增加模型工作量，但不改变来源覆盖。同一运行时中的并发请求合并处理，参与的摄取／删除调用
等待最终刷新后才返回完成。数据库代次与各字段的手动标记共同阻止过期结果及
并发模型响应覆盖手动修改。模型不可用或生成失败时，描述回退为来源数量和类型，
多来源标题回退为资料集名称，不用第一篇的主题代表全库。删除最后一篇来源后，
仅自动字段恢复为空库文案。
...
| 护栏 | 值 |
| --- | --- |
| `NOTEBOOK_METADATA_BATCH_CHARS` | 默认 12000，范围 4096–100000；每批来源／分组摘要输入字符数，不含固定提示词 |
| `NOTEBOOK_METADATA_BATCH_SOURCES` | 默认 16，范围 2–128；递归合并的稳定分组大小，不是来源总量上限 |
| `NOTEBOOK_AUTO_NAME_MAX_CHARS` | 120；模型输出超限则整轮回退，不静默截断 |
| `NOTEBOOK_AUTO_DESCRIPTION_MAX_CHARS` | 1000；模型输出超限则整轮回退，不静默截断 |

SQLite v75／PostgreSQL 0055 保守保留旧库中全部非占位标题，因为旧数据没有记录标题
是否由用户手动设置；仅旧占位标题进入自动模式。已有描述的自动／手动状态保留。
不推断历史归属，也不在启动时调用模型回填。副本继承字段归属，但显式指定副本
名称时，该名称为手动。

## 当前范围

Silicon Notebook 是团队知识工作区，后端使用 FastAPI，前端使用 Next.js。
开发和生产默认部署 PostgreSQL 16，SQLite 仍受支持；每次部署通过 `DATABASE_URL`
选择一个后端，不做双写或双读。安装、模型服务、离线回退与不使用 Docker 的部署方式见
[部署与配置](./deployment-and-configuration_zh.md)。
...
- [全局问答](#全局问答)、[API](#api)与[当前限制](#当前限制)。

全新数据库仅初始化本机管理员，不预置演示笔记本或合成来源。
...
配置 MinerU 后由它解析 PDF／DOCX／PPTX／XLSX；不可用或产出不可用时按以下本地链回退：

| 格式 | 本地解析与限制 |
| --- | --- |
| PDF | PyMuPDF4LLM 版面 Markdown，再回退 pypdf。 |
| DOCX | mammoth 保留标题、列表和表格（`table_html` 元数据），最后回退 python-docx 平面抽取；不落内嵌图片，丢弃 base64 图片 URI。 |
| PPTX | python-pptx 保留表格、图表标题、组合形状与讲者备注，最后回退原始幻灯片 XML；不落图片。 |
| XLSX／XLSM | openpyxl 保留所有单元格值。 |
| 旧版二进制 XLS | 固定使用 `xlrd`，不走 MinerU。 |
| 旧版 DOC／PPT | 不支持，需先另存为 DOCX／PPTX。 |
...
紧邻图片的 blockquote 可提供 `metadata.description`，与图注合并成可检索元素文本，并在无图注时作为引用附图标题。允许空行分隔，不允许链接定义等实际源内容插在中间。首行必须仅为 **图片描述** 标记（加粗、末尾冒号均可省略），或标记后加冒号再接文本；「图片描述如下」不算。引用块必须含实际渲染为非空的描述，且不能含围栏／缩进代码或 HTML 块；列表、标题、表格及嵌套引用全部包含。只有标记、`<br>` 或空 alt 图片的块仍作为普通引用。

整段描述生成一个图片元素，不再重复生成段落元素；KG 抽取仍读取原始来源片段。多段描述不经 600 字符 chunker 拆分，向量化仍受既有输入截断上限约束，词法检索保留全文；引用附图标题仍受通常上限约束。本地图片可先用 [`embed_md_images.py`](../scripts/embed_md_images.py) 转为 data URI。
...
| 账号操作 | 契约 |
| --- | --- |
| `PATCH /api/admin/users/{user_id}/role` | 授予／撤销管理员；内置管理员及当前管理员自己的角色受保护。已有会话在下一次请求观察到变更。 |
| `PATCH /api/me/password`，body `{old_password, new_password}` | 旧密码错误或新密码为空：400；成功保留当前会话并撤销该用户其他浏览器会话。 |
| `POST /api/admin/users/{user_id}/reset-password`，body `{new_password}` | 必须为真实管理员会话（匿名 `auth_optional` 回退返回 403）；撤销目标用户全部浏览器会话。 |
| 修改／重置内置 `admin` 密码 | 409；应改环境变量并重启。界面隐藏自改入口并标记为受保护账号。 |

改密与重置均不撤销 Agent 长期凭据。网页按策略显示登录／注册，顶栏显示当前用户名与退出入口。

使用统计页先对完整 `GET /api/admin/users` 结果排序，再分页（默认 20 行，可选 20／50／100，各数据列可升降序）。展开用户显示摘要和完整的 owner-only `GET /api/admin/users/{user_id}/notebooks` 清单，每页 20 本。总览包含该用户在共享笔记本中的活动及删除笔记本后仍保留的活动，因此与自有笔记本明细不必相加一致：

| 字段 | 归因与范围 |
| --- | --- |
| `questions` | 用户拥有的全部状态 `ask_jobs` 提交（含失败／取消）及 `global_ask_jobs`；不计会话容器或其他成员的问题。全局任务不归入单库明细。 |
| 报告总量 | 按报告创建者，包含在共享库创建的报告。 |
| `sources`／`storage_bytes` | 可见来源数量／`file_size` 合计；存活库按 `COALESCE(uploaded_by, notebook.created_by)`，已删库保留项按保留的 actor，空值回退删除时的 owner。单库明细计自有库全部可见来源，不区分上传者。 |
| `last_active` | 可见上传、问答提交（含全局）、报告创建的最新 `created_at`，含已删库保留活动；后台更新与隐藏 Memory／Knowhow 来源不刷新它。 |
| `last_seen` | `users.last_seen_at`：登录及后续已认证浏览器请求按 `AUTH_SESSION_TOUCH_INTERVAL_SECONDS` 节流刷新；退出／撤销后仍保留。Agent 凭据不刷新；null 表示引入此列后尚未访问。 |
| `questions_30d` | 最近 30 天的笔记本／全局问答提交，含保留项。 |
| `questions_failed`／`reports_failed` | 仅 `status='failed'`，含保留项与全局问答；取消不计。 |
| `kg_builds` | 用户创建的全部状态 KG 任务；删除笔记本后不保留。 |
| `memory_count` | 存活库中该用户非 `rejected` 的 Memory。 |
| `knowhow_tables` | 存活库中该用户创建的表，包括归于复制者的深拷贝副本。 |
| `joined_notebooks`／`groups` | 加入他人存活共享库的成员关系数／群组成员关系数。 |
| `conversations` | 已废弃但兼容保留的字段，包含该用户的全局会话。 |

实际上传刷新提交者的 `last_active`，包括向他人的共享库上传。深拷贝不刷新原上传者或接收者；副本的空 `uploaded_by` 在资产统计中仍回退为库 owner。引入归因字段前的上传活动只能尽力归给当时的 owner。

展开摘要不新增主表列或排序键：第一行显示最近访问、存储、30 天提问、问答／报告总量及非零失败数、图谱整理次数；第二行显示记忆、Knowhow、加入共享库及群组。缺少访问时间显示「—」；容量按 1024 进制 KB／MB／GB／TB 保留一位小数，小于 1 KB 显示整数字节。

每库上传文档数默认 `USER_UPLOAD_DOCUMENT_LIMIT=20`，管理员拥有的库豁免。管理员通过 `PATCH /api/admin/settings/upload-limit-default` 修改全局默认，通过 `PATCH /api/admin/users/{user_id}/upload-limit` 设置用户覆盖值（`null` 恢复继承）。管理员可发布公共知识库：普通用户列表隐藏它，但参考库选择器可见，显式挂载后才参与检索。旧 schema-20 升级不自动回填挂载。
...
- 左栏：用户导入来源文件，实时显示 parse-status（绿色仅给 `extracted`，其余处理中为橙色），并按后果分级显示来源异常徽标（完整性问题如解析失败标红、仅影响检索的问题如部分内容未分析标黄；待补全等中性待办状态只在来源详情显示），支持详情预览、删除，以及由问答和新建深度报告共用的检索范围复选框。来源详情每次只取并渲染 40 个 element 的有界页（API 单次上限 100），前后页按需加载；引用跳转由后端解析到包含目标 element 的页，因此打开大文档不会一次水合和挂载全部元素。元素区是**阅读视图**而不是记录清单：元素连成一篇文档、不再每段一张卡片；只在相邻元素跨页/跨幻灯片/跨工作表处插一条细分隔线（`第 N 页` / `幻灯片 N` / `工作表 X`，取自元素 metadata）；标题元素按真正的标题渲染（按解析器层级落到 h3–h5）；解析器的 `location_label`（`PDF p.1 block 3`、`DOCX paragraph 12` 之类）一律不显示——它仍是问答/报告证据的引用坐标，引用跳转照旧滚到目标元素并闪烁高亮。类型标签只给正文形状看不出来的类型（演讲者备注、图片说明、经验表单元格）；段落、标题、表格、公式、图片、代码、列表项、幻灯片文本与表格行不贴标签，表格行改显示行号（`第 N 行`，DOCX 表格为 `表 T 第 N 行`）。规则集中在 `frontend/app/source-element-display.ts`。所有面向用户的来源计数只计这组可见的导入来源，排除隐藏的 `memory` / `knowhow` 投影来源。网络来源检索暂不开放。
- 主栏：四个 tab——**问答**（Ask）、**知识库**（Knowledge）、**记忆**（Memory）、**深度报告**（Deep Report）。Ask 提供逐句 `[k_i]` 引用、两种内置检索模式、多轮会话、实时推理轨迹与反馈；鼠标悬停问题气泡或回答卡时在下方显示时间，点击后固定显示，点击其他位置后恢复悬停逻辑。问题采用已持久化的网页端提交瞬间，回答采用 `AskResponse.answered_at` 返回的权威答案写入瞬间（旧 payload 从 `answers.created_at` 投影）。时间按浏览器本地时间格式化：今天只显示时间；本周（周一为一周起点）内的其他日期显示星期与时间；超出本周显示日期与时间，日期和星期二选一；今年省略年份，其他年份显示年份。会话历史收进 Ask 顶栏的单行 `历史 N` 入口和可展开管理面板，旁边的 `+` 会直接开始新会话。历史按带亚秒精度的最近活动排序并显示活动时间；首轮问题一提交就会立即出现，即使在 `started` 到达前切到同库旧会话，模型仍在回答时也能重新打开。终态历史摘要按当前 notebook 独立刷新，不依赖原 run 继续占有回答区；同库列表调用会收敛到最新请求。加载 notebook/会话的最新详情期间，输入框与模式控制保持禁用。Knowledge 负责动态类型浏览与治理；Memory 只显示当前用户绑定在此 notebook 的私有记录；Deep Report 负责两阶段报告、大纲审阅、进度、导出、取消和删除。问答输入框中 `Enter` 发送，`Shift+Enter` 保留换行；模型处理中锁定输入与模式切换，发送按钮切换为中断控制。**停止只有一种风格，笔记本内问答与全局问答共用**（规则、判据与提示文案的唯一出处是 `frontend/app/stopped-turn.tsx`）：还**没有过程输出**就停止——问题理解阶段、提交尚未返回、或轨迹里只有 `intent`/`start` 两类前置步——问题回到输入框，对话里不留记录；**已有检索 / 推理过程上屏**再停止，这一轮留在对话里（问题、已有轨迹与「已停止回答」提示），问题不回输入框，「编辑问题」把它放回输入框，**下一次提问替换这条记录**。笔记本内问答的被停止记录只活在页面内存里（后端对被取消的提问不落任何可见记录），刷新即无；全局问答的被停止作业持久存在，刷新后仍是会话里最新的一轮、仍可被下一次提问替换。两个面在「没有过程输出就停止」时给同一组一闪而过的轻提示——「已取消问题理解」「已返回修改问题」「已中断回答」（笔记本内问答走页面 toast，全局问答在输入区上方自带一条自动消失的状态行）；「已有过程输出再停止」不另给提示，留在对话里的那条记录自己会说。transport 断连只停止向当前客户端继续推送；导航、刷新或 transport 丢失后 detached Ask job 仍在后台运行并可保存最终回答。用户点击中断则调用 `POST /api/notebooks/{id}/ask/jobs/{job_id}/cancel`，由后端设置取消事件，使 worker / LLM 路径停止，且不保存被取消的最终回答；如果点击时首个 `started` 尚不可读，界面立即恢复草稿，但该 run 的 transport 只继续读取到取得 job id，完成后端取消后即 abort。主工作区保持两列且没有固定 Studio 右栏。
...
界面标签与可展示的后端 `user_error(...)` 消息遵循[界面词汇约定](./ui-vocabulary.md)，内部标识符与已持久化默认值保持不变。未知枚举使用中性文案；用户自定义对象类型／字段名可以有意原样显示。守卫实现与验证要求由[开发文档](./development_zh.md)负责。
...
全局问答入口是已登录集合页和笔记本工作区右下角的气泡。集合页上气泡为 58px；笔记本工作区里
它缩到 40px、退进问答输入框右下方的空白角——输入框不为气泡让位，左右留白保持对称。点击打开小聊天窗，放大后以完整
页面展示历史和问答。收起、重新打开和切换大小保留同一会话、草稿、选库范围与
任务进度；收起不取消任务。嵌入聊天不改动所在页面的 URL，首次打开后才加载问答数据，退出
登录清除本地状态。重新打开会刷新笔记本列表而不重置聊天；错误恢复会保留当前会话、草稿
范围和运行任务的进度轮询。`/ask` 仍可独立访问并支持会话链接。全局会话属于发起用户，
不创建用于承载会话的空笔记本，也不修改参考库挂载。页面沿用共享页头、配色、按钮反馈与
答案 Markdown 排版，提供历史会话、可搜索的笔记本选择器、持续可见的范围提示、后台任务
状态。引用与笔记本内问答共用同一张小卡片：点击行内引用标记，在标记旁弹出卡片，显示被引段落、
所属笔记本与库层级，并提供「打开笔记本」链接，跳到该笔记本并定位到被引来源（普通点击同时收起
聊天窗；按住修饰键点击在新标签页打开，聊天窗保持不动）。答案下方不再列出引用清单，也没有侧边
原文阅读栏；需要笔记本工作区承接的入口（查看原文、知识图谱、Knowhow 行、导入）不出现。
回答里的附图照常显示——正文内联的引用图片区块与引用卡里的「本段附图」都在，只是没有页面级的
放大预览。每张图经该引用**所属笔记本**的取图端点读取，服务端每次请求都复核当前用户对那个库的
读权以及资产所属库是否在该笔记本的有效参与集内。笔记本内问答仍恒经当前笔记本代理，口径不变。
卡片打开时按 Esc 只关闭卡片。移动布局保留范围选择、发送与停止操作。
问题在点击发送的当下就上屏（与笔记本内问答的 pending 轮同一时刻），`reasoning` 的问题理解步骤
实时画在它下面，用的是与笔记本内问答同一组前端合成步，并在作业建好后接在作业自己的实时轨迹
前面；取消或失败时问题回到输入框。
浏览器跟随进行中的作业走推送流（`GET /jobs/{job_id}/stream`），流打不开、出错、或在没有终态帧的
情况下结束时回落到轮询。页面隐藏不影响推送流——它是推送，没有需要暂停的东西。
回落后的轮询在无新进度时从 1.2 秒退避至最多 15 秒，浏览器页面隐藏时暂停，恢复可见立即查询；覆盖回执
与**轨迹步数**任一变化都算进度，所以逐步推理连续出步时轮询保持在 1.2 秒。
收窄范围时在输入区提示旧范围历史语境不再参与。
未充分接地的回答明确显示警示，即使包含部分引用也不隐藏。每完成或跳过一个库，单独更新
覆盖进度字段，轮询及检索中途取消均保留已完成进度。

全局问答与笔记本内问答用**同一套引擎**，输入区提供同一个引擎选择器，可选 `chunk`（默认）与
`reasoning`。部署扩展引擎不进入该选择器，后端对扩展引擎一律返回 422；无法识别的 `mode` 同样
422。作业完成后返回与单库问答同构的标准答案结构（字段 `answer`）；归一之前的历史作业保留旧形状
（字段 `response`）且只读，读取正文、引用与推理轨迹时两种形状给出同一份投影。答案不写进任何
笔记本的问答记录，只存在全局会话与任务里。`reasoning` 的提交先走一次不读语料的**问题理解预检**，
界面展示理解结果供审阅，确认后才带着理解结果建任务；运行中的推理轨迹随轮询逐步出现，完成后
以答案自带的轨迹为准。`retrieval_effort` 在全局问答 v1 恒为 `standard`：请求即使带 `deep` 也按
`standard` 执行，界面不提供档位——`deep` 的推理轮数乘以 8 个库超出检索阶段时限的合理范围，只会
把多数库报成未检索。

**记录口径**：一个全局问答作业留下与一条笔记本内问答作业**同一份记录**，唯一的差别是检索方式。
每个作业记录：问题、引擎（`mode`）、状态、答案（含引用与推理轨迹）、提交瞬间 `asked_at`、末次
转移瞬间 `updated_at`、这一轮的提交入口 `submitted_via`，以及反馈 `feedback` 与写入反馈的瞬间
...
**问答后学习链**：一次全局问答完成（作业落为 `done`）后，推进与笔记本内问答**同样的三条**后台链路，按跨库运行能够归因的方式计：笔记本理解的覆盖层链路对本次**实际检索到或引用到**的每个库各计一次「该成员在该库完成了一次提问」（一次联邦调用都没发生的作业按参与集扣除被跳过的库计），其轨迹取样按同一条归因规则把该成员计入该库的全局问答与笔记本内提问一起读（被解析但未检索、未引用的库不见到那次运行）；检索经验库只计入**全局分区**一次（仅 `reasoning`）——联邦运行的打法不描述任何单个库，而全局运行注入时读的也只是全局分区（锚点库只是命名，不带它的分区）；检索偏好归纳按人计一次。取消或失败的作业不计；一个库都没有检索到的完成作业（各腿全部跳过）仍按人计入检索偏好与全局经验分区并通知观察者，只是覆盖层无库可计。部署扩展的 `ask.completed_observer` 收到的是**一条**通知（与笔记本内问答相同）：每个观察者的上下文都带 `scope="global"`；持有 agent-profile 通知能力的观察者另得到 `notebook`（锚点库，即运行的 `nominal_active`，不一定在计数列表里）与 `notebooks`（本次计数的库列表，可能为空），其他能力的观察者 `notebooks` 恒为空——与它们本就看不到 `notebook` 同一条身份投影规则。
...
历史上下文重新进入模型。范围回执区分允许参与、实际检索、未检索及中文原因、部分检索腿失败
和实际引用的库。一次 `reasoning` 作业会发起多轮联邦检索，回执按整次作业聚合：某个库在**任何
一轮都没有成功**才报成未检索（原因取最近一轮），成功过但某一轮失败则计入降级清单；检索为空
仍计入已检索。因此降级清单现在只表示「该库有部分检索腿失败」，不再承担词法降级的逐库披露。
一次联邦调用都没发生的作业（文档概览、集合枚举这类直接作答）不把任何库报成未检索。
跳过原因只陈述发生了什么，
不猜测「索引损坏」之类的原因：只有**确实跑超时**的检索才请用户收窄范围；阶段时限到点时仍在排队、
一次查询都没发出的库说明「检索未开始」；其余一律请稍后重试。可供排查的原因码以内容无关的
`chunk_federation_skipped` 事件单独记录。权限变化
仍拒绝交付，不把「未引用」解释成「没有资料」。只有完成且范围兼容的轮次进入追问语境，
通过共用问题改写解析指代；既往助手回答仅用于理解追问，不作为证据。
提交时刻冻结每个参与库当前可见的来源清单；答案产生后逐条复核引用：来源必须仍在该清单内、
仍然可见；走过联邦原文通道的元素还要求检索时的文本指纹与此刻读到的一致，且仍归属同一来源。
「检索时的指纹」与这次 run 检索到的那段原文出自**同一个数据库快照**，且只有在那段原文此刻仍与
run 读到的逐字相同时才被采信：来源重新入库时元素 id 是确定性复用的，只按 id 单读指纹可能读到
的是**替换掉**答案所依据文字的新内容。段落被改写或已消失，则它背后的全部元素一律拒绝。
指纹这一关复核的是**整段原文的全部支撑元素**，不只引用卡上印的那一个：一段原文由若干来源元素
拼成，引用只带其中首个，其余元素在它旁边一并复核——引文中段被改与开头被改同样作废整份答案。
没走过该通道的引用（文档概览、集合枚举、图对象）只核天花板、可见性与来源归属。任何一条通不过，
**整份答案作废**并返回中文重试提示：标记为未接地、清空引用与证据附件，推理轨迹保留。不再剔除
失效引用后用剩余段落重新合成一次——重新合成会重新检索、重新冻结，得到的是另一份答案顶着这一份
的身份。

跨库检索走联邦原文通道：参与库 × 子查询的任务表一次性提交到进程级共享执行器，不逐库轮询。
没有知识图谱的库也能参与；普通参与库作为平等证据来源，冲突应保留来源与适用条件。候选与模型
上下文使用有界预算，全局范围不等于完整阅读每篇文档，也不承诺穷尽性清单或逐库完整审计。

全局提问没有「当前笔记本」：交给引擎的笔记本只是解析后范围里的第一个库，用作命名锚点，不带
任何检索特权，**每条引用都标出所属笔记本**（含第一个库）。因此按「当前库」定义的通道在全局
问答里整体不启用，如实披露而不是悄悄降级：库内隐藏的私有 Memory 投影、概念漫游（PPR）、所选
来源图激活、「需要建索引」提示，以及 `reasoning` 的原文元素
检索臂（含确定性通道全空时的自动补腿）；生成式问题补召回在联邦腿上同样关闭。表格分析臂只覆盖
第一个库，并按逐库来源天花板只减不增。社区/对比题的兄弟实体可用并覆盖全部参与库；`chunk` 模式
的对比子查询按库轮转并受总量上限约束，单库行为不变。这些通道的联邦化登记在 `fangan_todo.md`，
等待产品裁决。

双语关键词补召回臂不关闭，而是逐库联邦化（每条语义腿本来就带一路按子查询原文的全文检索；这里说的是
`chunk` 模式与 `reasoning` 无图首轮播种用的那一路关键词串检索）。它对每个参与库各跑一次单库同款
关键词检索，与语义腿共用同一个共享执行器、公平窗口与逐库预算，各自带该库冻结的来源天花板；没有
可见来源的库不发查询。它是 fail-open 的补召回，整条臂（含逐库准备读取）只给一份
`GLOBAL_ASK_NOTEBOOK_TIMEOUT_SECONDS`，不吃满整段检索阶段时限；它的超时也不会关掉该库语义腿的全文
检索。每个库沿用单库的召回窗（`CHUNK_RECALL`）；合并时按各库自己的
关键词得分排序后轮转交错（每库先出第 1 名，再出第 2 名……），同一段原文只保留先出现者，总数以
`GLOBAL_ASK_CANDIDATE_LIMIT` 封顶，在交给调用方的这份合并列表里一个库不能靠数量占满（`reasoning`
随后按分数从中重选播种席位）。某库关键词腿超时或失败只意味着该库这一路没有结果：不进入
「已检索/已跳过」回执（覆盖口径只由语义腿决定），任何失败都不上横幅。交给调用方的关键词段落同样
登记检索时刻证据，引用复核与语义段落同一口径，但不产生覆盖回执。这条臂每次调用发一条
内容无关的 `ask_stage` 事件（`stage: "global_keyword_arm"`），只含参与库数、有结果的库数、合并条数、
失败库数与耗时。`GLOBAL_ASK_KEYWORD_ARM_ENABLED`（默认 `true`）是回退开关：设为 `false` 时全局问答
重新关闭这条臂；单库问答不读它。

精确标识符查找臂同样逐库联邦化。问题里没有可探测名称时零开销：先用同一把零 I/O 的标识符判定，
不读参与库、不起任何一条腿、不发事件。否则对每个参与库各跑一次单库同款查找（每库沿用
`EXACT_LOOKUP_*` 上界），与关键词臂共用同一套扇出和同一份逐库预算，各自带该库冻结的来源天花板；
没有可见来源的库不发查询。精确查找按每库冻结天花板过滤命中：探测结果先按该库冻结的来源天花板
过滤，**再**分组、分节名额，所以天花板外的来源（隐藏的 Memory/Knowhow 投影、提问之后新增的来源）
既不占节名额也不会被整节取回；探测自身的 `EXACT_LOOKUP_FTS_K` 窗口在过滤之前取，这类行仍可能占掉
其中一部分。合并以**整节**为单位：每库先出第 1 节，再出第 2 节……，已取过的段落剔除，总数以
`GLOBAL_ASK_CANDIDATE_LIMIT` 封顶且绝不切断一节（剩余额度放不下的节整节跳过，后面更小的节仍可进）；
每条段落保留精确查找标记与所属笔记本。保底席位按库分配、总数不变：`chunk` 模式 `mix` 选择里的
`EXACT_SECTION_RESERVE`，以及 `reasoning` 单次合成前缀与按节合成里向每节注入的未绑定精确段落共用的
`REASONING_EXACT_RESERVE`，在返回了精确段落的库之间平均分配；优先顺序按各库最佳精确段落的相关度
降序（同分按首次出现），余数给排在前面的库，库数多于席位时只有前几个库各得 1 席，某库段落少于分到
的席位时空出的席位按同一顺序补给仍有段落的库——一个库的大节不能占满全部席位，也不会有席位在别的库
还有段落时空着。`chunk` 模式 `multi` 选择同样让每个库的精确段落各占一个配额组。段落只来自一个库时
（所有单库问答）这些都与以前完全相同。失败、超时、回执、横幅与证据规则同关键词臂：某库这条腿失败只
意味着该库没有精确段落，不进入「已检索/已跳过」回执、不上横幅；交给调用方的段落登记检索时刻证据。
`reasoning` 的一次 `exact_lookup` 动作仍按一次（现在是一次联邦查找）计入每 run 的次数上限。这条臂每次调用发一条
内容无关的 `ask_stage` 事件（`stage: "global_exact_arm"`），只含参与库数、有结果的库数、合并段落数、
合并节数、失败库数与耗时。`GLOBAL_ASK_EXACT_ARM_ENABLED`（默认 `true`）是回退开关：设为 `false` 时
全局问答重新关闭这条臂；`EXACT_LOOKUP_ENABLED=false` 仍全局关闭它；单库问答不读全局开关。

HTTP 入口均位于 `/api/global-ask`，要求登录：`POST /ask` 返回可轮询的任务，请求体可带
`replaces_job_id`（「编辑后重发」：只能指向该会话**最新**且状态为 `cancelled` 的作业，它在建新
作业的同一事务里被删除，仍带自动标题且别无他轮的会话标题随之改为新问题；过期的替换请求返回
409，同一 `client_request_id` 的重试照常回放）；作业在 `POST /ask` 的返回与此后每一次读取里都回带
它提交时的 `client_request_id`，响应丢失的客户端据此在重读的会话里认出**自己那条**作业——问题文本
相同不构成身份（两个标签页可以提交同一句话）；公开分享投影不含该字段；
`POST /intent` 与 `POST /intent/stream` 是 `reasoning` 的问题理解预检，请求体为
`{question, conversation_id?, notebook_scope?}`，返回与单库问答同一份问题理解合同。预检自己
鉴权，并按与提交**完全相同**的那段代码解析范围、复核读权，既不建任务也不写任何表；流式版本
**先开流**、把范围解析与鉴权也放进心跳覆盖的那段工作里：这一步要两次复核读权并冻结至多 8 个库的
可见来源清单，大库上是秒级，放在第一帧之前意味着这段时间响应一个字节都没发出去，代理 / 网关的
首字节或空闲超时会把连接掐掉，浏览器只能报「问题理解没能完成」（笔记本内问答的预检毫秒级就发出
第一帧，此后每 5 秒一次心跳）。拒绝并不因此丢失：`user_error()` 写给用户的 4xx 经 `error` 帧的
`status` + `message` 带出（整句中文、真实状态码），其余失败仍是不带内容的稳定错误码，并在服务端按
异常**类名**记一行日志；要真实 HTTP 状态码的客户端用阻塞的 `POST /intent`。客户端断开即取消该次
理解调用。
`GET /jobs/{job_id}` 读取进度或答案。
`GET /jobs/{job_id}/stream` 把同一个作业以 NDJSON（`application/x-ndjson`）推给已连上的客户端，
它只是那次读取的加速器，绝不是第二条执行通道：事件为 `started`（恒为第一帧）、`progress`
...
读权在**每一帧发出之前**复核（无人在看时不查），被收回即刻以 `gone` 结束，不再多推一步——与轮询
「每次读取当场鉴权」同口径；
...
向该客户端继续推送，与笔记本内问答同一条规则，唯一取消入口仍是 `POST /jobs/{job_id}/cancel`。
从未打开推送、中途丢失、或流在没有终态帧的情况下结束时，轮询仍是兜底；MCP 合同不变：MCP 依旧按页
读 `GET /jobs/{job_id}` 跟随作业，不走这个端点。
`POST /jobs/{job_id}/cancel` 停止执行，
`?discard=true` 表示「还没有任何过程输出就停止」：**本次调用读到时仍在运行**、随后停下的作业（须为会话最新一条；worker 抢先写下 `cancelled` 也算）连同
它刚开出来的空会话一并删除；已完成的作业、以及此前已经停止的作业（它可能正是按规则要留在对话
里的那条记录）绝不会被丢弃；
...
分支的原始分数直接用于全局排序。
模型上下文由同一套引擎按既有预算组织，全局问答不另建一份上下文编排。模型未知引用触发共用
答案重试，不保留无法绑定依据的结论。
`GLOBAL_ASK_HISTORY_TURNS` 默认 10、最小 0，控制可用的既往完成轮数。
`GLOBAL_ASK_MAX_NOTEBOOKS` 默认 8、取值 1–8：8 是产品上限而非部署旋钮，配置更大的值在启动校验
时失败；解析范围超过上限时拒绝整次请求，不静默截断。拒绝文案是可操作的，而不是只声明上限：
`all` 范围解析后超限时要求「选择不超过 8 个笔记本」——没提交过范围的调用方（尤其是 MCP 客户端）
无从「缩小」一个它没给过的范围。网页端先一步避免这种情况：用户可读笔记本超过 8 个时预选
来源数最多的 8 个、最多可改选 8 个；不超过 8 个时默认全选。
`GLOBAL_ASK_MAX_CONCURRENT` 默认 4、最小 1，限制每进程任务及正在受理的请求；满额拒绝新任务
并提示重试，并发预算纳入 PostgreSQL 连接池启动告警。
关闭服务先取消任务，最多等待 `GLOBAL_ASK_SHUTDOWN_TIMEOUT_SECONDS`（默认 5，大于 0）秒。
`GLOBAL_ASK_RETRIEVAL_TIMEOUT_SECONDS`（默认 30，大于 0）是**每一次联邦检索调用**的阶段时限，
不是整次作业的绝对时刻：`reasoning` 每轮检索各算一份，轮与轮之间的模型调用不占用它。
`GLOBAL_ASK_NOTEBOOK_TIMEOUT_SECONDS`（默认 5，大于 0）是逐库预算，从该库**真正向数据库发出
查询**的那一刻起算；排队、等待执行位都不计入，所以一个从未发出查询的库不会被说成「检索超时」。
参与库并行检索，进程级上界为 `GLOBAL_ASK_RETRIEVAL_CONCURRENCY`（默认 4，范围 1–8），
因此检索阶段不再是各库耗时之和。该上界是一个进程级共享执行器，公平份额按**在途联邦调用数**
分配（执行位数 ÷ 在途调用数，每次补位重算）：一次 `reasoning` 作业会同时发起多次联邦调用，
按作业分份额会让它独占整个池、其余腿一路排到时限耗尽。回执顺序与证据选材按解析后的参与库
顺序，不按完成顺序，同一问题在同一组库上给出同一份答案。
跳过原因码只有四种：`timeout`（逐库预算到期或数据库取消语句）、`saturated`（剩余预算内借不到
连接，查询根本没跑）、`queue_deadline`（阶段时限到点时该库仍在排队，一次查询都没发出）、
`unavailable`（其余）。
SQL 执行和连接池获取使用剩余时限；数据库网络故障的传输等待可能超过服务器 SQL 超时，
因此不承诺网络异常下的严格墙钟截止。原生 ANN 不能强制抢占，超时结果在加载候选或合成前丢弃。
共用问题改写与一次问题向量生成发生在此阶段之前，受已有模型服务超时控制，因此这些配置
不是整份答案的硬截止时间。冷加载守卫对每个参与库（含第一个库）一视同仁：**大库**这一腿只借
已经常驻的 scale 索引，不冷加载共享索引、ANN 句柄或整库向量矩阵，因此一次跨库提问不会挤掉
单库问答依赖的热索引。权限自始不变——水合后仍在打分前剔除冻结清单之外的候选。
语义召回不可用时采用有界词法召回和候选向量评分，包括跨语言召回可能不完整。任务和答案均返回
...
来源侧栏初始全选所有可见导入来源，每行提供复选框，并提供“全选”/“清空”。当前选择同时传入不读语料的问答意图预检、问答执行和新建深度报告，约束当前 notebook 的内容块、来源元素、知识证据与关系、图路径/PPR 输出以及报告检索。范围被收窄时，隐藏的 Memory/Knowhow 投影证据也不参与，因为这些内部来源没有面向用户的复选框。这份隐藏证据里两种来源的范围不同：Knowhow 投影是笔记本级共享的，会进入每位成员的上限；Memory 投影按创建者私有，只进创建者自己的上限，且过滤就发生在那一次读取里，所以共享笔记本绝不会把一位成员的私有 Memory 暴露给另一位。这条上限同样管住**知识证据的二次读取**：KG 命中进答案提示词时，它的定义、引文片段与引用定位不是复用召回时那份证据，而是按对象 id 重新查一次原始证据列；那次重查现在过同一条来源上限，所以一个对象可以凭范围内的来源被召回，但它贡献的那句原文、以及引用卡指向的来源，恒是范围内的那些；若一个对象在范围内一条证据都不剩，整条命中被丢掉，而不是留一个没有引文的名字。已挂载的参考库是独立参与者，始终保持在范围内。非当前笔记本的参与库按**该库当前可见的来源**这一条天花板贡献，所以它的隐藏 Memory/Knowhow 投影是结构性不参与跨库原文通道，而不是事后被过滤掉——请求用户通常不是参考库的成员，而那半边隐藏证据本来就按成员收。取消勾选某个参考库会在任何查询发出之前把它移出参与集，它因此对任何通道都零贡献。
...
两条二阶代价已登记并接受：图漫游与 PPR 的过滤在遍历/截断**之后**，因为联邦图按 scope 无关的键在进程级缓存——被排除库的节点仍占扩散名额、仍可当中转跳，但它的内容一个字都不会进渲染出的 prompt、也不可被引用。整图规模守卫同样保持 scope 盲，所以取消勾选一个超大参考库并不会把那道守卫重新关掉——这是刻意的：它守的那两张图仍然按**全部挂载库**建，只把守卫收窄等于放行它本来要拒绝的那次构建。
...
每个用户持久化一份 `user_profiles.ui_mode` 偏好（`"auto"` 默认 / `"advanced"`），经 `GET /me` 的 `UserProfile.ui_mode` 下发，并可通过自助端点 `PATCH /me/ui-mode`（body `{"ui_mode": "auto" | "advanced"}`；非法取值返回 422）修改。早于该字段的旧后端不会返回它，前端把缺失值按 `auto` 处理。高级模式就是本文档通篇描述的完整界面，与既有行为逐位一致：问答仍按前端选中的分组、引擎与检索档位执行。自动模式是从头像菜单开关的刻意精简界面：问答区不渲染「通用问答」「深入分析」或任何引擎/检索档位选择，只保留问答框；浏览器直接提交内置 `mode="reasoning"`、`retrieval_effort="standard"`，走与高级界面相同的 `/ask/intent` 意图预检——问题清晰时自动确认，出现阻断性歧义时内联审阅后再继续；后端不再有 corpus-blind 路由模型调用，也不存在 `chunk` 保守回退。深度报告研究深度控件仍隐藏并固定「标准」（`depth=2`），上文的来源/参考库范围复选框也隐藏——自动模式下被隐藏的控件永远在请求侧发送未收窄的默认值（当前 notebook 全部可见来源、全部已挂载参考库），绝不沿用高级模式会话残留的收窄勾选。自动模式创建的深度报告固定携带 `auto_generate=true`（参见上文「深度报告」条目及其可能触发的 `report_intent_auto_confirm_skipped` 事件）。问答输入框仅在该笔记本**尚无可用证据**且存在解析未到终态的来源时锁定——已有证据的笔记本上传新文档不会锁死输入框；能可靠统计处理中来源数（来源搜索框为空且当前已加载全部可见来源）时显示「N 篇文档处理中，完成后即可提问」，否则显示不带数字的「文档处理中，完成后即可提问」。切换界面模式即时生效；切回高级模式后，前端保留并继续尊重用户此前的具名 Ask 模式选择。
...
长度随用户数据增长的清单都分页，没有哪一行只能靠滚动一张无限长的页面才找得到；少数例外逐一
列在下文「刻意不分页」里。分三种形态。

**服务端分页**的清单每次只取一页：笔记本来源清单（50）、知识浏览器（50）、记忆（20）、管理员
提问分析（50）、「解析问题」页签（50，`offset` + `total`）、来源详情元素（40）。「加载更多」
形态把下一页追加在后面：`/dev/logs` 活动流与左栏笔记本的来源清单（各 50）、许愿墙、Agent
Profile 与 Token、命令目录候选、枢纽概念的成员、Knowhow 表历史。

**界面分页**的清单对一份完整的接口响应分页（这些端点刻意不分页）。它们共用同一个控件
（`frontend/app/Pagination.tsx` + `use-client-pagination.ts`）：只有超过一页时才出现，显示
当前行区间与总数、上一页/下一页、跳页，并带一个说明是哪份清单的无障碍名称。请求的页码按当前
长度夹紧，所以移除最后一页上唯一的一行后显示上一页，而不是一张空页。每页条数：

| 位置 | 每页 | 何时回到第一页 |
| --- | --- | --- |
| 主页集合——「我的笔记本」与「群组」两个分区（卡片与列表视图，两个分区各自独立） | 24 | 切换筛选页签、搜索词、排序或视图模式 |
| 「已分享」弹窗——已分享的笔记本；每本的只读链接成员 | 20 | 重新打开弹窗 |
| 问答历史会话 | 20 | 重新打开面板 |
| 知识浏览器——重复组 | 20 | 切换对象类型页签 |
| 知识对象出处（知识浏览器条目展开与知识图谱详情；序号跨页连续） | 10 | 选中另一个节点 |
| 知识图谱——待确认合并；选中节点的相邻关系 | 20 | 选中另一个节点（相邻关系） |
| 深度报告清单 | 20 | 换笔记本、提交新报告；打开的报告在后面的页上时，清单翻到它所在的页 |
| 待审核贡献队列 | 20 | 重新打开弹窗 |
| Knowhow 表卡片 | 24 | 切换状态筛选 |
| 群组页——知识库、可添加的笔记本、成员、待审批队列、本人申请 | 20 | 切换群组；可添加的笔记本在搜索词变化时也回到第一页（见[群组工作台](#群组工作台)） |
| 群组页——群组清单 | 20 | 切换「我加入的 / 全部」范围；选中的群组翻到它所在的页 |
| 笔记本「共享给群组」弹窗——已共享的群组；我的共享申请 | 20 | 换笔记本 |
| 管理员用户总览——展开行里的笔记本 | 20 | 展开另一位用户 |
| `/dev/logs` 左栏——笔记本 | 20 | 换被查看用户；选中的笔记本翻到它所在的页 |

管理员用户表保留自己的「先排序后分页」控件，可选每页 20/50/100 行。离开一个视图会卸载其中的
清单，所以回到该视图时每份界面分页清单都从第一页开始。

**刻意不分页：**问答会话正文及其轮次导航（一条连续的对话）；Knowhow 表的行网格与行管理清单
（行标题合并格跨多个物理行、行可以调整顺序，分页边界会把一组拆开）；知识图谱的节点总览（与画布
画的是同一批节点，由视图的「范围」选择器限定）；选择器与筛选下拉框；有已披露上限的清单——报告/
会话公开分享页、回答里的结构化结果卡片、关系审核队列（按优先级排序的一页，标题写明队列总数）、
Knowhow 单元格历史（已披露的最近条数上限）、Agent 调用/观察样本；以及两处固定条数的摘要——分析
看板里最新的 10 条低分提问，和分享笔记本预览里与来源总数并列显示的前 50 个来源标题。

### 问答欢迎页的候选问题

空白问答会话最多显示四个问题按钮。笔记本里非空的 `expected_questions`
优先，点击时提交完整的人工问题。否则欢迎页立即显示现有的来源主题模板，
并异步请求 `POST /notebooks/{notebook_id}/question-suggestions`（无请求体，
需要登录且具备笔记本读取权限）。模型成功生成后替换模板；模型不可用、输出
不合规、请求失败或没有可用文档时，保留模板供用户提问。后端判定不可问答
的笔记本不显示按钮，也不请求问题建议。按钮通过正常问答流程提交完整问题；
生成建议本身不创建会话。

响应包含 `status`（`ready` 或 `fallback`）、`questions`（每项含 `label`
和 `question`）、`sampled`、`source_count`、`sampled_source_count`。
模型输出最多接纳四个不同问题，按钮文案须非空且不超过 32 字符，完整问题
须非空且不超过 300 字符；格式错误、超限或重复的输出整组回退。模型输入是
本库可见导入文档的有界内容，包括标题、摘要与代表性正文摘录；排除私有记忆、
隐藏的经验表投影及挂载参考库，不随来源栏搜索或翻页改变取材范围。建议是
根据已解析的文档内容生成；导入进度和失败提示不作为证据，也不占用文档取样名额。
较长摘录可进一步缩短以适配总输入预算，仍明确披露为部分内容，不修改原始文档。
建议是笔记本级提问起点，实际问答仍遵守所选检索范围。输入仅覆盖部分内容时，
`sampled` 驱动界面显示“根据部分来源内容推荐”，不声称读完所有文档全文，
也不修改原始来源。模型指令要求根据已提供材料生成中文问题，并将来源文本
作为不可信数据处理。

后端复用 `notebook_metadata` 模型调用类型，在应用运行时中缓存校验通过的
结果，新会话可复用缓存；笔记本描述配置、来源内容或版本、模型配置或提示词
版本变化时失效。同一代的并发请求共用一次模型调用；失败生成有重试冷却期，
生成期间发生内容变化则丢弃结果。缓存淘汰或服务重启后可能重新生成。
前端取消过期请求，阻止旧用户、旧笔记本或旧工作区代次的结果显示在当前欢迎页。
可调取样与缓存预算见配对部署参考。
...
管理员必须把它当作 bearer 凭据，并在受邀范围变化时主动撤销或换新。owner 转让与删除群组放在设置页的独立确认区。
新建表单与设置页里的组名、组说明输入框按下文登记的上限截住输入，离上限还远时不出声；进入最后
一成后在输入框下方提示还可输入多少字，框被填满（包括粘贴被截短）时明说「已达上限 N 个字」，
截断不会静默发生。输入框按 UTF-16 码元计数，含 emoji 的文本会在服务端的码点上限之前停下，
不会越过它。页面复用集合页的壳层、字体、控件、
间距、颜色与响应式断点，不引入另一套视觉系统；当前群组和页签可通过 URL hash 定位与返回。
页面上的每份清单——左侧群组清单、群组知识库、可添加的笔记本、成员、待审批队列、本人发起的申请——
都对完整的接口响应分页，每页 20 条。切换群组时，该群组自己的几份清单回到第一页（可添加的笔记本
在搜索词变化时也回到第一页）；移除最后一页上唯一的一条后退回上一页。群组清单在切换「我加入的 /
全部」范围时回到第一页；选中的群组落在群组清单后面的页上时（深链、新建的群组、退出后改选），
群组清单翻到它所在的页。「添加知识库」里的勾选跨页保留。
...
- **清单端点无分页**——`GET /groups/{id}` 的成员清单、`?scope=all` 的全量群组、
  ...
  因此这些清单的分页发生在界面上，对完整响应分页（见[群组工作台](#群组工作台)与[列表分页](#列表分页)）。
...
`POST /ask` 按 `mode` 分派；`backend/app/services/ask_modes.py` 为注册表真源，默认 `chunk`。两种模式共用[参与集检索契约](#检索语义与跨库召回)。

流式 Ask 只在持久答案与浏览器 final 事件之后运行完成后 observer；全局问答在推送流终态帧之后按完成的作业驱动同一个 host 一次（见上文「问答后学习链」）。该 point 的协作式墙钟预算来自 `ASK_POST_COMPLETION_EXTENSION_TIMEOUT_SECONDS`（默认 `30`，有效范围 `>0..300` 秒）：已经进入同步调用的 callback 可安全完成，但 deadline 到达后 host 不再启动后续 contribution。这只是部署/内部扩展护栏，不改变检索、答案正文、引用或用户已收到的 final 事件。
...
`CHUNK_ANSWER_BUDGET_CHARS`，不声称已阅读全文。这个分类器只服务 chunk 通用问答这
一条通道：逐步推理里要不要列目录是模型经枚举工具自己的决定（见下文
[集合枚举工具](#集合枚举工具)），服务端不拿问法正则替它做这个判断。
...
检索路径。该路径只在提交 `chunk` 通用问答时触发；自动模式（简化界面）固定提交 `reasoning`，不会进入它。逐步推理里对应的是 `read_document` 动作（见[集合枚举工具](#集合枚举工具)一节）：目录导读由服务端确定性触发，按篇原文取样则是模型在拿到目录之后自己判断要不要深入的一步。
...
高级问答界面点选 `strict` 分组即直接选中 `reasoning`。当前组内只有这一个引擎，因此不显示第二层引擎按钮；检索档位仍可选择。部署引擎继续在 `extension` 组内显示引擎选择器。
...
确认 Ask 意图后先检索完整权威问题，再按必答主题轮转播种检索方向。无效确认在创建持久化会话／任务前返回 422；中止意图预检会向模型调用传递取消信号。

提到当前笔记本/库只确定检索范围，本身不代表要求穷尽输出。普通概述保持 ranked，完整性不设为必需；明确要求逐项列全、全量计数或逐篇介绍时，仍按 complete、aggregate 或 hybrid 处理。规划器不得给 ranked 概述另外追加“覆盖全部条目”的硬约束。
...
共享答案风格提示遵守用户要求的输出粒度。标题清单保留每条匹配来源及引用（包括同名的独立来源记录），不擅自追加摘要或配置细节，必要的覆盖披露仍保留。概述与机制比较保留主要推理、限定条件及用户所需细节。数值比较保持指标、单位、版本、配置和评测条件的对应；每个高低结论对应一个指标、版本和配置对，不同配置的斜杠数值组在方向不一致时不能共用一个高低结论。原文未明确斜杠数值与评测标签的映射时保留不确定性，不猜顺序。用户未要求完整矩阵或逐项结果时，先给结论和必要支持数字、决定性反例，避免先重复抄两侧全表再比较；明确的完整性要求仍须满足。来源的定性结论不能覆盖与其冲突的表格数值。共享/独立组件、架构/数值变化、理论可组合/实际兼容已验证也须区分。这些要求进入既有合成调用，不增加评估轮次、不降低输出预算；模型是否遵守及事实质量仍须实测。

最终答案提示提供一个严格合法的 JSON 对象示例，`answer` 与布尔类型的 `grounded` 位于同一个根对象。Markdown 段落换行、引号和 LaTeX 反斜杠必须在答案字符串内转义，不得输出第二个对象。这项输出指导不放松严格解析、schema 校验、字符串保真修复或既有重试上限。
...
同一个 scheduler 出口现在覆盖当前注册表里的**全部 27 个 chat workload**，而不只覆盖问答和深度报告。这道边界遵循 2026-09-14 定下的 harness 原则：prompt 要准确，共享边界吸收模型可能出现的偏差，只有各领域 parser 在「错了会写坏数据」的地方保持严格。具体地，严格 JSON 可解析之后，边界**只在回复对任何消费者都不可用时拒绝**——不是 JSON、不是对象、为空、被截断、带有 JSON 本身无法表示的数值（`NaN`、`Infinity`、溢出的指数——一个 `+inf` 的 confidence 会在每个数值消费者那里被钳成最大值），或对象里一个预期顶层字段都没有（含 `{}`）。字段级与 schema example 的每一处不一致按两种方式之一处理，且无论哪种都记一条 `model_json_shape` 事件（`status: "deviation"`、工作负载、安全 `support_id`，以及有界的 `path`/`reason`/`fix` 列表，例如 `enumerate.kind: invalid_enum`；绝不含回复正文），这样示例与模型实际写法漂移的 prompt 在 `events.jsonl` 里持续可见，却不再让调用方失去这次回复。只有一种合理读法的偏差由边界**直接吸收**（`fix` 记下做了什么）：hint 声明有值处给了 JSON `null` 就把该键删掉——缺席与 null 对每个消费者都是同一件事，而下游一次裸字符串转换会把它变成字面量 `None` 并当正文落库；布尔 hint 收到 `"true"`/`"false"`、数值 hint 收到数字字符串就强转；声明为标量列表处给了单个标量就包成单元素列表，而不是被逐字符迭代；声明为对象列表处给了单个标量没有任何读法，直接删键，遍历该集合的消费者看到的是缺席而不是异常；声明字段全为 `null`（或全被删掉）的回复视同一个字段都没有，照样拒绝。其余不一致——有多种读法的类型错误、取值不在 `a|b|c` 枚举里、声明为对象处给了标量、嵌套对象与示例没有任何共有键——**原样交付**（`fix` 为空），由领域 parser 施加自己的容错（收窄枚举、丢掉那一条不可用的项），而不是整份回复失败。改动之前这次遍历遇到第一处不一致就整份拒绝：一个幻觉出来的边类型会丢掉整个知识图谱抽取窗口，`"year": null` 会让每个非论文来源的元数据失败，parser 自己写好的容错永远跑不到。由于这里的 schema 是示例而不是完整 JSON Schema，未出现的可选字段仍允许省略，模型额外返回的字段也不由这层删改；JSON 修复路径按同一套宽松遍历判断字段形状，只保留修复本身才可能违反的那道硬门（修复后的字符串必须在原始回复里逐字存在）。凡仍在这个统一边界被拒绝的响应，系统都会把完整模型消息、schema 提示、原始响应、用户问题（有显式问题时）和安全 `support_id` 写入私有分析工件；普通日志和问题列表不含这些正文。没有请求级笔记本身份的后台/脚本调用仍会保存到 `_unscoped`，因此不会因缺少 Notebook 路由上下文而漏记。
...
**Legacy reflect 前缀布局。** Legacy 保留既有决策协议与候选投影。在同一条 user 消息内，全部与问题无关的指令（包括仅输出 JSON 的指令）排在引号短语说明、完整问题和原样候选摘要之前。消息角色、schema、动作可用性、解析、停止规则、检索/回答预算、候选顺序与摘录上限均不变；不新增策略开关、历史回放、证据绑定或详情展开。指令仍由既有工具门控制，因此只期望在相同配置间复用。此布局延长稳定前缀，尤其适用于引号词不同的跨问题请求；文本移位不保证模型决定完全相同，也不保证服务端缓存复用或降低耗时。验证以完整 Ask 时间与答案质量为准，服务端缓存计数可缺省。
...
意图预检把结果范围分为 `ranked`、`complete`、`aggregate`、`hybrid`，并带显式完整性标记。范围由**模型决定、确定性措辞兜底**（2026-09-14 用户裁决：模型是大脑，服务端只接住它出错的两种方式）：问题里有显式全集措辞（“所有 / every / 一共多少 …”，按子句就近判否定）时按措辞确定性定档，模型不得把“列出所有方法”降成相关性 Top-N；问题里显式拒绝全集（“不需要所有 …”）时封顶 `ranked`；两者都没有时采用模型的分类——包括放宽到 `complete` / `aggregate` / `hybrid`——但只在回复自洽（非 ranked 档必须同时 `completeness_required=true`）且 `confidence ≥ 0.5` 时生效，低置信或自相矛盾的放宽退回 `ranked`，因为集合枚举是昂贵的执行器，猜测不能选它。哪条规则拍板记入理解状态的 `scope_source`（`lexical` / `model` / `default`）。用户确认时会根据最终编辑后的权威措辞与澄清答案确定性重算 scope。确定性执行器只接收它能从物理行证明正确的语义：直接整表行/方法清单、直接物理行/记录计数，以及在此基础上的分析。条件子集、去重/种类计数（如“多少种”）、分组聚合目前回退到相关性检索，并明确说明尚不支持精确完整性。受支持的 100 行表可以返回 `100/100`。五档共用完全相同的完整枚举安全契约：
...
退役 id `fast`、`global`、`graph` 透明映射到 `chunk`，`auto` 透明映射到 `reasoning`（旧会话/书签不会 422）；其余未知 mode 返回 HTTP 422。

V2 实验协议与布局已退役。Ask、Deep Report 和 Knowhow 均使用 Legacy；不再提供 V2 开关、证据历史、自评或合成方面统计。已有历史轨迹仍保留其普通摘要。

### 集合枚举工具

逐步推理还能通过模型调用的 reflect 动作 `enumerate_elements`（来源元素）与 `enumerate_kg_objects`（知识对象），以及在这两个动作上给出的 `enumerate.collection` 参数值 `"sources"`（改为列出库里的文档），对这几类有界集合做**列举**而非只做排序。模型面的动作空间在这个特性内维持 10 个：第三个集合是一个参数值，不是第三个动作（[下文](#大纲便签与按节合成)的大纲便签之后另外新增了货真价实的第十一个动作 `update_outline`，与这里无关）。三个集合都由零 LLM 执行器提供：模型只决定是否枚举、枚举多少；覆盖率永远由执行器算出，绝不采信模型自报。这是与上文 Knowhow 物理行执行器**并行的独立机制**——共用同一张按档位缩放的预算表，但不共用同一个意图 scope 门，也不被 `result_scope` 自动触发。

**谁能触发枚举：只有模型。** 触发者就是 reflect 循环里的那两个模型动作，没有第三个。**目录由模型自己选**：`enumerate_elements` 配上参数 `enumerate.collection="sources"` 就是「列这个库有哪几篇文档」，服务端不按问法正则在首轮替它确定性地播一次目录——agentic 模式里目录该不该列是模型的判断，问法分类器（`overview_intent`）只服务 chunk 通用问答那一条通道。模型之后再请求**同一范围**的同一份目录按「本轮已全部列出」跳过（`already_enumerated`），不会把同一份清单列两遍、扣两次预算。**读哪几篇也由模型自己选**：拿到目录之后，用 `read_document` 深入某一篇原文（尤其是目录里没有存摘要的那几行）同样是模型基于目录内容自主决定的一步，服务端不会替它挑、也不自动播种。

**范围也是同一次调用的参数**：`enumerate.scope` 由模型自己填，只有两个合法值 `"all"` 与 `"current_notebook"`。**默认（缺省、留空或任何解析不出来的取值）是 `"all"`**：列出检索范围内的全部文档，即当前笔记本 + 勾选的挂载参考库。这与集合地图 `[Collections in scope]` 那行的 `sources` 计数**同一个联邦口径**，所以模型什么都不填时，它看到的数与它列到的数是对得上的；`"current_notebook"` 才收窄成只列当前笔记本的文档，也就是同一行括号里的 `current notebook` 计数。

这个参数**刻意写成字符串枚举而不是布尔**，而且这是模型面工具参数的一条通用纪律，不是这里的个人口味：JSON 校验层（`model_json._validate_against_example`）对布尔示例是硬类型校验，模型吐 `"true"` / `"yes"` 会被直接拒收、整轮 reflect 掉进 fail-open 兜底；字符串示例才享受「空串永远合法」这条宽容规则，而枚举值本身自描述，模型不必去猜 `true` 是哪一边。**非空非法字符串（`"yes"`、`"local"`）仍会被这一层按 `invalid_enum` 拒掉**，非字符串（`1`、`null`、`{}`）按 `invalid_type` 拒掉，两种情况下这一轮都记为兜底（见下文 reflect 兜底契约），而不是被悄悄改读成某个范围。所以对模型安全的答案只有两个合法值、或者干脆留空。校验层背后的解析器则自成一道防线：任何取不到的值都落回 `"all"`，绝不废掉这个动作，连 `fail_closed` 也不抛——范围不是动作合法性问题。

**范围是清单身份的一部分，而且看得见。** 续跑账目按范围记键，所以「先 `"current_notebook"` 列全、再请求 `"all"`」是**两条不同的链**：第二次是一次真的枚举，不是 `already_enumerated` 跳过；反向顺序同理。两条链共用同一个 run 级枚举预算池，所以重复列的代价自然被夹住，不需要特判。收窄过的清单在每一处报数的地方都说出这件事：`enumerate` 轨迹步带 `detail.local_only=true`（默认范围时不带这个键），上屏摘要、回喂给模型的枚举账目、以及**答案合成证据块里那份清单的分区标题**（`[Enumeration: documents（仅当前笔记本）, listed …]`）都带后缀「（仅当前笔记本）」，与元素清单限定单一来源时的「（限指定来源）」同形同位。三处共用同一份字面（`collection_enumeration.LOCAL_ONLY_SCOPE_SUFFIX`），所以措辞不会只改到其中一处。分区标题这一处是必需而不是锦上添花：一次 run 同时列出两个范围时，合成模型会拿到两份文档清单，它们的 `listed`/`total` 本就不同，没有后缀就没有任何字面能说明哪一份排除了参考库。

**结果卡带 `scope` 字段，并在标题上显示同一个后缀。** `TypedCollectionResult.scope` 取 `"all"` 或 `"current_notebook"`，由 outcome 的 `local_only` 映射而来；它只对 `sources` 有意义，其余集合恒为 `"all"`（`enumerate.scope` 是来源清单独有的参数，所以「收窄过的元素清单」不是 wire 能声称的东西）。这个字段是**加法**：默认值就是 `"all"`，所以早于它的持久化回答、以及任何不发这个键的客户端，都会被读回成它们当时真正的「没收窄」。前端 `answer-panel.tsx` 仍按 `collection` 查表拼出「来源清单」，再在 `scope === "current_notebook"` 时追加「（仅当前笔记本）」，于是卡片与轨迹、账目、分区标题读到的是同一句话。`scope` 同时进结果卡的 React key：`sources` 的 `element_kind`/`object_type`/`source_id` 全是空串，不并入范围的话两张范围不同的清单卡会撞成同一个 key，展开/收起状态互相串。

**reflect 兜底步是轨迹契约。** `reflect()` 的 fail-open 兜底（响应非对象、动作非法、JSON 形状被校验层拒收、模型调用失败、模型未配置）必须在轨迹上**自证来历**，不能伪装成模型判定「证据够了」。兜底轮的 `reflect` 步 `detail` 带稀疏键 `fallback_reason`（稳定机器码，例如 `invalid_enum` / `invalid_action:<动作>` / `non_object` / `provider_rate_limited` / `model_unconfigured`；非兜底轮**不带**这个键），`summary` 是中文整句——“反思结果无法采用（校验拒绝：`<原因>`），按直接作答处理”或“反思结果无法采用（模型调用失败），按直接作答处理”。标记走决定对象上的独立布尔字段而不是 `reason` 前缀：`reason` 是模型可控的自由文本，用它承载判据等于让被判据方自己写判据。
...
**按篇读取原文（`read_document`）。** 来源清单只给标题、类型与已存摘要；没有存摘要的那一行,模型此前只能靠 `add_subquery` 做相关度检索去猜正文讲什么。`read_document` 补上这一环：在同一份来源清单已经解析出来的花名册里，按标题（从清单里逐字复制）挑一篇，沿文档顺序做一次有界取样，回答「这篇文档本身讲了什么」，而不是 `search_chunks` 回答的「哪里提到了 X」。这个动作**只**能打开本 run 已经用 `enumerate.collection="sources"` 列出过的文档——没列过清单就先列清单，再回来读；标题在清单里零命中或多命中都会被跳过并在轨迹里说明原因（零命中是抄错了标题，重抄一次即可；多命中是这个动作按标题分不清楚，需要换个问法或明说做不到），从不退化成扫描全库标题。模型在反思轮里看到的是清单账目：每篇标题按显示名列出（无标题的文档显示为「未命名来源」，长标题截到 60 字，内部空白折叠），没有已存摘要的行后面跟着 `(无摘要)` 标记——这个标记就是选读依据，账目不带摘要正文。标题解析按模型抄回来的形态做几步**确定性**还原后再精确匹配：剥掉抄来的 `(无摘要)` 标记、一对书名号或引号、整行「标题 · 类型: 摘要」的尾巴，以及与账目里截断/折叠后的形态相等——都不是模糊匹配，还原后仍不相等照样零命中。`coverage` 参数决定取样方式：`"spread"`（默认，等距取样、必含文档最后一个位置，因为结论往往在末尾）或 `"opening"`（只读开头，最省成本）——写成字符串枚举而不是布尔，理由与 `enumerate.scope` 相同（见上文），模型不填也不会打不开这个动作。回来的始终是一份**有界样本**，自带覆盖率披露；回答只能引用样本或已存摘要写到的内容，不能声称读过全文。每次取样至多读 `REASONING_MAX_DOCUMENT_READS`（默认 4）篇，元素与字符各有一个 run 级预算池（元素池复用 `DOCUMENT_OVERVIEW_MAX_ELEMENTS`，与「通用问答的文档介绍」同一个旋钮；字符池取档位 `chunk_context_chars` 的四分之一），先读的文档不会把预算吃光，后面还能读的文档也各自留有一份可用额度。取样产物经独立的 `[k]` 号段进入合成证据块，紧跟在来源清单预览之后：模型先看到「库里有哪几篇」，再看到「其中这几篇我真的翻开读了几段」；结果卡仍是来源清单那张卡，不新增卡片类型。取样块的证据与覆盖率约束对任何问题都成立（只能引用取样或已存摘要写到的内容，**目录列全不等于已经阅读全文**）；逐篇导读的**形态**只在问题问「这些文档分别讲什么」这类概览时适用：每篇各自成段，用该篇标题打头，研究问题/方法/贡献只从这篇的取样或已存摘要写，证据不够就明说留空；花名册里既没有存摘要、本轮也没被读到的文档要逐一点名「依据不足以介绍正文」，不能沉默漏篇，也不能凭标题猜内容。聚焦型问题（比较两篇里的某个方法）只把取样当作那个问题的证据，不被迫写目录导读、也不介绍问题没问到的文档。这个动作总开关 `REASONING_DOCUMENT_READ_ENABLED`（默认开）与集合枚举总闸共用同一条纪律：任一必要条件不满足（部署关闭、没有来源端口、本轮没有花名册、次数上限为 0、或预算池配置到连一篇都读不起），动作从 prompt、schema、白名单三处一并消失，模型看不到也选不到；关闭时逐步推理逐字节回到接入前。它与集合枚举工具**同门**：请求侧按[来源选择收窄检索范围](#按来源选择检索范围)（`source_scope` 真正排除了部分来源）时，整套枚举工具连同 `read_document` 一并不提供——这不是这个动作单独的限制，而是收窄检索范围时枚举总闸本身就会关闭；把它做成能在收窄范围下继续工作（即让来源清单本身按选中的来源可寻址）已登记为后续任务。
...
**集合地图（先计数后取行）。** 在这些工具真正值得调用之前，每个 run 只构建一次一行文本，形如 `[Collections in scope] elements: formula 12 (3 sources), table 5, image 0, code_block 0 | KG objects: concept 1234, claim 567, formula 89, procedure 0 | knowhow tables: 2 | sources: 7 (current notebook: 2)`（每个白名单 kind/type 恒在场，缺省即 0，让模型能分清「数出来是 0」与「压根没上报」；硬顶 600 字符），注入 plan/reflect 上下文，作用域为 active notebook 加其当前有效的挂载参考库。行尾的 `sources` 是用户可见的文档数（与来源页签、文档数量上限同一口径），且与来源清单的分母出自同一处，不会出现「地图说 7、清单列 8」。它是**唯一渲染两遍**的计数——先联邦总数，再用括号给出当前笔记本的份额——因为来源清单是唯一带范围参数（`enumerate.scope`）的集合，模型要在两个范围之间选就必须同时看见两个数；括号里那个数无条件出现，所以「两个数恰好相等」不会与「这一版根本没报第二个数」长得一样。这些计数在服务端还有第二个消费者：清单规模守卫拿来与本轮剩余行数比较的那个分母，就是其中与本次请求**同集合、同 kind、同范围**的那一个（见「预算与续跑」）——`sources` 目录取联邦总数，收窄成 `current_notebook` 时取括号里那个数；元素/知识对象清单取该 kind、该类型自己的计数。所以地图**对象**——不只是渲染出来的那一行——会留到本轮结束。这不多花一次查询：同一次构建，只是渲染完不再丢掉。地图里没有对应的那个数时守卫不触发——地图压根没建成，或请求被收窄到单篇来源（地图没有「某一篇里有多少条」这个数）——「不知道有多大」不等于「已知很大」。计数是索引辅助的，不携带来源标题、文件名或摘录文本——它是 prompt 脚手架，不是证据。地图统计的每源集合与枚举执行器实际遍历的源集合按构造逐字一致，因此不会出现「地图说 12、清单只给 8」的假部分。
...
**尚未构建知识图谱的笔记本。** `search_chunks` 做语义加关键词原文检索，与 chunk 问答共用 `chunk_context_chars` 证据分区和引用绑定；`read_document` 按来源顺序取样一篇已列出的文档，有图 run 也支持两者。无图首轮按子查询确定性播种原文；执行过 `plan()` 时还用其高／低层关键词发起一次词法查询，先收到同一每子查询宽度再合并。关键词沿用提示词的默认 zh/en，不按语料语言生成；有图及已确认意图路径不走此词法臂。`REASONING_MAX_CHUNK_SEARCHES` 限制后续动作，不计首轮播种。

无图时，`expand_graph`、`ppr_retrieve`、`expand_community`、`follow_chain`、`enumerate_kg_objects` 从动作枚举、提示词和白名单一并移除，来源与元素枚举仍可用。轨迹披露「本笔记本尚未构建知识图谱,本轮只用原文检索与集合清单」；`kg_required=True` 保留整理图谱建议，但本身不拒答。

未收窄来源范围的 run，其内部准入至少需要一条可用通道：`REASONING_ENUM_TOOLS_ENABLED` 开启且参与集来源／元素／对象计数非空，或 `REASONING_CHUNK_SEARCH_ENABLED` 开启且原文参与集有可见来源。原文检索已跨库；仅关闭 `CHUNK_FEDERATION_ENABLED` 才将该准入计数退回 `active_sources`。集合地图未扣除超过 `CHUNK_FEDERATION_MAX_PARTICIPANTS` 的库，因此准入范围可能宽于实际有界生产者集合。两通道关闭、均为空、或集合地图构建失败时确定性早退：「当前笔记本没有可检索的来源；请先添加来源，或挂载/整理一个已建知识图谱的参考库。」

收窄的来源范围另有更早的准入路径以供选中来源的原文元素检索：非空 include 范围（或内部 exclude 范围）在上述两项开关检查前放行，此时仍禁用整库枚举。

**HTTP 可用性仍更窄：** 当前笔记本没有本地可用证据（`local_evidence_available=false`）、仅无图参考库有来源时，`NotebookSummary.ask_available` 仍禁用输入框，直接 Ask 请求返回 409。上述内部跨库准入不代表此场景端到端可用；剩余可用性修复登记于 [`fangan_todo.md`](../fangan_todo.md)。
...
**结果不完整提示何时省略。** 一个没有执行器能精确服务的完整性请求，回答末尾会简短提示本轮尚未验证为完整结果，不在开头列举产品支持的集合类型。只有四条同时成立时它才被抑制：意图 scope 不是 `aggregate`、意图合同里没有任何约束/排除项/前提、至少有一张清单结果卡返回了条目，且该卡的覆盖率为「完整」。其余情况一律保留。方向刻意偏向多警告：一张卡的覆盖率只证明「某个物理集合被完整走了一遍」，证明不了那个物理集合就是问题真正要的那个（经过条件筛选/分组/去重的）子集——而后者没有确定性判据，做出来只会是猜。

响应以可选的 `completeness_notice` 携带这句服务端提示，问答面板把它放在模型 Markdown 正文之外渲染，因此模型留下未闭合的代码围栏也不会吞掉提示。复制回答时仍包含提示；没有提示的响应省略该字段。

公开会话投影也在有提示时携带同一句话，并在分享页的答案 Markdown 之外显示；`answer_md` 仍保留已存答案原文，供引用选择和旧客户端使用。

`REASONING_ENUM_TOOLS_ENABLED=false` 禁用枚举动作与来源清单参数，不关闭独立的原文检索通道；该通道的准入仍可读取集合地图。无图早退遵循上文的通道及收窄来源规则，不由这一个开关单独决定。
...
| 每块变更历史环形缓冲 | 20 条 |
| 底座链路触发阈值 | 累计 5 次来源变更 |
| 覆盖层链路触发阈值 | 该成员累计 10 次已完成提问（全局问答按其实际检索到或引用到的每个库各计一次），或 1 次已完成深度报告（直接达阈） |
...
| `DELETE /notebooks/{id}/understanding/experiences` | 与上一行相同的权限口径；清空**这本笔记本**那一份（全局分区与别的库一行不动），返回 `{"removed": n}`。**该分区正在整理或排着队时返回 409**（与「立即整理」撞上同一批时同一句文案），一行也不删：那一批可能正卡在模型调用上，清完之后会把结果写回来，看起来就像删除没生效。**总闸关闭时照常可删**——关开关说的是「从现在起不再记」，不是把记过的行变成既看不到也删不掉的东西 |
...
在上面的笔记本理解块之外，还有第二份独立的记忆（Agentic Memory P2，2026-09-22 起按笔记本分区）：一张检索打法库，由已完成的 Ask run 离线蒸馏而来，作为它自己有界的 prompt 块注入同一套规划/反思 prompt。理解块描述的是**这一本**笔记本，一条经验条目描述的却是**一类问题的形态**——「在具备这些特征的问题下，这个检索动作值得／不值得用」——它不携带任何来源或用户身份。表按 `notebook_id` 分区（`''` = 全局分区）：一个笔记本的条目只由该笔记本自己的 reasoning 提问蒸出、只注入该笔记本自己的 run，全体成员共享；全局分区继续由独立链路蒸馏、单独保留，作为新库或条目不足时的回退——注入时先取本库分区，名额未满再从全局分区补齐。

**形态。** 一条经验是一对封闭的 IF→THEN：一个八键**情境**指纹（Ask 模式；`result_scope`；`retrieval_effort`；这次 run 是否要求完整性；实体数与必答主题数的**分桶**——`none`／`few`（≤2）／`many`；确认后的意图是否带约束；是否带排除项——每个值都取自封闭枚举，没有一个来自问题本身的措辞）映射到一个封闭**动作**词表里的一个词（reflect 循环自己的检索动作，折成它们的 trace-step 拼写：`retrieve`/`ppr`/`exact_lookup`/`expand`/`expand_community`/`follow_chain`/`enumerate`/`outline`/`search_chunks`/`read_document`——`enumerate` 是刻意的通配，代表 `enumerate_elements` 或 `enumerate_kg_objects` 两者之一，因为一次已完成 run 持久化的轨迹分不清它当时用的是哪一个；`search_chunks`/`read_document` 是唯二 trace-step 拼写与 reflect 动作 id 本就相同的两个），一个**极性**（`good`/`bad`），以及模型撰写的一句**理由**。`support` 计数记录有多少次蒸馏 run 支持这条结论；`adopted` 计数记录有多少次被展示过这条经验的 run 之后真的选了那个动作，它是表满之后淘汰排序的首要键。
...
**蒸馏是两条链路，同一份 worker，按分区参数化（2026-09-22 起）。** 全局链路不变：部署级进程内计数，阈值 `RETRIEVAL_EXPERIENCE_TRIGGER`（40，跨所有笔记本与用户），读取全部最近已完成提问——含全局问答的 `reasoning` 作业（它们只计入这条链、只被这条链取样；笔记本链路从不读它们），写入并淘汰全局分区（上限 300）。笔记本链路是新增的第二条：按 `notebook_id` 的进程内计数字典，阈值 `RETRIEVAL_EXPERIENCE_NOTEBOOK_TRIGGER`（默认 **10**）——同一笔记本累计完成 10 次 reasoning 提问即触发一批本库分区蒸馏，取数谓词加 `notebook_id=?`、只读该库最近 40 条已完成 reasoning 提问，写入并淘汰该库自己的分区（上限 100）。10 小于批读取上限 40 是刻意的：相邻两批有约 30 条重叠，重叠 run 的重复计数由 provenance 去重吸收（`support` 只对尚未出现在条目 provenance 里的 run 递增）。同一次提问完成最多同时让两个计数各自 +1、各自到阈值各自排一批；一个进程内单飞槽位保证同一时刻只跑一批，全局分区不进队列、认领时优先（唯一的例外：上一批就是全局且有笔记本在等，则让一位——饱和时两条链路严格交替，笔记本至少拿一半批次，全局也不可能被饿死），笔记本分区进一个有界去重队列（64 项，按到达顺序），队满时新到分区被拒绝但不丢已排队项、计数保留，落一条 `reason=queue_full`（带 `dropped` 计数）的 skip 事件——同一个笔记本在同一个饱和期内只报一次，拿到批次之后再被拒才会再报。事件 `retrieval_experience_distilled` 新增封闭字段 `partition`（`"global"`／`"notebook"`，不带具体笔记本 id）区分这批蒸馏出自哪条链路；`invalid_partition` 是另一个新增的 skip 原因（非字符串/空串分区被拒）。两条链路的进程内计数都在重启后归零（与今天同款登记）。它不需要任何游标表：重复处理一批有重叠的 run 是幂等的，因为一条经验的 `support` 只对**尚未出现在它自己那份有界、按最新排序的 `provenance` 列表里**的 run id 才递增——批次大小与 `provenance` 上限被钉在一起来保证这条不变式（一批读取的 run 数绝不能超过一条经验能记住的上限，否则被挤出尾部的 id 会在下一批重叠时被重复计数）。模型看到的是这批里出现频率最高的若干情境（每次调用有界，好让 prompt 有界而不必限制批次的 run 数），外加针对每个情境、库里已有的相似情境条目（Mem0 式的局部更新），并对每个情境返回 `ADD`/`UPDATE`/`NOOP`；畸形回复整份拒绝而不是修补，库保持原状。表满之后的淘汰按 `(adopted, support, updated_at)` 升序——先淘汰没被采用过的，再淘汰支撑薄弱的，最后按时间。

**被动注入做到任务级**（不管模型有没有主动要，每一次 run 的每一次规划/反思 prompt 都自动推送），由一把独立于蒸馏开关的**注入闸**控制（`RETRIEVAL_EXPERIENCE_INJECT_ENABLED`，**2026-09-22 起默认开**，此前默认关）——一个部署仍然可以只蒸馏、只观测而从不注入，那条退路正是设计文档为「效果可能不明显」登记的风险缓解手段，两把闸分开就是为了让它一直在。默认值翻过来的理由是作用域变了：按笔记本分区之后，一次 run 读到的首先是**这个库自己**攒下的经验，而不再是一张跨库全局表——「这个库里哪种动作好用」正是产品要的那句话。开启时，一次 run 先把当前笔记本自己的分区（有硬顶、常驻内存、纯函数缓存，按 `(notebook_id, version_signal)` 分区 LRU 64 项）按当前情境打分——深度报告逐节深挖场景在意图契约尚不存在时也能算出情境，缺失的键落回各自的 `unknown`/`none`/`false` 默认值——选出相似度最高的至多三条（相似度地板 0.5；低于它就判定这条经验说的是另一类问题，比不注入还糟，因为模型分不清两者）；本库分区名额未满时，再从全局分区（独立一份缓存）补齐，跨分区仍保持同动作唯一——**本库条目优先**是产品规则，它们才是「这个库」的经验。库从未攒出过自己的分区时（新库冷启动），这一步自然退化为纯全局，行为与分区化之前逐字相同。选中结果合成一个小块，紧跟在笔记本理解块之后、集合地图之前渲染，规划 prompt 与每一轮反思都会带上它。和理解块一样，它**只**是 prompt 脚手架——`ReasoningResult` 不为它新增字段，绝不是证据，也绝不可被 `[k]` 引用。轨迹步 `experience` 的 detail 新增 `notebook_entries` 字段：本次送达行里来自本库分区的条数（0 表示这次全靠全局回退）。consult_memory 与步级零命中提示读同一份「本库 ∪ 全局」合并候选，排序里「本库层优先」排在相似度、支持度两个既有键之后、条目 id 之前——只在前两个键打平时才落到这一层。深度报告一侧只接入每节自己的深挖检索（那条路径恒传 `intent_queries`，不调用规划模型），在没有完整意图契约时用该节自己持久化的 `result_scope`/`completeness_required`。`adopted` 只在极性为 `good`、且该条经验在块自身字符上限内**真正送达**（不是仅被选中——装不下的行永远不会被模型看到，也就谈不上被采用）、且模型自己那一轮 reflect 决策点名了那个动作（发生在块非空的那一轮）时才计数。

**两笔账如实登记，而不是假设它们不存在。** 第一笔，最坏送达：整块字符上限（600 字符，含固定表头与一句框定语）给行本身留下的预算大约只有 380 字符左右；当一条送达经验的理由逼近它自己 160 字符的上限时，被选中的三条里实际能装进去的大约只有一条——「至多三条」是**选中**宽度，不是**送达**保证，调用方因此报告的是块真正送达的条数而不是选中数，原因正在这里。第二笔，reflect 累计成本：这个块在**每一轮**反思都会重新拼进 reflect 的上下文，不是每次 run 只拼一次，而且它的体积不随检索档位缩小——在 `exhaustive` 档 50 步的上限下，一次 run 生命周期内累计重复的块文本大约是 3 万字符量级。这笔成本原本是注入闸默认**关**的理由；2026-09-22 按笔记本分区并在本机跑通整条链路后默认翻为**开**（同一个库 10 次 reasoning 提问蒸出条目，开闸后轨迹出现 `experience` 步且 `notebook_entries>0`），但成本本身没有变小——它仍然是两把闸必须分开、且关掉注入闸就逐字回到「只蒸馏不注入」的理由，一个觉得这笔 token 不划算的部署把 `RETRIEVAL_EXPERIENCE_INJECT_ENABLED` 设成 false 即可。

**模型主动拉取的回想：`consult_memory`（Agentic Memory P4，T5）。** 与上面的被动块并列的第二条路径，这一条才是真正的**步级**：一个零参数的 reflect **动作**——`consult_memory`——模型可以选择拿一轮反思去调它，而不是被动接收自动推送。它出现在 reflect schema 的 `next_action` 枚举里，要求**三个**条件同时成立：部署自己的总开关（`REASONING_CONSULT_MEMORY_ENABLED`，默认开）、这次 run 的 `retrieval_effort` 档位是 `deep`/`thorough`/`exhaustive`（`overview`/`standard` 永远不提供——与大纲便签「低档不值得付这份成本」同一条道理，但门槛更低，因为一次 `consult_memory` 调用只花一轮反思，不会像大纲便签那样触发按节合成），**以及**被动注入闸（`RETRIEVAL_EXPERIENCE_INJECT_ENABLED`，2026-09-22 起默认开）也要开着——把注入闸关掉会连带让这个动作从 reflect 枚举里消失，这是刻意的。第三个条件是对设计稿原始草图「总开关+档位」两条独立闸的刻意收窄，已在设计文档登记：注入闸关着时，整个部署都读不到经验库任何东西，若只按「总开关+档位」放行，会让模型看到一个能调、却注定永远拉不到任何东西的动作——那笔反思轮预算纯属浪费。深度报告的逐节深挖自动继承同一道闸——depth 按大纲便签（PR-5）同一张表映射到检索档位，深挖自身的 `limits`/`retrieval_experiences` 接线本就没变，depth 落在 `deep` 及以上档时这个动作自然可用，不需要为它新增任何接线。
...
| 项 | 数值 |
| --- | --- |
| 全局分区保留的条目上限（`RETRIEVAL_EXPERIENCE_MAX_ENTRIES`） | 300（质量上限，不是存储上限——淘汰按 `(adopted, support, updated_at)` 升序） |
| 单个笔记本分区保留的条目上限（`RETRIEVAL_EXPERIENCE_NOTEBOOK_MAX_ENTRIES`） | 100（2026-09-22 起，`ports.py` 协议常量而非 env；淘汰口径与全局分区相同；表总量上界 = 300 + 100 × 有流量的笔记本数） |
| 每条经验保留的出处 run id 数 | 60（按最新排序；与下面的批次大小构成不变式） |
| 蒸馏批次——已完成提问数 | 40（两条链路的取数批次大小相同；笔记本链路加 `notebook_id=?` 谓词） |
| 蒸馏批次——轨迹步行数 | ≤600（与 40 条提问样本分开的独立上限，理由与笔记本理解覆盖层的步上限相同） |
| 全局链路蒸馏触发阈值（`RETRIEVAL_EXPERIENCE_TRIGGER`） | 累计 40 次已完成提问，跨所有笔记本与用户（刻意与批次的提问数相等） |
| 笔记本链路蒸馏触发阈值（`RETRIEVAL_EXPERIENCE_NOTEBOOK_TRIGGER`） | 默认 **10**（2026-09-22 起，`ge=1`）：单个笔记本累计完成的 reasoning 提问数，小于批读取上限 40 是刻意的（§见上文蒸馏段落的重叠说明） |
| 笔记本分区蒸馏待跑队列上限 | 64 个笔记本（有界去重队列；队满新到分区被拒但不丢已排队项） |
| 注入侧分区缓存 LRU 上限 | 64 个笔记本分区（全局分区单独一份缓存，不占这个额度） |
...
单独进入 prompt**——镜像 P2 检索策略经验库当年「先接好管线、注入待验证」的姿态（经验库自己的注入闸已在 2026-09-22 按笔记本分区后翻为默认开，这里的判断不跟着它走：一个推断出来的用户偏好与一条封闭词表的检索打法不是同一类主张）：一个推断错的
...
| 归纳任务触发阈值 | 该用户累计 20 次已完成提问（含全局问答，`USER_SEARCH_PROFILE_TRIGGER`，默认） |
...
- **在途集合是精确的。** 条目只在 `ask_jobs.status = 'running'` 时存在——这是该表唯一的在途
  ...
  仍在跑的**全局问答**按同一条规则（`global_ask_jobs.status = 'running'`）进同一分组，带
  `scope: "global"`；笔记本内的提问带 `scope: "notebook"`。
- **属主隔离。** `ask_jobs.created_by` 必须是发起请求的人：别人在我库里跑的提问是他的动作、
  不是我的，我在别人库里跑的提问是我的。可见性叠加投影其余部分共用的那条规范读谓词与生命周期
  过滤——对那本库失权即出铃铛，恢复读权自动回来。全局问答不属于任何一本库：属主是
  `global_ask_jobs.user_id`，可见性看它的**整个参与库集合**——`resolved_notebook_ids` 里每一本
  都存活且可读时才出现，与全局会话读取的规则相同（任一参与库不可读，那次读取就 404，铃铛因此
  不给一个点不开的链接）。
- **字段**——`type: "ask"`、`state: "running"`、`job_id`、`notebook_id`、`notebook_name`、
  ...
  路径承担。全局条目的 `notebook_id` / `notebook_name` 为空，铃铛里标成「全局问答」，点击打开
  全局问答浮窗里的那个会话而不是某本笔记本（浮窗已经停在那个会话上时原样保留，草稿也不清；
  眼前的轮次里还没有那条在途作业时就地重读；提交或停止在途时点击，等它落定再生效）。
...
- **推送边界。** 每次提问（笔记本内或全局）只重发两次待办快照：起点一次、到达终态一次（全局
  问答的终态那次排在推送流终态帧与作业离开在跑登记表之后、问答后学习链之前；worker 没能启动的
  全局作业也补这一次终态推送）。停止全局问答、或删除它所在的会话时，由那次请求在行已终态或已
  删除时立刻推送，不等 worker 退出；worker 之后那次推送只是被按用户单调序号丢弃的重复帧。轨迹进度**绝不**推送
...
| 围栏 | 取值 |
| --- | --- |
| 每帧快照携带的在途提问条数 | 20，笔记本内与全局合在一起按时间取最新——整份快照一个上限，不是每类各一份（`RUNNING_ASK_ROWS`） |
...
**只有真的影响答案的失败才上横幅。** 通用 chunk 词法检索超时（PostgreSQL 对 FTS 臂的有界 `statement_timeout`，`ChunkLexicalSearchTimeout`）不是模型故障，不再进 `model_errors`：FTS 叶子已记 `ask_stage=timeout` 并关掉本轮词法臂，候选仍由 ANN 臂产出。此前它会显示成一行没有服务身份的「模型服务调用失败」，把少了一条补召回臂说成答案不完整；生产一周 34 条横幅事件里 27 条是它。词法臂的其它异常照旧报。`synthesis_failed` 的结论文案也不再猜原因（「模型可能把输出预算耗在思维链上」）：横幅已按 `code` / `detail` / `finish_reason` 说清是哪个模型、什么现象，而鉴权失败或熔断打开时那句猜测正好是错的。

**横幅每一行说清「哪个模型」和「什么现象」。** `model_errors[]` 的每一条都带物理服务显示名、实际模型名和工作负载显示名（`service_name` / `model` / `workload_label`），界面按「服务名 模型名（工作负载）调用失败：现象。」渲染，「本次回答可能不完整」只在横幅标题说一次。现象来自稳定 `message` code 的词表；`malformed_response` 另带两个闭集字段：`detail` 是这份回复被 JSON 协议边界拒绝的具体原因（`empty`、`invalid_json`、`incomplete_object`、`missing_expected_key`、`invalid_type`、`invalid_enum`、`unknown_key`……与私有分析工件记录的 reason 同一套词表，外加答案合成的 `empty_answer`：JSON 合规但 `answer` 两次都为空），`finish_reason` 是传输层报回的上游结束原因（`length` 会括注「输出达到长度上限被截断」——那是用户能动手改的一项）。两个字段在其它 code 下恒为空，历史回答缺席时按空处理。这些字段全部是闭集：后端 `model_safety` 只放行词表内的值，前端只从固定标签表查词，未知值退回中性短语，原始诊断串永远不上屏。消费者侧的裁决（例如答案两次为空的终态报警）现在按工作负载从注册表补上当时绑定的物理服务身份，横幅因此不再出现没有主语的「模型服务调用失败」；未绑定时身份保持为空、不凭调用方提示编造。`message` 词表同时补齐了 provider 早已分类却曾被压成 `upstream_error` 的部署形态 code（`unknown_model` / `model_not_found` / `model_rejected` / `protocol_mismatch` / `unsupported_protocol` / `capability_mismatch` / `unsupported_capability`），模型状态面板与横幅都按各自词条渲染。最后一次尝试以抛异常收场时，终态报警仍是 `upstream_error`、不冒充「答空」——现象已由按异常记下的那条说清。
...
共用引用语法接受 `[k1]` 与 `【k1】`，含半角／中文逗号分隔的复合形式；`[1, 2, 3]` 等数字组仅在映射到已知引用时可点击，均显示为紧凑编号引用。
...
**公开会话对等。** `PublicImage.reference_keys` 只复用绑定 token 别名图片的既有公开 `k1`/位置编号，不暴露任何内部 id；`PublicReference.is_image_reference` 也只公开内部元素 id 比较后的布尔结果，不公开任一 id。匿名 `/c/{token}` 页据此沿用站内 Ask 的正文块落点、只显示图片、可信度标签、caption 仅作 `alt`、图片元素描述隐藏、同图只在第一次出现以及页内放大规则。缺少 `reference_keys` 的旧载荷无法诚实恢复落点，因此明确降级为未定位的纯图片区，不丢图也不猜绑定；报告公开分享白名单仍不带图。`PublicReference.is_external`（由 `tier == "external"` 推出，见 [Reflect 插件动作](#reflect-插件动作askreflect_action)）同样只披露「这条引用来自库外」这一个事实——它的 `url` 从不投影，匿名读者只拿到标记加标题与摘录，没有任何可打开的链接。

**是装配增强，不是新的检索通道。** 一次共享的批量主键读取（`evidence_context.py`）拿一批候选 element id，过滤出 `element_type='image'` 且 `metadata.asset_id` 非空的行并返回映射；不产生任何新增模型/embedding 调用。它接在两处已经持有完整证据对象的装配点：`ask_chunk` 的 chunk 引用（按该 chunk 完整的 `element_ids` 反查，而不只是 `anchor.element_id`——它只是 chunk 的起始元素，供「查看原文」翻页定位——「一段正文 + 一张配图」恰恰是多元素 chunk 的典型形态，只看起始元素会漏掉图）、以及 `ask_reasoning` 的锚点装配（chunk/element/KG 锚点各自提供自己的候选 id）。天然被排除、无需特判：knowhow 格子投影行 `element_type` 固定为 `knowhow_cell`，永远不命中 `image` 过滤；Memory 派生源虽然会解析出图片元素，但摄取路径刻意不为它接 `persist_image` 闭包，这些行永远没有 `metadata.asset_id`。
...
公开页与站内共用引用、表格、代码和公式渲染；引用编号取 `kN` 的数字而非数组位置，点击定位本页摘录；标题与摘录均为空的条目省略。公开页不能打开认证来源。宽内容在块内滚动，并加载 KaTeX 样式。投影及可见截断披露遵循下列护栏。
...
正文完整返回。公开问题取创建时的 `reports.question`，不是确认后的 `understanding.resolved_question`。创建时超出 `REPORT_QUESTION_MAX_CHARS` 返回 422；输入框按 Unicode 码点计数，保留输入、显示当前／最大字数并禁用提交。历史超长问题在公开投影中限长，设置 `question_truncated` 并提示「研究问题过长，已截断」，不改存储数据。

引用标题／原始文件名／摘录超限时设置 `title_truncated`／`file_name_truncated`／`snippet_truncated` 并显示提示。服务端派生 `key`（24）、`location`（200）及时间戳（64）的限制不带截断标志。创建护栏定义于 `models/reports.py` 与前端 `REPORT_INPUT_LIMITS`，投影护栏定义于 `services/report_public_view.py`。下文会话常量独立，但披露规则相同。

### 问答会话公开分享护栏

会话创建者可将已完成轮次发布到 `/c/{token}`。`/notebooks/{nb}/conversations/{cid}/share` 的 `POST` 发布／更新，`GET` 向创建者返回 token／水位，`DELETE` 撤销；`GET /public/conversations/{token}` 与 `GET /public/conversations/{token}/assets/{alias}` 匿名读取。非创建者与不存在均为 404；空创建者或零答案会话为 409，且不保留新发 token。
...
页面上，卡片的编辑与删除入口只对作者和管理员显示；删除要在卡片内二次确认，编辑则在标题与正文的位置展开行内表单，校验规则与发布区一致。管理员在每张卡片上另有处理状态下拉。每个动作的结果都落在发起它的控件旁边：保存修改后卡片按服务端返回整条重绘，删除后卡片消失且总数同步，改动后不再匹配当前类型/状态筛选的卡片会离开列表。`priority` 排序下改状态或改类型会先按同一规则就地重排可见窗口（已完成/不采纳沉底），再按当前可见条数从服务端整窗重拉、采用其顺序并对齐「加载更多」的游标，与点赞后的做法一致；沉出窗口的条目保留在末尾，但游标只按服务端条目计数，后续翻页不会跳过挤进来的那条。对齐完成前「加载更多」禁用并显示正在刷新排序，因为旧游标会跳过挤进来的条目；对齐（含点赞后的窗口刷新）尚未完成时又落地的编辑或删除会重启对齐而不是丢掉它。同一卡片的编辑、改状态与点赞串行：一个在途时该卡片的其余控件禁用，整条覆盖的响应不会盖掉更新的内容或票数。对齐按请求代际跟踪，被作废后一直不返回的请求不会在替代它的对齐完成后仍拖住「加载更多」；切换筛选会整窗替换并顺带清掉这份登记；发布新内容之后的重拉本身也登记为在途的窗口替换（期间「加载更多」禁用），若编辑或删除在它在途时落地，则不论排序都重启它，新发布的条目不会静默消失。改状态一次只允许一个在途，在途期间所有状态下拉都禁用，而不是看起来可选却被静默忽略。对齐（含点赞后的窗口刷新、发布后的重拉）失败时不复用旧游标：「加载更多」变成重试对齐，成功后才恢复翻页；下一次编辑或删除也会重启对齐。变更落地时其卡片已不在窗口里（期间切换过筛选），则按当前筛选整窗重拉，让此刻匹配新筛选的条目出现，而不是留着过期的空列表。删除一次只允许一个在途，在途期间所有删除入口禁用，与改状态同理。编辑、删除、改状态成功后都会作废仍在途的列表读取（加载更多、点赞后的窗口刷新），迟到的响应不能把已删卡片写回或覆盖新内容；若此时筛选/排序切换已让页面进入加载态，则按当前条件重启这次加载而不是取消它。状态不是 `open` 的内容显示状态标签，已完成/不采纳的卡片以弱化样式呈现。工具栏在排序旁提供状态筛选。
...
- 命令目录路由及响应／错误契约见[命令目录（工具手册）](#命令目录工具手册)。
...
- `DELETE /api/notebooks/{id}/understanding/experiences` —— 权限口径同上；清空这本笔记本那一份，返回 `{removed}`；该分区正在整理或排队时 409 且一行不删；总闸关闭时照常可删
...
- `POST /api/notebooks/{id}/ask` — 接地问答（逐句 `[k_i]` 引用；`mode`：默认 `chunk` | `reasoning`；`auto` 为退役别名，映射到 `reasoning`（与 `fast`/`global`/`graph` 映射 `chunk` 同一机制，供尚未刷新的旧标签页/书签兼容，不是可展示的 Ask 引擎）；`reasoning` 可传 `retrieval_effort`，默认 `standard`；官方网页端以带时区的 `asked_at` 传入只用于显示的提交时间；响应以 `answered_at` 返回权威持久化完成时间；集合型回答可带结构化 `result_sets` 与精确覆盖率；逐步推理回答还可能带 `gap_suggestions`——笔记本之外的非证据线索，见[缺口外扩检索](#缺口外扩检索)——没有该插件的部署此字段为空且不出现在 JSON payload 里；逐步推理回答的 `citations`/`anchors` 同样可能出现一行 `tier="external"`、带非空 `url` 的证据，见 [Reflect 插件动作](#reflect-插件动作askreflect_action)——没有该插件的部署每一行都不带这个键；联合范围遵循上文各 mode 的边界）
- `POST /api/notebooks/{id}/ask/stream` — Ask 进度的 NDJSON stream（同样接受可选的带时区 `asked_at` 请求字段；先发带持久化 `job_id` 和 `conversation_id` 的 `started` 事件，再发进度/最终事件）；前端用该会话 id 在答案生成前立即入历史并支持重新打开。`mode` 在 job 创建时已经解析为具体注册表 id，`ask_jobs.mode` 全程即为该 id，没有延迟选择引擎的窗口；`started` 之后的合成 start 步随即发出。前端在 `started` 之前离开再回到同一笔记本时，仍从本地在途记录接回该提问并继续读同一条流。推理模式提交前的意图理解与澄清确认同样不因离开而中止：理解在离开期间继续，清晰则直接启动 Ask job，需要澄清则在重开笔记本时重新弹出确认。这两个阶段还按标签页存在浏览器会话存储里：同一标签页整页刷新后回到该笔记本，理解会重新发起、澄清确认卡会重开；关闭标签页则丢弃。请求另接受可选的 `client_request_id`（字母、数字、`.`、`_`、`:`、`-`，至多 128 字符），每次提交一个键，官方浏览器每次提交都带上；同一用户带同一个键重复 POST 不会建第二个 job——流会以既有的 job 与会话 id 重发 `started`，回放已持久化的进度步骤，最后把已保存的答案作为 `final`（或该 job 的 `cancelled`/`error`）发出。刷新落在推理预检交接之后、`started` 之前的窗口时就靠这一点自动续上：标签页用同一个键重发同一次提交并接回，而不是把问题退回输入框。接回的判定在路由自己的前置校验（可用性、检索范围、模式注册表）之前——只有笔记本鉴权挡在它前面——所以即使此刻新提问会被拒绝，重发仍回放原 job。同一用户在另一笔记本已用过的键会让流以 `error` 事件结束且不创建任何东西。同步的 `POST …/ask` 接受该字段但不持久化、不去重：永远新建。transport 断开连接只会停止当前客户端继续接收，后台 job 仍继续并可保存回答
...
- **图谱维护：** 以下三个动作均要求 `kg:write`，且没有配置 LLM 的前置条件。它们发起后台任务，返回 `{status, notebook_id, job_id}`，不在启动响应中返回结果数量。只需笔记本读权的状态端点返回 `{job_id, notebook_id, status, running, ...计数}`，其中 `status=running|succeeded|failed|idle`。
  ...
  | 界面动作 | `/api/notebooks/{id}` 下的 POST 路径 | 启动状态 | GET 状态路径 | 计数 |
  | --- | --- | --- | --- | --- |
  ...
  **补上关联** 为零模型确定性动作，显式端点与成功构建尾段共用槽。尾段抢槽失败只发含 notebook id／holder 的 `kg_relink_skipped`；若被重新合并占用，工作直接跳过、不延后，缺少的关联需人工补。逐来源读取，包括空／已删来源分区及跨来源邻接；逐来源提交，后续分区失败仍发布已写部分的 KG 变更。每节点边上限生效时，稳定插入顺序可能选到与旧更新时间顺序不同的合法同源伙伴，但边数和孤立计数不变。

  **删除知识图谱** 要求 `kg:write` 且界面二次确认。它删除用户文档对象、关系、事实、向量、抽取记录／搜索行、簇、合并候选与分析产物，再重置统一状态；保留来源、解析元素、隐藏 Memory／Knowhow 投影，后者仍可使 `kg_ready` 成立。不清理 `kg_conflict_candidates`（可能仍指向已删对象，无界面展示），挂载方停止检索已删知识。提交后，在预算内为残留隐藏对象重发预览，无对象／超预算则退役预览；刷新失败或槽占用不使删除失败，旧预览可保留到后续刷新／索引构建。

  删除在抢槽后额外检查一次其他进程的 running `kg_build_jobs`，不覆盖随后才出现的外部任务。它不设置 build marker，`kg_building` 仍专指抽取。清理前将已终结构建行设为 `stage='cleared'`，保留统计记录、不触碰 running 行；最新 cleared 行投影为 `kg_build: null`，部分删除失败也不恢复旧历史，下一次分析正常展示。错误文本不外发，事件为 `kg_deleted`（计数）或 `kg_delete_failed`。删除笔记本会等待正在删图的任务，删图遇笔记本删除墓碑则停止；删图运行于重维护队列，与删除笔记本队列独立。

  增量 `POST .../kg/build` 最后至多补三轮，追上元素晚于游标落地的来源且不越过 `target_limit`。仍有未尝试来源时返回 `status=succeeded`、`error_code="kg_backfill_partial"`，交由下一轮增量构建继续。
...
- 群组、成员、邀请与笔记本授权路由见[群组共享端点](#端点面)，权限与生命周期在同章定义。
...
- `/api/notebooks/{id}` 下的深度报告路由遵循[生命周期与执行护栏](#深度报告生命周期与执行护栏)：
  - `POST /reports` 接收 `{question, depth?, auto_generate?}`，返回 `{report_id}`；`GET /reports/{rid}` 返回持久化理解、状态／进度、大纲、`content_md` 及实时 `section_status`。每节大纲包含 `intent_ids`、`intent_questions`、可编辑的 `sub_queries` 与客观 `coverage`，以及视角、张力和充分性。
  - `POST /reports/{rid}/intent` 接收 `{resolved_question, answers:[{id,answer}]}`；原子确认成功返回 `{status:"planning"}`，重复／过期确认返回 409。规划停在 `outline_ready`，除非原请求要求自动生成。
  - `PATCH /reports/{rid}/outline` 接收 `{sections}`，仅限 `outline_ready`，保留服务端意图目录；没有有效章节、丢失必答主题最后一个章节绑定或章节／方向超限均为 422。`REPORT_MAX_SECTIONS` 与 `REPORT_MAX_SUBQUERIES_PER_SECTION` 经 `/api/system/config` 同步约束浏览器，绝不静默裁剪输入。
  - `POST /reports/{rid}/generate` 接收 `{depth?}`，仅允许 `outline_ready` 或仍保有大纲的 failed 报告（否则 409）；重试保留已确认意图／大纲并原子清除生成产物。
  - `GET /reports`、`POST /reports/{rid}/cancel`、`DELETE /reports/{rid}` 分别列出、取消、删除。
  - `POST /reports/export` 接收 `{report_ids}`，按 notebook／创建者筛选已完成且非空的报告并返回 `reports.zip`。单个 `report.exporter` 必须返回完整有序批次，核心拒绝畸形／部分结果并负责 ZIP；默认 Markdown 保持原文件名与字节。浏览器单篇 Markdown 下载仍在本地完成。
  - 生成章节返回后端判定的 `evidence_level`／`grounded`，引用可含精确 `source_id`／`element_id`；报告并发遵循专用护栏，不跟随 `KG_JOB_CONCURRENCY`。
...
受信代理导入的内网地址豁免只保留给仍指向初始归一化 origin 的重定向；跳到其它 origin 时须通过公网地址检查，即使新 origin 另在名单内。
...
生产扩展点 `ask.gap_consult` 在逐步推理问答的库内检索不够用时提供笔记本**之外**的资料线索。建议不是笔记本证据：不打分、不绑定 `[k]` 锚点，也不改变已草拟的正文。独立、明确标记为未经核验的外部补充段可用 `[Xn]` 链接概述建议摘要。用户导入链接仍是按普通来源处理，走自己的解析和权限。

**触发条件**：仅逐步推理 Ask 生效；深度报告和 Knowhow 行补全不经过 `_run_reasoning_stage`。所有检索档位适用，每 run 至多在 **response-draft 返回之后**、持久化之前执行一次。触发判据只读草拟前事实：终态 `intent_coverage_incomplete` 仍有已确认却未执行的方向，或证据池（`top_hits + chunks + elements`）薄于当前档位的 `ranked_final_floor`；检索自身降级也可落入后一支。判断触发本身不增加检索查询或模型调用；触发后，来源选择与可选补充段各使用自己的模型 workload。草拟 envelope 没有外扩建议或轨迹，正文因此不会随站外结果变化。

**来源选择与外发面**：contributor 可通过 `describe_sources()` 给出 `SourceDescriptor`（`source_id`、显示名、语言、内容类型、检索建议）；提供给模型的 source ID 必须唯一，畸形、重复、不可用或未描述的来源不会成为候选。`gap_consult_query` 模型至多选择 `GAP_CONSULT_MAX_QUERY_SOURCES` 个候选，每个来源至多 `GAP_CONSULT_MAX_QUERIES_PER_SOURCE` 条检索词。它只接收可外发的 `question`、有界 `gaps` 和来源描述；对话历史、笔记本证据、私有身份、未经用户确认的风格或意图改写都不进输入。没有描述、模型未配置/失败、输出无效或未选中来源时，本轮**零次** `consult()`；宿主不会退回调用全部 contributor。只有明确选中的来源会执行。

插件收到的有界 `GapConsultQuery` 中，`question` 只有真走过澄清门才采用用户审阅后的最终措辞，否则采用原始问题，不外发自动确认的模型改写；上限为 `GAP_CONSULT_QUESTION_MAX_CHARS`。`gaps` 最多 `GAP_CONSULT_MAX_GAP_PHRASES` 条已确认未执行方向，每条上限 `GAP_CONSULT_PHRASE_MAX_CHARS`；两者剥除引用标记。`source_queries` 把选中来源 ID 映射到有界专属检索词。调用上下文没有笔记本/用户/笔记本来源身份或证据；这里的 source ID 只是站外路由键。`AskResponse.gap_egress` 持久记录 core 在调用 contributor 时**实际交给它**的可读来源名称与每源检索词，零结果或失败也记录；没有实际调用则缺席。core 无法观察 contributor 后续发给第三方的请求。每条接纳建议还可带有界的 `actual_query`，这是插件自报的实际检索词，core 不独立核验。

**预算与 fail-open**：冻结的 `GapConsultHost` 把一次 contribution 的可用性探测与它的 `consult` 调用一起放进一条私有 `daemon=True` 线程执行——不用线程池，一个挂死插件只在受影响的那次请求上泄漏一条线程，绝不占用全部署都依赖的共享 worker；也不 `contextvars.copy_context()`，插件不会因为跑在这条请求底下就继承它冻结的检索范围、检索 run 或叶子 I/O 扇出槽位——主线程按 50ms 分片 `join`，对着一个覆盖**整次调用**（探测与 `consult` 两半都算）的硬墙钟 deadline（`ASK_GAP_CONSULT_TIMEOUT_SECONDS`，默认 4.0 秒，`0 < x ≤ 30`）。取消与这个 deadline 在 join 循环的**每一趟**都会重读，包括观察到 worker 已经跑完的那一趟：预算花光之后才答出来的 contributor 就是答晚了，晚到的结果与挂死的一样被丢弃——只在线程还活着时读 deadline，会让结果取决于那 50ms 分片恰好落在线程死亡的哪一侧。这份预算花在读者拿到答案**之前**——与上文那些协作式的完成后 deadline 不同，这里每一秒都是答案的等待时间——所以默认值小、上限也收得紧。**一个真挂死的插件每次受影响的请求都会泄漏一条 daemon 线程；这是已登记、已接受的代价，不是缺陷——熔断器留待后续。** 除了干净、按时、形状正确的响应之外，其余一律 fail-open：没有注册的 host、休眠的扩展点（`has_contributions()` 为 `False`）、畸形调用上下文、调用方给出的空问题（host 同样不信任自己的调用方——实践中这条不可达，因为 `AskRequest.question` 早已强制 `min_length=1`，一个空问题根本走不到这个调用点）、抛异常的 contributor、预算耗尽，或者 host 答出一个端口从未承诺过的形状——全部返回 `()`，答案与接入前逐字相同。唯一的例外是取消，它会继续传播（`AskCancelled`）。「fail-open」指答案不变、不出错误横幅/诊断——**不**指 `gap_consult` 披露步会一并消失：这一步存在的意义就是告诉读者发生过一次有界外发，所以已发出的 contributor 调用即使超时、抛异常或返回畸形，也记下这次调用、建议数如实记 0；尚未调用 contributor 的阶段失败不生成外发记录（codex #584 R4 建议失败时抑制该步，已驳回；见 `ask_service.py::_consult_gap_sources`）。净化（title 非空；`url` 限定 `http`/`https`、host 非空、无控制字符，且绝不截断——截短的 URL 是一个悄悄指向别处的错误目的地，超长的直接丢弃；summary/source_label 裁剪截断）发生在 host 里、插件返回**之后**，插件自己够不到这一步。`status` 为 `UNAVAILABLE` 的结果**一条都不接纳**，无论它带没带条目：那个状态就是 contributor 自己声明「这次我服务不了」，它的载荷没有资格反驳自己（这些 URL 也因此不进跨 contributor 的去重集合，真正能服务这次调用的插件不会被一条已被撤回的链接挡住）；`PARTIAL` 说的是「只有一部分」而不是「一条都没有」，仍照常接纳。接纳这件事**本身的工作量**也是两个维度都有界的——最多只检查「还能收几条」的一个有界倍数，每个字符串在被走过之前先按自己的上限加余量切一刀——所以插件返回一份无界载荷，付出的是一次有界扫描而不是无界扫描，且这已经发生在上面那个 deadline 被兑现之后。这一刀切的是插件产出，绝不是用户数据。

宿主准入时，非空的 `source_queries` 必须是符合下表来源数、每源检索词数和词长护栏的原生映射；任一项畸形会使整个 query 无效。宿主不另设键名白名单；core 的规划器在构造映射前只选择已提供的来源 ID。来源描述字段、补充段使用的站外与库内摘录在进入模型前均有界；公开的来源描述护栏列在下表，补充段内部 prompt 投影不会改变公开响应上限。

**同一时间预算**从 `describe_sources()` 之前开始，覆盖来源描述、`gap_consult_query` 模型、宿主的可用性探测和选中来源的 `consult()`，以及有摘要时的 `external_evidence_answer`。各阶段不会分别获得新的 4 秒。查询规划本身至多使用配置预算的 40%，也不能延长共同 deadline。模型客户端获取失败、超时、模型输出或 JSON 畸形时，已草拟的正文保持原样；来源选择失败则不调用任何站外来源。
...
**导入**：建议面板「导入」仍走普通核心 URL 导入端点（`POST /notebooks/{id}/sources/url`、`sources:write`；只读成员看不到按钮）。PDF 直链继续支持。初始 origin 精确命中部署的 `URL_IMPORT_TRUSTED_PROXY_HOSTS` 时，响应也可为 `text/markdown`：探测将受信代理快照命名为 `.md`，解析下载随后交给内建 Markdown 解析器，不经过 MinerU。普通界面的「导入」按钮和插件导入端口都可走这条路径；其它 origin 仍只收 PDF，落地页和摘要页不算导入。只有继续指向同一归一化受信 origin 的重定向才保留内网豁免；跨 origin 跳转即使目标也另在名单内，仍须通过公网地址检查。每一跳的 URL 形态检查照常生效。拒绝原因在按钮旁展示，笔记本满额时先置灰而不发探测请求。

**从头到尾都不是笔记本证据**：`AskGapSuggestion`（`title`/`url`/`summary`/`source_label`/`actual_query`，没有笔记本证据 ID、分数或引用键）挂在 `AskResponse.gap_suggestions`，由 core 在草拟阶段**之后**填充。可选的 `external_evidence_answer` workload 使用有摘要的建议与有界库内摘录，另写 `AskResponse.external_evidence`（`text` 和结构化 `conflicts`：库内说法、站外说法、说明）。`[Xn]` 对应原建议列表中的位置，链接到该 URL、悬停显示标题；未知编号按原文显示。界面将该段标为未经核验，单独展示冲突。建议和补充段均不改正文、`[k]` 引用、`anchors`、`grounded`、`evidence_level` 或覆盖率。`gap_egress`、建议、补充段可在私有历史回答重开，但公开会话投影不带。`gap_consult` 轨迹步（「外扩」）只记录触发条件和建议数，不记问题和 URL；可选补充段另记「外部补充」步骤。
...
| `GAP_CONSULT_QUESTION_MAX_CHARS`（问题与每条专属检索词） | 300 |
...
| `ExternalEvidenceConflict.kb_says` / `external_says` / `note` 最长字符数 | 400 / 400 / 300 |
| `ASK_GAP_CONSULT_TIMEOUT_SECONDS`（默认值；部署可配置，`0 < x ≤ 30`） | 4.0 |

### Reflect 插件动作（`ask.reflect_action`）

一个新的生产扩展点让部署插件把自己的一个函数（典型是互联网或文献检索）借给逐步推理 Ask 的检索 Agent。注册一次只贡献**一个**动作：它的名称、一段描述与各参数描述会被投影进 reflect 的 prompt、reflect 的 schema hint 与动作白名单，由模型在库内检索通道反复空手之后自行决定要不要调用。与[缺口外扩检索](#缺口外扩检索)不同——那是确定性触发、每 run 恰好一次、只在 response-draft 阶段返回**之后**才调用、产出从不是证据、动不了正文一个字——这里是**模型触发**、在 reflect 循环**内部**调用，产出是真正的外部证据：进合成、可用 `[k]` 引用、带一个可打开的 URL。两个扩展点并存，本节不涉及 `ask.gap_consult` 的退役。

**触发条件**：同样只在逐步推理 Ask 生效，`ask.gap_consult` 够不到的调用点（报告逐节深挖、Knowhow 行补全都各自构造 `ReasoningRetriever` 直接调 `run()`，拿不到这份 run 级状态）这里同样够不到。「哪些动作已注册且当前可用」只在**run 一开始问一次**——不会每轮 reflect 重新探测——所以这是一个 run 级拓扑事实，不是逐轮事实：管理员在 run 进行中把某个插件关掉，这个 run 剩下的轮次里它仍然形式上「已提供」；但宿主自己会在每一次 `invoke()` 调用内重新核对实时可用性，所以针对它发出的调用仍会以一条普通的 `plugin_action_failed` skip 收场，而不会真的打到那个已被关闭的插件——开关对「本轮是否提供」这件事的影响，要到这个 run 下一次重新开始时才生效。在 run 级拓扑非空的前提下，另外四个条件在每一轮 reflect 都重新判定：档位不是本仓库档位表里最省的那一档——`overview|standard|deep|thorough|exhaustive`，只有 `overview` 关闭、其余全开，而完全没有档位概念的调用方（报告逐节深挖、Knowhow 补全、窄测试替身）按关闭档处理，因为这条通道的成本必须由某个明确的档位选择买单；`REASONING_MAX_PLUGIN_ACTIONS` 大于零；调用方真的接上了那把外泄闸后的问题串（空串直接 fail-closed 关掉整条通道，而不是退回一个审查更松的替代品）；以及本 run 已经在库内某个通道至少空手过一次——某个动作的连续零命中计数大于零，或者上一轮 reflect 是 `no_progress` 或 `stale`。最后这条是唯一一条真正意义上的事实闸而非路由规则：它从不替模型做选择，只是在库内确实试过还没找到之前不给出这个选项——包括第一轮本身检索就空手的情形，那时第一轮 reflect 就已经满足条件。run 级拓扑为空的一轮，或者四个逐轮条件里任一不成立的一轮，其 `reflect_prompt`/`reflect_schema_hint`/白名单就与没有这个插件的部署逐字节相同。

同一轮的库内事实闸因无进展、证据陈旧或零命中打开，且 reflect 插件动作确已提供时，core 在送给模型的该轮 summary 末尾追加固定中文引导：问题可能依赖站外或近期信息时可调用已列出的站外检索动作，确认不适用再直接作答。引导不引用插件文本；动作闸关闭时不出现。

**外发面**：一个 `ReflectActionCallContext`：`question`（用户实际看过的措辞——合成的 `research_question` 意图契约永不外送，与 `ask.gap_consult` 同一条规则）加 `arguments`，模型自己为该动作声明参数写的一份扁平字符串映射，已经按描述符校验过（未识别的枚举值在到达插件之前就被清空；自由文本夹到 `REFLECT_ACTION_ARGUMENT_MAX_CHARS`；未填的可选参数以空串出现，因此每个声明过的参数键永远都在）。候选文本、子查询、缺口短语，以及任何 notebook/actor/source 身份都不会进入这个上下文——这个调用上下文类型压根没有对应字段。参数会逐字写进持久化的 `plugin_action` 轨迹步 detail，这正是本特性用来代替内容过滤的透明机制：提问的人始终能看到到底有什么文字以他的名义离开了部署。

**预算与 fail-open**：推理宿主把一次 contribution 的可用性检查与调用一起放在一条私有 `daemon=True` worker 上、按切片 `join`，对着一个硬墙钟 deadline——`REASONING_PLUGIN_ACTION_TIMEOUT_SECONDS`（默认 8.0 秒）——这与 `ask.gap_consult` 同一套不进线程池、不 `contextvars.copy_context()` 的形状，理由也相同：一个挂死插件只在受影响的这次请求上泄漏一条线程，不占全部署共享的 worker；插件也不会因为跑在这条请求底下就继承它冻结的检索范围或叶子 I/O 扇出槽位。三重预算叠加：描述符自己的 `max_calls_per_run` 与部署策略的 `REASONING_MAX_PLUGIN_ACTIONS`（默认 2）取两者较小值作为该动作自己的单动作上限；`REASONING_MAX_PLUGIN_ACTIONS` 同时也是整个 run 跨全部动作的调用总额；`EXTERNAL_EVIDENCE_MAX_PER_RUN`（默认 10）则是一个 run 跨全部调用、全部动作累计能接纳的条目上限。上面这两笔单动作/run 总量预算都在**发起调用之前**扣，不是拿到结果之后——按成功才扣会让一个总是超时的插件把每一轮都白送模型一次 8 秒的墙钟等待，所以一次只是失败或超时的调用也照样花掉一格额度。真正被记住的是（动作名，规范化参数）这个组合**成功过**：只有真的接纳到材料的一次调用才会把这个组合标记为「见过」，因此同样的参数在一次失败之后可以重试（重试风暴改由上面那笔发起前就扣掉的预算挡住）——会被跳过的是重复发出**已经成功过**的那组参数。skip 步 `detail.reason` 里固定出现六种码，措辞一律写成教学式而非指责式：`plugin_action_disabled`（纵深防御——本轮其实没提供这个动作）、`plugin_action_missing_argument`（某个必填参数为空，点名缺哪个、该给什么）、`plugin_action_cap`（单动作、run 总量或接纳预算三者之一已经用完）、`duplicate_plugin_action`（同一个动作名与规范化参数在本 run 内已经成功取回过材料）、`plugin_action_last_turn`（已经是最后一轮，没有下一轮 reflect 能消费结果，与 `consult_memory` 同款末轮判据）、以及 `plugin_action_failed`（宿主这次什么都没能取回——超时、取消、结果畸形或状态为 `UNAVAILABLE`、或 contributor 直接抛出，全部折成这一个码，宿主自己的稳定子码写进 `detail.code`；这条步与成功的 `plugin_action` 步一样**逐字**带上 `arguments`——请求在失败可被观测之前就已经离开部署，所以「有什么替我发出去了」这个审计问题的答案不该取决于它成没成功——外加 `attempted: true`，用来把「尝试过」与成功步的「送达了」区分开）。取消从不会从宿主里抛出来：`invoke` 用 `plugin_action_cancelled` 这个失败码作答，reflect 循环随即重读**自己**的取消令牌并从那里继续传播——取消依旧在第一时间终止整个 run，只是判据留在循环自己手里。一次调用接纳到零条目不算失败：它照常记一条 `found: 0` 的 `plugin_action` 轨迹步，并给该动作名计一次零命中，喂给的正是触发闸读的那份「库内已经空手」的事实。这里出的任何差错——没有插件、动作不可用或行为异常、返回形状畸形——除了留下那一条 skip 步之外，run 与没有这个插件的部署逐字节相同。

**可用性**：插件像其它任何 contributor 一样，经自己 manifest 的 `provides` 声明它探测的 capability，再加管理员的运行时插件开关（#635 三闸口）。刻意不加独立的 `*_ENABLED` 设置：插件注册本身已经是显式的、启动期冻结的配置，管理页开关已经是一个实时开关，而 `REASONING_MAX_PLUGIN_ACTIONS=0` 本身就是一个完整的部署级 kill switch——一到零，prompt 行、schema 分支与白名单条目三处同时消失；再加第四把开关只是多一处必须同步的地方。没有注册任何动作的部署零探测调用、零事件——宿主自己的 `specs()` 立即返回；调用方所在的这一轮仍会照常算它自己那一份固定的每轮 deadline，与通道因任何其它原因关闭时一样。

**外部证据**：被接纳的条目成为证据时带 `tier="external"`、`object_type="external"`，`source_id`/`element_id` 永远为空——是结构性的空，不是没填，因为外部材料在任何 notebook 的来源或元素表里都没有一行。它身上的每个字段——标题、摘录、来源标签、位置标签——在接纳那一刻就会过一道控制字符折叠：任意一串控制字符、行分隔符或双向覆盖字符都会折成一个空格，这正是阻止一段带恶意换行的摘录在下面两种块里伪造出第二条证据行的机制（两种块都是一行一条，反查回某个已接纳条目正是靠这个形状撑住的）。reflect 自己的候选摘要预览与最终合成上下文渲染的是**同一批**已接纳条目，但用的是两种不同的形状，不是一种。**预览块**（`_external_block_text`，每次调用被接纳后重建——包括零条目但带 `note` 的那一次，此时抬头换成 `[External evidence] (nothing admitted so far)`、块里只有 `Note:` 一行，插件对空手检索的解释照样进下一轮；只有条目与 note 都为空时整块才不出现）以一句固定的抬头开始：`[External evidence] (outside the library; citable, labelled [external · <source>])`，随后每条一行：`x{n} · [external · <source_label>] · <title> · <excerpt>`——`x{n}` 只是这份预览自己的显示编号，绝不是引用真正使用的 `[k]` 号——摘录截到前 160 字符（够模型判断这份材料讲的是什么，不是把材料再喂一份全文）；放不下的条目由一行可选的 `…(+N)` 交代还有多少条被这份预览自己的 `EXTERNAL_EVIDENCE_REFLECT_BLOCK_CHARS` 预算（默认 1,600 字符，按整条花费）挤掉；再往后是一行可选的 `Note: <note>`，装插件自己那句一句话观察。**合成块**（`evidence_context.external_context`）渲染的则是 `k{n}: [external · <source_label>] <title> (<location_label>) — <excerpt>`，摘录是完整版、号是真正的 `[k]` 号，花的是它自己独立的 `EXTERNAL_EVIDENCE_CONTEXT_CHARS` 预算（默认 4,000 字符），拼在库内各段**之后**。`url` 为空或不是 `http://`/`https://` 开头的条目在合成块里整条跳过，不会半渲染，也不占号——已接纳的条目编号永远连续不留空洞。两种块里，放不下的条目都整条丢弃，不切半条，因为一句被截断、却仍署名某个外部来源的引文是一次误引，而不只是一次缩短的引用。按节合成时，**每一节**都会拼上同一份合成块（与只留在回退路径上的 Memory/推导链/集合地图三段刻意相反），因为外部材料不像那些内容那样绑定在某一节的大纲上，只喂给某一节会让合并后的引用表里出现一个某一节自己的草稿从没见过的 `[k]` 号。`answer_prompt` 只在合成上下文里**真的**出现过 `[external · …]` 条目时才追加一条编号规则：像其它条目一样用 `[k]` 引用，但绝不描述成笔记本或库内内容，遇到与笔记本内容矛盾时要点明哪边是哪边、不得混为一谈；没有外部证据的一轮里 `answer_prompt` 与接入这个特性之前逐字节相同。引用只会发给真的进过模型看到的那次 prompt 的条目——被单条预算挤掉、或没通过 URL 校验的条目永远进不了引用列表，轨迹用 `external_included`/`external_dropped` 两个数稀疏地披露这个差值（只在真的有外部证据浮现的一轮才出现，绝不会给一次普通问答的持久化 detail 里恒零地多写两个键）。`grounded`/`evidence_level` 的语义不变：一个外部锚点走的是与精确检索命中同一条确定性、非排序的证据键判据，从不为它凭空造一个相关度分数——毕竟没有任何部署内部件真的给这份材料打过分。

**前端**：`object_type == "external"` 的引用卡上会出现「打开链接」动作（只在 URL 是 `http`/`https` 时渲染，新标签页打开、带 `rel="noopener noreferrer"`）与「导入为来源」动作，后者复用与站外来源建议（Gap consultation）导入按钮完全同一个核心 URL 导入端点与回调（`POST /notebooks/{id}/sources/url`，`sources:write` 能力）；只读工作区从不接上这个回调，两个按钮都不渲染。两个面共用一份按 URL 索引的行状态机：在站外建议面板或外部证据引用卡任一处导入同一个链接，另一处会立刻显示「已导入」，因为后端导入端点本身既不单飞、也不按 URL 去重——跨轮再导入同一个 URL 是一次普通的重新导入，不是空操作。「来源分布」汇总标签的第三格「外部 N」只在这个数大于零时才渲染；没有外部证据的一轮里，这枚标签与接入这个特性之前逐字相同。`plugin_action` 轨迹步的前端标签是「扩展检索」；它自己的 skip 步原样渲染后端给出的 `summary` 文案，与其它 reflect skip 步同一惯例。模型替用户发出的参数分两层披露：折叠态的步骤行只显示夹到 120 字符的摘要（以 `…` 标出）；展开态逐项列出每个非空参数的完整值，一行一个 `参数名: 值`，一个字不夹——成功的 `plugin_action` 步如此，带 `attempted: true` 的 `plugin_action_failed` skip 步同样如此，因为请求无论成败都已经离开部署；没有 `attempted` 标记的 skip 步（预算、重复、末轮）从未发出任何东西，也就没有可披露的参数。
...
| 设置 | 默认值 | 说明 |
| --- | ---: | --- |
| `REASONING_MAX_PLUGIN_ACTIONS` | 2 | run 级、跨全部动作的调用总额；`0` 是一个完整的部署级 kill switch。 |
| `REASONING_PLUGIN_ACTION_TIMEOUT_SECONDS` | 8.0 | 单次调用的硬墙钟 deadline，可用性检查与调用本身共用同一份预算。 |
| `EXTERNAL_EVIDENCE_MAX_PER_RUN` | 10 | 一个 run 跨全部动作、全部调用累计能接纳的条目上限。 |
...
页签。每个用户行同时保留「查看提问」与「LLM 日志」两个入口：前者进入该用户问答提问的
分析页签，后者进入按该用户过滤的「模型调用」日志视图。提问分析可在「问答」与「深度报告」之间切换，
选中一份报告即只读显示其正文与引用；解析问题可按用户、状态、类型和
...
- `GET /api/admin/analysis-issues?owner_id=&status=&category=&model_area=&offset=&limit=` —— 仅管理员可读；
  `status` 为 空/`open`/`resolved`，`category` 为 空/`source_parse`/
  `spreadsheet_analysis`/`model_output`，`model_area` 为 空/`ask`/`report`/`source`/
  `knowledge`/`memory`/`knowhow`/`retrieval`，`offset` 为 `>= 0`（默认 0），`limit` 为 `1..500`
  （默认 200）。按最近更新倒序；响应包含 `items` 与 `total`（同一组筛选与过期清理之后的全部匹配数），
  因此超过一页的记录不会被静默丢掉。「解析问题」页签每页请求 50 条，改动任一筛选条件都回到第一页。
  刻意没有配套的 POST/PATCH/DELETE。
...
- **范围** —— 被查看用户自己的笔记本列表（名字 + 来源/提问/报告计数），可展开看该库的来源清单（显示名 + 解析状态徽章）。点笔记本把中栏活动流过滤到该库；点来源在右栏详情打开它（**不**过滤中栏，见下文「未做」）。笔记本列表整份取回、每页显示 20 本；选中的笔记本在后面的页上时（例如解析问题深链直接选中的那本），列表翻到它所在的页。来源清单先取前 50 个，仍有剩余时如实显示「已显示 N / M 个来源」，「加载更多来源」追加下一批 50 个（`offset` = 已显示条数，按来源 id 去重）。该页在途时按钮禁用；追加失败只在按钮旁报错，已列出的来源保留。换被查看用户或重新取第一页时，迟到的追加页作废。
- **活动流** —— 提问 / 来源 / 报告三类条目按时间倒序混合成一条流，按 `(created_at DESC, id DESC)` 复合游标翻页（「加载更多」），默认每页 50 条（`limit`，上限 200）。类型筛选可在全部活动和只看提问/来源/报告之间切换；只看提问时中栏改称「提问概览」，每条问题原文完整显示。**全局问答**的作业同样以 `type="ask"`、`scope="global"` 混进这条流，按提交者 `user_id` 取、带「全局」标签，`notebook_id` 恒为空串——它不属于任何笔记本，所以显式按笔记本筛选（`notebook_id=`）时不出现，混合流与只看提问都会包含。本人自助读取这条列表时按与笔记本内提问同一条规则过滤：参与库集合必须**整体**仍可被该用户读到，否则这一行不出现（判定与笔记本内提问一样在各表 `LIMIT` **之前**生效，页不会被过滤掉的行吞掉，`has_more` 与游标口径与其余三类相同）；管理员审计不过滤，与已失权共享库提问同口径。打开单条详情时套用同一条判定，见下文 `GET /admin/users/{user_id}/asks/{job_id}`。全局问答行的失败原文取 `error_detail`（仅管理员），该列上线前的失败行回落到给用户看的那句固定文案。用户使用总览里的「查看提问」直接进入该用户的全部提问，用 `ask_jobs.created_by` 与总览计数保持同一口径，包含该用户在共享笔记本中的提问；管理员仍可按自有笔记本和日期缩小范围，用来整体查看用户关注点。只看报告时中栏改称「报告概览」，按同一规则与总览报告计数的 `reports.created_by` 口径对齐，包含该用户在共享笔记本中建的报告。筛选在 repository 的各表 `LIMIT` **之前**生效，不是在浏览器拿到混合页后再过滤，因此更晚的来源/报告不会挤掉提问，所有问题都能通过复合游标继续翻到。原生日期选择器 + 「全部时间」驱动一个半开区间 `[since, until)`。区间边界带**浏览器 UTC 偏移**（`2026-08-04T00:00:00+08:00`），所以「一天」指的是浏览者本地日历日——与界面上一切时间的渲染时区同一个；两端各自算偏移，夏令时切换日不会差一小时。畸形边界返回 `400` 加可上屏的中文文案（`X-User-Message`），既不静默放宽窗口，也不 500。三类条目在两个数据库后端都经同一个 `parse_activity_instant`（`app/core/activity_time.py`）归一时间：SQLite 侧经 `julianday()` 按绝对时刻比较（历史行是裸 naive 文本、新行带 offset，混合格式统一按裸串=UTC 读），PostgreSQL 侧直接按 `timestamptz` 原生比较，并带与 SQLite 同形的 `COALESCE` 兜底（`ask_jobs.created_at` 在 PG schema 里可空，而 PG 的 `DESC` 默认 `NULLS FIRST`，少了兜底这行会被顶到第 1 页并让 Python 归并直接 `TypeError`）。两侧把不可解析 / NULL 的值折到**同一个哨兵时刻**：它既排到最末、又能经游标往返，`next_cursor` 因此不会发出下一页解析不了的值。三类各自独立 keyset 分页、在 Python 内按时间归并，不做三表 `UNION ALL`——大库上可诚实回报「本页覆盖到 HH:MM 为止」的有界边界，而不是一次全局排序爆炸。
  共享库提交按调用者权限区别处理：管理员保留按创建者归属的完整审计历史，即使被查看用户后来失去共享库读权仍可查看；普通用户查看自己时，只能看到笔记本当前仍通过规范实时读权谓词的提交。提问详情与报告详情复用同一条自助读权检查，因此保留旧 job id 或报告 id 也不能在失权后绕过清单读取内容。失败原文只作为管理员诊断：`ask_jobs.error` / `reports.error` 是执行路径捕获异常后原样落库的串，可能带内部模型网关地址或服务端路径，因此活动流里的提问条目与两个详情端点只在调用者是管理员时返回它（管理员本就能按用户查看模型调用日志原文）。本人自助读取拿到空 `error`、`status` 照常返回，详情栏对失败的提问或报告显示固定文案——与来源条目只给 `parse_failed` 布尔、深度报告工作区界面只显示固定失败文案同一方向。这条规则只覆盖活动可观测面；其它携带失败原文的 API（例如笔记本内的报告详情、提问任务状态与流式事件、本人自助的模型调用日志）维持各自既有契约。
  删除笔记本另有一条有界规则：删除聚合的同一事务会把其中的可见来源、提问和报告投影成一份**不带 notebook 外键**的最小分析摘要。默认从删除时刻起留存 **180 天**，可用 `USER_ACTIVITY_RETENTION_DAYS` 配置（`1..3650`）。摘要只保留归属、原笔记本 id/名称、活动时间、问题/报告提示、提问/报告的 `submitted_via` 调用方式、来源显示名/文件元数据、状态以及删除/到期时间；答案正文、引用、推理轨迹、来源正文/元素、报告章节与参考文献仍随笔记本立即删除。若离线合库已在同 id 的存活笔记本旁放入旧摘要，只要笔记本仍在线，这组留存数据就不参与统计、活动流或详情；之后实际删除时会先丢弃整组旧摘要，再按当前聚合重新投影，并从这次真实删除时刻重新开始留存窗口。窗口内，管理员用户使用总览的来源/提问/报告总数和未按笔记本收窄的管理员活动流继续计入这些摘要，条目带 `notebook_deleted_at`/`retained_until` 并标记「原笔记本已删除」；已删除笔记本不再出现在范围栏，也不能作为笔记本筛选项。本人自助读取仍要求笔记本实时可读，因此删除后的摘要只对管理员可见。`expires_at` 是强读闸，到期后统计、活动流和详情立即不可见；启动恢复或下一次删除笔记本时再做物理清理。这是滚动时间留存，不是备份，也不是无限审计日志。
- **详情** —— 选中仍存活笔记本的提问显示完整问题、答案、引用与推理轨迹（复用既有 Ask 面板渲染件，只读）；删除后留存的提问只显示问题/状态摘要及删除、到期说明。选中全局问答的作业额外显示「参与笔记本」——上文 `notebook_ids`/`notebook_names` 投影出的参与库集合。失败的提问或报告对管理员显示记录下来的失败原文；没有原文返回时（本人自助读取一律如此，或该行本就没有记录）显示固定文案，服务重启中断的提问会说明中断。选中仍存活笔记本的报告显示状态、深度、耗时，以及带引用的只读报告正文（复用深度报告的 Markdown 渲染件，不加载附图、不跳转来源）；尚未生成正文时明确说明，删除后留存的报告只显示研究问题/状态摘要及删除、到期说明。选中来源显示显示名、解析状态与派生诊断；选中模型调用（仅「模型调用」视图内）复用既有 prompt/response 详情面板，未改动。
