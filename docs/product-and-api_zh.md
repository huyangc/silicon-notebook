...
另有两个与档位无关、也**不是部署参数**的 v2 协议常量，约束的是模型在同一次 reflect 里回传的方面自评载荷：一个方面最多带 **8** 个证据键（`REFLECT_ASPECT_MAX_EVIDENCE_KEYS`），一个方面的 `gap`（模型写的「还缺什么」）最多 **240** 字符（`REFLECT_ASPECT_GAP_MAX_CHARS`）。它们没有环境变量，因为改动它们就是改协议本身而不是调预算。⚠ 这两个上限**只针对模型生成的内部载荷**：超限的处理是把这一轮的决定整份折成 invalid（走与其它零 I/O skip 相同的记账），绝不用来裁剪用户的主题、问题或约束——用户内容在方面记录里一个字都不截。
