...
`group_routes.py` 里 21 个端点（含 P2 审批流 7 个），另加笔记本 router 上一个只读端点。
...
**outcome 口径是已登记的 v1 限制，不是疏漏。** 「步→锚点归因」——「这一个检索步产出的证据，就是最终答案实际引用的那些」——在今天的持久化里不可恢复：`ask_trace_steps.detail` 对 `retrieve`/`ppr`/`exact_lookup`/`expand`/`enumerate` 只记计数，从不记结果 id；只有 `outline` 步（仅 `exhaustive` 档）持久化原始证据 id。因此 v1 只能在**步粒度**观测失败（零命中的动作、整次 run 的引用/接地结果），只能在**run 粒度**观测成功（这次 run 自己的动作组合，以及自己的引用数/`evidence_level`）——说不出「答案实际引用的是哪一步的结果」。要恢复步级成功归因，得改热路径的 trace 写入面，并配一份新的脱敏论证，v1 刻意把它留给后续阶段。
...
**只经由轨迹可见。** v1 不为它新增 `AskResponse` 字段，也不做管理面板——一张全局的、跨用户的、带 `support`/`adopted` 计数的封闭词表表，即便已经封闭化，也仍然是关于别人用量的信息，所以唯一面向用户的界面是轨迹里的一步轻量展示（界面标签「打法」），报告送达条数与字符数，与集合地图同一条「集合地图式内部脚手架」登记口径；面向管理员查看库内容的界面留给后续阶段。
