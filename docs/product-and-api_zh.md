...
**收尾那一轮必须自评。** 判据是**收尾载荷**本身：`next_action=answer`，或任何带 `sufficient=true` 的动作（`update_outline` 这类不产证据的动作与 `sufficient=true` 并存是合法的，而穷尽档最常见的收尾恰恰是「最后一批绑定补上、同一轮宣布够了」）。这两种载荷都会当场终止整次检索，而那一刻方面账就是这次检索唯一的成色记录。这样一份载荷若一个方面都没自评，服务端**整轮退回**（一条零 I/O 的观察，原因 `missing_assessment`，与其它 skip 走同一份步数/无进展记账，但**对 stale 持平、不递增**——与送达了内容的 consult 轮同款判据，追问上限已由 `REFLECT_ASSESSMENT_MAX_PROMPTS=1` 兜住，不会被反复利用），并在下一轮的服务器状态块里逐个列出方面 id 追问；同一轮如果带着 `update_outline` 的载荷，那份大纲**先应用再退回**，模型这一轮真正做成的绑定不会因为追问而丢掉。追问句只挂被退回的**下一轮**那一次，不是此后每一轮。追问**最多一次**，第二次仍然不给就照原样收尾，把还没被判断过的方面记为「问过了、模型不给」（快照字段 `assessment_omitted`，收尾那条跳过步上的计数是 `aspects_assessment_omitted`）——它与「模型根本没走到收尾那一步」在放量评估里因此可以分开数。**步数已经用尽的那一轮不退回**：追问的全部价值在于下一轮补上，没有下一轮时退回只会把模型自己的判读改写成「没步数了」，所以直接按「第二次沉默」接受收尾并记 `assessment_omitted`。
