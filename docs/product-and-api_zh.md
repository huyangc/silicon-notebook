...
- 混合检索：CJK 感知 bi-gram 关键词 + float32 语义检索（每 notebook 独立缓存）。服务生成的已确认意图查询把完整问题契约保留给候选生成、语义 embedding、编排与轨迹，只有 KG 关键词/RRF 打分使用精确契约分隔符之前的检索方向；确认后的约束仍参与召回，重复模板也不会稀释关键词覆盖。拉丁字母模型名在召回与打分两侧获得窄范围分隔符别名（`Cosmos3` ↔ `Cosmos 3`，连字符/点号变体同理），同时不改写 `chapter 4` 这类编号散文标签或 `ZXCV9000` 这类普通标识符。SQLite FTS5 保留整句精确匹配加分，同时以安全引用的 OR 词项召回拉丁字母/数字词、重叠中文三字片段，以及 `_`/`-`/`.` 连接的完整标识符（`set_db` 这类，受「须含字母、长度 ≥4、至多 16 个」约束）；PostgreSQL 在原生 trigram 候选生成前拆分同一组有界词项，并对 `ILIKE` 分支转义 LIKE 元字符，使 `set_db` 这类词项保持字面量，不会退化成通配把 `setXdb` 也拉进候选。带索引的 KG 检索与普通 Ask 的带索引 Chunk 检索仍合并有界 ANN 与词法候选窗口；深度报告 planning/generation 默认只用范围完整的 chunk ANN 核，不再为每个自然语言子查询重复叠加通用词法候选。冻结范围同时含已索引与尚未 fold 的来源时，保留已索引 ANN 候选，只给 sidecar 中缺失、已授权且确有 chunk 的来源补一次有界词法检索；没有 producer scope 的报告调用会在每个候选 producer 前冻结 actor 的授权来源全集，并在本 retrieval run 内复用。该补偿恢复的是词法可见性，不是对已索引来源内部变更或无词面重合 delta 的语义强一致；这些内容仍须完成 fold 或开启 `SCALE_SEARCH_INCLUDE_DELTA=true`。这条仅报告生效的快路绝不关闭独立的精确短语/标识符通道；chunk ANN 缺失或不可用时仍在同一冻结 ceiling 下走有界词法回退。`CHUNK_FTS_WITH_ANN_ENABLED=true` 可让报告恢复历史 ANN∪FTS 合并。带索引的 Relation 检索按方向平衡补入与词法命中 KG 端点相邻的关系并保留端点顺序。纯词法候选按 keyword-only 参与融合，不会被写入伪造的零语义分。
...
- 两层知识库：每个 notebook 带 `tier`（`base` | `personal`，默认 `personal`）。`chunk` 基线只从当前 active notebook 读取 chunk；可选 KG overlay / PPR 才可能加入 federated KG 上下文与 base-backed chunk，`reasoning` 使用 federated KG 路径。exact-score 的 `base` 次序只适用于知识对象命中：`federated_retrieve()` 不改相关度分数，分数更高的 personal hit 仍排在前面；`federated_retrieve_relations()` 的关系命中仍只按 score 排序。回答合成阶段另有独立规则：当 base 与 personal 证据冲突时，以 base 立场为准并指出差异。引用携带其 tier（`AnswerAnchor.tier`），Ask 在每条引用上渲染 `base`/`personal` 标记。`Citation.notebook_id` / `AnswerAnchor.notebook_id`只在证据来自其它参与库时非空（等于当前笔记本时归一为 `""`）；所有 Ask 模式——包括 graph 模式的子图锚点、挂载的私有库与 spreadsheet 工作簿——都遵守这条规则，界面据此显示来源库名，「在图谱中定位」也定位到该库。
- **用户系统**：自助注册（用户名规则：单个字母 + 8 位数字，如 `a12345678`，存储为小写）+ 密码登录，使用不透明 Bearer 会话 token。每个 notebook 由其创建者所有；用户库包含自己拥有的 notebook，以及主动加入的大型只读共享 notebook。首次启动时自动创建内置 `admin` 账号（登录用户名 `admin`，密码来自 `SILICON_NOTEBOOK_ADMIN_PASSWORD`，本地默认 `admin`；production/对外监听必须修改），并由它持有原有 notebook。管理员可在用户使用总览通过 `PATCH /api/admin/users/{user_id}/role` 授予或撤销 `admin` 角色；内置管理员和当前操作管理员自身不可被降级，已有会话会在下一次请求时读取到新权限。用户使用总览先对 `/api/admin/users` 返回的完整集合排序，再按页显示；默认每页 20 条，可切换为 50/100 条，并可点击各数据列表头切换升降序。总览及 `GET /api/admin/users/{user_id}/notebooks` 的问答用量字段统一为 `questions`，按归属目标用户的持久 `ask_jobs` 提交次数计数（失败/取消任务也计入），而非 `conversations` 会话容器数量；同一会话内连续提问会分别累计，共享成员的提问不会算到笔记本所有者名下。用户总数包含其在加入的只读共享笔记本中的提交；`GET /api/admin/users/{user_id}/notebooks` 刻意保持 owner-only，因此只分解自有笔记本里的提问，其合计不要求等于用户总数。报告数同此口径：总览按报告创建者计数（含在共享笔记本中自建的报告），明细仍只列自有笔记本，合计同样不要求等于总数。用户级 `sources` 总数按上传者归因（见下文），而每库明细按自有笔记本里的全部可见来源计、不问上传者，因此只要存在共享库上传或留存来源，两者合计同样不要求相等。总览的 `last_active` 取可见来源上传、Ask 提交和深度报告创建三类用户动作中最新的 `created_at`，删除笔记本后仍在留存期内的活动也计入；后台解析或生成后续更新不会把已经离开的用户显示成刚刚活跃，Memory/Knowhow 的内部合成来源也不会误刷新。总览的 `sources` 总数与 `last_active` 的上传候选同一谓词：只算 live 笔记本、只算可见来源类型，并按 `COALESCE(uploaded_by, 笔记本.created_by)` 归因——往共享笔记本上传的人计入实际上传者而非笔记本 owner；`uploaded_by` 为 NULL 的行（深拷贝副本或早期未回填行）归笔记本 owner，这是与 `last_active` 的动作口径刻意不同的资产口径决定；已删除笔记本里仍在留存期内的来源按留存快照的 actor 归因，actor 为空（留存时 `uploaded_by` 为 NULL）则回落到删除当时的笔记本 owner，删除笔记本不会让这类来源从所有人的计数里消失。总览同时为每个用户返回一组使用强度与采纳度信号；它们不新增主表列或排序键，只在展开某行的「用户摘要」区里呈现。`last_seen` 即 `users.last_seen_at`：登录与之后每次已认证请求都会刷新它，受 `AUTH_SESSION_TOUCH_INTERVAL_SECONDS` 节流；登出或会话被吊销后该列仍保留（不像底层的会话行那样被删除），Agent 长期凭据不会刷新它，`null` 表示该账号自加入这一列以来还未上线过。这与 `last_active` 是刻意不同的两种度量：`last_seen` 回答「用户是否来看过」，`last_active`（口径不变）只回答「用户是否产出了一次上传、提问或深度报告」——这一行同时展示两者，而不是用一个取代另一个。`storage_bytes` 与上文 `sources` 用完全同一套归因与过滤对 `file_size` 求和。`questions_30d` 统计最近 30 天内创建的 `ask_jobs` 提交（含仍在留存期内的）。`questions_failed` 与 `reports_failed` 只统计 `status='failed'` 的记录（取消不算），含仍在留存期内的活动。`kg_builds` 按创建者统计该用户的 `kg_build_jobs`，所有状态都算；这类记录不进留存快照，因此笔记本删除后其构建记录随之消失。`memory_count` 统计该用户在 live 笔记本里 `status <> 'rejected'` 的 Memory 条数。`knowhow_tables` 按创建者统计 live 笔记本里的 Knowhow 表，含深拷贝在接收方名下建出的副本。`joined_notebooks` 统计作为成员加入他人 live 共享笔记本的数量。`groups` 统计所属群组数。主表的列集合与排序键不变。展开某行后，在既有的笔记本明细之上新增两行「用户摘要」：第一行是使用强度——最近上线时间、存储、近 30 天提问数、提问与报告各自附带的失败数，以及标注为「图谱整理」的图谱构建次数（与界面其它处「整理知识图谱」动作同词）；第二行是采纳度与协作——「记忆」（Memory）、Knowhow 表、加入的共享库、群组。失败数为 0 时不显示括号；`last_seen` 为空显示「—」；字节按 1024 进制（KB/MB/GB/TB）显示，保留一位小数；不足 1 KB 时按整数字节显示。旧 `conversations` 字段为 API 兼容继续返回并标记 deprecated。每个笔记本对用户上传的文档数量设有上限（默认 20，可由 `USER_UPLOAD_DOCUMENT_LIMIT` 配置）；管理员在用户使用总览调整——设置全局默认（`PATCH /api/admin/settings/upload-limit-default`）并为单个用户设置覆盖值（`PATCH /api/admin/users/{user_id}/upload-limit`，传 `null` 清除覆盖、回落全局默认）；管理员拥有的笔记本不受此限。任何管理员都可将 notebook 发布为公共知识库。公共知识库对普通用户的列表隐藏，但可在每个笔记本的参考库选择器里发现，仅对显式挂载了它们的笔记本参与检索。升级到 schema 20 不会回填挂载：所有既有笔记本挂载数清零，联邦检索对它们全部停止，直到用户自己显式挂载一个参考库。非内置用户可在头像菜单自助修改密码，走 `PATCH /api/me/password`（body `{"old_password", "new_password"}`；当前密码错误或新密码空白返回 400）：成功后当前会话保持登录，该用户其他浏览器会话全部吊销。管理员可在用户使用总览通过 `POST /api/admin/users/{user_id}/reset-password`（body `{"new_password"}`）重置某用户密码；目标用户的浏览器会话全部吊销、须用新密码重新登录，且请求必须来自真实登录的管理员会话——`auth_optional` 的匿名回退被拒绝（403）。Agent 长期凭据不在两种吊销范围内。内置 `admin` 账号在两条路径都被拒绝（409）：它的密码每次启动都会按 `SILICON_NOTEBOOK_ADMIN_PASSWORD` 重新写入，改密请修改环境变量后重启；界面对它隐藏「修改密码」入口，用户总览该行密码列显示「受保护」。本地/测试场景可设置 `SILICON_NOTEBOOK_AUTH_OPTIONAL=true` 跳过登录。前端在首次加载时显示登录/注册界面，顶栏展示已登录用户名和退出按钮。
- **`last_active` 的来源归因**：可见来源上传按实际提交用户归因，包括上传到其他所有者的共享笔记本；深拷贝不是上传，不会刷新原上传者或接收者。升级前已经存在的可见来源因旧 schema 没有保存实际上传者，只能尽力回填为当时的笔记本所有者。
...
- 知识图谱以全屏浮层打开：object 级 KG 节点（Concept / Claim / Formula / Procedure），类型形状，边关系标签，多选类型过滤，按类型分组侧栏（选中节点聚焦画布）。从问答知识对象引用打开时会精确定位对应节点：目标不在有界高连接度核心图时，前端按引用真实来源 notebook（包括挂载 base）叠加其有界一跳邻域，纯 graph-BFS anchor 也保留所属库 id。浏览器仍以当前 active notebook 过权限，后端只在其有效 participant 集内校验或解析对象并内部代理 base 的邻域/详情/context 读取；挂载公共 base 不会授予该 base 的直接成员权限。引用携带的原始 Concept id 由邻域接口通过单 id 聚类查询解析成 canonical `focus_id`，同时保留 raw object id 供 context 读取，不能用合成图节点 id 查询 `knowledge_objects`。大库没有折叠图产物时，接口会显式返回暂不可定位而不进入全量 cluster-map fallback，前端也不会留下无法消费的 pending focus。这不再是「正在构建、稍等就好」的短暂窗口：服务进程**根本不再构建**大库的折叠图产物——请求线程上不建，后台 daemon 也不建，因为两者都要在 API 进程里把全部对象、全部关系和完整 cluster_map 物化成一份图。产物只由 scale 索引构建发布：手动构建，或合并重建入队的那次离峰构建——后者只在该库仍满足自动建索引条件时才发生（功能开关开启、没有构建正在跑或已排队、库大到不可拷贝、且索引缺失/仅被建议/已陈旧），合并重建会重新打开这次评估，因此同一服务进程里的第二次重建不会被静默跳过。在这样一次构建发布之前，一次合并重建只会让上一份折叠图以陈旧状态继续供图，而陈旧图由热缓存供出，不再每次请求都重新从磁盘读一遍。因此 `GET .../unified-kg` 对「大库且无产物」如实回 `viz_building: false` 加 `viz_unavailable: true`（两者由后端造成互斥，前端不需要排优先级），图谱画布随之多出第四态——「库规模较大，图谱预览将在下一次索引构建后可用」——取代会误导的「没有匹配的节点」。引用定位的提示同样指向那次索引构建，不再承诺「完成后请重试」。侧栏的「出处」以结构化证据卡片展示，长标题、位置、公式与中英混排正文都限制在面板内；来源元素类型为 `formula` 的摘录会走共用块级 KaTeX 渲染，不再直接显示命令文本。
...
- 「分析」菜单本身只包含晋升队列（admin）、发布/撤回公共知识库（admin）与边审查队列。看板、全屏知识图谱是其他顶栏动作；「图谱 Schema」已从顶栏移入知识图谱视图头部，不再是独立顶栏动作。管理员维护全局对象类型基线；笔记本 owner 可查看当前生效基线、copy-on-write 改写继承定义、仅在本库停用该类型，或新建本库专属类型；删除覆盖会恢复全局定义。只读成员可查看同一份生效定义但没有写控件。管理员还可切换到全局基线视图，其改动只影响尚未覆盖同名类型的笔记本。面板按三段编排，把「现在有哪些类型」和「改哪一个」分开：顶部一行**作用范围**（当前笔记本／全局基线，仅管理员可切换）决定看到的是哪一份注册表、以及写入落到哪里；左栏是**类型清单**，按待批准的候选／生效中／已停用分三组，每行给出类型标识、显示名、字段数与来源徽章（继承／覆盖／自建／内置），「新增类型」与「从当前笔记本归纳候选类型」收在清单栏底部；候选在批准前不遮蔽同名的继承类型，因此两者在清单里各占一行、可分别选中；右栏是**选中类型的定义**，默认只读，按「编辑」才换成表单，候选类型在这里给出归纳理由与批准／拒绝。编辑草稿按类型分格留存，切到别的类型再切回来不丢输入，清单行上以圆点标记未保存。写动作以回执为准——只有后端写成功且清单已重新拉回，面板才退回只读态或清空新增表单；拿不到回执时保留输入并在按钮紧邻处说明原因。当前不再暴露已退役的内容生成或派生规则动作。现有 notebook 分析视图提供独立的 Memory 和 Knowhow 内容资产卡片：Memory 指标严格限定为当前登录用户和当前 notebook（admin 也不跨用户汇总），Knowhow 指标遵循 notebook 的既有读取权限。卡片只展示计数、健康度/最近活动摘要和跳转入口；浏览与编辑仍复用现有的 Memory、Knowhow 页面和编辑器。
...
浏览器的 `kgAvailable` 判据同样按「这次勾了哪些库」计算，而不是「挂了哪些库」。`NotebookSummary` 并列下发 `base_kg_notebook_ids`——它是 `base_kg_available` 的**分解**，列出挂载的参考库里哪几个已建知识图谱。零新增查询（`mounted_bases_row` 的每一行本来就带 `has_kg` 列，聚合布尔只是把它 `any(...)` 掉了），回填路径与 `base_kg_available` **逐字相同**，两者必须自洽（非空 ⟺ 为真）——它们是同一次读取的两种投影。前端把它算成「本库有图 **或** 本次勾选的参考库里有带图的」，「将借用参考库「…」推理」也只点名**既被勾选、又已建图**的那几个库：读聚合布尔会在「本库无图 + 唯一带图的参考库被取消勾选」时点名一个本轮不参与的库。字段缺席（版本 skew）时退回「聚合布尔 ∩ 勾选集」，只会比原判据更保守，不会更宽。

**`kgAvailable` 不再决定内置逐步推理能不能提交。** 无图笔记本上逐步推理现在同样能拿到与 chunk 通用问答同预算的原文证据（见[集合枚举工具 → 尚未构建知识图谱的笔记本](#集合枚举工具)），后端注册表把内置 `reasoning` 的 `requires_kg` 改成 `false`，前端 `ask-modes.ts` 内置表同步；`requiresKg(mode) && !kgAvailable` 这道提交闸对内置 `reasoning` 因此恒不触发。`kgAvailable` 现在只决定问答区提示文案与「将借用参考库」措辞——它继续拦截声明了 `requires_kg=true` 的**扩展引擎**（插件模式），那是插件自己声明的硬前提，不受这次改动影响。提示文案仍按**成因**分成两支、出路不同：挂了已整理图谱的参考库、只是这次没勾 → 「已整理知识图谱的参考库这次都没勾选，深入分析将只用原文检索；在来源面板重新勾选可增强」，且刻意**不给**「整理知识图谱」按钮（出路是勾回来，不是花钱整理）；一个带图谱的参考库都没挂 → 「该笔记本尚无知识图谱，深入分析将只用原文检索；整理知识图谱可增强问答效果」，按钮保留且可点击。合成一支会在用户只需点回一个复选框时，误导他去跑一次整库图谱整理（真金白银的模型调用）。

顶层复选框范围是当前 notebook 的硬上限，也是检索范围的**唯一**来源。模型既不提议也不收窄或扩大它：语料盲意图规划器不输出任何来源身份，run 内也没有任何动作能改变哪些来源在范围内。全选 include 快照保留正常图通道，私下快照当时已有的隐藏 Memory/Knowhow 参与者，并冻结可分区候选和结果校验。验证后新增或删除可见来源/隐藏参与者时，在 I/O 前跳过高风险图通道；可分区检索仍按冻结上限继续——这份上限对每一个冻结范围都下推，漂移只改变这些 producer 走哪条 lane，从不改变上限是否约束它们。挂载参考库仍是独立参与者。带索引的 chunk/元素检索会在 scale 工件中持久化逐行来源代码；HNSW 在进入 Top-K 前应用允许来源谓词，hydrate 后还会在评分/合成前复核。旧发布索引缺少这个可选 sidecar 时仍可加载，但会先使用有界来源内 FTS，直到重建或 delta fold 写入映射。KG、PPR 与精确查找等确定性种子结束后，若证据仍完全为空，逐步推理会在让反思模型判断充分性之前确定性补一次有界原文元素检索。无法在遍历前安全应用当前来源谓词的持久化通道（当前 notebook 的全图/PPR/关系扩展、精确章节查找与报告整库画像）会在收窄或参与者全集漂移时被跳过；事后过滤不能作为授权，因为未选候选仍可能占满 Top-K 或提供不可见的图前提。按来源限定的 chunk、元素和 KG 直接检索仍正常执行，base 库 KG 种子也可直接映射回 base 原文，不经过组合全图遍历。
...
所选来源激活契约由 Ask 与深度报告共用，但它始终是内部检索实现细节。对真正收窄的本地来源范围，Ask 的 `chunk`、`reasoning` 与深度报告都会在历史 baseline 冻结后调用同一个所选来源激活服务。省略 scope 或全选 snapshot（包括单来源 notebook 中选中唯一来源）不进入该服务，保持历史响应形状。`SELECTED_SOURCE_GRAPH_ROLLOUT_MODE=shadow` 为默认值：它构建并测量 `G` 但仍返回 `B`；`allowlist`、稳定 hash `rollout` 与 `on` 必须同时通过受信、无正文 attestation 及精确 corpus/model pin。active 输出严格为 `B` 后追加 `G`；`G` 的默认独立预算为 4,000 tokens，不能驱逐、重排或重打分 `B`。snapshot/partition 失败、scope 漂移、任何终态越界候选或非零 baseline eviction 都 fail closed 回 `B`。mounted base 证据继续走独立历史 lane，绝不参加当前 notebook 的所选来源图遍历。rollout 状态不得进入公开 Ask/报告字段、推理轨迹、进度 stream 或 UI；只有无正文内部 `selected_source_graph` 事件携带它，浏览器还会过滤历史持久化的 `source_subgraph` 步骤。

无正文 `selected_source_graph` 事件仅属于运维内部 telemetry，所有用户可读 debug 日志的列表、统计、搜索、翻页和详情响应都会过滤它。

### 界面模式（自动/高级）

每个用户持久化一份 `user_profiles.ui_mode` 偏好（`"auto"` 默认 / `"advanced"`），经 `GET /me` 的 `UserProfile.ui_mode` 下发，并可通过自助端点 `PATCH /me/ui-mode`（body `{"ui_mode": "auto" | "advanced"}`；非法取值返回 422）修改。早于该字段的旧后端不会返回它，前端把缺失值按 `auto` 处理。高级模式就是本文档通篇描述的完整界面，与既有行为逐位一致：问答仍按前端选中的分组、引擎与检索档位执行。自动模式是从头像菜单开关的刻意精简界面：问答区不渲染「通用问答」「深入分析」或任何引擎/检索档位选择，只保留问答框；浏览器提交请求级 `mode="auto"`，后端在创建持久会话/任务之前复用 corpus-blind 问题理解模型分析当前问题（可参考同一会话最近五条用户问题，绝不读取语料或助手回答），再把请求收敛到 `chunk` 通用问答或 `reasoning` 深入分析。需要完整枚举、多个必答方向，或比较、诊断、设计、审阅的明确结构化意图走 `reasoning`；普通单方向问题走 `chunk`；问题仍有关键歧义、模型未配置或理解失败时保守回退 `chunk`。自动路由若选择 `reasoning`，同一份无歧义问题理解合同直接作为自动确认的检索合同，不再重复调用理解模型，检索档位固定「标准」。深度报告研究深度控件仍隐藏并固定「标准」（`depth=2`），上文的来源/参考库范围复选框也隐藏——自动模式下被隐藏的控件永远在请求侧发送未收窄的默认值（当前 notebook 全部可见来源、全部已挂载参考库），绝不沿用高级模式会话残留的收窄勾选。自动模式创建的深度报告固定携带 `auto_generate=true`（参见上文「深度报告」条目及其可能触发的 `report_intent_auto_confirm_skipped` 事件）。问答输入框仅在该笔记本**尚无可用证据**且存在解析未到终态的来源时锁定——已有证据的笔记本上传新文档不会锁死输入框；能可靠统计处理中来源数（来源搜索框为空且当前已加载全部可见来源）时显示「N 篇文档处理中，完成后即可提问」，否则显示不带数字的「文档处理中，完成后即可提问」。切换界面模式即时生效；切回高级模式后，前端保留并继续尊重用户此前的具名 Ask 模式选择。

来源列表搜索通过「搜索」按钮或回车提交，并去除首尾空白。提交空字符串或纯空白时，清除搜索条件与输入框，重新加载未筛选的第一页并更新可见来源总数；检索范围的复选框保持原有勾选。加载期间搜索按钮禁用并显示忙碌状态，成功或失败后都会释放，空白提交不会让控件一直卡在加载中。
...
作废旧链接）或撤销。复制结果只显示在刚按下的按钮上（「复制」→「已复制」或「复制失败」）并
自动还原，不再同时生成页面级提示横幅。链接里的 bearer token 会跨登录/注册门保留；认证完成后浏览器先从 URL
...
深拷贝——用户给自己拷一份，以及把分享出来的小笔记本投递进接收者账号的那一份——
**携带源库的挂载边**，而不是空着参与集到达；每一条携带来的边都按**新笔记本**用
检索路径同一条实时谓词重判。`tier='base'` 的公共知识库与带 `everyone` 授权的库随
每一次拷贝走；私有库只在「副本的 owner 仍是该库的 owner」时随走（自己拷自己的库）；
借入边只在接收者**自己**对该库有读权时随走——所以拷贝一本共享库拿到的可达范围，
与共享它本身给出的完全一致，不多一分。判不下来的边被**丢弃**，而不是像源库那样保留
置灰：源库上的灰边是 owner 自己做过、还可能恢复的配置，副本上的同一条边则是接收者
从未做过、通常也永远恢复不了的配置——接收者重新挂载他真正够得着的库即可。这一条对
拷贝那一刻只是**暂时**失效的边同样成立（公共库被短暂降级、权限被短暂收回）：源库会
在条件恢复后自动重新生效那条边，副本却是**永久**丢弃，接收者等它重新可达时自己挂
回去。

携带来的边登记为接收者自己的配置（`created_by` 与 `created_at` 改写成这次拷贝），
源库的边一条不动；被丢弃的条数落**日志**而不进返回值：拷贝端点返回的是一个普通
`NotebookSummary`，不为一个诊断计数新开字段。挂载边同时也是拷贝表里**唯一**允许
源侧与副本侧行数不等的一张，因此刻意不参与深拷贝的行数一致性自检。
...
|---|---|---|
| 尚无 KG（新库） | **不**自动抽 | 按需构建：`POST /api/notebooks/{id}/kg/build`（界面：notebook 的**「构建知识图谱」**动作；在无 KG 的库上选「深入分析」组——即 `strict` 的 `reasoning`——时也会提示构建） |
...
构建总数的语义是**起跑即准，不是受理即准**。受理一次构建不再在请求路径上枚举整库，
...
「已完成 0/0 项内容」，改动前这里是 0/N。
总数在跑动中仍可能再抬一次：补漏轮会把计数之后才落齐的来源并进来，规则同样是单调
不减（分母不会倒退）。
...
`KG_LLM_MAX_RETRIES` 次（默认 `2`，允许 `0..3`）。KG 请求走流式传输，该超时约束的是
连接阶段与相邻两个流式分块之间的等待，而不是整条回复的总墙钟：持续产出 token 的长抽取
不会在 60 秒被截断，只有上游沉默才会。若服务持续不可达，或认证失败/
请求被永久拒绝，本次任务共享的中断控制会阻止继续发起请求，取消尚未开始的
source/window 工作，在首个窗口确认熔断时、窗口级与来源级 drain 开始前就持久化
`stopping`，再等待已经开始的调用安全退出。探测与抽取调用会把该控制器的中断信号
交给共享的流式 JSON 传输；因此超时约束的是首个响应及相邻 chunk 之间的无数据时间，
不是持续有输出的长回复总墙钟时长。若上游网关缓冲流或另设请求总时长硬上限，则仍不在
这个保证内。传输会请求 OpenAI-compatible 的可选 usage trailer，并从最终的空 choices
chunk 中采集精确的 prompt、completion 与 total token，写入既有按用户 LLM 日志。
provider 明确拒绝该参数时回退普通流式传输，物理 client 会记住这一能力结论，后续调用
不再重复失败探测；provider 接受或忽略参数但仍不返回 usage 时，日志保持 token 不可用，
不会在本地估算。中断范围只限当前 notebook 的本次 KG
任务，不影响其他 notebook 或之后重新发起的任务。可用性探测会显式绕过 LLM 响应
缓存且不回写缓存，旧的成功探测不能在当前模型已经不可用时放行破坏性重建。
探测复用已配置的短输出预算，不再另设更小的探测专用上限，因为推理型模型可能在输出
可见 JSON 前就耗完过小的预算。HTTP 成功但正文为空、被截断或 JSON 无效时，任务记录
`model_response_invalid`；界面明确显示模型响应不可用而不是误称人工中断，并保留继续分析入口。
...
`POST /ask` 按 `mode` 分派——注册表 `backend/app/services/ask_modes.py` 是唯一真源（默认 `chunk`）。联合范围按路径区分：`chunk` 基线 active-only；可选 KG overlay / PPR 可加入 federated KG 与 base-backed chunk；`reasoning` 走 federated KG。`federated_retrieve()` 的知识对象命中不改 score，只在完全平局时以 `base` 为第二排序键；`federated_retrieve_relations()` 的关系命中仍只按 score 排序。这些排序信号不进入接地阈值。
...
| **`reasoning`** | strict | 否（有图时增强） | agentic 迭代 plan → retrieve → reflect → answer；原文段落检索与集合清单不需要图，图动作只在范围内有图时提供（流式输出实时轨迹）。 |

`reasoning` 首轮检索之后即由模型判定是否可以直接作答：首次 `reflect()` 一旦返回 `answer`/`sufficient`，反思循环就不再进入下一轮，也不会再付任何额外检索。无图 run 的首轮原文证据来自两条臂：向量臂（对每条子查询各跑一次 `search_chunks`）与关键词词法臂（`plan()` 里 `expand_query` 产出的高/低层关键词拼成一个空格分隔串，只跑一次词法检索，并入前先按 relevance 收到与向量臂每条子查询同一个宽度），两者与 chunk 通用问答同法合并去重后进入证据池；这里的关键词是 prompt 的 zh/en 默认双语，不是 chunk 通用问答那样按语料语言双语——对齐需要给 `plan()` 传 `corpus_langs`，那会一并改到有图 run 的规划 prompt，因此登记为后续（见 `fangan_todo.md` 的「检索」一节）；有图 run 不加词法臂，已确认意图路径（跳过 `plan()`）没有关键词，同样不跑词法臂。成本上，直接作答时 reasoning 比 chunk 多付一次 `reflect` 调用（高级界面另有一次可编辑合同预览；自动模式下两者都要先付一次路由理解）。

**通用问答的文档介绍。** 明确的库内文档介绍问题（例如“这个库中的文档分别介绍了什么内容”）
在相关性检索前复用授权范围内的来源目录，不要求知识图谱或向量索引。已有类型化来源结果卡
返回每篇文档的标题、类型与已存摘要摘录；摘要缺失时明确说明，不能仅凭标题推断正文。
目录枚举覆盖与实际进入合成的预览覆盖分别记录。该路径复用请求的枚举预算和
`CHUNK_ANSWER_BUDGET_CHARS`，不声称已阅读全文。

支持“介绍一下这个notebook中的文章”等中英混合范围名称。目录介绍默认只包含当前笔记本
可见且已选中的文档；问题明确要求包括参考库时才纳入挂载库，并始终与请求授权的参考库
选择取交集。按标题点名的单篇查找仍可定位获准访问的挂载文档。目录遍历和结束时的一致性
检查使用相同的有效范围，计数不会包含被排除的参考库。

导读按目录顺序为每篇已返回文档保留一项，使用真实显示标题；研究问题、核心方法和主要
贡献分别依据证据说明。模型漏项时明确回退到已存摘要摘录；缺少证据或未进入合成预览的
文档明确标注，不静默漏篇。文章关系和阅读顺序只使用已知文档引用。摘要为空时，可在剩余
共享字符预算内读取有界原文；这些文档共同分配 `DOCUMENT_OVERVIEW_MAX_ELEMENTS` 元素
额度，来源卡仍展示已存摘要。回答另行披露模型介绍与回退数量；预览覆盖只代表模型输入，
不证明语义上完整理解了每篇文章。
目录合成沿用共享语言规则、已存风格偏好及经作用域处理的近期用户问题，以保留会话中的
表达要求；不传入既往助手回答，避免挂载库证据经会话历史重新进入仅本库的介绍。

明确的单篇介绍按唯一标题（建议用《完整标题》）或有效范围内唯一文档定位。目录尚不完整、
标题不存在或目标有歧义时要求收窄来源选择，不猜测。正文按来源详情的稳定顺序有界分布读取，
包含最后一个位置；默认最多 64 个原文元素（`DOCUMENT_OVERVIEW_MAX_ELEMENTS`，最小 2），
共享原文字符预算在样本间分配。短文只有全部解析正文均进入预算且解析版本前后不变时，
才报告完整的解析元素覆盖。长文或截取正文明确披露取样，不能据此声称覆盖所有章节。
来源详情沿用 `(created_at, id)` 稳定排序，不是新建的语义章节顺序，最后一个取样位置也不保证
就是文档的逻辑结论。解析版本变化时丢弃混合证据。回答保留原始元素引用，按 `mode="chunk"` 持久化，证据级别
保守标为 `overview`。该路径识别明确的中英文介绍表达；专题、比较和未识别问题保留现有
检索路径。自动界面的引擎选择规则不变。

chunk 检索、chunk×graph mix 与 `reasoning`（含分节合成）三条答案合成路径共用同一份合成规则
（`answer_prompt` 规则 12）：回答必须原样保留问题里的限定词——范围、工况、方向（如 TX/RX）、
周期性、「只/仅/除了」、点名的子部件——按限定后的问题作答，不得悄悄换成更宽的问题或邻近对象。
证据只覆盖未限定情形或邻近对象时，回答须用一句话明说，并把对该情形的外推标为「（推断）」。这条
规则不新增任何 `[k]` 引用绑定授权，`grounded` 的判定不变。

同一份规则集里的规则 13 要求推断状态传递：结论、总结、「因此/所以」句或最终建议只要依赖任何一句
已标「（推断）」（英文回答为「Likely,」）的前提，自身也须标同一语言的推断前缀且不挂 `[k]`；只有每条前提都已 `[k]` 引证的结论才可
不标。这条规则同样不新增任何 `[k]` 引用绑定授权，`grounded` 的判定不变。答案与报告正文里句首/
段首出现的「（推断）」/「(推断)」/「Likely,」与「【通识】」标记，在界面上渲染为不可点的行内标签，
与引用徽章同属一个视觉家族；渲染只加样式，不改变文本内容与复制结果。标记开句但要排在列表序号、项目符号或标题语法之后（写 `1. （推断）…`，不写 `（推断）1. …`），否则 Markdown 不再把该行当列表项；渲染前对四个模型正文渲染面做一次确定性归一，把写反的「标记 + 列表/标题语法」对调为「列表/标题语法 + 标记」，只交换两个 token 的位置、不改一个字（原有空白原样保留，围栏代码块内不处理），历史回答同样受益；复制出去的文本仍是模型原文。
...
上表的内建 id（`chunk` / `reasoning`，以及分组 id `general` / `strict`）是**协议**：`POST /ask` 收的是它，历史会话与书签存的是它，后端注册表 `backend/app/services/ask_modes.py` 声明的也是它。它们是稳定量，不因为「名字不好听」而改。部署方的 `ask.engine` 以「本插件 id + `.`」为前缀增加命名空间化 mode id；该 id 在引擎存续期间同样进入持久会话，但只能运行时发现，绝不编进公网前端。

界面上**显示**的名字是另一层，纯 UI。`frontend/app/ask-modes.ts` 仍是两个内建引擎的静态真源，并在运行时合并 `GET /api/ask-modes` 返回的脱敏部署投影：
...
该注册表的 `groupLabel()` / `modeLabel()` 是内建文案的唯一读取口：前端任何其它文件都不得硬编码内建显示名，散文里提到就用模板插值；插件 label/description 只能来自实时脱敏投影。两边由 `ask-modes.test.mjs` 强制——它递归扫描 `frontend/app`，当前内建显示名出现在注册表之外即失败，退休名（严格推理 / 深挖推理 / 图谱多跳 / 关联追溯）复活也失败。因此改内建显示名只是改注册表一行，不动任何 id、请求/响应载荷或已存会话；`scripts/check_ask_modes_contract.py` 继续跨栈锁住内建 id 集合，并拒绝把插件 mode 字面量编进前端。
...
| 工作形态 | 当前网页端入口 | 交付与断连合同 |
|---|---|---|
| 请求内模型任务 | Ask 意图理解、回答转 Memory 预览、Knowhow 表达优化/单行补全/格式化 | 共享 NDJSON 请求流。结果只属于眼前这次交互；断连设置检索/模型 I/O 共用的协作式取消事件。 |
| 持久长任务 | Ask 回答、深度报告、来源解析、索引/KG 维护、命令目录、笔记本理解 | 启动并 detached，任务状态持久化；通过重连或轮询恢复。导航/transport 丢失不取消，只有各自显式取消动作可以取消。 |
| 有界同步任务 | CRUD、分页读取、状态/配置、确定性校验与准入 | 保持普通 JSON/HTTP，不伪造进度事件。 |
| 请求体或二进制传输 | 上传正文、`reports.zip` 等二进制下载 | 保持普通 HTTP 并显示 busy 状态；上传准入后，来源解析转后台执行。不得向文件 payload 混入 NDJSON 字节。 |

共享请求内协议位于 `POST /notebooks/{id}/ask/intent`、`POST /answers/{answer_id}/memory-preview` 以及 Knowhow `optimize`、`complete`、`reformat` 动作各自的 `.../stream` 变体。它立即发送 `started`，此后每 **5 秒**发送一次只含固定 `stage` 与单调递增 `elapsed_ms` 的无正文心跳，最后恰好发送一个 `final`、稳定错误码 `error` 或 `cancelled`。响应类型是 `application/x-ndjson`，并带 `Cache-Control: no-cache, no-transform` 与 `X-Accel-Buffering: no`。异常原文、prompt、notebook/source 身份、证据与模型正文绝不进入心跳/错误帧。权限与廉价、确定性的资源/输入/模型可用性校验先于第一帧完成，以保留真实 HTTP 状态；开始流式之后的失败只发送稳定错误码，由前端换成已审阅兜底文案。浏览器断连、ASGI 帧发送失败、用户显式中止/关闭或重试换代都会协作取消 worker；后台标签页暂停 `requestAnimationFrame` 时，读取器用有界定时回退继续消费终态。Ask 意图与 Memory 预览会显示已等待时间；Knowhow 继续使用既有 loading/队列状态，心跳负责让 transport 保活。原 JSON 端点继续供兼容客户端使用，官方网页端改走流式变体。

这条请求内协议与持久 Ask 的断连语义刻意不同：浏览器离开后，服务端会发出取消信号并观察 worker 收尾，provider 不得把取消吞成正常 fallback。未来若某项动作必须跨导航或进程重启继续，它就应进入上表的持久任务，而不是继续扩大这条请求流。
...
`search_chunks`（原文段落检索）每 run 的调用次数上限由部署参数 `REASONING_MAX_CHUNK_SEARCHES` 控制（默认 3，与 PPR、精确查找一致），不是档位字段；无图 run 首轮的确定性播种不计入这个上限，每子查询取的候选条数是该档位的「每查询相关性结果」（`ranked_per_query_take`）而不是 `search_chunks` 动作自身的 MMR k。
...
退役 id `fast`、`global`、`graph` 透明映射到 `chunk`（旧会话/书签不会 422）；其余未知 mode 返回 HTTP 422。
...
**尚未构建知识图谱的笔记本。** 这些工具不需要图谱，逐步推理还额外提供一个一等的原文段落检索动作 `search_chunks`：按语义 + 关键词检索来源原文段落，命中并入与 chunk 通用问答同一个大预算分区（`chunk_context_chars`），同引用绑定；有图笔记本上同样提供这个动作。无图 run 首轮会确定性地对每条子查询播种一批原文段落（不赌模型主动检索），并额外并入一次关键词词法臂（`plan()` 里 `expand_query` 产出的高/低层关键词拼成一个空格分隔串，与 chunk 通用问答同法检索、同法合并）；词法通道交回的是召回窗而不是选择结果，所以这条臂在并入前先按 relevance 收到档位的每子查询纳入数——与一条向量子查询同宽，否则整个召回窗会在合成侧把向量臂挤出预算。这里的关键词是 prompt 的 zh/en 默认双语，不是按语料语言双语（对齐一事登记在「检索」一节）。有图 run 与已确认意图路径不跑这条词法臂。每 run 的检索次数上限由 `REASONING_MAX_CHUNK_SEARCHES` 控制（首轮播种不计入这个上限）。与此同时，`expand_graph`/`ppr_retrieve`/`expand_community`/`follow_chain`/`enumerate_kg_objects` 五个只对图有意义的动作会从动作枚举、规划/反思提示与白名单三处一并收缩——模型看不到、也选不到必然空手的动作；首轮会记一条披露步「本笔记本尚未构建知识图谱,本轮只用原文检索与集合清单」（这句是逐字文案，与代码里的 `summary` 一致，含其中的半角逗号）。`enumerate_elements` 与来源清单（`enumerate.collection="sources"`）不受影响，一个只解析了来源、还没做知识图谱分析的笔记本——自动抽取默认关闭，这本就是常态——照样可以被问「有哪些公式/表格/图片/代码块」「库里有哪几篇」。响应里仍会如实报告该笔记本没有知识图谱（`kg_required=True`，「整理知识图谱」的提示继续显示，但这是纯披露，不再拒答）。原文段落检索与 chunk 模式同为**当前笔记本**范围：`search_chunks` 复用的就是 chunk 模式的检索原语，索引与库读都是 notebook-local，所以挂载的参考库只经知识图谱与元素/知识对象/来源清单参与，它的原文段落不在这条通道里（联邦化登记为独立待办，见 `fangan_todo.md` 的检索一节）。早退条件因此收窄为**两条通道都无从下手**：枚举那条按参与集（当前 notebook + 勾选的参考库）的元素/知识对象/来源计数判，原文那条只按**当前笔记本**的用户可见来源数判——用参与集的来源数给一条够不着参考库的通道放行，换来的是一轮播种与动作都空手的空转。两条都不成立、或集合地图取不到（fail 到早退方向，与现状同）时才会拒答，文案是「当前笔记本没有可检索的来源；请先添加来源，或挂载/整理一个已建知识图谱的参考库。」（提到图谱是参考库那条出路的真实条件——参与集里有图时根本走不到这条早退；图仍然不是当前笔记本作答的前提）。来源数**如今确实计入**这道闸——「来源数刻意不算」是过时表述：代码自 codex R5 P1 起已把 `collection_map.sources > 0` 计入放行，因为被挡住的恰是「库里有哪几篇 / 逐篇分析当前 notebook」这类只需要来源清单、不需要元素或知识对象的场景。放行的两条接线判据分别对应集合枚举与原文段落检索各自的部署开关（`REASONING_ENUM_TOOLS_ENABLED`、`REASONING_CHUNK_SEARCH_ENABLED`），任一开着且对应集合（枚举侧按参与集、原文侧按当前笔记本）非空即放行；两把开关都关闭时逐字回到接入前的行为（早退恒定触发，无图库不再有任何检索通道）。放行之后，来源清单与另外两个集合一样可用。
...
**同一小节还显示 Agent 到底拿这个库做了什么（调用记录）。** 每一次通过鉴权、落到这个
笔记本上的 MCP 工具调用都记一行——哪个 Agent、什么时候、按哪一档能力——存在**同一张**
`agent_observations` 表里，用 `kind='call'` 区分（`'note'` 是 Agent 经 `add_observation`
自己写下的短句，也是本列出现之前每一行的取值）。用列而不是新建一张表：两种行的归属键
逐字相同 `(notebook_id, owner_id, agent_profile_id)`，生命周期也相同——成员被移出时一起
清、笔记本删除时一起级联、深拷贝一起不带。
...
再补记，而那正是单一收口要消灭的「新加的工具静默不记」——上面那三道鉴权则是一个封闭、可
枚举的集合，要么住在共享 helper 里，要么在这里被点名。记账写失败只吞掉并记一条**不带任何内容**的日志（不带异常文本——关于一次调用的账不是
这次调用的一部分，而一个数据库异常会捎上路径与 SQL 片段，那是仓库日志规则明令挡在外面的）。

写侧**叠在 `AGENT_PROFILE_ENABLED` 之上**而不是与它并列：这份记账唯一的读处就是那个面板，
而总闸关掉时它的入口按钮一个节点都不渲染，此时还记就是在攒没人打开得了的行。读与清空则
**两把闸都不跟随**——今天翻一个开关，不该让一份用户有权删除的数据变成他既看不到也删不掉的
东西，所以 `GET` 永远回读这份记账，`DELETE ...?kind=call` 在总闸关闭时同样放行（其它形态的
`DELETE` 在那里照旧 409，不变）。面板与之一致：总闸关闭时不再整页早返回，而是在「这项功能
当前未开启。」旁边照常挂上「Agent 记录」那一节。那种状态下这一节的全清按钮会把自己收窄成 `kind=call` 并改名——在那里
发一次不收窄的 `DELETE` 是一颗服务端必然拒绝的按钮。已登记的边界：这个面板的**入口按钮**
同样跟随 `AGENT_PROFILE_ENABLED`，所以在总闸关闭的部署里，这一节只在已经打开的面板窗口内
可达；要在那种部署下清空，走端点本身（它不跟随任何一把开关）。

记账行同样带着 `add_observation` 那套成员移除补偿：鉴权与写入是两步，中间被移出的成员可能
留下一条重新加入即复活的孤儿行。写完之后复核**笔记本读权**（不是整个令牌校验——令牌被吊销
不代表这一行不该存在），确认访问确实没了就清掉该成员在这个库里的记录，复核本身出错则保留。
与 `add_observation` 不同的是它**从不报错**：这次工具调用本身是合法的。这也是记账在「一次
写」之外还要付一次有界读的唯一地方。`GET .../agent-observations` 把它作为**独立的** `calls` 清单返回，带自己的
`call_limit`（默认 20，上限 200），不与 `items` 混在一起：共用一个上限时，一个繁忙 Agent
的调用记账能把成员手写的短句整个挤出这次响应。`calls_enabled` 单独报 `AGENT_CALL_LOG_ENABLED`
那把开关，面板据此说「这个部署没有开启调用记录」，而不是把空清单渲染成「从来没有 Agent
调用过这个库」。`DELETE .../agent-observations` 接受可选的 `kind`（`note`/`call`，按封闭集合
校验——认不出的值一律 400，绝不静默删零行，那种静默与「本来就没有」长得一模一样）；省略时
两种都清，成员移除路径要的正是这个语义。
...
铃铛（`GET /api/me/pending-actions` 及其推送流）在「深度报告待确认 / 治理队列 / 索引与论文
元数据状态」之外多出第四个只读分组：当前用户**仍在跑的提问**。它服务的是「我离开页面（或刷新）
之后，想找回刚才那个问题」这件事：只做投影与导航，不新增端点、不新增写路径，也不提供取消入口
（停止提问仍是会话内的「停止」按钮）。

- **在途集合是精确的。** 条目只在 `ask_jobs.status = 'running'` 时存在——这是该表唯一的在途
  状态（没有 `queued`：job 行插入时就是 running）。终态 `done` / `failed` / `cancelled`，以及
  重启兜底把残留 `running` 改写成的 `interrupted`，一律不出现。谓词是正向精确匹配而不是
  `NOT IN (<终态…>)`：否定式会让将来新增的中间状态悄悄拿到一个可点击的深链。
- **属主隔离。** `ask_jobs.created_by` 必须是发起请求的人：别人在我库里跑的提问是他的动作、
  不是我的，我在别人库里跑的提问是我的。可见性叠加投影其余部分共用的那条规范读谓词与生命周期
  过滤——对那本库失权即出铃铛，恢复读权自动回来。
- **字段**——`type: "ask"`、`state: "running"`、`job_id`、`notebook_id`、`notebook_name`、
  `conversation_id`、`title`（截断后的问题摘要，提示词全文绝不进铃铛）、`asked_at`（浏览器记录的
  提交时刻；该行没有时回落到服务端写入时刻）。点击打开对应笔记本与会话，接回由既有的会话详情
  路径承担。
- **它不响铃。** 进行中的提问不计入响应的 `count`，客户端也把它排除在未读徽标之外——这是与
  「索引构建中 / 论文信息补全中」两类条目的**刻意分歧**（那两类仍计未读）。那是用户没发起、
  可能已经忘了的后台活；提问是他几秒前自己发起的，给它记未读会让铃铛每问一个问题就亮一次。
- **推送边界。** 每次提问只重发两次待办快照：起点一次、到达终态一次。轨迹进度**绝不**推送
  （重算快照是一次数据库读取，按步推会变成查询风暴）；终态那次严格排在终态事件入队**之后**，
  因此不可能拖慢答案投递。
- **快照按用户单调。** 每次快照重算在读库**之前**先取一个该用户的单调序号，已连接的流丢弃序号
  不比上一次下发更新的快照（连接的初始快照同样取号）。同一用户两个提问并发结束时，铃铛因此不会
  把已结束的提问一直显示为进行中：后开始的重算一定已看到先前那次终态的提交，先开始、后送达的
  那份才是旧的，直接丢弃。流还会在计算初始快照**之前**先注册连接，这样计算期间落地的终态推送会被
  送达，而不是被「无人订阅」闸门跳过。

| 围栏 | 取值 |
| --- | --- |
| 每帧快照携带的在途提问条数 | 20，取最新（`RUNNING_ASK_ROWS`） |
| 问题摘要长度 | 先把空白归一为单个空格，再截到 60 字（`ASK_QUESTION_PREVIEW_CHARS`） |
| 铃铛里「已进行多久」的刷新间隔 | 30 秒，且只在面板打开且确有在途提问时走表 |
...
章节内的结论或小结只要依赖任何一句已标「（推断）」或「【通识】」的前提，自身也继承「（推断）」（章节合成规则 2）；只有每条前提都已 `[k]` 引证的结论才可不标。终审阶段生成的执行摘要同样保留来源章节发现的「（推断）」/「【通识】」标记：摘要句若是从带标记的章节发现提炼而来，标记留在句首；直接答案本身依赖这类发现时，答案以「（推断）」开头。任何阶段都不会把带标记的推断或通识发现提升为不带标记的既定事实。
...
**公开会话对等。** `PublicImage.reference_keys` 只复用绑定 token 别名图片的既有公开 `k1`/位置编号，不暴露任何内部 id；`PublicReference.is_image_reference` 也只公开内部元素 id 比较后的布尔结果，不公开任一 id。匿名 `/c/{token}` 页据此沿用站内 Ask 的正文块落点、只显示图片、可信度标签、caption 仅作 `alt`、图片元素描述隐藏、同图只在第一次出现以及页内放大规则。缺少 `reference_keys` 的旧载荷无法诚实恢复落点，因此明确降级为未定位的纯图片区，不丢图也不猜绑定；报告公开分享白名单仍不带图。

**是装配增强，不是新的检索通道。** 一次共享的批量主键读取（`evidence_context.py`）拿一批候选 element id，过滤出 `element_type='image'` 且 `metadata.asset_id` 非空的行并返回映射；不产生任何新增模型/embedding 调用。它接在两处已经持有完整证据对象的装配点：`ask_chunk` 的 chunk 引用（按该 chunk 完整的 `element_ids` 反查，而不只是可能为空的 `anchor.element_id`——「一段正文 + 一张配图」恰恰是多元素 chunk 的典型形态，只看 `element_id` 会漏掉图）、以及 `ask_reasoning` 的锚点装配（chunk/element/KG 锚点各自提供自己的候选 id）。天然被排除、无需特判：knowhow 格子投影行 `element_type` 固定为 `knowhow_cell`，永远不命中 `image` 过滤；Memory 派生源虽然会解析出图片元素，但摄取路径刻意不为它接 `persist_image` 闭包，这些行永远没有 `metadata.asset_id`。
...
**Ask 前端的落点与展示。** 图片直接穿插在回答正文里，位于绑定引用所在的最小完整 Markdown 块之后：段落/标题写完再放，列表项和引用块放在各自内部，表格则等整张表结束再放，绝不从句子中间切开。复合引用按正文引用顺序合并成一个图片区；同一资产在后文再次被引用时只在第一次命中处展示。区块只显示鉴权图片与紧凑的「引用 [n] / 模型未直接读取图片」可信度标签：caption/description 继续用于检索与图片无障碍 `alt`，但不会在正文区块或 Ask 引用浮层里重复显示成文字；引用元素本身就是该图片时，其解析器生成的 snippet/quoted span 也隐藏，而文字证据只是附近带图时保留真实摘录。正文图片加载完成后，页面壳的无关状态更新——尤其是在下一轮 Ask 输入框里打字——会保留同一个图片节点与 objectURL，不会退回加载态，也不会重复请求同一资产。点击正文或引用浮层里的图片会打开 root-modal coordinator 管理的页内预览：移除原来的大白色可拖动/缩放卡片，让图片直接位于页面级压暗遮罩上；`react-zoom-pan-pinch` 提供有界的滚轮/按钮/双击缩放、拖动查看与触控手势，轻量悬浮控制条显示当前比例并可复位，同时保留关闭按钮、点遮罩、Escape 和焦点返回。图片打开即**居中**：缩放内容盒以 `!important` 铺满舞台——库把自己那份单类 CSS module（`display:flex` + `width/height: fit-content`）在运行时注入 `<head>` 末尾，同特指度下它会赢，再配上库固定的 `transform-origin: 0% 0%`，图片就会钉死在舞台左上角。铺满舞台同时让平移边界与舞台同尺寸，100% 时钉在居中位置不会漂移。**多图切换。** 打开预览时冻结一份画册：本次渲染**真正落进正文**的那些引用绑定图片，按落位顺序。顺序不做第二次推导——`rehypeCitationImages` 一边插入图片区块一边记账（`CitationImageOrder`），预览在点击那一刻读这本账。按 reference 数组推导会在两者不一致处出错：没有绑定 anchor 时 references 回退成 `citations` 数组顺序，正文先写 `[2]` 后写 `[1]` 时渲染出的是图 2 在前而数组说的相反；从未在正文出现过的引用还会贡献出读者根本看不到的图片。正文图片区块与引用浮层附图共用这本账。因此 `←`/`→` 与两侧切换按钮走遍的是整条回答，而不是点进去的那一组。下标**夹取**而非循环（到头/到尾的按钮禁用并变灰），`n / N` 计数如实显示当前位置，每次切换都在 layout 相位（新图被绘制**之前**）把缩放与位移复位，下一张绝不会哪怕一帧地带着上一张的倍数与偏移出现。切换同时重置鉴权取图状态：新图取回之前显示加载态，而不是上一张那个**已被回收**的 objectURL；某一张取图失败也不会让之后每一张都停在失败文案上。只有一张图时不渲染计数与切换按钮，方向键也不做任何事。方向键挂在 `window` 上——舞台没有焦点陷阱，浏览器可能把焦点留在 `body`；被更高层弹窗盖住（`interactive=false`）时不挂，键落在 input/textarea/select/contenteditable 上时也不抢（那里方向键是移动光标）。问答会话公开分享页复用同一套舞台，画册按轮组织。预览不会跳去来源详情；其持有焦点期间引用浮层保持挂载，确保关闭后仍有真实的焦点返回目标。MinerU 图片只要摄取时已保存字节并给出 `asset_id`，就走完全相同的展示；只有文字描述、没有资产的 image 元素仍无图可渲染。正文区块与详情都复用既有鉴权、按视口懒加载的 `AuthedImage`。

**深度报告正文的落点。** 站内报告正文按与 Ask **完全相同**的契约内联引用图片，走的是同一个 `rehypeCitationImages` 与同一个内联图片组件，而不是报告侧另写一份：落在最小完整 Markdown 块之后，列表项与引用块的槽位留在各自内部，表格整体原子对待，图注只作 `alt`（正文里绝不重复成可见文字），**按资产在整份报告范围内**首次出现去重——不是按节、也不是按引用，所以同一张图被两条引用绑定时也只出现一次——并同样按视口懒加载，区块滚进视野之前不发任何图片请求。key 直接对得上：`report_engine` 在写入 `content_md` 之前已经把每节的标记全局重编号成一个 `k1, k2, …` 命名空间，`references[i].key` 就是那个 `kN`，因此不涉及任何按节偏移的换算。点开图片走的是 Ask 侧**同一格** `answer-image-preview` root modal 席位——协调器已登记的那一格，不是第二个不受管辖的弹窗——画册是整份报告按正文顺序的那一本，读的也是同一本 `CitationImageOrder` 账。既有的引用详情「本段附图」区块保留：它回答的是另一个问题（**这一条**引用挂了哪些图，图注可见），与正文那份去重后的落位不重叠。导出不受影响：`.md` 下载、复制正文与批量导出都逐字消费已存的 `content_md`（核心导出路径甚至会拒收与已存正文不逐字节相同的 Markdown 渲染结果），所以内联落位只发生在渲染期，不给任何导出产物增加内容。公开分享页 `/r/{token}` 刻意除外（见上文「刻意排除的两处」）：它的白名单不投影 `asset_id`/`element_id`，报告也没有任何匿名读资产的端点（对比 `/c/{token}` 那条 token 别名图片路径），要在那里出图得同时改投影并新开一个匿名端点。
...
**引用浮层切换。** 已有引用浮层打开时，单击同一回答里的另一枚引用标签会直接切换到该引用；点浮层外部、外部滚动或按 Escape 仍会关闭浮层。
...
- **问题**：`AskRequest.question` 带 `max_length=ASK_QUESTION_MAX_CHARS`，因此 `POST /notebooks/{id}/ask`、`POST /notebooks/{id}/ask/stream`、`POST /notebooks/{id}/ask/intent` 与 `POST /notebooks/{id}/ask/intent/stream` 超限一律 422，前端提问框同值拦住提交。**MCP 工具 `ask_notebook` 同样执行这条闸**，并给自己的可读文案而不是抛一个裸校验错误：长期 token 的 Agent 客户端此前可提交任意长度的问题，现在会收到 `question too long: … the maximum is 4,000 …`。这是一次**刻意**的行为变化——MCP 客户端与浏览器一样是写入侧，而超过这个长度的材料应当作为来源上传、而不是塞进提问。
...
登录用户可从全局 `/wishes` 进入**许愿墙**，它不隶属于某一本笔记本。所有用户都能发布 `bug`（问题反馈）和 `feature`（功能需求），只有管理员能发布 `plan`（更新计划）。标题会去除首尾空白、不能为空，最多 120 字符；详细说明同样会去除首尾空白、不能为空，最多 4,000 字符。每位用户对每条问题反馈或功能需求最多一票，再按一次即取消；更新计划不参与点赞。默认 `priority` 排序先展示更新计划，再按问题/需求的点赞数降序，最后用发布时间与 id 做稳定的新到旧排序；也可选择 `latest`。列表使用 offset 分页，默认每页 50 条，单页上限为 100。HTTP 端点为 `GET /api/wishes`、`POST /api/wishes` 与 `POST /api/wishes/{wish_id}/vote`。
...
管理员可从全局 `/admin/questions` 进入**提问分析**，把普通问答任务与深度报告中的用户原始提问汇总到一个地方，不再要求先按用户下钻。通过既有校验的原始提问会完整保存和返回，提问分析视图不会再额外截断；对于历史上已完成的问答任务，优先使用答案记录中的完整提问恢复旧任务里被缩略的正文。笔记本删除后保留的提问投影，在配置的用户活动保留期内仍会进入分析；已到期投影，以及仍存在同 id 笔记本的保留行不会重复计入。页面支持按来源类型（`ask` 或 `report`）、用户和提问正文筛选；结果按新到旧展示用户、笔记本、状态和创建时间；汇总卡片按当前筛选条件统计提问总数、问答数、深度报告数与去重后的活跃用户数。搜索上限为 `ADMIN_QUESTIONS_QUERY_MAX_CHARS = 200` 个 Unicode 码点；offset 分页默认每页 `ADMIN_QUESTIONS_DEFAULT_LIMIT = 50` 条，单次请求最多 `ADMIN_QUESTIONS_MAX_LIMIT = 200` 条。HTTP 端点为 `GET /api/admin/questions`，仅管理员可读，并沿用 `USER_ACTIVITY_VIEW_ENABLED` 用户活动可观测总闸；总闸关闭时账户菜单隐藏入口，直接访问页面也会明确说明当前部署未开启该能力。首版提供可搜索、可筛选的人工分析视图，不会把提问内容额外发送给模型。

## API

当前 beta 的关键 API：

- `GET /api/notebooks`、`POST /api/notebooks`、`PATCH /api/notebooks/{id}`、`DELETE /api/notebooks/{id}` —— 删除端点返回 **202**、body 为 `{"status":"deleting"}`，不再是 204：它先提交一次单行状态变更（`notebooks.status='deleting'`）并立即返回；从这一刻起该笔记本在列表/详情/搜索/挂载等所有读侧视图里立即不可见（`GET /api/notebooks/{id}` 立即 404），但**owner** 重复 `DELETE` 同一本笔记本会拿到 **409**（codex #659 R6 P2：删除端点的守卫是 owner 专属判定，刻意不套用其它写端点那条「生命周期存活」过滤，就是为了让重试/重复的删除请求真正走到这个 409，而不是一个会误导人的 404）——**非 owner** 不管重试几次都仍是 404（不向无权限的调用方泄露笔记本是否存在）。真正的数据库与磁盘清理由此后的一个后台作业完成（物化来源文件路径、等待该笔记本上已在跑的知识图谱重建自行停下、再用一个原子的收尾步骤把被删内容归档进用户留存活动历史并删除该行）。浏览器一侧本就把成功的删除当作发后不理处理（客户端先把卡片标记删除、再刷新列表），所以这只对直接调用 API、且曾经把 204 当作「已经彻底删完」信号的调用方是行为变化——可轮询 `GET /api/notebooks/{id}`（404 即确认不可见），或（仅管理员）用 `GET /api/admin/users/{user_id}/activity` 确认留存归档已经落地。归档记录的 `deleted_at` 时间戳是收尾步骤真正执行的那一刻，比删除请求本身晚几秒到几十分钟不等（取决于笔记本规模）。整库 KG **重建**自身的删除相位（与删除笔记本是不同路径）同样先按有界分页排水再做单次原子重置；页/阈值行预算为 `KG_GRAPH_DRAIN_PAGE_ROWS`（默认 2000，允许 50–20000——上限压在 SQLite 默认 SQLITE_MAX_VARIABLE_NUMBER 之下，因为排水页把选中 id 作为 SQL 参数逐个绑定）。
...
- `SILICON_NOTEBOOK_UI_PLUGINS` 是纯前端构建期输入（`:` 分隔的本地插件包目录列表，默认未设置/空），只被 `frontend/scripts/sync-ui-plugins.mjs` 消费，绝不会到达运行中的后端进程。该脚本写出 `frontend/.local/ui-extension-contract.json`——一份部署期对账输入（不是前端或后端任何一侧的运行时依赖），形状为 `{api_version, contributions: [{plugin_id, version, contribution_id, slot, capability}, ...]}`，与 `backend/tests/fixtures/ui_extension_contract.json` 同形同排序键 `(plugin_id, version, contribution_id, slot, capability)`：内容就是那份 fixture 的内建行，拼上各已配置包 `ui-plugin.json` 里的行。浏览器侧，插件被注入的 `actions.api` 端口把每次请求限定在 `/api/extensions/{plugin_id}/` 之下，并剥离插件自带的 `authorization`/`cookie` 请求头，固定 `tag`/`auth`/`unauthorized`，插件无法覆盖它们。`requestTask()` 是插件后端共享请求级 NDJSON 任务流的唯一合法消费面：继续沿用同一套路径/请求头/鉴权限制，只把无正文的已等待时间与固定阶段码交给回调，后端错误帧只会落插件提供的安全兜底文案。插件自己的后端路由除真正的会话失效外绝不能返回 401：端口恒设 `unauthorized: "clear-and-reload"`，裸上游 401（例如插件后端依赖的某个第三方凭据过期）会把已登录用户的 token 清掉并整页刷新——插件后端必须把上游 401 翻成 `502`/`424` 再返回。无正文的响应必须经 `requestVoid` 读取，不能用 `requestJson`——后者会对响应体调 `.json()`，遇到 `204`/空体当场抛解析错误。`GET /api/admin/extensions`（仅管理员）支撑管理员页面 `/admin/extensions`——一个拓扑总览页，列出运行中后端实际加载的扩展拓扑（内建与部署注册的 contribution，检索类与 UI 类都在内）供运维查看；该页的运行时开关控件说明见下文[部署插件](#部署插件)一节，此处不重复。它与 `SILICON_NOTEBOOK_UI_PLUGINS` 无关，后者只影响前端构建。
...
- `POST /api/notebooks/{id}/ask` — 接地问答（逐句 `[k_i]` 引用；`mode`：默认 `chunk` | `reasoning`，另接受请求级 `auto` 并由后端问题理解模型收敛为 `chunk`/`reasoning`——本同步端点在持久任务创建前收敛，`/ask/stream` 则在 job 建立后于 detached worker 内收敛；`auto` 不是可展示的 Ask 引擎，只有流式路径会在选择引擎期间把它临时记在 `ask_jobs.mode` 上；`reasoning` 可传 `retrieval_effort`，默认 `standard`；官方网页端以带时区的 `asked_at` 传入只用于显示的提交时间；响应以 `answered_at` 返回权威持久化完成时间；集合型回答可带结构化 `result_sets` 与精确覆盖率；逐步推理回答还可能带 `gap_suggestions`——笔记本之外的非证据线索，见[缺口外扩检索](#缺口外扩检索)——没有该插件的部署此字段为空且不出现在 JSON payload 里；联合范围遵循上文各 mode 的边界）
- `POST /api/notebooks/{id}/ask/stream` — Ask 进度的 NDJSON stream（同样接受可选的带时区 `asked_at` 请求字段；先发带持久化 `job_id` 和 `conversation_id` 的 `started` 事件，再发进度/最终事件）；前端用该会话 id 在答案生成前立即入历史并支持重新打开。auto 模式同样先建会话与 job（`ask_jobs.mode` 暂记为 `auto`）并立即发出 `started`，引擎选择在 detached worker 内进行，选定后把真正执行的引擎写回 job 行、再发合成 start 步；因此选择引擎期间刷新、关标签或断连都不再丢问题，只有显式 cancel 会中止选择。前端在 `started` 之前离开再回到同一笔记本时，仍从本地在途记录接回该提问并继续读同一条流。推理模式提交前的意图理解与澄清确认同样不因离开而中止：理解在离开期间继续，清晰则直接启动 Ask job，需要澄清则在重开笔记本时重新弹出确认。这两个阶段还按标签页存在浏览器会话存储里：同一标签页整页刷新后回到该笔记本，理解会重新发起、澄清确认卡会重开；关闭标签页则丢弃。请求另接受可选的 `client_request_id`（字母、数字、`.`、`_`、`:`、`-`，至多 128 字符），每次提交一个键，官方浏览器每次提交都带上；同一用户带同一个键重复 POST 不会建第二个 job——流会以既有的 job 与会话 id 重发 `started`，回放已持久化的进度步骤，最后把已保存的答案作为 `final`（或该 job 的 `cancelled`/`error`）发出。刷新落在推理预检交接之后、`started` 之前的窗口时就靠这一点自动续上：标签页用同一个键重发同一次提交并接回，而不是把问题退回输入框。接回的判定在路由自己的前置校验（可用性、检索范围、模式注册表）之前——只有笔记本鉴权挡在它前面——所以即使此刻新提问会被拒绝，重发仍回放原 job。同一用户在另一笔记本已用过的键会让流以 `error` 事件结束且不创建任何东西。同步的 `POST …/ask` 接受该字段但不持久化、不去重：永远新建。transport 断开连接只会停止当前客户端继续接收，后台 job 仍继续并可保存回答
...
- `GET .../concepts/{canonical_id}/detail?limit=&after=`——概念详情按 hub 簇成员做 keyset 分页（`limit` 默认与上限都是 200，`after` 传上一页最后一个成员的 `id` 作游标）；响应新增 `member_total`（口径与分页查询一致，剔除已废弃成员）与 `next_cursor`（还有下一页时为本页最后一个成员 `id`，否则为空）。`member_total` 只在首页（`after` 为空）重算——整簇 COUNT 与这次分页要替换的整簇读代价同量级，每页「加载更多」都重算会把这项优化省下的开销大半吃回去；之后每页都返回 `member_total: null`，前端沿用首页拿到的总数，不再重新请求。`attached`（相关节点）与 `evidence`（出处）从「整簇」改为**按当前页成员**计算——每页只报该页成员的邻接节点与证据，翻完全部页仍能看到完整集合，是分页而非截断（少数概念对应上万成员的 hub 簇此前会整簇连同全部 payload/evidence 一次性返回）。该详情只供图谱视图的详情面板消费（「加载更多成员」翻页），不参与图谱本体渲染。`GET .../objects/{object_id}/context`
...
- 边可信与策展：`GET /api/notebooks/{id}/edge-review-queue` → `{items: EdgeReviewItem[], total}`（按审核优先级降序排列的边，`limit` 截断，默认 100；`total` 是队列的真实总量，与 `limit` 无关，前端可据此展示真实剩余总数而不只是拿到的这一页——它是与 `items` 同版本的队列总量：两者永远从同一个 seq-gated 排名 memo 一起读出，不会出现分别取数导致的跨版本不一致），`POST /api/notebooks/{id}/relations/{rel_id}/review`
- 治理 / 晋升：`POST /api/notebooks/{id}/knowledge/{knowledge_id}/promote`、`GET /api/promotion-queue`、`POST /api/promotion-queue/{candidate_id}/approve|reject`——两条 `promote` 路由（知识条目与 `POST /api/memories/{memory_id}/promote`）都接受可选的 `target_base_id`：只挂一个公共知识库时默认取它；挂多个时必须显式指定；未挂任何公共知识库、或目标不在挂载集合内，返回 `400`。批准读取候选上存储的目标；缺目标的存量候选在 `scripts/backfill_promotion_targets.py` 补齐前不能批准（队列里批准按钮置灰）。
...
`/api/extensions/{plugin_id}/…` 是部署插件自有 HTTP 路由的唯一挂载面：router 级会话依赖意味着没有匿名面，路由工厂拿到的是 9 个字段的 `PluginRouteContext`——`plugin_id`、`settings`、`require_notebook_capability`、`require_notebook_read`、`current_actor`、`user_error`、`url_sources`、`task_stream`、`emit_event`——绝不给 repository、全局 `Settings`、model client、FastMCP host 或原始 bearer token。**这些接缝背后的每个 core 端口都自己给请求的当前用户做授权判定**——例如 `url_sources.import_urls` 会对调用用户核对 `sources:write` 能力,不通过就 404——所以挂载点自身的 `{notebook_id}` 路径形状守卫只是纵深防御,不是授权边界本身。URL 导入端口一份实现两种调用形态：同步 (`def`) handler 本来就在 FastAPI 线程池里、直接调 `import_urls`；`async def` handler 跑在事件循环线程上，必须 `await import_urls_async(...)`——它把同一份阻塞工作（数据库写入，外加每个 URL 一次串行远端探测）连同请求上下文一起挪进线程池，因此两条路的授权判定完全一致。从 async handler 调 `import_urls` 会在动任何东西之前抛 `RuntimeError`：这是开发者错误，以带 traceback 的 `500` 呈现（文案点名该 await 哪个方法），绝不作为面向用户的文案。受信代理 SSRF 豁免（部署级）：core 的 URL 导入拒绝解析到私网/回环地址的 URL，而与本服务同机/同内网部署的插件代理天然落在私网 origin 上；部署方把这类受信代理 origin 写进 `URL_IMPORT_TRUSTED_PROXY_HOSTS`（逗号分隔；origin 精确匹配——scheme/host 统一小写、默认端口显式归一，不同端口就是不同 origin，path 不参与）。命中的 URL 只跳过「公网地址」这一条检查——导入探测与解析下载两处同享同一份白名单（它被一路穿进解析链），豁免过探测的来源不会落成「已创建但解析失败」；协议/凭证/端口形态检查照常生效，且豁免路径不触发 DNS 解析（受信代理 host 可能根本无法解析）。白名单只来自部署配置——请求级输入永远改不了它；探测半程的豁免只由 core 在这个端口背后的适配器注入（浏览器 URL 端点与 MCP 工具从不传它），解析下载半程则按同一份部署白名单对所有 origin 命中的 URL 来源生效、含 reparse——名单里公网可解析的 origin 因此也会让浏览器建的同 origin 来源在解析下载（含重定向链）获得豁免，这正是「声明该 origin 受信」的含义；名单为空（默认）时行为与接入前逐位一致。`task_stream.response(request, work, stage=...)` 是独立的请求级慢任务端口：`work(cancellation_event)` 必须同步，在线程池执行，并收到浏览器断连时会置位的协作取消事件；`stage` 必须是固定小写码，由 core 再加 plugin id 命名空间。响应复用共享的 `started` / 无正文心跳 / `final` NDJSON 协议、关闭代理缓冲，并把 worker 的任何异常压成不含异常原文的固定 `extension_task_failed` 帧。它只用于页面直接等待的交互，不是持久任务机制：`ask.engine`/`ask.gap_consult` 已在 durable Ask job 内，parser/indexing 已在后台摄取/重建任务内，完成 observer 在 durable commit 后运行，`report.exporter` 继续返回二进制。插件代码**抛出**（`fastapi.HTTPException` 或 `starlette.exceptions.HTTPException` 皆可——前者是后者的子类，两者被接住的方式完全一样）**或返回**的 401 都会被翻译成 424（并记一条事件），不会被误当成会话失效。「插件代码」既指 handler，**也指插件自己的 `Depends(...)` 依赖（嵌套多少层都算）**：FastAPI 在调 endpoint 之前就把依赖解完了，所以把上游检查写进依赖（这是最常见的写法）否则会让一个真 401 漏到浏览器、把用户登出。依赖只覆盖「抛出」那一半——依赖的返回值是作为参数注入的，永远不会变成响应。core 自己的依赖按对象身份**加**定义所在模块双判排除在外，所以 core router 级会话门产生的真 401 仍然原样是 401；生成器（`yield`） 依赖与 security scheme 不动。
...
| `INDEXING_PIPELINE_PUBLISH_DRAIN_TIMEOUT_SECONDS`(发布器在原子发布前等在飞已准入导入收敛;超时弃 stage、job 落 failed 可重试) | 600 | `0<x..3,600` |
...
新增生产扩展点，让部署插件在逐步推理问答自己的检索不够用时，提供笔记本**之外**的资料线索。它返回的内容绝不是证据：不检索、不打分、不绑定 `[k]` 锚点，不可能改动答案的任何一个字；读者要用它必须先把这条链接导入成一个普通来源，走它自己的解析与权限。

**触发条件**：只在**逐步推理**问答生效——深度报告、Knowhow 行补全的检索都够不到这个调用点（它们都不经过 `_run_reasoning_stage`：报告与 Knowhow 各自构造自己的 `ReasoningRetriever` 并直接调 `run()`）；每个检索档位都生效；每次 run **恰好一次**：在 **response-draft 阶段返回之后**、持久化之前发起，触发判据读的是草拟之前就冻结的检索事实（含所选来源图激活与检索器自身的 fail-open 降级）。排在草拟之后正是隔离得以**结构化**的原因——注入的 `ResponseDraftStage` 从它的 envelope 里看不到任何 gap 痕迹（没有披露步、没有计数、没有建议），正文因此不可能因外扩结果而变。两个条件之一，都是从这次 run 已经产出的东西里读出来的——零新增查询、零新增模型调用：这次 run 的终态披露步（就是轨迹里已经展示给读者的那条 `intent_coverage_incomplete` skip 步）仍列着一个确认过却从未执行的方向，**或者**证据池（`top_hits + chunks + elements`）薄于当前档位自己的 `ranked_final_floor`。两条都不成立就是零调用。检索本身 fail-open 降级也会落进「证据薄」这一支——轨迹步的措辞刻意不点名原因，因为笔记本确实证据稀薄、和检索自己降级触发的是同一句摘要。

**外发面**：只有一个有界对象 `GapConsultQuery`——插件拿到的仅此而已。`question` 是用户**实际见过**的措辞：只有本轮真走过澄清门（确认契约的 `needs_clarification` 为真，用户在门里审阅并可编辑最终问题）才用审阅后的最终问题；意图清晰的自动确认不经人眼，其 `resolved_question` 是模型改写（可能吸收此前轮次的措辞），绝不外发——此时与未确认意图一样用原始问题原文。截到 `GAP_CONSULT_QUESTION_MAX_CHARS`；`gaps` 最多 `GAP_CONSULT_MAX_GAP_PHRASES` 条已确认却未执行方向的短标签，每条截到 `GAP_CONSULT_PHRASE_MAX_CHARS`。两者外发前都会剥除 `[k]`/【k】引用标记。意图契约的复合串 `research_question`（目标 + 必答主题 + 约束 + 假设）绝不外发——只有用户自己审阅过的原话才会。`GapConsultCallContext` 不携带 notebook id、actor id、source id、证据或检索范围：这一点上插件拿到的身份天生就是空的，靠字段集合本身保证，不靠谁记得去过滤。

**预算与 fail-open**：冻结的 `GapConsultHost` 把一次 contribution 的可用性探测与它的 `consult` 调用一起放进一条私有 `daemon=True` 线程执行——不用线程池，一个挂死插件只在受影响的那次请求上泄漏一条线程，绝不占用全部署都依赖的共享 worker；也不 `contextvars.copy_context()`，插件不会因为跑在这条请求底下就继承它冻结的检索范围、检索 run 或叶子 I/O 扇出槽位——主线程按 50ms 分片 `join`，对着一个覆盖**整次调用**（探测与 `consult` 两半都算）的硬墙钟 deadline（`ASK_GAP_CONSULT_TIMEOUT_SECONDS`，默认 4.0 秒，`0 < x ≤ 30`）。取消与这个 deadline 在 join 循环的**每一趟**都会重读，包括观察到 worker 已经跑完的那一趟：预算花光之后才答出来的 contributor 就是答晚了，晚到的结果与挂死的一样被丢弃——只在线程还活着时读 deadline，会让结果取决于那 50ms 分片恰好落在线程死亡的哪一侧。这份预算花在读者拿到答案**之前**——与上文那些协作式的完成后 deadline 不同，这里每一秒都是答案的等待时间——所以默认值小、上限也收得紧。**一个真挂死的插件每次受影响的请求都会泄漏一条 daemon 线程；这是已登记、已接受的代价，不是缺陷——熔断器留待后续。** 除了干净、按时、形状正确的响应之外，其余一律 fail-open：没有注册的 host、休眠的扩展点（`has_contributions()` 为 `False`）、畸形调用上下文、调用方给出的空问题（host 同样不信任自己的调用方——实践中这条不可达，因为 `AskRequest.question` 早已强制 `min_length=1`，一个空问题根本走不到这个调用点）、抛异常的 contributor、预算耗尽，或者 host 答出一个端口从未承诺过的形状——全部返回 `()`，答案与接入前逐字相同。唯一的例外是取消，它会继续传播（`AskCancelled`）。「fail-open」指答案不变、不出错误横幅/诊断——**不**指 `gap_consult` 披露步会一并消失：这一步存在的意义就是告诉读者发生过一次有界外发，所以超时、抛异常或畸形结果同样要记下这次尝试、建议数如实记 0，绝不能对「这次外发发生过」本身保持沉默（codex #584 R4 建议失败时抑制该步，已驳回；见 `ask_service.py::_consult_gap_sources`）。净化（title 非空；`url` 限定 `http`/`https`、host 非空、无控制字符，且绝不截断——截短的 URL 是一个悄悄指向别处的错误目的地，超长的直接丢弃；summary/source_label 裁剪截断）发生在 host 里、插件返回**之后**，插件自己够不到这一步。`status` 为 `UNAVAILABLE` 的结果**一条都不接纳**，无论它带没带条目：那个状态就是 contributor 自己声明「这次我服务不了」，它的载荷没有资格反驳自己（这些 URL 也因此不进跨 contributor 的去重集合，真正能服务这次调用的插件不会被一条已被撤回的链接挡住）；`PARTIAL` 说的是「只有一部分」而不是「一条都没有」，仍照常接纳。接纳这件事**本身的工作量**也是两个维度都有界的——最多只检查「还能收几条」的一个有界倍数，每个字符串在被走过之前先按自己的上限加余量切一刀——所以插件返回一份无界载荷，付出的是一次有界扫描而不是无界扫描，且这已经发生在上面那个 deadline 被兑现之后。这一刀切的是插件产出，绝不是用户数据。
...
表头识别保留结构上最早成立的候选行，年份等数值标签同样是合法表头；后续全是文本的数据行
不会静默取代它。

只有 `reasoning` 问答会消费快照。冻结的用户勾选来源范围里没有已编译工作簿，或问题不具备
表格分析意图时，这条分支完全不运行；include/exclude 范围与检索共用冻结 scope 的 `allows`
语义，并遍历当前笔记本及所有获准挂载的参与库；被排除的工作簿和参考库绝不会进入规划模型。
意图必须是明确工作簿/表格上下文配合分析短语，
或无歧义的表格运算短语；无关词语里碰巧出现单个“行”或“列”不算命中，英文 `count`、
`top`、`group` 等单个动词也必须带表格上下文，除非组成 `sum Amount by Region` 这类明确
结构。命中时最多复用一次
部署既有的 `reasoning_agent` chat
workload 做有界规划，不引入 Anthropic SDK 一类通用 Agent。模型结果必须通过白名单校验，
操作只允许 `profile`、`aggregate`、`top`、`filter`，聚合与筛选算子也都是封闭集合，实际计算
在本地确定性执行。单个规划最多包含 8 个筛选条件和 20 个结果列；超过任一固定边界会让整份
规划失效并回退本地概览，不会只执行用户所要求条件或列的前缀却声称结果完整。规划超时或
形状错误同样回退本地概览，整条分支异常则 fail-open 回到原有逐步
推理回答。成功结果新增 `spreadsheet` 轨迹步、可引用的有界证据和一个
`result_sets[].kind="spreadsheet"` 结果卡；卡片明确披露扫描/结果/展示行数、公式缓存缺口与
是否只是部分预览。规划模型看到的是选中工作簿/工作表/表头目录按聚合字节预算形成的
“整字段前缀投影”，不会从中间截断某个用户字段；目录不完整时结果卡会明确提示。持久化及
浏览器收到的结果另受行数、单元格数（含表头）和序列化表格字节三重约束，最终合成模型只
接收再经过行数与字节双重约束的预览。
在该预览字节护栏内获准进入的每一行，会完整保留在可引用证据映射中，不再接受第二次隐藏的
字符切片。
...
页签。每个用户行同时保留「查看提问」与「LLM 日志」两个入口：前者进入固定只看提问的
分析页签，后者进入按该用户过滤的「模型调用」日志视图；解析问题可按用户、状态、类型和
...
- **活动流** —— 提问 / 来源 / 报告三类条目按时间倒序混合成一条流，按 `(created_at DESC, id DESC)` 复合游标翻页（「加载更多」），默认每页 50 条（`limit`，上限 200）。类型筛选可在全部活动和只看提问/来源/报告之间切换；只看提问时中栏改称「提问概览」，每条问题原文完整显示。用户使用总览里的「查看提问」直接进入该用户的全部提问，用 `ask_jobs.created_by` 与总览计数保持同一口径，包含该用户在共享笔记本中的提问；管理员仍可按自有笔记本和日期缩小范围，用来整体查看用户关注点。筛选在 repository 的各表 `LIMIT` **之前**生效，不是在浏览器拿到混合页后再过滤，因此更晚的来源/报告不会挤掉提问，所有问题都能通过复合游标继续翻到。原生日期选择器 + 「全部时间」驱动一个半开区间 `[since, until)`。区间边界带**浏览器 UTC 偏移**（`2026-08-04T00:00:00+08:00`），所以「一天」指的是浏览者本地日历日——与界面上一切时间的渲染时区同一个；两端各自算偏移，夏令时切换日不会差一小时。畸形边界返回 `400` 加可上屏的中文文案（`X-User-Message`），既不静默放宽窗口，也不 500。三类条目在两个数据库后端都经同一个 `parse_activity_instant`（`app/core/activity_time.py`）归一时间：SQLite 侧经 `julianday()` 按绝对时刻比较（历史行是裸 naive 文本、新行带 offset，混合格式统一按裸串=UTC 读），PostgreSQL 侧直接按 `timestamptz` 原生比较，并带与 SQLite 同形的 `COALESCE` 兜底（`ask_jobs.created_at` 在 PG schema 里可空，而 PG 的 `DESC` 默认 `NULLS FIRST`，少了兜底这行会被顶到第 1 页并让 Python 归并直接 `TypeError`）。两侧把不可解析 / NULL 的值折到**同一个哨兵时刻**：它既排到最末、又能经游标往返，`next_cursor` 因此不会发出下一页解析不了的值。三类各自独立 keyset 分页、在 Python 内按时间归并，不做三表 `UNION ALL`——大库上可诚实回报「本页覆盖到 HH:MM 为止」的有界边界，而不是一次全局排序爆炸。
  共享库提交按调用者权限区别处理：管理员保留按创建者归属的完整审计历史，即使被查看用户后来失去共享库读权仍可查看；普通用户查看自己时，只能看到笔记本当前仍通过规范实时读权谓词的提交。提问详情复用同一条自助读权检查，因此保留旧 job id 也不能在失权后绕过清单读取内容。
  删除笔记本另有一条有界规则：删除聚合的同一事务会把其中的可见来源、提问和报告投影成一份**不带 notebook 外键**的最小分析摘要。默认从删除时刻起留存 **180 天**，可用 `USER_ACTIVITY_RETENTION_DAYS` 配置（`1..3650`）。摘要只保留归属、原笔记本 id/名称、活动时间、问题/报告提示、来源显示名/文件元数据、状态以及删除/到期时间；答案正文、引用、推理轨迹、来源正文/元素、报告章节与参考文献仍随笔记本立即删除。若离线合库已在同 id 的存活笔记本旁放入旧摘要，只要笔记本仍在线，这组留存数据就不参与统计、活动流或详情；之后实际删除时会先丢弃整组旧摘要，再按当前聚合重新投影，并从这次真实删除时刻重新开始留存窗口。窗口内，管理员用户使用总览的来源/提问/报告总数和未按笔记本收窄的管理员活动流继续计入这些摘要，条目带 `notebook_deleted_at`/`retained_until` 并标记「原笔记本已删除」；已删除笔记本不再出现在范围栏，也不能作为笔记本筛选项。本人自助读取仍要求笔记本实时可读，因此删除后的摘要只对管理员可见。`expires_at` 是强读闸，到期后统计、活动流和详情立即不可见；启动恢复或下一次删除笔记本时再做物理清理。这是滚动时间留存，不是备份，也不是无限审计日志。
- **详情** —— 选中仍存活笔记本的提问显示完整问题、答案、引用与推理轨迹（复用既有 Ask 面板渲染件，只读）；删除后留存的提问只显示问题/状态摘要及删除、到期说明。选中来源显示显示名、解析状态与派生诊断；选中模型调用（仅「模型调用」视图内）复用既有 prompt/response 详情面板，未改动。
...
- `GET /admin/users/{user_id}/activity?activity_type=&notebook_id=&since=&until=&before_ts=&before_id=&limit=` —— 上文活动流；可选 `activity_type` 只能为 `ask`、`source` 或 `report`，省略时仍返回混合流。
