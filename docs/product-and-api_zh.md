...
**跨库条目。** 条目若属于一个挂载参考库（而非 active notebook）的集合，会标注「来自参考库《名》」且保持只读：不提供跳转来源详情、也不发起图片请求——挂载一个库不等于持有该库的直接成员权限。服务端可以随条目带出上面所述的有界原文引用，使清单卡和答案引用详情无需浏览器侧特权读取即可展示标题、位置与摘录。仍保留的一项诚实边界是「按纯文本兜底解析（无结构化解析）」来源的计数披露：这类来源的元素本身就不存在，元素类枚举因此完全看不到它们；覆盖率只对**已经入库的元素**声明完整性，不对作用域内的每个来源声明完整性。
