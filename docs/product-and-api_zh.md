...
仅本档对可识别的 Markdown 表格和分号分隔的重建表格使用 `REASONING_REFLECT_TABLE_EXCERPT_CHARS`（默认 1,200 字符；校验范围 240–4,000）。按查询相关性选取完整数据行并保留列头和可识别表格说明（包括单位与评测条件），显式说明省略行，使比较问题有机会看到表尾目标行。普通正文沿用原摘录上限；表格解析不可靠或空间不足时回退到原有有界摘录。这是现有分档总证据预算内的单卡额度，不新增检索或模型调用。没有方面或大纲绑定时，K/D 中已展示的证据也可随查询获得补充卡。补充按版本追加，不改写已发送的 K/D 字节，并跨轮继续有界扫描；压缩时按上述相关性规则选择原卡或最后已发视图，平局取最新。这些变化改善反思可见内容，但不代表服务端缓存已命中或延迟已有改善。
