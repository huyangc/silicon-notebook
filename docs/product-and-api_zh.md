...
兜底仍在：连续 `--max-consecutive-model-failures`（默认 `32`）个来源都因模型问题
失败、中间没有任何一个成功时，升级为原来的任务级熔断，照常公布
`stopping`/`kg_build_circuit_opened`。阈值必须高于 `--workers`——一次瞬时抖动会同时
打中所有在飞来源。该计数是任务级的、跨目标分页存活：抽取目标按 500 行原始来源翻页，
