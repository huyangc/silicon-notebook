...
代价是同一档下的两个工具在记录里读起来一样（已登记）。被拒绝的调用不留痕（一份「谁被拒了」
的日志是另一种东西，隐私后果也不同）——**包括被收口之后那些闸拒掉的**。有两个工具带着这样
一道闸：写类工具在 `require_agent_access` 之后还有 owner-only 检查，`get_memory` 读候选条目
则要求 `memory:read_candidates`。两者都关掉自动记账、在自己那道闸放行之后再记，所以这条
不变式是「所有闸都过了才记」，而不是「在收口处记」。（已登记的边角：`get_memory` 查一个
不存在的 id 同样不记——它在闸判完之前就抬手了，而记账记的是「到达了这个库的数据」，不是
「有人试过这个 id」。）记账写失败只吞掉并记一条**不带任何内容**的日志（不带异常文本——关于一次调用的账不是
