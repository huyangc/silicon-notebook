...
同样这三个请求还接受可选的顶层 `base_scope`：`{ "mode": "include" | "exclude", "notebook_ids": string[] }`，最多 1000 个 id。它与 `source_scope` 形状、校验和 API 入口的 exclude→include 冻结完全对称，只是粒度是整库：`include` 只放行列出的已挂载参考库，`exclude` 放行除列出者以外的全部已挂载参考库。id 只能指向当前 notebook 当前已挂载的参考库，否则返回 422。省略该字段保持“所有挂载参考库无条件全量参与”的历史行为，因此从不发送该字段的客户端，其请求形状与行为都逐字不变。`exclude` 配**空列表**——浏览器“全选参考库”的紧凑表示，每次提交都会带上——按等价于省略该字段处理，而不会展开成列出每一个挂载库 id 的 include 列表：两者是同一个选择，展开只会让冻结列表的长度跟着挂载数量走，撞上下面那个挂载 API 自己并不设限的上限。参考库 id 刻意不并进 `SourceScope`：两者粒度不同、生命周期也不同，而且只收窄了一个维度的报告不能被替另一个维度伪造选择——伪造的本地范围会在确认时被重新冻结成“当时可见的全部来源”，伪造的参考库范围则会把此后才挂载的参考库永久锁在用户从未收窄过的报告之外。
...
这个顶层复选框范围是当前 notebook 的硬上限，与问题明确点名来源时生成的 `QueryIntentContract.source_scope` 审阅子集并非同一层合同。逐步推理会将审阅子集与复选框上限取交集：模型只能保持或进一步缩小用户选择，不能扩大。收窄复选框时，无法在遍历前安全应用当前来源谓词的持久化通道（当前 notebook 的全图/PPR/关系扩展、精确章节查找与报告整库画像）会被跳过；按来源限定的 chunk、元素和 KG 直接检索仍正常执行，base 库 KG 种子也可直接映射回 base 原文，不经过组合全图遍历。
