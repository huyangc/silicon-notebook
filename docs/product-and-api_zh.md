...
`REASONING_REFLECT_OPTIMIZATION=prefix_delta_evidence` 是需要显式选择的 v2 实验档，复用 `prefix_delta` 的证据卡、冻结 S/C 前缀、有界 K/D 快照与增量，以及不可逆的快照回退。它移除逐项自评指令与 schema 字段、自评吸收和追问、动态方面状态块，以及自评派生的合成缺口提示。用户确认的必答主题和约束仍按既有输入边界保留在 C；按问题覆盖情况决定停机的常规指令、工具权限、枚举完整性、大纲绑定、证据/历史预算和模型调用预算均保留。由于不再有方面绑定，证据卡优先档只剩大纲绑定；检索决策与最终证据不保证和启用自评的实验档相同。

仅本档对可识别的 Markdown 表格和分号分隔的重建表格使用 `REASONING_REFLECT_TABLE_EXCERPT_CHARS`（默认 1,200 字符；校验范围 240–4,000）。按查询相关性选取完整数据行并保留列头和可识别表格说明（包括单位与评测条件），显式说明省略行，使比较问题有机会看到表尾目标行。普通正文沿用原摘录上限；表格解析不可靠或空间不足时回退到原有有界摘录。这是现有分档总证据预算内的单卡额度，不新增检索或模型调用。没有方面或大纲绑定时，K/D 中已展示的证据也可随查询获得补充卡。补充按版本追加，不改写已发送的 K/D 字节，并跨轮继续有界扫描；压缩时为入选键保留最后已发送的卡片版本。这些变化改善反思可见内容，但不代表服务端缓存已命中或延迟已有改善。
