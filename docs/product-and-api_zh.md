...
**每 run 一行的闭集投影为此新增十一列**（`app/domain/reasoning_trace_stats.py` 的 `RUN_PROJECTION_KEYS`；这是**离线分析面**，不是任何 API 响应字段）：`run_wall_ms` 一次 run 的墙钟（轨迹里没有这件事实——按步耗时求和漏掉排队与步间空隙——所以只有 rig 手上有，只读导出恒缺）；`model_calls_real` 这次 run **真正发出去**的模型请求数（各 reflect 步之和，与「日志行数」不是同一个数：`response_format`/`stream_options` 被拒后的重建会多发一次请求却不另写日志行）；`attempts_observed` 上面那个数全不全（每一条 reflect 步都带 ⇒ 是；只带了一部分 ⇒ 否，此时那个和只是下界；一条都没带 ⇒ 缺）；`optimization` 这条 run **声明**跑的是哪一格 `REASONING_REFLECT_OPTIMIZATION`（声明而不是从轨迹反推，所以只读导出恒 unknown）；`context_chars` 最后一轮那五个块的字符数加整条消息的字节数（短码 `s`/`c`/`k`/`d`/`t`/`bytes_total`；⚠ 最后那一格的单位是**字节**，它既不是前五格之和、也不该拿去和它们比大小）；`prefix_bytes_median`/`prefix_bytes_min`/`prefix_turns` 逐轮公共前缀的三格聚合（逐轮细节按设计只留在 rig 的 per-call 表里）；`response_chars_total` 各 reflect 步交还给调用方的正文字符数之和。最后两列是 `prefix_delta` 专属的**行为事实**（PR-3）：`context_rebuilds` 这次 run 一共**做过几次**重建证据卡快照 K 的动作——**含最后那次被丢弃的**（触发回退的那一轮先压了一版 K、再判定仍装不下，于是整版丢掉：那次全池重选的开销真的付了，所以它计数；但模型看过的 K 版本数因此比这一列少一格，读这一列时不要把它读成「消息里换过几版 K」）——取各 reflect 步的**最大值**（max-over-present，同 `stale` 的口径，不是「任一步缺 ⇒ 整列 unknown」那一种：写侧是 run 级单调累计的计数，取最大值才是「总共重建了几次」，喂逐轮布尔进去会让一条重建过五次的 run 在表上显示成 1）；`context_fallback` 这次 run 有没有不可逆地退回有界证据选择——**任一** reflect 步为真 ⇒ 整列真（回退不可逆，「发生过一次」与「一直在发生」是同一件事）。⚠ 这两列 **`0` / `false` 与缺值必须分得开**：`context_rebuilds == 0` 是「量到了，答案是零次」，`None` 是「没量」（非 `prefix_delta` 臂，或旧轨迹）——`test_context_rebuilds_zero_is_distinct_from_unknown` 钉住这件事。同一批写侧还有第三个键 `delta_blocks`（这一条消息里有几块 D），它**已登记进 reflect 步的稀疏键集但刻意不出顶层列**：它不是累计量（重建会清空已发出的块），一个 run 一格装不下它，要看它就读那几步的 detail。**没量到 = 缺值，不是 0**：旧轨迹以及 `legacy` / `off` / `prefix_snapshot` 的行里这十一列一律缺值（前九列还要求测量开着；最后两列与测量开关**无关**、只与臂有关——`prefix_delta` 的行即使测量关着也带着它们，见部署文档 `REASONING_REFLECT_MEASURE_CONTEXT` 那条已登记偏离），而 `model_calls_real`/`response_chars_total` 这两列在**任一** reflect 步缺观测时整列 unknown，不按现有的几步求和凑一个偏低的数。这份闭集的真源就是 `backend/app/domain/reasoning_trace_stats.py`——**后续修正**：2026-09-08 那份 T0 轨迹分析设计稿 §3 列的键表**早于**这十一列（也早于 `context_chars` 里那个字节口径的 `bytes_total` 短码），它记的是当天的决定，不要拿它当闭集清单读。
