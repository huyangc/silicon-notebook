...
- **回退之后**（拍板 Q4，不可逆）本 run 剩余轮改用 `prefix_snapshot` 的有界证据选择，**已经发出的 D 原样留在上文**、S 里那几句解释 D 怎么读的规则也照旧发（`test_delta_static_prompt_keeps_the_rules_while_any_block_is_still_sent`）——抽掉模型已经读过的上文，或留着块却撤掉读法，两种都比多付几千字符更糟。回退不清空候选池、不撤销任何绑定资格、不重置配额、不多一次模型调用或轨迹步。⚠ 回退之后每一轮的 K 都按「**档位证据池 − 保留 D 已占的证据半**」构造，而保留 D 里**已经可见的那些键**被排除在 K 的候选之外（近期观察窗同理先减去保留 D 占的历史半）：所以一条消息的证据合计仍然 ≤ 档位证据池，同一条证据也不会以两份都不带版本标记的写法出现两遍（`test_delta_keeps_the_cumulative_evidence_budget_across_blocks` 的回退轮上界、`test_delta_fallback_turn_keeps_its_own_cards_out_of_the_kept_blocks`）。代价如实：保留 D 把池子吃得多时，回退之后的 K 可以是空的——那时模型手里那几块 D 正是它已经读过的那批证据。
