...
**预算与续跑。** 一次动作在其 run 级预算内（`enum_rows_per_run` 总行数、`enum_pages_per_run` 额外翻页次数）自动翻页，直到游标耗尽或触及某项上限；只有每个被访问分片（元素按来源、知识对象按参与的库）的第二页及之后才计入页上限，因为一个分片的第一页是唯一能看到它的方式。该游标是 run 内部句柄，不出现在响应里——客户端只看到覆盖率徽章；截断时看到的是「同一轮推理内可以继续列出」，而不是一个可操作的续跑入口。`complete=false` 恒含一个可续跑的游标，唯一例外是 `truncated_reason=concurrent_change`——此时作用域在两次调用之间发生了变化，链条绝不能静默从头重来。对尚未列完的集合重复请求，若本 run 预算尚有行数/翻页余量则从该游标续跑；预算耗尽时改为按「已达本轮上限」跳过（仍披露为部分结果），而不是按「已枚举过」跳过——只有链条已经 `complete=true` 的集合才会被跳过为「已枚举过」，因为再问一次只会重复已经报过的条目。
...
将 `REASONING_ENUM_TOOLS_ENABLED` 设为 `false` 可完全禁用这两个工具：不构建地图、两个动作都不提供，逐步推理回到接入前的行为，零额外查询开销。
