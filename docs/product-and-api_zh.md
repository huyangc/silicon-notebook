...
**范围是清单身份的一部分，而且看得见。** 续跑账目按范围记键，所以「先 `"current_notebook"` 列全、再请求 `"all"`」是**两条不同的链**：第二次是一次真的枚举，不是 `already_enumerated` 跳过；反向顺序同理。两条链共用同一个 run 级枚举预算池，所以重复列的代价自然被夹住，不需要特判。收窄过的清单在每一处报数的地方都说出这件事：`enumerate` 轨迹步带 `detail.local_only=true`（默认范围时不带这个键），上屏摘要与回喂给模型的枚举账目都带后缀「（仅当前笔记本）」，与元素清单限定单一来源时的「（限指定来源）」同形同位。
