...
- **回退之后**（拍板 Q4，不可逆）本 run 剩余轮改用 `prefix_snapshot` 的有界证据选择，**已经发出的 D 原样留在上文**、S 里那几句解释 D 怎么读的规则也照旧发（`test_delta_static_prompt_keeps_the_rules_while_any_block_is_still_sent`）——抽掉模型已经读过的上文，或留着块却撤掉读法，两种都比多付几千字符更糟。回退不清空候选池、不撤销任何绑定资格、不重置配额、不多一次模型调用或轨迹步。⚠ 回退那一轮的证据字节因此**可以高于档位证据池**：K 由有界选择按整份硬预算重选，保留的旧块跟着走，越出的部分正是那些旧块。
