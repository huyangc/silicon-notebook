...
历史上下文重新进入模型。范围回执区分允许参与、实际检索、未检索及中文原因、词法降级和
实际引用的库。检索超时或未完成跳过对应库并披露局部覆盖，检索为空仍计入已检索。跳过原因只陈述发生了什么，
不猜测「索引损坏」之类的原因：只有**确实跑超时**的检索才请用户收窄范围；阶段时限到点时仍在排队、
一次查询都没发出的库说明「检索未开始」；其余一律请稍后重试。可供排查的原因码以内容无关的
`global_retrieval_skipped` 事件单独记录。权限变化
仍拒绝交付，不把「未引用」解释成「没有资料」。只有完成且范围兼容的轮次进入追问语境，
通过共用问题改写解析指代；既往助手回答仅用于理解追问，不作为证据。
完成前核验引用段落的全部元素，以来源标识和文本指纹比较，忽略无关元数据更新。
失效证据剔除后，使用剩余段落重新合成一次；再次变化或没有有效支撑时返回不接地的重试
提示，不仅删除引用标记而保留失去依据的结论。
...
`GLOBAL_ASK_RETRIEVAL_TIMEOUT_SECONDS`（默认 30，大于 0）限制检索阶段，
`GLOBAL_ASK_NOTEBOOK_TIMEOUT_SECONDS`（默认 5，大于 0）限制其中每个库的检索。
参与库并行检索，进程级上界为 `GLOBAL_ASK_RETRIEVAL_CONCURRENCY`（默认 4，范围 1–8），
因此检索阶段不再是各库耗时之和。该上界由所有在跑任务共享，每个任务只占自己的公平份额
（执行位数 ÷ 活跃任务数，每完成一个库重算一次），因此在别的任务运行期间提交的任务照样有进度，
不会一直排队到自己的时限耗尽。单库时限从该库真正开始检索时起算，而不是从排队时起算；
阶段总时限是绝对的。回执顺序与证据选材按解析后的参与库顺序，不按完成顺序，
同一问题在同一组库上给出同一份答案。
...
冻结来源范围覆盖索引来源目录时使用已暖 ANN；目录缺席时（与下文库内通道不同）改用**验证式超取**
后也使用已暖 ANN。chunk 索引由该库全部 chunk 建成，因此也含全局天花板按来源类型排除的
Memory/Knowhow 投影 chunk，单纯取 Top-K 可能整批都是会被天花板丢掉的行。所以目录缺席时先只读
命中行的来源身份来验证：一个都没被丢弃即为精确结果；否则按几何倍数放大 k 重查，最多两轮。
仍不足召回目标且索引尚未取完时，该库回落到在 SQL 内应用天花板的精确分页通道；超出该通道规模
上限时保留已有幸存候选，计入 `degraded_notebook_ids`，并留下一条内容无关的事件回执。取完整个
索引后幸存数少于目标不算饿死——那批幸存就是被允许的全部。只有该通道**确实贡献了幸存候选**才算
语义已检索，不因为「调用过向量查询」就算。权限自始不变——水合后仍在打分前剔除冻结清单之外的
候选。小库还可按批流式读取已存向量进行语义召回，
只保留靠前候选，不写共享矩阵缓存。规模闸在首批与扫描结束后各校验一次；扫描期间涨过上限的库
不发布任何语义结果。`GLOBAL_ASK_SMALL_NOTEBOOK_MAX_CHUNKS` 默认 20000、
