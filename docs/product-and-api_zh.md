...
来源侧栏初始全选所有可见导入来源，每行提供复选框，并提供“全选”/“清空”。当前选择同时传入不读语料的问答意图预检、问答执行和新建深度报告，约束当前 notebook 的内容块、来源元素、知识证据与关系、图路径/PPR 输出以及报告检索。范围被收窄时，隐藏的 Memory/Knowhow 投影证据也不参与，因为这些内部来源没有面向用户的复选框。已挂载的参考库是独立参与者，始终保持在范围内。

`AskIntentPreviewRequest`、`AskRequest` 和 `ReportCreate` 接受可选的顶层 `source_scope`：`{ "mode": "include" | "exclude", "source_ids": string[] }`。`include` 只放行列出的当前 notebook 可见来源；`exclude` 放行除列出来源以外的所有当前 notebook 可见来源。省略该字段保持历史整库范围，`exclude` 加空列表是前端表达“当前所有可见导入来源”的紧凑形式。API 入口会把每个显式排除式范围冻结为明确的 include 列表，候选生成器在有界 FTS/SQL 的 `LIMIT` 之前应用该列表，因此并发新增来源不会扩大已开始的运行。后端对外库、隐藏或已失效的来源 id 返回 422；若本地有效范围为空且未挂载参考库，问答/意图预检/报告创建返回 409。浏览器同步禁用问答输入和新建报告控件，但仍可查看既有报告。报告会把解析后的范围持久化在问题理解合同中，并在意图确认与生成前重新校验，使规划和写作持续使用同一份选择。

这个顶层复选框范围是当前 notebook 的硬上限，与问题明确点名来源时生成的 `QueryIntentContract.source_scope` 审阅子集并非同一层合同。逐步推理会将审阅子集与复选框上限取交集：模型只能保持或进一步缩小用户选择，不能扩大。挂载参考库仍是独立参与者，除非审阅后的意图范围进一步限定它们。收窄复选框时，无法在遍历前安全应用当前来源谓词的持久化通道（当前 notebook 的全图/PPR/关系扩展、精确章节查找与报告整库画像）会被跳过；按来源限定的 chunk、元素和 KG 直接检索仍正常执行，base 库 KG 种子也可直接映射回 base 原文，不经过组合全图遍历。
