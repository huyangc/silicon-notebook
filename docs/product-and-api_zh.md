...
登记检索时刻证据，引用复核与语义段落同一口径，但不产生覆盖回执。每次运行发一条
...
没有可见来源的库不发查询；查找本身没有来源谓词，所以某库当前可见来源已不等于其冻结天花板时
只跳过这一个库。合并以**整节**为单位：每库先出第 1 节，再出第 2 节……，已取过的段落剔除，总数以
`GLOBAL_ASK_CANDIDATE_LIMIT` 封顶且绝不切断一节（第一节放不下就停）；每条段落保留精确查找标记与
所属笔记本。保底席位按库分配、总数不变：`chunk` 模式 `mix` 选择里的 `EXACT_SECTION_RESERVE` 与
`reasoning` 合成前缀的 `REASONING_EXACT_RESERVE`，在返回了精确段落的库之间平均分配（余数给最先出现
的库；库数多于席位时只有前几个库各得 1 席），一个库的大节不能占满全部席位；段落只来自一个库时
（所有单库问答）两者与以前完全相同。失败、超时、回执、横幅与证据规则同关键词臂：某库这条腿失败只
意味着该库没有精确段落，不进入「已检索/已跳过」回执、不上横幅；交给调用方的段落登记检索时刻证据。
`reasoning` 的一次 `exact_lookup` 动作仍按一次（现在是一次联邦查找）计入每 run 的次数上限。每次运行发一条
