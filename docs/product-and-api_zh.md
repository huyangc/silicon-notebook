...
`POST /ask` 按 `mode` 分派——注册表 `backend/app/services/ask_modes.py` 是唯一真源（默认 `chunk`）。`chunk` 基线与 `reasoning` 的 `search_chunks` 共用的那条原文段落通道现在是**参与集**口径（当前笔记本 + 本次勾选的参考库），挂载参考库自己的段落因此会进入回答，每条候选带自己的归属库。开关是 `CHUNK_FEDERATION_ENABLED`（默认 true；false 是唯一的回退路径，行为回到只召回当前库原文），上界是 `CHUNK_FEDERATION_MAX_PARTICIPANTS`（默认 8；超出时按确定的挂载顺序截断并发一条内容无关的 `chunk_federation_truncated` 事件，不静默）。非当前笔记本的参与库只贡献它当前可见的来源——隐藏的 Memory/Knowhow 投影结构性不参与这条跨库通道；非当前库的大库只借用已常驻的索引、不冷加载，没有暖索引时退回有界词法召回。可选 KG overlay / PPR 在此之上再加入 federated KG 与 base-backed chunk；`reasoning` 走 federated KG。overlay 命中参考库的知识对象时，evidence 反查回该库自己的原文段落，受与上面同一套逐库天花板约束（只取该库当前可见来源；被取消勾选的参考库整库跳过；只走已回填的 element→chunk 反查索引，未回填的库这一轮不出原文并发 `kg_source_chunks_peer_unindexed` 事件）。`federated_retrieve()` 的知识对象命中不改 score，只在完全平局时以 `base` 为第二排序键；`federated_retrieve_relations()` 的关系命中仍只按 score 排序。这些排序信号不进入接地阈值。
...
- 删除知识图谱（界面：知识图谱视图「图谱处理」里的**「删除知识图谱」**，仅可编辑成员可见，点击后先确认）：`POST /api/notebooks/{id}/kg/delete`（notebook `kg:write`）启动**后台**任务并返回 `{status: "deleting", notebook_id, job_id}`，没有 LLM 前置条件。它删除该笔记本从来源分析出的知识图谱——用户文档的知识对象、关系、事实、向量、抽取记录与图谱检索行，概念合并簇与合并候选，以及图谱分析产物——并原地重置统一图状态；大图先按有界分页排空，再走一次最终重置事务（与「全部重新分析」重抽前执行的是同一套删除）。来源及其解析内容保留，之后可以重新整理；隐藏的 `memory` / `knowhow` 投影对象同样保留，因此有已确认记忆或 Knowhow 知识的笔记本删除后仍可能 `kg_ready`。与「全部重新分析」一样，它不清理 `kg_conflict_candidates`：待处理的冲突候选可能仍指向已删除的对象，目前没有界面展示它们。图谱行提交之后，任务还会刷新图谱视图的独立预览产物，使删除后的第一次图谱读取不再画出已删除的节点：在进程内预览构建预算以内，为剩余的隐藏对象重新发布预览（什么都不剩时直接退役预览）；超出预算时只退役不构建，视图显示暂无预览，直到 scale 索引构建发布新的预览。这一步 fail-open：预览的认领被别处占着或刷新失败时，删除照样成功，旧预览可能继续被返回，直到之后的某次刷新成功（预算以内是图谱读取自带的后台刷新，超出预算则是下一次 scale 索引构建）。把本笔记本当作参考库挂载的笔记本也将检索不到被删除的知识。单飞**与补上关联、重新合并共用同一个任务槽**：三者任一在跑时点击都返回 409 并点名真正占着槽的动作（删除占槽时是「当前笔记本正在删除知识图谱，请等它完成」）；与分析作业的交叉互斥与上文完全一致——分析在跑时删除得到「当前笔记本已有知识图谱分析任务正在运行」（本进程内的分析靠进程内标记识别；另一进程建的 running `kg_build_jobs` 行——例如与后端并跑的 `batch_ingest` CLI 分析——在占槽成功后立即探测一次；该探测之后另一进程才建的分析行看不见，与任务槽本身同为按进程的限制），删除在跑时发起整理 / 全部重新分析 / MCP build_kg 被拒并点名删除（切换索引管线保存意图后返回它自己的可重试 409）。删除任务刻意不占进程内的分析标记，所以 `NotebookSummary.kg_building` 仍只表示「分析正在运行」。排空第一页之前，该笔记本所有已结束的 `kg_build_jobs` 行都被标成 `stage='cleared'`：行本身保留（管理员用量按行计数），运行中的行永不触碰。笔记本摘要把「最新一行已清除」投影为 `kg_build: null`，因此删除后笔记本显示为尚未整理，而不是复述上一次分析的结果；下一次分析作业照常呈现。删除失败时这段历史仍保持已清除，因为已提交的排空分页可能已经改动了图谱。完成信号是 `GET /api/notebooks/{id}/kg/delete/status`（notebook 读权限），返回 `{job_id, notebook_id, status, running, objects_deleted, relations_deleted}`，`status` ∈ `running` / `succeeded` / `failed` / `idle`，两个计数（删除的用户文档对象数 / 关系数）在成功结束前都是 0。与补上关联一样，任务槽与状态都是**按进程**的（生产固定 `--workers 1`）：`idle` 同时覆盖「从没跑过」「进程重启过」和「这一格被另一种维护动作占着」，浏览器的有界轮询因此一定会收工。任何错误原文都不越过这个边界；事件只带计数（`kg_deleted` 带两个计数，`kg_delete_failed`）。删除笔记本会等待在跑的删除知识图谱（quiesce 腿 B），删除知识图谱遇到删库墓碑会立即失败。任务跑在与补上关联、重新合并相同的重活 worker 队列上，而不是删除笔记本的队列，因此等它的删库任务不可能反过来把它挡在自己身后。
...
- `GET /admin/users/{user_id}/asks/{job_id}` —— 单条提问详情；存活提问包含问题/答案/轨迹，未到期的删除后摘要只包含问题/状态/删除信息，`answer=null` 且轨迹为空。`error` 只对管理员携带失败原文，本人自助调用恒为空串。`job_id` 不属于 `user_id`、本人自助调用已无笔记本实时读权、或摘要已到期时均为 404。`job_id` 同时服务全局问答作业：响应带 `scope="global"`、参与库集合 `notebook_ids`，以及其中该用户当前仍可读的库名 `notebook_names`（已删除或已失权的库不在其中）；管理员保留完整审计记录，即使被查看用户之后失去了某个参与库的读权，详情仍可打开——和笔记本内提问同一条规则。本人自助读取则套用全局问答页面自己的读权规则：**参与集合里任何一个库不再可读，整条详情就是 404**，不会只隐藏那一个库、露出其余部分。
