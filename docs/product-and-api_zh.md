...
长度随用户数据增长的清单都分页：没有哪一行只能靠滚动一张无限长的页面才找得到，也没有哪一行
被静默截掉。分三种形态。
...
| 深度报告清单 | 20 | 换笔记本；打开的报告在后面的页上时，清单翻到它所在的页 |
| 待审核贡献队列 | 20 | 重新打开弹窗 |
| Knowhow 表卡片 | 24 | 切换状态筛选 |
| 群组页——群组清单、知识库、可添加的笔记本、成员、待审批队列、本人申请 | 20 | 切换群组（见[群组工作台](#群组工作台)） |
| 管理员用户总览——展开行里的笔记本 | 20 | 展开另一位用户 |
| `/dev/logs` 左栏——笔记本 | 20 | 换被查看用户；选中的笔记本翻到它所在的页 |

管理员用户表保留自己的「先排序后分页」控件，可选每页 20/50/100 行。

**刻意不分页：**问答会话正文及其轮次导航（一条连续的对话）；Knowhow 表的行网格与行管理清单
（行标题合并格跨多个物理行、行可以调整顺序，分页边界会把一组拆开）；选择器与筛选下拉框；以及
有已披露上限的清单——报告/会话公开分享页、回答里的结构化结果卡片、关系审核队列（按优先级排序的
一页，标题写明队列总数）、Knowhow 单元格历史（已披露的最近条数上限）、Agent 调用/观察样本。
...
都对完整的接口响应分页，每页 20 条。切换群组时各清单回到第一页；移除最后一页上唯一的一条后退回
上一页；选中的群组落在群组清单后面的页上时（深链、新建的群组、退出后改选），群组清单翻到它所在
的页。「添加知识库」里的勾选跨页保留。
