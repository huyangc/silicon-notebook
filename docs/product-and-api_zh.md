...
- **详情** —— 选中仍存活笔记本的提问显示完整问题、答案、引用与推理轨迹（复用既有 Ask 面板渲染件，只读）；删除后留存的提问只显示问题/状态摘要及删除、到期说明。失败的提问或报告对管理员显示失败原文，对本人自助读取显示固定失败文案。选中仍存活笔记本的报告显示状态、深度、耗时，以及带引用的只读报告正文（复用深度报告的 Markdown 渲染件，不加载附图、不跳转来源）；尚未生成正文时明确说明，删除后留存的报告只显示研究问题/状态摘要及删除、到期说明。选中来源显示显示名、解析状态与派生诊断；选中模型调用（仅「模型调用」视图内）复用既有 prompt/response 详情面板，未改动。
