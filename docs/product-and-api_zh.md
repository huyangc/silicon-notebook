...
| reason | 触发 | 界面短句 |
| --- | --- | --- |
| `model_sufficient` | 模型明确结束且自报充分，方面记录里没有已知未解决项 | 检索结束：每个必答方面都已找到支撑 |
| `model_partial` | 模型选择结束但仍有未确认/部分/冲突；或自报充分与方面记录矛盾 | 检索结束：仍有必答方面没有完整支撑 |
...
这份事实随证据一起走到 Ask 的单次/按节合成与报告的节撰写，合成 prompt 因此多出一段简短的**服务端事实**：结束原因、没解决的必答问题原文、没恢复的通道。它明确标为**不是知识条目、不带 `[k]` 编号、绝不可引用**，也明确说明它只表示「这次检索停在了哪里」，不是「库里没有这些内容」的证明——模型该做的是在正文里如实说明缺口，不是整段拒答。模型自己写的 `gap` 文本不进这个块。

**最终装配之后**，服务端用真正进入了合成 prompt 的证据身份复核每个方面，并把三个口径分开记账：`model_supported`（模型说这个方面有支撑）、`synthesis_admitted`（它绑的证据里至少有一条真的进了 prompt）、`answer_cited`（最终答案里真的出现了绑到那条证据的引用锚点）。三者**不是包含关系**：进入 prompt 不能证明答案引用正确，真实引用仍由既有的绑定与依据校验回答。一个方面绑过证据、而那些证据被预算或过滤**全部**移除时，它降为「未送达」，不再显示为「已支撑」；只被移除一部分的不算未送达（模型确实看见了其中一条）。这次复核不要求提前执行最终重排，也不为它再读一次库。后续补取解决了缺口可以在最终合成里使用，但不改写反思当时已经停止的历史事实。

这些事实沿既有的**私有**轨迹与报告章节路径存储：Ask 记在收尾那条跳过步与合成终步的诊断字段里，深度报告记在章节行上。界面复用已有的实时轨迹与完成后的合成说明，显示结束原因短句、「已处理 x/N 方面」，必要时补「y 项未送达」「z 条通道未恢复」（只报条数，内部动作名不上屏）；普通回答**不加**固定错误横幅——一次部分完成的检索不是错误。历史轨迹没有这些字段时逐字照常渲染。公开分享面不变：会话与报告的匿名投影是显式白名单，整个推理面（含轨迹）本来就不在其中，方面自评与结束事实一个字都不跨出匿名边界。
