...
- `GET .../concepts/{canonical_id}/detail?limit=&after=`——概念详情按 hub 簇成员做 keyset 分页（`limit` 默认与上限都是 200，`after` 传上一页最后一个成员的 `id` 作游标）；响应新增 `member_total`（口径与分页查询一致，剔除已废弃成员）与 `next_cursor`（还有下一页时为本页最后一个成员 `id`，否则为空）。`attached`（相关节点）与 `evidence`（出处）从「整簇」改为**按当前页成员**计算——每页只报该页成员的邻接节点与证据，翻完全部页仍能看到完整集合，是分页而非截断（少数概念对应上万成员的 hub 簇此前会整簇连同全部 payload/evidence 一次性返回）。该详情只供图谱视图的详情面板消费（「加载更多成员」翻页），不参与图谱本体渲染。`GET .../objects/{object_id}/context`
