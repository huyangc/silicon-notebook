...
将 `REASONING_ENUM_TOOLS_ENABLED` 设为 `false` 可完全禁用这三个工具：不构建地图、三个动作都不提供、无图早退恢复，逐步推理回到接入前的行为，零额外查询开销。
