...
这个顶层复选框范围是当前 notebook 的硬上限，也是检索范围的**唯一**来源。模型既不提议也不收窄或扩大它：语料盲意图规划器不输出任何来源身份，run 内也没有任何动作能改变哪些来源在范围内。挂载参考库仍是独立参与者。收窄复选框时，无法在遍历前安全应用当前来源谓词的持久化通道（当前 notebook 的全图/PPR/关系扩展、精确章节查找与报告整库画像）会被跳过；按来源限定的 chunk、元素和 KG 直接检索仍正常执行，base 库 KG 种子也可直接映射回 base 原文，不经过组合全图遍历。
