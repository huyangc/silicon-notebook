...
`POST /ask` 按 `mode` 分派——注册表 `backend/app/services/ask_modes.py` 是唯一真源（默认 `chunk`）。`chunk` 基线与 `reasoning` 的 `search_chunks` 共用的那条原文段落通道现在是**参与集**口径（当前笔记本 + 本次勾选的参考库），挂载参考库自己的段落因此会进入回答，每条候选带自己的归属库。开关是 `CHUNK_FEDERATION_ENABLED`（默认 true；false 是唯一的回退路径，行为回到只召回当前库原文），上界是 `CHUNK_FEDERATION_MAX_PARTICIPANTS`（默认 8；超出时按确定的挂载顺序截断并发一条内容无关的 `chunk_federation_truncated` 事件，不静默）。非当前笔记本的参与库只贡献它当前可见的来源——隐藏的 Memory/Knowhow 投影结构性不参与这条跨库通道；非当前库的大库只借用已常驻的索引、不冷加载，没有暖索引时退回有界词法召回。可选 KG overlay / PPR 在此之上再加入 federated KG 与 base-backed chunk；`reasoning` 走 federated KG。`federated_retrieve()` 的知识对象命中不改 score，只在完全平局时以 `base` 为第二排序键；`federated_retrieve_relations()` 的关系命中仍只按 score 排序。这些排序信号不进入接地阈值。
