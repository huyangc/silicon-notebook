...
每个方面的状态是 `unknown` / `partial` / `supported` / `conflicting` 之一，由模型在**同一次 reflect** 的附加 `assessment` 里给出——不新增任何模型调用。模型没报告的方面保留现状（省略是「这一轮没说它」，不是「删掉它」，必答清单只能由用户的契约改变）；报告了的方面则**全量替换**它的支撑判断，不像大纲证据键那样取并集，否则一次错误的「已支撑」再也撤不回来。服务端只确认三件事：方面 id 合法、引用的证据键是服务端签发过且**曾真实展示**给模型的身份（候选池里的细粒度证据键，或下面说的集合完整性键）、**未完整**枚举的集合身份不能冒充细粒度证据；剔除非法键之后没有支撑的项不得保持「已支撑」。语义支撑始终标为模型判断，不新增 grounded 分数，也不改动 evidence_level 阈值。

**收尾那一轮必须自评。** 判据是**收尾载荷**本身：`next_action=answer`，或任何带 `sufficient=true` 的动作（`update_outline` 这类不产证据的动作与 `sufficient=true` 并存是合法的，而穷尽档最常见的收尾恰恰是「最后一批绑定补上、同一轮宣布够了」）。这两种载荷都会当场终止整次检索，而那一刻方面账就是这次检索唯一的成色记录。这样一份载荷若一个方面都没自评，服务端**整轮退回**（一条零 I/O 的观察，原因 `missing_assessment`，与其它 skip 走同一份步数/无进展/熔断记账），并在下一轮的服务器状态块里逐个列出方面 id 追问；同一轮如果带着 `update_outline` 的载荷，那份大纲**先应用再退回**，模型这一轮真正做成的绑定不会因为追问而丢掉。追问句只挂被退回的**下一轮**那一次，不是此后每一轮。追问**最多一次**，第二次仍然不给就照原样收尾，把还没被判断过的方面记为「问过了、模型不给」（快照字段 `assessment_omitted`，收尾那条跳过步上的计数是 `aspects_assessment_omitted`）——它与「模型根本没走到收尾那一步」在放量评估里因此可以分开数。**步数已经用尽的那一轮不退回**：追问的全部价值在于下一轮补上，没有下一轮时退回只会把模型自己的判读改写成「没步数了」，所以直接按「第二次沉默」接受收尾并记 `assessment_omitted`。
