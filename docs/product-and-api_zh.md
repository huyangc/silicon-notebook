...
默认态澄清：`SOURCE_PARTITIONED_GRAPH_ARTIFACTS_ENABLED` 与 `SOURCE_PARTITIONED_PPR_ENABLED` 现均默认开启且可独立回滚；上文“默认关闭”仅描述已结束的灰度阶段。reader 仍只服务不可见的统一内部激活层，不可用时 fail closed 回 B。
