...
**公开会话对等。** `PublicImage.reference_keys` 只复用绑定 token 别名图片的既有公开 `k1`/位置编号，不暴露任何内部 id。匿名 `/c/{token}` 页据此沿用站内 Ask 的正文块落点、只显示图片、可信度标签、caption 仅作 `alt`、同图只在第一次出现以及页内放大规则；报告公开分享白名单仍不带图。
...
**Ask 前端的落点与展示。** 图片直接穿插在回答正文里，位于绑定引用所在的最小完整 Markdown 块之后：段落/标题写完再放，列表项和引用块放在各自内部，表格则等整张表结束再放，绝不从句子中间切开。复合引用按正文引用顺序合并成一个图片区；同一资产在后文再次被引用时只在第一次命中处展示。区块只显示鉴权图片与紧凑的「本段附图 [n] / 模型未直接读取图片」可信度标签：caption/description 继续用于检索与图片无障碍 `alt`，但不会在正文区块或 Ask 引用浮层里重复显示成文字。点击正文或引用浮层里的图片会打开 root-modal coordinator 管理的页内预览（关闭按钮、点遮罩、Escape、焦点返回；桌面可拖动/缩放，移动端接近全屏），不会跳去来源详情。MinerU 图片只要摄取时已保存字节并给出 `asset_id`，就走完全相同的展示；只有文字描述、没有资产的 image 元素仍无图可渲染。本期深度报告继续沿用既有引用详情图片区，报告正文内联图片明确留到第二期。正文区块与详情都复用既有鉴权、按视口懒加载的 `AuthedImage`。
