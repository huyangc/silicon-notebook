...
**集合地图（先计数后取行）。** 在这些工具真正值得调用之前，每个 run 只构建一次一行文本，形如 `[Collections in scope] elements: formula 12 (3 sources), table 5, image 0, code_block 0 | KG objects: concept 1234, claim 567, formula 89, procedure 0 | knowhow tables: 2 | sources: 7 (current notebook: 2)`（每个白名单 kind/type 恒在场，缺省即 0，让模型能分清「数出来是 0」与「压根没上报」；硬顶 600 字符），注入 plan/reflect 上下文，作用域为 active notebook 加其当前有效的挂载参考库。行尾的 `sources` 是用户可见的文档数（与来源页签、文档数量上限同一口径），且与来源清单的分母出自同一处，不会出现「地图说 7、清单列 8」。它是**唯一渲染两遍**的计数——先联邦总数，再用括号给出当前笔记本的份额——因为来源清单是唯一带范围参数（`enumerate.scope`）的集合，模型要在两个范围之间选就必须同时看见两个数；括号里那个数无条件出现，所以「两个数恰好相等」不会与「这一版根本没报第二个数」长得一样。联邦 `sources` 计数在服务端还有第二个消费者：它就是目录规模守卫拿来与本轮剩余行数比较的那个分母（见「预算与续跑」），所以地图**对象**——不只是渲染出来的那一行——会留到本轮结束。这不多花一次查询：同一次构建，只是渲染完不再丢掉。地图压根没建成时守卫不触发——「不知道有多大」不等于「已知很大」。计数是索引辅助的，不携带来源标题、文件名或摘录文本——它是 prompt 脚手架，不是证据。地图统计的每源集合与枚举执行器实际遍历的源集合按构造逐字一致，因此不会出现「地图说 12、清单只给 8」的假部分。
