...
关键词检索，与语义腿共用同一个共享执行器、公平窗口、阶段截止与逐库预算，各自带该库冻结的来源
天花板；没有可见来源的库不发查询。每个库沿用单库的召回窗（`CHUNK_RECALL`）；合并时按各库自己的
关键词得分排序后轮转交错（每库先出第 1 名，再出第 2 名……），同一段原文只保留先出现者，总数以
`GLOBAL_ASK_CANDIDATE_LIMIT` 封顶，一个库不能靠数量占满。某库关键词腿超时或失败只意味着该库这一路
没有结果：不进入「已检索/已跳过」回执（覆盖口径只由语义腿决定），超时不上失败横幅。每次运行发一条
