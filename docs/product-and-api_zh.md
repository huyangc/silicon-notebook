...
**每 run 一行的闭集投影为此新增九列**（`app/domain/reasoning_trace_stats.py` 的 `RUN_PROJECTION_KEYS`；这是**离线分析面**，不是任何 API 响应字段）：`run_wall_ms` 一次 run 的墙钟（轨迹里没有这件事实——按步耗时求和漏掉排队与步间空隙——所以只有 rig 手上有，只读导出恒缺）；`model_calls_real` 这次 run **真正发出去**的模型请求数（各 reflect 步之和，与「日志行数」不是同一个数：`response_format`/`stream_options` 被拒后的重建会多发一次请求却不另写日志行）；`attempts_observed` 上面那个数全不全（每一条 reflect 步都带 ⇒ 是；只带了一部分 ⇒ 否，此时那个和只是下界；一条都没带 ⇒ 缺）；`optimization` 这条 run **声明**跑的是哪一格 `REASONING_REFLECT_OPTIMIZATION`（声明而不是从轨迹反推，所以只读导出恒 unknown）；`context_chars` 最后一轮那五个块的字符数加整条消息的字节数（短码 `s`/`c`/`k`/`d`/`t`/`bytes_total`；⚠ 最后那一格的单位是**字节**，它既不是前五格之和、也不该拿去和它们比大小）；`prefix_bytes_median`/`prefix_bytes_min`/`prefix_turns` 逐轮公共前缀的三格聚合（逐轮细节按设计只留在 rig 的 per-call 表里）；`response_chars_total` 各 reflect 步交还给调用方的正文字符数之和。**没量到 = 缺值，不是 0**：旧轨迹、测量关、以及 `legacy` 与 `off`（测量关）的行里这九列一律缺值，而 `model_calls_real`/`response_chars_total` 这两列在**任一** reflect 步缺观测时整列 unknown，不按现有的几步求和凑一个偏低的数。这份闭集的真源就是 `backend/app/domain/reasoning_trace_stats.py`——**后续修正**：2026-09-08 那份 T0 轨迹分析设计稿 §3 列的键表**早于**这九列（也早于 `context_chars` 里那个字节口径的 `bytes_total` 短码），它记的是当天的决定，不要拿它当闭集清单读。
