...
共享答案风格提示遵守用户要求的输出粒度。标题清单保留每条匹配来源及引用（包括同名的独立来源记录），不擅自追加摘要或配置细节，必要的覆盖披露仍保留。概述与机制比较保留主要推理、限定条件及用户所需细节。数值比较保持指标、单位、版本、配置和评测条件的对应，逐对核对方向，明确混合结果与例外；来源的定性结论不能覆盖与其冲突的表格数值。共享/独立组件、架构/数值变化、理论可组合/实际兼容已验证也须区分。这些要求进入既有合成调用，不增加评估轮次、不降低输出预算；模型是否遵守及事实质量仍须实测。
