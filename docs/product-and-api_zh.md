...
当前选择同时传入不读语料的问答意图预检、问答执行和新建深度报告。来源维度约束当前 notebook 的内容块、来源元素、知识证据与关系、图路径/PPR 输出以及报告检索；来源范围被**收窄**时，隐藏的 Memory/Knowhow 投影证据也不参与，因为这些内部来源没有面向用户的复选框。**未收窄的请求则保留这份证据**：正因为它们没有复选框，永远进不了提交的选择，所以入口层在冻结上限时把当时存在的隐藏投影来源与全部可见来源一并冻进去，只有真收窄才把它们排除。两种情形下的冻结保证不变——校验之后新增的来源，无论可见还是隐藏，都在上限之外；可见与隐藏两半出自冻结本来就要做的那一次读取，不多付往返。参考库维度则把未勾选的参考库从集合清单与集合地图、联邦检索、共提/社区扩展、图漫游、`follow_chain` 与证据装配中移除。
