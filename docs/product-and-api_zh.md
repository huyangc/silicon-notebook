...
**收尾那一轮必须自评。** 模型选 `answer` 且 `sufficient=true`，字面意思就是「每个必答方面都已经有支撑」，而方面账是这句话唯一的落点。这样一份载荷若一个方面都没自评，服务端**整轮退回**（一条零 I/O 的观察，原因 `missing_assessment`，与其它 skip 走同一份步数/无进展/熔断记账），并在下一轮的服务器状态块里逐个列出方面 id 追问；追问**最多一次**，第二次仍然不给就照原样收尾，把还没被判断过的方面记为「问过了、模型不给」（快照字段 `assessment_omitted`，收尾那条跳过步上的计数是 `aspects_assessment_omitted`）——它与「模型根本没走到收尾那一步」在放量评估里因此可以分开数。`sufficient=false` 的部分收尾**不强制**：那句话本来就是「我知道还差东西」，与一张未确认的方面账并不矛盾。

**目录题的支撑：集合完整性键。** 枚举结果按合同不进相关性候选池，条目 id 也不对模型可见，所以「本库有哪些文档」这一类方面要的支撑不是任何一条细粒度证据，而是「这个集合已经被完整列出」这件事本身。服务端为**本 run 内覆盖率报告完整**的每一条枚举链签发一个确定性的集合身份键（`enum:` 前缀 + 集合名/子类型/限定来源/范围，与续跑身份一一对应，不含任何用户内容），并在证据卡区位单独列给模型看；只有真的列出来过的键才能写进 `assessment` 的 `evidence_keys`。没列完（还能续、或枚举期间资料变了）的链**不签发**任何键，模型自己拼一个同样被剔除——合法集是服务端算出的那一份，不是按前缀放行。集合键不参与「送达」复核：它不走证据预算那条路，也不产生 `[k]` 锚点。
