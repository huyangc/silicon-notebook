...
**Reflect v2 模型协议（设计稿 2026-09-07 §5；默认关闭，见 `REASONING_REFLECT_V2_ENABLED`）。** 动作空间仍是同样的 13 个 id，传输仍是一次 JSON 对话补全；变的是每一轮都由一次纯内存的能力投影生成。某个动作若每 run 配额已耗尽、被调用方策略关掉、在缩小后的来源范围下不安全、无图时没有意义，或（对 `follow_chain` 而言）当前候选里没有任何合法起点，它就会**同时**从模型看到的动作清单与解析白名单里消失，并附一条有界的不可用原因，好让模型换通道而不是反复请求一条死路；它的参数分支一并消失。响应 schema 的 `next_action` 枚举则刻意始终列出全部 13 个可识别 id：通用形状闸把枚举提示当闭集，按可用性收窄会让「可识别但本轮不可用」的选择在**传输层**就被拒，而不是变成下面那条观察——重试耗尽后终止的是整个循环，而不只是花掉一轮。响应改用单一 `arguments` 对象取代逐动作的分支字段，所选动作的字段按类型校验：`sufficient` 必须是真布尔值，`arguments` 必须是对象，`next_action` 必须落在 schema 枚举出的 13 个 id 之内。这三类畸形在生产里从不会走到这个解析层：共享的传输层形状闸（`validate_model_json_shape`/`_validate_repaired_shape`）先以 `invalid_boolean`/`invalid_type`/`invalid_enum` 拒绝，重试耗尽后按既有 fail-open/fail-closed 合同终止整个循环——不是下面那条记账里的一次观察。解析层里对应的三个分支（`invalid_sufficient`、`invalid_arguments_object`、`unknown_action`）仅作为纵深防御保留，只有测试替身或直接构造载荷的 fail_closed 调用方才可能触达。检索动作携带 `sufficient=true` 是矛盾，整份决定被拒（`update_outline` 与 `consult_memory` 不产生证据，因此可以合法地与它并存）；`answer` 与 `consult_memory` 必须提交空的 `arguments` 对象，给它们附上检索参数记 `unexpected_arguments`，而不是静默忽略。可识别但本轮不可用的动作、缺失参数或参数值未通过校验（唯一一处刻意的例外是 `enumerate.scope`，见下面枚举工具那一节）、上述矛盾、以及多余的 `arguments`，都会被记成一条**零工具 I/O** 的观察，并走与其它 skip 完全相同的步数 / 无进展 / 熔断记账——所以反复提交非法动作规避不了空转熔断；预算尚有余量时循环继续。当投影只剩 `answer` 一个可执行动作时，服务端直接以稳定原因 `no_executable_action` 收尾，不再花一次模型调用去请求一个只可能回 answer 的决定，也不伪造一条「模型判定证据充分」的反思步。provider/JSON 失败沿用既有的 fail-open / fail-closed 合同，绝不被解释成模型认为证据够了；取消与阶段边界异常照常上抛。这是内部的模型决策协议：不给任何 Ask 或 MCP 请求新增字段。Knowhow 智能补全显式留在 legacy 协议上。

**Reflect v2 的预算（设计稿 2026-09-07 §4）。** 一轮反思可用的证据预算按档位走，但放在自己的部署映射里而不是上面这张表里——反思与合成要的信息粒度不同，两者不能互相按比例换算：
...
另有两个与档位无关、也**不是部署参数**的 v2 协议常量，约束的是模型在同一次 reflect 里回传的方面自评载荷：一个方面最多带 **8** 个证据键（`REFLECT_ASPECT_MAX_EVIDENCE_KEYS`），一个方面的 `gap`（模型写的「还缺什么」）最多 **240** 字符（`REFLECT_ASPECT_GAP_MAX_CHARS`）。它们没有环境变量，因为改动它们就是改协议本身而不是调预算。⚠ 这两个上限**只针对模型生成的内部载荷**：超限的处理是**按方面拒绝**——那一个方面保留旧状态、原因在下一轮披露给模型，而同一份载荷里其它方面照常落账，**这一轮的检索动作也照常执行**（这一轮被拒的方面**合起来**记成一条零 I/O 的 `skip` 步——原因码取第一条被拒的那个，被拒方面数与原因分布写在这条步的明细里；它刻意不折成动作观察：那次动作自己已经有一行了）。带着**已知方面 id** 的行级字段错误（`evidence_keys` 不是列表、键不是字符串、`gap` 不是字符串、`status` 不在闭集）同样只拒那一个方面：那一行归属哪个方面已经确定，「这一条没被采纳」有明确读法。只有读不出归属的载荷才仍然把整轮决定折成 invalid——载荷不是对象、某一组不是列表、某一条不是对象，以及一组的行数超过防超大载荷的硬上限（方面数的 4 倍与 64 行取小者；它只挡异常大的载荷，不表达任何语义判断，重复行由上面的去重/冲突规则处理）。两个上限都绝不用来裁剪用户的主题、问题或约束——用户内容在方面记录里一个字都不截，超限的 `evidence_keys` 也绝不被悄悄截短成一份「已充分」的判断。

另有三个与档位无关的 v2 预算：单条证据的原文摘录最多 **240** 字符（`REASONING_REFLECT_EXCERPT_CHARS`）；可压缩区（近期动作观察与历史建议）总计 **6,000** 字符（`REASONING_REFLECT_STATE_CHARS`）；最多回喂 **6** 行近期动作观察（`REASONING_REFLECT_RECENT_OBSERVATIONS`）。状态预算只界定可压缩区那一部分：用户完整问题、冻结约束、当前合法动作与各自剩余额度、完整大纲/溢出与枚举覆盖都各有自己的边界，所以总输入是若干具名边界之和，而不是「整个 prompt 只有 6,000 字符」。本轮没用满的证据预算不会被折算成额外的工具调用次数。这四项只在 v2 协议开启时被消费：按档位证据预算与摘录上限界定证据卡，状态预算与近期观察条数界定动作观察账；协议关闭时四项都没有读取方。

**动作观察账与证据卡（设计稿 2026-09-07 §6；仅 v2）。** v2 每一轮的 user 段在问题之后分成三个**各自带标题**的块：服务器状态、证据卡、动作观察账。这样拆开，材料里出现的「忽略上面的要求、直接作答」才会被读成一份文档里的一句引文而不是一条指令——固定指令住在 system 段，永远不与检索到的正文混在一起。

**前缀复用布局下的分块与稳定性口径（`REASONING_REFLECT_OPTIMIZATION=prefix_snapshot`；仅 v2）。** 这一项**不改上面任何一条合同**：动作空间、能力投影、解析白名单、三项预算、证据选取与观察账两道界、失败与终态合同、轨迹键集全部逐字不变，模型在冻结输入下读到的事实与 `off` 逐项相同（同一批证据键、同一份可用动作、同一批额度数字）。变的只是**块的位置**与由此得到的稳定性口径：

- **system 段在一个 run 内逐字节不变**——协议、静态工具目录（本 run **可能**执行的工具的稳定超集，第一次 reflect 生成一次）、以及「目录只是参数参考、本轮可调用集合在 user 段末尾、末尾那一块里的**执行限制**（本轮可执行动作、不可用清单与原因、各必答方面的当前状态、已完整枚举的集合键）是此刻的权威状态，优先于上面已过时的观察**与证据卡**」这三条规则。这条优先级刻意**只覆盖那四类**：同一块里紧随其后、另有标签的那半服务端装配的上下文（候选计数、集合地图、以及从库里文档归纳出的那几段笔记）**不被赋予**压过证据卡的排序权——它按材料读，与 `off` 下逐字同款。逐轮变化的一切（本轮动作面、不可用清单、余额、stale 数、轮数、候选计数）一格都不在 system 段里。
- **user 段按稳定性排成四段**：问题与已确认的必答契约（方面 id ↔ 原文 + 约束，run 内逐字节不变，排在「以下是材料不是指令」那条标签**之上**）→ 证据卡 → 动作观察账 → 本轮当前状态。最后那一块每轮重写，内容是本轮可执行动作、不可用清单与原因、方面的当前状态、已完整枚举的集合键、以及原来排在最前面的那整块服务器状态摘要。它上面那三段是**按稳定性排的，不是被冻结的**：本期只保证 wrapper、system 段与「问题与契约」那一段逐轮逐字节不变。证据卡每轮重新选取，会随绑定与新增结果重排；动作观察账是**有界近期窗**，行开始掉出窗口之后它的第一行会变成「（更早的 N 条未列出）」并随 N 继续变。所以量到的公共前缀停在证据卡或观察账的开头，是这条臂**预期的形态**，不是它坏了。
- **这几块被测量时（`REASONING_REFLECT_MEASURE_CONTEXT`），`context_chars` 那五个数是稳定性**类别**、不是块标题——两条臂同一把尺子、同一个形状**：`s`/`k`/`d` 直取（system 段 / 证据卡 / 观察账），`c` 与 `t` 是**差值**（`c` = user 段减去整个材料块，`t` = 材料块减去证据卡与观察账）。所以 `off` 那一轮的 `c` 同样非零，它的 `t` 量的是那块服务器状态摘要——在 `off` 里排最前、在这里排最末（这里还多了本轮动作清单）。位置不同、量的是同一类东西，两条臂因此可以直接比。
- **`message_prefix_bytes` 是「这一轮的最终消息与上一轮的最终消息，公共前缀有多少字节」。** 量的是那一轮真正交给上游的**整条序列**——含服务端 wrapper 与序列化帧开销——的 UTF-8 字节，不是任何单独一块的长度；同一轮里若因输出预算打满而重试第二次，两次尝试都与**上一轮**比，不互相比。首轮如实**缺这个键**（那时没有可比的上一轮），读作 unknown 而不是 0。同一轮 detail 里那个字节总数与它**同口径**（都是这一串字节），所以「这一轮 N 字节里有多少落进了公共前缀」那道除法两边同为字节。它是一个**客户端结构指标**：它说的是前缀有多稳，**不是**上游服务复用了多少——那件事上游不下发，本指标也不据此推算，所以这一族键里没有任何一个以「命中」或命中率命名。（上游自己报的那一格是 `usage.cached_tokens`，缺失即缺失、绝不写 0；而 LLM 调用日志里那个 `status="cache_hit"` 说的是**本进程自己**的整答复缓存，与前缀复用是两回事——reflect 压根不参与那个缓存，它是 opt-in 的、判据是调用方传了响应校验器，而反思调用不传。）
- **静态目录是超集，不授予任何调用资格。** 目录一个 run 只定型一次，所以某个工具的额度用光、或它依赖的条件不再成立之后，它仍然写在目录里；模型硬选一次时按既有的 `unavailable_action:<原因>` 记一条零 I/O 的观察、循环继续，与 `off` 下同一套原因词表。收窄与放宽一律由每轮的当前状态如实说明。

**增量上下文的分块与稳定性口径（`REASONING_REFLECT_OPTIMIZATION=prefix_delta`；仅 v2）。** 上面那一段**逐条仍然成立**——消息形状与 `prefix_snapshot` **完全相同**（`system(S)` + 一条 `user(C+K+D+T)`，两条臂逐块同形，`test_delta_layout_takes_the_same_prefix_message_shape`），动作空间、能力投影、解析白名单、失败与终态合同、轨迹键集同样一格不变。差别只在 K/D 两块的**内容判据**：

- **逐轮逐字节不变的是 wrapper、system 段、「问题与契约」那一段，以及**上文里**全部已经发出的 D**。** 这比 `prefix_snapshot` 多了最后一样：D 是**只追加**的，一轮至多一块，已经发出去的块不会被改写、不会被重排、不会消失（`test_delta_only_appends_and_never_rewrites_an_earlier_block`）。K 那一块在**阶段内**也不变——它只在两个时刻变：一次重建（证据池或历史池的**累计**用量将被下一块 D 顶破），或本 run 不可逆地回退。那道重建判据算的是**完整的待追加块**，口径与累计账完全一致：块头与卡片节计证据池，观察行与「已接受的方面更新」那一句计历史池——所以只带观察行的那些轮同样被判，而不是无条件把块头追加进去。**刻意留一条越界例外**：没有候选新卡、而块头（或那句 note）在重建之后仍装不下时，那一块照发——观察行是本轮那次动作在上下文里的唯一记录；记账如实记成超出，于是下一轮必然重建。所以量到的公共前缀会覆盖 S、C、K 与上一轮之前的每一块 D，止于本轮新增那一块的开头；这是这条臂**预期的形态**。
- **一块 D 的内部结构**：块头一句（说明这一块是追加的、同 key 的新卡是同一条证据的新摘录、观察行按上方观察账的读法读）之后是三节，缺哪节不出哪节——本轮新展开的证据卡（含同 key 的补充卡，排在新卡**之后**）→ 本轮的动作观察行 → 上一轮服务端已接受的方面更新那一句（标明「模型判断，非原文」）。方面的**当前状态**整块仍在末尾那块 T，不进 D：那份渲染带着两处消费副作用（追问句与未采纳披露），搬进一块再也不重渲染的块里就永远消费不掉了。三节全空 ⇒ 这一轮**不发块**（不发一个只有标题的空块）。
- **证据卡展示后保持不变**：一张卡第一次真的发出去时字节被冻结，重建与后续增量都从那张表取字节，不按本轮检索词重算摘录。摘录以后升级不就地改写，而是**追加**一张 `key=` 逐字相同、标着版本的补充卡，旧卡仍然有效、绑定用同一个键（`test_supplement_card_repeats_the_pool_key_verbatim`）；同一段摘录不会被追加第二次（`test_supplement_is_not_appended_for_an_excerpt_already_sent`）。「仅在池中、从未展示」的材料仍然不获得绑定资格，被预算或卡数上限挤出这一块的候选一个都不登记（`test_delta_still_rejects_keys_the_model_never_saw`）。
- **回退之后**（拍板 Q4，不可逆）本 run 剩余轮改用 `prefix_snapshot` 的有界证据选择，**已经发出的 D 原样留在上文**、S 里那几句解释 D 怎么读的规则也照旧发（`test_delta_static_prompt_keeps_the_rules_while_any_block_is_still_sent`）——抽掉模型已经读过的上文，或留着块却撤掉读法，两种都比多付几千字符更糟。回退不清空候选池、不撤销任何绑定资格、不重置配额、不多一次模型调用或轨迹步。⚠ 回退之后每一轮的 K 都按「**档位证据池 − 保留 D 已占的证据半**」构造，而保留 D 里**已经可见的那些键**被排除在 K 的候选之外——**全部三档**，来源多样性那一档也在内：那一档的键序是在选取内部从池子里算出来的，只滤调用方手上那两份键序对它一点作用都没有（近期观察窗同理先减去保留 D 占的历史半）：所以一条消息的证据合计仍然 ≤ 档位证据池，同一条证据也不会以两份都不带版本标记的写法出现两遍（`test_delta_keeps_the_cumulative_evidence_budget_across_blocks` 的回退轮上界、`test_delta_fallback_turn_keeps_its_own_cards_out_of_the_kept_blocks`）。代价如实：保留 D 把池子吃得多时，回退之后的 K 可以是空的——那时模型手里那几块 D 正是它已经读过的那批证据。
- **`context_chars` 的五格口径两条臂同形，`d` 是位置口径**：`d` = 观察账那一块 + 本轮消息里**全部**增量块（不只是最新那一块），`k` 仍然只是 K 的证据卡块。所以恒等式 `s + c + k + d + t` = 两条消息正文字符数之和在这条臂上照旧成立（`test_delta_measures_the_ledger_and_every_block_in_the_d_slot`）。池账（K + 各 D 的**累计**用量，用来决定何时重建）是服务端内部判据，**不是**这五格里的任何一个——两条臂能直接比大小的前提就是这五格按**位置**定义。
- **`message_prefix_bytes` 在这条臂上的期望形状**：口径一个字不变（仍是「本轮最终消息与上一轮最终消息的公共前缀字节数」，首轮如实缺值）。预期读数是「随轮数增长、在重建那一轮明显回落」——不重建的轮里公共前缀覆盖到上一轮那块 D 的末尾，重建那一轮 K 被换掉、已发出的 D 被清空，前缀塌回 S+C 的边界。**回落不是缺陷**，它正是重建这件事在字节上的样子；`context_rebuilds` 那一列与它是同一批事实的两种呈现。它仍然是**客户端结构指标**，不是上游复用了多少，这一族键里没有任何一个以「命中」或命中率命名。

关闭态（`off`，以及 v2 总闸关闭与 Knowhow 智能补全）走原来那一份布局与原来那个顺序，逐字节不变。

**每 run 一行的闭集投影为此新增十四列**（`app/domain/reasoning_trace_stats.py` 的 `RUN_PROJECTION_KEYS`；这是**离线分析面**，不是任何 API 响应字段）：`run_wall_ms` 一次 run 的墙钟（轨迹里没有这件事实——按步耗时求和漏掉排队与步间空隙——所以只有 rig 手上有，只读导出恒缺）；`model_calls_real` 这次 run **真正发出去**的模型请求数（各 reflect 步之和，与「日志行数」不是同一个数：`response_format`/`stream_options` 被拒后的重建会多发一次请求却不另写日志行）；`attempts_observed` 上面那个数全不全（每一条 reflect 步都带 ⇒ 是；只带了一部分 ⇒ 否，此时那个和只是下界；一条都没带 ⇒ 缺）；`optimization` 这条 run **声明**跑的是哪一格 `REASONING_REFLECT_OPTIMIZATION`（声明而不是从轨迹反推，所以只读导出恒 unknown）；`context_chars` 最后一轮那五个块的字符数加整条消息的字节数（短码 `s`/`c`/`k`/`d`/`t`/`bytes_total`；⚠ 最后那一格的单位是**字节**，它既不是前五格之和、也不该拿去和它们比大小）；`prefix_bytes_median`/`prefix_bytes_min`/`prefix_turns` 逐轮公共前缀的三格聚合（逐轮细节按设计只留在 rig 的 per-call 表里）；`response_chars_total` 各 reflect 步交还给调用方的正文字符数之和。接着两列是 `prefix_delta` 专属的**行为事实**（PR-3）：`context_rebuilds` 这次 run 一共**做过几次**重建证据卡快照 K 的动作——**含最后那次被丢弃的**（触发回退的那一轮先压了一版 K、再判定仍装不下，于是整版丢掉：那次全池重选的开销真的付了，所以它计数；但模型看过的 K 版本数因此比这一列少一格，读这一列时不要把它读成「消息里换过几版 K」）——取各 reflect 步的**最大值**（max-over-present，同 `stale` 的口径，不是「任一步缺 ⇒ 整列 unknown」那一种：写侧是 run 级单调累计的计数，取最大值才是「总共重建了几次」，喂逐轮布尔进去会让一条重建过五次的 run 在表上显示成 1）；`context_fallback` 这次 run 有没有不可逆地退回有界证据选择——**任一** reflect 步为真 ⇒ 整列真（回退不可逆，「发生过一次」与「一直在发生」是同一件事）。⚠ 这两列 **`0` / `false` 与缺值必须分得开**：`context_rebuilds == 0` 是「量到了，答案是零次」，`None` 是「没量」（非 `prefix_delta` 臂，或旧轨迹）——`test_context_rebuilds_zero_is_distinct_from_unknown` 钉住这件事。同一批写侧还有第三个键 `delta_blocks`（这一条消息里有几块 D），它**已登记进 reflect 步的稀疏键集但刻意不出顶层列**：它不是累计量（重建会清空已发出的块），一个 run 一格装不下它，要看它就读那几步的 detail。最后三列是 PR-4 加的**自评读数**，与「跑哪一格」无关、**v2 四臂通写**（`off` / `prefix_snapshot` / `prefix_delta` 下模型仍每轮重述全量，这几列同样有意义；只在 `prefix_delta_lean` 写的话，D↔L 的配对表上它们恒缺一半）：`assessment_rows_total` 各 reflect 步「本轮**实际落进方面账**的自评行数」之和（不是载荷里写了几行——逐方面被拒的行不算，整份形状越界被折的那一轮记 0）；`assessment_observed` 上面那个和全不全（每一条 reflect 步都带 ⇒ 是；只带了一部分 ⇒ 否；一条都没带 ⇒ 缺）；`aspects_unassessed` 终态那条跳过步上的「模型一次都没判断过的方面数」。⚠ 前两列是一对，**求和口径是 sum-over-present**（同 `context_rebuilds` 那种 max-over-present 的先例，不是 `model_calls_real` 那种「任一步缺 ⇒ 整列 unknown」）：provider 故障的 fail-open 轮，以及服务端把决定折成伪动作的那些轮（首次抖动降级、解析失败、参数越界），压根没有模型的自评意图可言，那些 reflect 步**两个 detail 键都不写**——沿用全或无口径会让一次偶发抖动把整条 run 读成 unknown，所以「缺了几轮」由 `assessment_observed` 单独披露，而 `assessment_observed=False` 时那个和是**真实**的部分和、不是下界（这一点与 `attempts_observed` 不同）。同一批写侧的第四个键 `assessment_absent`（这一份载荷压根没带 `assessment`）**已登记进 reflect 步的稀疏键集但刻意不出顶层列**：它是逐轮的布尔，一个 run 一格装不下，而顶层要的那件事已经由上面两列表达了；要按轮看就读那几步的 detail。⚠ 这三列 **`0` 与缺值必须分得开**：`assessment_rows_total == 0`（量到了、这条 run 一行都没落账）与 `None`（没量）不是一件事，`aspects_unassessed == 0`（问过了、没有遗漏）与缺席（没量到 / legacy 没有方面账 / 没走到终态）同理——`test_assessment_rows_total_zero_is_distinct_from_unknown`、`test_aspects_unassessed_reads_the_termination_skip_step_not_any_step` 钉住这两件事。**没量到 = 缺值，不是 0**：旧轨迹以及 `legacy` / `off` / `prefix_snapshot` 的行里 PR-2/PR-3 那十一列一律缺值（前九列还要求测量开着；`context_rebuilds`/`context_fallback` 与测量开关**无关**、只与臂有关——两条 delta 臂的行即使测量关着也带着它们，见部署文档 `REASONING_REFLECT_MEASURE_CONTEXT` 那条已登记偏离），而 PR-4 那三列里前两列的门是测量开关（四臂通写，关着就缺）、`aspects_unassessed` 与测量开关无关、v2 四臂无条件写（拍板 Q10 的已登记偏离，`legacy` 没有方面账因此恒缺）；`model_calls_real`/`response_chars_total` 这两列在**任一** reflect 步缺观测时整列 unknown，不按现有的几步求和凑一个偏低的数。这份闭集的真源就是 `backend/app/domain/reasoning_trace_stats.py`——**后续修正**：2026-09-08 那份 T0 轨迹分析设计稿 §3 列的键表**早于**这十四列（也早于 `context_chars` 里那个字节口径的 `bytes_total` 短码），它记的是当天的决定，不要拿它当闭集清单读。**PR-5(T4 实验通道)投影列本期一列未加**:设计 §10 的五条采用门全部从这十四列 + 既有 T0 列读出,新增的读法落在 `scripts/analyze_reasoning_trace.py` 的参数上(`--baseline-arm`/`--pair-rows`/`--key-set`),不在数据形状上。附带一句:`optimization_pair_table` 的基线臂现在可配(`--baseline-arm`,默认仍是 `off`),不再硬编码。

动作观察账记录服务端**实际执行**了什么，一次动作一行：序号、这一步来自首轮确定性播种还是模型主动选择的动作、动作 id、规范化请求身份（决定两次请求算不算同一次的那些参数：types/prefer、关系类型与方向、枚举的集合与子类型）、模型那一轮自己写下的一句目的、执行状态、返回条数与真正新增条数、已知截断、稳定原因码、剩余步数。状态是 `success`/`empty`/`duplicate`/`unavailable`/`invalid`/`failed`/`partial` 七态并刻意分开：空结果不是执行失败，返回的全是已持有的候选不叫有新发现，没执行过的动作绝不记成已检索。每一行都由那一步执行结果的结构化记录折出，绝不从模型自述的理由反推。观察账是**投影不是第二份权威**：判重、已展开节点、已精确查找过的名称、枚举 cursor 覆盖与大纲溢出仍各由原处拥有，观察账不改变其中任何一项。历史目的明确标注为「当时的模型判断」，不证明覆盖、范围或通道可用；空转熔断算法不变，模型自报的覆盖变化不能让它清零。

证据卡只用**已经在候选池里**的材料生成：不新增模型调用、不查库、不补 hydrate。一张卡包含证据类型、稳定引用键、来源与位置（池里有才显示）、这段文字是原文逐字还是抽取摘要、一段有界的问题相关摘录、以及适用条件（有才显示）。知识对象没有原文片段，它的卡用已有的 name/steps/validity_scope 等抽取字段并**标明粒度**，不冒充逐字原文；查询期推导保留既有的「仅查询期成立」标识，不会被升级成可直接引用的图谱事实。选取是确定性的：先是大纲当前已绑定证据的代表，再是本轮新增或升级的，最后是历史高相关代表与来源多样性补位；同一条证据只渲染一次，排序与 tie-break 可重复，装不下的明确披露省略了几条。摘录窗口优先取覆盖「原问题与当前动作查询里有效检索词」的那一段（复用既有分词与 `"..."` 精确短语规则），无命中才取前缀，两种情况都带省略标记说明这是局部摘录。只有**卡真的渲染出来**的键才算对模型展示过；被预算挤出窗口的候选不会因为「它在池子里」就取得大纲绑定资格。

**必答方面与结束原因（设计稿 2026-09-07 §7；仅 v2）。** 必答清单**从不由模型决定**：它来自用户已经审阅过的冻结契约——Ask 取意图合同的 `mandatory_topics`，深度报告取本节的 `intent_questions`，没有正式契约的兼容路径以整条输入问题作为唯一方面。方面 id 按契约顺序确定性生成（`a1..aN`，不含用户内容），最多 16 个（复用意图合同 `mandatory_topics` 的既有上限）。相关**约束**（「只看 7nm」这类）随方面清单一起渲染进每一轮的服务器状态块，但**不各自成为一个方面**：约束是答案的谓词，不是可以被证据独立支撑的检索对象，给它一个能被标成「已支撑」的 id 只会造出一批永远停在未确认的方面。

每个方面的状态是 `unknown` / `partial` / `supported` / `conflicting` 之一，由模型在**同一次 reflect** 的附加 `assessment` 里给出——不新增任何模型调用。模型没报告的方面保留现状（省略是「这一轮没说它」，不是「删掉它」，必答清单只能由用户的契约改变）；报告了的方面则**全量替换**它的支撑判断，不像大纲证据键那样取并集，否则一次错误的「已支撑」再也撤不回来。载荷的顶层形状接受两种写法：`{supported:[...], unresolved:[...]}` 列表形，或按方面 id 直接映射的写法（`{aid: {status?, supported?, evidence_keys?, gap?}}`）——服务端在校验前把后者归一成前者，下面的判据不区分来源。形状判据是「顶层出现 `supported`/`unresolved` 任一键即按列表形」（陌生顶层键忽略），映射形里 `supported` 与 `status` **冲突时取保守的一边**：`supported: true` 且 `status` 缺省或就是 `supported` 才算已支撑；`status` 明确给了别的值就以 status 为准记为未解决；`status: "supported"` 而 `supported: false` 记为未解决的 `unknown`。服务端只确认三件事：方面 id 合法、引用的证据键是服务端签发过且**曾真实展示**给模型的身份（候选池里的细粒度证据键，或下面说的集合完整性键）、**未完整**枚举的集合身份不能冒充细粒度证据；剔除非法键之后没有支撑的项不得保持「已支撑」。同一个方面出现两行时按确定性规则处理：**完全相同**的两行（规范化之后状态、留下的键、gap 全等）去重，**互相冲突**的两行拒掉这个方面并保留它的旧状态——服务端绝不替模型挑首条或末条。语义支撑始终标为模型判断，不新增 grounded 分数，也不改动 evidence_level 阈值。

**收尾那一轮必须自评。** 判据是**收尾载荷**本身：`next_action=answer`，或任何带 `sufficient=true` 的动作（`update_outline` 这类不产证据的动作与 `sufficient=true` 并存是合法的，而穷尽档最常见的收尾恰恰是「最后一批绑定补上、同一轮宣布够了」）。这两种载荷都会当场终止整次检索，而那一刻方面账就是这次检索唯一的成色记录。「没有自评」的判据是**这一轮真正落进方面账的更新为零**，不是「载荷里有没有写行」：一份写满了行、每一行却都被逐方面拒掉（方面 id 不在清单里、`status` 不在闭集、证据键超限……）的收尾载荷，在方面账上留下的读数与彻底沉默逐字相同，所以它与沉默同样处理——照样退回、照样追问，那几条「服务端未采纳」的披露也因此有机会在下一轮渲染给模型。只要有**任意一个**方面真的落账，这一轮就算自评过了，收尾照常成立。这样一份载荷若一个方面都没落账，服务端**整轮退回**（一条零 I/O 的观察，原因 `missing_assessment`，与其它 skip 走同一份步数/无进展记账，但**对 stale 持平、不递增**——与送达了内容的 consult 轮同款判据，追问上限已由 `REFLECT_ASSESSMENT_MAX_PROMPTS=1` 兜住，不会被反复利用），并在下一轮的服务器状态块里逐个列出方面 id 追问；同一轮如果带着 `update_outline` 的载荷，那份大纲**先应用再退回**，模型这一轮真正做成的绑定不会因为追问而丢掉。追问句只挂被退回的**下一轮**那一次，不是此后每一轮。追问**最多一次**，第二次仍然不给就照原样收尾，把还没被判断过的方面记为「问过了、模型不给」（快照字段 `assessment_omitted`，收尾那条跳过步上的计数是 `aspects_assessment_omitted`）——它与「模型根本没走到收尾那一步」在放量评估里因此可以分开数。**没有下一轮时不退回**（步数已经用尽；或者收尾动作是 `update_outline` 而大纲额度只剩这最后一格——下一轮除 `answer` 外已无可执行动作，追问句根本送不到模型面前）：追问的全部价值在于下一轮补上，没有下一轮时退回只会把模型自己的判读改写成「没步数了」/「无可执行动作」，所以直接按「第二次沉默」接受收尾并记 `assessment_omitted`。追问已经发出、却因为下一轮提前收尾而一次都没送达时，run 结束时同样记 `assessment_omitted`。

**`prefix_delta_lean` 那条实验臂上，「收尾必须自评」这条不成立**（`REASONING_REFLECT_OPTIMIZATION=prefix_delta_lean`，默认 `off`，见部署文档那一条）：那条臂的自评合同在系统段里已经明说「没走到的方面留着不评、服务端不会为补齐账目退回一轮」，所以收尾缺自评是**照章办事**而不是不合作，上面那一整条退回—追问链在它上面一次都不走。**三格状态因此要分得开，别让它们互相冒充**：① `model_assessed=False` = 「这一格**没有**模型的判断」，它不解释为什么没有（run 早早熔断、模型没走到收尾，同样是这个读数）；② `assessment_omitted` = 「服务端**问过之后**它仍然不给」，判据里含着一次真实发生过的追问，所以**在 `prefix_delta_lean` 下永不置位**——那条臂一次都没问过，把没问过记成「问过了它不给」会让放量评估把一次照章省略读成一次协议不合作；③ 结束事实上的一格 run 级布尔 = 「这条臂**本来就不问**」，合成侧据它决定要不要多出下面那一行披露，而不是从 per-aspect 的沉默里反推。**终态那条跳过步的 detail 上多一格 `aspects_unassessed`**（模型一次都没判断过的方面数），由 **v2 四臂无条件写**（拍板 Q10 的已登记偏离；只在 L 写的话 D↔L 配对表上这一列恒缺一半，而「L 少核验了几个方面」正是它要回答的问题）——`0`（问过了、没有遗漏）与缺席（没量到 / legacy / 没走到终态）必须分得开。终态 `reason` 的七值闭集**一格不动**：L 下未评估的方面照旧可能把这次 run 归到 `model_partial`，改闭集会毁掉四臂横向可比。

**目录题的支撑：集合完整性键。** 枚举结果按合同不进相关性候选池，条目 id 也不对模型可见，所以「本库有哪些文档」这一类方面要的支撑不是任何一条细粒度证据，而是「这个集合已经被完整列出」这件事本身。服务端为**本 run 内覆盖率报告完整**的每一条枚举链签发一个确定性的集合身份键（`enum:` 前缀 + 集合名/子类型/限定来源/范围，与续跑身份一一对应，不含任何用户内容），并在**服务器状态块**里（紧接方面清单之后）单独列给模型看——模型先读到「a2 还没支撑」，再读到「有哪些键能支撑它」；只有真的列出来过的键才能写进 `assessment` 的 `evidence_keys`。没列完（还能续、或枚举期间资料变了）的链**不签发**任何键，模型自己拼一个同样被剔除——合法集是服务端算出的那一份，不是按前缀放行。集合键不参与「送达」复核：它不走证据预算那条路，也不产生 `[k]` 锚点。

检索结束时服务端生成一份类型化的**结束事实**，`reason` 取自七个值的闭集，各有一句固定的中文说明：

| reason | 触发 | 界面短句 |
| --- | --- | --- |
| `model_sufficient` | 模型明确结束且自报充分，方面记录里没有已知未解决项 | 检索结束：每个方面都已找到支撑 |
| `model_partial` | 模型选择结束但仍有未确认/部分/冲突；或自报充分与方面记录矛盾 | 检索结束：仍有方面没有完整支撑 |
| `step_budget` | 正常步骤预算耗尽 | 检索结束：步骤预算用完 |
| `stale` | 既有无进展熔断触发 | 检索结束：连续多轮没有新进展 |
| `no_executable_action` | 除作答外无可执行动作 | 检索结束：除作答外已无可执行的动作 |
| `model_degraded` | 反思调用在既有重试之后**连续两轮**仍失败而降级（v2；legacy 仍是一轮即降级） | 检索结束：反思调用失败，按降级收尾 |
| `retrieval_degraded` | 检索通道异常触发既有 fail-open 收尾，证据收集未正常完成 | 检索结束：检索通道异常，证据收集未正常完成 |

`retrieval_degraded` 的判据有**两个前件，缺一不可**：这次 run 没有模型正常结束标记，**且**时间线上最后一次真实 I/O 执行的观察是失败。单个可恢复的工具失败若随后（哪怕换了另一条通道）继续完成了检索，只作为观察留存，不把整个 run 标成降级；反过来，最后一次去查时查不动、随后走熔断或预算收尾的，这个原因盖过 `stale`/`step_budget`。没有被恢复的通道另记一份**纯披露**的 `unrecovered_channels`（口径是「该通道最后一次执行仍是失败」），不参与 `reason`：「哪条路没走通」与「这次检索有没有正常收尾」是两个口径，不互相冒充。取消与不可恢复的阶段错误仍然是异常/取消，绝不被包装成一份「成功生成的结束事实」。收尾前的大纲溢出纠错沿用原规则，不能把已经发生的熔断/预算原因改写成「充分」。

**一轮反思失败不再终止整次检索（仅 v2）。** provider 或 JSON 在既有重试之后仍失败时，v2 把这一轮折成一条零 I/O 的降级观察（原因 `model_degraded:<稳定原因码>`，账面状态是「失败」），当作没有进展的一轮继续走——已有的证据与刚播下的方向不再因为一次抖动全部作废，而反复失败仍然会被既有的无进展熔断收住。只有**连续两轮**都失败才按既有 fail-open 收尾，终态记 `model_degraded`；首轮同样适用，中间任何一轮成功都清零。失败的原因码逐字进回喂给模型的那行观察，模型据此知道该缩短输出还是换个问法。取消与阶段边界异常照常上抛，`fail_closed` 调用方（Knowhow 智能补全）的行为不变。原因码里新增一个 `output_budget_exhausted`：正文为空**且** provider 报告这次是被输出上限截断（`finish_reason=length`）时给它，其余空正文仍记 `empty`——两者的补救方向相反（加预算 vs 重试/换问法）。命中前者时服务端在**同一轮**原样重试一次并把这次调用的输出上限翻倍（`REASONING_MAX_TOKENS × 2`），成功即当无事发生。`finish_reason` 同时无条件写进 LLM 调用日志。同一条终态记录还带两个数值键：`attempts` 是这一次逻辑调用**真正发出的请求数**——瞬时重试、`response_format` 被拒后的明文重试、`stream_options` 被拒后的重建流各算一次，所以它可能大于 1；后两种不另写日志行，只有这个数字看得见它们。中途的 `status="retry"` 行**不带**这个键：一次调用的请求数只在终态行上报一次，按行累加会把同一次调用重复计入。`response_chars` 是**交还给调用方的正文长度**，不经日志正文的截断（`LLM_LOG_MAX_CHARS`），所以从日志读一个长回复的大小不会被少报。两者都只是数字，日志正文一个字都没有多写。

这份事实随证据一起走到 Ask 的单次/按节合成与报告的节撰写，合成 prompt 因此多出一段简短的**服务端事实**：结束原因、没解决的必答问题原文（最多列 6 条，其余只补一个「还有 N 条」的数）、没恢复的通道。它明确标为**不是知识条目、不带 `[k]` 编号、绝不可引用**，也明确说明它只表示「这次检索停在了哪里」，不是「库里没有这些内容」的证明——模型该做的是在正文里如实说明缺口，不是整段拒答。模型自己写的 `gap` 文本不进这个块。

**`prefix_delta_lean` 下这个块多一行「未逐项核验」的披露。** 上面那份「没解决的必答问题」在三臂下混着两种东西：模型查过、确实没找到的方面，与模型压根没对它表过态的方面。三臂下第二种是异常（收尾缺自评会被退回追问），而 L 下它是**常态**，于是同一行文本的含义会悄悄从「查过没有」漂成「没查过」，合成读到的却是前者，答案里就会出现一句「笔记本里没有这份材料」的假结论。这一行把差额如实说出来，顺序是**先事实、后自报**：有几个方面没被逐项核验、是哪几个（复用上面那份最多 6 条 + 「还有 N 条」的截断口径），再带上模型自己的结束判断（报没报「证据已足」，与 `reason` 分开的那一格），最后钉一句这**只**说明「这次检索没有逐项核对它们」，**不是**「笔记本里缺这份材料」的结论——那半句更宽松的自报判读刻意放在计数与列举之后、不放句首，免得给合成模型留一句可以援引来跳过缺口说明的话。**它的边界有三条**：(a) 判据是**两个条件都要**——是 L 这条臂，**且**这一份终态里真有未评估的未解决方面；只看后者的话，三臂下一次早早被熔断的 run 也会为真，那会改掉三臂的合成字节。(b) 它是上面那份未解决清单的**子集**（同一份清单再过一道「这一格根本没有模型判断」的闸），所以上面那一行为空时这一行结构性也为空，不会出现「没有未解决方面却说有几个未核验」。(c) **被熔断的 L run 也会出这一行**：`model_degraded` / `step_budget` 这类终态下模型本来就没走到收尾，未评估的方面照样在，披露照样出——这一行说的是「这次检索没逐项核对」，不是「模型收尾时偷懒了」。`directive` 那条尾句（不许把这段读成「库里没有」）一字不改：这一行是**披露**，不是新指令。

块尾那条**祈使句**（「在正文里说清哪些点没被覆盖」）与事实分开发放：单次合成给一份；按节合成时**每一节都拿到完整的事实**、但祈使句只随**最后一节**给——每节都被要求写一遍缺口说明，读者会在每一节末尾各读到一段免责声明，而缺口本来只需要在全篇说一次。`model_sufficient` 且既没有未解决方面、也没有未恢复通道时，这条祈使句在任何模式下都省掉：它指向的是一个空集合，留着只会诱导模型编一个缺口出来交差。这段块的输入成本因此有明确上界：单次合成 1 份、按节合成 K 份（K = 实际合成的节数），每份 ≈ 固定前后缀 + 至多 6 条方面原文（长度由用户契约的单条上限决定）+ 通道名列表；它不占 `[k]` 号段、不进 `id_map`、不参与合成计数。

**最终装配之后**，服务端用真正进入了合成 prompt 的证据身份复核每个方面，并把三个口径分开记账：`model_supported`（模型说这个方面有支撑）、`synthesis_admitted`（它绑的证据里至少有一条真的进了 prompt）、`answer_cited`（最终答案里真的出现了绑到那条证据的引用锚点）。三者**不是包含关系**：进入 prompt 不能证明答案引用正确，真实引用仍由既有的绑定与依据校验回答。一个方面绑过证据（状态是已支撑、部分支撑或证据冲突）、而那些证据被预算或过滤**全部**移除时，它降为「未送达」，不再显示为「已支撑」；只被移除一部分的不算未送达（模型确实看见了其中一条）。「证据冲突」与「部分支撑」同构地计入这条判据：三者说的都是「模型看见过那几条材料并据此作了判断」，把冲突项排除在外会让「两条互相矛盾的证据全被挤出合成」这一种静默通过，而那恰恰最该报出来。

「真正进入了 prompt 的证据身份」按最终 `id_map` 的 `object_id` 计，并把**同 canonical 簇被折叠掉的成员**一并算作送达：知识对象证据按簇去重，进 prompt 的是代表那条命中的 id，被折叠掉的成员的内容随代表一起送达了，只是身份换成了代表的。折叠表由证据装配本身顺手交出（零新增查询）；代表自己没能进 prompt 时它不在 `id_map` 里，成员因此也不算送达——这条口径在边界上天然保守。**深度报告一侧眼下不带这张表**，因此是保守口径：跨库同簇的成员可能被多报成「未送达」，反过来不会发生（真的没送达的不会被说成送到了）。这次复核不要求提前执行最终重排，也不为它再读一次库。后续补取解决了缺口可以在最终合成里使用，但不改写反思当时已经停止的历史事实。

这些事实沿既有的**私有**轨迹与报告章节路径存储：Ask 记在收尾那条跳过步与合成终步的诊断字段里，深度报告记在章节行上。两个落点的方面计数**拼法不同**且都稳定：收尾那条跳过步记 `aspects` / `unresolved_aspects`，合成终步与报告章节行记 `aspects_total` / `aspects_pending`（后者带前缀是因为它与 `aspects_undelivered` 等几个口径同处一份 detail，裸 `aspects` 在那里读不出是哪一个口径）。前端按「先取带前缀的、再取裸的、都必须是数字」兼容读两种拼法，缺键就不渲染——历史轨迹没有这些字段，把「不知道」显示成「0」是一句假话。界面复用已有的实时轨迹与完成后的合成说明，显示结束原因短句（**由服务端给**，前端不另立一份 reason 码→文案映射）、「已处理 x/N 方面」，必要时补「y 项未送达」「z 条通道未恢复」（只报条数，内部动作名不上屏）；收尾那条步虽然复用 `skip` 类型，标签显示为「结束」而不是「跳过」——这一步不是跳过了什么，它就是这次检索的结束。普通回答**不加**固定错误横幅——一次部分完成的检索不是错误。历史轨迹没有这些字段时逐字照常渲染。公开分享面不变：会话与报告的匿名投影是显式白名单，整个推理面（含轨迹）本来就不在其中，方面自评与结束事实一个字都不跨出匿名边界。
...
`REASONING_REFLECT_OPTIMIZATION=prefix_delta_evidence` 是需要显式选择的 v2 实验档，复用 `prefix_delta` 的证据卡、冻结 S/C 前缀、有界 K/D 快照与增量，以及不可逆的快照回退。它移除逐项自评指令与 schema 字段、自评吸收和追问、动态方面状态块，以及自评派生的合成缺口提示。用户确认的必答主题和约束仍按既有输入边界保留在 C；按问题覆盖情况决定停机的常规指令、工具权限、枚举完整性、大纲绑定、证据/历史预算和模型调用预算均保留。实际证据绑定仅来自大纲；重建时另按下述规则保留既有卡片。检索决策与最终证据不保证和启用自评的实验档相同。

本档 K 重建先满足真实大纲绑定，再按固定原问题的词项覆盖、既有检索分和稳定池序保留曾展示卡，沿用原有 fresh 证据预留。D 可从当前授权候选池恢复曾展示、当前不可见且匹配本轮动作查询的旧卡；这是复用内存证据，不算新增检索命中。准入保留首张有效 fresh 卡与真实绑定的优先级，再将恢复卡和剩余 fresh 卡交错，服从既有卡数/字符上限；只有一个卡位时仍先展示 fresh，没有 fresh 才先满足真实绑定。原卡与最后已发补充视图按相应问题/查询覆盖选取，平局取最新，已有字节和版本标记保持不变。当前已可见或已移出当前池的键不能经恢复通道进入。这些规则不生成方面自评、不改变检索权限、轨迹新增命中口径或模型调用预算。

仅本档对可识别的 Markdown 表格和分号分隔的重建表格使用 `REASONING_REFLECT_TABLE_EXCERPT_CHARS`（默认 1,200 字符；校验范围 240–4,000）。按查询相关性选取完整数据行并保留列头和可识别表格说明（包括单位与评测条件），显式说明省略行，使比较问题有机会看到表尾目标行。表格解析不可靠或空间不足时回退到有界摘录。这是现有分档总证据预算内的单卡额度，不新增检索或模型调用。没有方面或大纲绑定时，K/D 中已展示的证据也可随查询获得补充卡。补充按版本追加，不改写已发送的 K/D 字节，并跨轮继续有界扫描；压缩时按上述相关性规则选择原卡或最后已发视图，平局取最新。这些变化改善反思可见内容，但不代表服务端缓存已命中或延迟已有改善。

`REASONING_REFLECT_PROSE_DETAIL_ENABLED` 在这个显式实验档内启用有界普通原文详情与可见性反馈（默认 true；false 恢复本档之前的短正文行为并移除反馈），不会自动打开 v2 或选择本档。每次 reflect 投影最多选择 `REASONING_REFLECT_PROSE_DETAIL_CARDS` 个高相关原文 chunk/element 键（默认 2；范围 1–4），单卡额度为 `REASONING_REFLECT_PROSE_DETAIL_CHARS`（默认 1,000 字符；范围 240–4,000）。K、D 与补充卡共用该候选集合；之前冻结的详情可继续留在历史中，因此它不是所有历史详情卡的累计上限。选择依据为问题/本轮查询、既有检索相关性，并兼顾来源多样性；KG 摘要与可识别表格不占普通原文详情名额。短段落可以完整展示，长段落仍是明确披露的局部窗口，并在可能时保留句子/公式边界。详情服从原有证据预算和 fresh 预留，空间不足时保留普通摘录；不新增检索、模型调用或全局证据额度。详情更新仍按版本追加，沿用当前候选池的授权边界。

展开按需触发：第一次 reflect 的输入与关闭详情时完全相同，也不为增强预留空间。模型合法选择继续检索且请求完成处理后（包括按既有规则跳过重复请求），后续 reflect 才允许详情；初始播种、无效模型响应、仅修改大纲或查阅记忆不触发。检索不必得到新 key，重复命中时深读之前截断的来源同样有用。当前既无有效普通原文详情候选，也无已发详情可保留或恢复时，整个增强通道保持中性，包括补充过滤和历史预算处理。因此一轮结束的回答与仅使用 KG 卡片的检索保留此前输入行为。

详情补充仍先满足真实绑定；放不下、只能退回已可见摘录的详情不能持续占用补充名额。delta 回退后，之前的短卡仍可通过重建的 K 获得带版本号的详情。保留 D 的字节不改写，只有通过最终 K 准入的版本才登记为已发出；回退后仍遵守同一证据/保留历史预算与当前候选池授权。

只有最终 K/D 中实际可见已发普通原文详情，且原历史预算仍足以容纳完整可选提示时，T 才可报告本次 run 中首次成为可读内容的证据视图数。不提前为反馈预扣额度；空间不足就省略提示，不为它改写历史或触发重建；没有显示提示时仍登记实际可见内容。计数按最终准入摘录计算，不按候选池计数。身份是证据 key 与摘录内容；仅修改卡头/版本号或重新展示相同视图不算新可读依据。检索没有新候选时，真正新展示的详情仍可计数。这是可见性观察，不是事实支撑自评，也不证明问题覆盖完整；不改变既有 hard stale/步数上限或强制作答，供模型区别重复检索和读到新证据。其他 v2 档和 Legacy 忽略这些设置。

自评合同在 run 开始时冻结，通过 `ReflectContext.assessment_enabled` 和 `RetrievalTermination.assessment_enabled` 传递。新档后者为 false，方面快照和未解决 id 为空表示**未测量**，不表示覆盖已核验。`model_sufficient` / `model_partial` 保留模型收尾时自报充分/未自报充分的判断，不再检查方面账；两者均显示“检索结束：模型决定作答”，正常模型收尾不向合成追加终态块。预算耗尽、无进展停机、模型降级和未恢复工具失败仍作为执行事实披露，但不要求模型编写方面缺口。收尾的方面计数为 null，每轮自评测量及合成方面计数缺席，因此既有界面不显示方面完成比例。原有四种 v2 模式与 Legacy 保持原行为，总闸、默认优化策略和默认 A/B 实验档均不变。配对实验命令见 [scripts/README.md](../scripts/README.md)。
...
**谁能触发枚举：只有模型。** 触发者就是 reflect 循环里的那两个模型动作，没有第三个。**目录由模型自己选**：`enumerate_elements` 配上参数 `enumerate.collection="sources"` 就是「列这个库有哪几篇文档」，服务端不按问法正则在首轮替它确定性地播一次目录——agentic 模式里目录该不该列是模型的判断，问法分类器（`overview_intent`）只服务 chunk 通用问答那一条通道。模型之后再请求**同一范围**的同一份目录按「本轮已全部列出」跳过（`already_enumerated`），不会把同一份清单列两遍、扣两次预算。

**范围也是同一次调用的参数**：`enumerate.scope` 由模型自己填，只有两个合法值 `"all"` 与 `"current_notebook"`。**默认（缺省、留空或任何解析不出来的取值）是 `"all"`**：列出检索范围内的全部文档，即当前笔记本 + 勾选的挂载参考库。这与集合地图 `[Collections in scope]` 那行的 `sources` 计数**同一个联邦口径**，所以模型什么都不填时，它看到的数与它列到的数是对得上的；`"current_notebook"` 才收窄成只列当前笔记本的文档，也就是同一行括号里的 `current notebook` 计数。**reflect v2 协议下同一个参数走在那个单一 `arguments` 对象里**（`{"collection":"sources","scope":"current_notebook"}`），取值集只有一份 —— `reasoning_actions.ENUMERATE_SCOPES`，legacy 的 schema hint、v2 的能力投影与两条协议的解析层读的都是它，所以两条协议不可能给出不同的范围。v2 的 hint 把 `arguments` 写成开放对象，由此有两处不同：传输层在那里不校验这个值，所以非法的 `scope` 真的会走到解析层、按上面那条 fail-open 回退，而不是白扣一步；而且 `scope` 是 v2 **唯一不折成** `invalid_argument:` 观察的枚举参数（`kind`、`object_type`、`direction`、`prefer` 都折）。这条例外是刻意的：`kind`/`direction` 错了执行出来的是**另一件事**，而范围错了执行出来的是同一件事的超集，何况拿回去的清单自己会说明范围。范围同时进 v2 动作观察账的请求身份（恒渲染成 `scope=…`）：续跑账目按范围记键，身份串里没有这一格的话，两条链在账上会读成同一次重复请求。
...
同一行还会进入**写答案的那次模型调用**：作为原文证据分区最前面的一小块服务端确定性输出（固定表头 + 已封顶的地图行，整块不超过 800 字符；因为它是服务端算出来的、不是检索到的条目，所以不带 `[k]` 引用）。这是在补一个真实的缺口：reflect 提示语明确要求模型「集合远大于本轮清单额度时，别翻页，直接用计数作答」——两套协议都说这句话，legacy 写在 prompt 正文里，reflect v2 写在能力投影的 `enumerate.scope` 参数说明里，并点名模型真正看得到的那两个数（地图行的 `sources` 计数，与同一行末尾的 `listing allowance left: N rows`）——而答案合成是另一次调用，本来根本看不到那个数——极端情况下（集合很大、又没检索到别的证据）合成压根不会触发。只要枚举工具开着且地图建成，这一块就会注入，与本轮是否真的枚举过无关；它排在所有证据块最前，避免预算吃紧时第一个被牺牲的恰好是它。

**覆盖率合同。** 每个已枚举集合都携带一个 `TypedCollectionCoverage`：`returned_total`（跨续跑链累计已返回的条数）、`total`（地图给出的已知规模，取不到时为 `null`——渲染为总数未知，绝不写成 `/0`）、`complete`、`truncated_reason`（`budget` / `payload` / `concurrent_change` / `oversize_sample`）与 `overflow_semantics`（沿用共享的 `explicit_partial`）。`complete=true` 要求游标耗尽、首页前与末页后的作用域身份一致，且——跨续跑链——累计条数与已知规模相符；其余情况一律 `complete=false`。`oversize_sample` 是清单规模守卫自己的原因码，**刻意不并进** `budget`：本轮的行池并没有被用光——服务端是**有意**只要了一页，因为地图上这个集合的计数远大于这一轮可能列出的量（见下文「预算与续跑」）——所以剩下的额度仍然留给后面的动作，而诚实的说法是「这个集合远大于一轮能列的量，这里给出样本与计数」，不是「清单额度用光了」。「剩下的额度」这半句必须是真的，所以守卫只在行池**多于一页**时介入：只剩不到一页时它连行数都改不动，那一档照旧报 `budget`。它与其他部分结果一样携带可续跑的游标。这里的「作用域身份」包含参与的库集合本身，并在收尾时重新解析一次：翻页途中挂载、卸载或使某个参考库失效，会改变「整个作用域」指的是什么，此时结果按并发变化报告，绝不说成「全部」。覆盖率之外，结果还携带 `synthesis_rows` / `synthesis_complete`：已枚举的清单与真正进入答案合成 prompt 的有界预览分开统计，因此「枚举 200/200、预览 100」会披露为「枚举完整、分析部分」——与 Knowhow batch 自身的枚举/合成两轨披露同构。这份预览额度在本轮的各个集合之间**分配**而非先到先得：先给每份清单保底等分，某份装不满的余量再按顺序让给后面的。于是一轮里列了两个集合就会预览两个，而不是被第一份吃光额度、让多集合问题只按一张卡作答。
...
**预算与续跑。** **远超额度的清单只取样本，不翻页。** 仅在 reflect v2 下：集合地图里**本次请求那个集合**的计数超过本轮剩余行数的四倍时，服务端把这一个动作的行上限压成一页（`min(剩余行数, enum_page_size)`），而不是把整轮剩余额度交给它；这份清单报 `truncated_reason=oversize_sample`。分母恒取与该请求对齐的那个数——默认 `all` 范围的 `sources` 目录取联邦总数，`current_notebook` 取本库那个数，元素/知识对象清单取该 kind、该类型自己的计数——所以一份只有 3 条公式的清单不会因为库里有 48 839 篇文档而被砍成样本。地图定不出价的请求（没有地图，或清单被收窄到单篇来源）一律照旧。行池只剩不到一页时守卫同样不介入：那时它连行数都改不动，而「额度没用光」在那一档是假话，所以照旧报 `truncated_reason=budget`。倍数是常量 4 而不是部署旋钮：它是「把额度花光也看不到四分之一」这句话的算术形式，那不是一份清单、是一段前缀。动作既不被拒绝也不被改写——请求的范围原样保留、计数照报、链条照样可续，真想要更多的模型可以在额度还剩的时候再问一次。没有这道守卫时，48 839 篇的检索范围会让一轮的 300 行池全部花在一段任意的 0.7% 前缀上，此后这一轮里的每次枚举都只剩「已达本轮上限」的跳过。关闭态（reflect v2 关）逐字节不变——它把同一件事写在自己的prompt 文本里。除此之外，一次动作在其 run 级预算内（`enum_rows_per_run` 总行数、`enum_pages_per_run` 额外翻页次数，以及共享的 `structured_payload_chars` 结构化载荷上限）自动翻页，直到游标耗尽或触及某项上限；只有每个被访问分片（元素按来源、知识对象按参与的库）的第二页及之后才计入页上限，因为一个分片的第一页是唯一能看到它的方式。三个上限都是**每次请求**的，不是每个动作各来一份：每次动作只拿到本 run 的剩余额度，因此一轮深度检索里的多次枚举，累计返回的结构化载荷也不会超过文档写明的请求级上限。载荷上限在两个位置、按两种量法各收一次口，因为它们回答的是两个问题：执行器在遍历时按紧凑的内部行计费（这道闸拦的是「读得比该请求允许产出的还多」），响应映射再按**序列化后的行**计费——那才是真正下发与持久化的形状，而且明显更宽。被第二道闸拦停的那一份，自己报 `truncated_reason=payload`，`returned_total` 等于实际送达的条数；合成预览也按送达的那份渲染，所以 prompt 的覆盖率块头与结果卡永远报同一个数。该游标是 run 内部句柄，不出现在响应里——客户端只看到覆盖率徽章；截断时看到的是「同一轮推理内可以继续列出」，而不是一个可操作的续跑入口。`complete=false` 恒含一个可续跑的游标，唯一例外是 `truncated_reason=concurrent_change`——此时作用域在两次调用之间发生了变化，链条绝不能静默从头重来。对尚未列完的集合重复请求，若本 run 预算尚有行数/翻页/载荷余量则从该游标续跑；预算耗尽时改为按「已达本轮上限」跳过（仍披露为部分结果），而不是按「已枚举过」跳过——只有链条已经 `complete=true` 的集合才会被跳过为「已枚举过」，因为再问一次只会重复已经报过的条目。

**限定单一来源。** 内部来源 id 从不上屏，也从不给模型看——候选摘要与引用里出现的一直是来源标题——所以「只列《某某》里的公式」是按**标题**表达的。服务端在地图已经规划要走的那批来源里做确定性解析：去掉首尾空白、忽略大小写后的精确匹配，绝不做模糊或按相关度排序的匹配，并且有界，避免一个很大的挂载参考库把一次反思变成全库标题扫描。名字没有匹配、或匹配到多个来源时，这一个动作被跳过并在推理轨迹里说明原因：既不会悄悄扩成对整个作用域的枚举（那是在回答另一个问题），也不会在同名文档里替用户挑一个。若作用域内含该类条目的来源多到超过解析器允许查看的范围，它直接拒绝作答，而不是拿看过的那一部分下结论——「唯一」是整个作用域的性质，同名的第二个来源可能就落在没看到的那一段里。reflect v2 的枚举参数表干脆不再提供 `source_id` 这个字段——legacy 协议照旧提供，它的 prompt、schema 与轨迹逐字节不变——因为模型能填进去的只可能是猜的内部 id，而猜出来的 id 按构造必然不在范围内。硬塞一个进来时它照样按限定单一来源分派（默认掉等于悄悄回答「列出整个集合」这个另外的问题），只是这一个动作被跳过时用自己的原因码：轨迹说明点名的来源不在检索范围内、请按名称给出（`source_title`），而不是与「未知条目类型」共用同一句「请求的范围不可用」。v2 载荷**两个都给**时以标题为准：分派本身是 id 压过标题（只有没给 id 时才去解析名字），否则一个猜来的 id 会把模型如实给出的书名整段吃掉，把一个本可以成立的请求变成一条「来源不在检索范围内」。这条取舍只住在 v2 的解析层——非字符串的 `source_id` 照旧按类型拒收，而 legacy 协议与分派处的「id 压过标题」逐字节不变。
