...
The portable phases `ingest`, `kg`, `index`, `all`, `embed`, `metadata`, `reparse`, and
`backfill-source-index` select SQLite or PostgreSQL from `DATABASE_URL`. Direct PostgreSQL
...
The `backfill-source-index` subcommand proactively populates `knowledge_object_sources`, the reverse-lookup table (`object_id, source_id`) used when a source is deleted or reparsed. A backfilled notebook resolves affected KG objects directly through that index. An unbackfilled notebook deliberately does **not** pay a whole-notebook lazy-backfill cost inside an interactive delete/reparse: keyset-paged database-native evidence filters select references for the current source, and the request neither deserializes every object in Python nor writes index rows for unrelated objects. The fallback can still scan that notebook's legacy KG rows, so large libraries should prebuild the index rather than rely on repeated fallback scans.

The explicit command remains the offline prebuild and repair path. It makes no model call and is idempotent/restartable: each run clears and rebuilds the selected notebook's index rows from current evidence in bounded-memory batches, prints progress, and then marks the notebook as backfilled. Use `--notebook-id` for one notebook or `--all-notebooks` for the whole database; re-run it if the reverse index may have drifted after an abnormal interruption. Regardless of index state, online cleanup locks the source row before projection cleanup, caps each affected-object delete statement at 500 ids (a SQL-parameter bound, not a constant-time/background-job guarantee), and fetches/deletes referenced image-asset rows in one database round trip before unlinking their files. The source row in the UI switches to a deleting state immediately and disables its delete action until the request settles; a notebook-scoped tombstone also prevents stale navigation/list responses from restoring the deleted row.
...
Roll out in this order: enable `SOURCE_SUBGRAPH_PPR_ENABLED` (and, for oversized sources, build companions before enabling `SOURCE_PARTITIONED_PPR_ENABLED`), set `SELECTED_SOURCE_GRAPH_ROLLOUT_MODE=shadow`, and inspect the content-free `selected_source_graph` events. After the canonical gate passes, configure the trusted attestation path plus exact corpus/model pins, then move through `allowlist` or stable-hash `rollout` before `on`. Roll back with the single `SELECTED_SOURCE_GRAPH_ROLLOUT_MODE=off` switch; this immediately restores the historical `B` path and does not require deleting artifacts. Never copy an attestation between a different corpus signature or model contract.
