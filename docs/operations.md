...
- `events.jsonl` — async source pipeline: per-stage timings (`parse` / `embed` / `extract`) and every status-machine transition. A "stuck" upload shows exactly which stage is running and for how long; failures record the real exception (and the source's `error_message`). A KG extraction that lost windows also emits one `kg_window_failures` event per source (`failed` / `total` plus `reasons`: a count per stable cause code such as `provider_unavailable` or `malformed_response:invalid_json`, never reply text). The source's own warning ("部分内容未完成分析（N/T 段失败）") deliberately names no cause — the per-window exceptions are swallowed for isolation, so this event is the only place the cause is recorded.
...
For XLSX/DOCX/PPTX sources (not PDF, whose tables are already bounded by their rendered page), `parsers.py` splits an overlong table into several `table` elements on row boundaries, capped by row count and by character count — the character cap is `CHUNK_TARGET_CHARS` itself, the same target `chunking.py` uses for plain text, so there is no separate knob to tune and changing it only takes effect on the next reparse (exact limits: [Source uploads and parsing](./product-and-api.md#source-uploads-and-parsing)). This is what keeps a wide sheet (hundreds of rows) from folding into one element that `chunking.py` never subdivides, which used to truncate late rows out of both embedding and answer context. Each part after the first carries the detected header row's text ahead of its own for retrieval (dropped when the header itself is too long to fit half the budget), and `metadata.table_group`/`table_part`/`table_parts` let the source detail view merge consecutive parts back into one rendered table with a single header row.
...
The `backfill-chunk-elements` subcommand populates `chunk_elements`, the element -> chunk reverse index. `chunks.element_ids` stores the forward direction, so answering "which chunks contain this evidence element" used to scan every chunk row of the notebook and JSON-decode each one, once per index generation. After a backfill that becomes an indexed point lookup bounded by the handful of evidence elements one query actually hit. For a notebook read as the **active** one, not having been backfilled keeps the legacy whole-notebook scan with identical results, so there the command is a performance knob — optional, recommended for large libraries. For a notebook that other notebooks **mount as a reference library** it is a functional prerequisite, not a knob: when the KG overlay hits one of that library's knowledge objects, the evidence -> chunk lookup that fetches the passage backing it is allowed to use the indexed point lookup only (the legacy path is a whole-library scan of a library the asking user is typically not a member of, a cost federation must not impose), so an un-backfilled reference library contributes **no passages at all** for that turn — the knowledge object is still cited, its source text is simply absent. Each such turn emits one content-free `kg_source_chunks_peer_unindexed` event carrying `notebook_id` (the asking notebook), `peer_notebook_id` (the reference library that yielded nothing) and `elements` (how many evidence elements went unresolved). Treat a recurring `peer_notebook_id` in that event as the work order: run `backfill-chunk-elements --notebook-id <peer_notebook_id>` against that library, after which the passages appear with no other change. The event never carries source text or query text.
...
| Main-index array headers | every required `.npy` (including `idf.npy` and `chunk_index.npy` — neither used to be on the header-probe list at all) must have a readable header regardless of whether the manifest declares a count for it, and `graph.npz` a readable matrix shape (header-only reads: milliseconds even on a multi-GB package). Where a count IS declared, row counts must also agree: `idf.npy` against `n_nodes` (the same entry as `node_ids.npy`), `chunk_index.npy` against `n_chunks` | `import` refuses, naming the file as `truncated or malformed` (or the row-count mismatch). A present-but-unreadable array used to be skipped as "unchecked": the package published, replaced a healthy live index, and only then was rejected by `load_scale_index`, leaving the notebook with no scale core until an operator noticed. A manifest that declares no count for an array, or a count for an array the package does not carry, still passes readability and is unaffected on the count comparison (older-index-stays-valid) |
| Source-partition companion format | the companion root's manifest — inside the package for `import`, and the live one for `export` — must have `format_version` equal to this checkout's `SOURCE_PARTITION_FORMAT_VERSION`; only the ROOT manifest is checked, not each per-source partition manifest — the reader applies the same gate itself when it opens a source and fails closed for just that one, so the root-level check exists to stop the heavier harm of replacing an entire healthy companion outright | `import`/`export` both refuse, naming `format_version` and explaining the package/live companion was built by an older checkout and needs rebuilding or re-exporting with the current one. Previously a companion whose `parent_version`/`parent_build_id` paired correctly published or exported regardless of its format, and the reader's identical gate then immediately called it `unavailable` — silently swapping a healthy live companion for an unusable one |
...
For recovery or replacement in the administrator page, select the original account from the account inventory and verify the displayed account before issuing the grant. Selection supplies its exact internal user ID; the external identity to authenticate is entered separately. Enrollment does not reuse a selected existing account.
