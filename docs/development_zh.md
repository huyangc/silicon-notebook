...
外部 Agent MCP 面由一个 API-owned registration host 和启动期冻结目录组成。它先把 `app.api.mcp_tools` 的固定 bundle 捕获为精确 22-tool core 前缀，再追加显式信任的进程内 `agent.tool_provider` contributor 标量 descriptor。core handler 保持既有 validation/auth/I/O 顺序；provider handler 复用实时权威、owner-write、progress 与 output 边界，拿不到 FastMCP、repository、原始凭据或通用 service locator。插件异常只映射稳定公开码，core 只发 content-free 的 tool/plugin/status audit，结果 rail 在递归复制过程中执行。默认 provider topology 为空，注册/listing 为零 repository/model 工作。
