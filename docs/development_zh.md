...
- 模块化扩展底座与 Phase-1 retrieval host 合同已经生效。稳定跨层值放在 `backend/app/domain`，该包不得 import services、repositories、API、infrastructure、extensions 或 feature 实现；`backend/app/repositories/ports.py` 不得 import `app.services.*`，backend 静态 import 图必须保持无环。全部 repositories→services 的既有反向 import 按 SQLite/PostgreSQL/other 分区记录为只许下降、不许增长的债务上限。`backend/app/extension_sdk` 定义低依赖 manifest、五种彼此独立的 contribution（Provider、ProviderChain、Contributor、Auditor、Observer）、point-specific retrieval context/result/budget/cancellation/provenance 与脱敏 failure；capability `requires` 与插件 `depends_on` 分开，contribution 默认按稳定 ID 而不是整数 priority 排序。`backend/app/extensions` 负责组合、启动冻结拓扑、capability 判定入口目录和 Ask/Report 共用的 `RetrievalContributorHost`；freeze 只校验 required capability 的判定入口，availability 每次消费实时解析。extension 包之外只有 `app.main` 与 `app.repositories.factory` 可以 import 组合面，workflow 只拿注入的窄 host，不能把 registry 当 service locator。没有适用 contributor 时，host 会在 context、cancellation/availability、clock、event、fan-out 或 I/O 前原样返回同一个 baseline 对象。共享 runner 用 `selected_evidence` 与 `chunk_candidates` 区分物理时点：selected-source graph 留在更强 baseline/attestation guard 后，generated-question 留在 MMR/fusion 前；scope/provenance 一次批量复检，禁止 per-hit N+1；核心取消传播，插件局部失败/超时 fail-open。两个真实能力在各自迁移 PR 前都不移动。`scripts/check_architecture_boundaries.py` 直接进入 G1 contracts lane，同时允许当前 repository facade 公开面收缩但不允许增长。新增扩展点前先读[模块化插件架构设计](./modular-plugin-architecture-design-2026-08-21.md)；review 与合入按[交付流水](./modular-plugin-architecture-delivery-plan-2026-08-21.md)执行，两路独立 subagent review 和 CI 全绿后才 squash merge。
...
- **访问权变动之后必须连当前工作区一起对账**：独立群组页里的退出、移出成员、删组或撤销共享都可能使此前打开的 Notebook 失权。`page.tsx` 的 `reconcileOpenNotebook(remaining)` 是唯一实现，「退出只读共享」与群组页的 `onChanged` 共用它。远端撤销没有推送通道，只在标签页重新可见时节流复核；取数失败不执行对账。这是尽力而为不是保证，回归门在 `frontend/tests/guards/group-sharing-guard.test.mjs`。
