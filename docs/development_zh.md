...
- Parser ProviderChain 现已在启动时冻结 self-hosted MinerU → MinerU cloud → builtin 三环 DAG；link 用 `after`/`before` 边和稳定 ID tie-break 表序，禁止整数 priority。Core route policy 先于实时 availability，插件 probe 无持久化/资产端口，只有 core admission 通过才调用 core materializer。PR-04 中该拓扑刻意 dormant：生产 ingestion 与它物理断开，PR-05 一次性切流并删除旧 dispatcher 之前，legacy dispatcher 仍是唯一入口；禁止引入可选双路。
