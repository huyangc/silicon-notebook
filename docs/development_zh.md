...
- 模块化扩展底座与 Phase-1 retrieval host 合同已经生效。稳定跨层值放在 `backend/app/domain`；repository ports 不得依赖 services，静态图保持无环，repositories→services 债务上限只许下降。低依赖 SDK 拥有类型合同，`backend/app/extensions` 拥有冻结 registry、实时 capability 判定与共享 host，唯一把扩展和 adapter 接起来的外层组合根是 `app.bootstrap`；workflow 只依赖 domain host port。availability probe 无 I/O，执行 context 保持惰性，manifest 声明与实时判定共同决定每个插件可见的 reader/model port；invocation 路由和 core admission policy 启动时快照。空路径在任何工作前原样返回 baseline。proposal 先有界，再经一次 core-owned 权威 hydrate 后合并，禁止 per-hit N+1；畸形插件 fail-open，强合同 lane 可用整批 atomic admission。`selected_evidence` 留在 selected-graph attestation 后，`chunk_candidates` 留在 generated-question 的 MMR/fusion 前；核心取消传播。插件实现不得 import 具体 repository、facade 或 runtime。两个真实能力各自迁移 PR 前不移动。G1 守卫固定这些规则，facade 公开面只可收缩。新增扩展点先读[设计](./modular-plugin-architecture-design-2026-08-21.md)，合入按[流水](./modular-plugin-architecture-delivery-plan-2026-08-21.md)：两路独立 subagent review 和 CI 全绿后 squash merge。
