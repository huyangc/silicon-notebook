...
- `backend/app/application` 持有低依赖的不可变 stage 合同。Ask reasoning 显式跨越 prepare、retrieval-evidence、response-draft 与 committed-answer 边界；runtime 权威携带同一个 source scope、请求级 retrieval run（也就是 leaf-I/O semaphore 的所有者）、取消 token、trace sink 和无 I/O 连接探针。Stage 边界违约必须作为核心错误响亮失败，不能伪装成可选检索 miss。Report 的可变 `ReasoningResult` 工作副本继续保留；只有既有 KG/chunk/element/PPR leaf 调用取得 run slot，任何 stage wrapper 都不得持有数据库连接或外层 slot。本 PR 不增加 auditor、observer 或插件 seat。
