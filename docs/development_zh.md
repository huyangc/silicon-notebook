...
- 模块化扩展底座与 Phase-1 retrieval host 合同已经生效。稳定跨层值放在 `backend/app/domain`；repository ports 不得依赖 services，静态图保持无环，repositories→services 债务上限只许下降。低依赖 SDK 拥有类型合同，`backend/app/extensions` 拥有冻结 registry、实时 capability 判定与共享 host，唯一把扩展和 adapter 接起来的外层组合根是 `app.bootstrap`；workflow 只依赖 domain host port。availability probe 无 I/O，执行 context 保持惰性，manifest 声明与实时判定只投影窄 capability port；一个 capability 不可用时只关闭对应 contribution，workflow 仍进入共享扩展点并保留其他 contributor 输出；invocation 路由和 core admission policy 启动时快照。空路径或无适用 contribution 时在任何工作前原样返回 baseline。proposal 先有界，再经一次 core-owned 权威 hydrate 后合并，禁止 per-hit N+1；畸形插件 fail-open，强合同 lane 可用整批 atomic admission。Selected-source graph 已成为首个内建 `selected_evidence` contributor。请求级 core bridge 保留 legacy activation result，插件看不到 baseline、graph service 或 repository 协作者，因此 attestation、rollout、scope drift、duplicate-support overlay、独立预算、status 与 fail-closed 行为保持等价；Ask/Report 已删除 service 直调。SQLite/PostgreSQL 执行上下文探针在持有 transaction/pool lease 时于 host fan-out 前阻断，PostgreSQL conformance 强制 pool size 1。`chunk_candidates` 仍留在 generated-question 的 MMR/fusion 前，generated-question 本身在 PR-03 前仍走直路；核心取消传播。插件实现不得 import 具体 repository、facade 或 runtime。G1 守卫固定这些规则，facade 公开面只可收缩。新增扩展点先读[设计](./modular-plugin-architecture-design-2026-08-21.md)，合入按[流水](./modular-plugin-architecture-delivery-plan-2026-08-21.md)：两路独立 subagent review 和 CI 全绿后 squash merge。
- Point-specific proposal source 与通用 admission reader 是两个独立 domain port。Selected-source graph 的权威复验只读请求级内存 map，零新增 DB/leaf；只有其他未解析 proposal 才调用一次 repository-runtime reader，并在 SQL 返回行之前应用 notebook/source 谓词。
