...
- **访问权变动之后必须连当前工作区一起对账**：群组弹窗从工作区顶栏也打得开，退出群组／被移出／删组／撤销共享都可能把用户正站在里面的那本库从脚下抽走。只刷 `notebooks` 清单不够——工作区开在 `currentNotebook` 这份**另一个** state 上，一动不动，人会继续待在一本已经读不到的库里，整屏毫无反应直到手动刷新。`page.tsx` 的 `reconcileOpenNotebook(remaining)` 是唯一实现，「退出只读共享」与群组弹窗的 `onChanged` 共用它；判据是「重取回来的清单里还有没有它」而不是「刚才那个动作针对的是不是它」——同一本库可经多条路进来（另一个群组、直接成员、`everyone` 边），退掉其中一条不必然失去访问。远端撤销（别人移出你／删组／撤销共享）没有推送通道，只在标签页重新可见时复用同一条对账路径复核（节流，取数失败即整条不执行）；这是尽力而为不是保证。 回归门在 `frontend/tests/guards/group-sharing-guard.test.mjs`。
