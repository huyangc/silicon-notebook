...
- 模块化扩展 Phase 0 边界已经生效。稳定跨层值放在 `backend/app/domain`，该包不得 import services、repositories、API、infrastructure、extensions 或 feature 实现；`backend/app/repositories/ports.py` 不得 import `app.services.*`，backend 静态 import 图必须保持无环。全部 repositories→services 的既有反向 import 按 SQLite/PostgreSQL/other 分区记录为只许下降、不许增长的债务上限。`backend/app/extension_sdk` 定义低依赖 manifest、五种彼此独立的 contribution（Provider、ProviderChain、Contributor、Auditor、Observer）、最小 context/result 原语与脱敏 failure；capability `requires` 与插件 `depends_on` 分开，contribution 默认按稳定 ID 而不是整数 priority 排序。`backend/app/extensions` 负责组合与启动冻结拓扑，availability 仍按请求实时判定。Phase 0 registry 为空且既有 workflow 不消费它，所以无插件路径就是原路径。`scripts/check_architecture_boundaries.py` 直接进入 G1 contracts lane，同时允许当前 repository facade 公开面收缩但不允许增长。新增扩展点前先读[模块化插件架构设计](./modular-plugin-architecture-design-2026-08-21.md)；review 与合入按[交付流水](./modular-plugin-architecture-delivery-plan-2026-08-21.md)执行，两路独立 subagent review 和 CI 全绿后才 squash merge。
