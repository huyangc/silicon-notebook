...
- `source.element_enricher` 是 accepted parser materialization 与 core element 事务之间的独立 dormant 批量 Contributor point。默认生产组合不注入 host seat，所以 no-plugin 路径在 snapshot、clock、probe、event 或 I/O 前就是原路径。Active contributor 每个来源/贡献只调用一次，不持数据库 lease，只拿不可变最小元素视图与 request-local opaque ref，只能返回有界的命名空间 metadata。Core 保留 text、type、location、顺序、identity 与 parser/table/image/asset/section provenance；caption 必须已出现在解析文本中。普通失败和非法输出整批回到 parser 输出。不得通过本 point 加逐元素调用、直接持久化/模型/资产 capability、Memory/Knowhow 路由或图片检索语义。
