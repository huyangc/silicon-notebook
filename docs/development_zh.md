...
- 结构回归测试只使用 public HTTP contract 或显式 domain seam，不得绑定 private aggregate helper、源码位置、行数或 route/model 总数。typed root-modal coordinator 与 FastAPI lifespan/application lifecycle composition 仍是独立债务。
