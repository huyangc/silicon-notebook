...
- `frontend/features/extension-sdk` 是唯一 build-time workspace UI registry/host。canonical slot 只有 `workspace.side_panel` 与 `source.detail_section`。精确静态 plugin/version/contribution、已认证 `/system/extensions` 的实时 availability、壳层 `workspaceCapabilities` 权限投影与 normalized UI mode 四门全真才渲染。production registry 为空时必须在 request/controller/timer 前短路并返回 exact `null`。服务端只投影闭合脱敏元数据；UI 代码静态构建且只拿 readonly 摘要，不接 page setter 或领域 owner 内部状态。跨栈 parity 测试必须包含 non-empty frontend-only/backend-only/plugin/id/slot/capability mismatch，防止 empty/empty 假绿。
