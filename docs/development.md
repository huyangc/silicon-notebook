...
- The modular-extension foundation and Phase-1 retrieval host contract are enforced. Stable cross-layer values live in `backend/app/domain`; repository ports cannot import services, the static graph stays acyclic, and repository-to-service debt ceilings may only fall. The dependency-light SDK owns typed contracts; `backend/app/extensions` owns the frozen registry, live capability decisions, and shared host; `app.bootstrap` is the only outer root that joins extensions to adapters. Workflows depend on the domain host port, not the registry. Availability probes are I/O-free, execution context stays lazy, and manifest declarations plus live decisions project only narrow capability ports. An unavailable capability disables only its contribution; workflows still enter the shared extension point and retain independent contributor output. Invocation routing and core admission policy are startup snapshots. The empty/no-applicable path returns the exact baseline before any work. Bounded proposals go through one core-owned authoritative hydrate before merge, never per-hit N+1; malformed contributions fail open and strong lanes may use all-or-nothing atomic admission. Selected-source graph is the first built-in `selected_evidence` contributor. Its request-local core bridge retains the legacy activation result and exposes neither baseline nor graph/repository collaborators to the plugin, so attestation, rollout, scope drift, duplicate-support overlay, independent budget, status, and fail-closed behavior remain equivalent. Ask/Report no longer call the service directly. SQLite/PostgreSQL execution-context probes block host fan-out while a transaction/pooled lease is held; the PostgreSQL test forces pool size 1. `chunk_candidates` remains before generated-question MMR/fusion and generated-question itself stays on its direct path until PR-03. Core cancellation propagates. Plugin implementations may not import concrete repositories, facade, or runtime. The G1 architecture guard enforces these rules and facade surface may shrink but not grow. See [the design](./modular-plugin-architecture-design-2026-08-21.md) and [delivery pipeline](./modular-plugin-architecture-delivery-plan-2026-08-21.md); each PR needs two independent subagent reviews and green CI before squash merge.
- Point-specific proposal sources and the generic admission reader are separate domain ports. Selected-source graph authority resolves from its request-local map with zero added DB/leaf work; only unresolved proposals call the repository-runtime reader, once, with notebook/source predicates applied in SQL before rows are returned.
