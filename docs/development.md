...
- The modular-extension foundation and Phase-1 retrieval host contract are enforced. Stable cross-layer values live in `backend/app/domain`, which cannot import services, repositories, API, infrastructure, extensions, or feature implementations. `backend/app/repositories/ports.py` cannot import `app.services.*`, and the backend static import graph must remain acyclic. All existing repository-to-service imports are partitioned into SQLite/PostgreSQL/other debt ceilings that may fall but not grow. `backend/app/extension_sdk` defines dependency-light manifests, five distinct contribution kinds (Provider, ProviderChain, Contributor, Auditor, Observer), point-specific retrieval context/result/budget/cancellation/provenance, and sanitized failures. Capability `requires` are separate from plugin `depends_on`, and contributions default to stable ID order instead of integer priority. `backend/app/extensions` owns composition, startup-frozen topology, the capability decision-entry catalog, and the shared `RetrievalContributorHost`; freeze validates required decision entries but live availability is resolved per consumption. Only `app.main` and `app.repositories.factory` may import the composition surface outside the extension packages; workflows receive the injected host rather than a registry/service locator. Ask and Report share the process host. No applicable contributor returns the exact baseline object before context, cancellation/availability, clock, event, fan-out, or I/O. The common runner exposes separate `selected_evidence` and `chunk_candidates` invocations because selected-source graph stays after its stronger baseline/attestation guard while generated-question stays before MMR/fusion. Scope/provenance admission is one batch, never per-hit N+1; core cancellation propagates, while optional extension failure/timeout is fail-open. Neither real capability moves until its dedicated PR. `scripts/check_architecture_boundaries.py` runs directly in the G1 contracts lane and permits the current public repository-facade surface to shrink but not grow. See [the modular plugin architecture design](./modular-plugin-architecture-design-2026-08-21.md) before adding an extension point; review and merge these PRs through [the delivery pipeline](./modular-plugin-architecture-delivery-plan-2026-08-21.md), which requires two independent subagent reviews and green CI before squash merge.
