...
- `frontend/features/extension-sdk` is the sole build-time workspace UI registry/host. Only `workspace.side_panel` and `source.detail_section` are canonical. Rendering is the conjunction of an exact static plugin/version/contribution, a live authenticated `/system/extensions` availability row, the shell's `workspaceCapabilities` projection, and normalized UI mode. The empty production registry short-circuits before every request/controller/timer and returns exact `null`. The server projection is metadata-only and closed; UI code is static and receives readonly summaries rather than page setters or domain-owner internals. Cross-stack parity tests include non-empty frontend-only/backend-only/plugin/id/slot/capability mismatches so empty registries cannot make the guard vacuous.
