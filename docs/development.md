...
- The modular-extension Phase 0 boundary is now enforced. Stable cross-layer values live in `backend/app/domain`, which cannot import services, repositories, API, infrastructure, extensions, or feature implementations. `backend/app/repositories/ports.py` cannot import `app.services.*`, and the backend static import graph must remain acyclic. `backend/app/extension_sdk` defines dependency-light manifests, the five distinct contribution kinds (Provider, ProviderChain, Contributor, Auditor, Observer), minimal context/result primitives, and sanitized failures; `backend/app/extensions` owns composition and the startup-frozen topology. Availability remains request-time/live. The Phase 0 registry is empty and existing workflows do not consume it, so the no-plugin path is the original path. `scripts/check_architecture_boundaries.py` runs directly in the G1 contracts lane and also freezes the current public repository-facade surface. See [the modular plugin architecture design](./modular-plugin-architecture-design-2026-08-21.md) before adding an extension point.
