...
- The modular-extension Phase 0 boundary is now enforced. Stable cross-layer values live in `backend/app/domain`, which cannot import services, repositories, API, infrastructure, extensions, or feature implementations. `backend/app/repositories/ports.py` cannot import `app.services.*`, and the backend static import graph must remain acyclic. All existing repository-to-service imports are partitioned into SQLite/PostgreSQL/other debt ceilings that may fall but not grow. `backend/app/extension_sdk` defines dependency-light manifests, the five distinct contribution kinds (Provider, ProviderChain, Contributor, Auditor, Observer), minimal context/result primitives, and sanitized failures. Capability `requires` are separate from plugin `depends_on`, and contributions default to stable ID order instead of integer priority. `backend/app/extensions` owns composition and the startup-frozen topology. Availability remains request-time/live. The Phase 0 registry is empty and existing workflows do not consume it, so the no-plugin path is the original path. `scripts/check_architecture_boundaries.py` runs directly in the G1 contracts lane and permits the current public repository-facade surface to shrink but not grow. See [the modular plugin architecture design](./modular-plugin-architecture-design-2026-08-21.md) before adding an extension point; review and merge these PRs through [the delivery pipeline](./modular-plugin-architecture-delivery-plan-2026-08-21.md), which requires two independent subagent reviews and green CI before squash merge.
