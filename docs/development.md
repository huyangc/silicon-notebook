...
- The parser ProviderChain now freezes a self-hosted MinerU → MinerU cloud → built-in DAG at startup. Links declare `after`/`before` edges and stable-ID ties instead of integer priority. Core route policy runs before live availability; plugin probes receive no persistence/asset port, and only a successful core admission may invoke the core materializer. This topology is deliberately dormant in PR-04: production ingestion remains physically disconnected and the legacy dispatcher is still its sole route until PR-05 switches once and removes it. Do not introduce an optional dual route.
