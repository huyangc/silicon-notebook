...
- `SelectedSourceGraphActivationService` is the only Ask/Report consumer seam. Callers must finish and freeze historical `B` before invoking it; the service reads only a server-frozen, genuinely narrowed `include` scope, builds the bounded snapshot, tries online scoped PPR/neighbor memberships and then the source-partition companion when needed, rechecks every returned source id, and passes `G` through `BaselineProtectedEnrichmentService`. Whole/all-selected scopes return before snapshot I/O. Default invisible shadow returns `B`; approved active modes return `B + G`; every failure returns `B`. Status is internal observability only and may not enter public payloads, traces, streams, or UI. Do not add a second rollout parser, direct graph consumer, or client-computed narrowing rule.
...
The current schema version is 53. This is the SQLite schema version. The committed v9 compatibility fixture
...
- Group-facing panels (`frontend/app/groups-panel.tsx` and the "共享给群组" section in `frontend/app/notebook-group-share.tsx`) share one `.group-*` class set in `globals.css`. A row's horizontal layout belongs to `.group-row`, never to an inline style at the call site: these rows used to be the block-level `.checklist-row` plus inline `alignItems`/`gap`, which are silently inert on a block box and collapsed each row into one unspaced run of text. `.checklist-row` itself stays a block — other call sites use it as a text row. Read-only labels use `.group-chip`, not the 42px solid primary-button `.new-pill`; the expanded group's detail is a `.group-detail` card whose title is an `h3` rather than the uppercasing `.section-title`. `frontend/tests/guards/group-layout-guard.test.mjs` is the regression gate; the full rationale lives in the AGENTS.md frontend chapter.
...
- An access-rights change must reconcile the **open workspace**, not just the list. The groups modal opens from the workspace header too, and leaving a group / being removed / the group being deleted / a share being revoked can pull the notebook the user is standing in out from under them. Refreshing `notebooks` alone is not enough — the workspace renders from the separate `currentNotebook` state and does not move, leaving the user inside a notebook they can no longer read with no visible response until a manual reload. `page.tsx`'s `reconcileOpenNotebook(remaining)` is the one implementation, shared by "leave read-only share" and the groups modal's `onChanged`; the test is "is it still in the refetched list", not "was it the target of that action" — the same notebook can arrive through several paths (another group, direct membership, an `everyone` edge), so dropping one does not necessarily end access. A revocation made elsewhere (someone removes you, deletes the group, or revokes the share) has no push channel and is re-checked through the same path when the tab becomes visible again (throttled; a failed fetch skips reconciliation entirely) — best effort, not a guarantee. Gate: `frontend/tests/guards/group-sharing-guard.test.mjs`.
