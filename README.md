...
- KG extraction persists immutable source-generation facts and normalized evidence-element bindings before global fusion. A backend-neutral, generation-keyed selected-source snapshot can read only authorized objects, relations, chunks, facts, evidence bindings, and cluster memberships through bounded, source-first queries whose cache-hit identity is constant-cost. Shadow-only local graph tools cover neighbors/expansion, two-hop chains, relation recall, exact sections, cursor enumeration, and default-off sparse PPR built inside that snapshot; PPR reset, degree normalization, cluster routers, chunk ranking, and hydration never include an excluded source or a whole-library CSR. For a source too large for the online snapshot rail, an optional scale-index companion publishes one identity-bound CSR partition per visible source and lazily opens only selected partitions; legacy, missing, or stale companions fail closed instead of traversing the whole graph. A protected dual lane freezes the historical baseline first and admits graph chunks only under an independent shadow budget; duplicate chunks merge provenance without changing baseline content, score, position, or citations, and any baseline eviction discards the whole enrichment lane. Ask/Report still have no graph consumer, so current ranking, prompts, and answers remain unchanged.
