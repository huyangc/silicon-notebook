...
Retrieval scope is chosen in the source sidebar along two independent dimensions shared by Ask and new Deep Reports, both fully selected by default: one checkbox per imported source of the current notebook, and one per *whole* mounted reference library. The source selection is a hard active-notebook ceiling that model-planned source scope may only narrow; unchecking a reference library keeps it out of that run without reducing the current notebook's own retrieval. Knowledge-graph availability follows the same selection: a notebook with no graph of its own whose graph-bearing libraries are all unchecked reports no knowledge graph for that run rather than traversing one the run may not read. `全选` / `清空` act on both dimensions at once. The Ask composer and new-report controls are disabled only when both dimensions are empty — no local evidence left in scope *and* no selected reference library. "No local evidence" is not the same as "no imported file selected": a notebook whose content lives in Knowhow tables or confirmed Memory has no visible source rows and stays fully askable.
