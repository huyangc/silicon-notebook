...
- On a large PostgreSQL deployment, inspect and install the notebook-aware lexical GIN indexes with `PYTHONPATH=backend python scripts/build_postgres_retrieval_indexes.py` and the same command plus `--apply`. The online builder uses `CREATE INDEX CONCURRENTLY`, is resumable, and leaves the legacy global indexes in place by default. It changes only candidate pruning, not lexical predicates, scores, limits, or result ordering; see [Operations](./docs/operations.md#postgresql-notebook-aware-lexical-indexes). For the largest notebooks, the default-off `POSTGRES_LEXICAL_KNN_ENABLED` flag additionally enables a GiST `<->` early-stop probe for the knowledge-object name arm (same doc section for the index DDL and rollout gates).
