...
- Text wrapped in **ASCII double quotes** is searched whole and never tokenized: the span enters lexical candidates as one indivisible term, counts as a single unit of keyword coverage (a document scattering those words earns nothing for it), and additionally earns an exact-locate probe. Quoting is a strong preference, not a hard filter. ASCII quotes only, at least 3 characters inside, and more than 4 quoted spans in one text disables the syntax for that text; the ask composer and deep-report input echo the recognised phrases as you type so a constraint that did not take effect never passes silently.
