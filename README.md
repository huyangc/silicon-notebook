...
6. Share a notebook by link: small notebooks are copied; large notebooks are joined read-only. A group admin can instead share it with a whole group (project / department / domain), which puts it in each member's **群组** list partition and lets them mount it. Live collaborative editing is not part of the beta. A shared library's workspace header names where it came from ("只读／可管理 · 来自群组《X》") without ever hiding the library's own name, and if you leave a group, are removed from one, or a share is revoked while you have an affected library open, the UI moves you out on the spot — no manual reload. Deleting a group only withdraws access; the libraries themselves stay with their owners.
