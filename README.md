...
Each imported source has a retrieval checkbox shared by Ask and new Deep Reports. All visible sources are selected by default; clearing them makes the active notebook's local retrieval scope empty, while mounted reference libraries remain available. With neither a selected local source nor a mounted reference library, the Ask composer and new-report controls are disabled.
