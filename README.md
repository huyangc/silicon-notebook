...
- Multi-account ownership, public reference libraries, share links, copy/read-only membership, group knowledge sharing (groups with a two-level in-group role, authorization edges, read access implying mountability), and admin controls, including a paginated, sortable user-usage table whose Ask usage counts durable question submissions rather than conversation containers. The user total includes questions asked in joined notebooks; the expanded inventory remains limited to notebooks that user owns.
