...
- Two-tier knowledge base: each notebook has a `tier` (`base` | `personal`, default `personal`). `base` is the authoritative reference KG (e.g. an analog-design textbook); `personal` is the user's own notes. `federated_retrieve` gathers candidates across `base ∪ active personal` and tags each hit with its tier, but ranking remains tier-blind: relevance scores are unchanged, and `base` is the secondary ordering key only when their relevance scores are exactly equal. A higher-scoring personal hit still wins. Separately, when base and personal evidence contradict during answer synthesis, the answer defers to the base position and surfaces the discrepancy. Citations carry their tier (`AnswerAnchor.tier`) and Ask renders a `base`/`personal` badge per cited anchor. The notebook actions menu ("分析") offers "设为基准库 / 取消基准库" to mark a notebook as the base KG and back (via `POST /api/notebooks/{id}/tier`)
...
- Main column: two tabs — **Ask** (grounded Q&A with `[k_i]` sentence citations rendered as clickable numbered references, including valid numeric groups such as `[1, 2, 3]`, across three retrieval modes — see [Retrieval modes](#retrieval-modes-ask) — multi-turn conversation list, live collapsed reasoning trace with expandable details, 👍/👎 feedback) and **Knowledge** (browse any object type dynamically from `/knowledge-types`, with status lifecycle, duplicate detection, and conflict detection). In Ask, `Enter` submits, `Shift+Enter` keeps a newline, and while a model response is running the input/mode controls are locked while the send button becomes an interrupt control. Navigation, refresh, or transport loss stops delivery to that browser, but the detached Ask job continues in the background and may persist its final answer. Clicking interrupt is a distinct cancellation action: the client calls `POST /api/notebooks/{id}/ask/jobs/{job_id}/cancel`, which sets the backend cancellation event so the worker/LLM path stops and does not save a cancelled final answer. The inactive Studio right sidebar is not shown in the primary workspace, so the Ask panel can use the freed width.
...
`POST /ask` dispatches on `mode` — the registry `backend/app/services/ask_modes.py` is the single source of truth (default `chunk`). Every mode federates retrieval across `tier=base` ∪ the active personal notebook, emits sentence-level `[k_i]` anchors, and grades grounding identically: `classify_evidence` → `grounded` / `overview` / `inferred` against the calibrated `EVIDENCE_TAU_*` thresholds. **Ranking signals such as rerank and RRF only reorder candidates — they never feed the grounding threshold** (which reads each item's fused keyword+semantic relevance). Tier does not change a score; `base` is used only as the secondary key on an exact score tie.
