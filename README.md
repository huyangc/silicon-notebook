...
- KG extraction persists immutable source-generation facts and normalized evidence-element bindings before global fusion. A backend-neutral, generation-keyed selected-source snapshot can read only authorized objects, relations, chunks, facts, evidence bindings, and cluster memberships through bounded, source-first queries whose cache-hit identity is constant-cost. Shadow-only local graph tools now cover neighbors/expansion, two-hop chains, relation recall, exact sections, and cursor enumeration. A protected dual lane freezes the historical baseline first and admits graph chunks only under an independent shadow budget; duplicate chunks merge provenance without changing baseline content, score, position, or citations, and any baseline eviction discards the whole enrichment lane. Ask/Report still have no graph consumer, so current ranking, prompts, and answers remain unchanged.
