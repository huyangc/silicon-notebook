...
- Graph object types use an administrator-managed global baseline with per-notebook customization. Notebook owners can override inherited labels, fields, descriptions, or status and create notebook-only types; removing an override restores inheritance, while read-only members can inspect the effective definition without changing it.
