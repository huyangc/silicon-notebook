...
- `--workers` — source-pipeline concurrency; falls back to `KG_JOB_CONCURRENCY`. It dispatches source jobs in `all` and file parsing in `ingest`. In `vectors-to-blob`, it instead remains the parse/re-encode process-pool size (default `min(8, cpu_count())`; `1` disables that pool).
