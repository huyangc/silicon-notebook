...
A barrier-aware verifier now compares a SQLite source snapshot with a PostgreSQL repeatable-read/read-only snapshot, excludes only keys proven concurrent by the retained change log, and performs structural, domain, embedding, bilingual-retrieval, and frozen-cutover gates with redacted reports. This is still only a migration primitive: without the operator CLI and supervised worker, `SHADOW_DATABASE_URL` remains inert.
