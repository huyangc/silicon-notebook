...
SQLite schema v31 contains only the inert, payload-free internal tables needed by run-scoped forward-shadow capture. Capture/freeze triggers and the four logical-key guards are absent by default and are installed only by explicit migration control primitives. Once installed, the guards immediately enforce uniqueness; capture/freeze behavior remains disabled until the run control state enables it. Setting `SHADOW_DATABASE_URL` alone still enables nothing. PostgreSQL schema v9 is the paired business schema. The temporary shadow module now has a strictly read-only, UTF-8-first preflight, identity-bound control/guard primitives, atomic SQLite snapshots, and bounded resumable 60-table PostgreSQL baseline COPY with strict transforms, explicit historical ordinals, sequence reseeding, and an atomic H0 checkpoint. PostgreSQL control mutations acquire the migration/control locks and exact catalog validation. It still has no incremental replicator, operator CLI, or end-to-end migration worker; do not treat these primitives as a usable migration command or change `DATABASE_URL` because they exist.
