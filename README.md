...
- Deep report (background job): a notebook-level "深度报告" action turns one question into a multi-section technical report. An LLM plans an outline, each section runs a full `reasoning` deep-dive independently (sections run in parallel, each with its own retrieval budget), each is drafted with a three-tier evidence discipline (`[k]` in-corpus citation / `（推断）` in-corpus inference / `【通识】` general-knowledge, marked and flagged unverified), then a summary pass adds an executive summary, references, an auto-detected **knowledge-gap** section (dry sub-queries + unconnected cross-section concept pairs + unsupported sections), and the analysis plan. A "faster ↔ smarter" slider (`depth`, per-section reflect-step budget, default 2) trades depth for latency; sections deep-dive in parallel up to `KG_JOB_CONCURRENCY`, and the UI shows live per-section progress (`section_status`). Runs as a cancellable background job with progress; results persist and export as `.md`
...
- Deep report: `POST /api/notebooks/{id}/reports` body `{question, depth?}` (`depth` 1–16, default 2 — per-section reflect-step budget, i.e. the "faster ↔ smarter" slider) → `{report_id}` (starts a background job; 409 if the reasoning LLM is unconfigured), `GET /api/notebooks/{id}/reports` (list), `GET /api/notebooks/{id}/reports/{rid}` (poll status + `content_md` + live `section_status`), `POST .../reports/{rid}/cancel`, `DELETE .../reports/{rid}`. Sections deep-dive in parallel up to `KG_JOB_CONCURRENCY`.
