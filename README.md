...
- `RepositoryFacade` is backend-neutral over an injected `RepositoryRuntime` bundle; `SQLiteRepository` is the narrow compatibility wrapper that owns SQLite migration and maintenance adapters. Application services do not assemble product SQL. Stores own product SQL and raw row selection; established application/query components may assemble domain/application projections such as `NotebookSummaryQuery.from_row`. SQLite stores share one `SqliteDatabase` connection factory/write lock and one version-gated `SqliteMigrator`, while services retain sequencing and policy. Every facade operation is an explicit compatibility adapter or one of the source-checked one-hop delegates whose real target must match the ownership manifest. Consumers use executable, consumer-specific Protocols from `backend/app/repositories/ports.py`; dependencies point one way — factory/wrapper → facade → runtime → services → stores. `sqlite_identity.py` and `sqlite_notebook_sharing.py` remain compatibility re-export shims, and the legacy request-context, `_COPY_CHUNK`, and `_remap_json_ids` exports stay importable.
- `RepositoryRuntime` owns or references composed runtime state; `REPORT_CANCELLATIONS` remains the intentionally process-global canonical owner, and the runtime, report coordinator, and module compatibility functions share that same identity reference. Other mutable operational state (storage root, embedder, language caches, build sets, Ask cancellation registry, and artifact caches) is runtime-owned; replacing supported compatibility properties after composition updates every retained consumer. Every Ask entry surface uses the durable job/final-save lifecycle. Blocking `POST /ask` and MCP `ask_notebook` keep that job id internal and return only their normal answer payload; the NDJSON stream alone exposes `job_id`, progress, explicit cancellation, and detached recovery behavior. Synchronous Ask/report submission failures mark the already-created durable job/report failed, unregister the cancellation entry, and re-raise the submission error; successful worker ordering and the existing Ask transaction checkpoints remain unchanged.
- Databases created before the refactor keep loading unchanged. `scripts/verify_repository_snapshot.py` uses exact per-version migration and stable-seed manifests, percent-encodes SQLite URI paths, constructs the repository only on a temporary backup, and reports the retained backup path if cleanup fails without printing private rows. It guards the original database/WAL metadata plus SHM existence and size; for a live WAL attachment only SHM mtime is exempt because SQLite may rebuild it.
  For the v24 cluster cleanup, the verifier streams the exact BINARY-ordered survivor projection into a bounded count/digest and permits only that deterministic duplicate removal; a wrong survivor, changed field, or unrelated deletion still fails closed.
...
`ask_notebook` runs through the same hidden durable Ask job and atomic final-save
lifecycle as blocking HTTP Ask, but its MCP result contains no `job_id` and does
not expose the stream's progress, reconnect, or explicit-cancel protocol.
...
- `POST /api/notebooks/{id}/ask` — blocking grounded Q&A with `[k_i]` citations (`mode`: `chunk` default | `graph` | `reasoning`; federation follows the mode-specific boundaries above). It uses a hidden durable job and atomic final save, but the public `AskResponse` has no `job_id` and this endpoint does not provide stream reconnect/progress semantics
...
- `GET /api/notebooks/{id}/conversations`, `GET|PATCH|DELETE /api/conversations/{id}`; explicit delete returns 409 while that conversation has a running Ask, otherwise atomically purges its answers and terminal Ask job/trace data
- `DELETE /api/notebooks/{id}/conversations?older_than_days=N` — bulk cleanup returns both the backward-compatible `deleted` count and authoritative `deleted_ids`; conversations refreshed after the client snapshot or still used by a running Ask are skipped
...
DATABASE_URL            # formal repository URL (default SQLite at .local/silicon_notebook.db)
SHADOW_DATABASE_URL     # parsed only; remains inert until a later shadow-sync phase
