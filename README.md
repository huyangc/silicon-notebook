...
- Intent-first two-stage deep reports with a frozen shared scope/completeness contract, transparent "资料基础" and citation-coverage disclosures, optional reviewable comparison frames, and (at detailed/exhaustive depth) retrieve → report-wide synthesis blueprint → parallel section drafting. Structured additions fail open; the final editor audits but never rewrites the body. Completed reports retain exact browser-local completion timestamps, relative age, and full generation duration; legacy reports never invent missing timing.
