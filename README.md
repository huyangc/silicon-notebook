...
- Deep report: `POST /api/notebooks/{id}/reports` body `{question, depth?}` (`depth` 1–16, default 2 — per-section reflect-step budget, surfaced as five named levels 概览/标准/深入/详尽/穷尽) → `{report_id}` (starts a background job; 409 if the reasoning LLM is unconfigured), `GET /api/notebooks/{id}/reports` (list), `GET /api/notebooks/{id}/reports/{rid}` (poll status + `content_md` + live `section_status`), `POST .../reports/{rid}/cancel`, `DELETE .../reports/{rid}`, `POST .../reports/export` body `{report_ids}` → a `reports.zip` of the selected done reports. Sections deep-dive in parallel up to `KG_JOB_CONCURRENCY`.
