...
Projection status is a table-completion contract, not a per-row progress shortcut: rows remain `pending`/`syncing` until the table-wide chunks, embeddings, knowledge objects/relations, mutation sequence, and graph-cache notifications have finished. A row is published as `synced` only after that terminal work succeeds, so callers that observe every row settled can immediately read the completed graph without a scheduling race.
