...
当前目标是可供真实团队使用的本地 beta：后端采用 FastAPI，并可选择 SQLite 或 PostgreSQL repository；前端采用 Next.js。发行默认的 SQLite 快速启动不要求 Docker、GPU、数据库服务或本地模型服务；选择 PostgreSQL 时需要可访问的 PostgreSQL 服务。OpenAI 兼容的聊天、嵌入、重排和 MinerU 服务都是可选的 URL 集成；未配置时，确定性降级仍可维持核心流程。模块化扩展底座现已建立低依赖的 `domain` 契约层、启动冻结的 Extension SDK registry、实时最小授权 capability 判定，以及 Ask/Report 共用且保护 baseline 的 retrieval contributor host。Selected-source graph activation 已成为首个内建 contributor：它通过请求级窄 capability 保留原有 attestation、rollout、scope drift、独立预算、baseline manifest 与整段 fail-closed 合同；generated-question 在其独立迁移前仍走原路径。一个 capability 不可用时只关闭对应 contribution，不能跳过共享 host 或吞掉其他 contributor 的输出。没有适用 contribution 时，host 仍在任何工作前原样返回 baseline。Point-specific proposal port 与 core 权威 reader 分离：graph proposal 从请求内存复验，其他被接纳 proposal 只走一次带 notebook/source SQL 上限的批量读取。Report fallback 读取仍共用 retrieval leaf gate；兼容请求即使省略 scope，私有 Memory 也始终在 SQL 内按 actor 过滤。Contribution 默认按稳定 ID 排序，manifest 中 capability requirement 与插件依赖保持为两个独立概念。模块化架构 PR 按 `docs/` 下的交付计划执行：两路独立 subagent review 与 CI 全绿后才 squash merge。
