...
- KG 抽取会在全局融合前持久化不可变的来源代次事实及规范化证据元素绑定。后端无关、按代次缓存的所选来源快照通过有界、来源优先且缓存命中身份为常量成本的查询，只读取获授权的对象、关系、chunk、事实、证据绑定和 cluster membership。当前已有仅供 shadow 的邻居/扩展、两跳 chain、关系召回、精确小节与游标枚举原语。受保护双通道先冻结历史 baseline，图 chunk 只使用独立 shadow 预算；重复 chunk 只合并 provenance，不改变 baseline 正文、分数、位置或引用，任何 baseline 驱逐都会丢弃整段增强。Ask/Report 仍未消费图增强，因此当前排序、prompt 与答案不变。
