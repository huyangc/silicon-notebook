...
- KG 抽取会在全局融合前持久化不可变的来源代次事实及规范化证据元素绑定。现在已有后端无关、按代次缓存的所选来源快照，通过有界、来源优先且缓存命中身份为常量成本的查询，只读取获授权的对象、关系、chunk、事实、证据绑定和 cluster membership；它尚未接入 Ask/Report，因此当前排序和通道选择不变。
