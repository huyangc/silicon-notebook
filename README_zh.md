...
- 意图优先的两阶段深度报告：冻结共用的范围/完整性合同，以 SQL 聚合的有界「资料基础」和引证覆盖率透明披露语料边界；身份未知来源逐份可见，集中度按保守上界计算；可选、可确认的比较框架；在详尽/穷尽档按「并行检索 → 全篇综合蓝图 → 并行分节写作」生成，低档保持逐节流水线。分节撰写、全篇综合和最终终审分别使用部署可配置的输出预算，整篇准入与单篇节级扇出则独立限制数据库压力，不拿模型容量直接放大连接需求。有信息量的综合失败/跳过与账本可用性对用户可见并安全回退，预期的低档 `not_requested` / `0/N` 回执默认静默；没有任何有效正文的运行落为失败而非假完成，保留已确认大纲的失败报告可在详情页原地重试。终审只接收合法有界的跨节上下文、只审计不改正文。已完成报告保留浏览器本地精确完成时间、相对时间与完整生成阶段耗时；旧报告不会编造缺失时间。
...
- KG 抽取会在全局融合前持久化不可变的来源代次事实及规范化证据元素绑定。后端无关、按代次缓存的所选来源快照通过有界、来源优先且缓存命中身份为常量成本的查询，只读取获授权的对象、关系、chunk、事实、证据绑定和 cluster membership。当前仅供 shadow 的局部图工具覆盖邻居/扩展、两跳 chain、关系召回、精确小节、游标枚举，以及完全在该 snapshot 内构建、默认关闭的稀疏 PPR；PPR reset、度数归一化、cluster router、chunk 排名和 hydration 均不包含被排除来源，也不先切整库 CSR。单个来源若大到越过在线 snapshot 护栏，可选的 scale-index 伴生产物会为每个可见来源发布独立且绑定主索引 identity 的 CSR partition，运行时只惰性打开被选 partition；旧版、缺失或陈旧伴生产物一律 fail closed，绝不遍历整库图。受保护双通道先冻结历史 baseline，图 chunk 只使用独立 shadow 预算；重复 chunk 只合并 provenance，不改变 baseline 正文、分数、位置或引用，任何 baseline 驱逐都会丢弃整段增强。Ask/Report 仍未消费图增强，因此当前排序、prompt 与答案不变。
...
所选来源图的激活还受离线配对质量门保护：baseline 与 shadow 必须使用完全相同的冻结模型、采样、语料与来源绑定；硬隔离或 baseline 保留失败、任一案例质量回退、成本越界都会生成不批准且不含正文的 attestation。本阶段仍为内部能力，不改变问答或深度报告输出。
