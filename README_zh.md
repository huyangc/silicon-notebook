...
- 带紧凑引用的多轮问答，会话历史按最近活动排序（首轮生成中会话即使立即切走也可重新打开）；问题和回答气泡都支持悬停显示、点击固定浏览器本地时间，其中问题采用网页端提交瞬间，回答采用权威持久化完成时间；支持 `chunk`、`reasoning` 和实验性的 `graph` 检索模式。逐步推理会在检索前做不受语料影响的问题理解：意图清晰时自动继续，存在会改变检索方向的歧义时先请用户确认，确认后的合同支配所有检索阶段。实时推理轨迹覆盖整轮——从问题理解一直到答案生成——而不只是检索阶段。
...
- 词法检索保留整句精确匹配作为排序加分，但会独立召回拉丁字母/数字词项及重叠中文三字片段，不再强制整段查询连续出现；SQLite 安全引用 FTS5 clause，PostgreSQL 应用相同的有界词项并集并转义 LIKE 元字符，使 `set_db` 这类词项保持字面量。带索引的 Chunk 与 KG 检索使用有界的 `ANN ∪ FTS` 候选；带索引的 Relation 检索还会按方向平衡补入与 FTS 命中 KG 端点相邻的有界关系候选。
