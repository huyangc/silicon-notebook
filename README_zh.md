...
- 带紧凑引用的多轮问答，会话历史按最近活动排序（首轮生成中会话即使立即切走也可重新打开）；引用卡可经当前笔记本打开精确原文元素，即使证据来自已挂载的公共参考库；论文标题与上传文件名不同时会另外标明原始文件，绝不显示 MinerU 中间产物名。问题和回答气泡都支持悬停显示、点击固定浏览器本地时间，其中问题采用网页端提交瞬间，回答采用权威持久化完成时间；支持 `chunk`、`reasoning` 和实验性的 `graph` 检索模式。逐步推理会在检索前做不受语料影响的问题理解：意图清晰时自动继续，存在会改变检索方向的歧义时先请用户确认，确认后的合同支配所有检索阶段。问题明确点名 manual/文件来源时，意图预检会在读取证据前解析并确认严格来源集合；运行中不得扩大范围，名称不存在或不唯一时失败关闭，绝不退回全库搜索。来源范围审阅是短时有效的服务端签发能力：伪造、过期或未明确确认的范围会在创建持久 conversation/job 前被拒绝。超出首轮宽度的已确认必答主题种子会在步骤预算内顺延执行而非被丢弃，仍未覆盖的方向会在轨迹中披露；跨工具映射类问题会为每个被点名工具生成独立必答主题，目标侧检索配目标工具名与功能描述词。实时推理轨迹覆盖整轮——从问题理解一直到答案生成——而不只是检索阶段。
...
- 意图优先的两阶段深度报告：冻结共用的范围/完整性合同，以 SQL 聚合的有界「资料基础」和引证覆盖率透明披露语料边界；身份未知来源逐份可见，集中度按保守上界计算；可选、可确认的比较框架；在详尽/穷尽档按「并行检索 → 全篇综合蓝图 → 并行分节写作」生成，低档保持逐节流水线。有信息量的综合失败/跳过与账本可用性对用户可见并安全回退，预期的低档 `not_requested` / `0/N` 回执默认静默；终审只接收合法有界的跨节上下文、只审计不改正文。已完成报告保留浏览器本地精确完成时间、相对时间与完整生成阶段耗时；旧报告不会编造缺失时间。
...
检索范围在来源侧栏按**两个互相独立的维度**选择，由问答与新建深度报告共用，且都默认全选：当前笔记本每个导入来源一个复选框，每个已挂载参考库**整库**一个复选框。来源选择是当前 notebook 的检索硬上限，模型规划的来源范围只能进一步缩小；取消勾选某个参考库只是让它不参与本轮检索，不会削弱当前笔记本自身的检索能力。知识图谱的可用性也跟着这份选择走：本身没有图、又把带图的参考库全部取消勾选的笔记本，本轮按“没有知识图谱”处理，而不是去遍历一份本轮不许读取的图。“全选”/“清空”一并管两个维度。只有两个维度**同时**为空——本地已经没有可检索证据，也没有勾选任何参考库——问答输入与新建报告控件才会被禁用。“本地没有证据”不等于“没有勾选任何导入文件”：内容放在 Knowhow 表或已确认 Memory 里的笔记本本来就没有可见来源行，它照常可以提问。
