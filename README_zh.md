...
- 与笔记本绑定、仅创建者可见的 Memory；Ask 引用会保留并展示来源、位置、摘录和原始文件身份，同时通过受限 MCP 向外部 Agent 提供访问。该接入面涵盖检索、问答、knowhow、引用还原，以及在独立的 owner-only scope 之后的来源归档/重新解析/删除与构建触发；Agent 只能删除 Agent 自己添加的来源，而 knowhow 格子代码写入刻意保持 scope 驱动而非 owner-only，只读成员也可写。新签发 token 旁会显示公开、机器可读的接入说明链接，用户可把链接与 token 分开交给 Agent，由它自行配置 MCP。
...
- 多账号所有权、公共参考库、分享链接、复制/只读成员、群组知识共享（群组与组内两级角色、授权边、读权⇒可挂载）、把一份已完成的深度报告或一条已完成的多轮问答会话发布成免登录只读链接（会话既可从历史列表整条分享，也可从某条回答下的分享按钮只分享到那条回答为止），和管理员控制；用户使用总览支持分页和按表头排序，其中问答用量展示已创建持久任务的用户提问次数，而不是会话容器数量。用户总数包含其在加入笔记本中的提问，展开明细仍只列该用户拥有的笔记本。
