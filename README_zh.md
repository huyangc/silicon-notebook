...
- 逐步推理还能按需列出（而非只做相关性排序）全库的公式/表格/图片/代码块清单，以及概念/论断/公式/过程知识对象清单，每份清单都带“已列出/总数”完整性徽章；截断时明确标注为部分结果，同一轮推理内可继续列出。细节见[产品与 API 参考](./docs/product-and-api_zh.md#集合枚举工具)。
