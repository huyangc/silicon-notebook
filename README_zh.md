...
前置:`.env` 配好 EMBED 与 `KG_LLM`(KG 抽取缺省回退全局 `OPENAI_COMPAT_*`)。EMBED 未配时 CLI **默认拒绝运行**——要无向量导入须显式 `--allow-no-embed`(此时跳过 chunk/KG 向量),绝不静默;KG 抽取在无可用 LLM 时报错。中断后重跑会自动续，但要清楚续跑依据是什么：**续跑是从数据库状态推导出来的，不依赖任何进度文件**。各子命令各问各的问题——`ingest` 看内容哈希**并且**看该来源是否真的产出了 elements，`kg` 看该来源最近一次 KG 抽取是否完成，`embed` 看每条向量在不在。所以中断后无需任何参数即可接着跑。`<storage>/batch_ingest/<notebook>.jsonl` 是**只写的运行日志**，供事后查看，没有任何代码会读回它。
