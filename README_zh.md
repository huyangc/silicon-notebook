...
- 意图优先的两阶段深度报告：检索前先做不接触语料的问题澄清，并通过原子确认冻结用户已审阅的合同；大库也从有界 chunk 候选恢复精确元素，再提供可编辑的覆盖大纲、优先显示解析论文名且另存原始上传文件名的引用、真实 grounded 校验、分节推理、实时进度、浏览器本地精确完成时间、相对时间与确认大纲后完整报告生成阶段的总耗时、取消，以及 Markdown/ZIP 导出。
