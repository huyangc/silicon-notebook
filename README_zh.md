...
检索范围在来源侧栏按**两个互相独立的维度**选择，由问答与新建深度报告共用，且都默认全选：当前笔记本每个导入来源一个复选框，每个已挂载参考库**整库**一个复选框。来源选择是当前 notebook 的检索硬上限，模型规划的来源范围只能进一步缩小；取消勾选某个参考库只是让它不参与本轮检索，不会削弱当前笔记本自身的检索能力。“全选”/“清空”一并管两个维度。只有两个维度**同时**为空——本地已经没有可检索证据，也没有勾选任何参考库——问答输入与新建报告控件才会被禁用。“本地没有证据”不等于“没有勾选任何导入文件”：内容放在 Knowhow 表或已确认 Memory 里的笔记本本来就没有可见来源行，它照常可以提问。
