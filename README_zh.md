...
- 大型 PostgreSQL 部署应先用 `PYTHONPATH=backend python scripts/build_postgres_retrieval_indexes.py` 检查 notebook-aware 词法 GIN 索引，再给同一命令追加 `--apply` 在线安装。工具使用 `CREATE INDEX CONCURRENTLY`、可续跑，默认保留旧的全局索引；它只改候选裁剪，不改词法谓词、分数、limit 或结果顺序。详见[运维文档](./docs/operations_zh.md#postgresql-notebook-aware-词法索引)。
