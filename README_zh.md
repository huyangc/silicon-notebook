...
前端 workspace 正按状态所有权拆分且不改变产品行为：`use-source-library.ts` 已在显式 user/notebook identity transition 后独占来源列表/范围、详情、删除 tombstone、重解析与解析轮询，`page.tsx` 继续作为壳层编排器并保持既有请求次数。
