...
- 意图优先的两阶段深度报告：冻结共用的范围/完整性合同，透明披露「资料基础」和引证覆盖率；可选、可确认的比较框架；在详尽/穷尽档按「并行检索 → 全篇综合蓝图 → 并行分节写作」生成。新增结构失效时安全回退，终审只审计、不改正文。已完成报告保留浏览器本地精确完成时间、相对时间与完整生成阶段耗时；旧报告不会编造缺失时间。
