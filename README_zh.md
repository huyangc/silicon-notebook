...
现有 22 个内建 MCP 工具现由固定 capability bundle 在一个薄 `mcp_server` 组合根中显式注册。工具顺序、schema、实时 token/成员权复核、owner-only 写闸、Memory candidate 审核边界、progress 行为及 repository/model I/O 均保持不变。这只是内部高内聚重构，不开放动态工具注册表或 `agent.tool_provider` 接口。
