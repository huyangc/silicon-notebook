...
- 两阶段深度报告：大纲编辑、分节推理、实时进度、取消，以及 Markdown/ZIP 导出。
...
SQLite schema v31 目前只加入 run-scoped forward-shadow capture 所需的两张惰性、无 payload 内部表。capture/freeze trigger 与四个逻辑键 guard 默认不存在，只能由显式迁移控制原语安装；guard 一经安装就立即强制唯一性，capture/freeze 行为则在 run control 状态启用前保持禁用。仅设置 `SHADOW_DATABASE_URL` 仍不会启用任何同步。配对的 PostgreSQL 业务 schema 为 v9。临时 shadow 模块目前已有 preflight/control/guard、原子 SQLite snapshot、有界可续跑的 60 表 baseline COPY/H0，以及 fail-stop 单消费者正向 apply 引擎。它连续消费 SQLite 全局 seq，在短只读 snapshot 中仅为 upsert hydration 当前行，delete 保持 key-only、hydrated bytes 为零，并保留历史 ordinal，每批目标收敛状态与 checkpoint 同事务提交。同一 stable key 在 accepted prefix 内按最后 seq 合并，raw seq 校验/checkpoint 仍连续，每个 identity 以最终 actual apply 覆盖 synthetic dependency contribution；只有 dependency-only identity 才引用计数一次 synthetic 行及其 bytes。短读窗口若在 allocated high-water 之前结束，会在 hydration/apply 前立即判定 suffix gap；满窗口低于 high-water 时还会在同一 snapshot 探测相邻 seq，缺失即失败。批次硬上限为 4096 events/64 MiB；仅一个 final bundle 可独占超出 byte cap，同 key replacement 若在已有其他 actual bundle 时使 bytes 超限，会回滚该 replacement 并留到下一批；FK 父闭包只读取同一个已验证 source snapshot，每事件最多 64 行（固定 v9 图最多需要 9 个 row slots）、计入 bytes，并在批内去重，不扫描后续 change-log metadata。目标仅延后 FK/UNIQUE ordering SQLSTATE，CHECK/NOT NULL 立即 poison。由精确 PG9 catalog 派生的停车方案覆盖全部 82 个 unique surface：nullable 列使用 NULL；无 FK/CHECK 的 text/bigint 列使用按其他唯一列的非 NULL 等值/NULL `IS NULL` 与固定 predicate 限定范围的确定性候选（`C` collation 文本最大值拼接 `chr(1)`，或先走可索引的 bigint MIN/MAX 快速路径选择 min−1/max+1，仅在两个 int64 边界都已占用时扫描首个 gap）；仅对静态证明无入向 FK 且批内有 current-final 恢复行的叶表允许同事务 delete/reinsert。每个 stagnant pass 会停车所有可独立停车的冲突。最多 8 passes、每 apply 32 条实际 statements、总计 16384 条实际 statements，每次候选查询都计入预算，`ProgramLimitExceeded`/`DataError` 候选失败及候选搜索、候选 UPDATE 或 ordering 容量耗尽保持 non-poison；`QueryCanceled` 仍是整事务重试的瞬态错误。`run_forever` 遇 ordering-blocked 会从 256 events/8 MiB 倍增到硬上限，硬上限仍阻塞则保持 non-poison。apply 事务 claim worker 后、任何业务 DML 前会复查该 run/direction 的既有 poison。poison 发布也会在 binding/checkpoint 校验后锁定并检查该方向的任意既有记录：完全相同视为 ACK-loss 成功，不同记录视为 stale，绝不写第二条。apply、ack-loss 判定和 poison 发布同时绑定 source 与 live target identity；snapshot 与 apply 前都要求 `progress.applied_seq` 等于 checkpoint。每个有效 batch 结局恰好发出一条脱敏 metric，使用实际 accepted raw-event 数和尽可观测的 retries。目标 apply 锁定 migration ledger 和全部业务表并复核精确 catalog，SQLite path/file binding 失败使用专用 identity 异常而不靠文本落入 conversion；已证明的确定性错误只写一条脱敏 poison 后阻断，数据库瞬态错误只做有上限的整事务重试。但尚无运维 CLI 或端到端迁移 worker，所以 `SHADOW_DATABASE_URL` 仍保持 inert，这些原语还不是可执行迁移命令。

目前还增加了 barrier-aware verifier：它比较 SQLite source snapshot 与 PostgreSQL repeatable-read/read-only snapshot，只排除 change log 可证明的 concurrent key，并执行结构、领域、embedding、双语检索和冻结切换门禁，持久化内容脱敏的报告。它仍只是迁移原语；在运维 CLI 与受监督 worker 完成前，`SHADOW_DATABASE_URL` 仍保持 inert。
