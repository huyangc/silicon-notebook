...
- 「AI 对这个库的理解」以低成本、后台巡固的方式维护一份笔记本理解摘要——这个库大致收了什么、反复出现的主题、已知的空缺，以及每位成员私有的、行之有效的问法与问过但没找到答案的方向——可在面板中查看与编辑，并悄悄融入逐步推理与深度报告自己的检索 prompt，但从不进入答案正文本身。
...
7. 单份深度报告可另行发布为**免登录**的只读页面：链接由 owner 生成、随时可撤销，页面只含正文与「引用出处」（标题/位置/摘录），不含来源 id、不可打开原始资料。正文与站内共用同一套渲染（公式、表格、可点引用编号），编号点开跳到本页的摘录条目。研究问题原样呈现、不截断（创建时即有 4,000 字上限，前端同步显示护栏）；引用的标题/原始文件名/摘录仍有长度上限，但一旦触顶会显式标注「已截断」，不静默丢尾。
