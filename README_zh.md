...
- KG 抽取会在全局融合前持久化不可变的来源代次事实及规范化证据元素绑定。后端无关、按代次缓存的所选来源快照通过有界、来源优先且缓存命中身份为常量成本的查询，只读取获授权的对象、关系、chunk、事实、证据绑定和 cluster membership。当前已有仅供 shadow 的邻居/扩展、两跳 chain、关系召回、精确小节与游标枚举原语；Ask/Report 仍未消费它们，因此当前排序和通道选择不变。
