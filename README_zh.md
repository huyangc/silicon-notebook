...
- 逐步推理提供 `overview` / `standard` / `deep` / `thorough` / `exhaustive` 五档有界检索力度。显式的完整列举、计数或混合请求改走带覆盖率的 Knowhow 游标枚举，例如返回 `100/100`；触及安全上限时只会明确标为部分结果，绝不冒充“全部”。精确阈值见[产品与 API 参考](./docs/product-and-api_zh.md#逐步推理档位与完整集合请求)。
