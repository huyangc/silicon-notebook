...
单独暂存 Markdown 时，如果正文引用了未随文档打包的本地/相对图片，或服务端不会代为下载的远程图片，添加来源弹窗会持续提示，但仍允许上传有价值的文字。在 Ask 中，只要 MinerU 或 Markdown 图片已经持久化并绑定引用，就会紧跟在对应段落/列表项/引用块/标题/整张表之后穿插显示，不重复展示 caption 文字；同一资产只在第一次引用处出现，点击可在本页放大。公开问答会话通过 token 别名沿用同一落点；深度报告正文内联留到第二期。
