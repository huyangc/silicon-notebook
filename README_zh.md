...
当前目标是可供真实团队使用的本地 beta：后端采用 FastAPI，并可选择 SQLite 或 PostgreSQL repository；前端采用 Next.js。发行默认的 SQLite 快速启动不要求 Docker、GPU、数据库服务或本地模型服务；选择 PostgreSQL 时需要可访问的 PostgreSQL 服务。OpenAI 兼容的聊天、嵌入、重排和 MinerU 服务都是可选的 URL 集成；未配置时，确定性降级仍可维持核心流程。模块化扩展 Phase 0 现已建立低依赖的 `domain` 契约层，以及启动后冻结、当前为空的 Extension SDK registry；尚无产品流程由插件驱动，因此空 registry 下所有既有路径保持不变。Contribution 默认按稳定 ID 排序，manifest 中的 capability requirement 与插件依赖保持为两个独立概念。
