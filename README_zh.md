...
检索范围在来源侧栏按**两个互相独立的维度**选择，由问答与新建深度报告共用，且都默认全选：当前笔记本每个导入来源一个复选框，每个已挂载参考库**整库**一个复选框。来源选择是当前 notebook 的检索硬上限，模型规划的来源范围只能进一步缩小；取消勾选某个参考库只是让它不参与本轮检索，不会削弱当前笔记本自身的检索能力。“全选”/“清空”一并管两个维度。只有两个维度**同时**为空——既没有勾选任何本地来源，也没有勾选任何参考库——问答输入与新建报告控件才会被禁用。
