...
Workspace UI 扩展只允许随构建静态进入。当前 build registry 没有真实 contribution，因此不会新增请求、wrapper DOM、timer 或插件 chunk。首批 canonical slot 只有 `workspace.side_panel` 与 `source.detail_section`；本地 contribution、已认证 `/system/extensions` 的实时 availability、核心权限快照与已归一化 UI mode 四门必须同时成立才渲染。端点只返回脱敏元数据与固定 unavailable reason，绝不返回 capability 名、endpoint、路径、凭据或异常文本；插件组件只收到只读 user/notebook/source 摘要，不接 workspace setter 或其他领域 owner 内部状态。
