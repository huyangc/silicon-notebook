...
# ===========================================================================
# T-W4-5: the element leg's cross-table OR -> two-leg equivalent UNION (SQLite)
# ===========================================================================
# ``sqlite/query_store.py:search_notebook``'s element leg used to spell one
# cross-table boolean OR whose ``LOWER(s.title)`` arm lives on ``sources`` while
# the other two live on ``source_elements``. It is now two independent legs
# UNION-ed on ``se.rowid`` — the PostgreSQL twin
# (``postgres/search.py:notebook_element_rows``) carries the full equivalence
# argument and the index reasoning; SQLite has no trigram index to gain from and
# takes the rewrite so the two backends' SQL does not diverge.
#
# TWO SQLite-specific things these tests pin:
#
#  * ``ORDER BY se.rowid`` where PostgreSQL orders by ``ordinal``. SQLite's
#    ``source_elements`` has no ``ordinal`` column; the implicit rowid is its
#    counterpart, per the standing ruling in ``sqlite/migrations.py`` (the same
#    one ``chunks_by_section`` follows).
#  * The order is now DETERMINISTIC where it previously was not: the retired
#    query had no ORDER BY at all and simply stopped at ``LIMIT``. That is why
#    the equivalence test below compares SETS on needles whose match count fits
#    under the cap (the old shape's answer is not defined once the cap binds),
#    and the ordering/truncation contract gets its own absolute test.

_SQ_NEEDLES = ("zqxjcross", "zqxjtitleonly", "zqxjtextonly", "zqxjloc", "zqxjmix")
_SQ_BAIT = " ".join(_SQ_NEEDLES)
# 24 rounds, not 12, and the number is load-bearing. Measured with EXPLAIN
# QUERY PLAN: SQLite drives BOTH legs from `sources` and reaches the elements
# through idx_source_elements_source, so an unordered leg would hand back rows
# grouped by source id — not in rowid order. That only produces a DIFFERENT
# answer when one source alone can fill a leg's cap; at 12 rounds the second
# source's early rows still slip in under the cap and a leg that dropped its
# ORDER BY would pass by accident (verified: it did).
_SQ_MIX_ROUNDS = 24


def _legacy_sqlite_element_ids(repo, notebook_id, needle, limit):
    """The RETIRED cross-table-OR predicate, verbatim, as the oracle.

    A hand-written copy on purpose: it is the shape the rewrite replaced, so it
    must keep behaving like the old code however the real query changes next.
    """
    like = f"%{needle}%"
    with repo._connect() as db:
        return {
            ...
            for row in db.execute(
                ...
                "(LOWER(se.text) LIKE ? OR LOWER(se.location_label) LIKE ? "
                "OR LOWER(s.title) LIKE ?) "
                "LIMIT ?",
                (notebook_id, like, like, like, limit),
            ).fetchall()
        }


@pytest.fixture
def element_union_notebooks(repo):
    """Four scenarios plus the two exclusion baits; returns (nb_id, mix_order).

    ``mix_order`` is the truncation block's element ids in INSERTION order,
    which for SQLite is rowid order — the contract the rewrite now owes.

    The mix block deliberately spreads each leg's hits over TWO sources and
    round-robins the inserts, so rowid order differs from the source-grouped
    order a scan naturally produces. Put both legs' hits in one source each and
    a leg that capped WITHOUT ordering would still return the right rows by
    accident, and this fixture would be pinning nothing.
    """
    nb = repo.create_notebook(NotebookCreate(name="element-union"))
    other = repo.create_notebook(NotebookCreate(name="element-union-other"))
    now = _now()
    mix_order: list[str] = []

    def add_source(db, source_id, notebook_id, title, source_type="document"):
        db.execute(
            "INSERT INTO sources (id,notebook_id,title,source_type,file_name,file_path,"
            "file_size,file_hash,summary,doc_type,parse_status,created_at,updated_at) "
            "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",
            (source_id, notebook_id, title, source_type, f"{source_id}.md",
             f"/tmp/{source_id}.md", 0, source_id, "", "", "extracted", now, now))

    def add_element(db, element_id, source_id, text, label="p1"):
        db.execute(
            "INSERT INTO source_elements (id,source_id,element_type,location_label,"
            "text,metadata,created_at) VALUES (?,?,?,?,?,?,?)",
            (element_id, source_id, "paragraph", label, text, "{}", now))

    with repo._write() as db:
        for source_id, title in (
            ("sq-cross", "Cross Zqxjcross Study"),
            ("sq-titleonly", "Titleonly Zqxjtitleonly Study"),
            ("sq-plain", "Plain Study"),
            ("sq-mixtitle-a", "MixA Zqxjmix Study"),
            ("sq-mixtitle-b", "MixB Zqxjmix Study"),
            ("sq-mixtext-a", "MixA Plain Study"),
            ("sq-mixtext-b", "MixB Plain Study"),
        ):
            add_source(db, source_id, nb.id, title)
        # Bait: hidden synthetic sources in this notebook, and a neighbouring
        # notebook's ordinary source. All carry every needle in BOTH the title
        # and an element, so a leak in either leg surfaces — and, since the
        # oracle hides them too, set-equality alone could not catch a bug that
        # hit both shapes.
        add_source(db, "sq-hidden-mem", nb.id, f"Hidden {_SQ_BAIT}", "memory")
        add_source(db, "sq-hidden-kh", nb.id, f"Hidden {_SQ_BAIT}", "knowhow")
        add_source(db, "sq-other", other.id, f"Other {_SQ_BAIT}")

        # Scenario 1 — crossed hit: matched by both legs, by the title leg
        # alone, and by the text leg alone, all on one needle.
        add_element(db, "sel-cross-a", "sq-cross", "body zqxjcross here")
        add_element(db, "sel-cross-b", "sq-cross", "body without it")
        add_element(db, "sel-cross-c", "sq-plain", "body zqxjcross here")
        # Scenario 2 — title only.
        add_element(db, "sel-title-a", "sq-titleonly", "quiet body")
        add_element(db, "sel-title-b", "sq-titleonly", "quiet body")
        # Scenario 3 — element text only, and location_label only.
        add_element(db, "sel-text-a", "sq-plain", "prose zqxjtextonly tail")
        add_element(db, "sel-loc-a", "sq-plain", "prose", "zqxjloc §7")
        # Scenario 4 — over-cap truncation, four sources round-robined.
        for index in range(_SQ_MIX_ROUNDS):
            for source_id, text in (
                ("sq-mixtitle-a", "quiet body"),
                ("sq-mixtext-a", "prose zqxjmix tail"),
                ("sq-mixtitle-b", "quiet body"),
                ("sq-mixtext-b", "prose zqxjmix tail"),
            ):
                element_id = f"sel-{source_id}-{index:03d}"
                add_element(db, element_id, source_id, text)
                mix_order.append(element_id)
        for source_id in ("sq-hidden-mem", "sq-hidden-kh", "sq-other"):
            add_element(db, f"sel-bait-{source_id}", source_id, _SQ_BAIT, _SQ_BAIT)
    return nb.id, mix_order


_SQ_BAIT_IDS = {
    "sel-bait-sq-hidden-mem", "sel-bait-sq-hidden-kh", "sel-bait-sq-other",
}
...
@pytest.mark.parametrize(
    "needle,expected",
    [
        # Crossed hit: one element matched through both legs (the UNION must
        # dedupe it), one through each leg alone.
        ("zqxjcross", {"sel-cross-a", "sel-cross-b", "sel-cross-c"}),
        # Title only: the element leg returns nothing.
        ("zqxjtitleonly", {"sel-title-a", "sel-title-b"}),
        # Element text only, and location_label only.
        ("zqxjtextonly", {"sel-text-a"}),
        ("zqxjloc", {"sel-loc-a"}),
        # Nothing carries it.
        ("zqxjabsent", set()),
    ],
)
def test_sqlite_element_union_matches_the_retired_or_predicate(
    repo, element_union_notebooks, needle, expected
):
    """Set equivalence with the retired predicate, below the cap.

    Compared as SETS, not row for row: the retired query had no ORDER BY, so
    only its membership was ever defined. Each needle here matches fewer rows
    than ``SEARCH_HIT_CAP``, so the cap does not bind and membership is the
    whole contract.
    """
    notebook_id, _mix_order = element_union_notebooks
    hits = repo.search_notebook(notebook_id, needle).hits
    found = {hit.element_id for hit in hits if hit.scope == "Element"}

    assert found == expected
    assert found == _legacy_sqlite_element_ids(
        repo, notebook_id, needle, SEARCH_HIT_CAP
    )
    # Not derivable from the oracle: both shapes hide the same rows.
    assert not (_SQ_BAIT_IDS & found), needle


def test_sqlite_element_union_returns_the_smallest_rowids_in_rowid_order(
    repo, element_union_notebooks
):
    """The ordering contract SQLite did not have before this rewrite.

    Both legs saturate their own cap here (24 matches each against a cap of 20),
    so the answer is decided entirely by the per-leg ``ORDER BY se.rowid LIMIT``
    and the outer one: it must be the globally smallest rowids, in rowid order.
    The retired shape had no ORDER BY at all — it returned whichever rows the
    scan reached first.
    """
    notebook_id, mix_order = element_union_notebooks
    hits = repo.search_notebook(notebook_id, "zqxjmix").hits
    element_ids = [hit.element_id for hit in hits if hit.scope == "Element"]

    # Both legs are over the cap: 12 rounds x 2 sources per leg.
    assert _SQ_MIX_ROUNDS * 2 > SEARCH_HIT_CAP
    # The element leg runs after the source leg, and the response is truncated
    # to SEARCH_HIT_CAP as a whole, so how many element hits survive depends on
    # how many the source leg contributed (the two "Mix… Zqxjmix Study" titles).
    assert len(hits) == SEARCH_HIT_CAP
    assert len(element_ids) == SEARCH_HIT_CAP - sum(
        1 for hit in hits if hit.scope != "Element"
    )
    assert element_ids == mix_order[:len(element_ids)]
    assert not (_SQ_BAIT_IDS & set(element_ids))
