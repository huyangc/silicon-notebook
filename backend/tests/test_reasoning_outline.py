...
def _v2_outline_action(sections, reason="整理大纲"):
    """v2 的单一 `arguments` 载荷(legacy 的 `outline.sections` 分支已并入它)。"""
    return {"next_action": OUTLINE_ACTION, "sufficient": False,
            "reason": reason, "arguments": {"sections": sections}}


def test_v2_spent_outline_budget_leaves_the_action_out_of_the_prompt(repo):
    """额度用尽后动作从 prompt 消失;模型硬选它由投影拦下,而不是走执行处的
    `outline_budget` 分支。两处的读数差一,就是白烧一轮反思或提前一轮摘掉它。"""
    notebook = _seed(repo)
    repo.settings.reasoning_reflect_v2_enabled = True
    repo.settings.reasoning_stale_limit = 99
    llm = _V2SeqLLM([
        _v2_outline_action([{"id": f"s{i}", "title": f"第{i}节"}])
        for i in range(_MAX_OUTLINE_UPDATES + 1)
    ...
    assert len(_steps(result, "outline")) == _MAX_OUTLINE_UPDATES
    assert OUTLINE_ACTION in llm.prompt_actions(_MAX_OUTLINE_UPDATES - 1)
    assert OUTLINE_ACTION not in llm.prompt_actions(_MAX_OUTLINE_UPDATES)
    reasons = [step.detail.get("reason") for step in _steps(result, "skip")]
    assert "unavailable_action:outline_budget" in reasons
    assert "outline_budget" not in reasons
