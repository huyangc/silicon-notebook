...
# ------------------------------------- T4-B:结束事实在按节合成里的转发口径
# 设计稿 2026-09-07 §7.2。事实每节都给(一节的合成模型只看得见自己那份证据,
# 不给它就无从知道自己写的这段落在一次没查完的检索上),块尾的祈使句只随最后
# 一节给(否则每一节末尾各挂一段免责声明,而缺口本来只需要在全篇说一次)。


def _termination(reason="model_partial", *, aspects=(), unresolved=(),
                 channels=()):
...
_FACT_HEAD = "Retrieval status (server fact"
_DIRECTIVE = "Say plainly in the answer"


def test_every_section_prompt_carries_the_termination_facts(repo, monkeypatch):
    """按节合成:**每一节**的 prompt 都带服务端事实块。

    变异:`_draft_reasoning_response` 不再把 `termination` 转发给
    `_answer_reasoning_sections`(或后者不再逐节渲染)⇒ 这条红。零覆盖正是本次
    评审点名的缺口:接入时只有单次合成那条路径被断言过。
    """
    notebook = _notebook(repo)
    _stub_run(monkeypatch, _reasoning_result(termination=_termination(
    ...
    for prompt in llm.prompts:
        ...
        # 位置不变:它是指令,不是证据(不挨着知识条目、排在 Question 之前)。
        assert prompt.index(_FACT_HEAD) < prompt.index("Question:")
    # 结束事实不改按节合成的既有产物。
    assert response.answer.startswith("## 第一节")


def test_only_the_last_section_gets_the_gap_directive(repo, monkeypatch):
    """块尾的祈使句只出现在最后一节;其余各节只拿事实。

    变异:每节都给祈使句(`directive` 恒 True)⇒ 这条红。
    """
    notebook = _notebook(repo)
    _stub_run(monkeypatch, _reasoning_result(termination=_termination(
        aspects=(("a1", "没查着的那件事", "unknown", ()),),
    ...
    first, last = llm.prompts
    assert _DIRECTIVE not in first
    assert _DIRECTIVE in last


def test_one_shot_synthesis_still_gets_the_gap_directive(repo, monkeypatch):
    """单次合成不变:事实与祈使句一起给(按节那条规则不外溢到这条路径)。"""
    notebook = _notebook(repo)
    _stub_run(monkeypatch, _reasoning_result(
        outline=[_section("s1", "唯一一节", "e1")],
    ...
    assert len(llm.prompts) == 1, "只有一个可装配节 ⇒ 单次合成"
    assert _FACT_HEAD in llm.prompts[0]
    assert _DIRECTIVE in llm.prompts[0]


def test_a_clean_sufficient_run_states_the_fact_without_the_directive(
    repo, monkeypatch
):
    """`model_sufficient` 且无未解决方面、无未恢复通道 ⇒ 尾句省掉(§7.2 P3-3)。

    那句"说清哪些点没被覆盖"指向的是一个空集合,留着只会诱导模型编一个缺口
    出来交差。事实照常给,连最后一节也不再挂那句祈使。
    """
    notebook = _notebook(repo)
    _stub_run(monkeypatch, _reasoning_result(termination=_termination(
        "model_sufficient",
        aspects=(("a1", "查到了的那件事", "supported", ("e1",)),))))
    llm = _CaptureAnswerLLM()
    _ask(repo, notebook, llm)

    for prompt in llm.prompts:
        assert "every mandatory aspect reported supported" in prompt
        assert _DIRECTIVE not in prompt


def test_no_termination_leaves_every_section_prompt_byte_identical(
    repo, monkeypatch
):
    """关闭态(legacy / v2 关闭):`termination is None` ⇒ 每节 prompt 一个字不多。"""
    notebook = _notebook(repo)
    _stub_run(monkeypatch, _reasoning_result())
    llm = _CaptureAnswerLLM()
    _ask(repo, notebook, llm)
    ...
    assert len(llm.prompts) == 2
    for prompt in llm.prompts:
        assert _FACT_HEAD not in prompt
        assert _DIRECTIVE not in prompt
