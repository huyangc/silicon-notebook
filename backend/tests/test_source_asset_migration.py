...
def test_fresh_db_has_notebook_assets_source_id(tmp_path):
    """全新库:SCHEMA_VERSION 已到 23（notebook_assets.source_id 列在 v19 落地，
    ...
    最新状态表、v24 加了写锁瘦身改造点 2 的 kg_canonical_scratch 表，均不影响本
    列）；notebook_assets 带可空 source_id 列 + idx_notebook_assets_source 索引；
    user_version 已盖到 SCHEMA_VERSION。"""
    repo = _repo(tmp_path)
