...
def test_reasoning_output_budget_is_one_number_for_plan_and_reflect(rrepo):
    """Planning and reflection share the configured reasoning output budget."""
    from app.services.reasoning_retrieval import ReasoningRetriever
...
def test_legacy_exact_lookup_shape_gate_stays_in_the_executor(rrepo):
    """Exact lookup shape validation stays in the Legacy executor."""
    nb = _seed_manual_notebook(rrepo)
    rrepo.settings.graph_ppr_enabled = False
    bind_chat_client(rrepo, "reasoning_agent", _SeqLLM(
        plan={"sub_queries": [{"query": "布局布线"}]},
...
def test_legacy_prose_ledger_reaches_the_reflect_prompt_verbatim(rrepo):
...
def test_legacy_trace_keeps_its_step_sequence_and_duration_keys(rrepo):
    """Legacy traces retain action sequence and duration fields."""
...
def test_trace_recorder_propagates_every_core_cancellation_from_the_callback():
    ...
    class OtherCancellation(CoreCancellation):
        pass

    def cancel(step):
        raise OtherCancellation()

    trace = []
    recorder = _TraceRecorder(trace, None, cancel)
    with pytest.raises(OtherCancellation):
        recorder(TraceStep(step_type="ppr", summary=""))
    assert trace and trace[0].step_type == "ppr"


def test_legacy_closing_rerank_adds_no_trace_step_or_clock_read(rrepo, monkeypatch):
    import app.services.reasoning_retrieval as rr
    nb = _seed_two_nodes(rrepo)
    retriever = rr.ReasoningRetriever.from_repository(rrepo, rrepo.settings)
    _stub_rerank_legs(monkeypatch, retriever)

    def unexpected_call(*args):
        raise AssertionError("Closing rerank must not add instrumentation")

    monkeypatch.setattr(rr.time, "perf_counter", unexpected_call)
    collected = {"A": _rk("A", 0.0), "B": _rk("B", 0.0)}
    for queries in (["q1", "q2"], ["q1"]):
