...
def test_schema_on_utf8_database_with_non_c_default_collation(
    ...
    assert PostgresMigrator(postgres_non_c_database).migrate() == 56
...
def test_packaged_migrations_are_idempotent_from_empty_schema(postgres_database):
    ...
    assert migrator.migrate() == 56
    assert migrator.migrate() == 56
    assert migrator.current_version() == 56
    assert POSTGRES_SCHEMA_MANIFEST.postgres_version == 56
...
def test_pg_trgm_is_shared_outside_disposable_schema_lifetimes(postgres_scope):
    ...
    try:
        ...
        assert extension_schema == "public"
        assert PostgresMigrator(databases[1]).migrate() == 56
    finally:
...
def test_packaged_index_migration_phases_are_exact():
    ...
    assert [(version, migrations[version].name) for version in migrations] == [
        ...
        (53, "wish_status"),
        (54, "question_submitted_via"),
        (55, "notebook_metadata"),
        (56, "global_ask"),
    ]
    ...
    # Migration 41 mirrors SQLite v63's deployment-plugin runtime toggle
    # table. No index: the table only ever holds a few dozen rows, so the
    # admin page's full listing and the admission-refresh read are both a
    # cheap sequential scan (`enabled` is not part of the primary key, so
    # the primary key index could not serve that filter anyway).
    ...
    # Migration 44: notebook deletion-independent, content-minimal activity
    # projection plus its actor/owner/expiry indexes.
    v44_indexes = index_declarations(44)
    assert v44_indexes == [
    ...
    # Migration 50 (Ask submission idempotency key): one nullable column plus
    ...
    assert (
        "ALTER TABLE ask_jobs ADD COLUMN IF NOT EXISTS client_request_id text "
        'COLLATE "C";' in v50_ddl_only
    )
    assert "ON ask_jobs(created_by, client_request_id)" in v50_ddl_only
    assert "WHERE client_request_id IS NOT NULL" in v50_ddl_only
