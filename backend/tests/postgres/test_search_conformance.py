...
# ===========================================================================
# T-W4-5: the element leg's cross-table OR -> two-leg equivalent UNION
# ===========================================================================
# ``postgres/search.py:notebook_element_rows`` used to spell one cross-table
# boolean OR (`se.text ILIKE … OR se.location_label ILIKE … OR s.title ILIKE …`)
# whose third arm lives on ``sources`` while the first two live on
# ``source_elements`` -- the shape migration 0048 proved no index can serve.
# It is now two independent legs UNION-ed on ``se.id``. The whole point of the
# rewrite is that both shapes return the SAME rows, so the semantic tests above
# cannot catch a regression: what follows is (1) an ORACLE that runs the retired
# predicate verbatim and demands byte-identical results including order, and
# (2) a plan pin that the title leg really does enter through migration 0048's
# lower(title) partial GIN -- the only reason the rewrite exists.

_ELEMENT_NOTEBOOK = "nb-elem-union"
_ELEMENT_OTHER_NOTEBOOK = "nb-elem-union-other"
# Every needle a hidden (memory/knowhow) source and the neighbouring notebook's
# source carry in BOTH their title and their elements' text, so any scenario
# below that leaked them would fail its exclusion assertion.
_ALL_NEEDLES = ("zqxjcross", "zqxjtitleonly", "zqxjtextonly", "zqxjloc", "zqxjmix")
_LEAK_BAIT = " ".join(_ALL_NEEDLES)
# The interleaved truncation block: 24 title-leg elements on odd ordinals and 24
# text-leg elements on even ordinals, starting at 401. A per-leg cap that lost
# the ordinal order, or a UNION taken before either leg was capped, changes
# which rows survive `LIMIT 8`.
_MIX_PAIRS = 24
_MIX_ORDINAL_BASE = 401


def _legacy_notebook_element_rows(connection, notebook_id: str, needle: str, limit: int):
    """The RETIRED cross-table-OR predicate, verbatim, as the oracle.

    Deliberately a hand-written copy rather than an import: it is the thing the
    rewrite replaced, so it must keep behaving like the old code even after the
    real function changes again. Its correctness is not in question here -- it
    shipped for the entire life of the search box.
    """
    pattern = f"%{needle}%"
...
def _insert_element_source(db, source_id: str, notebook_id: str, title: str, source_type: str):
    ...
    db.execute(
        ...
        "VALUES (%s,%s,%s,%s,'extracted','extracted',%s,'uploads/x.pdf',1,%s,'','',%s,%s)",
        (source_id, notebook_id, title, source_type, f"{source_id}.pdf", source_id, now, now),
    )


def _insert_element(db, element_id: str, source_id: str, ordinal: int, text: str, label: str):
    from app.repositories.postgres._store_utils import normalize_timestamp

    db.execute(
        "INSERT INTO source_elements"
        "(id,source_id,element_type,location_label,text,metadata,created_at,ordinal) "
        "VALUES (%s,%s,'paragraph',%s,%s,'{}'::jsonb,%s,%s)",
        (element_id, source_id, label, text, normalize_timestamp(NOW), ordinal),
    )


def _seed_element_search_corpus(database) -> None:
    """Four scenarios in one corpus, plus the two exclusion baits.

    Ordinals are written EXPLICITLY (the column is ``GENERATED BY DEFAULT``, so
    that is allowed) because the equivalence argument the rewrite rests on is
    about ordinal order: the union's first ``cap`` rows must be contained in the
    two legs' own first ``cap`` rows. Letting the identity sequence assign them
    would make the interleaving an artifact of insertion order.
    """
    from app.repositories.postgres._store_utils import normalize_timestamp

    now = normalize_timestamp(NOW)
    with database.write() as db:
        for notebook_id in (_ELEMENT_NOTEBOOK, _ELEMENT_OTHER_NOTEBOOK):
            db.execute(
        ...
        for source_id, title in (
            ("src-cross", "Cross Zqxjcross Study"),
            ("src-titleonly", "Titleonly Zqxjtitleonly Study"),
            ("src-plain", "Plain Study"),
            ("src-mixtitle", "Mix Zqxjmix Study"),
            ("src-mixtext", "Mix Plain Study"),
        ):
            _insert_element_source(db, source_id, _ELEMENT_NOTEBOOK, title, "pdf")
        # Exclusion bait: hidden synthetic sources in the notebook under test,
        # and a neighbouring notebook's perfectly ordinary source. Both carry
        # every needle in the title AND in an element, so a leak in EITHER leg
        # surfaces -- and, because the oracle hides them too, byte-equality
        # alone could not catch a bug that hit both shapes.
        for source_id, source_type in (
            ("src-hidden-mem", "memory"),
            ("src-hidden-kh", "knowhow"),
        ):
            _insert_element_source(
                db, source_id, _ELEMENT_NOTEBOOK, f"Hidden {_LEAK_BAIT}", source_type
            )
        _insert_element_source(
            db, "src-other", _ELEMENT_OTHER_NOTEBOOK, f"Other {_LEAK_BAIT}", "pdf"
        )

        # Scenario 1 -- cross hit: one element matched by BOTH legs, one by the
        # title leg alone, one by the text leg alone, all on one needle.
        _insert_element(db, "el-cross-a", "src-cross", 101, "body zqxjcross here", "p1")
        _insert_element(db, "el-cross-b", "src-cross", 102, "body without it", "p2")
        _insert_element(db, "el-cross-c", "src-plain", 103, "body zqxjcross here", "p3")
        # Scenario 2 -- title only: nothing in these elements carries the needle.
        _insert_element(db, "el-title-a", "src-titleonly", 201, "quiet body", "p1")
        _insert_element(db, "el-title-b", "src-titleonly", 202, "quiet body", "p2")
        # Scenario 3 -- text only, and location_label only (the second arm of
        # leg A, which nothing else in this corpus exercises).
        _insert_element(db, "el-text-a", "src-plain", 301, "prose zqxjtextonly tail", "p4")
        _insert_element(db, "el-loc-a", "src-plain", 302, "prose", "zqxjloc §7")
        # Scenario 4 -- over-cap truncation with the two legs interleaved.
        # Inserted in REVERSE ordinal order on purpose: with insertion order
        # equal to ordinal order, a leg that capped WITHOUT ordering would still
        # return the right eight rows by accident (a seq scan hands rows back in
        # heap order), and the assertions below would be pinning nothing.
        for index in reversed(range(_MIX_PAIRS)):
            _insert_element(
                db,
                f"el-mixtitle-{index:03d}",
                "src-mixtitle",
                _MIX_ORDINAL_BASE + index * 2,
                "quiet body",
                f"t{index}",
            )
            _insert_element(
                db,
                f"el-mixtext-{index:03d}",
                "src-mixtext",
                _MIX_ORDINAL_BASE + index * 2 + 1,
                "prose zqxjmix tail",
                f"x{index}",
            )
        for index, source_id in enumerate(
            ("src-hidden-mem", "src-hidden-kh", "src-other")
        ):
            _insert_element(
                db, f"el-bait-{index}", source_id, 900 + index, _LEAK_BAIT, _LEAK_BAIT
            )


@pytest.fixture
def element_search_database(postgres_database):
    from app.repositories.postgres.migrator import PostgresMigrator

    assert PostgresMigrator(postgres_database).migrate() == 51
    _seed_element_search_corpus(postgres_database)
    return postgres_database


_BAIT_ELEMENT_IDS = {"el-bait-0", "el-bait-1", "el-bait-2"}


@pytest.mark.parametrize(
    "needle,limit",
    [
        # Scenario 1: crossed hit -- the same needle matches through both legs,
        # and one element matches through both at once (the UNION must dedupe).
        ("zqxjcross", 8),
        ("zqxjcross", 2),
        # Scenario 2: title only -- leg A returns nothing at all.
        ("zqxjtitleonly", 8),
        ("zqxjtitleonly", 1),
        # Scenario 3: text only / location_label only -- leg B returns nothing.
        ("zqxjtextonly", 8),
        ("zqxjloc", 8),
        # Scenario 4: both legs saturate their own cap and interleave.
        ("zqxjmix", 8),
        ("zqxjmix", 1),
        ("zqxjmix", 20),
        ("zqxjmix", 200),
        # A needle nothing carries: both legs empty.
        ("zqxjabsent", 8),
    ],
)
def test_element_union_matches_the_retired_or_predicate_row_for_row(
    element_search_database, needle, limit
):
    """The oracle. Byte-identical rows, in the same order, on every scenario."""
    with element_search_database.connect() as connection:
        rewritten = notebook_element_rows(
            connection, _ELEMENT_NOTEBOOK, needle, limit
        )
        legacy = _legacy_notebook_element_rows(
            connection, _ELEMENT_NOTEBOOK, needle, limit
        )

    assert list(rewritten) == list(legacy), (
        f"needle={needle!r} limit={limit}: "
        f"{[row['id'] for row in rewritten]} != {[row['id'] for row in legacy]}"
    )
    assert len(rewritten) <= limit
    # Not derivable from the oracle: both shapes hide the same rows, so a bug
    # that leaked a hidden source's or a neighbour notebook's element through
    # BOTH would still compare equal.
    assert not (_BAIT_ELEMENT_IDS & {row["id"] for row in rewritten}), needle


def test_element_union_keeps_the_interleaved_order_across_the_cap(
    element_search_database,
):
    """The truncation scenario, pinned absolutely rather than against the oracle.

    Leg A (element text) owns the odd ordinals of the mix block and leg B
    (source title) the even ones, 24 each. With ``limit=8`` the answer must be
    the eight globally smallest ordinals -- four from each leg. Capping the legs
    without ordering them, or taking the UNION before either leg is capped, both
    produce a different eight.
    """
    with element_search_database.connect() as connection:
        rows = notebook_element_rows(connection, _ELEMENT_NOTEBOOK, "zqxjmix", 8)

    assert [row["ordinal"] for row in rows] == list(
        range(_MIX_ORDINAL_BASE, _MIX_ORDINAL_BASE + 8)
    )
    assert [row["id"] for row in rows] == [
        "el-mixtitle-000", "el-mixtext-000",
        "el-mixtitle-001", "el-mixtext-001",
        "el-mixtitle-002", "el-mixtext-002",
        "el-mixtitle-003", "el-mixtext-003",
    ]
    # source_title is the column the caller builds its label from; the join back
    # to ``sources`` must still supply it for rows found through EITHER leg.
    assert {row["source_title"] for row in rows} == {
        "Mix Zqxjmix Study", "Mix Plain Study",
    }


class _CapturingConnection:
    """Records the SQL the adapter really issues, then delegates.

    Same technique as ``test_hotpath_indexes_batch4_live.py``'s
    ``list_sources_page`` guard: asserting against a hand-copied SQL string
    would let the copy drift out of sync with the real one.
    """
...
def test_element_leg_issues_the_two_leg_union_shape(element_search_database):
    """Shape guard against a regression to the cross-table OR.

    The oracle above cannot catch that regression -- the whole point of the
    rewrite is that both shapes answer identically.
    """
    with element_search_database.connect() as connection:
        spy = _CapturingConnection(connection)
        notebook_element_rows(spy, _ELEMENT_NOTEBOOK, "zqxjmix", 8)

    assert len(spy.captured) == 1
    statement, _params = spy.captured[0]
    assert "se.id IN (" in statement, statement
    assert statement.count(" UNION ") == 1, statement
    assert "UNION ALL" not in statement, (
        f"UNION ALL would let a two-leg hit through twice if the semi-join "
        f"were ever replaced by a join:\n{statement}"
    )
    assert "OR s.title ILIKE" not in statement, (
        f"the retired cross-table OR is back:\n{statement}"
    )
    assert "LOWER(title) LIKE" in statement, (
        f"the title leg must be spelled against migration 0048's lower(title) "
        f"expression key -- a bare-column ILIKE cannot use it:\n{statement}"
    )
    # Three spellings: the outer query's, leg A's, and -- the load-bearing one --
    # leg B's, whose INLINE literal is what makes migration 0048's partial index
    # implication hold under a parameter-value-blind generic plan.
    assert statement.count("source_type NOT IN ('memory','knowhow')") == 3, statement
    # Per-leg caps: without them the UNION is taken over the whole match set and
    # the rewrite buys nothing.
    assert statement.count("ORDER BY ea.ordinal LIMIT") == 1, statement
    assert statement.count("ORDER BY eb.ordinal LIMIT") == 1, statement


# The plan pin's own corpus, sized after ``test_hotpath_indexes_batch4_live.py``'s
# (which pins this same index for ``list_sources_page``, and whose docstring
# works through why each number is load-bearing): the neighbour notebooks make
# the leading ``notebook_id`` key mean something, and the target notebook has to
# be big enough that "walk a notebook_id btree and filter" is the LOSING plan.
_ELEMENT_PLAN_NOTEBOOK = "nb-elem-plan"
_ELEMENT_PLAN_SOURCES = 20000
_ELEMENT_PLAN_NOISE_NOTEBOOKS = 2
_ELEMENT_PLAN_NOISE_SOURCES = 5000


def _seed_element_plan_notebook(db, notebook_id, prefix, count, now):
    ...
    # One element per source: enough that leg A is a real relation rather than
    # an empty one the planner can collapse away.
    db.execute(
        "INSERT INTO source_elements"
        "(id,source_id,element_type,location_label,text,metadata,created_at) "
        "SELECT s.id||':el', s.id, 'paragraph', 'p1', "
        "'Body text of '||s.id, '{}'::jsonb, %s "
...
@pytest.fixture
def element_plan_database(postgres_database):
    from app.repositories.postgres._store_utils import normalize_timestamp
    from app.repositories.postgres.migrator import PostgresMigrator
    ...
    with postgres_database.write() as db:
        _seed_element_plan_notebook(
            db, _ELEMENT_PLAN_NOTEBOOK, "esrc", _ELEMENT_PLAN_SOURCES, now
        )
        db.execute(
            "UPDATE sources SET title='Zqxjtitle Special Study' WHERE id=%s",
            ("esrc-000001",),
        )
    for index in range(_ELEMENT_PLAN_NOISE_NOTEBOOKS):
        with postgres_database.write() as db:
            _seed_element_plan_notebook(
                db,
                f"nb-elem-plan-noise-{index}",
                f"enoise{index}",
                _ELEMENT_PLAN_NOISE_SOURCES,
                now,
            )
    # NOT hygiene: a GIN with fastupdate on parks a bulk load's entries in an
    # unindexed PENDING LIST and ``gincostestimate`` charges every GIN plan for
    # scanning it, inflating the estimate roughly tenfold so the planner rejects
    # its own index. Migration 0048's header carries the measured before/after
    # and the operator-facing consequence. VACUUM cannot run inside a
...
@pytest.mark.xdist_group(name="postgres_element_union_plan")
def test_element_title_leg_enters_through_the_0048_partial_gin(element_plan_database):
    """The reason the rewrite exists, pinned on the SQL the adapter issues.

    NO PLANNER KNOBS -- no ``enable_seqscan=off``, no dropping of competing
    indexes. That is deliberate and is why this test carries a 30k-source
    corpus: a plan assertion that needs a knob pins the cost model rather than
    the change. Under the retired cross-table OR no index could serve the title
    arm at all; here leg B is an independent, separately-plannable relation
    rooted at its own ``notebook_id=`` equality, so migration 0048's
    ``idx_sources_nb_title_file_trgm`` applies -- provided the leg keeps
    spelling ``LOWER(title) LIKE`` (the index's expression key) and keeps its
    ``notebook_id`` / visibility literals INLINE (the partial index's
    implication under a generic plan). Break either and this goes red.
    """
    with element_plan_database.connect() as connection:
        spy = _CapturingConnection(connection)
        rows = notebook_element_rows(spy, _ELEMENT_PLAN_NOTEBOOK, "zqxjtitle", 20)
        statement, params = spy.captured[0]
        plan_text = "\n".join(
            str(next(iter(row.values())))
            for row in connection.execute(
                f"EXPLAIN (COSTS OFF) {statement}", params
            ).fetchall()
        )

    assert [row["id"] for row in rows] == ["esrc-000001:el"]

    # BOTH keys must be inside the Index Cond, which is why this reads the line
    # after the scan node rather than just grepping the index name. Measured:
    # spelling the leg ``title ILIKE`` instead of ``LOWER(title) LIKE`` still
    # names this index in the plan -- with ``Index Cond: (notebook_id = …)``
    # alone and the title match demoted to a heap ``Filter``, i.e. "read every
    # row of this notebook and recheck", the exact degradation migration 0048's
    # short-needle trade-off note describes. A test that only grepped the index
    # name would have called that a pass.
    lines = plan_text.splitlines()
    scan = next(
        (
            index
            for index, line in enumerate(lines)
            if line.strip() == "->  Bitmap Index Scan on idx_sources_nb_title_file_trgm"
        ),
        None,
    )
    assert scan is not None, (
        f"the title leg must enter through migration 0048's partial GIN:\n{plan_text}"
    )
    index_cond = lines[scan + 1].strip()
    assert index_cond.startswith("Index Cond:"), plan_text
    assert "lower(title) ~~ " in index_cond, (
        f"the trigram key must be inside index access, not a post-scan "
        f"recheck:\n{plan_text}"
    )
    # notebook_id INSIDE index access too -- the cross-notebook global-bitmap
    # failure docs/operations.md records for the legacy single-expression
    # trigram indexes.
    assert "notebook_id = " in index_cond, plan_text
    assert "Filter: (title ~~" not in plan_text, plan_text
    # The structural win, independent of any one index: two separately
    # plannable legs under an Append, so nothing is a hashed subplan.
    assert "Append" in plan_text, plan_text
    assert "SubPlan" not in plan_text, (
        f"a hashed SubPlan is the retired cross-table OR's signature:\n{plan_text}"
    )
