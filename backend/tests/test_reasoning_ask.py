...
# --------------------------------------------- T4-B:终态贯通到 Ask 合成
# 设计稿 2026-09-07 §7.2 后半:类型化终态经
# `ReasoningResult → ReasoningEvidenceSnapshot → ResponseDraftInput` 走到合成,
# 合成 prompt 拿到一段服务端事实,最终装配之后按**实际进入 prompt 的证据身份**
# 复核方面支撑。

class _RecordingSeqLLM(_SeqLLM):
    """记下每一次 answer 调用看到的 prompt(合成侧的断言要读它)。"""
    ...
    def chat_json(self, messages, schema_hint, **kwargs):
        if "sub_queries" not in schema_hint and "next_action" not in schema_hint:
            self.answer_prompts.append(messages[0]["content"])
        return super().chat_json(messages, schema_hint, **kwargs)


def _synthesis_detail(resp):
    steps = [s for s in (resp.reasoning_trace or [])
             if s.step_type == "synthesis"]
...
def test_flag_off_leaves_the_synthesis_prompt_and_detail_byte_identical(arepo):
    ...
    assert "Retrieval status (server fact" not in llm.answer_prompts[-1]
    detail = _synthesis_detail(resp)
    assert detail
    assert not [key for key in detail
