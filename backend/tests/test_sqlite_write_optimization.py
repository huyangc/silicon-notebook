...
def test_all_writes_go_through_write_lock():
    ...
    # _migration_9(mention_edges/concept_comentions) 的 CREATE TABLE DDL 里 "ON DELETE
    # CASCADE" 外键子句触发 DELETE 关键字误报(非真实 DML 写),故与 _migration_1 同列入
    # 豁免——同属版本化迁移步骤范畴。
    ALLOW = {"_migrate", "_migration_1", "_migration_6", "_migration_7", "_migration_8",
             "_migration_9", "_recover_interrupted_jobs", "_seed"}
    cur = None
    in_block = False
    block_indent = 0
    offenders = []
