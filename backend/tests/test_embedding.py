...
def test_dashscope_embedder_batches_and_no_retries(monkeypatch):
    ...
    e = mod.DashscopeEmbedder(
        Settings(_env_file=None), base_url="https://x", api_key="k",
...
def test_dashscope_connection_pool_matches_service_capacity(monkeypatch):
    ...
    embedder = mod.DashscopeEmbedder(
        settings, base_url="https://safe.example/v1", api_key="key",
        model="model", max_connections=5,
    )
