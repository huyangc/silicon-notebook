...
ALLOWED_DYNAMIC_USER_ERROR = {
    ...
    "app/api/ask_routes.py::_validate_confirmed_reasoning_intent": (
        "同一函数里有两个动态 detail 站点。其一是 "
        "repo.resolve_reasoning_followup(...).gate_message——它恒由 "
        "query_intent.followup_gate 经 clarification_gate_message(seed) 产出:固定中文"
        "前缀「问题仍有关键歧义，请先确认问题理解」加至多 8 条 "
        "seed['ambiguities'][*].question。该闸恒以 client=None 调 plan_query_intent,"
        "所以 question 只可能是 query_intent.py 里两条硬编码的确定性澄清句(指代不清 /"
        " 纯泛化请求),不含用户原文、不含异常原文。PR-C 给这个闸加了跟进改写:文案的 "
        "seed 恒取自**原句**(followup_gate 的合同),改写模型的产物与它的异常都不进 "
        "detail(AskService._rewrite_followup_query 把未配置/异常/空回都吞成原句)。"
        "若将来给这个闸传真实模型 client 判歧义,模型自报的歧义句会进入 detail,本条"
        "登记理由即失效,须重新评估。其二是 "
        "user_error(422, str(exc)),exc 来自 query_intent.validate_confirmed_intent:"
        "它只会抛 query_intent.py 里三个中文模块常量之一(_INTENT_MISMATCH_MESSAGE /"
        " _MISSING_ANSWERS_MESSAGE / _EMPTY_RESOLVED_MESSAGE),finalize_query_intent "
        "的 ValueError 在那里被重新映射为这三句,用户原文与 pydantic 内部串都到不了 "
        "detail。给 validate_confirmed_intent 加第四种 raise 时本条登记须同步。真实"
