...
def test_mineru_max_retries_env_and_bounds(monkeypatch):
    monkeypatch.setenv("MINERU_MAX_RETRIES", "0")
    assert Settings().mineru_max_retries == 0
    for invalid in ("-1", "6"):
        monkeypatch.setenv("MINERU_MAX_RETRIES", invalid)
