...
class Settings(BaseSettings):
    ...
    vector_cache_per_family_entries: int = Field(
        8, ge=1, validation_alias="VECTOR_CACHE_PER_FAMILY_ENTRIES"
    )
    ...
    vector_cache_max_bytes: int = Field(
        # ge=0:负值会让 VectorCache 的 `> 0` 判断静默关闭保护(codex #634 R3 P2);
        # 0 是文档化的「关闭」语义,负数只能是配置错误,启动期响亮拒绝。
        16 * 1024 ** 3, ge=0, validation_alias="VECTOR_CACHE_MAX_BYTES"
    )
    ...
    # 「Agent 记录」里查看与清空。
    #
    # **叠在 ``agent_profile_enabled`` 之上,不是与它并列**(codex #616 R1 P1):
    # 记账的读写口都住在「AI 对这个库的理解」这个面板里,而总闸关掉时那个面板
    # 的入口按钮一个节点都不渲染。两把闸真做成独立的,就会出现「一直在记、但
    # 没有任何人能打开来看」的状态——记的每一行都是纯成本。所以写侧要求两把闸
    # 同时开;记账本身仍然与那条链路无关:不进 prompt(读侧在 SQL 里钉死
    # ``kind='note'``),也不参与任何整理触发。
    #
    # 关掉即零写入:判据在唯一收口 ``_record_agent_call`` 的最前面,关掉之后
    # 连事务都不开。**但读与清空不跟随任何一把开关**:既有的行照常能看见、能
    # 删掉——关开关是「从现在起不记」,不是「把记过的藏起来」。一份用户有权
    # 删除的数据,不该因为管理员翻了个开关就变成他既看不到也删不掉的东西。
