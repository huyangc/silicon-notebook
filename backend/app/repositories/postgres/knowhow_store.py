...
class KnowhowStore:
    ...
    def knowhow_table_title(self, table_id: str) -> str:
        """See the SQLite mirror / ``KnowhowStorePort.knowhow_table_title``.
        One point lookup by primary key, no rows/cells/columns touched."""
        with self.database.connect() as db:
            row = db.execute(
                "SELECT title FROM knowhow_tables WHERE id = %s", (table_id,)
            ).fetchone()
        if row is None:
            raise KeyError(table_id)
    ...
    def add_knowhow_row(
        ...
        with self.database.write() as db:
            ...
            # Parse the actual new cell text so direct row creation cannot
            # bypass rendered-asset validation.
            self._lock_required_assets(
                db,
    ...
    def update_knowhow_cells(
        ...
        with self.database.write() as db:
            ...
            # the same notebook.
            current_by_row = {
                row["row_id"]: row["content_md"]
                for row in db.execute(
                    "SELECT row_id,content_md FROM knowhow_cells "
                    "WHERE column_id=%s AND row_id=ANY(%s)",
                    (column_id, row_ids),
                ).fetchall()
            }
            self._require_assets_exist(
    ...
    def delete_source_asset_rows(self, source_id: str) -> list[dict]:
        ...
        with self.database.write() as db:
            rows = db.execute(
                "DELETE FROM notebook_assets WHERE source_id = %s RETURNING *",
                (source_id,),
            ).fetchall()
        return [_compat_row(row, "created_at") for row in rows]
