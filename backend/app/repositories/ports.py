...
class NotebookAccessRepository(Protocol):
    ...
    # 目标端写入围栏的读侧(docs/incremental-sync-design.md §5):非空 ⇒ 这本笔记本
    # 是从别的环境同步来的镜像,值是源环境标识;本地库与不存在的行都返回 ''。
    # 它**不是**授权判定——`_CAPABILITY_MIRROR_FENCE` 在能力守卫放行之后才查它,
    # 所以它既不带生命周期过滤,也不回答「存不存在」。
...
class AskStateRepository(Protocol):
    ...
    def guarded_ask_detail(
        self, job_id: str, *, actor_id: str, reader_id: str | None
    ) -> ContextManager[dict]: ...
...
class NotebookStorePort(Protocol):
    ...
    def apply_meta_for_notebook(self, notebook_id: str, *, guard_name: str, guard_generation: int, name: str, purpose: str) -> None: ...
...
class UnifiedKgStorePort(Protocol):
    ...
    @staticmethod
    def graph_seq_row(db: object, notebook_id: str) -> tuple[int, int, int, int]:
        """O(1) single-row ``(kg_mutation_seq, cluster_mutation_seq,
        mention_seq, kg_reset_epoch)``; ``(0, 0, -1, 0)`` when the notebook
        has no state row. ``kg_reset_epoch`` MUST stay the trailing element —
        existing callers consume this tuple by position (spread or ``[0]``).
    ...
    @staticmethod
    def clear_catchup_marker(db: object, notebook_id: str, ts: str, published_generation: int) -> None: ...
    @staticmethod
...
class RetrievalPort(Protocol):
    ...
    # The raw-passage retrieval pair (reasoning's ``search_chunks`` channel).
    # Spelled here as well as on ``AskCandidatePort`` because reasoning reaches
    # them through its own ``RetrievalPort`` handle, not through the ask
    ...
    # (``reasoning_retrieval.kg_in_scope_for``), not through the ask candidate
    # producer. Declaring it is what lets that call site be a plain attribute
    # call instead of a ``getattr`` probe with a catch-all fallback.
    def has_kg(self, notebook_id: str) -> bool: ...
...
class SharingStorePort(Protocol):
    ...
    def user_can_admin_notebook(self, notebook_id: str, user_id: str) -> bool: ...
    def user_can_read_notebook(self, notebook_id: str, user_id: str) -> bool: ...
    # 镜像标记的读/写(SQLite v79 / PostgreSQL 0059)。读侧喂目标端写入围栏;
    # 写侧只有跨环境导入器用——它走 repository 层,刻意绕过服务层围栏。
    def notebook_sync_origin(self, notebook_id: str) -> str: ...
