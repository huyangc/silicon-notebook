...
from app.repositories.knowhow_asset_refs import (  # noqa: F401 — re-export
...
# ``_rows_content`` / ``_column_cells_content`` / ``content_strings_in_payload``
# moved to ``app.repositories.knowhow_asset_refs`` (backend-neutral): BOTH
# maintenance adapters scan ``knowhow_changes`` for surviving ``asset://``
# references now, and the PostgreSQL one must not import this SQLite module
# to get at the extractor.  Re-exported here so the historical import site
# keeps working.
