...
# v84 adds source-side change capture, paired with PostgreSQL
# 0064_sync_change_capture.sql: the one-row gate ``sync_capture_control``
# (seeded by nobody -- capture stays off until an operator turns it on), the
# append-only ``sync_change_log`` plus idx_sync_change_log_table_seq, a
# unique surface for the two synced tables that never had a row identity
# (knowledge_object_sources, community_members -- de-duplicated first), and
# 138 AFTER-row triggers (three per synced business table) generated by
# ``app.migration.sync.capture``. See ``_migration_84``'s own docstring.
# v85 adds the two watermark columns the incremental exporter needs --
...
class SqliteMigrator:
                ...
                while budget > 0:
                    with self._connect() as db:
                        _n, after = UnifiedKgStore.reap_derived_generations_page(
                            ...
                            _RECOVERY_REAP_PAGE_ROWS, after=after)
                    budget -= 1
                    if after is None:
                        break
    ...
    def _migration_84(self) -> None:
        """Source-side change capture for cross-environment notebook sync.

        Paired with PostgreSQL ``0064_sync_change_capture.sql``; both
        backends' capture DDL is rendered from
        ``app.migration.sync.capture``, never written twice by hand.

        ``sync_capture_control`` is a one-row gate that this migration
        deliberately does NOT seed: an absent row reads exactly like a
        disabled one, so an upgraded deployment captures nothing until an
        operator enables it, and ``scripts/verify_repository_snapshot.py``
        can hold every migration-added table to "still empty".
        ``sync_change_log`` is the append-only log of changed row
        identities -- key columns only, never row content.

        The two synced tables that never had a row identity get one here.
        SQLite cannot add a primary key to an existing table, and rebuilding
        the table would register as a hard ``schema-sql-changed`` difference
        in the snapshot verifier, so the composite key is carried by a
        UNIQUE INDEX instead (PostgreSQL uses a real PRIMARY KEY). The two
        are equivalent where it matters: both back an ``ON CONFLICT`` target
        and both give the exporter/importer a unique surface to match rows
        by, and a guard pins the pair on both backends. Pre-existing
        duplicates are collapsed to their lowest rowid first -- these tables
        are derived KG membership, so a duplicate row carries no
        information the surviving one does not.

        Triggers are dropped by name and recreated rather than created with
        ``IF NOT EXISTS``: the text SQLite keeps in ``sqlite_master`` is then
        byte-equal to the generator's output, which is what lets the
        snapshot verifier compare all 138 of them literally.
        """
        # Local import: the generator reaches the PostgreSQL migration DDL
        # (for each table's primary key) and therefore psycopg, which a
        # SQLite-only process has no reason to load at module import time.
        from app.migration.sync.capture import sqlite_trigger_sql

        with self._connect() as db:
            db.executescript("""
                CREATE TABLE IF NOT EXISTS sync_capture_control (
                ...
                CREATE INDEX IF NOT EXISTS idx_sync_change_log_table_seq
                    ON sync_change_log(table_name, seq);
                DELETE FROM knowledge_object_sources WHERE rowid NOT IN (
                    SELECT MIN(rowid) FROM knowledge_object_sources
                    GROUP BY object_id, source_id
                );
                DELETE FROM community_members WHERE rowid NOT IN (
                    SELECT MIN(rowid) FROM community_members
                    GROUP BY community_id, canonical_id
                );
                CREATE UNIQUE INDEX IF NOT EXISTS uq_knowledge_object_sources_sync_key
                    ON knowledge_object_sources(object_id, source_id);
                CREATE UNIQUE INDEX IF NOT EXISTS uq_community_members_sync_key
                    ON community_members(community_id, canonical_id);
            """)
            for name, (_table, sql) in sqlite_trigger_sql().items():
                db.execute(f'DROP TRIGGER IF EXISTS "{name}"')
                db.execute(sql)

    def _migration_85(self) -> None:
        """Export-watermark state the incremental exporter reads back.

        Paired with PostgreSQL ``0065_sync_export_snapshot.sql``. See
        docs/incremental-sync-design.md section 7.

        ``sync_export_state.captured`` records whether the capture gate was
        OPEN while this watermark was being written. Only then does the
        change log actually contain every write made up to
        ``exported_through_seq``, which is the precondition for reading the
        next export as a window over the log instead of a full snapshot. It
        defaults to 0, so every watermark row that already exists when this
        migration runs reads as "not captured" and the first export after the
        upgrade is necessarily a full one -- correct, because those rows
        predate the column and carry no snapshot either.

        ``sync_export_state.exported_snapshot`` stores PostgreSQL's
        ``pg_current_snapshot()::text`` as taken inside the export's own read
        transaction. The next export needs it to compensate for transactions
        that were still in flight then and committed afterwards: their log
        rows carry a ``seq`` BELOW the stored watermark yet became visible
        only later, so a plain ``seq > watermark`` window would skip them
        ...
        with self._connect() as db:
            self.add_column_if_missing(
                db, "sync_export_state", "captured", "INTEGER NOT NULL DEFAULT 0"
            )
            self.add_column_if_missing(
                db, "sync_export_state", "exported_snapshot", "TEXT"
            )
            db.execute(
