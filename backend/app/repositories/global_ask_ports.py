...
class ReplacedJobUnavailable(Exception):
    """``create(replaces_job_id=…)`` named a job that is not the conversation's
    newest stopped job any more (already replaced, answered, deleted, or never
    was). The insert rolls back with it: nothing is created and nothing deleted."""
...
class GlobalAskStorePort(Protocol):
    ...
    def create(self, job: GlobalAskJob, user_id: str, request_id: str | None, request_json: str, submitted_via: str, *, new_conversation: bool, replaces_job_id: str | None = None) -> None: ...
    def discard_cancelled(self, job_id: str, user_id: str) -> bool: ...
    # ``error_detail`` is the raw failure text of a ``failed`` save; it lands in
    # its own administrator-only column, never in the owner-facing payload.
    def save(self, job: GlobalAskJob, user_id: str, *, error_detail: str = "") -> bool: ...
    def save_progress(self, job: GlobalAskJob, user_id: str) -> bool: ...
    # Administrator activity detail: the OWNER's job plus ``error_detail`` and
    # ``submitted_via``; ``None`` for a foreign or missing job.
    def admin_job_record(self, job_id: str, user_id: str) -> dict | None: ...
    # The same record under the activity-detail locking invariant: a context
    # manager whose transaction holds the job row and every participant
    # notebook's read-authority chain until the caller has assembled its
    # response. Yields ``None`` for a foreign/missing job, or -- when
    # ``reader_id`` is given (owner self-service) -- when any participant is no
    # longer readable by that reader. The record also carries
    # ``notebook_names`` (participants the owner can still read).
    def guarded_admin_job_record(self, job_id: str, owner_id: str, *, reader_id: str | None): ...
