...
* :class:`AskExecutionCoordinator` — ``start()`` runs the frozen baseline
  ...
  through the copied-context job helper
...
class AskExecutionCoordinator:
    ...
    def start(
        ...
        *,
        ...
        ``mode`` 必须是已解析的注册表引擎:路由层在建立任何持久状态之前就把
        请求里的 mode 归一成 AskMode(退役别名在那里映射掉),所以 ``ask_jobs.mode``
        从建行那一刻起就是真正执行的引擎 id,不存在事后改写。
        ...
        键在另一笔记本已用过 → 流只含一个 ``error``。
        """
        if mode is None:
            raise ValueError("start() needs a resolved mode")
        ...
        # ordering for both Ask paths).  The getattr is fail-open only for the
        # narrow unit-test doubles that intentionally omit the method.
        ...
        # 合成 start 步:只上流、不落 ask_trace_steps(基线语义)。紧随 started,
        # 报告的就是 job 行上那个真正执行的引擎。
        ...
        def worker() -> None:
            try:
                ...
                self._note_ask_completed(notebook_id, user_id, mode_id)
            except AskCancelled:
