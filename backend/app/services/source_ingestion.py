...
class SourceIngestionService:
    ...
    def __init__(
        ...
        *,
        ...
        # codex #638 R4 P2: the review-queue ranking memo used to need the same
        # explicit invalidation seam here (P0, R3 T-A1/T-A2) because
        # ``begin_extraction_run(preserve_existing=False)`` deleted the
        # source's graph without bumping ``kg_mutation_seq``. That gap is now
        # closed at the source (``RepositoryFacade._begin_extraction_run``
        # bumps in the SAME transaction as the clear), so the memo's own seq
        # gate is sufficient and this seam was removed — see
        # ``_invalidate_corpus_scale_memos``'s docstring.
    ...
    def _invalidate_corpus_scale_memos(self, notebook_id: str) -> None:
        ...
        codex #638 R4 P2:**审核队列排名** memo 曾经也在这里显式失效
        (R3 T-A1/T-A2,P0)——那时 ``begin_extraction_run(preserve_existing=
        False)`` 走 ``clear_source_graph_state`` 删掉该源全部
        knowledge_relations/knowledge_objects 却不 bump ``kg_mutation_seq``
        (只删不写)。这条豁口现已在源头堵上:``RepositoryFacade.
        _begin_extraction_run`` 把 clear 与 ``mark_unified_kg_dirty_in_tx``
        放进同一个写事务(见该方法 docstring),所以 seq 本身就会前进,
        ``ReviewQueueMemo`` 的 seq 闸单独就能挡住陈旧条目——这里的显式失效因此
        是冗余的,已移除。见
