...
class CommunityQueryService:
    ...
    def mounted_base_ids(self, active_notebook_id: str) -> List[str]:
        ...
        这是横向对比(共提/社区)唯一的库枚举入口:reasoning 的
        ``expand_community`` 与 chunk 模式的对比子查询各自逐库调
        ``resolve_comparison_peers``。收窄放在这里而不是两个调用点,是因为它们
        问的是同一个问题——「本轮可以向哪些参考库借同类实体」。

        必须收窄:兄弟**实体名**是从那个库的内容里读出来的,它们会原样进可见
        轨迹、进 ``used_queries``,并被回喂进 reflect prompt —— 取消勾选的库
        「一个字都不进 prompt」在这条通道上就不成立了。结果由 ``search``
        过滤并不足以补救:泄漏的是查询词本身,不是命中。

        判据必须是库维度而不是 ``source_scope_restricted()``:只取消参考库时
        后者恒为 False(R1),这条通道会全程敞着。
        """
