...
def reflect_prompt(
    ...
    *,
    ...
    read_document_action = (
        ...
        "afterwards. Prefer the rows the roster ledger marked (无摘要) — those "
        "have NO stored summary and are the documents nothing else in this run "
        "can describe; copy only the title, not the marker. It is "
...
def query_intent_prompt(question: str, max_topics: int = 6,
    ...
    return (
        ...
        "Your result_scope decides the retrieval executor unless the question's "
        "explicit wording says otherwise, so report confidence honestly: a "
        "widening beyond ranked with confidence below 0.5 is treated as a guess "
        "and ranked retrieval is used instead.\n"
