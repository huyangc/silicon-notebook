...
class GraphRetrievalService(_RetrievalState):
    ...
    def node_context(self, *args, **kwargs):
        # Deliberately ungated: this method's two callers each gate at their own
        # boundary, and neither is served by a gate here.  RetrievalService
        # .node_context filters the row it gets back (reasoning's chain
        # hydration).  EvidenceContextService.knowledge_context skips an
        # out-of-scope hit BEFORE calling this at all — it has to, because
        # emptying the row would still leave the object's NAME rendering into
        # the prompt behind a live `k{n}` anchor, the row being only the
        # definition/snippet half of what it prints.
