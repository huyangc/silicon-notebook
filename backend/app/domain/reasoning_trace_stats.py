"""检索 run 的内容无关统计投影(纯函数、零 I/O)。

这个模块是 `scripts/export_reasoning_traces.py`(导出)、
`scripts/analyze_reasoning_trace.py`(聚合)与本机检索测试
**共用的同一份投影**,理由与 `app.domain.retrieval_experience`
...
RUN_PROJECTION_KEYS: frozenset[str] = frozenset({
    ...
    "trace_steps",
    "trace_truncated",
    # 只有直接计时的调用方能填墙钟;仅有轨迹时保持 unknown。
    "run_wall_ms",
...
#: 是服务端收尾记账,进 `action_seq` 会给每条轨迹尾巴上挂
...
#: 从 reflect 的停止决定反推;fallback 与主动作答保持不同口径。
TERMINATION_MODEL_END = "model_end"
TERMINATION_MODEL_DEGRADED = "model_degraded"
TERMINATION_REASON_VALUES: tuple[str, ...] = (
    "step_budget", "stale", "no_executable_action",
    TERMINATION_MODEL_DEGRADED, TERMINATION_MODEL_END,
    # Persisted historical facts remain readable after their runtime is retired.
    "model_sufficient", "model_partial", "retrieval_degraded",
)
...
def _recorded_termination(steps: Sequence[Mapping]) -> str | None:
    """Read persisted terminal facts before applying current-policy inference.

    A present but unrecognized value is unknown, never permission to infer a
    successful stop from an earlier reflect decision. These historical fields
    are read-only; they do not enable a retired retrieval strategy.
    """
    for step in reversed(steps):
        detail = step["detail"]
        if (step["step_type"] == "skip"
                and _reason(detail) == "retrieval_termination"):
            return _closed_exact(
                detail.get("termination"), TERMINATION_REASON_VALUES)
    for step in reversed(steps):
        detail = step["detail"]
        if (step["step_type"] in ("answer", "synthesis")
                and "termination_reason" in detail):
            return _closed_exact(
                detail.get("termination_reason"), TERMINATION_REASON_VALUES)
    return None
...
def project_run(
    ...
    *,
    ...
    termination = _recorded_termination(normalized)
...
def project_search_run(
    ...
    *,
    ...
    `ReasoningRetriever.run`,换一份投影只会让两边口径分叉);这里只补
    `project_run` 从轨迹里读不到的范围与合成事实:
