...
def _snapshot_origin_sidecar(snapshot: Path, source: Path) -> Path:
    """Per-source origin sidecar path, keyed by a digest of the resolved source.

    Snapshot names are content-addressed, so two byte-identical databases at
    different paths seal to the same snapshot file. A per-source sidecar means
    binding one source writes its own file and never touches another's — there is
    no shared record to read-modify-write, so concurrent seals of the same
    snapshot cannot drop each other's binding.
    """
    token = hashlib.sha256(str(Path(source)).encode("utf-8")).hexdigest()[:16]
    return snapshot.with_name(f"{snapshot.name}.{token}{_SNAPSHOT_ORIGIN_SUFFIX}")


def _write_snapshot_origin(snapshot: Path, source: Path) -> None:
    """Record that a sealed snapshot was created from ``source``.
    ...
    from a *different* database; a per-source origin record lets reuse fail closed
    on a mismatch instead of importing the wrong data under a receipt that names
    the selected source.
    """
    sidecar = _snapshot_origin_sidecar(snapshot, source)
    payload = (
        json.dumps({"source_path": str(Path(source))}, ensure_ascii=False).encode(
            "utf-8"
        )
        + b"\n"
    )
...
def _verify_snapshot_origin(snapshot: Path, expected_source: Path) -> None:
    sidecar = _snapshot_origin_sidecar(snapshot, expected_source)
    if sidecar.is_symlink():
        raise SqliteToPostgresMigrationError(
            "snapshot origin sidecar must not be a symlink"
        )
    try:
        # A missing sidecar (this source never sealed this snapshot) raises
        # FileNotFoundError here and is treated as a fail-closed mismatch below.
        ...
        raise SqliteToPostgresMigrationError(
            "existing snapshot was not sealed from the selected SQLite source "
            "(no matching origin record); re-create it for this source"
        ) from exc
    recorded = payload.get("source_path") if isinstance(payload, Mapping) else None
    if not isinstance(recorded, str) or str(Path(recorded)) != str(
...
def create_consistent_snapshot(
    *, source_path: Path, work_dir: Path, progress: Callable[[str], None]
    ...
    try:
        ...
        # Bind the sealed snapshot to its source (per-source sidecar) so a later
        # --snapshot reuse can reject a mismatched --source; a byte-identical
        # snapshot from another path writes its own record without touching this
        # one.
