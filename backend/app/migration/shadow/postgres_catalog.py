"""Migration-derived PostgreSQL v10 business-catalog validation."""
...
def _expected_columns() -> dict[str, dict[str, ColumnContract]]:
    ...
    for migration in _MIGRATIONS:
        ...
        for alter in re.finditer(
            rf"\bALTER\s+TABLE\s+({_IDENT})\s+ADD\s+COLUMN\s+({_IDENT})\s+([^;]+);",
            text,
            re.I | re.S,
        ):
    ...
    *,
    ...
    if collation_oid:
        ...
        if collation is None or _value(collation, "namespace", 1) != "pg_catalog":
            raise ValueError("PostgreSQL v10 index collation schema drifted")
    ...
    if opclass is None:
        raise ValueError("PostgreSQL v10 index opclass is missing")
    if not bool(_value(opclass, "opcdefault", 1)):
        if _value(opclass, "namespace", 2) != "public":
            raise ValueError("PostgreSQL v10 non-default index opclass schema drifted")
    ...
    *,
    ...
    for row in rows:
        ...
        if not isinstance(name, str) or name not in expected:
            raise ValueError("PostgreSQL v10 internal trigger contract drifted")
        grouped.setdefault(name, []).append(row)
    if set(grouped) != set(expected):
        raise ValueError("PostgreSQL v10 foreign-key trigger set drifted")
    for name, contract in expected.items():
        ...
        if contract.referenced_table is None:
            raise ValueError("PostgreSQL v10 foreign-key contract is incomplete")
        ...
        if (
            ...
            raise ValueError("PostgreSQL v10 foreign-key trigger contract drifted")
...
def _validate_identity_sequences(
    ...
    *,
    ...
    if len(rows) != len(expected_tables) or actual_tables != expected_tables:
        raise ValueError("PostgreSQL v10 identity sequence set drifted")
    for row in rows:
        if (
            ...
            raise ValueError("PostgreSQL v10 identity sequence contract drifted")


def validate_postgres_v10_catalog(
    conn: Any,
    *,
    business_schema: str,
    manifest: Manifest,
    ...
    """Reject missing, extra, invalid, or same-name semantic v10 drift."""
    ...
    if actual_tables != set(catalog_tables):
        raise ValueError("PostgreSQL v10 business table catalog drifted")
    for row in table_rows:
        if not _table_semantics_are_exact(row):
            raise ValueError("PostgreSQL v10 business table semantics drifted")
    ...
    if inherited is not None:
        raise ValueError("PostgreSQL v10 business table inheritance drifted")
    ...
    if any(
        ...
        raise ValueError("PostgreSQL v10 business table behavior drifted")
    ...
    if actual_columns != expected_columns:
        raise ValueError("PostgreSQL v10 column catalog drifted")
    ...
    if actual_names != set(expected_constraints):
        raise ValueError("PostgreSQL v10 constraint catalog drifted")
    for row in rows:
        ...
        if (
            ...
            raise ValueError("PostgreSQL v10 constraint definition drifted")
    ...
    if guard_names != expected_guard_names:
        raise ValueError("PostgreSQL v10 shadow guard contract drifted")
    actual_names = {str(_value(row, "index_name", 1)) for row in rows}
    if actual_names != set(expected_indexes):
        raise ValueError("PostgreSQL v10 index catalog drifted")
    for row in rows:
        name = str(_value(row, "index_name", 1))
        if not _index_flags_are_live(row):
            raise ValueError("PostgreSQL v10 index is not valid, ready, and live")
        ...
        if int(_value(row, "indnatts", 9)) != key_count or actual != expected:
            raise ValueError("PostgreSQL v10 index definition drifted")
    ...
    if extension is None or _value(extension, "nspname", 0) != "public":
        raise ValueError("PostgreSQL v10 pg_trgm extension contract drifted")
