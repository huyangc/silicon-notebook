...
（**接力必须一起推进**，否则前缀里的闸与真跑答得不一样），前缀外每窗按 1 次计，触顶必须
回报 `sampled`，并用 `skipped_windows_in_prefix` 解释「为什么调用数远小于窗口数」。
**窗口数不得只用字符算术**：`⌈全文字符 ÷ WINDOW_CHARS⌉` 会低报两次（元素整放留下的预算
空隙 + 密度拆窗，三个 7000 字符元素实际 3 窗、算术说 2 窗），两者都只增不减，所以它是
**下界**。前缀恰好覆盖全文（`sampled=false`）时取前缀真跑的窗口数（**精确**且零额外成本
——前缀本来就要读来估调用数）；触顶时取「前缀窗口数」与算术下界的**大者**并按**显式下界**
报出（前端文案「至少约 N 段」，`sampled=false` 才写「约」）。绝不为了数准而先把要估的那次
全文扫描做一遍。`sampled` 的条数那一半按**元素总数 > 读回行数**判，**不是**「读回行数 ≥ 上限」
