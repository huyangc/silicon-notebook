...
- **长任务按钮的忙碌态**：点一下就触发长任务的按钮（排后台 job、同步等完的重活、大体积上传），点完必须**立刻**不可点并换成按该动作语义写的进行态文案（「补齐向量」→「补齐中…」，不是笼统的「处理中」）；纯图标按钮改用 `.busy-spin` 转圈并同步 `title`/`aria-label`。理由是硬的：`backfill-vectors`/`reparse` 后端**没有单飞守卫**，POST 立即返回而活在后台跑，按钮不变化用户就反复点、每点一次就再排一份同样的活。两种形态二选一：`disabled={busy}` + 文案切换，或忙碌时整排 CTA 换「取消」/不渲染；不能什么都不做。解除忙碌位要**按证据**，且证据按**修复形状**选、不能一刀切（真源=`frontend/app/checkup-view.ts` 的 `repairRelease`/`isRepairing`，有单测）：逐轮修一批的（`reparse`）存触发时的 count，count 一变就恢复可点；一次修全库的（`backfill_vectors`）⚠**不能**用 count——H4/H5 口径排除活跃租约、job 跑的过程中 count 就递减，按它解锁会排出并发全库补齐（正是后端 `checkup.py` H4/H5 注释点名要防的重复模型调用），只在该组从体检结果消失时解除；⚠ 另注意 H4/H5 走带 30 秒 TTL 的进程内 memo，**补齐完成到计数归零之间还可能多等至多 30 秒**，忙碌位晚一个 TTL 解除是刻意接受的（提前放行 = 重复付费模型调用）。**按笔记本单飞的后台任务（「补上关联」「重新合并」）的忙碌位存的是「哪个库在忙」而不是裸布尔**——裸布尔会在「点完 A 切到 B」时让 B 的按钮变灰、让 A 的轮询刷 B 的图谱并清掉 B 的展开节点、回 A 又永远等不到刷新；判据/解除是纯函数 `busyForNotebook` / `claimNotebookSlot` / `releaseNotebookClaim`（`frontend/app/notebook-busy-set.ts`，两个动作共用同一份实现、有单测；`kg-relink-status.ts` 只是按历史命名再导出一遍），解除必须是「只清自己那一格」的函数式更新。⚠ 服务端把这两件事放进**同一个** per-notebook 单飞槽（重新合并整表重写 `concept_clusters` 与板块划分，补上关联只是把关系边追加进这张聚类正在读的同一张图——两者共槽是因为并发不只是多花一份钱，而是一方会在另一方还在读的输入上发布结果，不是因为两者做的是同一件事）：409 必须点名真正占着槽的那个动作，而各自的 status 端点对**另一种**任务一律如实回 `idle`，否则一条轮询会替它没发起过的任务空等到上限。轮询另需**尝试上限**（服务端只在进程内记状态，任务卡死时会一直如实回报 running），超限用中性文案。忙碌位还要随体检结果收敛（摘掉已消失的分组键），否则旧条目会被重新出现的同名问题继承、锁死新卡片。有界轮询窗口关闭与切库是共用兜底。⚠ `.sort-button`/`.icon-button`/`.ghost-button` 写死了 background/color，浏览器默认 `:disabled` 变灰对它们无效——`globals.css` 那条共用 `:disabled { opacity: .55 }` 是让禁用看得见的必要条件。真源见 `AGENTS.md` 前端章节，回归门是 `frontend/app/long-task-button-guard.test.mjs`（按 `onClick` 源码文本认入口、不含行号；`disabled={false}` 这类恒假值同样报红）。
