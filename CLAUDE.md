...
- **数据库后端选择**：DATABASE_URL selects the formal repository backend through one repository factory. Exactly one active repository backend is selected centrally from `DATABASE_URL`. SQLite and PostgreSQL are both available direct backends; SQLite remains the shipped default. 选择只存在于 factory；service/store 不判断 dialect，也不 import 对侧 adapter。`SHADOW_DATABASE_URL` 只保留且不参与选择/同步；当前 SQLite schema v31 仅包含两张 inert、无 payload 的 shadow capture 内部表，run-scoped guard/trigger 由迁移工具另行 disabled 安装，配对 PG 业务 schema 为 v9；PG 向量存 `bytea`，不安装也不需要 pgvector，生产仍固定 `--workers 1`。
