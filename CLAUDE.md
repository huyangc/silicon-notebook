...
- **来源选择检索范围**：每个可见导入来源都有由问答意图预检/执行和新建深度报告共用的复选框，默认全选。`include` / `exclude` 只能指向当前 notebook 的可见来源，已挂载的参考库仍参与；收窄范围时隐藏 Memory/Knowhow 投影证据不参与。本地范围为空且未挂库时，前端禁用问答输入和新建报告，API 同样拒绝；报告的提交范围贯穿意图确认、规划与生成。省略范围字段保持历史整库行为。
