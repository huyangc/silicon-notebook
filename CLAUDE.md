...
- **闸只有这两个，缺一不可**：🔴 P0/P1 与 ⚪️（解析不出标签但正文很长）**一律不自动合**——先修掉并重审到非阻塞，修不动或意见站不住时才停下来问人；CI 未全绿也不合。判 CI 用 `gh pr checks <PR号>`，只有全部 `pass` 才算绿——`mergeStateStatus: CLEAN` 只说没有冲突/没有必需检查在拦，不等于检查跑绿了。
