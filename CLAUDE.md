...
- **生产启动生命周期**：`npm run start` 默认先把 `backend/requirements.txt` 安装到 `PYTHON_BIN` 环境，并以 `npm ci` 按 lockfile 重建前端依赖；只有明确预装依赖的部署才用 `SKIP_INSTALL=1`。随后在前台 build，再用脱离 terminal 的后台进程拉起前后端；只有在 curl 连续两次确认后端 `ready=true` 和前端可访问、且本次两个 PID 仍存活后才成功退出。成功前的进程退出、超时或中断必须清理本次启动；正常停服只走 `npm run stop`。
