...
- **来源选择检索范围**：每个可见导入来源都有由问答意图预检/执行和新建深度报告共用的复选框，默认全选。`include` / `exclude` 只能指向当前 notebook 的可见来源；API 入口冻结选择，并用当前可见来源总数在服务端计算、覆盖 `narrowed`，所以冻结后的全选 include（包括单篇 notebook）仍是整库图路径，不得清空历史或关闭图通道；全选运行会在服务端快照当时已有的隐藏 Memory/Knowhow 参与者（不暴露、不写入公开 scope），真正收窄才排除它们。**进入上限的隐藏参与者按种类分别定范围**：Knowhow 投影是 notebook 级共享的（表格本身对每位成员可见），Memory 投影**按创建者私有**、只进创建者自己的上限——归属谓词（`memory_items.created_by`，与 Memory 检索同一条判据）必须写在取数那条 SQL 里而不是结果侧，别人的 Memory 源 id 因而根本不进本进程；否则共享笔记本里任何一位成员的默认全选请求都能经普通候选生成、以及未收窄时重新打开的全图/PPR 通道读到别人的私有 Memory。隐藏那一半刻意**单独一条查询**（`hidden_source_ids(notebook_id, owner_id)`）而不与可见那半合并：紧凑 `include` 冻结只读所请求的 id 加一个计数，合并会把整库来源行重新搬回热路径，去回答一个本来只受投影数约束的问题；它也只在未收窄时才发。孤儿 Memory 源 EXISTS 不成立即丢弃、fail closed。全选仍是可分区候选与结果的冻结上限，并发新增来源不得混入；若执行时可见来源或隐藏参与者全集已漂移，无法安全预过滤的整图/PPR/关系/精确章节/整库画像通道必须在 I/O 前关闭——**这次漂移复读必须用冻结时的那个身份**（随冻结带走，不在 worker 里重取）：隐藏那一半是按 owner 过滤的，拿不过滤或按别人过滤的实时读去比，共享笔记本每次请求都会被判成漂移、把这些通道永久关掉，而且不会有任何行为断言变红。真正收窄时，来源谓词必须在候选进入 Top-K 前下推并在 hydrate/合成后复核：chunk ANN 通过持久化的逐行来源 sidecar 在 HNSW 内过滤，旧索引缺 sidecar 时只降级到有界来源内 FTS，重建/fold 后恢复；KG、PPR 与精确查找种子都结束后若证据仍完全为空，先确定性补一次有界原文检索再交给 reflect。事后过滤不能单独授权图扩展，因为未选候选会占 Top-K 或提供隐藏推理前提。用户勾选是当前 notebook 检索范围的唯一来源，模型不得收窄或扩大；已挂载参考库仍参与，base KG 种子仍可不经组合图而直接映射 base 原文。本地范围为空且未挂库时，前端禁用问答输入和新建报告，API 同样拒绝；报告的解析范围在意图确认与生成前重验并贯穿规划、生成。省略范围字段保持历史整库行为。
