...
- **问答引用定位图谱节点**：从知识对象引用打开全屏知识图谱时必须选中并居中目标；目标不在有界高连接度核心图时，定向拉取并叠加其一跳邻域。引用携带的原始 Concept id 只可通过有界 `cluster_fold_rows` 解析为接口返回的 canonical `focus_id`，不得物化全 notebook cluster map。
