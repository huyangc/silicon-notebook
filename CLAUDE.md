...
- **解析能力注册表与生成问题索引**：上传校验、内置 parser 分派、云端格式准入、登录后系统配置与来源导入/详情 UI 必须从同一后端解析注册表派生；前端只收顺序、能力、执行边界、可用状态与固定原因枚举，绝不收 endpoint/凭证/路径/原始异常。路由自动选择，已配置自托管 MinerU 绝不能静默切公共云。生成问题召回是部署级可选补充，不是用户 scope 或 evidence：默认 `off` 零成本，`shadow` 只做有界评估与无正文计数且逐字返回 baseline，只有显式 `on` 才能在 baseline 后追加原 chunk，不能驱逐/重排；离线 `question-index` 只用 `chunk_question_generation` + `chunk_embedding`，在线先下推冻结来源上限、超扫描 cap 直接跳过，最终只 hydrate/引用原 chunk。空结果持久记完成、失败可重试，重解析/删除/复制/迁移保留原 chunk 所有权；精确数值只登记在 `docs/product-and-api*.md`。
