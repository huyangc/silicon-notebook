...
- **schema**：加表或改结构必须**追加** `_migration_N` 并 bump `SCHEMA_VERSION`，不要塞进已封版的旧迁移——版本闸会对已部署库短路，`IF NOT EXISTS` 救不了没被执行到的语句。当前 SQLite schema 为 v35；PostgreSQL checksummed schema 为 v13；除关系端点 keyset 覆盖索引和关系补全的来源代次水位外，两侧还包含生成中 Ask 的浏览器提交时间 `ask_jobs.asked_at`。
