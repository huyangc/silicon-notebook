...
- **深度报告可信度与全篇综合**：报告复用共享意图并持久化 `result_scope` / `completeness_required`；完整枚举未接入时不得把相关性检索伪装成“全部”。假设不是证据也不进入检索串。规划、正文和界面共用同一份有界「资料基础」画像与保守可区分资料分组。深度 ≥ 8 时必须先完成全部章节检索，再至多调用一次全篇综合蓝图，最后并行分节写作；可选框架、蓝图或主张账本畸形时 fail-open。高风险引证扫描只代表锚点覆盖、不代表语义蕴含；趋势置信度按身份可确认的可区分引用来源封顶，来源身份缺失时只能标为研究性；终审只审计、不改写正文。数值上限只登记在 `docs/product-and-api*.md`。
