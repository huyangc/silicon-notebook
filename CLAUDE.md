...
- **Markdown 单波浪线不是删除线**：渲染模型产出文本的 GFM 插件真源只有 `frontend/app/markdown-gfm.ts` 的 `remarkGfmPlugin`＝`[remarkGfm, { singleTilde: false }]`，Ask 回答/Memory 卡片、深度报告、报告公开分享页共用，调用方不得再传裸 `remarkGfm`。remark-gfm 4.0.1 默认 `singleTilde: true`（github.com 行为，GFM 规范不允许），**单个** `~` 就是删除线定界符：中文技术答案里 `7~5nm`、`80~90%`、`2~3 周`、`~3GHz` 这类写法同段出现两个，中间整段正文就被 `<del>` 划掉，无任何报错。规范的 `~~删除线~~` 逐字不变。唯一豁免是 knowhow 格子预览（`knowhow-cell-editor.tsx`）——它与 `md_normalize.py` 是按「单 `~` 是删除线」互相论证的孪生实现，单方面改会让护栏与渲染口径分叉，属独立一件事。回归门 `frontend/tests/guards/markdown-single-tilde-guard.test.mjs`（裸 `remarkGfm` / 真源被翻回 true / 绕过 JSX 直接 import 三条，外加三个渲染面的空转保护）与 `frontend/tests/component/answer-markdown-tilde.component.test.tsx`。
