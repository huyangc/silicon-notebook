...
- **清单出处与答案归因**：每个送达的元素/KG 清单条目至多携带一条仍存活的有界 `Citation`（KG 取首个有效 evidence element）；跨库证据必须由服务端在 active notebook 的 participant 集内解析，浏览器只展示摘录，绝不直连参考库成员端点。真正进入合成预览的条目使用隔离的 `k5001+` 命名空间和反向映射；只有答案实际绑定的这些锚点才能为清单内容归因，且仅带存活 `source_id`/`element_id` 的绑定键可把确定性枚举行判为 grounded。枚举答案一个锚点都没绑时，不得拿无关 ranked citations 冒充清单来源。
