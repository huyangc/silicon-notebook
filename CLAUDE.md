...
- **数据库后端选择**：DATABASE_URL selects the formal repository backend through one repository factory. Exactly one active repository backend is selected centrally from `DATABASE_URL`. SQLite and PostgreSQL are both available direct backends; SQLite remains the shipped default. 选择只存在于 factory；service/store 不判断 dialect，也不 import 对侧 adapter。`SHADOW_DATABASE_URL` 单独设置仍不参与选择/同步；当前 SQLite schema v31 仅包含两张 inert、无 payload 的 shadow capture 内部表。临时 shadow 边界已有严格只读 UTF8-first preflight、脱敏身份确认、checksummed PG control、revision CAS 和 SQLite/PG 两侧独立报告约束的四个 logical-key guard，固定配对为 SQLite31/PG9/epoch1、60 张复制表。Guard 安装后立即强制唯一性，capture/freeze 行为在 run control 状态启用前保持禁用。所有 PG control mutation 固定按 migration lock → control lock → 精确 catalog 校验；需要 SQLite live gate 时，必须先取得 PG pool/双锁/run row，之后才短暂 `BEGIN IMMEDIATE`，禁止持有 SQLite 时等待 PG pool/advisory lock。但 snapshot/COPY、replicator、运维 CLI 和端到端 worker 尚未实现，不能据此切换 `DATABASE_URL`。PG 向量存 `bytea`，不安装也不需要 pgvector，生产仍固定 `--workers 1`。
