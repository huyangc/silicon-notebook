...
- **PostgreSQL KG 名词法 KNN 早停**（`POSTGRES_LEXICAL_KNN_ENABLED` 默认关）：现状 `ORDER BY similarity` 的 LATERAL 无法早停，9.1M 行 base 上常见短词单词项 7.4s、多词项超时、整条词法臂 fail-open 阵亡；GiST `<->` 距离序按相似度增量吐行、到 LIMIT 即停（同库实测 123ms，60×，planner 经 ANALYZE 后自然选中）。**两阶段是证明不是启发**：只走 `%` 臂的 KNN 页取满 = legacy top-k 内不可能有 ILIKE-only 行（其相似度必低于 `%` 阈值），两径至多在等相似度并列类内选不同成员（实测 base 有 285 行同名 "DAC" 的 sim=1.0 类——登记接受的取舍，也是默认关、须真机 A/B 后才开的原因）；KNN 页不满的词项用 legacy 语句整体重跑并**替换**（不是并集），rank 重映射必须保留。路由三条件缺一即回 legacy 且 SQL 逐字不变：开关开、run 未按来源收窄（`allowed_source_ids is None`——受限 run 的词法是唯一候选来源且 scoped 形状无 bench）、库为大库（`copyable=False`，与自动建索引同判据；小库已有复合 GIN，KNN 反而要在全局距离序里翻找自己的行）。判据集中算在 `_lexical_object_hits` 一处；SQLite 适配器收下 `allow_knn` 并忽略（签名对等，service 不判 dialect）。启用还需一个**形状匹配**的 GiST 索引（`gist_trgm_ops` 单键、表达式为 payload name、partial 或全量皆可）——按形状而非名字探测（生产 bench 索引可直接转正，不必为命名重建 2.5GB），缺索引静默走 legacy；探测按 (dbname, 表 OID) 进程级缓存。chunk 正文与精确通道**刻意不接**：整段文本对短词的 trgm 签名是另一个长度域，无 bench 不搭车。
