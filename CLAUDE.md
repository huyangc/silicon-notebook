...
- **来源选择检索范围**：每个可见导入来源都有由问答意图预检/执行和新建深度报告共用的复选框，默认全选。`include` / `exclude` 只能指向当前 notebook 的可见来源，API 入口把排除式选择冻结为 include 硬上限并下推到有界候选生成；它与模型确认的 `QueryIntentContract.source_scope` 取交集，后者只能继续缩小。已挂载的参考库仍参与；收窄范围时隐藏 Memory/Knowhow 投影证据不参与，无法安全按来源预过滤的当前库全图/PPR/关系/精确章节/报告整库画像通道跳过，base KG 种子仍可不经组合图而直接映射 base 原文。本地范围为空且未挂库时，前端禁用问答输入和新建报告，API 同样拒绝；报告的解析范围在意图确认与生成前重验并贯穿规划、生成。省略范围字段保持历史整库行为。
