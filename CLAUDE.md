...
- **图谱类型覆盖**：管理员维护的全局类型是所有笔记本的默认基线；笔记本 owner 可在当前笔记本内 copy-on-write 改写继承类型、局部停用或新建本库类型，删除覆盖表示恢复继承，只读成员只能查看。全局写接口仍只认管理员，笔记本写接口仍走 owner 守卫；归纳候选按笔记本/创建者隔离。Ask 知识块、知识浏览/类型标签必须读取同一份 notebook 有效注册表，不能回退到裸全局表；来源的核心图谱抽取仍由独立的四类边契约约束，本注册表变更不重写历史对象也不扩张该抽取器。存在该类型知识对象时不得删除 notebook-only 定义。
