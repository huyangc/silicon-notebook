...
- **密码管理**：自助改密 `PATCH /me/password` 校验当前密码后改写，并吊销该用户**其他**浏览器会话（`keep_token` 保留当前请求的会话）；管理员重置 `POST /admin/users/{id}/reset-password` 吊销目标**全部**浏览器会话，且必须由真实登录会话发起（auth_optional 匿名回退 403）。Agent 长期凭据（`agent_access_tokens`）刻意不在两种吊销范围内。内置管理员 `user-local` 两条路径一律 409（`BuiltinAdminPasswordError`）——它的密码每次启动都被 seed 按 `SILICON_NOTEBOOK_ADMIN_PASSWORD` 重写，在线改动会被下次重启静默回滚；前端对它隐藏「修改密码」入口、用户总览该行密码列显示「受保护」，本人行显示「本人」（管理员重置自己会当场吊销自己全部会话，UI 不提供该入口）。
