...
- Ask 的原文段落通道（`chunk` 基线与 `reasoning` 的 `search_chunks` 共用）按参与集读取 chunk：当前 notebook 与本次勾选的参考库各出一条召回腿；可选 KG overlay / PPR 才可能加入 federated KG 上下文与 base-backed chunk，`graph` / `reasoning` 走 federated KG 路径。
- **联邦 chunk 召回（§11 检索和推理策略 / §19.2 两个检索平面与权威，`search_chunks` 联邦化）**：`backend/app/services/chunk_federation.py` 把「逐库」与「逐子查询」两层扇出压成一张扁平任务表，由 `CHUNK_FANOUT_MAX_WORKERS`（默认 8，等于联邦化之前那个字面量）这一个上限约束，所以数据库连接峰值不随挂载库数增长。参与集只经 `_RetrievalState._retrieval_participants` 这一个座位读取（确定序：active 在前，其余按挂载顺序），超过 `CHUNK_FEDERATION_MAX_PARTICIPANTS`（默认 8）按该序截断并发内容无关的 `chunk_federation_truncated` 事件。参与集 ≤1 时整条联邦路径短路回联邦化之前的单库腿，返回值逐值不变；`CHUNK_FEDERATION_ENABLED=0` 是唯一回退开关，把参与集收成当前库一本。跨库合并复用 `global_evidence.peer_evidence` 的「可比模式」（`CHUNK_FEDERATION_PEER_FLOOR`，默认 0.5：峰值低于全场最高 × 该值的库没有保底名额，剩余名额按跨库归一的分数/名次排；设 0 逐值回到每库保底一条的旧口径，全局问答那条调用点不传本参数因此逐字不变），交给它之前先在**库内**按 `source_chunk_content_key` + `prefer_stronger_chunk_candidate` 折叠（与单库多子查询腿同一对函数，所以同一段正文被语义臂与生成问题索引双命中时代表仍是带语义 support 的那条）；按参与集顺序聚池而不是线程完成序，所以同题同库两次结果一致；各库矩阵先按已选中的 chunk 取行再 `vstack`（整库矩阵不在内存里汇合），维度锚取 `resolve_runtime_dim` 或多数维、不符的那一库整份丢弃并发 `chunk_federation_dim_mismatch`（fail-soft），MMR 因此不会把外库候选系统性判成「完全不相似」。非当前笔记本的参与库把「该库当前可见来源」下推到生产者的 `LIMIT` 之前（`producer_explicit=False`，run 级 memo），隐藏的 Memory/Knowhow 投影结构性不进这条跨库通道；大库参与腿只借用已常驻的暖索引（`_RetrievalState._peek_warm_chunk_index`），暖索引缺席时直落 FTS 降级（事件原因 `peek_no_warm_ann`，与「大库没建索引」的 `large_library_no_ann` 分开，`scripts/diag_slow.py` 分桶），并跳过生成问题索引的补召回接缝，绝不冷加载 scale 索引、绝不写共享向量矩阵缓存、绝不为外库付问题索引的扫描解码成本。被取消勾选的参考库在参与集里就已跳过，零候选零 SQL。非当前库的参与腿一律跳过生成问题索引的补召回接缝（不只是大库那一支）：那个接缝以**请求方的** `actor_id` 在外库上跑，而「请求方不是该库成员」这条语义没有用例保证，成本又是每个（库, 子查询）一次无 memo 的扫描解码；越权面不变（天花板仍下推，可见来源按构造排除隐藏投影）。**选择层是联邦感知的**：`per_query` 仍然是每个子查询一组（各库对同一子查询的命中并进同一组），所以挂载库数不会重切 `quota_fuse` 的配额分母；组里每条候选带的是该子查询**自己**的相关度、provenance 取跨子查询合并后的代表（与单库腿 `_retrieve_chunks_multi` 的 `per_query` 同形——统一用折叠后的最大值会让重叠的召回窗在 `quota_fuse` 里并列、并列取最小下标后整片塌进第一组，后面的查询方向一席不得）；跨库合并按「基线优先」两轮走：`peer_evidence(limit=CHUNK_RECALL)` 只花在基线候选上，generated-question-only 的补充候选走独立的第二轮再追加（同 `hit.text` 的补充不额外占位），因此补充只加宽候选池、绝不挤掉另一个库的基线段落，`docs/product-and-api.md` 的 append-only 契约跨参与集同样成立；当前笔记本另有保底份额 `CHUNK_FEDERATION_ACTIVE_RESERVE`（默认 0.25 → `ceil(chunk_mmr_k × 0.25)` 席），两条分支共用一个纯函数 `retrieval.enforce_active_floor`，都在**最终选择之后**执行一次——multi 腿在 `quota_fuse_baseline_first` 返回前、single 腿在 MMR 之后（`ask_chunk` 在那之前已把关键词/精确命中并进候选，所以那里就是最终选择）。保底不写进 `quota_fuse` 的分组，因为 `ask_chunk` 在拿到分组之后还会追加自己的关键词/精确命中分组，写在输入里的规则会被后加的分组绕过。floor 由候选映射自己带着穿过 `ask_chunk`（`chunk_federation.FederatedCollected.active_reserve`，端口层按来源范围重建后用 `with_active_reserve` 接回；没有保底时仍是普通 dict）。让出席位的是排名最靠后的参考库条目（先让 generated-question-only 的补充项），当前库的条目一条不动、长度不变，候选按 `hit.text` 去重（与 `peer_evidence` 同一把尺），所以同一段正文的多个来源副本只花一个席位；合格候选不足时以实际数量为上限，设 0 关闭、回到纯相关度。保底所需的当前库候选在**跨库合并那一步**就被先行保留（`chunk_recall` 的预算是硬上限：强库淹没时当前库可能只剩个位数候选进池，两条选择分支都只能从池里挑），保留占用预算而不是额外追加，并且一样要过 `peer_evidence` 的合格线（按保留前的本库峰值算，被它拒掉的候选也不再回到竞争里）。逐库准备（可见来源枚举）在父线程里做，所以每个参考库各自做失败隔离：读不到清单就跳过那一个库并发一条内容无关事件，绝不退化成不带天花板去查它，也不影响健康的库与当前笔记本，被跳过的库不出现在本次的参与者列表里；KG 反查腿的逐 owner 天花板（`_kg_peer_source_ceilings`）同款隔离，事件是 `kg_peer_ceiling_skipped`。当前库自己的命中**不打 `notebook_id`**（保持空串，与单库路径一致），「空 = 当前库」因此是唯一判据。扇出闸下沉到每一次真实的 `_retrieve_chunks` 调用（`chunk_federation._retrieve_for`、单参与者短路腿、`_retrieve_chunks_multi` 的每个子任务），编排者不再持槽：Ask 路径不设 `fanout_limit` 因而逐字 no-op，报告路径下跨库叶子 I/O 真正受 `REPORT_RETRIEVAL_FANOUT` 约束。候选池入口量级如实记：每条召回腿仍是 `CHUNK_RECALL`（默认 200），所以上界是「参与库数 × 子查询数 × 200」条带正文的候选进折叠与合并，合并后由 `peer_evidence(limit=CHUNK_RECALL)` 压回单库同量级；生产建议先按 `CHUNK_FEDERATION_MAX_PARTICIPANTS=4` 灰度。无图早退的放行判据（`AskService._no_kg_scope_admits_run`）随之改成参与集口径（`collection_map.sources`），回退开关下退回 `collection_map.active_sources`——这只是**判据口径**对齐；「空笔记本 + 无图参考库」端到端仍不可用，因为 `NotebookSummary.ask_available` 的参考库判据还是「参考库有可用知识图谱」，已登记在 `fangan_todo.md` 检索一节。跨库段落进入回答后 `Citation.notebook_id` 为来源库、tier 为 `base`。**mix 的第 2 路（KG-overlay 源 chunk）随之对称**：`_chunk_kg_overlay` 今天就会返回参考库的知识对象，但 `_kg_source_chunks` 把它们的 evidence element 反查回 chunk 时钉死 `ce.notebook_id = active`，结果是「引了参考库的知识对象，却拿不出它的原文」；现在 `_mix_retrieve` 把 `render_subgraph_context` 已写进 id_map 的归属（已过共享的 `foreign_notebook_id` 归一，覆盖整个 1-hop 子图而不只是种子）整理成 `object_owners` 传下去，反查按 owner 分组、各查各库。**外层两层循环一字未动**，所以喂给 rerank 的 first-seen 序（`object_ids` 输入序 → 对象内 evidence 序 → 该 element 的 chunk 列表序）逐字成立；`object_owners=None`（`CHUNK_FEDERATION_ENABLED=0`，或本轮 overlay 压根没命中外库对象）时逐值回到改动之前。参考库这一侧的约束与 peer 召回腿同源：库维度用 `notebook_in_scope` 先判（纯 ContextVar 读，被取消勾选的库连一次来源枚举都不发生），来源天花板走 `all_visible_source_ids` 并**复用同一个 run 级 memo key** `("federated_chunk_visible", nid)`（两条腿看到同一份冻结快照，不会「向量腿引得到、KG 腿引不到」），再与 `scoped_allowed_source_ids` 取交，过滤发生在候选被构造出来之前、更早于 rerank 输入，结果边界的 `filter_retrieval_items` 仍是 fail-closed 后盾。有界性：evidence 与 element→chunk 都是按 id 的点查，`rows_by_ids` 不带库谓词因此一次读齐；参考库一侧**只允许**已回填的 `chunk_elements` 点查，`_elem_chunks_scoped` 的另一支（对整库 chunks 全量扫描 + 逐行 `json.loads` 的 `_elem_chunk_map`）对外库禁用——未回填的参考库这一轮不出原文并发内容无关的 `kg_source_chunks_peer_unindexed` 事件，当前库两支照旧。参考库的 chunk 打 `notebook_id=owner`，当前库的仍不打标。
...
- 备注：`/notebook-templates` 端点与 `notebook_templates.py` 曾长期无人使用，已于 2026-09-07 随待办收尾 PR-1 删除（见「待办对账（2026-09-07）」条目）。
...
- **官方 MCP Streamable HTTP**：`/mcp` 提供十一个工具：Memory/context 七工具 `list_notebooks`、`select_notebook`、
  ...
  `knowledge:read`、代码写入需 `knowhow:code`）。**以上是本条目交付当时的工具面；后续已扩展至二十八个工具
...
- **生产热路径修复批 3·W3（大库禁用切换索引管线，决策 D3，2026-09-04）**：修审计 WR-2 的先行方案——「保存索引管线」的发布事务（48k 次 N+1 聚合 + 整库 staging 进内存 + 单事务重写整库）在 9M 对象量级上事实不可完成，每次点击白付 30s 连接 + 整轮回滚。按计划决策点 D3 用一天级显式禁用替代一周级重构：`begin()` 在铸 generation 之前、rebuild-active 检查之后，按活跃对象数阈值（`INDEXING_PIPELINE_SWITCH_MAX_OBJECTS` 默认 20 万，`count_active_objects` 的 seq-gated memo——刻意不用低三个数量级的拷贝阈值）拒绝**非内建目标**的变更；**切回内建（desired 为空）豁免**——卡在缺席/失败自定义管线上的大库被 `require_write_admission` 全写 fail-closed，切回内建是唯一自助出口，恢复重建再失败也只回 retryable。干净态的无变化幂等保存仍走早退。路由 409 明确文案；GET 投影新增 `large_library_locked` 服务端真值，前端只禁自定义选项与「重试当前自定义管线」（内建恢复失败态保持可重试），切回内建保持可点并就地说明。重构排到有真实需求时。

- **生产热路径修复批 3·W4 PR-A（写路径杂项收尾 T1/T2/T4 + T5 证伪，2026-09-04～09-05）**：设计真源 `docs/superpowers/specs/2026-09-04-batch3-w4-misc-design_zh.md`（v3，两轮内评 + 每任务实施后双内评）。**T1（WR-3）**：「全部重新分析」受理路径不再整库枚举——create_job 落 `total_sources=0`（stage=probing，前端既有「正在连接模型服务…」无数字），worker 起跑用与主循环同一 `target_mode`+limit+retry_partial（rebuild 在删除相位后）计数并经 `extend_total_sources` 在切 extracting 前回填；`total_targets` 改读 worker 自算值（快照恒 0 会静默跳过起始探测——变异+retry_partial 全-partial 库双反向钉子）；total 语义「起跑即准」双语登记（终态 total=0 = 无目标或计数前失败/中断，「已中断 0/0」文案 delta 已登记）；端口 +`extend_total_sources`（ports 棘轮 945→946）。**T2（WR-7）**：`delete_source` 唯一写事务退出 notebooks 行 FOR NO KEY UPDATE 容量锁（调用点传 None），只留 sources 行 FOR UPDATE 载重；四条论证入注释，其中死锁一条为**有条件**表述（依赖「notebook 出生即种 unified_kg_state 行→尾部 upsert 恒走 DO UPDATE 不取 FK 父行锁」不变量）；PG 锁行为 + 调用形状双回归钉。**T4（QueryCanceled 统一出口）**：全局 handler 非流式 503+`code=query_timeout`+事件带请求 id；流式腿由最外层纯 ASGI 观察者记账（BaseHTTPMiddleware 的 body 迭代在装饰函数栈外，handler 与 log_requests except 都到不了——实测钉住）；维护站点普查（负结果清单进 PR 描述）查出五处「整表 DELETE 裸跑默认 30s」同形站点，统一 `_raise_statement_timeout_floor` 事务内**取底**放宽（GREATEST 不压运维值、0=禁用保持，真库三分支验证）。**T5（SR-1 遗留，UNION 拆腿）——实施后被真机差分测量证伪并整体 revert**：旧 OR 形态靠 `uq_source_elements_ordinal` 早停、title 臂是喂早停最密的一臂，拆腿后腿 A 无索引退路必须自证为空，PG title 密集命中 179–870×、SQLite 千倍级退化，收益场景仅 0.86–0.90×；SR-1 按「测量后不做」结案，重试前置 T-0 生产测量（证伪全文与数据留档设计稿 §T-W4-5）。

- **生产热路径修复批 3·W4 PR-B（T3/WR-9 scale build 图侧有界化，2026-09-05）——热路径修复计划全部实施项至此收官**：设计同真源 §T-W4-3（实施偏离三处已回写设计稿）。**T3a 图侧取数 keyset 分页**：objects/chunks 按全局 UNIQUE `ordinal`、relations 按 `id COLLATE "C"`、clusters 按 `(canonical_id, member)` 且 **published 代次循环前求值一次逐页绑定**（标量子查询逐页重求在 READ COMMITTED 下翻代提交落在两页间会跨页混代——同一 member 挂两个 hub 且撕裂图带新代版本身份）；`notebook_object_evidence_rows` **双模**——在线检索入口逐字保留旧无序 fetchall（新 `ORDER BY id` 实测 +31% 且单页库落盘排序，踩检索红线），分页只走 `_paged` 独立入口供 scale build gather；`id_element_rows` 按 ordinal（id 序会改未回填库 KG 溯源首见 chunk）。**T3a ANN 分页喂入**：`vector_pages` 每页独立取连接（原版连接横跨小时级 add_items/knn_query，POOL_MAX_SIZE=1 合法部署下 API 全卡死）、resize 几何增长、KG 腿第二遍分页读按 id 映射回第一遍 hnsw label（漂移按 id 交集丢行 fail-safe）、`keyset_pages` 游标严格单调断言（`>`→`>=` 变异原本是 1.5GB RSS 死循环不是测试红）、同义边 fail-open 收窄到 hnswlib（DB 错必须响亮，不许静默持久化空边集 manifest）。A/B：16k 语料 +2.99%（门 ≤10%）；900 页规模以 page=500 EXPLAIN（全部腿 range 续扫非重扫）外推，复现脚本 `scripts/bench_scale_build_paging.py` 入库（非 CI）。**T3b viz 大库闸**：`_spawn_viz_build` 两调用点按 `viz_sync_build_max_objects` 都闸（无产物分支 count 构造性恒超限=删掉 API 进程内大库懒 viz 生产者，如实注释）；stale 供图按 **disk_signature (mtime_ns,size,ino)** 进热缓存槽（发布 rename 换 inode 自然失效；闸后无缓存=每请求全量重载百万级 list）；rebuild 尾部大库分支 **rearm_auto_index**（once-per-process set 唯一 re-arm 点在 rebuild 刻意不走的 dirty choke point 上，「离峰恢复」原本常态为空；真实复现形是被检索/入库路径消耗过的 once-set）；`maybe_auto_index` 前置四道如实登记（auto_enabled+**NOT** copyable+状态闸+once-set）。前端画布第四态「库规模较大，图谱预览将在下一次索引构建后可用」，裁决 visibleNodeCount>0 恒显 graph（base 挂载深链叠加邻域不被卡片盖住）；`viz_building` 改真值+互斥 `viz_unavailable`；product-and-api 双语。每小项 spec+quality 双 opus 内评，T3a 双阻塞、T3b 双 P2 全修。剩余均用户侧：T-0 生产只读测量、PG 调参 sudo、批 1 索引 --apply 窗口。

- **待办对账（2026-09-07）**：`fangan_todo.md` 按当前代码逐条重写。确认已落地但此前仍挂在待办上的项：KG 切窗跳过 references / bibliography / index / glossary 等 backmatter 小节（`kg/filters.py`）；跨文档合并候选改批量单事务写入；rep-pair 配对改 hnswlib ANN top-k（`kg_merge.py`）；分窗改贪心打包、抽取并发按模型服务 `max_concurrency`；Knowhow 版本历史 / diff / 回退端点；逐步推理大纲协同三期（PR #407 / #411 / #418）；在途提问接回（PR #661 / #662 / #664 / #665）。`docs/superpowers/specs/2026-07-28-reasoning-enumeration-tools-design.md`、`docs/superpowers/specs/2026-07-08-in-progress-action-resilience-design.md` 与多领域基准库 followups 的状态头同批改为已交付。仍未做的项见 `fangan_todo.md`。同批小清理（计划 `docs/superpowers/plans/2026-09-07-todo-closeout.md` PR-1）：删除零调用方的 `GET /notebook-templates` 端点、`NotebookTemplate` 模型、`notebook_templates.py` 与门面 / 端口方法（ports 棘轮 949→948 同 diff 下调，OpenAPI 契约 fixture 再生成）；删除 `repository_facade._citation()` 死代码；扩展 SDK `deployment.py` 的 `TODO(T7)` 经核实两份部署插件 SOP 早已写明 `configure` 成本规则与 capability 命名规则，改为指向 §3.2 / §3.3；KG 抽取超时按代码结案——KG 请求走流式传输，`KG_LLM_TIMEOUT_SECONDS` 约束的是连接与相邻分块间等待而非整条回复墙钟，默认 60s 不截断持续出 token 的长抽取，契约句已在 `docs/product-and-api*.md` 补明。
- **待办收尾 PR-2：多领域基准库合入后遗留 A1–A9 全部收口（2026-09-07）**：真源 `docs/superpowers/specs/2026-07-19-multi-domain-bases-followups.md` 对账表。A1/A2：引用来源库 id 归一化收敛为唯一 helper `app/domain/citation_origin.py::foreign_notebook_id`，`test_citation_notebook_id_guard.py` AST 守卫钉住 Citation/AnswerAnchor 构造关键字、五处 id_map builder 的字典写入、splat/别名与 canonical import；graph 模式 `render_subgraph_context` 改为必填 `active_notebook_id`，挂载的私有库节点与 spreadsheet 工作簿引用也带库 id 与 tier。A3/A4/A5：晋升目标数据源统一为 `NotebookSummary.base_notebooks`（删 owner-only `listBases` 拉取与假「需先挂载」门控），共享 `PromotionTargetModal`（≥1 候选皆为可点击项、无自动提交），`target_base_id` 为空的存量候选批准置灰并紧邻提示回填脚本。A6：深拷贝携带对接收者仍有效的挂载边（导入的 `MOUNT_VALID_EXPR` 以目的库求值；校验与插入之间基库被删按丢弃计），无效边丢弃并记日志，`notebook_bases` 是唯一允许行数不等的拷贝表。A7：SQLite `migrate()` 对库版本高于代码记日志并抛「schema contains a future version」，`docs/development*.md` 写明单向性。A8：`test_mount_sql_contract.py` 形状守卫（运行时提取、两侧独立比对、相对路径豁免）。A9：`docs/product-and-api*.md` 补 promote 端点 `target_base_id` 与 400 态。每任务 spec+quality 双内评 + 修复轮；本机 PG lane 675 项通过。
- **待办收尾 PR-5：架构渐进整改阶段 5「前端 workspace 状态拆分」三片交付（2026-09-07）**：计划 `docs/superpowers/plans/2026-09-07-todo-closeout.md`（三段「设计定稿」实施前写入）。slice 1（PR #686）：晋升队列与关系审核队列从 `page.tsx` 抽成 `use-promotion-queue.ts`+`promotion-queue-modal.tsx`、`use-edge-review-queue.ts`+`edge-review-modal.tsx`，协调器接触面收窄为 `Pick<RootModalCoordinator,…>`，关闭回调带 reason 保住按槽位 backdrop 策略；`root-modal-boundary` 守卫加强（close sink 两个 case 块调用清单精确等于 `clearQueue`）。slice 2（PR #687）：来源栏「本库来源」搜索/列表/分页抽成 `source-list-panel.tsx`（Fragment 保持 `.sources-body` 直接子节点），`source-title.ts` 单一定义；守卫按模块各钉徽章与 Loader。slice 3：全屏知识图谱视图抽成 `kg-graph-view.tsx`（ForceGraph2D 动态导入与画布绘制函数随迁，状态/ref/effect 留在 page 与 KG hooks，分析浮层经 `children` 原位渲染），共用叶子迁 `kg-object-cards.tsx` / `relative-time.ts`；`long-task-button-guard` 按模块查找，`root-modal-boundary` 反回填。三片均零行为变化（评审做 token 级 diff）、双内评、codex 闭环；`page.tsx` 8903 → 8021 行。真机对照截图未做（本机端口被开发服务占用），以组件测试与结构守卫为证据；后续继续减负登记在 `fangan_todo.md`。
- **Deep Report 正文引用图片内联（第二期）**：已交付（2026-09-07）。一期只做 Ask 与公开会话，二期把同一条管线接进站内报告正文——复用同一个 `rehypeCitationImages`（块级定位、按资产全篇去重、`CitationImageOrder` 记账）与抽成共享件的 `inline-citation-images`，点开走 Ask 同一格 `answer-image-preview` root modal 席位；引用详情「本段附图」区块保留（它回答的是「这一条引用挂了哪些图」，与正文去重后的落位不重叠）。key 直接对齐：`report_engine` 写 `content_md` 前已把各节标记全局重编号成一个 `k1, k2, …` 命名空间。导出零影响（`.md` 下载/复制/批量导出都逐字消费已存 `content_md`）。公开报告页 `/r/{token}` 仍不出图：其白名单不投影 `asset_id`/`element_id`，报告也没有匿名读资产的端点，要做需同时改投影并新开匿名端点——该决定由 `frontend/tests/guards/citation-image-surface-guard.test.mjs` 反向钉住。
- **待办收尾 PR-3：待办中心露出「进行中的提问」（2026-09-07）**：在途提问接回设计 §6.3 的可选项落地。`pending_actions` 投影经既有 `pending_actions_projection_rows` 接缝新增本人 `ask_jobs.status='running'` 项（`created_by = user` + `read_access_clause()` + `NOTEBOOK_LIVE_SQL`，SQLite/PG 同构；`ask_jobs` 无 `queued`，重启恢复把残留 `running` 改 `interrupted`，故按正匹配而非终态排除），投影字段为 job/notebook/会话 id、笔记本名、问题预览（截断）与开始时间；`pending_bus` 只在 ask 起跑与终态各推一次（流式 worker 首句与 `events.put(None)` 之后的 `finally`、提交失败路径；同步 `ask_current` 外层 `try/finally`），轨迹步不推。前端铃铛新增「进行中的提问」分组（首位、显示经过时长、面板打开且有项时每 30 秒刷新），**不计入未读**（用户刚发起的动作不该点亮铃铛），点击经原子 notebook opener 打开笔记本与会话并复用既有接回路径。行数/预览长度/刷新周期只登记在 `docs/product-and-api*.md`。双内评通过；本机 PG lane 通过。
...
- **v1.0 企业**：RBAC / source 级权限 / 企业业务审计 / 真实IDaaS联调与SSO生产环境验收和切换（插件及迁移代码见第3节） / 私有部署 / Confluence·SharePoint·Jira·Git·Slack connectors / 多 notebook 搜索 / rule version diff。
