...
- **用户闭环**：记录行详情和以行标题组织的矩阵分支都提供“智能补全空列”；前端会提交该行全部可补空列，服务端只为本次请求中当前值严格为空字符串或尚无 cell 记录的非行标题列生成建议，返回建议、置信度、依据与参考行，不自动写入。用户逐项确认后才复用既有单元格保存或合并单元格批量保存路径。
- **有界、保守推断**：新增 `POST /api/notebooks/{notebook_id}/knowhow/{table_id}/rows/{row_id}/complete`，只使用同表 schema、当前行已知列和最多 8 条至少含一个目标列值的相似参考行作为不可信证据；候选扫描、参与评分的列/文本和最终 prompt 均有硬上限（附近候选 512 行、已知列 32 个、评分文本每格 1000 字符、prompt 96000 字符）。缺少依据时必须 abstain，数字、型号、命令和路径等不能凭空补写。
- **模型与故障语义**：新增稳定交互 workload `knowhow_complete`，通过统一模型服务注册表和调度器调用；未配置时返回可操作的 400，解析器、上游调用或返回结构失败时记录脱敏模型错误并返回安全 502，不提供伪装成模型输出的离线填充。
...
- **验证**：补全 API、提示预算、权限/模型故障、并发保护、历史来源、前端纯逻辑和组件交互均有回归测试；完整 `scripts/check.sh` 与前端 production build 已通过。
