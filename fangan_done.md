...
- **整图规模守卫按本次 run 的参与集判（§11 检索和推理策略）**：`_federated_graph_is_large`（`backend/app/services/retrieval_candidates.py`）过去读挂载表的裸 id 清单、从不过库维度勾选，于是一个刚被用户取消勾选的大参考库仍然把 run 撑成「大图」，`_chunk_kg_overlay` 与 PPR 回退照样以 `reason=large_notebook` 拒绝、事件也误描述了这次 run 的语料。现在它读参与集座位（`_RetrievalState._retrieval_participants`），所以答案覆盖的正是这次 run 真要搜的库：库勾选与参与集覆盖两个维度都算数。方向只会**放松**守卫（座位的输出恒是它原先所读挂载清单的子集），被放行的通道读什么一字未改、仍在各自边界被 `scoped_subgraph_nodes` / `filter_retrieval_items` 过滤，所以放松守卫不会让被排除库的内容变得可引用，只是不再让那个库把「本 run 真在读的库」的通道一起关掉。那道守卫是逐库一次的 `copyable` 查表而不是建图，读 scope 不花成本、也不发布任何 scope 盲的缓存条目——这与图漫游/PPR 仍然接受的那条二阶代价不是一回事。
