...
- **Schema 归纳（建议态，§开放发现）**：`POST /notebooks/{id}/schema-proposals` 用 LLM 从笔记本内容提议新类型（offline 为 no-op），按 notebook 和创建者存为 `status='proposed' source='induced'`，绝不自动启用、也不占用其它 notebook 的同名类型；前端在当前笔记本视图审核（批准→active / 拒绝→删除）。
