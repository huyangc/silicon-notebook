...
- **错误人话层（`frontend/app/errors.ts`）**：后端 `detail` 不再透传给用户（此前直出，用户会看到英文原文和裸状态码）。模块分两条通道：**可展示通道**只认 JSON 响应体里的 `detail`/`message` 字符串字段（`pickUserDetail`），非 JSON 正文（网关 HTML、代理错误页、堆栈）一律没资格上屏——含中文也不行；**诊断通道**拿原始正文，压单行 + 统一截断（`pickDiagnostic`）后只进 console。所有 fetch 失败分支走 `throwHumanizedHttpError(res, tag)`：诊断（状态码 + statusText + 原始正文 + `X-Request-Id`）进 console，用户只拿按状态码映射的中文；后端刻意写成中文的 4xx `detail` 原样透传（比泛化文案具体），5xx 一律泛化。所有 catch 分支走 `toUserMessage(error, fallback)`——它才是覆盖面最大的一条：`fetch` 自身 reject（断网/后端没起来）根本没有 `Response` 可读，进不了 `throwHumanizedHttpError`，以前 `TypeError: Failed to fetch` 会直接印在界面上；流式 `error` 事件、`ask_job.error`、来源 `error_message` 这些「以 JSON 字段形态送达的后端英文串」同理。被换掉的原文一律进 console。`errors-guard.test.mjs` 扫全量前端源码防复发：禁裸抛状态码，且 `errors.ts` 之外**任何** `.message` 读取都必须逐行登记。
