...
- 已完成（2026-08-24，§6.4 / §9.1）：KG 起始探活复用统一短输出预算，不再以探测专用小上限截断推理型模型的可见 JSON；HTTP 成功但空白、截断或无效 JSON 的响应落成 `model_response_invalid`，界面明确显示“模型响应不可用”并保留继续分析入口，不再泛化为人工“分析已中断”。
