...
- **深度报告可信度修复批（同一功能，2026-08-02）**：用户确认框架在生成阶段保持权威，终审上下文按节预留正文/账本预算并只裁剪完整 JSON 记录；高风险扫描修正英文子串、中文单字、无单位编号与英文切句误报，审计始终披露而自动降级默认关闭。资料基础改为 SQL 聚合、有限分布桶与代表页，不再把全量来源族映射复制进意图/大纲/prompt；仅对实际触达来源有界解析身份，覆盖与充分性按去重后的相关证据—资料组统一计量。低档恢复逐节检索→撰写流水线，高档综合区分可用、无证据跳过、模型失败与校验失败并上屏；来源层级徽章复用 Ask 统计，框架维度可直接删除。G1、PostgreSQL lane（238 passed / 2 skipped）及五组删除/移动变异均通过，不新增 migration。
