...
- **所选来源子图检索与质量门控激活（§6.5 / §11，2026-08-04）**：真正收窄到一篇/几篇来源时，Ask `chunk`、`reasoning`、实验 `graph` 与深度报告共用 `SelectedSourceGraphActivationService`，不再显示泛化的“限定来源下已关闭无法安全隔离的图扩展通道”。历史检索 `B` 必须先完整产出；来源子图 snapshot、邻居/membership、scoped PPR 及超大来源的按来源 partition 只生成独立预算的 `G`，active 也只能按 `B + G` 追加，不能驱逐、重排或重打分 `B`。全范围/全选（含单来源 notebook 全选唯一来源）在 snapshot I/O 前保持历史路径和旧响应形状。默认 `off`；`shadow` 只评估不改变答案；`allowlist`、稳定 hash rollout 与 `on` 必须通过 canonical 五场景配对评测、受信无正文 attestation 及精确 corpus/model pin。scope 漂移、越界候选、工件不可用、图失败或 baseline eviction 都整段丢弃 `G`；mounted base 保持独立历史 lane。Ask/报告前端显示同一份 active/shadow/off/degraded 状态，reasoning trace 使用明确的 `source_subgraph` 步骤。已通过完整 `scripts/check.sh`：contracts、6513 项后端泳道测试（1 skip）、1814 项前端 Node 测试、38 个组件文件/227 项测试及 Next.js production build。
