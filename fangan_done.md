...
- **联邦 chunk 召回（§11 检索和推理策略 / §19.2 两个检索平面与权威，`search_chunks` 联邦化）**：`backend/app/services/chunk_federation.py` 把「逐库」与「逐子查询」两层扇出压成一张扁平任务表，由 `CHUNK_FANOUT_MAX_WORKERS`（默认 8，等于联邦化之前那个字面量）这一个上限约束，所以数据库连接峰值不随挂载库数增长。参与集只经 `_RetrievalState._retrieval_participants` 这一个座位读取（确定序：active 在前，其余按挂载顺序），超过 `CHUNK_FEDERATION_MAX_PARTICIPANTS`（默认 8）按该序截断并发内容无关的 `chunk_federation_truncated` 事件。参与集 ≤1 时整条联邦路径短路回联邦化之前的单库腿，返回值逐值不变；`CHUNK_FEDERATION_ENABLED=0` 是唯一回退开关，把参与集收成当前库一本。跨库合并复用 `global_evidence.peer_evidence`（每库保底一条 + 库内名次），按参与集顺序聚池而不是线程完成序，所以同题同库两次结果一致；各库矩阵按 chunk_id 去重后 `vstack`，维度不符的那一库整份丢弃（fail-soft），MMR 因此不会把外库候选系统性判成「完全不相似」。非当前笔记本的参与库把「该库当前可见来源」下推到生产者的 `LIMIT` 之前（`producer_explicit=False`，run 级 memo），隐藏的 Memory/Knowhow 投影结构性不进这条跨库通道；大库参与腿只借用已常驻的暖索引（`peek_warm_chunk_index`），暖索引缺席时直落 FTS 降级，绝不冷加载 scale 索引、绝不写共享向量矩阵缓存。被取消勾选的参考库在参与集里就已跳过，零候选零 SQL。无图早退的放行判据（`AskService._no_kg_scope_admits_run`）随之改成参与集口径（`collection_map.sources`），回退开关下退回 `collection_map.active_sources`。跨库段落进入回答后 `Citation.notebook_id` 为来源库、tier 为 `base`。
