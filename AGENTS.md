...
Errors shown to a user are always Chinese, and the translation happens only in the frontend — `frontend/app/errors.ts` is the single source. Backend `detail` strings stay as they are and are never rewritten to please the UI: they are the contract for MCP agents, logs, and triage. Every `fetch` call site that can fail routes its error branch through `throwHumanizedHttpError(res, tag)`, which reads the body once, pulls `X-Request-Id`, writes the raw diagnostic (status + statusText + backend `detail` + request id) to `console.error`, and throws the user-facing Chinese message. `humanizeHttpError(status, detail)` maps by status code, with one deliberate exception: a **4xx** `detail` that already contains Chinese is backend-authored user copy (`用户名已被占用`, `格子定位不合法`, `仅管理员可设置基准库`) and is passed through verbatim, because it is strictly more actionable than a generic status-code message. English `detail` never reaches a user, and **5xx** is always generalized regardless of language so internal failures cannot leak. Call sites that catch rather than fetch (toasts, error bars) use `toUserMessage(error, fallback)`, which shows an already-translated Chinese message — preserving the 401/403/404/409 distinction instead of flattening every failure into one sentence — and falls back for English technical exceptions such as `Failed to fetch`. Do not hand-roll an error branch: `frontend/app/errors-guard.test.mjs` recursively scans `frontend/app` and fails on any `new Error(...)` whose argument interpolates `.status`, and separately pins that each self-contained API client imports the error layer. Its allowlist is line-exact and covers only throws that are swallowed by their own `catch` as internal control flow (SSE reconnect backoff, authenticated image loading) and therefore never reach a user.
