...
- The notebook collection reads each user's persisted, last-known model-service status only. Status reads must never probe an upstream provider or delay/hide the collection. On settings save, compare all status-service effective fingerprints before and after the write and invalidate only identities that changed; no-op saves preserve status, and an inherited LLM variant changes only when its resolved fallback changes. Invalidated configured services read as untested until an explicit test or observed failure records a result.
