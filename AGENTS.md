...
- Main column: source-grounded knowhow tools, exposed as four tabs: **问答** (Ask), **知识库** (Knowledge), **记忆** (Memory), and **深度报告** (Deep Report).
  ...
  - Every Ask entry surface uses the durable job lifecycle and atomic final save. Blocking `POST /notebooks/{id}/ask` and MCP `ask_notebook` keep the job id internal—their public response has no `job_id` and they do not claim stream reconnect/progress behavior. Only `POST /notebooks/{id}/ask/stream` exposes the started `job_id`, progress events, detached recovery, and explicit-cancel protocol.
  - Conversation deletion is a lifecycle operation. Explicit delete must return 409 and retain all state while any Ask job is running; bulk cleanup skips running or newly refreshed conversations and returns both `deleted` and authoritative `deleted_ids`. For an actually deleted conversation, answers, terminal `ask_jobs`, and `ask_trace_steps` are purged in the same transaction. The frontend may clear the current session only when its id appears in `deleted_ids`, never from a stale client-side age snapshot.
...
- Repository access goes through the composed repository boundary. `RepositoryFacade` is backend-neutral over an injected `RepositoryRuntime` bundle; `SQLiteRepository` is the narrow compatibility wrapper that owns SQLite migration and maintenance adapters. Application services do not assemble product SQL. Stores own product SQL and raw row selection; established application/query components may assemble domain/application projections such as `NotebookSummaryQuery.from_row`. Facade methods must remain explicit compatibility adapters or source-checked one-hop delegates whose actual targets match the ownership manifest. New consumers depend on the executable, consumer-specific Protocols in `app/repositories/ports.py`. Dependencies point factory/wrapper → facade → runtime → services → stores; extracted services must never import the facade or a concrete backend back. `app/services/sqlite_identity.py` and `app/services/sqlite_notebook_sharing.py` are compatibility re-export shims (no mixin inheritance); keep them and the legacy request-context, `_COPY_CHUNK`, and `_remap_json_ids` exports from `sqlite_repository.py` compatible.
...
- Schema changes stay version-gated behind `SqliteMigrator` (append `_migration_N` + bump `SCHEMA_VERSION`); startup recovery/seed/admin-upgrade run every boot outside the version gate. Pre-refactor databases must keep loading: the frozen v9 fixture replay (`backend/tests/fixtures/repository_v9/`, `test_legacy_db_compat.py`) and the backup-only real-database verifier `scripts/verify_repository_snapshot.py` are the guards. The verifier uses exact per-version migration and stable-seed manifests, percent-encodes SQLite URI paths, never constructs the repository on an original database/storage path, and reports a retained temporary backup on cleanup failure without private row data. Original DB/WAL metadata and SHM existence/size are guarded; on a live WAL attachment only SHM mtime is exempt.
  The v24 cluster cleanup is the sole migration data-change allowance: compute its exact BINARY-ordered survivor projection as a bounded streamed count/digest plus removed count, then require the post-migration table to match it exactly. Never add `concept_clusters` to the globally materialized startup-normalization tables; a wrong survivor, field mutation, or unrelated deletion must fail closed.
...
- `batch_ingest` mutation phases are SQLite-only; they must reject a PostgreSQL `DATABASE_URL` through the neutral URL identity helper before constructing `SQLiteRepository`, without exposing credentials. PostgreSQL operators use normal app/API upload and KG/reindex flows. `--dry-run` remains filesystem-only and backend-independent. Within the supported SQLite path, `batch_ingest` uses three independent controls: `--workers` for source jobs, `--llm-conc` for a process-wide traditional-LLM hard cap, and `--embed-conc` for a process-wide embedding hard cap. CLI values override `KG_JOB_CONCURRENCY`, `KG_EXTRACT_WORKERS`, and `EMBED_CONCURRENCY` respectively; omitted CLI values inherit them. `_batch_concurrency_scope` is the sole scheduler owner: it configures the source-job and LLM-window limits and activates the one process-wide `ModelConcurrencyState`, which owns the shared LLM gate and shared embedding executor. Phase helpers only submit work; parallel or nested model-concurrency owners are prohibited because they would create multiple nominal global caps. Never reintroduce `workers × embed-conc` per-source pool multiplication. Gate waits must not hold SQLite write transactions.
...
- Both active backends auto-run their own migrations at startup. Production and packaged launch stay `--workers 1`. PostgreSQL deployments retain finite pool acquisition, statement, and lock timeouts. PostgreSQL requires `pg_trgm` in the `public` schema; the application database owner must be able to run `CREATE EXTENSION pg_trgm` or the DBA must preinstall it there before first startup. Readiness and status errors expose only redacted database identity.
- Early setup and prerequisite documentation must scope “no database server” to the shipped-default SQLite quick start. Selecting PostgreSQL requires an accessible server, installed dependencies, and `DATABASE_URL` configuration; startup migrates the selected datastore rather than unconditionally creating a SQLite schema.
...
- `scripts/check.sh` stays offline and must never start/contact PostgreSQL. `scripts/check_postgres.sh` requires a dedicated `TEST_POSTGRES_URL`; every target uses a strict single-`sslmode` query allowlist, rejects identity/credential/service/file/`options`/unknown/repeated overrides before connecting, and rechecks effective host/port/user/database. The parent only parses; a password-free preflight helper and pytest run consecutively with the exact same minimal allowlisted environment, sanitized URLs, and pgpass file, so parent libpq variables cannot redirect one phase. Exact per-target credentials precede copied inherited pgpass lines in a mode-0600 temporary `PGPASSFILE`. A launcher resource owner exists before scoped SIGINT/SIGTERM handlers. The handler records only the first pending signal and non-blockingly requests termination only for an already registered child; pgpass/Popen factories register returned resources before a checkpoint honors pending state. Cleanup boundedly terminates/reaps the exact child/session, removes pgpass, restores prior handlers, and returns 130/143; a child signal exit is normalized to `128+signum`. The lane runs only `-m postgres_integration`. CI owns a separate PostgreSQL 16 job using a least-privilege app role, a UTF8/non-C-default database for schema parity, and a non-UTF database that must reject migration 0001 transactionally before ledger/business DDL. Local PostgreSQL 16 is the matching ordinary lane; the PostgreSQL 16 auxiliary checks are authoritative in CI.
