...
- PostgreSQL + pgvector remain the future production/team-beta direction. SQLite remains the shipped default and formal runtime for now. Settings accepts `postgresql://` and legacy `postgres://` URLs (normalizing the latter), and parses `SHADOW_DATABASE_URL` without allowing it to select the active backend. URL diagnostics must redact credentials and options. Configuring PostgreSQL does not perform a migration or switch the repository; it only validates configuration for later adapter/shadow work. Unsupported schemes still fail closed and must never silently fall back to `.local`.
