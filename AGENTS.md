...
- Main column: source-grounded knowhow tools, exposed as three tabs: **Ask**, **Knowledge**, and **Deep Report**.
...
- Do not show a fixed right-column Studio sidebar in the primary notebook workspace.
  - Keep the source column + three-tab main column as the two-column workspace.
...
- Current user-facing knowledge capabilities are Ask (`chunk` / `graph` / `reasoning`), the Knowledge browser, and Deep Reports. The current report contract is the `reports` table and `/reports` APIs; retired Scenario/Case/Checklist and content-studio endpoints are not current capabilities.
...
- Repository access goes through the composed repository boundary. `SQLiteRepository` is the compatibility facade over `RepositoryRuntime`: SQL for the main business database exists only in the domain stores under `app/repositories/sqlite/` (plus the explicitly SQLite-only maintenance adapter), application services own orchestration, and new consumers should depend on the consumer-specific Protocols in `app/repositories/ports.py`. Dependencies point facade → runtime → services → stores → `SqliteDatabase`; extracted services must never import the facade back, and a future PostgreSQL repository swaps the store layer behind the same ports. `app/services/sqlite_identity.py` and `app/services/sqlite_notebook_sharing.py` are compatibility re-export shims (no mixin inheritance); keep them and the legacy request-context, `_COPY_CHUNK`, and `_remap_json_ids` exports from `sqlite_repository.py` compatible.
- Schema changes stay version-gated behind `SqliteMigrator` (append `_migration_N` + bump `SCHEMA_VERSION`); startup recovery/seed/admin-upgrade run every boot outside the version gate. Pre-refactor databases must keep loading: the frozen v9 fixture replay (`backend/tests/fixtures/repository_v9/`, `test_legacy_db_compat.py`) and the backup-only real-database verifier `scripts/verify_repository_snapshot.py` are the guards — the verifier never constructs the repository on an original database/storage path and tolerates no row changes beyond the documented startup normalizations.
