...
- `batch_ingest --workers` controls source/document jobs only and falls back to `KG_JOB_CONCURRENCY`. Every chat, embedding, and rerank call from every batch phase must use the same `RuntimeModelProvider` and per-service `ServiceScheduler` as online requests. The bound service's TOML `max_concurrency` is the sole model-capacity setting; never add batch/environment model-capacity overrides or multiply capacity by source workers. Scheduler waits must not hold SQLite write transactions.
