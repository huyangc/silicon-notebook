...
- Only confirmed Memory can enter creator-proposed Memory→KG promotion. The admin queue reviews sanitized extraction candidates and server-validated evidence, not raw Memory revisions/provenance. Approval revalidates current confirmed status and creator access, reuses existing dedupe/merge, and creates or merges one or more Base KG objects; the API/audit result must retain the complete `base_object_ids`. It does not change the private Memory's owner/tier or expose its private task context. Notebook deletion cascades all members' bound private Memory, and the UI warning must state that fact without revealing member identities, content, or counts.
