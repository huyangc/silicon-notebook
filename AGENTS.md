...
- `SHADOW_DATABASE_URL` is reserved and inert. Changing `DATABASE_URL` does not copy, migrate, or synchronize existing data. `scripts/migrate_sqlite_to_postgres.py` is the single owned SQLite→PostgreSQL snapshot-import boundary: dry-run by default; target URL from a named environment variable; read-only SQLite backup-API snapshot; schema upgrade only on a private working copy; exact manifest/empty-target guards; FK-ordered bounded COPY; reviewed JSON/timestamp/legacy-vector/NUL transforms; historical rowid→ordinal preservation and reseed; content checksums for every table; transactional index rebuild/ANALYZE/data commit; credential-free receipt. It must never import MySQL, copy storage files, change `DATABASE_URL`, silently discard non-empty retired tables, expose credentials/row values, or permit a non-empty PostgreSQL target. Importer-owned sealed snapshots may be reused only explicitly after validating their directory, name/hash, quick_check, version, and absence of WAL/SHM sidecars.
