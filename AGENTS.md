...
- `scripts/check.sh` stays offline and must never start/contact PostgreSQL. `scripts/check_postgres.sh` requires a dedicated `TEST_POSTGRES_URL`, validates every configured target and rejects caller-owned libpq `options`, redacts its preflight, gives pytest only password-free URLs with per-target credentials in a mode-0600 temporary `PGPASSFILE`, and runs only `-m postgres_integration`. CI owns a separate PostgreSQL 17 job using a least-privilege app role, a UTF8/non-C-default database for schema parity, and a non-UTF database that must reject migration 0001 transactionally before ledger/business DDL. Local PostgreSQL 16 is valid for the ordinary lane; the PG17 auxiliary checks are authoritative in CI.
