...
- Interactive row/table reformat batches freeze one complete-table snapshot for their lifetime. Merged shared-cell saves atomically revalidate target content, the current anchor designation, and exact anchor-group membership; any drift rejects the whole unit and defers the parent-table reload until the batch modal closes.
