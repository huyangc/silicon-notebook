...
run control state enables it. The paired PostgreSQL business schema remains v9.
...
- `SHADOW_DATABASE_URL` remains inert by itself. The temporary `migration/shadow` boundary currently provides a strictly read-only, UTF8-first preflight, redacted identity-bound confirmation, checksummed/removable PostgreSQL control schema, revision-CAS run state, and independently committed SQLite/PostgreSQL guards for the exact SQLite v31/PostgreSQL v9/epoch-1 pair (60 replicated tables, four `shadow_unique` guards). It also provides run-bound atomic SQLite backup snapshots and bounded, resumable PostgreSQL baseline COPY: per-batch rows plus prefix checkpoint commit together, resume revalidates the exact target prefix without truncating/deleting business tables, legacy JSON vectors alone are encoded to float32 bytea while BLOBs remain opaque bytes, seven historical rowids copy as explicit ordinals and their catalog-resolved identity sequences reseed before completion, and the final forward checkpoint advances atomically to snapshot H0 only after the v9 ledger, FKs, guards, and ANALYZE pass. Every safety-critical PG control mutation takes the migration lock then the control lock and validates the exact live control catalog. A transition needing live SQLite evidence acquires the PG pool/locks/catalog/run row first and only then holds a short SQLite `BEGIN IMMEDIATE`; never wait for a PG pool or advisory lock while holding SQLite. Capture cannot be acknowledged or enter `sqlite_to_postgres` until both reports commit. The incremental replicator, operator CLI, and an end-to-end worker are not implemented yet, so these primitives are not an operational migration path. Changing `DATABASE_URL` does not copy, migrate, or synchronize existing data. A switch is an operational stop/change/start boundary with quiesced writes, verified backups, one URL change, readiness/auth/count/read verification, and a rehearsed rollback condition. PostgreSQL-only writes are never replayed into SQLite.
- Baseline snapshot/COPY requires an owner-only non-symlink snapshot directory, fully qualifies every business-table statement to the run-bound schema, revalidates enabled live SQLite capture under a short `BEGIN IMMEDIATE` at initial binding, each committed batch/completion, and final H0 publication, and never holds that SQLite fence across PG prefix proof or `ANALYZE`. Snapshot and live-fence access must open a dedicated fresh connection to the file currently named by `SqliteDatabase.db_path`, never the repository thread-local connection; bind and recheck the resolved path plus `(st_dev, st_ino)` across open/transaction and immediately before snapshot publication or PG commit. This is a cooperative-operations boundary: replacing the file outside those checks is unsupported and the next check fails closed. JSONB prefix proofs canonicalize JSON numeric leaves only (bool excluded; int/float/Decimal share exact finite decimal semantics and negative zero becomes zero) while non-JSON SQL numeric columns retain their distinct type semantics. Resume uses bounded named server cursors; PG statements have a bounded timeout and cancellation polls surround proofs/migration/analysis. Full initial/final validation is derived from the checksummed packaged migrations and covers the exact v9 tables, columns, PK/FK/unique/check groups, operational and GIN indexes, and `public.pg_trgm`; lightweight per-batch validation must not rescan the 60-table catalog.
- The final SQLite fence is a lease across commit, not a point-in-time check: acquire it only after PG migration/control/run/table locks and the long 60-table proof/`ANALYZE` phase, retain it while writing and committing the PG H0 checkpoint plus run progress, then release it. A PG transaction/commit failure rolls PG back and releases SQLite with no H0 publication; never acquire a pool/advisory lock or run long PG work while holding this fence.
