...
- Only confirmed Memory can enter creator-proposed Memory→KG promotion. Admin approval reuses the existing dedupe/merge and creates or merges a base KG object; it does not change the private Memory's owner/tier or expose its private task context. Notebook deletion cascades all members' bound private Memory, and the UI warning must state that fact without revealing member identities, content, or counts.
