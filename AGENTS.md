...
- Manual notebook KG build/rebuild is a durable, notebook-scoped task in `kg_build_jobs`, with at most one `running` row per notebook and observable `probing→extracting→stopping→finished` state. KG LLM calls must use the task-scoped client, `KG_LLM_TIMEOUT_SECONDS`, and bounded `KG_LLM_MAX_RETRIES` (`0..3`); persistent unavailability/auth/rejection aborts only that notebook's current task, stops new source/window requests, and cancels/drains outstanding futures. Preserve committed source results, never persist an interrupted source's partial extraction, and let the next incremental build select only unfinished sources. Rebuild must probe before deleting existing KG. The frontend must resume the durable state after refresh, show the safe failure message/progress, and expose `继续分析未完成内容`; explicit rebuild remains the only destructive retry.
