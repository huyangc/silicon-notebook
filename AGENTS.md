...
Errors shown to a user are always Chinese, and the translation happens only in the frontend — `frontend/app/errors.ts` is the single source. Backend `detail` strings stay as they are and are never rewritten to please the UI: they are the contract for MCP agents, logs, and triage. The module keeps two strictly separate channels. The **displayable** channel has exactly one source: a `detail` / `message` **string field inside a JSON body** (plus FastAPI's `[{msg}]` 422 array), extracted by `pickUserDetail()`. A non-JSON body — a gateway HTML page, a proxy error page, a stack trace — is never displayable no matter what it contains: `<html>访问被拒绝 — nginx request id=req-1</html>` is a leak, not a message. `isDisplayableUserText()` additionally rejects anything multi-line, tag-bearing, brace-bearing, or longer than 200 characters, so even a hand-rolled call site cannot push raw body text at a user. The **diagnostic** channel gets the raw body verbatim, collapsed to one line and truncated once in `pickDiagnostic()`, and goes only to `console.error`.

Every `fetch` call site that can fail routes its error branch through `throwHumanizedHttpError(res, tag)`, which reads the body once, pulls `X-Request-Id`, writes the diagnostic (status + statusText + raw body + request id) to `console.error`, and throws the user-facing Chinese message. `humanizeHttpError(status, userDetail)` maps by status code, with one deliberate exception: a **4xx** `userDetail` that already contains Chinese is backend-authored user copy (`用户名已被占用`, `格子定位不合法`, `仅管理员可设置基准库`) and is passed through verbatim, because it is strictly more actionable than a generic status-code message. English `detail` never reaches a user, and **5xx** is always generalized regardless of language so internal failures cannot leak.

Everything that is caught rather than fetched goes through `toUserMessage(error, fallback)`. That is the single exit for every `catch` branch, not a convenience for a few toasts, because the largest failure class never reaches `throwHumanizedHttpError` at all: when `fetch` itself rejects (offline, DNS, backend down) there is no `Response` to read, and the raw `TypeError: Failed to fetch` would otherwise be printed straight into the UI. The same holds for backend error strings that arrive as JSON fields rather than status codes — the ask stream's `error` event, a persisted `ask_job.error`, a source's `error_message` (the backend writes `f"{type(exc).__name__}: {exc}"` there, which is for logs and MCP, not for a person). `toUserMessage` keeps an already-translated Chinese message — preserving the 401/403/404/409 distinction instead of flattening every failure into one sentence — falls back to the caller's copy for everything else, and always writes the discarded original to `console.error` so triage loses nothing.

Do not hand-roll an error branch. `frontend/app/errors-guard.test.mjs` recursively scans `frontend/app` and enforces two shapes: no `new Error(...)` whose argument interpolates `.status`, and — because that first rule cannot see `setStatusText(\`服务异常：${err.message}\`)` — **no reads of `.message` at all** outside `errors.ts` unless the exact line is registered in `ALLOWED_MESSAGE_READS`. Registered entries are only cases where the value is not raw exception text: a backend `model_errors[].message` field, a sentinel comparison, a state field already written by `toUserMessage`. The guard also pins positively that `reportError`, the ask stream's `error` event, `job.error`, and `extractErrorMessage` (a thin alias of `toUserMessage` shared by ~20 knowhow call sites) still route through the layer. Both allowlists are line-exact, so a new leak cannot inherit an old exemption.
