...
- **Knowhow row completion is suggestion-only.** The explicit physical-row action is available from a record row's detail drawer and from each branch in an anchor-group concept matrix. Only a missing cell or exact empty string is eligible; whitespace-only stored content is not silently treated as empty. Completion may inspect no more than eight same-table reference rows (same anchor/row-title group first, then known-column similarity and coverage) and returns structured suggestion/abstain, confidence, basis, and references through `POST /api/notebooks/{id}/knowhow/{table_id}/rows/{row_id}/complete`; it must not write cells, use KG retrieval, or fabricate an offline result when `knowhow_complete` is unavailable. The review dialog is draggable and requires per-item human acceptance. Acceptance must use the ordinary cell PATCH with `expected_before=""` and `origin="llm_complete"`, so concurrent fills return a conflict rather than being overwritten and normal history/projection semantics remain intact.
