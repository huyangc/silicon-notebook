...
- Center column: source-grounded knowhow tools, exposed as tabs.
  ...
  - Ask is tier-aware: it federates across the active personal notebook plus any `base` notebook, tags citations with their tier (rendered as a `base`/`personal` badge on each cited anchor in the Ask panel), and defers to base on contradiction. The `mode` field selects the retrieval/answer path: `chunk` (default), `reasoning`, and the opt-in/experimental `graph` (PPR with bounded rustworkx fallback and `chain_trust`). Retired `fast`/`global` ids map to `chunk` only for persisted-session compatibility. The Ask panel exposes all three current modes. Do not regress the default to graph mode or oversell it as on-by-default.
...
- Two-tier KB & tier-aware retrieval (Wave 1+2): `notebooks.tier` is `base` | `personal` (default `personal`), set via `SQLiteRepository.mark_notebook_base()` / `set_notebook_personal()` and the `POST /notebooks/{id}/tier` route (body `{tier}`; returns the updated `NotebookSummary`, 400 on bad tier, 404 on missing notebook). The notebook actions menu surfaces a "设为基准库 / 取消基准库" toggle that calls it. `federated_retrieve` (`sqlite_repository.py`) gathers candidates across `base ∪ active personal` and stamps each hit/anchor with its tier (`AnswerAnchor.tier`). Retrieval scores are tier-blind: sort by relevance first and use `base` as the secondary ordering key only on an exact score tie, so a higher-scoring personal hit still wins. This ranking policy is separate from `answer_prompt` (`prompts.py`), whose synthesis rule defers to base and notes the discrepancy when personal and base evidence contradict.
...
- User accounts: self-service registration (username rule: exactly one ASCII letter + literal `00` + 6 digits, e.g. `a00123456`; stored lower-cased) + password login (PBKDF2-SHA256). Auth uses opaque DB session tokens (`auth_sessions` table) passed as `Authorization: Bearer`; resolution is read-mostly and sliding-expiry writes are throttled by `AUTH_SESSION_TOUCH_INTERVAL_SECONDS` (default 300). Each notebook is owned by its creator (`notebooks.created_by`); users see only their own notebooks. The built-in `user-local` account is upgraded in-place to the `admin` account (id unchanged, login username `admin`, password from `SILICON_NOTEBOOK_ADMIN_PASSWORD`, local default `admin`, reset on every boot). Production mode and non-loopback `scripts/prod.sh` startup require a non-default admin password. Admin owns pre-existing notebooks and is the only user who can toggle the base-KG tier. Base notebooks are excluded from regular users' notebook lists but are still retrieved as authoritative context at ask time. `SILICON_NOTEBOOK_AUTH_OPTIONAL=false` by default (login required); set `true` only for local/testing. New routes: `POST /api/auth/register`, `POST /api/auth/login`, `POST /api/auth/logout`, `GET /api/auth/me`. Auth is enforced at the router level via a `get_current_user` dependency (`app/api/deps.py`); synchronous SQLite authorization work must stay off the FastAPI event loop. Frontend: login/register gate on first load, user menu with username display and logout; logout must abort local streams and remount/reset all user-owned state. There is intentionally no change-password / sharing / collaboration in this iteration.
...
- Repository access should go through a repository boundary. `SQLiteRepository` is the current public facade; cohesive SQLite domains should be extracted incrementally while preserving its interface for a future PostgreSQL repository. Identity, per-user model settings, admin usage, and auth-session persistence belong to `app/services/sqlite_identity.py`; share tokens, deep-copy lifecycle/cleanup, reader membership, and read-access ownership belong to `app/services/sqlite_notebook_sharing.py`. Keep the legacy request-context, `_COPY_CHUNK`, and `_remap_json_ids` exports from `sqlite_repository.py` compatible.
