...
full-table scan); v31 adds reports.understanding_json for the durable,
confirmation-gated Deep Report question-understanding contract.
...
- `SHADOW_DATABASE_URL` is reserved and inert. Changing `DATABASE_URL` by itself does not copy, migrate, or synchronize existing data. `scripts/migrate_sqlite_to_postgres.py` is the single owned SQLite→PostgreSQL snapshot-import and local-activation boundary: dry-run by default; target URL from a named environment variable; read-only SQLite backup-API snapshot; schema upgrade only on a private working copy; exact manifest/empty-target guards; FK-ordered bounded COPY; reviewed JSON/timestamp/legacy-vector/NUL transforms; historical rowid→ordinal preservation and reseed; content checksums for every table; per-table checkpoint commits (a run header keyed to the sealed snapshot hash plus one committed row per verified table) so a stopped import resumes from the last completed table instead of restarting, with idempotent finalize (ordinal reseed/index rebuild/ANALYZE); bounded session bulk-load tuning (`--maintenance-work-mem`, `--max-parallel-index-workers`, and `synchronous_commit`/`idle_in_transaction_session_timeout`/`statement_timeout` set for the offline load); credential-free receipt. Default preview/apply never changes `DATABASE_URL`. Explicit activation requires both `--activate-env` and `--confirm-service-stopped`; it must re-snapshot the stopped SQLite source (the cutover anchor) and exactly match the sealed snapshot and receipt, and by default recompute every PostgreSQL table checksum — `--fast-activation` skips only that redundant second target-checksum pass while keeping the source anchor and schema/manifest checks — then preserve file mode, write/fsync a restricted backup and same-directory temporary file, `os.replace` the env file, fsync its directory, set PostgreSQL as the sole `DATABASE_URL`, and retain the old SQLite URL only as inert `SHADOW_DATABASE_URL`. The CLI never stops/restarts services. It must never import MySQL, copy storage files, silently discard non-empty retired tables, expose credentials/row values, or permit a non-empty PostgreSQL import target. Importer-owned sealed snapshots may be reused only explicitly after validating their directory, name/hash, quick_check, version, and absence of WAL/SHM sidecars.
- An online import is rehearsal-only because later SQLite writes are not captured. A formal switch is an operational stop/change/start boundary with all writers quiesced, a final migration into a new empty target, shared/copied storage verification, one URL change, readiness/auth/count/search/read/canary-write verification, and a rehearsed rollback condition. PostgreSQL-only writes are never replayed into SQLite; before the first PG business write URL rollback is allowed, afterward external PG→SQLite reconciliation is mandatory.
- `batch_ingest` mutation phases are SQLite-only; they must reject a PostgreSQL active backend before constructing SQLite maintenance state. PostgreSQL uses normal application/API ingestion and KG/index flows; dry-run remains backend-independent.
...
- Within the supported SQLite path, `batch_ingest --workers` controls source/document jobs only and falls back to `KG_JOB_CONCURRENCY`. Every chat, embedding, and rerank call from every batch phase must use the same `RuntimeModelProvider` and per-service `ServiceScheduler` as online requests. The bound service's TOML `max_concurrency` is the sole model-capacity setting; never add batch/environment model-capacity overrides or multiply capacity by source workers. Scheduler waits must not hold SQLite write transactions.
