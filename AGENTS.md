...
- `SHADOW_DATABASE_URL` is reserved and inert. Changing `DATABASE_URL` does not copy, migrate, or synchronize existing data. A switch is an operational stop/change/start boundary with quiesced writes, verified backups, one URL change, readiness/auth/count/read verification, and a rehearsed rollback condition. PostgreSQL-only writes are never replayed into SQLite.
