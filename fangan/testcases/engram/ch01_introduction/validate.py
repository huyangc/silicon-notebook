#!/usr/bin/env python3
"""Validator for ch01_introduction gold.yaml (schema v0.3.3 invariants).

Implements the CI invariant checklist from article_research_gold_spec.md
(Part 3). Exits non-zero on any failure.
"""
import os
import re
import sys
...
def main():
    ...
    # 2/3. spans + 4. structural refs + 11 (atom no confidence)
    for a in atoms:
        ss = a["source_span"]
        raw = a["raw_text"]
    ...
    for c in chunks:
        ...
        for k in ("central_atom_ids", "gold_must_cover_atoms"):
            for aid in c.get(k, []):
                check(aid in c["atom_ids"],
                      "chunk %s.%s atom %s not in atom_ids" % (c["id"], k, aid))
    for a in atoms:
    ...
    for p in packages:
        ...
        for oid, fields in p.get("expected_local_fields", {}).items():
            ...
            if obj:
                for f in fields:
                    check(f in obj["payload"],
                          "expected_local_fields %s field %s not in payload" % (oid, f))
    # every core chunk has exactly one package
    ...
    # 13. raw/normalized cross-span audit: non-formula normalized must not introduce
    # multi-digit numbers absent from raw_text (single-digit transliterations like O(1)
    # are allowed). Citations stripped from normalized is fine (subtractive).
    for a in atoms:
        ...
        for num in re.findall(r"\d[\d.]*\d|\d", norm):
            check(num in raw, "%s normalized introduces number %r absent from raw" % (a["id"], num))
