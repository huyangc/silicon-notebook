#!/usr/bin/env python3
"""Validator for ch04_pretraining gold.yaml (schema v0.3.3 invariants).

Checks the article_research gold-fixture invariants from
fangan/article_research_gold_spec.md Part 3:
  1  yaml parses; schema_version == 0.3.3
  ...
  8  relations: endpoints in objects; evidence atoms exist; labels present; no confidence
  9  mentions: atom_id exists
 10  do_not_extract: ref/atom_id (if present) exists
 12  payload.children (if any) are objects
 13  raw/normalized table_row NUMERIC audit: every number in normalized_text is in raw_text
...
def main():
    ...
    for a in atoms:
        ...
        if vs:
            check(vs.get("viewer_only") is True,
                  "viewer_span.viewer_only must be true for %s" % a["id"])
        ...
        check(a["source_element_id"] in se_ids,
              "%s.source_element_id not in source_elements" % a["id"])
        check("confidence" not in a, "%s has confidence" % a["id"])
        # 13. table_row NUMERIC over-span audit
        if a["atom_type"] == "table_row_atom":
            for tok in NUM.findall(a["normalized_text"]):
                check(tok in raw, "%s normalized number %r not in raw substring" % (a["id"], tok))
