#!/usr/bin/env python3
"""Validator for ch03_scaling_laws gold.yaml (schema v0.3.3 invariants).
...
confidence; GLOBAL no-orphan; expected_local_fields subset of payload; plus a
NUMERIC over-span audit (every number in a non-formula normalized_text must
appear in its raw_text span).
"""
import os
import re
import sys
...
# Numeric tokens: integers/decimals, optionally with a percent. Examples:
#   1.7248  0.0139  10  20  25  75  80  100  568  993  106  99  46  43  5.7  9.9
# We compare raw-vs-normalized at the level of digit-runs to catch over-span
# numbers regardless of formatting (e.g. '2 x 10^20' digits 2,10,20 cover '2e20').
...
def main():
    ...
    for a in atoms:
        ...
        # 13 + numeric audit: non-formula normalized numbers must be in raw span.
        if a["atom_type"] != "formula_atom":
            raw_nums = numbers(raw)
