#!/usr/bin/env python3
"""Validator for ch04_analog_subcircuits gold.yaml (schema v0.3.3-textbook).

Checks the textbook gold-fixture invariants from
fangan/article_research_gold_spec.md Part 3 (profile-agnostic process):
  1  yaml parses; schema_version == 0.3.3-textbook
  2  authoritative source_span: source_file[span] == raw_text (every atom)
  3  optional viewer_span: source.md[span] == raw_text + viewer_only:true
  4  structural refs: atom.section_id in section_tree; source_element_id in source_elements
  5  chunk coverage: every atom in some chunk; central/gold_must_cover subset of atom_ids
  6  packages: package.atoms == chunk.atom_ids; expected_objects subset of objects;
     expected_local_fields fields subset of object payload
  7  object evidence: local in home-chunk; supporting NOT in home-chunk; object in
     home_package.expected_objects; no confidence; gold_label/difficulty/evidence_strength
  8  relations: endpoints in objects (esp. circuit_block_composed_of_block);
     evidence atoms exist; labels present; no confidence
  9  mentions: atom_id exists
 10  do_not_extract: ref/atom_id (if present) exists
 12  payload.children (if any) are objects
 13  NUMERIC over-span audit: every number in a formula/given/usage/result atom's
     normalized_text appears in its raw_text (allowing a small whitelist of glossed
     constants explicitly carried from neighbouring equations)
 14  formula/condition normalized has NO prose gloss past the formula body
...
def main():
    ...
    for r in g["relations"]:
        ...
        check(r["source_object_id"] in object_ids, "relation %s bad source %s" % (r["id"], r["source_object_id"]))
        check(r["target_object_id"] in object_ids, "relation %s bad target %s" % (r["id"], r["target_object_id"]))
        for aid in r["evidence_atom_ids"]:
            check(aid in atom_ids, "relation %s atom %s unknown" % (r["id"], aid))

    # explicit: all circuit_block_composed_of_block endpoints are objects
    for r in g["relations"]:
        if r["relation_type"] == "circuit_block_composed_of_block":
            check(r["source_object_id"] in object_ids and r["target_object_id"] in object_ids,
                  "composed_of relation %s endpoint not an object" % r["id"])
    ...
    # 13. NUMERIC over-span audit. FORMULA atoms are audited STRICTLY: every number
    # in the formula body of normalized_text must appear in the (de-spaced) raw_text;
    # numbers in a trailing parenthetical gloss that is explicitly carried from a
    # neighbouring equation ("Eq.N" reference) are exempt (raw_vs_normalized exception
    # for formula_atom via supported_by_context_atoms).
    # given/usage/result/problem atoms are prose SUMMARIES of a worked example: their
    # normalized text may reference other in-example values, so they are not subject
    # to the strict per-number audit (their faithfulness is checked by the verbatim
    # source_span match in step 2).
    def despace(s):
        # MinerU spaces digits: '1 9. 7' -> '19.7', '0. 0 5' -> '0.05'
        return re.sub(r"(?<=\d)\s+(?=[\d.])", "", re.sub(r"(?<=\d\.)\s+(?=\d)", "", s)).replace(". ", ".")

    for a in atoms:
        if a["atom_type"] != "formula_atom":
            continue
        raw_ds = despace(a["raw_text"])
        norm = a["normalized_text"]
        # the formula body is everything before the first " (" gloss
        body = norm.split(" (", 1)[0]
        for tok in NUM.findall(body):
            check(tok in raw_ds or tok in a["raw_text"],
                  "NUMERIC over-span: formula %s body number %r not in raw substring" % (a["id"], tok))

    # 14. formula/condition atoms: normalized must be formula-only (no long prose
    # gloss). We allow a short parenthetical label but flag obvious sentence gloss.
    for a in atoms:
        if a["atom_type"] == "formula_atom":
            norm = a["normalized_text"]
            # the formula body is everything before the first " (" gloss; ensure the
            # body contains a math operator and the atom is not a prose sentence.
            check("=" in norm or ">=" in norm or "<=" in norm or "<<" in norm or ">>" in norm,
                  "formula_atom %s normalized has no relation/operator (looks like prose)" % a["id"])
    ...
    context_only = {a["id"] for a in atoms
                    if a.get("metadata", {}) and a["metadata"].get("context_only")}
    for a in atoms:
