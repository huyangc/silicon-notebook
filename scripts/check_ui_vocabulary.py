...
CJK_TERMS = {
    ...
    # 内部实现词，界面词是「重新整理」。「画像」/「底座画像」/「覆盖层画像」**不**收进
    # 这里——图谱质量分析视图早已把「来源画像」用作合法界面词（source profile，见
    # kg-analysis-view.tsx），裸「画像」子串匹配会连它一起误杀；与「节点」/「边」同一
    # 处理：登记进 NOT_LINTABLE，靠人工把关而非词法规则。
    "巡固": re.compile(r"巡固"),
