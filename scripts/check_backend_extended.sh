...
# and repository-wide semantic source scans remain available, but they do not
# compete with every edit-time verification run. PostgreSQL has its own
# authoritative check_postgres.sh lane.
PYTHONPATH="$ROOT_DIR/backend" "$PYTHON_BIN" \
  -m pytest -p no:cacheprovider -n "$BACKEND_EXTENDED_PYTEST_WORKERS" \
  -m "slow or architecture_contract" \
