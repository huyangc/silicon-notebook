...
的产物修复或删除。批 1 每条都是普通 btree（其中一条 partial、一条表达式）索引，批 2 的 payload 一条是 GIN，单条
