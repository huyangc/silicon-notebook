...
- **要无偏样本就用 `--sources K`,别用 `--sources 0` + 上限**:截断留下的是索引前缀,
  不是随机样本。全量模式触顶时报告会明说「可能有偏」并指回 `--sources K`(来源随机抽,
  前缀落在随机来源上)。刻意不做蓄水池/`ORDER BY RANDOM` —— 那要把全表 payload 扫一遍,
  在千万级目标库上是几十分钟到几小时,会让「先抽样快速看一眼」这唯一的使用姿势不成立。
  取舍是**不消除偏差、但绝不隐瞒偏差**,有反向护栏钉住那句声明必须出现。
