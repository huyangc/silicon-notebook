...
| `export_reasoning_traces.py` / `analyze_reasoning_trace.py` / `reflect_shadow_rig.py` | 只读轨迹导出、离线统计、Legacy 检索及测试库 Ask/Report，见下 |

### 检索轨迹统计与 Legacy 影子运行

`export_reasoning_traces.py` 只读导出 Ask/Report 轨迹，
`analyze_reasoning_trace.py` 离线聚合不含内容的闭集投影，
`reflect_shadow_rig.py` 提供单一 Legacy 流程的测试入口。
V2、多臂 A/B、`prefix-probe`、`state-probe` 及对应选择参数已删除。

```bash
# 导出必须显式给出连接；不隐式读取 .env，也不修改数据库。
python scripts/export_reasoning_traces.py \
  --database-url "$DATABASE_URL" --reports --out .local/trace-export.jsonl

# 聚合不连接数据库或模型；每项指标分别报告可观察样本数。
python scripts/analyze_reasoning_trace.py \
  .local/trace-export.jsonl \
  --out-md .local/trace-analysis.md --out-json .local/trace-analysis.json

# 先检查计划；dry-run 不连接数据库、不启动后端、不调用模型。
python scripts/reflect_shadow_rig.py --dry-run seed
python scripts/reflect_shadow_rig.py --dry-run --limit 2 ask
python scripts/reflect_shadow_rig.py --dry-run --limit 2 search

# 对已有笔记本只读运行检索（不创建库、不建图、不合成最终答案）。
python scripts/reflect_shadow_rig.py \
  --database-url "$DATABASE_URL" \
  --source-notebook-a "$NOTEBOOK_A" --source-notebook-b "$NOTEBOOK_B" \
  --env-file "$MODEL_ENV_FILE" --out-dir .local/legacy-search \
  --limit 2 search
```

`seed` 在一次性测试库中准备语料和临时后端，支持从 `--source-db-url` 的
`--source-notebook` 只读取得 A 语料，以及从 `--corpus-dir` 取得 B 语料。
测试库连接用 `--database-url`，建库维护连接用 `--admin-url`；已有测试库可用
`--skip-create-db`。`--skip-kg` / `--skip-embed` 用于不调模型的解析冒烟。
`--env-file` 选择模型配置，worktree 通常需显式指向已有配置文件。

`ask` 对测试后端调用完整 Ask，包含检索、答案与引用绑定；`report` 对测试库运行
ReportEngine。`restart` 按 `rig-state.json` 中保存的端口、数据库及模型配置重启
该测试后端；`export` 合并本次测试的结果投影；`teardown` 仅处理本 rig 记录的
测试后端和测试库。写路径保留一次性测试库限制，不能用作生产库写入入口。

`search` 只读使用已有笔记本，在同一个生产检索器上准备意图并执行 plan/reflect。
`--no-intent` 改为由各 run 自行规划；`--only-question`、`--only-cell` 先筛选再应用
`--limit`。`--concurrency` 控制本机并发，SQLite 强制为 1，PostgreSQL 受连接池
容量限制；模型服务的并发容量仍由部署配置决定。取消会结束批次，失败保留失败状态。

普通投影只输出闭集维度、计数与耗时，缺失数据记为 unknown。显式保存的原始轨迹、
意图、答案、引用和 `rig-state.json` 可能含来源信息、模型文本或本机配置，留在
`.local/`，不提交仓库。`--keep-raw-trace` 仅用于需要人工核对的检索批次。
模型与事件日志保持本次目录隔离；日志关联工具保留，不再报告 V2 布局或自评指标。
检索投影写入 `search.jsonl`，报告逐步投影写入 `report-trace.jsonl`；
可选原始检索轨迹位于 `raw/<question_key>_<corpus_cell>_<effort>.json`。
`export` 同时关联本次目录内的模型与事件日志，写入 `calls.jsonl`，展示每次调用的
耗时、重试和已知用量；日志缺失时保留 unknown，没有日志时不生成虚假调用样本。
历史轨迹已有的明确终态优先于 Legacy 推断，不能把旧失败重新解释成模型正常作答。

评估速度时以完整 Ask 墙钟为主，同时核对反思轮数、模型失败与答案事实/限定词/引用。
`search` 只覆盖检索阶段，不能代替端到端答案测试。公共前缀长度是输入结构观测，
没有服务端计数时不能换算缓存命中率，也不能单凭它宣布提速。
