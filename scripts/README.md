...
本档还为表格保留查询相关的整行与列头，并允许已展示证据在没有自评绑定时追加
...
同一份新契约。策略变更后的复测使用新输出目录，保留原始批次以供核对。

库范围与输出完整性拆开后的意图提示使用新的缓存策略版本，旧目录会被明确拒绝。
性能复测必须同时核对回答质量：标题清单是否保留每条来源及重复项、机制是否准确、
数值是否与对应版本/配置一致，不能用少答内容换取加速。共享合成提示会同时影响
Legacy 和 V2；只有证据保留/恢复的策略分叉属于实验档独有，报告中须区分这些改动。
...
- **`intents.jsonl` 不进数据集、不进仓库**。意图契约每题只算一次并缓存在 `--out-dir`
  ...
  缓存行带 `intent_cache_policy`；缺少当前策略版本的旧文件会被明确拒绝，须使用新
  `--out-dir`，不能沿用旧的自动澄清答案或旧提示扩展出的必答范围。当前策略下的必填
  澄清在本批共享为阻断结果；A/B 每臂和 search 每个 run 都记录 `clarification_required`
  失败，未启动 Ask 的墙钟留 unknown，并正常写出批次收尾工件。
