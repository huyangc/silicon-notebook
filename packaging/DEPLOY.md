...
`.env.example` 是常用部署起步模板；注释项按需取消注释，未设置时沿用后端
`Settings` 默认值。模型服务连接与容量仍由 TOML 管理。高级调优、实验灰度、
插件预算和独立批处理参数不再全部列入主模板；完整说明位于源码仓库的
`docs/deployment-and-configuration.md` / `_zh.md` 和 `scripts/README.md`
（本离线包不包含这些源码文档及全部独立工具）。维护模板不会修改已有 `.env`；
升级时已有显式覆盖值会继续生效，确认不再需要后再移除，才能跟随新代码默认值。
