...
} from "../../app/ask-intent-trace";
import type { QueryIntentContract } from "../../app/ask-intent-model";
...
test("问题理解阶段在轨迹面板里渲染,并与后端步骤同列", async () => {
  ...
  const { container } = render(
    <ReasoningTracePanel
      steps={[
        ...
        { step_type: "synthesis", summary: "已生成答案，引用 2 处证据",
  ...
  expect(labels[3]).toContain("2 处引用");     // anchors=2,不是 citations=4
});
