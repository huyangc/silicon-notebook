 ...
 * 那半句就不出。
 *
 * `stale` 是两条线的**三值**合取:true = 至少一条明确落后;false = 两条都对齐;
 * **null = 在场却报不出合并世代**(依赖主题板块的两份数据沿用了库里现成的板块划分,
 * 而那批划分建在哪一次合并上没有地方记),或者这份数据压根缺席(靠 `present` 分辨)。
 * null 不等于 false —— 把「无从判断」显示成「与当前一致」正是这个视图存在理由的反面。
 */
