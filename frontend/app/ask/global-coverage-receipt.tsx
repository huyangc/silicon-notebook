...
export function GlobalCoverageReceipt({ job, notebookNames }: { job: GlobalJob; notebookNames: Map<string, string> }) {
  ...
  return <div className={`global-coverage-receipt${skipped.length ? " partial" : ""}`} aria-label="本轮检索回执">
    ...
    {/* 「降级」在两代作业里不是同一件事：旧流水线（job.response）指词法降级；共享引擎的
        作业指该库有部分检索没有完成。文案只说现象，不替后者猜原因。 */}
    {degraded.length > 0 && (job.response
      ? <p>{degraded.length} 个笔记本使用词法降级检索，跨语言召回可能不完整。</p>
      : <p>{degraded.length} 个笔记本有部分检索未完成，这些笔记本的资料可能覆盖不全。</p>)}
    {skipped.length > 0 && <>
