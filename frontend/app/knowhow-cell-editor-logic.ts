...
// 在服务端提交之后的那个窄窗口里，仍会留下一条没有任何东西引用的资产行；它现在由
// 服务端的孤儿资产回收（sweep_orphan_assets）兜底，但**不是立刻**回收：
//   · 触发点挂在该 notebook 的 knowhow 表投影完成之后（另起后台 job，不占投影的
//     单飞窗口），并按 notebook 前沿限流。若此后不再编辑该表，就要等它下次被投影。
//   · 宽限期算的是「**持续未被引用**了多久」，不是「上传至今多久」：计时从这条资产
//     第一次被扫描观察到无引用起跑、跨多次 sweep 累计，引用一旦回来标记立刻清掉。
//     这样才护得住服务端根本看不见的活引用——「图片已粘贴、格子还没保存」和「图片
//     正从一个格子搬到另一个格子」，它们此刻只存在于浏览器草稿里。
// 即：这条孤儿行终会被回收，只是会在服务端多留一阵。它不影响用户看到的内容，故
