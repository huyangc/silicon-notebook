...
Ask reasoning 的应用编排开始从 service 内部隐式共享状态迁到 `backend/app/application` 的不可变 stage 合同：prepared input、retrieval evidence、response draft、committed answer 是四个真实所有权交接点。Point-specific runtime 显式绑定冻结 source scope、同一个 request-local retrieval run、取消权威、trace sink 和连接探针；retrieval run 继续是 query-embedding single-flight 与 leaf-I/O semaphore 的唯一所有者。`ReasoningRetriever.run_stage` 只在边界验证这些权威，再调用逐字原算法；不在 stage 外层占 fan-out slot，也不移动 KG/chunk/element/PPR leaf。连接持有或 authority 漂移抛不可被普通检索降级吞掉的 `StageBoundaryError`。Report 仍保留原 `ReasoningRetriever.run` 与可变 `ReasoningResult`，等其独立 stage PR 再迁移；本阶段未开放 Ask auditor/observer 插件位。
