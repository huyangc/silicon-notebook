...
- `frontend/app/use-ask-session.ts` 是 Ask 状态的唯一 owner：草稿/对话、意图确认、持久 stream/reconnect、会话历史/tombstone 与会话 mutation 都按 actor + notebook + workspace owner 收口。导航只 detach durable job；显式 Stop 在 `started` 前保持 transport 读到 job id，再执行一次 cancel 后 abort。cancel-request 的客户端等待值由 `/api/system/config` 按数据库 busy timeout 加传输余量派生，不能短于合法数据库等待。`page.tsx` 仍拥有 notebook/source paired snapshot、Memory answer-link 批次和跨域展示，只显式触发一次历史恢复并消费 readonly view/具名 command。
