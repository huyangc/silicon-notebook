...
解析元素 enrichment 是独立的 `source.element_enricher` Contributor point，物理位置固定在 ParserChain materialization 与 core-owned `replace_elements` 事务之间，并继续处于同一 source generation lock 内。默认 topology 为空，`app.bootstrap` 会把它折叠成 `None`，因此旧路径不构造 element snapshot、不读 clock/availability/connection、不发 event，也不增加 DB/model/embed/parser I/O。Active host 每个 contributor/来源只调用一次，SDK 只暴露不可变的 type/location/text/既有 caption 与 opaque request-local ref；返回值只能是有界 metadata patch。Core 按 stable contribution ID 把补丁放在 `metadata.extensions` 命名空间，并保留 element 数量、顺序、identity、text/type/location 与 parser/table/image/asset/section provenance；caption 仅当已是 parser text 子串时接纳，故本 PR 不改变 chunking 或图片检索语义。Host 不在数据库 lease 内运行，plugin 没有 path/URL/title/repository/asset/model/persistence port；Memory/Knowhow direct writer 不经过该 point。普通失败或非法结果整批 fail-open，BaseException 仍由既有 ingestion finally 处理未提交资产。
