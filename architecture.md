...
解析元素 enrichment 是独立的 `source.element_enricher` Contributor point，物理位置固定在 ParserChain materialization 与 core-owned `replace_elements` 事务之间，并继续处于同一 source generation lock 内。默认 topology 为空，`app.bootstrap` 会把它折叠成 `None`，因此旧路径不构造 element snapshot、不读 clock/availability/connection、不发 event，也不增加 DB/model/embed/parser I/O。Active host 在启动冻结时强制 manifest 把元素正文访问列为 required capability，并在每次调用通过实时 decision 后，才向 contributor 暴露不可变的 type/location/text/既有 caption 与 opaque request-local ref；每个 contributor/来源只调用一次，返回值只能是有界 metadata patch。Core 按 stable contribution ID 把补丁放在 `metadata.extensions` 命名空间，字节预算覆盖该 namespace 与 plugin id/version，并保留 element 数量、顺序、identity、text/type/location 与 parser/table/image/asset/section provenance；caption 仅当已是 parser text 子串时接纳，故本 PR 不改变 chunking 或图片检索语义。Host 不在数据库 lease 内运行，plugin 没有 path/URL/title/repository/asset/model/persistence port；Memory/Knowhow direct writer 不经过该 point。每个 contribution 原子接纳，普通失败或非法结果只丢当前 contribution，不会清空其他已接纳结果或 parser baseline；BaseException 仍由既有 ingestion finally 处理未提交资产。

Knowledge candidate projection 是独立的 `knowledge.candidate_projector` Contributor point，唯一生产挂点位于旧 `extract_graph → build_records → relink → partial-retry` core 序列之后、`KnowledgeLifecycleService.store_kg` 的唯一来源代次事务之前。默认 topology 为空且 `app.bootstrap` 折叠为 `None`，所以旧路径在 schema read、element snapshot、clock/availability/connection/event/I/O 前保持逐字原路；Memory/Knowhow hidden source 也在这些动作前短路。Active host 启动时要求 manifest 把 `data:scoped_source_elements:read` 列入 required capabilities；实时通过后只投影不可变的来源内 element/active-schema view 与 opaque request-local ref。插件不接触 baseline KG dict/list，只返回有界的 contribution-atomic object/relation proposals。Application adapter 用 exact ref + 原文子串重建 evidence，复检 payload schema、central edge pair、owner/version/bytes 与剩余 point budget，生成 namespace-safe local id，只把合法候选追加到 baseline prefix；review status、source generation、source facts、global id remap、单一写事务、embedding/retrieval publication继续由 core 拥有。普通失败或非法贡献仅丢本贡献；core cancellation 传播，插件伪造取消在权威 token 为 false 时仍是 optional failure。Concrete host 只在 `app.extensions`，services 只依赖 `app.domain.knowledge_projection`，constructor-only seat 经唯一 `app.bootstrap` 透传且不增加 facade public surface、migration 或 repository port。

Ask reasoning 与 Deep Report 的应用编排都已迁到 `backend/app/application` 的不可变 stage envelope。Ask 的 prepared input、retrieval evidence、response draft、committed answer 是四个所有权交接点；Report 明确交接 confirmed planning、generated sections、core final audit artifact 与 committed report。application 的精确 import allowlist 禁止 implementation/SDK/registry 反向依赖。两条流水都显式绑定 source scope、point-specific retrieval run、取消权威、非空 actor 与注入连接探针；retrieval run 仍是 embedding single-flight 与 leaf-I/O semaphore 的唯一所有者，stage wrapper 不占外层 slot，也不移动任何 KG/chunk/element/PPR leaf。Report planning 与 generation 各创建新 run，保留可变 `ReasoningResult` 作为 generation 内的独占工作副本，不做 evidence/id-map JSON 或递归 deep copy。多节 all-retrieval barrier → 至多一次 synthesis → 并行 drafting、单节零 synthesis、final editor、claim ledger、citation remap、整篇图片 batch、zero-body failed 与 retry 顺序均不变；final-audit 边界额外禁止改写 section Markdown。连接持有或 authority 漂移抛显式 boundary error，不能伪装成 optional retrieval miss。

流式 Ask 的完成后扩展有两个独立 point-specific host：`answer.audit` 在默认空拓扑下严格零工作，若注册 auditor 也只投影已持久答案的无正文结构计数，审计结果不能替换正文、引用或终态；`ask.completed_observer` 把既有 agent-profile、retrieval-experience 与 search-profile 三段后处理迁成三个内建 contribution。唯一组合根把 host 作为 domain port 注入 runtime，workflow 不 import SDK/registry。执行顺序保持 answer save → job done/unregister → browser final → auditors → agent-profile → reasoning-only retrieval-experience → search-profile → sentinel；三个 observer 仍串行、各自 fail-open，身份能力分别只有 notebook+actor、零身份、actor。入口/每贡献边界都用无 I/O connection probe 防止带 lease 调用插件；无插件或无适用 contribution 不触碰 clock/event/context/I/O。同步 POST Ask 与 MCP Ask 不进入该 streaming completion 口径，facade 没有新增公开插件 seat。

Deep Report 的终态扩展同样拆为 `report.audit` 与 `report.completed_observer`。SQLite/PostgreSQL 以 `WHERE status='generating'` 的同构原子完成写发布正文与 `done`，取消也只允许从非终态 CAS 到 `cancelled`，因此任一终态胜出后都不可反转；只有完成 CAS 成功才构造 `CommittedReport`。manual generate 与 auto-generate 都由 coordinator 汇入同一个 post-terminal 路径，且先退出 generation gate 与 report retrieval/source/model contexts，再运行默认空 auditor 与内建 agent-profile observer，最后按 identity 注销取消事件，以保留既有 active-job 窗口。Auditor 仅见结构计数，observer 仅见 actor/notebook/report ref 和 opaque at-most-once core access；两者不能回写正文、引用、参考文献、claim ledger 或终态，也不新建 retrieval/model/I/O 工作。
...
- `backend/app/api/routes.py` composes the domain FastAPI routers；aggregate 只负责组合顺序，不承载产品 endpoint body，也不提供兼容导出。边界契约直接检查各 domain router 的 endpoint 所有权，并以语义 AST 固定 aggregate 的组合清单与 `include_router` 调用；不依赖框架是否把子路由平铺（新版 FastAPI 会保留 lazy included-router 节点）。`system_routes.py`、`notebook_routes.py`、`source_routes.py`、`knowhow_routes.py`、`knowledge_routes.py`、`ask_routes.py`、`report_routes.py`、`kg_routes.py` 与 `admin_routes.py` 各自拥有领域 endpoint；`memory_routes.py`、`auth_routes.py`、`content_overview_routes.py`、`debug_logs.py` 与 Agent Knowhow router 保持独立。`mcp_server.py` 提供默认二十二个 core 工具（七个 Memory/context、四个 knowhow、一个引用点查、五个来源管理、三个构建与两个库理解）的 scoped Streamable HTTP 面；`PUBLIC_TOOLS` 是默认 core 兼容导出，实际权威清单来自 server-local frozen catalog；`deps.py` 承载访问控制依赖。
...
默认 core 公开二十二个工具，`mcp_server.PUBLIC_TOOLS` 是由同一 core catalog 派生的兼容导出；实际部署的权威清单是 server-local frozen catalog：Memory/context 七工具
