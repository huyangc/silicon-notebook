...
Ask reasoning 的应用编排已从 service 内部隐式共享状态迁到 `backend/app/application` 的不可变 stage envelope：prepared input、retrieval evidence、response draft、committed answer 是四个真实所有权交接点；application 的模块级 allowlist 当前只开放 application、core.ask_retrieval_policy、domain.cancellation、models.ask，裸 `import app` 同样禁止，新增合同按需显式扩展。响应 envelope 独占传递 typed response 对象图，不做会打断 collection card/citation 共享身份的 JSON/deepcopy。Point-specific runtime 显式交叉绑定冻结 source scope、同一个 `ask_reasoning` request-local run、取消权威、非空持久化 actor、trace sink 和 AskService 注入的同一个连接探针；retrieval run 继续是 query-embedding single-flight 与 leaf-I/O semaphore 的唯一所有者。`ReasoningRetriever.run_stage` 只在边界验证这些权威，再调用逐字原算法；不在 stage 外层占 fan-out slot，也不移动 KG/chunk/element/PPR leaf。连接持有或 authority 漂移抛不可被普通检索降级吞掉的 `StageBoundaryError`。Report 仍保留原 `ReasoningRetriever.run` 与可变 `ReasoningResult`，等其独立 stage PR 再迁移。
