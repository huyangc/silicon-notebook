...
- `scripts/migrate_sqlite_to_postgres.py` 是唯一 SQLite→PostgreSQL 存量导入与本地激活边界：默认 dry-run，SQLite 只读 backup-API 快照与工作副本升级，目标空库/manifest 守卫，按 FK 排序的有界 COPY，JSON/时间/旧 JSON 向量/NUL 的显式兼容转换，rowid→ordinal 保留与 reseed，全表内容 checksum，索引重建/ANALYZE/数据单事务提交和无凭据 receipt。默认 preview/apply 不进入应用 runtime、不修改 `DATABASE_URL`；显式 `--activate-env ... --confirm-service-stopped` 会重新快照停写 SQLite、按 receipt 重算 PG 全表 checksum，再通过同目录临时文件/fsync/`os.replace` 原子切换 `.env` 并保存权限受限回退副本。它不复制 storage、不读取 MySQL，也不提供持续同步或反向回放。
