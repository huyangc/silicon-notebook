...
Parser ProviderChain 的启动拓扑已经组合为 self-hosted MinerU → MinerU cloud → builtin 三环；链序用 `after`/`before` DAG 表达并以稳定 ID 处理并列，不允许整数 priority。Runner 在 core route policy 后才做实时 availability，并把插件 probe 与 core admission/materialization 物理分层：插件上下文没有持久化或资产端口，拒收/禁止/失败都不能触发 core commit。PR-04 刻意不把该 host 注入 ingestion，`services/parsers.py` 与 `source_ingestion.py` 仍是唯一生产 dispatcher；PR-05 必须一次切换并删除旧 dispatcher，不能长期保留双路真源。
