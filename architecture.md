...
- `frontend/features/extension-sdk` 是 build-time workspace UI registry 与窄 host contract。首批只承认 `workspace.side_panel` / `source.detail_section`；后端 manifest 的 metadata-only UI declaration 在启动时冻结，而 `/system/extensions` 每次请求重新判定对应 capability、只投影 plugin/display/version/contribution availability 与固定原因。浏览器按 local exact tuple ∧ live server row ∧ core permission ∧ normalized UI mode 四门渲染；默认 registry 为空，所以不请求 endpoint、不挂 DOM、不建 timer、不加载插件代码。前端 registry、后端 frozen topology 与 non-vacuous synthetic parity guard 保持一一对应，禁止远程 JavaScript、runtime register、全局 store 或向插件泄露 page/domain owner。
