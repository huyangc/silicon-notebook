...
Repository 侧的 persistence 与业务编排已按上表分层完成；应用边界已完成 router、model facade 与 shared transport 的领域分工。collection、来源、Ask、Report 与 KG workspace 状态已分别迁入独立 owner hook；`page.tsx` 仍承担 typed root-modal 等 shell 组合状态，FastAPI lifespan/application lifecycle composition 也尚未独立，后续整改继续以现有 facade 和测试为保护层逐域迁移。
...
4. **前端 workspace 状态拆分**（进行中）：`useNotebookCollection`、`useSourceLibrary`、`useAskSession`、`useReportWorkspace` 与 `useKgWorkspace` 已各自成为领域 owner；后续仅串行抽 typed root-modal coordinator。不引入新全局状态库，不改请求数量或轮询节奏。
