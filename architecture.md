...
模块化扩展的 Phase 0 与 Phase-1 retrieval host 合同已落地，但尚未迁移真实 product contributor。稳定跨层值下沉到 `backend/app/domain`；repository ports 只依赖 domain/models/core，不再反向 import service，当前 backend 静态 import 图为 0 SCC。全部 repository 的既有 services import 按 SQLite/PostgreSQL/other 分区记录为只许下降的债务上限。`backend/app/extension_sdk` 提供 manifest、五类 contribution、point-specific retrieval context/result/budget/cancellation/provenance 和脱敏失败合同；capability `requires` 与插件 `depends_on` 分开，contribution 默认按稳定 ID 排序。`backend/app/extensions` 负责启动期组合、冻结 registry 拓扑、冻结 capability 判定入口目录并实现 Ask/Report 共用的 `RetrievalContributorHost`；required capability 在 freeze 时只校验判定入口存在，availability 仍在每次调用时实时判定。`app.main` 与 `app.repositories.factory` 解析同一个 process-wide extension runtime，后者把同一个 host 注入 `RepositoryRuntime`，再分别传给 Ask 与每次构造的 Report engine；业务 workflow 不 import registry/SDK。Host 共享执行循环而不强迫统一物理时点：`selected_evidence` 保留在 selected-source graph 的最终 baseline/attestation 边界，`chunk_candidates` 保留给 generated-question 的 MMR/fusion 前候选边界。Scope/provenance 由 core 一次批量判定，禁止 per-hit N+1；核心 request cancellation 传播，插件局部失败/超时 fail-open。当前没有适用 contributor，所以 host 在构造 context、检查 cancellation/availability、读 clock、发 event、拿 fan-out slot或 I/O 之前原样返回同一个 baseline 对象；selected-source graph 与 generated-question 仍走原直接路径且检索顺序、预算和更强合同不变。`scripts/check_architecture_boundaries.py` 在 G1 contracts lane 直接检查无环、domain/SDK 依赖方向、`ports.py` 反向依赖、repository 反向依赖债务上限、extension composition root 隔离以及 facade 公开面只减不增。后续扩展迁移的顺序和安全下界以 `docs/modular-plugin-architecture-design-2026-08-21.md` 为准，PR review/CI/合入按 `docs/modular-plugin-architecture-delivery-plan-2026-08-21.md` 执行。
