...
当前 schema 版本为 37。这里指 SQLite schema。已提交的 v9 兼容 fixture 会经由 v10–v37 migration
...
配对 PostgreSQL 业务 schema 为 v15。SQLite store
...
- Baseline snapshot 目录必须 owner-only 且不可为 symlink；snapshot/live fence fresh 打开 `SqliteDatabase.db_path` 当前文件而不复用线程缓存连接，跨 open/transaction 及发布/PG commit 前复核 resolved path + device/inode。COPY 将全部业务 SQL 全限定到 run 绑定 schema，使用 named server cursor 有界复核 prefix，并用 statement timeout/阶段间取消轮询约束长操作。起始绑定、逐批提交/完成点和最终 H0 前均短暂取得 live SQLite `BEGIN IMMEDIATE` 来复核 capture 仍启用，但最终 64 表 proof/`ANALYZE` 期间不持有 SQLite；JSONB prefix proof 仅在 JSON 子树内统一有限 int/float/Decimal 的精确十进制语义（bool 排除、负零归零），普通 SQL 数值列仍保持类型差异；起始和最终另以 checksummed migration 派生契约验证精确 v15 table/column/PK/FK/unique/check、operational+GIN index 与 `public.pg_trgm`，逐批只走轻量 run/control/identity gate。
- 最终 live SQLite fence 是跨 commit 的 lease：只在 PG 双锁/run/table lock 与 64 表 proof/`ANALYZE` 完成后取得，持有期间写入并实际提交 PG H0 checkpoint + run progress，成功后才释放；PG 事务/commit 失败则不落 H0 并释放 SQLite。该 fence 期间不得再等 PG pool/advisory lock 或执行长 proof。
...
- `retrieval.py`、`retrieval_service.py`、`reasoning_retrieval.py`、`structured_retrieval.py` 与 `ask_modes.py` 负责关键词/向量召回、候选融合、查询改写、Knowhow 稳定游标枚举、mode 注册和 reasoning 迭代；`core/ask_retrieval_policy.py` 集中声明五档预算与完整枚举安全线。`collection_catalog.py`（零 LLM、索引辅助的集合地图，含来源元素按类型计数与 KG 对象按类型计数的有界缓存）与 `collection_enumeration.py`（对地图同一物理源集合做稳定游标遍历的枚举执行器）为 reasoning 的 `enumerate_elements`/`enumerate_kg_objects` reflect 动作供数；两者与地图注入、reflect prompt、schema 分支、allowed_actions 共用同一个 `REASONING_ENUM_TOOLS_ENABLED`（默认 true）总闸。
...
限定单一来源按**标题**表达：内部 source id 从不上屏也从不给模型看，reflect 的 `enumerate.source_title` 由服务端在地图已规划的那批来源里做确定性解析（去空白 + 忽略大小写的精确匹配，按窗口批量读标题并有界，绝不模糊匹配）；零命中或多命中都跳过该动作并在轨迹里说明，绝不悄悄扩成全作用域枚举。同时给了 `source_id` 时以 id 为准。

因为这两个工具不依赖图谱，逐步推理里「本笔记本尚未构建知识图谱」的早退只在作用域**同时**没有任何可枚举集合时才触发；放行后照常跑完整循环，`kg_required` 仍如实为 `True`。接线判据与上面的总闸是同一个函数，关掉 kill switch 会同时恢复早退。`complete=false` 恒意味着游标可续跑，唯一例外 `truncated_reason=concurrent_change`——两次调用之间作用域发生变化时终止且绝不静默重来；未耗尽预算的续跑请求走该游标继续，预算耗尽则跳过为 `enumeration_budget`（仍是部分结果），只有链条已 `complete` 才跳过为 `already_enumerated`。支撑地图的按源计数在某源元素刚落盘、其变更信号尚未更新时会低报为零，该源因此不进遍历计划、不被走到，地图与清单在零点上保持一致、缺口在下次状态写入时自愈；显式点名的 `source_id` 绕开这个问题，直接查询该源而非把「不在计划」当作「为空」。挂载参考库的跨库条目 v1 只读展示（标注来源库名，不提供来源跳转/图片加载），因为挂载不等于持有该库直接成员权限；浏览器侧参与者作用域代理读取与「纯文本兜底解析来源」的覆盖披露均登记为独立后续任务。总闸 `REASONING_ENUM_TOOLS_ENABLED`（默认 true）关闭时两个动作都不提供、地图也不注入。

上述地图/枚举共用五个 T2/T3 新增的有界 repository 端口，双后端（SQLite/PostgreSQL）adapter 均实现：`SourceStorePort.source_change_signal_rows`（T2，每 notebook 一条查询取全部源的不透明变更信号）、`SourceStorePort.element_type_count_rows`（T2，按白名单批量 `GROUP BY source_id, element_type`）、`SourceStorePort.element_page_rows`（T3，一个源一种 kind 的 keyset 分页）、`SourceStorePort.source_display_rows`（T3，来源标题/论文标题批量查询）与 `KnowledgeStorePort.knowledge_object_page_rows`（T3，一个 notebook 一种 object_type 的 keyset 分页，复用地图同一条 `USABLE_STATUSES` 谓词）。
...
| Retrieval / Ask | `retrieval.py`、`retrieval_service.py`、`reasoning_retrieval.py`、`structured_retrieval.py`、`collection_catalog.py`、`collection_enumeration.py`、`core/ask_retrieval_policy.py`、`ask_modes.py` 与 facade 中的兼容方法 | 分数、grounding、tier 次序与集合完整性保持分离；mode registry 是 mode 真源，effort policy 是档位与枚举阈值真源；collection_catalog/collection_enumeration 是集合地图与元素/KG 对象枚举执行器的唯一实现，共用 `REASONING_ENUM_TOOLS_ENABLED` 总闸。 |
