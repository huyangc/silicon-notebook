...
`reasoning_prose_excerpt.py` 拥有同一显式实验档的普通原文详情窗口与已展示摘录内容身份，不执行检索或模型判断。`reasoning_context.py` 在当前授权候选池内选择少量详情键，K/D/补充共用；详情仍通过原卡的预算、fresh 与绑定准入，不就地修改已发送前缀。检索编排只在已有合法继续检索决定之后允许增强；首轮与无有效普通原文详情的路径保持原输入与历史预算。首次可见视图及已发详情身份由 run 局部 `ReflectDeltaState` 持有，最终 K/D 成型后才登记 key 与摘录内容摘要，卡头/版本变化和相同视图恢复不算进展。反馈只在详情实际可见且原历史预算有余量时进入可变 T，不提前预扣预算，也不为反馈重建历史；无空间显示时仍记录实际可见身份。该机制不拥有候选新增计数、hard stale、问题覆盖或逐项自评，关闭详情不改变原模型输入或预算。配置与数值边界由产品/API 配对文档拥有。
...
- `retrieval.py`、`retrieval_service.py`、`reasoning_retrieval.py`、`structured_retrieval.py` 与 `ask_modes.py` 负责关键词/向量召回、候选融合、查询改写、Knowhow 稳定游标枚举、mode 注册和 reasoning 迭代；`core/ask_retrieval_policy.py` 集中声明五档预算与完整枚举安全线，`services/reports/policy.py` 集中深度报告充分性和 reasoning 动作线。`services/retrieval_run.py` 用 request/report-stage 级 ContextVar 状态在 worker 间共享 query embedding single-flight 和叶子 I/O 扇出闸，不跨请求、规划或生成阶段；报告等待槽位时感知取消，并在真正发起 leaf I/O 前复查。`collection_catalog.py`（零 LLM、索引辅助的集合地图，含来源元素按类型计数、KG 对象按类型计数与用户可见来源数的有界缓存/派生）与 `collection_enumeration.py`（对地图同一物理源集合做稳定游标遍历的枚举执行器）为 reasoning 的 `enumerate_elements`/`enumerate_kg_objects` reflect 动作及其 `collection="sources"` 参数值供数；两者与地图注入、reflect prompt、schema 分支、allowed_actions 共用同一个 `REASONING_ENUM_TOOLS_ENABLED`（默认 true）总闸。`ReasoningRetriever.run` 的**首轮**（run 级账目初始化 → 理解块/打法块/集合地图注入 → 规划 → 初检索 → PPR seed → 精确查找 seed → 空证据兜底 → 已确认方向补种）已抽成 `_run_first_round` 编排 + 若干 `_first_round_*` 阶段方法，run 级状态经模块级 `_ReasoningRunState` 显式交接、轨迹记账经 `_TraceRecorder`；该状态对象是**一次性交接**而非全程权威——`run` 只解包一次，可变容器是同一批对象，标量字段在 reflect 循环开始写局部名之后即陈旧。阶段顺序本身是合同（精确查找 seed 必须排在 PPR seed 之后）；长期回归由意图、PPR、精确检索、兜底、方向补种账目、枚举与合成的聚焦行为测试承担，不再保留重构期的逐字黄金快照。reflect 循环本体与它的三个嵌套 def 尚未拆分，是登记在案的下一件事；`ppr_retrieve`/`search_chunks`/`expand_community` 三个动作的执行体已按同一条理由下沉为 `_action_*` 方法。`services/reasoning_actions.py` 是 reflect **能力投影**的唯一产地（纯数据：13 个动作的 id/参数形状/是否产生证据/预算类别/重复身份/依赖条件，加一次纯内存的 `build_reflect_capabilities`）——`REASONING_REFLECT_V2_ENABLED`（默认 false）开启时，模型看到的动作说明（`prompts.reflect_v2_system_prompt`）、schema 的 `next_action` 枚举（`reflect_v2_schema_hint`）与解析白名单（`reasoning_retrieval.parse_reflect_v2`）三处共用同一个不可变对象，任何一处不同步在结构上不可能发生；该模块不做 I/O、不读 settings、不认识 `ReasoningRetriever`，全部运行时事实由 `ReasoningRetriever._reflect_capabilities` 一次性折成 `ReflectCapabilityFacts` 喂入，投影只决定「摆不摆给模型看」，执行前的 scope/actor/取消实时校验一律照跑。总闸关闭时不构造投影，`run` 逐字节回到接入前。**reflect 上下文的前缀复用**（`REASONING_REFLECT_OPTIMIZATION`，默认 `off`）与它的**测量**（`REASONING_REFLECT_MEASURE_CONTEXT`，默认 false）叠在这条 v2 通道上，两条判据各有全仓**唯一读点**——`ReasoningRetriever.reflect_optimization()` 与 `reflect_measures_context()`，彼此正交（`off` 臂也能开测量，对照实验要的正是这个形态）；`core/config.py` 只**声明**这两个字段（字段定义，加策略位那一个 `@field_validator`）而一次都不读，唯一性走 AST 判据钉住（`test_reasoning_retrieval.py::test_reflect_optimization_has_exactly_one_settings_read_point`：`Settings` 上多长出一个便利读者、或读点搬出那两个函数体即打红；注释与 docstring 里提到字段名不误伤）。静态工具目录的**产地**是 `reasoning_actions.static_catalog_facts(facts)` 这个纯函数——把逐轮项一律归一（各 `*_left` 归 1、`last_turn`/`terminal_overflow_repair`/`outline_repair_available`/`scope_restricted` 归 False、`has_candidates` 归 True），只留 run 级部署与调用方通道位，所以目录**恒为超集**、绝不因范围收窄而少掉动作——再经同一份 `build_reflect_capabilities` 与同一份 `ACTION_DEFINITIONS` 折成目录；`_prime_static_catalog(state, facts)` 在本 run **第一次**能力投影时缓存它，且**仅当** `reflect_optimization() != "off"`（纯测量臂没有消费者，不构造）。目录是「可能执行」的超集、**不授予资格**，执行前的实时校验一律照跑。布局本身落在 `ReflectContext` 上两个**互斥**的消费点：`as_user_block()` 是 `off` 的原顺序，`as_prefix_user_block()` 是 S/C/K/D/T 的**三条前缀臂共用**的排法（消息形状 `system(S)` + 一条 `user(C+K+D+T)`，`prefix_snapshot` / `prefix_delta` / `prefix_delta_lean` 逐块同形，前者与后两者差别只在 K/D 两块的内容判据，后两者之间只差自评合同的措辞）——前者遇到非空的 `contract`/`turn_state`/`static_prompt`/`delta` **四格中任意一格**抛错（`_PREFIX_ONLY_FIELDS`；`delta` 是 PR-3 加的第四处，与前三格同一条判据、同一处实现），后者遇到非空 `server_state` 抛错，所以「填了 P 的格却按 off 渲染」是响亮失败而不是静默丢块。测量的**落账点**是 `_TraceRecorder.__call__`（判据 `measurement is not None and step.step_type == "reflect"`）：那条 reflect 步唯一的入口，因此 `run()` 一条语句都不用加（同 `defer` 的理由）；写侧键集越出 `domain/reasoning_trace_stats.py` 的 `REFLECT_MEASUREMENT_DETAIL_KEYS` 时 `raise` 而不是 `assert`（`python -O` 会把断言整条删掉，而这是「键名合法但走错写点」唯一的判据）。测量自身关在 `_measure_reflect_messages_safely` 的守卫里，失败只表现为那一轮**那几个键缺席**，永不改变发出去的消息与模型的结论。离线一侧，`backend/app/eval/reflect_context_bench.py` 是 per-call 表的纯分析（llm.jsonl ⋈ events.jsonl on `support_id`，零 I/O、零模型，闭集行键 + 归因残缺状态），`scripts/reflect_shadow_rig.py` 只做薄 CLI 适配。`services/reasoning_observation.py` 与 `services/reasoning_context.py` 是同一条 v2 通道上的另外两个纯模块（设计稿 §6，同样不做 I/O、不读 settings）：前者定义一条**动作观察**的形状、一份「trace step_type → 动作/计数键」的对照表和有界渲染，后者构造证据卡（只用候选池内材料，零 LLM、零查库、零补 hydrate）并把 v2 的 user 段装配成「服务器状态 → 证据卡 → 动作观察账」三个各自带标题的块（问题与冻结契约那一段由 `prompts` 拼在这三块之前，不属于本模块）。观察由 `_TraceRecorder` 上一个 v2 专用的账本对象（`ActionObservationLedger`）在**每次轨迹记账时**从那一步的结构化 detail 折出——所以首轮确定性播种与循环动作共用同一次转换，`run` 的十几条动作分支一行都不用改；关闭态该字段恒为 `None`，不构造账本、不多付任何转换。这份账目是**投影不是权威**：判重仍属 `attempted`/`visited`/`exact_terms_done`/`follow_chain_done`/`enum_chains` 的 cursor 覆盖，大纲溢出仍属 `outline_overflow`，观察账不写它们中的任何一个；「曾真实展示」的键（`ever_shown_outline_keys`，大纲绑定合法集的事实层）只登记**真的渲染出来**的证据卡，被预算挤出窗口的候选一个都不登记。**六项 v2 预算每个字段恰一个读点**：四项既有的（`REASONING_REFLECT_EVIDENCE_CHARS_BY_EFFORT`/`_EXCERPT_CHARS`/`_STATE_CHARS`/`_RECENT_OBSERVATIONS`）在 `ReasoningRetriever._reflect_v2_context` 方法体内读完，两个 PR-3 新增的（`REASONING_REFLECT_DELTA_CARDS_BY_EFFORT`/`_COMPACTION_TARGET_RATIO`）各在自己那个专用 helper（`_delta_max_cards`/`_delta_target_ratio`）里读一次，而那两个 helper 各只有一个调用点、就在同一个方法里；判据是 AST 的（`test_reflect_optimization_has_exactly_one_settings_read_point` 六项逐项参数化），因为同一个字段有两个读点时「两条臂用了不同的预算」在字节上看着完全正常。**`prefix_delta` 的增量上下文**（PR-3）在这条前缀通道上再叠一层，`run()` **零改动**：run 内的渲染缓存住在 `_ReasoningRunState.reflect_delta`（`ReflectDeltaState`，`@dataclass(slots=True, eq=False)`，末尾带默认值 ⇒ `_new_run_state` 一行不改），三条硬约束写在它的类 docstring 上并由 `test_delta_state_holds_only_rendered_text_and_counters` / `test_delta_state_repr_carries_no_document_text` 钉住——(a) 只存已渲染的字符串、一张卡片文本表和几个整数游标/计数；(b) 不持有候选池、额度账、方面账的**任何**引用（要重建时权威状态由调用方现取现给）；(c) 把它整个删掉，除 K/D 的**字节组织**外一切逐字节不变（动作选取、绑定资格、配额、终止判据一格都不读它）。它把「在池子里 / 曾经展示过 / 此刻还在消息里」分成三格：候选池仍在 `state.collected`/`elements`/`chunks`，`frozen_cards` 是**曾展示**的字节表（只增不减，重建时不动），**此刻可见**分成 `snapshot_keys`（当前 K 的键，重建时整体替换）与 `block_keys`（全部已发出 D 块的键，随 `blocks` 清空）两格——分两格是因为回退之后 P 那一支要问的是「**保留 D 里**已经可见的是哪些键」，存一份并集会把那一版已经不在消息里的 K 的键也排除掉；`rebuilt_last_turn` 是重建**迟滞**那一格（上一轮刚压紧、这一轮又一张新卡都装不下 ⇒ 不做第二次重建而走回退，而它只看**上一轮**：中间隔一轮没重建就清零，`test_delta_hysteresis_clears_after_a_turn_without_a_rebuild`），`supplement_last_key` 是补充卡轮转的**续接点**（候选集恒是「本轮已绑定 ∩ 曾展示」，从上一轮服务的最后一个键在本轮候选序里的位置往后取 `max_cards` 个；存键不存位置——候选序每轮都可能移位，位置游标会把队尾的键饿死）。一轮的装配下沉到 `_reflect_delta_context` 的**七步**（取/建投影 → 形成待追加事件 → 检查预算 → 必要时重建 K → 装不下最小有效新证据就不可逆回退 → 追加 D 并更新冻结表与两个累计计数 → 成型），而这条顺序**天然就是** `_reflect_v2_context` 在 `run()` 循环里的位置（每轮一次、排在 `self.reflect(...)` 之前、排在上一轮动作执行与记账之后），所以顺序不需要在别处再摆一次。布局分派的全部改动是模块级两个**派生**的元组这**一个单点**（`_DELTA_LAYOUTS = ("prefix_delta", "prefix_delta_lean")`，`_PREFIX_LAYOUTS = ("prefix_snapshot", *_DELTA_LAYOUTS)`；`_reflect_prefix_layout` 第二个判据读后者，「这一格走不走 delta 装配」的两处判断读前者，PR-4 把此前两处 `== "prefix_delta"` 的字面相等换成了成员判断），另加一条**导入期对账守卫**：`set(REFLECT_OPTIMIZATION_IMPLEMENTED) != {"off"} | set(_PREFIX_LAYOUTS)` 时 `raise` 而不是 `assert`（`python -O` 会把断言整条删掉，而「放开一格策略位却忘了接线」的故障形态是那条臂**能起来、却发 off 的布局**，rig 的 `assert_optimization_matches_evidence` 拦不住它；用例 `test_wired_prefix_layouts_match_the_implemented_closed_set`）。派生而不是各写一份字面量，正是为了让那条守卫**连第四格一起盖住**：只放开 `_PREFIX_LAYOUTS`、漏掉 `_DELTA_LAYOUTS` 的那种漏接，故障形态是 `prefix_delta_lean` 发出一条不带 D 通道的消息（用例 `test_prefix_delta_lean_reaches_delta_assembly_not_the_snapshot_fallback`）。**回退的判据不是第二次读策略位**——它是投影上那格 `ReflectDeltaState.fallback`，为真时 `_reflect_v2_context` 直接走 P 的有界选择那一支，消息形状一格不变；已经发出的 D 原样跟着走（触发回退的那一次重建清掉的 `blocks` 在置位之前被原样还原），S 里那几句 delta 读法规则的判据与「有没有 D 这条通道」**同源**（`static_delta=(delta is not None)`），所以两者不可能分歧。回退时 `evidence_chars`/`history_chars` 两笔累计账**收缩成保留 D 那一半**（各减掉那一版 K 的对应半，K 的两块字节同时清空），因为 P 那一支每轮读它们两次：K 的额度是「档位证据池 − 保留 D 的证据半」、近期观察窗是「`state_chars` − 保留 D 的历史半」，而保留 D 里已可见的键（`block_keys`）排除在 K 之外——少了这三格，回退轮及其此后每一轮的一条消息里会有两份都不带版本标记的同一张卡，两个池子各涨到档位的两倍（回退不可逆 ⇒ 保留块永不清）。另外两件事各自成立：上文里已发出的 D 一个字节没变，`ever_shown_outline_keys` 只登记真发出去过的键。`context_rebuilds` 计的是重建**动作**次数（含触发回退那一轮被丢弃的那次：全池重选的开销真的付了），比模型看过的 K 版本数多一格。**新纯函数的产地**：历史折算 `fold_observation_counts` 在 `reasoning_observation.py`（与观察行的状态词表同源，`render_observations`/`render_observation_row`/`ActionObservationLedger` 一个字节未改），冻结卡与增量块那一族（`build_delta_evidence_block`/`build_delta_block`/`compose_snapshot`/`render_pool_cards`/`ReflectDeltaState`）在 `reasoning_context.py`；`render_pool_cards` 是**补充卡检测唯一的卡面产地**（按给定的几个键现渲染，不选卡、不看预算——池键有三种形状、卡片构造要按对象类型分派，在装配侧复制一份就会漂出「补充卡的 `key=` 与池键对不上」这种静默失配）。两个纯模块**互不 import**：`reasoning_context` 不 import `reasoning_observation`（反之亦然），K 的历史半与折算计数在 `reasoning_retrieval` 的 `_build_delta_snapshot` 里合成，两个观察切片刻意不相交。legacy 每轮那四段散文账目（已展开节点、邻居截断、子查询、精确查找）已原样抽成模块级 `legacy_action_ledger_note`，v2 不再拼它们（同一批事实由观察账一次性投影）。`domain/retrieval_termination.py` 与 `services/reasoning_aspects.py` 是同一条 v2 通道上的第三对（设计稿 §7，同样零 I/O）：前者是纯 DTO 层——必答方面快照、结束事实（七个 reason 的闭集）、最终装配复核结果 `AspectDelivery`，外加**四个** prompt 与校验共读的协议常量（`REFLECT_ASPECT_MAX_EVIDENCE_KEYS`/`REFLECT_ASPECT_GAP_MAX_CHARS`，加 PR-4 从 `reasoning_aspects.py` 挪过来的 `REFLECT_ASPECT_GROUP_ROWS_FACTOR`/`REFLECT_ASPECT_GROUP_ROWS_HARD_MAX`），放在 `app.domain` 是因为它要跨到 application 层，那一层只许 import 具名 domain 模块；后两个挪家的理由与前两个同款——组内行数上限 `min(factor×方面数, hard max)` 既由 `reasoning_aspects._group_row_cap` 用来**校验**载荷，又要被 `prompts` 的 lean 自评段**插值给模型看**，两侧插同一个常量才谈得上「prompt 说的上界与校验器执行的上界不可能分歧」，而让 `prompts` 反向 import 一个 service 模块只为两个整数是更差的接法；后者持有全部判据——按用户冻结契约建方面账（`AspectLedger`，唯一写入口 `apply` 是全量替换、非法键剔除后不得保持 supported）、渲染方面块、`classify_termination`（`retrieval_degraded` 要求「无 model_end 标记」且「时间线上最后一次真实执行 failed」两个前件，没恢复的通道另走纯披露的 `unrecovered_channels`）、合成侧的服务端事实块与 `review_aspect_delivery`。**`prefix_delta_lean` 的自评合同**（PR-4）在这一对上再叠一层，`run()`/`_new_run_state`/`_run_enumeration` 三个热函数**零改动**（三者的 `function_length_ceiling` 是精确相等，长短都红——包括不能顺手删掉 `run()` 里那条从此不可达的 `missing_assessment` stale 持平分支）：服务端侧的全部接线收敛成 `AspectLedger.lean_assessment` 这**一格 run 级冻结**的开关。它是只读 property（唯一写点是 `__init__`，没有 setter，建账之后 `ledger.lean_assessment = ...` 结构性地拒成 `AttributeError`），策略位只在建账那一刻读一次——中途翻位改不动这次 run 的追问合同，否则臂标签就是假的。本模块里建账的地方有**三处**（首轮的 `_open_v2_ledgers`，加总闸中途被翻开时 `_reflect_v2_context` / `_absorb_assessment` 各自那条防御重建），所以三处全部走私有 helper `_v2_build_aspect_ledger`，并有一条守卫钉住「`build_aspect_ledger(` 在本模块只允许有这一个直调点」（AST 判据 `test_the_aspect_ledger_has_exactly_one_construction_point`）——漏掉一条路的后果是最难看见的那一种：那条路上的 run 挂着 L 的臂标签、跑着 D 的追问合同，不崩不报错，只在 A/B 表上比出一个假差异。**闸落在 `note_missing_assessment` 的三条路里排第一条**：为真时直接返回 False，`may_prompt` 与追问额度两格从头到尾都是初值，`_absorb_assessment` / `_nudge_missing_assessment` / `run()` 一行没改；另两条路是既有的——`may_prompt=False`（调用方已知没有下一轮）与追问额度用尽，那两条写 `assessment_omitted`。**per-aspect 标记的语义分工**是这里的硬约束：`model_assessed=False` 只说「这一格没有模型判断」、不解释为什么；`assessment_omitted` 的判据里含着一次真实发生过的追问，所以 **L 下永不置位**（L 走的正是不写这一格的那条路）；「这条臂本来就不问」是 run 级事实、挂 `RetrievalTermination` 上那一格布尔，由 `classify_termination`（全仓唯一的 `RetrievalTermination(...)` 构造点）从账本承接过去，合成侧据它决定要不要多出「未逐项核验」那一行。共用面上新增的两个 `lean` 参数（`build_aspect_ledger` 与 `render_aspect_status_block`）**默认值都是中性的 `False`**，`prompts.reflect_v2_static_prompt` 的那一个同款，所以 `off`/`prefix_snapshot`/`prefix_delta` 与关闭态一律拿到默认值、逐字节回到 PR-3 合入态（`test_the_lean_wiring_leaves_the_other_three_arms_untouched`）。**状态拥有者**：方面账住在 `_ReasoningRunState.aspects`（写点是每轮一次的 `_absorb_assessment`，以及它折出来的 `_nudge_missing_assessment`——收尾载荷一个方面都没自评时整轮退回并逐个 id 追问，追问次数与「问过了、模型仍不给」这两格状态由 `AspectLedger` 自己持有；读点是收尾的 `_run_termination`），关闭态恒 `None`、不构造；反思调用**连续**失败的轮数住在 `_ReasoningRunState.reflect_failures`（唯一写点 `_survive_reflect_failure`，任何一轮成功清零，连续两轮才按 fail-open 收尾），同样是 run 局部、关闭态不读；结束事实是 run 收尾一次性生成的不可变值，此后只被读。**贯通链**是 `ReasoningResult.termination → ReasoningEvidenceSnapshot.termination → ResponseDraftInput.termination`（每一跳都 `getattr` 兜底、缺省 None），Ask 在 `_draft_reasoning_response` 里读它两次——合成**之前**渲染 `answer_prompt` 的服务端事实块（登记在 `prompt_layers.L2_BLOCKS` 的 `termination_block`），合成**之后**用最终 `id_map` 的证据身份与解析回来的锚点做方面复核；报告侧 `_draft_section` 用同一份纯函数、同一套口径读 `_deep_dive` 结果上的同一个字段。三个口径（`model_supported`/`synthesis_admitted`/`answer_cited`）分开记，公开分享面不变（两条匿名投影是白名单，本来就不带推理面）。**PR-5(T4 实验通道)在这条通道上再叠三件事**：`app/core/llm.py` 上加了一道**默认关闭**的实验标记接缝——模块级 `ContextVar` + `provider_messages` 第三参数 `markers`，配一个 `experiment_message_markers(head, tail)` 上下文管理器，三条判据钉死（`markers=None` 时 `provider_messages`/`chat_json` 逐字节回到接入前；`_EXPERIMENT_MARKERS.set(` 在 `backend/app/` 下的唯一 set 点由 AST 判据锁定；`backend/app/**` 除 `core/llm.py` 与 `eval/` 外全禁的负向断言——扫的是整棵树而不是层白名单，认 import/`Name`/`Attribute`/别名/`getattr` 字符串五种写法），生产路径零字节变化。离线一侧的模块分工由此扩成四格：`reflect_context_bench.py`（per-call 纯分析，既有）、`reflect_prefix_probe.py`（E1 计划与统计）、`reflect_state_probe.py`（E2 驱动器与代理）、`reflect_manifest.py`（manifest 纯构造 + 闭集 + 隐私断言），`scripts/reflect_shadow_rig.py` 的三条通道（`prefix-probe`/`state-probe`/`ab`）都只做薄 CLI 适配，不重复实现任何一个纯函数。E2 的接缝是 `_construct_reasoning_retriever` 里已经存在的 `model_clients` 参数——一个 `__getattr__` 委托到真实 repo、只覆写 `chat("reasoning_agent")` 的代理（`ProbeModelClients`），服务端与三个热函数（`run`/`_new_run_state`/`_run_enumeration`）零改动，这正是把它设计成代理而不是给 `reflect()` 加一个注入参数的直接收益。
