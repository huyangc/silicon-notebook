...
`reasoning_prose_excerpt.py` 拥有同一显式实验档的普通原文详情窗口与已展示摘录内容身份，不执行检索或模型判断。`reasoning_context.py` 在当前授权候选池内选择少量详情键，K/D/补充共用；详情仍通过原卡的预算、fresh 与绑定准入，不就地修改已发送前缀。首次可见视图的去重集合由 run 局部 `ReflectDeltaState` 持有，最终 K/D 成型后才登记 key 与摘录内容摘要，卡头/版本变化和相同视图恢复不算进展。该反馈只进入可变 T，服从既有状态预算；它不拥有候选新增计数、hard stale、问题覆盖或逐项自评，关闭详情时整条新路径不运行。配置与数值边界由产品/API 配对文档拥有。
