...
- owner hook 的隐藏态回退值一律是稳定引用（冻结常量或 per-instance ref，见 `hook-view-stable-empty-guard`），actor 激活/离开的扇出只在 `page.tsx` 的 `activateWorkspaceOwners` / `leaveWorkspaceOwners` / `leaveActorOwners` 三个入口（见 `workspace-owner-transition-guard`）。
