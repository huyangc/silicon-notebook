...
预检。凭据通过权限 0600 的临时 `PGPASSFILE` 传入；launcher 以 `Popen` 独占当前 child/session，
在 pgpass 生命周期内临时接管 SIGINT/SIGTERM，中断时先以有界 TERM→KILL fallback 终止并
reap child，再清理 pgpass、恢复原 handler，返回 130/143；child 的负 signal returncode 也
统一为 `128+signum`。然后只运行 `-m postgres_integration`。CI 的独立 PostgreSQL 17 job 用非 superuser app role，
