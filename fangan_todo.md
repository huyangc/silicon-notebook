...
- [ ] **全局回答的 👍/👎 反馈仍未进管理端提问分析 / 笔记本分析口径的聚合**：`POST
      ...
      这是产品决定，尚未拍板。
- [ ] **全局问答的进行中提问铃铛与删库后可读性未做**：记录口径对齐（SQLite v81 / PostgreSQL 0061）
      与问答后学习链（覆盖层按参与库各计一次、经验库只计全局分区、检索偏好按人一次，注入只读全局
      分区）已补齐；仍未触碰、各需单独产品决定的有：(b) 待办铃铛「进行中的提问」不列出正在运行的
      全局作业；(c) 全局会话的所有者一旦其中任一参与笔记本被删除，整条会话对所有者即变 404（没有留存
      投影、没有到期规则）——笔记本内问答有 `retained_user_activity` 留存投影，全局问答没有对应机制；
      (d) `ask_stage` / `retrieval_run_stats` 事件给全局 run 打的仍是某个锚点笔记本 id，没有专门的
      全局标记。全局反馈的聚合面缺口见上一条，不在此重复登记。
