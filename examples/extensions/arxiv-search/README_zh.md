...
`MAX_QUERY_TERMS` 个词以内，因为它的查询是这个插件从选中的来源专属检索词
（或旧式直接调用的问题/缺口）以及 reflect 模型自己写的
...
`ASK_GAP_CONSULT_TIMEOUT_SECONDS` 抬到超过 13.25 秒（核心侧的上限是
30），并为来源描述和查询规划留出额外时间；或者把 `timeout_seconds` 压低，让
整段流程都能塞进现有 deadline。补充段只能使用剩余时间。这是
刻意的行为：一个在核心 deadline 之后才到达的答案不会被任何人读到，发起一次注定
赶不上的请求纯属浪费。

### 4.2 外扩需要拉丁字母检索词

抽取查询词这一步会扫描**为 arXiv 选中的专属检索词**中的拉丁字母词项
（`consult.py::_query_terms`，底层是共用的 `terms.py::latin_terms`）。如果一个词都抽不出来，外扩会直接返回一个稳定
代码（`arxiv_no_latin_terms`），**零网络调用、零占用节流锁**——连请求都不会尝试
发起。理由是 arXiv 是拉丁关键词索引；没有这类词项的请求只会白白消耗一次
节流配额和往返。来源选择模型可以把中文问题改写成英文检索词，因此原问题的
语言本身不会关闭这条路径。
...
2. **Agent 触发的缺口外扩。** 一次逐步推理问答收尾时，核心的 `ask.gap_consult`
   扩展点会向已描述且被模型选中的插件来源询问笔记本之外的线索。本插件描述
   arXiv 索引，只使用为 arXiv 选中的检索词，并在每条建议的 `actual_query` 中
   回报实际提交给上游的词。回答卡上那个导入建议的按钮是
