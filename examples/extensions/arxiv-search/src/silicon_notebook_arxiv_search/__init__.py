...
* ``atom``, ``client`` and ``terms`` know arXiv and nothing else — no ``app.*``
  import, no settings model, no extension SDK.  Swapping in a different
  upstream (an internal IEEE mirror, say) means rewriting these three and
  nothing more.
* ``settings``, ``routes``, ``consult``, ``reflect_search`` and ``bundle`` are
  the Silicon Notebook adapter: they map deployment settings onto the client
  and expose the result through the SDK's contribution points.
